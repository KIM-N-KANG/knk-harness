...
KNK-1120의 게시물 검수 API는 미구현입니다. 작업 브랜치에는
`prompt/moderation/MODERATION-TEMPLATE.md` 초안이 있으며 API 라우터·요청 및 응답 모델·
이미지 입력 전달·대체 호출·검수 관측 연결은 아직 없습니다. 프롬프트 초안은 이미지 판독 불가를
`rule: null` 항목으로 알리는데, 서버가 이를 `IMAGE_UNREADABLE`로 바꾸고 `issues`를 비웁니다.
초안의 보류 판정 언급은 제거해야 하며, 실제 모델 품질은 미실측입니다.
현재 공통 LLM 메시지는 텍스트 전용(`dict[str, str]`)이라 이미지를 넣으려면 메시지 내용을
텍스트·이미지 조각 목록으로 넓히고 등록부에 이미지 입력 지원 표시를 추가해야 합니다.
이미지 생성 통로가 있다는 사실을 이미지 검수 지원으로 해석하지 않습니다.

구현 시 구조는 2026-09-23 합의를 따릅니다. 라우터는 `src/api/v1/moderation/story.py`에 두어
이후 채팅 검수를 옆에 추가할 수 있게 합니다. 모델은 `MODERATION_MODEL`(기본 `gpt-5.6-luna`)·
`MODERATION_FALLBACK_MODEL`(기본 `deepseek-flash`)로 고르고 기동 검사에 포함합니다. OpenAI 추론 강도는
`high`입니다. SDK 내부 재시도는 끄고 서비스가 OpenAI 1회·DeepSeek 1회를 셉니다. 이미지는 백엔드가 준
공개 서빙 URL을 서버가 내려받아 base64로 전달하며, 기존 업로드와 같은 허용 호스트 목록으로 제한합니다.
시간 제한 3종(이미지 준비 30초·호출 60초·요청 전체 150초)은 환경 변수로 둡니다. 실행 실패는 기존 생성
API처럼 Sentry에 보고하고 호출마다 모델·토큰을 기록합니다.

승인된 입력·판정·오류 계약은 [Spec §5-3-6](../spec/5-ai-server-spec.md#5-3-6-게시물-검수)과
[§5-9-6](../spec/5-ai-server-spec.md#5-9-6-게시물-검수)에 있습니다.
OpenAI `gpt-5.6-luna`와 DeepSeek `deepseek-flash`는 현재 모델 등록부에 등록돼 있지만,
검수용 기본·대체 호출이 연결됐다는 뜻은 아닙니다. 호출 순서와 서버의 응답 조립 책임은
[검수 결정 기록](../adr/3-ai-server-adr.md#게시물의-명백한-금지-내용-검수와-실패-시-게시-차단)에 둡니다.
이 절은 현재 상태를 설명하며 기존 생성 API의 설정·오류 처리·관측 구조를 변경하지 않습니다.
