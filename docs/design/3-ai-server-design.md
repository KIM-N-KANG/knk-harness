...
이미지 입력 전달·대체 호출·검수 관측 연결은 아직 없습니다. 프롬프트 초안은 최종 응답 계약의
`error_code` 분리와 보류 판정 제거를 포함해 구현 시 대조해야 하며, 실제 모델 품질은 미실측입니다.
현재 공통 LLM 메시지는 텍스트용이므로 이미지 생성 통로가 있다는 사실을 이미지 검수 지원으로
해석하지 않습니다.
