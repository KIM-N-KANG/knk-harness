...
**계획(KNK-1161, 미구현)** 일반 제작 등록·수정을 검수 제출본으로 접수하고 커밋 뒤 검수해 승인 시에만 라이브에 반영할 예정입니다. 제출본·실패 복구·검수 완료 알림의 계약은 [Spec의 검수 제출 흐름](../spec/4-backend-server-spec.md#스토리-검수-제출-흐름), 결정은 [BE-048](../adr/2-backend-server-adr.md#be-048)을 따릅니다. 아래 현재 구현 구조에 이 계획이 반영된 것으로 해석하지 않습니다.
