...
| 사용자 | `user_consents` | (V84, KNK-1334) 약관·개인정보 처리방침·만 14세 확인의 문서 버전별 동의 이력. `user_id`(FK users, `ON DELETE` 없음: 탈퇴 후에도 보존) · `doc_type`(CHECK `TERMS`·`PRIVACY`·`AGE14`) · `version`(웹 콘텐츠 버전 문자열, AGE14는 `1`) · `agreed_at`. PK `(user_id, doc_type, version)`, append-only이며 기존 행의 `agreed_at`은 갱신하지 않습니다. 현행 요구 버전은 `manyak.legal.*-version` 설정값([§4-3-5](../spec/4-backend-server-spec.md#4-3-api-계약) 약관 동의) |
