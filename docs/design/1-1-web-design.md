...
| localStorage 제작 | 편집 초안 목록 `manyak:pending-creation-request`의 JSON 배열(항목은 판별 유니언, 구 형식의 단일 객체는 1건 목록으로 읽음)과 완성 요청 목록 `manyak:story-completion-requests`의 JSON 배열. [제작 저장소](../../../manyak-web/src/features/stories/_shared/utils/creation-request-storage.ts) |
...
- **프롬프트.** 제작 퍼널의 완성 요청 직후 [schedule-push-prompt](../../../manyak-web/src/features/my/_shared/utils/schedule-push-prompt.ts)가 회원별 기록(`manyak:push-prompt:{userId}`, 단계·거절 뒤 요청 횟수)을 갱신하고 물을 차례면 탭 내 외부 스토어를 대기 상태로 만듭니다. 퍼널은 제출 직후 제작 탭으로 이동하며 언마운트되므로 루트의 `PushPromptSheet`가 스토어를 구독해 이동 뒤에 엽니다. 권한·iOS·설치본 판정은 [push-permission](../../../manyak-web/src/features/my/_shared/utils/push-permission.ts)의 순수 함수와 `usePushPromptState`(`useSyncExternalStore`, 서버 스냅샷은 미지원)가 맡습니다.
