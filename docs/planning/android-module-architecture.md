...
[Android 현재 설계](../product-specs/3-3-android-design.md)가 위임하는 모듈 소유권·계층·의존 규칙의 정본입니다. 경로는 기존 참조를 유지하기 위해 planning 아래에 두지만 문서 책임은 현재 설계입니다. 선택 이유는 [Android ADR](../product-specs/3-3-android-adr.md), 실행·검증 기록은 [모듈 재구성 계획](../../../manyak-android/docs/plans/module-reorganization.md), 적용 상태는 [추적](./client-tracking.md)이 소유합니다.

## 1. 구조와 경계

큰 기능과 기반 기능을 최상위 Gradle 모듈로 구성합니다. 각 기능은 자신의 데이터 접근부터 화면까지 소유하고, 여러 화면을 포함하는 기능은 내부 하위 기능 패키지로 나눕니다. 기능은 최상위 모듈로 분리하고 최소 공통 기반만 하나의 `common` 모듈로 모읍니다.

현재 모듈 경계는 다음과 같습니다.

- 최상위: `app`, `auth`, `network`, `analytics`, `navigation`, `designsystem`, `common`, `chat`, `create`, `home`, `studio`, `my`, `story`, `login`, `legal`, `report`. 화면 기능에는 `notification`도 포함합니다.
...
제품 동작은 [공통 스펙](../product-specs/3-1-client-spec.md)·[Android 스펙](../product-specs/3-3-android-spec.md), 인증·저장·복구 구조는 [Android 설계](../product-specs/3-3-android-design.md), 이벤트 의미는 [분석](../product-specs/6-analytics.md)이 소유합니다. 실제 모듈 등록은 Android `settings.gradle.kts`를 확인합니다.
...
├── login/                      # 로그인 화면
├── legal/                      # 공용 웹 문서 화면
├── notification/               # 푸시 토큰 등록·알림 수신·수신 동의 설정
└── report/                     # 공유 신고 업무
```
...
정책 전달·숫자 변환은 common, 스켈레톤 맥박처럼 시각적인 로딩 표현은 designsystem이 소유합니다. common에서 designsystem을 역참조하지 않습니다.
...
업무 API·Repository의 모든 동작을 common에 모으지 않습니다. 아래 계약은 구현된 내부 경계이며 서버 API 신설을 뜻하지 않습니다.
...
| 푸시 토큰 등록·알림 수신·수신 동의 | notification | app의 세션 종료 조정자에는 notification/domain의 등록기 닫기·토큰 삭제 계약만, 루트에는 권한 요청 컴포저블만 공개. `FirebaseMessagingService`는 이 모듈 매니페스트에 선언하고 google-services 플러그인은 app에 남김(§8). 다른 화면 기능은 알림을 직접 알지 않음([`3-3-android-design.md §3-3-4`](../product-specs/3-3-android-design.md) 푸시 토큰 등록) |
...
동일 동작의 API·Repository·캐시를 화면별로 복제하지 않습니다. API 인터페이스는 소유 동작별로 나누며 Retrofit 애너테이션·직렬화 필드·인증 클라이언트 선택은 그대로 유지합니다. DTO가 같은 필드를 갖는다는 이유만으로 미리 공통화하지 않습니다.
...
신고의 소유권 선택 이유는 Android ADR을 참조합니다.
...
- auth는 토큰·제공자 정리, my는 프로필/초대 안내, notification은 표시 알림 정리를 제공합니다. 제작 두 테이블은 UserScopedStore 삭제 대상에서 제외하며 ownerId·SessionGate로 격리합니다. 정리 목록은 app의 중앙 등록에서 확인합니다.
...
- ManyakDatabase는 편집 슬롯과 완성 요청 테이블을 담으며 create가 소유합니다. 스키마 출력 경로를 이동할 때 기존 스키마 이력을 함께 옮기고 identity hash/SQL을 비교합니다.
