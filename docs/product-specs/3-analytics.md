...
| 포함 범위 | 설명 | 원본 섹션 |
| --- | --- | --- |
| 사용자 행동 이벤트 | 화면 진입, CTA 클릭, 선택, 제출, 완료 | `3-4. 이벤트 카탈로그` |
| 서버 처리 결과 이벤트 | 스토리 생성, AI 응답, 피드백 제출의 성공과 실패 | `3-4. 이벤트 카탈로그` |
| 핵심 퍼널과 지표 | 제작 퍼널, 채팅 활성화 퍼널, 전체 활성화 퍼널 | `3-5. 퍼널과 지표` |
| 운영 관측 | Sentry, CloudWatch, `ai_call_logs`, 실패 코드 | `3-6. 관측 구현` |
| 릴리스 검수 | P0 이벤트, 식별자, 원문 미수집, 로그 연결 확인 | `3-8. 검수 체크리스트` |
...
현재 `request_id`는 서버 내부 상관 키로 사용합니다. 분석 이벤트 프로퍼티로는 아직 보내지 않습니다. 자세한 기준은 `3-6-3. 요청 식별자와 상관관계`를 따릅니다.
...
server 이벤트의 `error_type`은 `network`, `validation`, `server` 중 하나만 사용합니다. 상세 매핑은 `3-6-7. 서버 분석 이벤트와 실패 타입`을 따릅니다.

### 3-4-3. impression 수집 기준
...
CloudWatch 이벤트와 `ai_call_logs` 기록 기준은 `3-6. 관측 구현`를 따릅니다.
...
### 3-6-5. 백엔드 CloudWatch 로그

모든 백엔드 로그는 JSON 형태로 남깁니다. CloudWatch `event_name`은 운영 로그 이름이며, `3-4. 이벤트 카탈로그`의 분석 이벤트 네이밍 컨벤션을 강제하지 않습니다.
...
### 3-6-7. 서버 분석 이벤트와 실패 타입

서버 분석 이벤트의 전체 목록은 `3-4-2. 페이지별 이벤트 카탈로그`에 있습니다. 백엔드는 퍼널의 결과 단계를 채우는 `server_*` 분석 이벤트를 발행합니다.
...
### 3-6-10. AI CloudWatch와 Sentry 기준
...
| 원칙 | 확인 방법 |
| --- | --- |
| P0 이벤트 우선 | 출시 전에는 `3-4-1. MVP 우선순위`의 P0 이벤트를 모두 확인합니다. |
| 원본 섹션 기준 | 이벤트는 `3-4. 이벤트 카탈로그`, 지표는 `3-5. 퍼널과 지표`, 관측 구현은 `3-6. 관측 구현`을 기준으로 확인합니다. |
...
- 새 이벤트를 추가하면 `3-4. 이벤트 카탈로그`에 이벤트명, 발생 시점, 고유 프로퍼티, 우선순위를 추가합니다.
- 새 이벤트가 지표에 쓰이면 `3-5. 퍼널과 지표`에 계산식과 집계 단위를 추가합니다.
- 새 이벤트가 Sentry breadcrumb 또는 CloudWatch 로그와 연결되면 `3-6. 관측 구현`에 연결 기준을 추가합니다.
- P0 이벤트가 바뀌면 이 문서의 출시 전 P0 체크리스트를 업데이트합니다.
- 원문 수집 가능성이 있는 프로퍼티를 추가하면 `3-7. 개인정보와 원문 수집 원칙`과 `3-8-5. 개인정보 검수 기준`을 함께 확인합니다.
...
- 서버 분석 이벤트의 `error_type`이 `3-6-7. 서버 분석 이벤트와 실패 타입` 기준을 따릅니다.
...
- `error_code`가 `3-6-9. ai_call_logs 기록 기준` 기준으로 `error_type`에 매핑됩니다.
