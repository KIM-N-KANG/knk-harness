...
Phase 1 범위 용어입니다. 크레딧은 회원 전용이며, 게스트는 크레딧 없이 디바이스 ID별 체험 한도로 사용하고 한도 소진 후 로그인으로 유도합니다. 계약·구조와 수치는 [`4-backend.md §4-3-7`](./4-backend.md)이 정본입니다.

| 공식 용어 | 영문 식별자 | 정의 | 지양 표기 |
| --- | --- | --- | --- |
| 크레딧 | credit | AI 기능(스토리 제작·채팅) 사용에 소모하는 서비스 내 재화 | "포인트" · "코인" |
| 크레딧 지갑 | `credit_wallets` | 사용자별 크레딧 잔액 보관 단위. `balance`는 원장 합계의 캐시 | — |
| 원장 | `credit_transactions` | 크레딧 증감의 불변(append-only) 기록. 환불은 행 수정이 아니라 `REFUND` 행 추가 | "거래 내역"(사용자 노출 화면 명칭으로만 허용) |
| 적립 | earn / `reason` 적립 계열: `SIGNUP_REWARD` · `INVITE_REWARD` · `ATTENDANCE_REWARD` | 보상으로 크레딧이 늘어나는 것. 제품 표시 정책은 회원가입 1,000 크레딧, 초대자·피초대자 각각 2,000 크레딧, 출석체크 350 크레딧(출시 이벤트 기간 한시적 700)이다(2026-08-30 결정, 백엔드 정책 동기화 필요). 초대자 보상은 다른 회원이 내 코드를 입력한 건에 대해 KST 월 10회까지만 적립한다. 보상 이벤트(사유) 관점의 "크레딧 보상" 표기는 허용 | "충전"(Phase 3 유료 결제 `PURCHASE` 전용) |
| 소모 | spend / `reason` 소모 계열: `STORY_CREATION` · `CHAT_TURN` | 기능 사용으로 크레딧이 줄어드는 것. 제품 표시 정책은 스토리 간편 제작(컴파일) 200 크레딧, 채팅 턴과 AI 응답 재생성 20 크레딧이며 스토리라인 생성·재생성은 무료다(2026-08-30 결정, 백엔드 정책 동기화 필요). 처리 전에 미리 소모하는 것은 "선차감(pre-charge)"으로 표기(허용) | "차감" 단독 표기(회수·정산과 혼동 — "선차감"은 예외), "사용" 단독 표기 |
| 환불 | refund / `REFUND` | 선차감 후 AI 실패 시 자동으로 크레딧을 되돌리는 것 | "취소" |
...
| 체험 한도 | trial limit | 게스트가 크레딧 없이 쓸 수 있는 디바이스 ID별 사용량 상한. Phase 1 기준은 스토리라인 생성·재생성 5회, 스토리 간편 제작(컴파일) 1회, 모든 채팅방 합산 채팅 턴 5회(재생성 포함)다. 일반 제작 등록은 제외한다 | "무료 크레딧"(게스트는 크레딧이 없음) |
