...
| 엔딩 | `ending` | 스토리의 종결 정의. Phase 1부터 스토리당 3개(유형별 1개)로 두고, 본문을 제작 시 쓰지 않고 유형 · 조건 · 에필로그 가이드로 정의해 도달 시 생성한다. 스키마·도달 계약은 [`4-backend.md §4-3-10`](./4-backend.md), 현행 구현(제목·내용·`condition_text`)은 레거시로 비활성 보존 | — |
