...
| 스토리 이미지 | `story_images` | 일반 제작·수정에서 스토리에 등록하는 이미지(스토리당 최대 10). 이름 · 트리거 문구(`trigger_text`) · 이미지 키로 구성하며, 채팅 중 표시 규칙은 채팅 확장 스펙이 정의(Phase 1) | — |
...
| 일반 제작 | general creation | 제작 폼에 스토리 구성 항목을 직접 입력하는 제작 방식(Phase 1). 기본 정보 · 스토리 설정 · 시작 설정 · 추천 입력을 직접 입력하며, 주요 사건·엔딩·이미지는 KNK-444·이미지 출력 스펙 확정 전까지 편집 폼 저장·왕복용 초안으로만 다룬다 | "일반 모드" · "일반 모드 제작". 채팅 입력 모드 "일반 입력"(`NORMAL`)과 다른 개념(§0-5) |
...
| 체험 한도 | trial limit | 게스트가 크레딧 없이 쓸 수 있는 기기 단위 사용량 상한. Phase 1 기준은 AI 비용이 드는 스토리 간편 제작(컴파일) 횟수 + 채팅 턴 수이며, 일반 제작 등록은 제외한다(수치 미정) | "무료 크레딧"(게스트는 크레딧이 없음) |
