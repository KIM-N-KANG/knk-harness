...
| 적립 | earn / `reason` 적립 계열: `SIGNUP_REWARD` · `INVITE_REWARD` · `ATTENDANCE_REWARD` | 보상으로 크레딧이 늘어나는 것. 회원가입은 500 크레딧, 초대는 초대자·피초대자 각각 500 크레딧, 출석체크는 250 크레딧을 적립한다. 초대 보상은 보상 수령 계정별 KST 월 10회까지만 적립한다. 보상 이벤트(사유) 관점의 "크레딧 보상" 표기는 허용 | "충전"(Phase 3 유료 결제 `PURCHASE` 전용) |
...
| 초대 코드 | invite code, `invite_code` | 다른 사용자를 초대해 보상을 받는 코드. 다른 회원이 코드를 입력하면 양쪽에 적립한다(계정당 평생 1회 입력, 초대 보상은 보상 수령 계정별 KST 월 10회 상한 — KNK-567 개편, [`4-backend.md §4-3-7`](./4-backend.md) 결정 기록) | "초대 링크"(링크 어트리뷰션 방식은 KNK-567로 폐기) · "추천인 코드" |
