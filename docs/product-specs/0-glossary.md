...
| 키 문장 | `key_sentence` | 주요 사건마다 두는 기준 문장. 사용자 입력이 키 문장과 관련되면 해당 주요 사건으로 전개하며, 관련성 판정은 AI의 정성 판정이다([`5-1-ai-server-spec.md §5-3-4`](./5-1-ai-server-spec.md))(Phase 1) | "트리거 문장"(키워드북 트리거와 혼동) |
...
Phase 1 보상·소모와 Phase 3 결제 범위 용어입니다. 이프는 회원 전용이며, 게스트는 이프 없이 디바이스 ID별 체험 한도로 사용하고 한도 소진 후 로그인으로 유도합니다. 계약·구조와 수치는 [`4-backend.md §4-3-7`](./4-backend.md)이 정본입니다.
...
| 화자 라벨(인물명 라벨) / 지문 | `인물명: 대사` 표기 / `*별표*` 행동 묘사 — 프론트·백엔드·AI가 공유하는 와이어 표기 규약. 변경 계획(KNK-1002): AI 서버가 줄 머리의 이 라벨을 직접 찾아 인물 이미지 위치를 정한다(LLM 태그 대체, [`5-1-ai-server-spec.md §5-3-4`](./5-1-ai-server-spec.md)) |
