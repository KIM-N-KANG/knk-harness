...
| 적립 | earn / `reason` 적립 계열: `SIGNUP_REWARD` · `INVITE_REWARD` · `ATTENDANCE_REWARD` | 보상으로 크레딧이 늘어나는 것. 회원가입은 500 크레딧, 초대는 초대자·피초대자 각각 500 크레딧, 출석체크는 250 크레딧을 적립한다. 보상 이벤트(사유) 관점의 "크레딧 보상" 표기는 허용 | "충전"(Phase 3 유료 결제 `PURCHASE` 전용) |
...
| 초대 링크 | invite link, `invite_code` | 다른 사용자를 초대해 보상을 받는 링크. 피초대자 가입 완료 시 양쪽 적립 | "추천인 링크" |
