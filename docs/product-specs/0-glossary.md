...
| 오리지널 스토리 | original story | 마냑 공식 계정이 소유한 공개(`PUBLISHED`·`PUBLIC`) 스토리. 피드·검색이 나오기 전까지 홈(`/`)에 노출해 제작 없이도 플레이할 거리를 준다(Phase 1 — KNK-932·975·983). 홈은 오리지널만 표시하고 내가 만든 스토리는 제작 탭(`/create`)이 담당한다(KNK-988 — [`3-1-client.md §3-1-3`](./3-1-client.md#3-1-3-화면별-스펙)). 조회는 `GET /stories/originals`이며 인증이 필요 없다 | "공식 스토리", "추천 스토리"(추천 로직이 없음), "마냑 스토리" |
