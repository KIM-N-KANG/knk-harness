...
| 인물 이미지 이름 | `image_name`(AI 요청) · `imageName`(SSE·응답) | 한 인물의 이미지 한 장을 구분하는 이름(예: `세린_기본`). 컴파일 AI가 지어 `character_images[].name`으로 보내고, 백엔드가 uuid를 붙여 파일명으로 쓰고 `story_characters.image_name`에 저장한다. `Phase 2 · 계획`(KNK-1002, [`4-backend.md §4-3-9`](./4-backend.md) 변경 계획) | "표정"(이름의 일부일 뿐 이름 자체가 아님) |
