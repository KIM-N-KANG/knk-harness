...
Phase 1 범위 용어입니다. 크레딧은 회원 전용이며, 게스트는 크레딧 없이 제한된 체험만 하고 한도 소진 후 로그인으로 유도합니다. 세부 정책(지급량·소모량·체험 한도)과 데이터 식별자는 크레딧 스펙 반영에서 확정한 뒤 갱신합니다.

| 공식 용어 | 영문 식별자 | 정의 | 지양 표기 |
| --- | --- | --- | --- |
| 크레딧 | credit | AI 기능(스토리 제작·채팅) 사용에 소모하는 서비스 내 재화 | "포인트" · "코인" |
| 크레딧 지갑 | wallet | 사용자별 크레딧 잔액 보관 단위 | — |
| 적립 | earn | 보상으로 크레딧이 늘어나는 것(회원가입 · 초대 · 출석체크). 보상 이벤트(사유) 관점의 "크레딧 보상" 표기는 허용 | "충전"(Phase 3 유료 결제 전용) |
| 소모 | spend | 기능 사용으로 크레딧이 줄어드는 것 | "차감"(회수·정산과 혼동), "사용" 단독 표기 |
| 출석체크 | attendance | 하루 1회 접속 보상을 받는 행위 | "데일리 체크인" |
| 초대 링크 | invite link | 다른 사용자를 초대해 보상을 받는 링크 | "추천인 링크" |
