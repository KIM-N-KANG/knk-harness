...
| 썸네일 | thumbnail | 스토리의 대표 이미지. Phase 1에서는 팀이 사전 제작한 이미지를 제작 시 자동 연결한다(사용자 트리거 생성은 Phase 3) | "커버". 와이어 `coverImageUrl`은 현행 계약으로 유지하되 신규 명명은 thumbnail 계열 |
...
| 일반 제작 | general creation | 제작 폼에 스토리 구성 항목을 직접 입력하는 제작 방식(Phase 1). 입력 항목 구성은 제작 확장 스펙에서 확정한다 | "일반 모드" · "일반 모드 제작". 채팅 입력 모드 "일반 입력"(`NORMAL`)과 다른 개념(§0-5) |
...
| 체험 한도 | trial limit | 게스트가 크레딧 없이 쓸 수 있는 기기 단위 사용량 상한(스토리 제작 횟수 + 채팅 턴 수, 수치 미정) | "무료 크레딧"(게스트는 크레딧이 없음) |
