...
| 키 문장 | `key_sentence` | 주요 사건마다 두는 기준 문장. 사용자 입력이 키 문장과 관련되면 해당 주요 사건으로 전개한다(Phase 1) | "트리거 문장"(키워드북 트리거와 혼동) |
| 엔딩 | `ending` | 스토리의 종결 정의. Phase 1부터 스토리당 3개(유형별 1개)로 두고, 본문을 제작 시 쓰지 않고 유형 · 조건 · 에필로그 가이드로 정의해 도달 시 생성한다. 현행 구현(제목·내용·`condition_text`)과의 정합은 [`4-backend.md §4-8`](./4-backend.md) 간극표 참조 | — |
| 엔딩 유형 | `ending_type`: `HAPPY` · `NORMAL` · `BAD` | 엔딩 분류 3종. 스토리당 각 1개(Phase 1) | "해피엔딩/새드엔딩" 혼용 표기 |
| 엔딩 조건 | `ending_requirement` | 한 번에 엔딩을 보는 것을 막기 위한 도달 판정 조건 3종(모든 엔딩 공통): **최소 턴 수 충족 · 목적 달성은 하드 조건(둘 다 충족, AND)**이고, **거쳐온 주요 사건은 AI가 엔딩 개연성을 판단하는 정성 판정 입력**이라 특정 사건 경유를 강제하지 않는다. 엔딩별로는 수치·목적·참고 사건 목록만 다르다(Phase 1) | `condition_text`(현행 구현 표기 — 재정의 예정) |
| 엔딩 에필로그 | `ending_epilogue` | 엔딩 조건 충족 시 현재까지의 맥락과 유형을 반영해 엔딩 응답을 생성하기 위한 출력 가이드(Phase 1) | "엔딩 출력 가이드"는 설명문에서 허용 |
...
| 일반 제작 | general creation | 제작 폼에 스토리 구성 항목을 직접 입력하는 제작 방식(Phase 1). 기본 정보 · 스토리 설정 · 시작 설정 · 추천 입력을 직접 입력하며, 주요 사건·엔딩은 KNK-444 확정 전까지 편집 폼 저장·왕복용 초안으로만 다룬다. 이미지는 썸네일·채팅 이미지 표시 계약([`4-backend.md §4-3-9`](./4-backend.md))으로 런타임 반영이 확정됐다 | "일반 모드" · "일반 모드 제작". 채팅 입력 모드 "일반 입력"(`NORMAL`)과 다른 개념(§0-5) |
...
Phase 1 범위 용어입니다. 크레딧은 회원 전용이며, 게스트는 크레딧 없이 제한된 체험만 하고 한도 소진 후 로그인으로 유도합니다. 계약·구조는 [`4-backend.md §4-3-7`](./4-backend.md)이 정본이고, 지급량·소모량·체험 한도 수치는 미정(`계획`)입니다.
...
| 적립 | earn / `reason` 적립 계열: `SIGNUP_REWARD` · `INVITE_REWARD` · `ATTENDANCE_REWARD` | 보상으로 크레딧이 늘어나는 것(회원가입 · 초대 · 출석체크). 보상 이벤트(사유) 관점의 "크레딧 보상" 표기는 허용 | "충전"(Phase 3 유료 결제 `PURCHASE` 전용) |
| 소모 | spend / `reason` 소모 계열: `STORY_CREATION` · `CHAT_TURN` | 기능 사용으로 크레딧이 줄어드는 것. 재생성은 `CHAT_TURN`과 동일 소모. 처리 전에 미리 소모하는 것은 "선차감(pre-charge)"으로 표기(허용) | "차감" 단독 표기(회수·정산과 혼동 — "선차감"은 예외), "사용" 단독 표기 |
...
| 체험 한도 | trial limit | 게스트가 크레딧 없이 쓸 수 있는 기기 단위 사용량 상한. Phase 1 기준은 AI 비용이 드는 스토리 간편 제작(컴파일) 횟수 + 채팅 턴 수(재생성 포함)이며, 일반 제작 등록은 제외한다(수치 미정) | "무료 크레딧"(게스트는 크레딧이 없음) |
...
| NORMAL | enum 값 `NORMAL`은 `input_mode`(일반 입력)와 `ending_type`(노말 엔딩) 양쪽에 존재한다. 단독 표기하지 않고 필드명과 함께 쓴다 |
