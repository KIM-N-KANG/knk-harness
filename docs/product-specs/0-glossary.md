...
| 초대 링크 | invite link, `invite_code` | 다른 사용자를 초대해 보상을 받는 링크. 피초대자 가입 완료 시 양쪽 적립하되, 초대 보상은 보상 수령 계정별 KST 월 10회 상한을 따른다 | "추천인 링크" |
| 체험 한도 | trial limit | 게스트가 크레딧 없이 쓸 수 있는 디바이스 ID별 사용량 상한. Phase 1 기준은 스토리라인 생성·재생성 10회, 스토리 간편 제작(컴파일) 3회, 모든 채팅방 합산 채팅 턴 15회(재생성 포함)다. 일반 제작 등록은 제외한다 | "무료 크레딧"(게스트는 크레딧이 없음) |
