...
마냑의 스토리 제작·채팅 생성과 이를 평가하는 연구 시스템의 현재 기준입니다. 선택 배경·대안·변경 이력은 [의사결정 기록](./5-2-ai-server-ard.md)에 둡니다.

| 항목 | 기준 |
| --- | --- |
| 제품 구현 기준 | manyak-ai `dev` · `dcce0c7d9507`(2026-09-02). 기존 v2.22의 기준을 유지하며, 이번 문서 분리로 코드 대조 범위를 확장하지 않습니다. |
...
현재 제약은 전체 History·오프닝 시드 차이(A3·A4), 출력 한도 미정(A8), 엔딩 판정 강화 미실측(A10), 전체 시간 예산·취소 전파 부족(A1·A11·A12·A15·A18), 공급자 배치·계측 부족(A13), 모델 별칭(A14), 시간 초과 외 판정 실패의 상태 초기화 위험(A16), 요약 마커 미정(A20)입니다. 인물 이미지 매핑·극단 라벨 입력은 수용한 제약(A19·A22)입니다. 종전 추적 항목 전체와 판단 근거는 [ARD 추적 이력](./5-2-ai-server-ard.md#기존-제약과-후속-판단-이력)에 남깁니다.

다른 서비스의 배포 여부는 해당 서비스 명세가 정본입니다. 구현 동기화 때는 기준 코드 SHA를 갱신하고 현재 계약만 이 문서에 반영하며, 결정이 바뀌면 ARD에 근거를 남깁니다.
