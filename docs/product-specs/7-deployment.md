...
- **Langfuse 코드가 담긴 AI 릴리스가 배포됐는지** — 구현(KNK-624·640)은 dev에만 머지돼 있어, 운영 릴리스에 실리기 전에는 키를 넣어도 켜질 코드 자체가 없습니다.
- **배선이 구현됐는지** — 현재 Terraform·Compose는 Langfuse를 주입하지 않으므로(후속), 배선 전에는 켜지지 않습니다.
- **활성화 가드가 반영됐는지** — 키가 있어도 `LANGFUSE_HOST`가 JP가 아니거나 환경이 `prod`가 아니면 no-op + 오류 로그로 막는 가드([`5-ai-server.md §5-6`](./5-ai-server.md), `Phase 1 · 계획`). 현재는 키만 있으면 켜집니다.
- `LANGFUSE_HOST`가 JP(`https://jp.cloud.langfuse.com`)인지 — 지정하지 않으면 코드 기본값(JP 아님)으로 대화 원문이 다른 리전으로 전송됩니다.
- **백엔드 키워드 단계 개편(KNK-621)이 먼저 배포됐는지** — 개편이 커스텀 장르 입력을 400으로 차단해야 `genre_tags`가 사전 정의만 남습니다. 장르 차원은 저장된 트레이스에 소급 부착되지 않으므로, 켜기가 KNK-621 이후여야 초기 데이터부터 장르로 분해됩니다.
- **채팅 트레이스 장르 태그 제거가 반영됐는지** — 장르 태그는 스토리 제작 트레이스에만 싣습니다([`5-ai-server.md §5-6`](./5-ai-server.md), `Phase 1 · 계획`).
