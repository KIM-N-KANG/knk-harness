...
- Android는 Crashlytics(release 빌드의 크래시와 API 30+ ANR)와 Amplitude를 사용합니다. 크래시 리포트에는 직전 화면·행동 이벤트와 로그인 사용자 식별자가 붙고 debug 빌드는 수집하지 않습니다([`3-3-1-android-design.md §3-3-6`](3-3-1-android-design.md)). 모든 API·SSE 요청에는 `X-Manyak-App-Version`이 실려 서버가 요청의 클라이언트 버전을 볼 수 있습니다(서버 소비는 후속). **앱 버전으로 끊어 보는 대시보드는 아직 없습니다**([§7-11](#7-11-미정주의-항목)).
