...
| 항목 | 방향 |
| --- | --- |
| 저장소 | 비공개 S3 버킷 1개(+ CloudFront OAC). prefix로 구분 — `thumbnails/` · `backgrounds/` · `characters/`([`4-backend.md §4-3-9`](./4-backend.md) 자산 카탈로그). **썸네일만 반응형 변형 2개**(`{imageKey}.png` 원본 + `{imageKey}_sm.png` 축소 ~700px, 정적 사전 생성 — KNK-548), 배경·캐릭터는 단일 원본 |
| 업로드 경로 | 운영 시드 전용 — 자산 파일은 S3에 직접 업로드하고(썸네일은 원본·`_sm` 두 오브젝트), 카탈로그 등재는 시드 매니페스트 검증을 거칩니다(검증 위반은 시드 실패 — [`4-backend.md §4-3-9`](./4-backend.md)). 클라이언트 업로드 API·presigned URL 발급은 없습니다. 후속에 사용자 업로드류 기능이 도입되면 같은 버킷·CDN 기반을 재사용합니다 |
| 서빙 경로 | CDN(CloudFront) 배포가 버킷을 origin으로 공개 서빙. `imageKey` → 서빙 URL 변환은 백엔드 소유(썸네일은 원본·`_sm` 두 URL), 객체 키는 불변(교체는 새 키 — 장기 캐시 전제). **불변 캐시라 canonical 오브젝트 재업로드는 CloudFront invalidation 동반**. CDN 도메인·캐시 정책 수치는 `계획` |
