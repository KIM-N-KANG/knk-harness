...
| 로컬 저장(본인 리소스 캐시) | **Room** | 목록·상세·채팅 턴을 항목 단위로 조회하고 오래된 것부터 제거·전삭제해야 합니다. 목적은 오프라인 지원이 아니라 **TTI 단축**입니다(§3-3-4 로컬 캐시) |
...
| 로그아웃 | 캐시를 **전부 삭제**합니다(`MUST`). `device_id`를 새로 발급하는데 데이터가 남으면 다음 사용자에게 노출됩니다 |
...
**결정 기록 — Room을 도입하고 채팅을 로컬 캐시(2026-08-04 재결정)**
...
| 홈·스토리 상세까지 캐시 | 서재 목록과 크레딧 잔액은 **계속 변하는 데이터**라 낡은 값이 "내 스토리가 사라졌다"는 최악의 오해를 만듭니다. 반면 **완료된 채팅 턴은 서버에서도 변하지 않아 캐시가 틀릴 여지가 구조적으로 없습니다.** 로딩 길이보다 이 성질이 우선합니다 |
...
- **영향.** `:core:data`에 Room 계층이 생기고 `:core:domain`에 캐시 동작을 포함한 Repository 계약이 추가됩니다. 홈·스토리 상세로의 확장은 v1 범위 밖이며 후속 과제로 [`roadmap.md §R-5`](../planning/roadmap.md)가 추적합니다. TTI를 수치로 재는 계측도 같은 후속 과제이며, v1은 검수 항목으로 확인합니다(§3-3-7).
...
| 대안 | 채택 안 한 이유 |
| --- | --- |
| v1에 함께 도입 | 지금은 **효과를 측정할 대상이 없습니다** — 화면이 비어 있어 무엇이 얼마나 빨라지는지 근거를 만들 수 없습니다. 시작 경로(로그인·세션 판정·그래프 결정)가 v1 개발 내내 바뀌므로 지금 뜬 프로파일은 출시 전에 낡습니다. M1이 절대 기한인 일정에(아래 출시 일정) 기능이 아닌 최적화로 리스크를 더할 이유도 없습니다 |
...
- **토큰 세션** — Keystore 암호화 저장, 선제 재발급(만료 여유 60초·**이중 시계 앵커**), 전역 단일 비행과 회전 저장 순서, 반응형 401 재시도, 실패 분기 3종(401·403·네트워크), 앱 수준 세션 상태 `Flow`와 그래프 전환 배선, 로그아웃 시 세션 폐기와 `device_id` 재발급.
- **홈·채팅 목록** — 회원 서재 API 연동, 카드·썸네일·빈 상태·오류 상태, 하단 3탭과 탭 상태 보존, 당겨서 새로고침. **Room 배선(엔티티·DAO)과 목록 캐시** — 진입 시 캐시 선표시와 응답 교체, 갱신 중 표시, 잔액 스켈레톤, 보관 상한, 로그아웃 전삭제(§3-3-4).
...
- **채팅 방 캐시** — 완료된 턴 기록(`completed` 후 재조회 결과만)과 진입 시 선표시·응답 교체, 404 시 캐시 삭제(§3-3-4).
