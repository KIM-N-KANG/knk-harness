...
**완성 실패는 토스트로 알립니다**(`MUST`). 사유(크레딧 부족·그 밖의 실패)는 문구로만 갈리고, 복귀 지점은 언제나 입력 화면이며 재시도는 "스토리 완성하기"를 다시 누르는 것입니다. 채팅방의 크레딧 부족 안내와 같은 표현이라 같은 사유를 화면마다 다르게 알리지 않습니다(2026-08-30 결정 — CTA 위 인라인 문구에서 교체).

**완성 402는 크레딧 부족 문구까지만 대응합니다.** 앱은 로그인 필수라 402는 회원 크레딧 부족뿐입니다(§3-3-1 — 게스트 한도 분기 비적용). 웹의 크레딧 획득 유도 다이얼로그는 앱에 크레딧 화면·획득 수단이 없어 붙일 대상이 없으므로, 완성 실패 토스트를 "크레딧이 부족해요"로 구분해 안내하는 데서 멈추고 유도 UI는 크레딧 기능 도입과 함께 정합니다.
...
- 전송 버튼 왼쪽 `spacing.compact`(8) 자리에 3-1 계약대로 턴 비용 `20 크레딧`을 `typography.body-small`·`textSubtle`로 표시합니다. 두 입력 모드와 세 전송 아이콘 상태가 같은 툴바를 쓰므로 자리도 그대로입니다. 앱은 로그인 필수라 게스트 비노출 분기는 없습니다.
...
**`error`·네트워크 실패는 전송하며 비운 입력을 컴포저로 되돌립니다**(`MUST`). 턴이 열리지 못한 실패라 사용자가 쓴 문장을 없앨 이유가 없고, 크레딧 부족(402)처럼 다시 누르는 것이 유일한 복구인 실패에서 문장을 다시 쓰게 만들면 안 됩니다. 되돌릴 때 채우기로 넣어 둔 추천 원문 기억도 함께 돌아가 출처 판정이 어긋나지 않습니다. **EOF로 끊긴 실패는 되돌리지 않습니다**(`MUST NOT`) — 서버가 이미 턴을 저장했을 수 있어 다시 읽은 목록과 컴포저에 같은 문장이 겹칩니다. 재생성 실패는 컴포저를 비운 적이 없어 되돌릴 것이 없습니다.
...
마이 탭 화면입니다([`3-1-client.md`](./3-1-client.md) FE-SCREEN-008 마이 페이지 회원 상태 구성). 앱은 회원만 진입하므로 게스트 로그인 버튼·이관 안내는 비적용입니다(§3-3-1). 섹션 구성은 웹과 같습니다 — 프로필 헤더 · 크레딧 카드 · 이벤트(친구 초대) · 화면(테마 변경) · 기타 · 계정(로그아웃·회원 탈퇴). **기타 섹션의 항목과 순서는 앱이 갈립니다** — 피드백 · 서비스 안내 · 오픈소스 라이선스 · 버전 정보 순이고, 웹은 서비스 안내가 피드백 위이며 뒤 둘이 없습니다. 오픈소스 라이선스와 버전 정보는 배포 형태가 있는 앱에만 필요한 항목이라 웹에 대응 화면이 없습니다.
...
**크레딧 카드와 출석 보상** — 잔액·출석 여부의 정본은 프로필(`GET /auth/me`의 `creditBalance`·`attendedToday`)이고 화면 진입 시 한 번 다시 읽습니다. 출석 체크는 `POST /users/me/credits/attendance`(멱등 — 오늘 이미 받았으면 `rewarded=false` 200)이며 결과를 토스트로 안내합니다: 지급 "출석 체크 보상으로 {n} 크레딧을 받았어요" · 이미 출석 "오늘은 이미 출석 체크했어요" · 실패 "출석 체크에 실패했어요"(웹 토스트 문구와 동일). **지급 결과를 잔액에 직접 더하지 않고 프로필 재조회로 반영합니다**(`MUST`) — 잔액 정본을 화면이 복제하지 않습니다. `attendedToday`면 버튼은 "출석 완료" 비활성입니다.
...
- **배경.** 웹은 카카오 JS SDK 로 공유 카드(제목 "초대 코드 등록하고 2,000 크레딧 받기 🎁" · 설명 "초대 코드: {코드}")를 띄웁니다. 앱이 같은 카드를 띄우려면 카카오 공유 SDK 를 새로 실어야 합니다 — 지금 앱에 든 카카오 의존성은 로그인용(`v2-user`) 하나뿐입니다.
...
**서비스 안내(FE-SCREEN-011)** — 별도 네이티브 화면·모듈을 만들지 않고 약관·개인정보처리방침과 같은 웹 문서 화면(`:feature:legal`)으로 `{WEB_BASE_URL}/about`을 엽니다. 본문 정본이 웹 한 곳이라 크레딧 정책 수치가 갈라질 수 없습니다 — FE-SCREEN-010과 같은 논거입니다. 웹이 새 브라우저 탭으로 여는 것에 맞춰 메뉴 트레일링에 외부 열림 아이콘을 둡니다. 경로는 `/about` 으로 확정됐고(KNK-1043 — `dev` 에 병합된 [`3-1-client.md`](./3-1-client.md) FE-SCREEN-011), **배포만 남았습니다** — 배포 전까지 앱에서는 404 페이지가 보입니다. **웹 잔여** — 앱 WebView는 미로그인 상태로 페이지를 열므로, "게스트 이용 안내(웹 전용)" 섹션을 앱 노출에서 숨기는 판정(3-1 검수 기준)은 웹이 소유합니다.
...
| FE-SCREEN-005 | 채팅 기본(SSE 스트리밍·선택지·삭제) | MVP | 확정 | 구현 | **채팅 플레이 필수** | 구현 | 채팅 생성·상세 조회·채팅방 진입·조회 렌더(제목·프롤로그·턴 이력)에 더해 입력 컴포저(블럭·일반 두 모드)·SSE 턴 진행(낙관적 렌더·"작성 중" 시머·글자 단위 타자기·전송 턴 상단 앵커·원자적 확정 교체)·추천 입력·1회성 힌트·턴 비용 표시·크레딧 부족(402) 안내·실패 시 입력 복원·삭제까지 구현 — 구조·색·간격·파싱 규칙은 §3-3-3 채팅방 절이 정본. **사용자 입력을 오른쪽 정렬 말풍선이 아니라 전체 폭 밴드로 그리는 것이 앱의 확정 사항**(웹과 같음 — 먼저 만든 말풍선은 교체 대상). Enter 전송·헤더 show/hide·게스트 분기는 비적용 | §3-3-2 · §3-3-3 |
...
- **마이 화면은 보일 때마다 다시 읽습니다**(`MUST`). 크레딧은 채팅 턴·스토리 완성으로 줄어들고 그 화면들은 프로필을 갱신하지 않으므로, 화면을 처음 만들 때 한 번만 읽으면 탭을 오가는 동안 낡은 잔액이 남습니다. 화면 재진입은 구성 변경으로도 일어나므로 직전 조회로부터 짧은 간격(현재 5초) 안의 재요청은 건너뜁니다.
