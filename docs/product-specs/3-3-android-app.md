...
- **영향.** 이어서 만들기 배너와 제작 퍼널 진입 FAB의 소유자가 홈에서 제작 탭으로 옮겨 갑니다. 퍼널 이탈이 돌아가는 자리도 제작 탭입니다. 홈은 오리지널 스토리 탐색만 소유하며, Android에서는 목록 구현 전까지 진입 가능한 골격으로 남습니다.
...
**진입점은 제작 탭 우측 하단의 FAB입니다**(`MUST`). 웹 제작 화면의 FAB에 대응하며, 누르면 키워드 선택 목적지를 메인 백스택에 push합니다. 앱은 로그인 필수라 웹 FAB의 게스트 한도 분기(로그인 다이얼로그)는 비적용입니다(§3-3-1). FAB는 셸이 아니라 제작 화면이 소유합니다 — 다른 탭에서는 표시하지 않는 그 탭의 주 동작이기 때문입니다. 배경은 주 동작 색, 아이콘은 `+`이고 접근성 이름은 "스토리 만들기"입니다. 이어서 만들기 배너는 FAB와 함께 구현되어 있고, 내 스토리 빈 상태 CTA는 목록 구현과 함께 추가합니다.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| FE-SCREEN-001 | 홈 오리지널 스토리 목록 | Phase 1 | 확정 | 구현 | 홈 탭 진입점 필수 · 목록 범위 결정 필요 | 부분 구현 | 메인 탭 셸의 홈 진입점만 구현. 오리지널 스토리 목록은 잔여이고, 앱은 로그인 필수라 게스트 노출 분기는 비적용(§3-3-1) | §3-3-3 · §3-3-4 |
| FE-SCREEN-013 | 제작·내 스토리 목록 | Phase 1 | 확정 | 구현 | 제작 탭 진입점 필수 · 목록 범위 결정 필요 | 부분 구현 | 메인 탭 셸의 제작 진입점과 제작 퍼널 진입 FAB·이어서 만들기 배너만 구현. 내 스토리 목록은 잔여 | §3-3-3 · §3-3-5 |
...
| --- | --- | --- | --- | --- | --- |
| FLOW-002 | 스토리 생성 | 013 · 002 · 005 | 결정 필요 | 부분 구현 | 제작 탭 내 스토리 목록과 채팅 플레이(005 입력 컴포저·SSE 턴 진행) 잔여 — §3-3-3 |
