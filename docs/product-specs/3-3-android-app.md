...
> **KNK-1197 전환 중(2026-09-05).** [모듈 재구성 설계](../planning/android-module-architecture.md)를 사용자의 구현 요청에 따라 적용합니다. 아래 기존 모듈 표·화면 루트 패키지·문자열 전량 core:ui 규칙은 아직 이동하지 않은 코드의 기준입니다. 이동한 모듈은 새 설계의 기능 소유 계층·리소스·의존 규칙을 따릅니다. 이전 core는 소비자가 남은 동안 전환 모듈로 유지할 수 있으며, common으로 먼저 이동한 기능 계약은 해당 소유 모듈을 옮길 때 제거합니다. 구현 상태 매트릭스의 제품 기능 완료 여부는 그대로 유지하고 구조 전환 완료는 별도로 검증합니다.
...
include(":feature:legal")     // FE-SCREEN-010·011 약관·개인정보처리방침·서비스 안내 — 웹 문서 화면(인증·메인 공용)
...
include(":feature:share")     // FE-SCREEN-012 채팅 공유 열람 — 현재 비적용(§3-3-3 앱이 옮기지 않는 것)
```
...
| 5. 나머지 목표 범위 | 피드백이 목표에 들어올 때 `:feature:feedback` 등 화면별로 하나씩(서비스 안내는 모듈 없이 `:feature:legal` 웹 문서 화면을 재사용 — §3-3-3 마이) | §3-3-3에서 확정된 Android 목표 범위만 구현. 목표에서 빠진 feature 모듈은 만들지 않습니다 |
...
| 3 | `:core:common` | 오류 타입 기반 **공통 판정 헬퍼**(402 이프 부족 판정 등). 와이어도 문구도 모릅니다 |
...
- **영향.** 다이얼로그와 더보기 버튼 컴포넌트는 `:core:ui`가 소유해 두 목록이 같은 것을 씁니다. 오리지널 카드에는 여전히 아무 진입도 없습니다. 채팅 목록의 삭제 진입에 관한 아래 채팅 목록 절의 `MUST NOT`은 이 결정으로 대체됩니다.
...
- **이미지는 채팅방과 같은 컴포넌트입니다.** 비율·모서리·테두리·맞춤·로드 실패 처리는 아래 인물 이미지 절의 표시 규칙 그대로이고, **URL 허용 검사도 같은 함수를 씁니다.** 두 번째 사용처가 생겼으므로 컴포넌트와 검사를 `:core:ui` 로 올렸습니다(§3-3-2 모듈 구조의 승격 규칙). 상세의 URL 은 AI 본문 마커가 아니라 서버 필드에서 오지만 검사를 건너뛰지 않습니다 — 두 경로가 같은 허용 목록을 쓰지 않으면 한쪽만 조용히 넓어집니다.
...
**같은 검사와 아래 표시 규칙을 스토리 상세의 주변 인물 섹션도 씁니다**(위 스토리 상세 절). 컴포넌트와 검사 함수는 `:core:ui` 에 있고, 채팅은 이벤트·마커에서 얻은 URL 을, 상세는 서버 필드(`characters[].imageUrl`)를 넘깁니다.
...
**내역 탭** — 기존 이프 내역 목록을 그대로 옮깁니다. **화면을 채우는 지연 목록이고 마지막 항목이 보이면 다음 커서를 잇습니다**(`MUST`) — "더 보기" 버튼을 두지 않습니다. 목록 줄의 위아래 여백은 8dp이고 탭 내용의 위아래 여백도 8dp입니다(무료 충전 탭만 위 16dp). 사유 라벨과 "삭제된 스토리"·빈 상태 문구는 `:core:ui` 문자열 리소스가 소유합니다.
...
**서비스 안내(FE-SCREEN-011)** — 별도 네이티브 화면·모듈을 만들지 않고 약관·개인정보처리방침과 같은 웹 문서 화면(`:feature:legal`)으로 `{WEB_BASE_URL}/about`을 엽니다. 본문 정본이 웹 한 곳이라 앱이 옮겨 적은 문구가 웹과 갈라질 수 없습니다 — FE-SCREEN-010과 같은 논거입니다. **이 화면은 더 이상 이프 수치를 고지하지 않습니다**(KNK-1095) — 수치를 모아 보여주는 자리를 두면 각 사용 지점과 이중으로 관리해야 해서 절 자체가 없어졌고, 수치는 각 소모·적립 지점이 서버 정책에서 직접 읽습니다(§3-3-4 이프 정책 수치 패칭). 웹이 새 브라우저 탭으로 여는 것에 맞춰 메뉴 트레일링에 외부 열림 아이콘을 둡니다. 경로는 `/about` 으로 확정됐고(KNK-1043 — `dev` 에 병합된 [`3-1-client.md`](./3-1-client.md) FE-SCREEN-011), **배포만 남았습니다** — 배포 전까지 앱에서는 404 페이지가 보입니다. **웹 잔여** — 앱 WebView는 미로그인 상태로 페이지를 열므로, "게스트 이용 안내(웹 전용)" 섹션을 앱 노출에서 숨기는 판정(3-1 검수 기준)은 웹이 소유합니다.
...
| FE-SCREEN-003·005 | 스토리 신고 | Phase 2 | 확정 | 미구현 | **스토어 심사 필수** | 구현 | 두 화면 모두 헤더 오른쪽 더보기 메뉴의 "신고하기"로 같은 시트를 염(사유 3개·상세 500자). **앱 전용 진입 추가(KNK-1180)** — 내 스토리 카드·채팅 카드의 옵션 다이얼로그(더보기·길게 누르기)에도 "신고하기"가 있어 같은 시트를 열며, 채팅 카드의 대상은 `ChatSummary.storyId`. 메뉴 컴포넌트는 `:core:ui` 가 소유해 채팅방과 상세가 같은 팝업을 씀. 채팅방에서 열어도 대상은 그 채팅이 참조하는 스토리(`ChatDetail.storyId`). 전송은 `POST /stories/{storyId}/reports`이고 성공하면 시트를 닫고 실패하면 입력을 남김. 시트·상태·전송 절차는 `:core:ui` 가 소유해 두 화면이 공유하고, 각 ViewModel 은 상태 한 덩어리와 Intent 한 갈래만 가짐. Google Play 생성형 AI 정책의 인앱 신고 요건을 겸함. 앱이 웹보다 먼저 구현 | §3-3-3 |
| FE-SCREEN-003 | 주변 인물 목록(KNK-1058) | Phase 2 | 확정 | 구현 | **스토리 상세 필수** | 구현 | 상세 응답 `characters[]` 의 이름과 이미지를 주요 내용 아래 섹션으로 그림. 이미지는 채팅 인물 이미지와 같은 컴포넌트·URL 허용 검사이고 둘 다 `:core:ui` 로 승격. 이미지가 없는 인물은 이름만 남김(§3-3-3 스토리 상세 — 주변 인물) | §3-3-3 |
...
- **조회는 앱 수명 싱글턴이 한 번 합니다.** 저장소가 `StateFlow` 로 값을 들고 루트가 시작할 때 한 번 읽습니다 — 수치를 쓰는 화면이 세 기능 모듈(`:feature:create`·`:feature:chat`·`:feature:my`)에 걸쳐 있어 화면마다 부르면 같은 값을 여러 번 읽습니다. 화면에는 테마 토큰과 같은 방식으로 `CompositionLocal` 로 내립니다 — 화면이 조회를 시작하지 않고 값만 읽으므로 화면 상태 기계에 넣을 것이 없고, 다섯 화면의 MVI 배선을 늘리지 않습니다.
