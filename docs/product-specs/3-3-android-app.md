...
**구현 형태(2026-09-08).** DB v2는 편집 슬롯 `pending_story_creation`(단일 행)을 유지하고 `story_completion_request`(`requestId` 기본 키, 명령·생성 결과·진행 JSON, `submittedAt`, `status=PENDING|COMPLETED|FAILED`, `storyId`·`storyTitle`)를 더합니다. v1→v2 마이그레이션은 `STORY_COMPLETION` 행을 해석해 요청 행(PENDING)으로 옮기고 원본을 지우며, 해석할 수 없는 행은 원본을 남긴 채 무시합니다. 파괴적 폴백은 제거했습니다. 제출은 DAO 트랜잭션 하나에서 요청 행 삽입과 **같은 생성 결과를 담은 초안 행** 삭제를 처리하고, 영속 실패면 전송하지 않습니다. 전송·복구는 `:create`의 앱 스코프 실행자가 `SessionGate.withAuthWork/commit` 안에서 하며 requestId 단위로만 합류합니다 — POST 성공은 COMPLETED, 네트워크 오류·401/403은 미확정 유지, 그 밖의 상태 코드(409 포함)는 복구 GET으로 판정하되 GET 404(서버가 모르는 요청)만 FAILED로 확정합니다. 제작 탭 복귀·당겨서 새로고침이 미확정 요청을 GET으로 조회하고, GET 404는 접수 전 끊긴 요청으로 보아 같은 명령을 재전송합니다. 완료된 요청은 내 스토리 목록에 같은 `storyId`가 실리면 행을 지우고, 아직 없으면 목록을 한 번 더 읽는 동안 실제 제목의 완료 카드로 남깁니다. 두 테이블은 `UserScopedStore` 정리에 넣지 않습니다(아래 계정별 보존). 제작 탭은 `:common`의 `CreationProgressAccess`(초안 요약·요청 요약 흐름·초안 폐기·요청 새로고침/재시도/삭제)만 봅니다.
...
**복구 폴링 — 화면 수명 게이트, 백그라운드 작업 수단 불채택.** 스토리라인 생성의 복구 조회(`GET /stories/simple/creation-requests/{requestId}`, 3초 주기)는 복원된 단계 화면이 STARTED 인 동안만 수집되는 Flow 로 돌립니다. 완성 요청의 복구 조회는 주기 폴링이 아니라 제작 탭 복귀·당겨서 새로고침 때 한 번씩입니다(KNK-1222) — 3-1의 "백그라운드 중지·복귀 재개"를 라이프사이클 수집으로 충족합니다. WorkManager·Foreground Service 는 채택하지 않습니다(§3-3-2 미결 해소) — 서버가 연결과 무관하게 생성을 완주하므로 앱이 백그라운드에서 요청을 완주시킬 이유가 없고, 결과 반영은 어차피 화면 복귀 시점입니다.
