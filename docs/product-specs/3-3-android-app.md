...
| FE-SCREEN-008 | 인증·마이 기본 | Phase 1 | 확정 | 구현 | **인증 출시 필수** | 부분 구현 | 로그인·토큰 보관·재발급·로그아웃에 더해 마이 화면 구성(프로필 헤더·크레딧 잔액·출석 보상·테마 변경·하위 진입점) 구현(§3-3-3 마이). 로그아웃 기본색·회원 탈퇴 위험색 배치는 앱이 선행. 계정 연동·친구 초대·회원 탈퇴 본 기능·402 다이얼로그가 잔여(§3-3-4 계정 연동). 이관·게스트 분기는 비적용(§3-3-1) | §3-3-3 · §3-3-4 |
