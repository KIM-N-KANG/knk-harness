...
> **작성 상태** — 안드로이드 앱 확장은 **Phase 2** 범위입니다([`roadmap.md §R-4`](../planning/roadmap.md)). **아키텍처와 인증 세션(§3-3-2·§3-3-3 내비게이션·§3-3-4)은 확정되어 구현이 이 문서만 보고 착수할 수 있습니다.** 화면별 스펙(§3-3-3 매트릭스), 디바이스 지원(§3-3-5), 관측 배선(§3-3-6), 검수 범위(§3-3-7)는 아직 채워지지 않았습니다. 값이 없는 항목을 추측으로 채우지 않습니다.
...
| `localStorage` 게스트 서재·카운터·플래그, 상태 판정(SSR 3-state) | **앱 비적용** — 게스트가 없습니다. 세션 판정은 미확정·미로그인·회원 셋 (§3-3-1 · §3-3-4) |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| FE-SCREEN-001 | 스토리 목록 기본 | MVP | 확정 | 구현 | 결정 필요 | 미착수 | 회원 서재 API만 사용(게스트 로컬 서재 비적용 — §3-3-1) | §3-3-4 |
...
| FE-SCREEN-008 | 인증·마이 기본 | Phase 1 | 확정 | 구현 | 결정 필요 | 미착수 | 소셜 로그인·토큰 보관 확정(§3-3-4). 이관·게스트 분기는 비적용(§3-3-1) | §3-3-4 |
...
로그아웃은 네 가지를 정리하며 **다음 순서를 지킵니다**(`MUST`). 순서가 틀리면 각 단계가 막으려던 것이 그대로 남습니다.

| 순서 | 작업 | 이 순서여야 하는 이유 |
| --- | --- | --- |
| 1 | `POST /auth/logout`으로 서버 세션 폐기 | 본문에 실을 refresh 토큰이 **아직 로컬에 있어야** 합니다 |
| 2 | 로컬 토큰과 **만료 앵커 2종** 삭제 | 앵커를 남기면 다음 세션의 만료 판정에 옛 값이 섞입니다(위 이중 시계) |
| 3 | 사용자 귀속 로컬 데이터 삭제(프로필 캐시 등) | 남으면 다음 사용자에게 이전 회원의 내용이 보입니다 |
| 4 | `device_id` 재발급과 분석 SDK 사용자 식별자 해제 | 앞 세 단계가 모두 **옛 `device_id`·옛 세션을 전제**로 합니다(위 식별자 배선) |

- **절차는 앱 스코프에서 실행합니다**(`MUST`). 화면 스코프에서 실행하면 화면 이탈로 중단되어 부분 정리 상태가 남습니다(§3-3-2 코루틴 원칙).
- **1이 실패해도 2~4는 그대로 진행합니다**(`MUST`). 사용자가 로그아웃을 눌렀는데 네트워크 때문에 로그인 상태로 남는 쪽이 더 나쁘고, 오프라인에서도 로그아웃은 되어야 합니다. 실패는 사용자에게 알리지 않습니다.
- **각 정리는 여러 번 실행해도 안전하고, 한 항목의 실패가 나머지를 막지 않습니다**(`MUST`).
- **그래프 전환은 4단계를 모두 마친 뒤에 발행합니다**(`MUST`). 토큰 삭제(2)가 곧바로 세션 상태 발행이 되도록 배선하면 3·4가 끝나기 전에 인증 화면으로 넘어가 정리와 겹칩니다.
- **기기 귀속 설정은 보존합니다**(`MUST NOT` 삭제) — 테마 선택처럼 사용자 계정이 아니라 기기에 속한 값입니다.
- **재발급 실패로 인한 세션 폐기도 같은 절차를 탑니다**(`MUST`, 위 재발급 실패 분기). 다만 그 경로는 세션이 이미 죽어 1이 실패하는데, 바로 위 규칙이 이를 흡수합니다.

**정리 대상은 한 곳에 나열합니다**(`MUST`). 각 저장소가 자기 정리 동작을 제공하고 중앙 종료 흐름이 조합하므로(§3-3-2 의존성 주입 구성), **새 사용자 귀속 저장소가 목록에 추가되지 않으면 리뷰 diff에서 드러나야 합니다.** 이후 로컬 캐시를 도입한다면 그 시점에 이 목록에 들어갑니다.
...
두 제공자는 같은 시작 함수를 provider 인자만 달리해 호출하고, 성공 이후 처리(토큰 보관·완료 피드백)는 provider와 무관하게 동일합니다. 진행 중 잠금 규칙은 §3-3-3 로그인 화면을 따릅니다. 로그인 응답의 `isNewUser`가 참이면 신규 가입 온보딩을 표시합니다(공통 계약 FE-SCREEN-008).
...
- **카카오는 웹과 같은 앱을 씁니다.** 앱을 새로 만들면 `sub`가 앱별로 달라 같은 사용자가 웹과 앱에서 다른 계정이 됩니다. 같은 앱의 플랫폼별 키를 함께 허용하는 것은 `sub`가 동일해 안전합니다.
...
- **계정 연동 재인증의 토큰 신선도 확보 수단.** 서버는 재인증에 **방금 발급받은** ID 토큰을 요구하고 오래된 것을 거부합니다(공용 기기 보호). Credential Manager와 Kakao SDK가 캐시된 자격증명으로 오래된 토큰을 돌려주면 연동이 항상 실패합니다. **요구 자체는 계약이므로 확정이고, 이를 만족시키는 호출 옵션은 실측으로 확인한 뒤 채웁니다** — 문서만으로 단정하면 틀렸을 때 연동이 통째로 막힙니다. 검증 항목으로 §3-3-7에 남깁니다.
...
**확정된 것** — 분석은 Amplitude Android SDK, 크래시는 Firebase Crashlytics입니다(§3-3-2 확정 스택). 논리 식별자 매핑도 확정입니다 — `device_id`는 앱이 첫 실행 시 생성한 UUID이고 API 헤더와 분석 SDK가 같은 값을 쓰며 로그아웃 시 재발급합니다(§3-3-4 식별자 배선). `session_id`는 분석 SDK가 관리하는 값을 씁니다.

`결정 필요` 항목:

- 분석 SDK 초기화 구성과 이벤트 발화 배선(`:core:analytics`)
- Crashlytics 초기화, 디버그 빌드 수집 비활성, 릴리스 매핑 파일 업로드
- `device_id`를 분석 SDK에 주입하는 시점 — API 헤더와 분석이 같은 값을 보게 하는 배선

> **`device_id`는 로그인 구현의 선행 조건입니다.** 첫 로그인 요청에서 이 헤더가 비면 회원 체험 잔여가 소진 상태로 비가역 확정됩니다(§3-3-4 식별자 배선). 분석 SDK 도입 여부와 무관하게 값이 먼저 존재해야 하므로, 이 매핑은 관측 배선을 기다리지 않습니다.
...
| # | 확인 | 왜 필요한가 |
| --- | --- | --- |
| 1 | 실기기에서 구글·카카오 각각 로그인해 메인에 진입하고, 앱을 재실행하면 로그인 화면을 건너뜁니다 | 세션 복원 기본 동작 |
| 2 | **첫 로그인 요청에 `X-Manyak-Device-Id`가 실려 있고 값이 비어 있지 않습니다** | 빠지면 회원 체험 잔여가 소진 상태로 **비가역** 확정됩니다(§3-3-4 식별자 배선) |
...
| 5 | **기기를 재부팅한 뒤에도** 만료 판정이 유지됩니다 | 단조 시계가 0으로 돌아가 만료 토큰이 유효로 판정되는 회귀를 잡습니다(§3-3-4 이중 시계) |
| 6 | 네트워크 오류로 재발급이 실패해도 **세션이 유지**됩니다 | 연결이 끊긴 곳에서 앱을 열 때마다 로그아웃되는 것을 막습니다 |
| 7 | 로그아웃 뒤 사용자 귀속 데이터가 남지 않고 `device_id`가 재발급됩니다 | 공용 기기에서 다음 사용자에게 이전 회원 데이터·행동이 귀속되는 것을 막습니다 |
| 8 | 계정 연동에 실패해도 **로그인 세션이 풀리지 않습니다** | 연동 경로의 403·409를 세션 만료로 처리하지 않는다는 계약(§3-3-4) |
| 9 | 계정 연동 재인증이 통과합니다 | **재인증 토큰 신선도 확보 수단이 미정이므로**(§3-3-4) 이 검증으로 수단을 확정하고 문서를 채웁니다 |
| 10 | **스토어 배포본**에서 로그인이 동작합니다 | 스토어 앱 서명 키 지문을 빠뜨리면 로컬 빌드에서만 성공하고 배포본에서 실패합니다(§3-3-4 운영 선행 조건) |

3·4·5는 실기기 없이 가짜 서버로 검증할 수 있으므로 **단위 테스트로 CI에 물립니다.** 나머지는 수동 확인 대상입니다.
