...
**서비스 안내(FE-SCREEN-011)** — 별도 네이티브 화면·모듈을 만들지 않고 약관·개인정보처리방침과 같은 웹 문서 화면(`:feature:legal`)으로 `{WEB_BASE_URL}/about`을 엽니다. 본문 정본이 웹 한 곳이라 크레딧 정책 수치가 갈라질 수 없습니다 — FE-SCREEN-010과 같은 논거입니다. 웹이 새 브라우저 탭으로 여는 것에 맞춰 메뉴 트레일링에 외부 열림 아이콘을 둡니다. 웹의 `/my/about → /about` 경로 이전이 배포되기 전에는 404 페이지가 보입니다. **웹 잔여** — 앱 WebView는 미로그인 상태로 페이지를 열므로, "게스트 이용 안내(웹 전용)" 섹션을 앱 노출에서 숨기는 판정(3-1 검수 기준)은 웹이 소유합니다.
...
| FE-SCREEN-011 | 서비스 안내 | Phase 1 | 확정 | 구현 | 결정 필요 | 구현 | **본문을 복제하지 않고 약관과 같은 웹 문서 화면으로 `/about`을 엶**(§3-3-3 마이). 앱은 회원만 진입. 게스트 이용 안내 섹션의 앱 노출 숨김 판정과 `/my/about → /about` 경로 이전 배포는 웹 잔여 | §3-3-3 |
...
| FLOW-009 | 서비스 정보 열람 | 008 · 010 · 011 | 결정 필요 | 구현 | 마이에서 서비스 안내(011)를 웹 문서 화면으로 열고, 약관·개인정보처리방침(010)은 로그인·서비스 안내 본문에서 동작. 웹 `/about` 경로 이전 배포 전에는 서비스 안내에 404가 보임 |
