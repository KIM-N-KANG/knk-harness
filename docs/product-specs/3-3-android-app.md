...
**단계마다 별도 목적지를 둡니다**(`MUST`). 웹은 `/stories/new` 단일 라우트 안에서 로컬 상태로 4단계를 전환하지만, 앱은 키워드 선택·스토리라인 선택·추가 정보를 각각의 `NavKey` 목적지로 두고 다음 단계 이동을 백스택 push로 표현합니다. 완료(생성 로딩)는 별도 목적지가 아니라 추가 정보 화면의 종료 상태입니다 — 진행 표시기가 앞 3단계만 노출하는 3-1 계약과 맞습니다. 키워드 단계 안의 카테고리 탭 전환은 단계 이동이 아니므로 화면 로컬 상태로 남습니다.
...
- **영향.** 시스템 뒤로가기·헤더 뒤로가기가 곧 이전 단계 복귀이며, 뒤에 남은 단계 화면의 입력은 백스택이 살아 있어 그대로 유지됩니다. 키워드 단계에서 뒤로가기는 퍼널 진입 전 화면으로 나가며, 이때의 이탈 가드·임시 저장·백그라운드 복귀 대응은 §3-3-5 간편 제작 라이프사이클 절이 확정합니다. 단계 간 공유 상태(생성 ID·선택 스토리라인)는 목적지 키에 식별자만 싣는 라우트 규칙을 따라야 하므로, 구체 보관 위치는 구현 시 `:feature:create` 안에서 정합니다.
...
| FE-SCREEN-002 | 백그라운드 생성 복귀·제작 임시 저장 | Phase 1 | 확정 | 구현 | 간편 제작 필수 | 구현 | requestId 영속(단일 슬롯 DataStore)·복구 3초 폴링·`STORY_DRAFT` 임시 저장·홈 배너·재개 다이얼로그 구현. 소실 경고 다이얼로그는 발생 조건이 없어 비적용 — 단계 복귀가 상태를 보존(§3-3-5) | §3-3-5 |
...
**이탈 가드 — 앱의 이탈 지점은 키워드 단계 뒤로가기 하나입니다.** 단계 간 뒤로가기는 백스택 보존 복귀(§3-3-3 결정)라 뒤 단계의 입력·결과가 소실되지 않고, 퍼널 탈출은 키워드 단계에서 시스템·헤더 뒤로가기로만 일어납니다. 이탈 시 처리는 3-1의 내용 보존 원칙을 따릅니다 — 진행 중 레코드(`STORYLINE_GENERATION`·`STORY_COMPLETION`)가 있으면 유지하고, 없고 스토리라인 결과가 있으면 `STORY_DRAFT`를 저장하며, 두 경우 모두 토스트("스토리가 임시 저장되었어요") 후 즉시 이탈합니다. 둘 다 없으면(키워드 입력만 있음) 3-1의 키워드 단계 제외 규칙대로 조용히 이탈합니다. **소실 경고 다이얼로그는 두지 않습니다** — 웹은 뒤로가기가 곧 퍼널 전체 이탈이라 "복원할 결과가 없는 소실"이 생기지만, 앱은 단계 복귀가 상태를 보존해 그 조건이 발생하지 않습니다(플랫폼 차이).

- **로딩 중 인퍼널 뒤로가기** — 생성·완성 대기 중 뒤로가기는 이전 단계 복귀이며 클라이언트 대기만 취소합니다. 서버는 끝까지 진행하므로 레코드는 보존하고, 이후 같은 페이로드 재시도가 409(`PENDING` 재POST)로 거절되면 실패가 아니라 복구 폴링으로 결과를 되찾습니다(3-1 완성 재시도 멱등 조항 — 생성·완성 공통).
...
**백스택 복원 — 재개·복구 진입은 단계 체인 전체를 쌓습니다.** 홈 배너·재개 다이얼로그에서 퍼널로 들어갈 때 레코드의 단계까지 `[키워드, 스토리라인(, 추가 정보)]` 라우트를 한 번에 쌓아, 복원 후에도 단계 복귀 불변(§3-3-3)이 유지되게 합니다. 키워드 입력은 저장 범위 밖(3-1)이라 복원된 키워드 화면이 비어 있는 것은 수용합니다. 프로세스 재시작은 Navigation 3 의 백스택 직렬화가 라우트를 복원하고, 비어 있는 생성 스토어(Idle)를 단계 ViewModel 이 감지해 레코드 복원·폴링에 합류합니다 — §3-3-3 이 보류로 남겼던 "프로세스 재시작 시 실패 화면 합류"를 대체합니다.
