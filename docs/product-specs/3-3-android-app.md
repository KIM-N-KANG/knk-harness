...
| 제작 퍼널 진입·이탈 | 퍼널이 아래에서 올라와 탭을 덮고, 덮인 탭은 제자리에서 어두워짐. 이탈은 역방향 | 제작 탭·재개 배너 → 퍼널, 퍼널 → 제작 탭 |
...
| FE-SCREEN-002 | 생성 퍼널 기본 | MVP | 확정 | 구현 | 간편 제작 필수 | 구현 | 퍼널 3단계와 서버 연동 구현 — 태그 조회·스토리라인 생성·재생성·평가·스토리 완성 API, 생성·완성 로딩(타자기 문구·지연 힌트)과 실패 복귀(완성 실패는 사유별 토스트), 닫기(X) 헤더·덮개 진입·이탈 전환(§3-3-3 화면 전환)·추가 정보 재선택 경고·완성 비용 행, 스토리라인 단계의 선택 키워드 시트, 완성 직후 채팅 생성·채팅방 진입(§3-3-3), 이탈 가드(§3-3-5) 포함. 구성 변경에서 세 단계 입력·선택, 키워드 추가 시트, 스토리라인 읽던 위치, 생성 지연 안내 경과가 유지됨(§3-3-5 구성 변경) | §3-3-3 · §3-3-5 |
...
| --- | --- | --- | --- | --- | --- |
| FLOW-002 | 스토리 생성 | 013 · 002 · 005 | **퍼널 완성 → 채팅 플레이 필수** | 구현 | 퍼널 완성 → 채팅 생성 → 채팅방 플레이(005 입력 컴포저·SSE 턴 진행)까지 전 구간 동작 |
...
| FLOW-010 | 푸시 알림 | — (앱 전용) · 008 알림 설정 | **토큰 등록 → 수신·탭 진입 → 수신 동의 설정** | 구현(서버 `recipientId` 합의 대기) | 토큰 등록·로그아웃 삭제·권한 요청은 KNK-1133(§3-3-4 푸시 토큰 등록). 수신 동의 설정 화면은 KNK-1135(§3-3-3 마이 · 알림 설정). **수신 표시·채널·탭 진입은 KNK-1134 구현**(§3-3-4 알림 수신·탭 진입 — `service`·`marketing` 채널, `recipientId` 검증, `PushEntry` 라우트 매핑, 미로그인 보류 → 로그인 뒤 복원, 종료 정리 `cancelAll`). 서버가 `recipientId`를 실어야 표시되며(4-backend 푸시 발송 모듈 `제안 · 서버 합의 대기`) 그 전에는 앱이 모든 메시지를 버립니다 — 실기기 표시 확인은 서버 반영 뒤 |
...
- **수신자 판별 계약은 아래 알림 수신·탭 진입 절이 소유합니다**(KNK-1134) — 서버가 모든 페이로드에 싣는 `recipientId`를 프로필 캐시와 비교하고, 없거나 다르면 버립니다. 서버 필드 자체는 [`4-backend.md §4-3-5`](./4-backend.md#4-3-api-계약) 푸시 발송 모듈의 `제안 · 서버 합의 대기` 항목이며, 합의 전에는 앱이 메시지를 전부 버리므로 표시 기능은 미완료입니다. KNK-1133의 토큰 등록 완료와 알림 수신 기능 완료를 구분합니다.
...
### 알림 수신·탭 진입 — `Phase 3 · 구현`(KNK-1134, 서버 `recipientId` 합의 대기)
...
- 서버가 모든 페이로드에 싣는 `recipientId`(수신 회원 `public_id`)를 프로필 캐시(`GET /auth/me`의 `id`, §3-3-4 프로필 조회와 캐시)와 비교합니다. **`recipientId`가 없거나 다르면 표시하지 않습니다.** 이 필드는 서버 팀 합의가 끝나야 `구현`이며, 합의 전 서버가 보내는 메시지는 앱이 전부 버립니다 — 그 상태로도 이전 계정 알림은 새지 않으므로 앱 출시를 막지는 않습니다.
...
- **영향.** 서버가 `recipientId`를 싣기 전에는 앱이 모든 메시지를 버립니다. 새 시나리오는 `type`만 더하면 홈 진입으로 안전하게 표시되고, 전용 목적지가 필요하면 `PushEntry` 한 곳만 고칩니다.
