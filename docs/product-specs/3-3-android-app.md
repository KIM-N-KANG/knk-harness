...
마이 탭 화면입니다([`3-1-client.md`](./3-1-client.md) FE-SCREEN-008 마이 페이지 회원 상태 구성). 앱은 회원만 진입하므로 게스트 로그인 버튼·이관 안내는 비적용입니다(§3-3-1). 구성과 순서는 웹과 같습니다 — 프로필 헤더 · 크레딧 카드 · 이벤트(친구 초대) · 화면(테마 변경) · 기타(서비스 안내·피드백) · 계정(로그아웃·회원 탈퇴) 섹션.
...
**자리 화면** — 친구 초대·피드백·회원 탈퇴는 진입점(메뉴 항목)만 먼저 열고, 목적지는 뒤로가기 헤더와 중앙 "아직 구현 중인 기능이에요" 문구뿐인 자리 화면입니다. 본 기능이 구현되면 각 화면을 대체합니다.

**계정 섹션 — 로그아웃은 기본색, 회원 탈퇴가 위험색입니다**(2026-08-30 결정). 웹 현재 배포본은 로그아웃 단독을 위험색으로 그리지만, 앱은 로그아웃을 `{colors.text}` 기본색으로 두고 그 아래 회원 탈퇴 항목을 `{colors.text-danger}`로 둡니다 — 위험색은 되돌릴 수 없는 동작(탈퇴)에만 남깁니다. 로그아웃 진행 중에는 라벨을 "로그아웃하는 중"으로 바꾸고 스피너를 표시하며 항목을 잠급니다. 공통 계약(3-1)의 회원 탈퇴 항목 추가·로그아웃 색 정렬은 웹 개편이 확정될 때 함께 갱신합니다.
...
| FE-SCREEN-006 | 피드백 | MVP | 확정 | 구현 | 결정 필요 | 미착수 | 마이의 진입점과 자리 화면만 둠(§3-3-3 마이). 제출 폼·전송은 미착수이며 구현 시 `platform: ANDROID` 고정 전송 | §3-3-1 · §3-3-3 |
| FE-SCREEN-007 | 온보딩 | MVP | 확정 | 구현 | **비적용** | — | 로그인 화면이 첫 실행 경험을 대체(§3-3-1) | §3-3-1 |
| FE-SCREEN-008 | 인증·마이 기본 | Phase 1 | 확정 | 구현 | **인증 출시 필수** | 부분 구현 | 로그인·토큰 보관·재발급·로그아웃에 더해 마이 화면 구성(프로필 헤더·크레딧 잔액·출석 보상·테마 변경·하위 진입점)과 신규 가입 초대 코드 안내(바텀 시트·코드 등록·사유별 인라인 안내) 구현(§3-3-3 마이). 로그아웃 기본색·회원 탈퇴 위험색 배치는 앱이 선행. 계정 연동·친구 초대 화면(코드 카드·복사·카카오 공유)·회원 탈퇴 본 기능·402 다이얼로그가 잔여(§3-3-4 계정 연동). 이관·게스트 분기는 비적용(§3-3-1) | §3-3-3 · §3-3-4 |
