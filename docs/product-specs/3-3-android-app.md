...
| 스토리 제목 | 1줄, 넘치면 말줄임. **참조 스토리가 삭제돼 제목이 비면 그 자리에 옅은 글자색으로 "삭제된 스토리예요"** (`앱 반영 대기`, KNK-1091 — 웹 선행 KNK-1092) | 같음 |
...
**이프 카드** `앱 반영 대기`(KNK-1091 — 웹 선행 KNK-1092) — 잔액의 정본은 프로필(`GET /auth/me`의 `creditBalance`)이고 화면 진입 시 한 번 다시 읽습니다. **카드에는 잔액 오른쪽에 이프 충전으로 가는 강조색 버튼 "충전" 하나만 둡니다** — 이프를 얻는 수단은 모두 이프 충전 화면이 소유하므로 마이에는 그 화면으로 가는 입구만 남깁니다. 출석 체크 버튼과 "내역" 버튼은 카드에서 빼고 이프 충전으로 옮깁니다(아래 이프 충전 절).

**출석 보상** `앱 반영 대기` — 동작 계약은 그대로이고 자리만 이프 충전의 무료 충전 탭으로 옮깁니다. 출석 체크는 `POST /users/me/credits/attendance`(멱등 — 오늘 이미 받았으면 `rewarded=false` 200)이며 결과를 토스트로 안내합니다: 지급 "출석 체크 보상으로 {n} 이프를 받았어요" · 이미 출석 "오늘은 이미 출석 체크했어요" · 실패 "출석 체크에 실패했어요"(웹 토스트 문구와 동일). **지급 결과를 잔액에 직접 더하지 않고 프로필 재조회로 반영합니다**(`MUST`) — 잔액 정본을 화면이 복제하지 않습니다. `attendedToday`(프로필)면 버튼은 "출석 완료" 비활성입니다.

**이프 충전(FE-SCREEN-008 이프 충전)** `앱 반영 대기`(KNK-1091 — 웹 선행 KNK-1092) — 구성·문구·데이터는 공통 계약을 따르고, 아래는 앱에서만 정해지는 부분입니다. 기존 이프 내역 화면을 두 탭으로 넓힌 것이라 **새 목적지를 만들지 않고 같은 목적지를 고칩니다** — 진입 버튼만 "내역"에서 "충전"으로 바뀝니다.

**잔액 상자와 탭 줄은 화면 상단에 고정하고 탭 내용만 스크롤합니다**(`MUST`) — 잔액은 어느 탭에서 무엇을 하든 계속 보여야 하는 값이고, 스크롤을 따라 올려 보내면 출석하러 들어온 사용자가 지금 얼마인지 모르는 채로 버튼을 누릅니다. 탭은 무료 충전 · 내역 둘이고 기본값은 무료 충전이며 밑줄 지시자를 씁니다(웹과 같은 모양). 선택한 탭은 저장하지 않습니다 — 진입 경로가 마이의 "충전" 하나뿐이라 복원할 맥락이 없고, 구성 변경에서만 유지하면 됩니다(§3-3-5).
...
**친구 초대(FE-SCREEN-008 친구 초대)** — 마이와 이프 충전의 진입 줄은 "친구 초대" 라벨 아래에 작은 강조색 보조 문구 "하고 2,000 이프 받기"를 붙여 라벨과 이어 한 문장으로 읽히게 합니다(`앱 반영 대기`, KNK-1091). 화면 자체의 레이아웃·문구·간격은 웹(`manyak-web/src/features/my/invite`)을 그대로 옮깁니다. 내 코드 카드(조회 중 골격 · 실패 시 "초대 코드를 불러오지 못했어요"와 다시 시도하기 · 이번 달 보상 진행) · 복사·공유 버튼 쌍 · 받은 코드 등록 · 이용 안내 네 줄 순서입니다. 코드와 보상 진행은 `GET /users/me/invite` 이고 월 상한은 정책 수치라 응답이 싣는 값을 그대로 씁니다(화면이 갖지 않습니다). 등록은 `POST /users/me/invite/redeem` 이며 **신규 가입 안내 시트와 같은 실패 문구 계약을 공유합니다**(위 초대 코드 안내 문단).
...
| FE-SCREEN-008 | 이프 충전 개편(KNK-1091) | Phase 2 | 확정 | 구현 | **이프 충전 개편 필수** | 미구현 | 웹이 선행 구현(KNK-1092). 앱이 옮길 것: 마이 이프 카드를 "충전" 버튼 하나로 줄이고 출석 버튼을 이프 충전으로 이동, 같은 목적지를 무료 충전·내역 두 탭으로 넓히고 잔액 상자·탭 줄 고정, 무료 충전 탭에 출석 상자와 친구 초대 진입 줄 배치, 친구 초대 진입 줄에 보조 문구 추가(§3-3-3 마이 · 이프 충전). 새 API 없음 — 기존 프로필·출석·내역 호출을 그대로 씁니다 | §3-3-3 |
| FE-SCREEN-004 | 삭제된 스토리 채팅 카드(KNK-1091) | Phase 2 | 확정 | 구현 | **채팅 목록 필수** | 미구현 | 웹이 선행 구현(KNK-1092). `storyTitle`이 빈 채팅을 목록에서 버리지 않고 제목 자리에 "삭제된 스토리예요"를 옅은 글자색으로 표시(§3-3-3 채팅 목록) | §3-3-3 |
...
| 8 `앱 반영 대기` | 참조 스토리를 지운 뒤 그 채팅이 목록에 **남아 있고** 제목 자리에 옅은 글자색으로 "삭제된 스토리예요"가 나옵니다. 카드를 누르면 채팅방이 그대로 열립니다 | 제목이 비었다고 항목을 버리면 사용자가 이어가던 채팅에 닿을 길이 없어지고, 빈 제목을 그대로 그리면 무엇의 채팅인지 알 수 없는 줄이 남습니다(KNK-1091) |
