...
**셸을 두르는 목적지는 탭 넷뿐입니다**(`MUST`). 스토리 상세·제작 퍼널·채팅 화면·공용 웹 문서·마이 하위 목적지(친구 초대·피드백·회원 탈퇴)는 헤더도 하단 탭도 없는 전체 화면이며, 웹의 `(story)`·`(chat)`·`(legal)`·`(my)` 그룹에 대응합니다. 목적지가 늘어날 때의 판정 기준은 **하단 탭으로 직접 갈 수 있는 목적지인가**이고, 그 밖의 목적지는 전체 화면입니다.
