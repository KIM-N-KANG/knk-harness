...
| FE-SCREEN-008 | 이프 내역(KNK-1081) | Phase 2 | 확정 | 미구현 | **이프 내역 필수** | 부분 구현 | 마이 이프 카드의 "내역" 진입 버튼만 붙었고 화면은 미착수. 잔액 상자(친구 초대 코드 카드와 같은 상자) + 32dp 아래 커서 목록 구성과 사유 라벨·부호·만료일 표기는 §3-3-3 마이 · 이프 내역. 서버 계약은 이미 열려 있음(`GET /users/me/credits/transactions` — KNK-1044). 앱이 웹보다 먼저 구현 | §3-3-3 · §3-3-5 |
