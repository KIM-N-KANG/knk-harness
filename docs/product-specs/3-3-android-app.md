...
| 이력      | v0.5~v1.6(2026-08-02~04) 골격 작성과 리뷰 반영 — 직접 구현 MVI·멀티 모듈 12개·Crashlytics·전 기기 회전·FCM v1 편입 등 기술 선택 확정(각 결정의 배경·대안은 본문 결정 기록, 개정 단위 경위는 git 이력 참조). v2.0~v2.2(2026-08-04) Room 도입과 캐시 대상 확대 — 채팅 목록·방에서 시작해 회원 서재 목록·스토리 상세까지 출처·소유 기준으로 재정의. v2.3(2026-08-05) 정합성 리뷰 — 로그아웃 절차 5단계 신설, 회원 탈퇴 앱 구현 W3 배정, 절대 기한을 8/14 옵트인 완료로 교정, 교차 문서 정렬. v2.4(2026-08-05) 문서 정비 — 크레딧 잔액 위치 오기 수정, 채팅 방 보관 상한 30개 확정, M1 첫 심사 대기 리스크 명시. v2.5(2026-08-05) 편집 정비 — 중복 서술 통합(탈퇴 폴백·FCM 해제 사유·캐시 원칙), 결정 기록 배경 압축, 주관적 표현을 사실 서술로 교체, 깨지기 쉬운 숫자 참조 제거, "동상" 오기 수정 |
...
| 내비게이션 | **Navigation Compose**(타입 안전 라우트) | 공식 지원 라이브러리이며 Hilt 통합을 제공합니다. 타입 안전 라우트도 목적지별 딥링크 선언을 지원하므로 Phase 3의 App Link 도입이 프레임워크 교체를 강제하지 않습니다(§3-3-3 딥링크 이음매). Navigation 3은 구현 시점에 안정화 상태를 재확인합니다 |
...
- **배경.** 초기 결정(2026-08-03)은 Orbit MVI 채택이었으나, 채택 근거였던 "화면마다 3요소를 강제해 균일해진다"가 성립하지 않아 재결정했습니다 — Orbit이 타입으로 강제하는 것은 State와 SideEffect뿐이고 **Intent라는 개념이 없어**(공식 예제도 액션마다 public 메서드를 사용), 3요소의 균일성은 프레임워크가 아니라 팀 관례가 만듭니다.

| 대안 | 채택 안 한 이유 |
| --- | --- |
| Orbit MVI | 실제로 제공하는 것은 사이드이펙트 무손실 전달·reduce 직렬화·라이프사이클 인지 수집이고, 베이스 클래스 25줄과 `Channel(BUFFERED)`로 직접 구현되는 범위입니다. 반면 최근 메이저 버전에서 타입 계층을 전면 개명해(`ContainerHost` → `OrbitContainerHost`) 마이그레이션 부담이 실재하고, 외부 자료가 구 API 기준으로 남아 있어 학습에도 불리합니다 |
...
- **응답 모델을 기계적으로 이중화하지 않습니다.** 서버 응답과 화면이 쓰는 모양이 같으면 그대로 씁니다. 별도 도메인 모델은 **여러 응답을 합치거나, 화면이 쓰기에 응답 구조가 불편하거나, 서버 계약 변경으로부터 화면을 격리해야 할 때만** 만듭니다(`SHOULD`).
...
- **배경.** 초기 결정(2026-08-03)은 "단일 모듈로 시작하고 출시 후 전환"이었으나, 근거였던 초기 세팅 비용이 실측과 달라 재결정했습니다 — 참고 오픈소스 프로젝트는 24개 모듈을 **규약 플러그인 없이** 운영하고, 모듈별 `build.gradle.kts`가 14~91줄로 대부분 동일해 12개 모듈의 세팅이 마일스톤을 위협하는 규모가 아닙니다.
...
| Gradle 규약 플러그인 동시 도입 | 12개 규모에서는 모듈별 빌드 스크립트 복제 관리가 더 쌉니다(위 배경의 참고 프로젝트 실측). 모듈이 늘거나 버전 갱신 부담이 커지는 시점에 도입합니다(후속 과제 — [`roadmap.md §R-5`](../planning/roadmap.md)) |

- **영향.** `:core:domain`이 `kotlin-jvm` 모듈이 되어 안드로이드 프레임워크 참조가 **컴파일 에러**가 됩니다. 코드 리뷰가 지키던 규칙이 컴파일러 강제로 내려갑니다. 대신 Hilt·KSP가 모듈마다 실행되어 clean build가 느려집니다(증분 빌드는 빨라집니다). Version Catalog(`libs.versions.toml`)를 모든 모듈이 공유합니다.
...
**실행한 뒤 반드시 인텐트로 해제합니다**(`MUST`). 이유가 둘입니다. 첫째, 해제하지 않으면 회전할 때마다 `LaunchedEffect`가 다시 실행됩니다. 둘째, **`StateFlow`는 같은 값을 다시 방출하지 않으므로**, 해제해 `null`을 거치지 않으면 같은 요청이 연달아 발생했을 때(같은 대상을 두 번 공유하는 등) 두 번째가 화면에 도달하지 않습니다. 해제는 선택 가능한 정리 작업이 아니라 이 패턴의 성립 조건입니다.

채널 대신 상태로 두는 이유는 다음과 같습니다.

- 회전·프로세스 복귀에 안전합니다.
- 화면마다 채널을 새로 배선하지 않아 구조가 균일해집니다.
- 402처럼 **사건과 함께 화면별 상태 복구가 필요한 경우**(퍼널은 입력·선택 유지, 채팅은 낙관적 버블 제거 — 아래 에러 표현)를 같은 `reduce` 안에서 처리할 수 있습니다.

### 코루틴·Flow 사용 원칙

- 비동기 작업은 화면 스코프(`viewModelScope`)에서 실행합니다. `GlobalScope`를 사용하지 않습니다(`MUST NOT`).
- 화면보다 오래 살아야 하는 작업은 주입한 앱 스코프에서 실행하며, 허용 대상은 **토큰 재발급 · 분석 이벤트 발송 · 퍼널 슬롯 기록 · FCM 토큰 등록(§3-3-5) · 로그아웃 정리 절차(§3-3-4)**로 한정합니다. 그 밖의 앱 스코프 사용은 코드 리뷰에서 사유를 요구합니다.
...
| 채팅·재생성 SSE | 전체 상한 없음 · 이벤트 간 무수신 상한 150초 | 전체 상한은 공통 계약대로 백엔드의 두 층 상한(전체 상한·이벤트 간 idle 상한)에 위임합니다. 무수신 상한은 FIN 없는 단절로 죽은 연결의 영구 대기만 자르는 값입니다(아래 SSE 종료 정책 요약) |
...
**원천은 [`4-backend.md §4-3-3`](./4-backend.md)과 [`3-1-client.md §3-1-5`](./3-1-client.md#3-1-5-채팅-플레이와-sse-스트리밍)이며, 이 표는 옮겨 적은 것입니다.**
...
| 서드파티 SSE 라이브러리 | `okhttp-sse`가 공식·경량으로 계약을 충족해 추가 의존이 필요하지 않습니다 |

- **영향.** 스트리밍 전용 OkHttp 인스턴스를 별도로 구성합니다. 인증·식별 헤더 인터셉터가 스트림 요청에도 그대로 적용되어 선제 재발급이 자동으로 걸립니다. 화면 이탈 시 Flow 취소가 곧 스트림 중단입니다(계약의 "조용히 종료").
...
- 이벤트는 `callbackFlow`로 감싸 `Flow`로 노출하되 **스트림 Flow도 예외를 던지지 않습니다**(`MUST`) — `token`·`completed`·`error`·스트림 종료(EOF·무수신 상한 포함)를 sealed 이벤트로 정규화합니다.
- **ViewModel은 스트림 이벤트를 `onIntent` 쪽에서 수집해 `when`으로 소진하고, 각 분기를 `ReducerEvent`로 변환해 `dispatch`합니다**(§3-3-2 UI 상태 모델). 스트림 이벤트 타입과 `ReducerEvent`를 같은 타입으로 겸용하지 않습니다(`MUST NOT`) — 전자는 데이터 계층의 와이어 개념이고 후자는 화면 상태의 변이 원인이라, 겸용하면 `:core:data`의 타입이 화면 계약이 됩니다.
- 재생성 스트림도 같은 구현을 재사용합니다.
- 타임아웃 값과 이벤트별 종료 처리는 위 **SSE 종료 정책 요약**이 정본입니다.
...
- **화면 구성 범위** — v1은 위 구성 표의 요소만 담습니다. 채팅 예시를 보여 주는 확장형 소개 화면은 디자인 리소스 확보 후 재검토합니다.
...
| 퍼널 완료 → 채팅 화면 | `replace` | 퍼널을 백스택에서 제거 | 위와 같음 |
...
- **배경.** 이전 결정은 단조 시계(`elapsedRealtime`) 단독 앵커였습니다(근거: 시계가 틀어진 기기에서의 오판 방지). 그러나 `elapsedRealtime`은 부팅 후 경과 시간이라 재부팅하면 0부터 다시 셉니다. 부팅 후 시간이 저장된 앵커보다 커지면 재부팅 사실이 드러나지 않은 채 경과 시간이 실제보다 작게 계산되고, **만료 토큰이 유효로 판정됩니다.** 앵커의 영속화와 프로세스 재시작 후 판정 규칙도 이 결정에서 함께 정의합니다.
...
| 보관 — 채팅 턴 | **최근 진입한 방 30개**만 유지합니다(코드 상수 — 변경은 앱 업데이트). 기간 상한을 두지 않습니다 — 오래 안 들어간 방은 캐시해도 쓰이지 않습니다 |
...
| 캐시 없음 · 조회 중 | **스켈레톤** — 최종 레이아웃과 같은 크기를 차지합니다 |
| 캐시 없음 · 조회 실패 | 오류 상태 + 재시도 |

첫 실행과 캐시 비대상 필드(크레딧 잔액·본 엔딩 배지)는 캐시가 존재할 수 없으므로 항상 두 번째 행(스켈레톤)에 해당합니다.

**스켈레톤은 화면 전체가 아니라 필드 단위로 적용합니다**(`MUST`). 스토리 상세는 제목·소개·썸네일을 캐시에서 그리고, 캐시 비대상인 본 엔딩 배지만 스켈레톤으로 둡니다. 크레딧 잔액은 마이 화면(FE-SCREEN-008) 소속이며 같은 방식으로 잔액 필드만 스켈레톤을 적용합니다 — 홈 화면에는 크레딧 잔액이 표시되지 않습니다([`3-1-client.md`](./3-1-client.md) FE-SCREEN-001). 스피너나 빈칸을 쓰지 않는 이유는 **레이아웃 시프트**입니다 — 최종 크기와 다르면 값이 도착하는 순간 주변 요소가 밀립니다.
...
- **배경.** v1.0까지는 "관계형 데이터가 없고 오프라인을 지원하지 않는다"는 이유로 Room을 도입하지 않았습니다. 재검토에서 판단 기준을 데이터의 모양이 아니라 **"연결이 나쁠 때 사용자가 무엇을 보는가"**로, 목표를 오프라인 지원이 아니라 **TTI 단축**으로 다시 정하면서 도입으로 전환했습니다 — Room은 조인이 필요할 때만 쓰는 도구가 아니라 로컬 데이터를 항목 단위로 질의·교체·삭제할 때 쓰는 도구입니다.

| 대안 | 채택 안 한 이유 |
| --- | --- |
| 미도입 유지(v1.0 결정) | 화면에 진입할 때마다 로딩을 표시하게 됩니다. 재진입이 잦은 채팅형 UI에서 반복 로딩은 체감 성능을 떨어뜨립니다 |
...
- **범위 확대(2026-08-05).** 초판은 **채팅 목록·방만** 캐시했습니다. 회원 서재 목록은 크레딧 잔액과 별개 필드라 정책을 분리할 수 있고 본인이 만든 것만 담겨 피드처럼 매번 바뀌지 않으므로 편입했습니다. 스토리 상세는 홈 → 상세 → 채팅이 가장 흔한 경로(FLOW-003)인데 목록만 캐시하면 상세 진입에서 로딩이 남으므로 편입했습니다. 이에 따라 **대상을 화면이 아니라 출처·소유로 재정의**했고(위 대상 표), 상세에는 조건 둘이 붙습니다 — 캐시된 서재 목록에 있는 `storyId`만 캐시하는 **소유 판정**과, 플레이로 변하는 **본 엔딩 배지 제외**입니다. 목록·상세는 통째로 교체되고 서재 갱신 시 목록에 없는 상세를 삭제하므로 낡은 캐시가 남지 않습니다.

### 로그아웃 절차

로그아웃은 다섯 가지를 정리하며(FCM 토큰 해제 · 서버 세션 폐기 · 로컬 토큰 삭제 · 로컬 데이터 삭제 · `device_id` 재발급), **다음 순서를 지킵니다**(`MUST`). 순서가 틀리면 각 항목이 막으려던 사고가 그대로 남습니다. 절차는 **앱 스코프에서 실행합니다**(`MUST`, §3-3-2 코루틴 원칙의 허용 목록) — 화면 스코프에서 실행하면 화면 이탈로 중단되어 부분 정리 상태가 남습니다.
...
**1·2가 실패해도 3~5는 그대로 진행합니다**(`MUST`). 사용자가 로그아웃을 눌렀는데 네트워크 때문에 로그인 상태로 남는 것이 더 나쁘고, 오프라인에서도 로그아웃은 되어야 합니다. 실패는 사용자에게 알리지 않으며, 기록은 디버그 빌드의 경고 로그뿐입니다(릴리스 빌드는 로그를 출력하지 않습니다 — §3-3-6).
...
- **본인 리소스 캐시를 제외하면** 응답 캐시 계층을 두지 않습니다(위 로컬 캐시). 재조회 규칙은 셋이며, 캐시가 있는 화면도 이 규칙을 그대로 따릅니다(캐시는 조회를 대체하지 않습니다 — 위 로컬 캐시).
...
| 대상 | 앱 구현 |
| --- | --- |
| 채팅 공유 발급(FE-SCREEN-005) | 발급 API 호출 후 **OS 공유 시트**로 링크를 전달합니다. 확인 다이얼로그와 발급 계약은 공통 계약을 따르되, 공통 계약이 확정 버튼 문구를 플랫폼 소유로 두므로 앱은 **"공유하기"**를 쓰고, 발급 진행 중 상태의 접근성 이름은 "공유 준비 중"입니다 — 웹의 "링크 복사하기"를 그대로 쓰면 실제 결과(공유 시트)와 어긋납니다. 복사 완료 토스트는 표시하지 않습니다 |
...
- **영향.** 동일성 계약이 구조적으로 충족됩니다. 대신 **웹의 `/terms`·`/privacy`·`/my/about` 경로가 플랫폼 간 계약이 됩니다** — 경로를 바꾸면 배포된 앱이 깨지므로 [`3-2-web-app.md §3-2-3`](./3-2-web-app.md#3-2-3-라우팅레이아웃공통-셸)에 이 사실을 기록합니다. 오프라인에서는 열람할 수 없으며 수용합니다. 서비스 안내의 게스트 관련 안내는 앱 사용자에게 해당하지 않으므로, 웹 콘텐츠에서 해당 대목을 웹 이용 기준으로 표기합니다 — 웹 레포가 수행할 작업이며 [`3-2-web-app.md §3-2-3`](./3-2-web-app.md#3-2-3-라우팅레이아웃공통-셸)에 기록되어 있습니다.
...
| 항목 | 값 | 비고 |
| --- | --- | --- |
| 최소 OS | **Android 8.0(API 26)** | 빌드의 `minSdk`도 26으로 맞춥니다(W1 빌드 구성). 결정 시점 판단은 국내 API 26 미만 활성 기기 비중이 낮다는 것이며, 실제 비중은 출시 전 Play Console 기기·버전 통계로 재확인합니다. 알림·백그라운드 관련 API 기준선도 단순해집니다 |
| 화면 방향 | **전 기기 회전 지원** | 매니페스트에 `screenOrientation`을 선언하지 않고 센서를 따릅니다. 웹도 회전을 막지 않습니다 — 웹의 `max-w-md` 단일 컬럼은 레이아웃 결정이지 회전 금지가 아닙니다(아래 결정 기록) |
...
- **배경.** 초기 결정(2026-08-03)은 폰 세로 고정이었으나, 근거였던 "웹이 모바일 단일 컬럼인 것과 같은 기준"이 성립하지 않아 재결정했습니다 — 웹은 회전을 막지 않고 콘텐츠를 `max-w-md`로 중앙 정렬할 뿐입니다([`3-2-web-app.md §3-2-5`](./3-2-web-app.md#3-2-5-반응형접근성브라우저-지원)). 단일 컬럼 레이아웃과 회전 금지는 별개의 결정입니다.
...
| 토큰 등록 | 로그인 성공 직후와 `onNewToken`에서 서버에 등록합니다. **로그아웃 시 해제하며, 해제가 정리의 첫 단계입니다**(`MUST` — 순서의 이유는 §3-3-4 로그아웃 절차). 해제가 누락되면 이전 회원 귀속 토큰이 다음 사용자 기기로 발송됩니다 |
...
- **배경.** 이전 결정은 푸시 알림 전체를 Phase 3로 미루는 것이었습니다. v1 편입 근거는 둘입니다. 첫째, **권한 요청 타이밍은 미룰수록 비쌉니다** — `POST_NOTIFICATIONS`는 한 번 거부당하면 되돌리기 어려운데, 퍼널 완성 시점은 소요 시간 안내와 함께 알림의 필요성을 설명할 수 있는 유일한 자리입니다. Phase 3로 미루면 이미 설치된 사용자에게 맥락 없이 묻게 됩니다. 둘째, Crashlytics 채택으로 **Firebase가 v1에 이미 들어옵니다**(§3-3-6).
...
| 로그아웃 | 사용자 식별자를 해제하고 `device_id`를 새로 발급합니다(§3-3-4 로그아웃 절차 5단계) |
...
- **배경.** 초기 결정(2026-08-03)은 Sentry Android SDK였으나, 근거였던 웹·서버·AI와의 도구 통일이 앱에서는 성립하지 않아 재결정했습니다 — 앱은 크래시·ANR만 수집하고 성능 추적을 끄며, 서버와 잇는 `request_id` 상관은 웹에서도 미도입입니다([`6-analytics.md §6-6-4`](./6-analytics.md)). 반면 앱 크래시는 웹·서버·AI가 공유하는 Sentry 쿼터를 잠식하며, 출시 직후는 크래시가 가장 몰리는 구간입니다.
...
- **회원 탈퇴 진입점**(§3-3-4 `TBD`) — 8/15에 확정되는 B26 결정을 **이 주차에 구현합니다.** 최소 대응(웹 탈퇴 요청 페이지를 인앱 브라우저로 열기)이면 마이 메뉴 한 줄이고, 셀프서브 탈퇴를 택하면 확인 다이얼로그·API 연동·탈퇴 후 세션 정리(§3-3-4 로그아웃 절차 재사용)까지 이 주차 범위입니다. 결정이 늦어질 때의 폴백 규칙은 §3-3-4 회원 탈퇴 절을 따릅니다.
...
- 필수 단위 테스트 목록(§3-3-7 테스트 범위)의 전 항목이 존재하고 통과합니다.
...
| 회원 탈퇴 대응 방식 결정([`4-backend.md §4-8`](./4-backend.md) B26) | **8/15 결정 · W3(~8/25) 구현** | 계정 삭제 요건 미충족은 심사 거절 사유입니다. 결정이 늦어질 때의 폴백 규칙은 §3-3-4 회원 탈퇴 절을 따릅니다 |
...
- [ ] 위 필수 단위 테스트 목록의 전 항목이 존재하고 통과합니다.
- [ ] **캐시가 있는 홈·스토리 상세·채팅 목록·채팅 방에 다시 들어가면 빈 화면이나 스피너가 보이지 않습니다**(TTI 검수 — §3-3-4). 갱신 중 표시가 함께 보이고, 상세의 본 엔딩 배지는 스켈레톤이 자리를 지켰다가 값으로 교체되며 레이아웃이 흔들리지 않습니다.
...
- **코드 리뷰 체크리스트** — ① 응답 모델의 필드와 nullable 여부를 [`4-backend.md`](./4-backend.md) 계약과 대조 ② 사용자 노출 문자열은 리소스로 ③ 로그·크래시 리포트에 원문·토큰·공유 식별자 금지 ④ 앱 스코프 사용은 허용 목록(§3-3-2) 외에는 사유 필요 ⑤ 화면은 `UiState`·`Intent`·`ReducerEvent` 3요소 준수, 화면별 `SideEffect` 채널 금지·1회성 사건은 앱 스코프 헬퍼 또는 소비형 필드(§3-3-2) ⑥ `:feature:*` 모듈 간 직접 참조 금지 ⑦ 새 의존성 추가는 결정 기록 동반.
