...
**이탈 가드 — 이탈 지점은 퍼널 3개 목적지 전부입니다.** 스토리라인 단계가 키워드를 대체하므로(§3-3-3 결정 기록 수정) 생성 전 이탈은 키워드 단계에서, 생성 후 이탈은 스토리라인 단계에서 일어납니다. 키워드 단계는 3-1의 제외 규칙대로 조용히 이탈합니다. 스토리라인 단계 이탈은 3-1의 내용 보존 원칙을 따릅니다 — 진행 중 레코드(`STORYLINE_GENERATION`·`STORY_COMPLETION`)가 있으면 유지하고(생성 대기 중 이탈 포함 — 클라이언트 대기만 취소, 서버는 끝까지 진행), 없고 스토리라인 결과가 있으면 `STORY_DRAFT`를 저장하며, 두 경우 모두 토스트("스토리가 임시 저장되었어요") 후 즉시 이탈합니다. 보존할 것이 없으면(생성 실패로 결과 없음) **소실 경고 다이얼로그**("스토리를 그만 만들까요? 지금 나가면 만들고 있는 내용이 사라져요")를 표시하고 확정 시 이탈합니다.
