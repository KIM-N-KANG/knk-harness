...
**좋아요는 성공한 뒤에야 상태와 수를 옮깁니다**(`MUST`). `POST /stories/{storyId}/like`·`DELETE /stories/{storyId}/like` 는 204 로 끝나고 갱신된 수를 주지 않으므로, 화면이 들고 있던 수를 한 칸 올리거나 내리고(0 아래로 내려가지 않음) 다음 상세 조회가 서버 값으로 맞춥니다. **미리 옮겼다가 실패로 되돌리지 않습니다** — 눌린 하트가 깜빡였다 풀리는 편이 잠깐 늦게 칠해지는 것보다 나쁩니다. 요청 중에는 버튼을 잠가 중복 전송을 막고, 실패하면 상태와 수를 그대로 둔 채 토스트("좋아요 변경에 실패했어요")로 알립니다. 접근성 이름은 상태에 따라 "좋아요"·"좋아요 취소"입니다.
