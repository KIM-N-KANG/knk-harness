...
**프로필 헤더** — 아바타(56dp 원형)·닉네임·연동된 계정 배지입니다. 아바타는 `GET /auth/me`의 인라인 썸네일(`profileThumbnailBase64`)을 우선 그리고, 없으면 `profileImageUrl`, 그것도 없으면 빈 원입니다. **연동 시작 UI("연동하기" 칩)는 두지 않습니다** — 계정 연동 흐름이 잔여인 동안(§3-3-4 계정 연동) 눌러도 동작 없는 진입점을 만들지 않고, 연동된 provider 배지만 표시합니다. 연동 구현 시 웹 구성(연동하기 칩·확인 다이얼로그)을 따라 붙입니다.
...
| FE-SCREEN-008 | 인증·마이 기본 | Phase 1 | 확정 | 구현 | **인증 출시 필수** | 부분 구현 | 로그인·토큰 보관·재발급·로그아웃에 더해 마이 화면 구성과 신규 가입 초대 코드 안내, **친구 초대 화면**(코드 조회·복사·공유·받은 코드 등록·이용 안내)과 **회원 탈퇴**(확인 네 항목 → `DELETE /users/me` → 로그아웃과 같은 종료 흐름) 구현(§3-3-3 마이). 로그아웃 기본색·회원 탈퇴 위험색 배치는 앱이 선행. 카카오 공유는 시스템 공유 시트로 대체(앱 전용 확정). **계정 연동·402 다이얼로그가 잔여**(§3-3-4 계정 연동). 이관·게스트 분기는 비적용(§3-3-1) | §3-3-3 · §3-3-4 |
...
- **연동 진입점은 아직 없습니다** — 마이 프로필 헤더는 연동된 provider 배지만 표시하고, 연동 시작 UI는 이 절 구현과 함께 붙입니다(§3-3-3 마이).
