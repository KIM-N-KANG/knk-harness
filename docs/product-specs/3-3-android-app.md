...
| 이력      | v0.5(2026-08-02) 골격 작성. v0.7~v0.10(2026-08-04) 교차·아키텍처 리뷰 반영 — 용어 통일, 결과 타입·오류 모델을 `core/domain`으로 이동, 앱 수준 세션 상태 전파·SSE 무수신 상한(150초) 정의, 로그인 화면(앱 전용) 개칭, `feature`를 도메인 그룹형으로 재편. v1.0(2026-08-04) 멘토 리뷰 반영 — UI 상태 관리를 직접 구현 MVI로, 처음부터 멀티 모듈 12개로, 크래시 수집을 Firebase Crashlytics로, 전 기기 회전 지원으로 결정 변경, FCM 생성 완료 알림 v1 편입. v1.1(2026-08-04) App Link 표기를 "v1 미선언 · Phase 3 도입"으로 정리하고 딥링크 이음매 3건 기록. v1.2(2026-08-04) `compose_stability.conf` 등록 규칙 추가, 앱 스코프 헬퍼 인터페이스를 `:core:domain`으로 이동, `trySend` 유실 정책 명시. v1.3(2026-08-04) `reduce` 순수성 근거를 정확성으로 교체, 필수 테스트에서 `reduce` 상태 전이와 소비형 필드 해제 분리, 명시적 백킹 필드 금지. v1.4(2026-08-04) Baseline Profile v1 미도입 결정. v1.5(2026-08-04) 토큰 만료 판정을 이중 시계로 교정. v1.6(2026-08-04) SSE 종료 정책 요약 표 신설. v2.0(2026-08-04) Room 도입 — 채팅 목록·방 cache-first(목표: TTI 단축). v2.1(2026-08-04) 캐시 대상을 출처·소유 기준으로 재정의, 회원 서재 목록 편입, 표시 규칙 3종 명시. v2.2(2026-08-04) 스토리 상세 캐시 편입(소유 판정·본 엔딩 배지 제외). v2.3(2026-08-05) 정합성 리뷰 — Room 결정 기록 갱신, 로그아웃 절차 5단계 신설, 회원 탈퇴 앱 구현 W3 배정, 절대 기한을 8/14 옵트인 완료로 교정, 교차 문서 정렬. v2.4(2026-08-05) 문서 정비 — 크레딧 잔액의 홈 화면 오기 수정(잔액은 마이 화면 소속 — 3-1 FE-SCREEN-001·008), 앱 스코프 허용 목록에 FCM 토큰 등록·로그아웃 정리 절차 추가, 로그아웃 절차의 실행 스코프와 실패 기록 방식 명시, 채팅 방 보관 상한 30개 확정 표기, M1 트랙 첫 심사 대기 리스크 명시, 이력·결정 기록의 개정 경위 서술 압축 |
...
- **배경.** 2026-08-03 인터뷰에서는 Orbit MVI를 채택했으나 2026-08-04에 뒤집었습니다. 채택 근거였던 "화면마다 단일 상태·인텐트·사이드이펙트를 강제해 균일해진다"가 성립하지 않습니다 — Orbit이 타입으로 강제하는 것은 State와 SideEffect뿐이고 **Intent라는 개념이 없어**(공식 예제도 액션마다 public 메서드를 사용), 3요소의 균일성은 프레임워크가 아니라 팀 관례가 만듭니다.
...
- **배경.** 2026-08-03 인터뷰에서는 "단일 모듈로 시작하되 패키지를 모듈 경계로 나누고 출시 후 전환"으로 정했으나 2026-08-04에 뒤집었습니다. 근거였던 "모듈 부트스트랩과 Gradle 규약 플러그인 작업이 초기 며칠을 소모해 첫 배포 마일스톤과 충돌한다"는 실측과 다릅니다 — 참고 오픈소스 프로젝트는 24개 모듈을 **규약 플러그인 없이** 운영하고, 모듈별 `build.gradle.kts`가 14~91줄로 대부분 동일합니다. 12개 모듈이면 총 600줄 수준으로 반나절 작업입니다.
...
- 화면보다 오래 살아야 하는 작업은 주입한 앱 스코프에서 실행하며, 허용 대상은 **토큰 재발급 · 분석 이벤트 발송 · 퍼널 슬롯 기록 · FCM 토큰 등록(§3-3-5) · 로그아웃 정리 절차(§3-3-4)** 다섯입니다. 그 밖의 앱 스코프 사용은 코드 리뷰에서 사유를 요구합니다.
...
| 캐시 없음 · 조회 중 — 첫 실행과 캐시 비대상 필드(크레딧 잔액·본 엔딩 배지)는 항상 이 행입니다 | **스켈레톤** — 최종 레이아웃과 같은 크기를 차지합니다 |
...
- **배경.** 2026-08-03 인터뷰에서는 폰을 세로 고정으로 정했으나 2026-08-04에 뒤집었습니다. 근거였던 "웹이 모바일 단일 컬럼인 것과 같은 기준"이 성립하지 않습니다 — 웹은 회전을 막지 않고 콘텐츠를 `max-w-md`로 중앙 정렬할 뿐이며, 데스크톱에서도 모바일 셸 그대로 열립니다([`3-2-web-app.md §3-2-5`](./3-2-web-app.md#3-2-5-반응형접근성브라우저-지원)). 단일 컬럼 레이아웃과 회전 금지는 별개의 결정입니다.
...
- **배경.** 2026-08-03 인터뷰에서는 Sentry Android SDK를 택했으나 2026-08-04에 뒤집었습니다. 근거였던 웹·서버·AI와의 도구 통일이 앱에서는 성립하지 않습니다 — 앱은 크래시·ANR만 수집하고 성능 추적을 끄며, 서버와 잇는 `request_id` 상관은 웹에서도 미도입입니다([`6-analytics.md §6-6-4`](./6-analytics.md)). 반면 조직이 이미 세 곳에 Sentry를 쓰고 있어 앱 크래시가 공유 쿼터를 잠식하며, 출시 직후는 크래시가 가장 몰리는 구간입니다.
...
**이 일정에는 버퍼가 없습니다.** 8/14 옵트인 완료에서 M2(8/28 심사 등록)까지가 **정확히 14일**이라, 통상적인 "연속 14일" 요건을 이론적 최솟값으로 채우는 계획입니다. 옵트인이 하루라도 밀리거나 유지 기간 중 테스터 수가 요건 아래로 떨어지면 M2가 그만큼 밀립니다. 대응은 둘입니다 — 테스터를 **요건 인원보다 여유 있게** 모집해 이탈을 흡수하고(8/8 명단), 요건 인원·기간 수치를 명단 확정 시점에 Play Console 안내로 **한 번 더 실측**합니다. 그래도 밀리면 위 개발 조건대로 범위가 아니라 출시일을 조정합니다. 비공개 테스트 트랙의 **첫 업로드 자체도 심사를 거치며, 신규 계정의 첫 심사는 수일이 걸릴 수 있습니다.** 이 대기도 8/14 기한 앞에 놓이므로 첫 빌드는 W1이 끝나는 대로 지체 없이 업로드합니다 — 요건은 테스터 유지이지 기능 완성이 아니므로 완성본일 필요가 없습니다(위 개인 개발자 계정 요건).
