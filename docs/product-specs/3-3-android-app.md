...
| 이력      | v0.5~v1.6(2026-08-02~04) 골격 작성과 리뷰 반영 — 직접 구현 MVI·멀티 모듈 12개·Crashlytics·전 기기 회전·FCM v1 편입 등 기술 선택 확정(각 결정의 배경·대안은 본문 결정 기록, 개정 단위 경위는 git 이력 참조). v2.0~v2.2(2026-08-04) Room 도입과 캐시 대상 확대 — 채팅 목록·방에서 시작해 회원 서재 목록·스토리 상세까지 출처·소유 기준으로 재정의. v2.3(2026-08-05) 정합성 리뷰 — 로그아웃 절차 5단계 신설, 회원 탈퇴 앱 구현 W3 배정, 절대 기한을 8/14 옵트인 완료로 교정, 교차 문서 정렬. v2.4(2026-08-05) 문서 정비 — 크레딧 잔액 위치 오기 수정, 채팅 방 보관 상한 30개 확정, M1 첫 심사 대기 리스크 명시. v2.5(2026-08-05) 편집 정비 — 중복 서술 통합(탈퇴 폴백·FCM 해제 사유·캐시 원칙), 결정 기록 배경 압축, 주관적 표현을 사실 서술로 교체, 깨지기 쉬운 숫자 참조 제거, "동상" 오기 수정. v2.6(2026-08-05) 출시 일정·마일스톤·선행 작업을 [`android-v1-release-plan.md`](../planning/android-v1-release-plan.md)로 분리 — 일정은 실행에 따라, 스펙은 결정에 따라 바뀌므로 변경 주기를 분리했습니다 |
...
- 출시 일정, 주차별 마일스톤, 코드 외 선행 작업 (담당: [`android-v1-release-plan.md`](../planning/android-v1-release-plan.md))
...
| [`android-v1-release-plan.md`](../planning/android-v1-release-plan.md) | 앱 v1 출시 일정·마일스톤·선행 작업 | 이 문서의 계약을 언제 만드는지의 계획 |
...
- **일정** — 8/15 결정, 앱 구현은 **W3(8/19~8/25)**입니다([`android-v1-release-plan.md §AR-2`](../planning/android-v1-release-plan.md)). W4는 검수·제출 전용이라 개발 시간이 없으므로, 결정이 8/15를 넘기면 최소 대응(웹 탈퇴 요청 페이지 + 앱 진입점)으로 고정합니다. **이 폴백 규칙은 이 절이 정본이며 계획 문서가 참조합니다.**
...
| 항목 | 값 | 비고 |
| --- | --- | --- |
| 최소 OS | **Android 8.0(API 26)** | 빌드의 `minSdk`도 26으로 맞춥니다(아래 빌드·배포 구성). 결정 시점 판단은 국내 API 26 미만 활성 기기 비중이 낮다는 것이며, 실제 비중은 출시 전 Play Console 기기·버전 통계로 재확인합니다. 알림·백그라운드 관련 API 기준선도 단순해집니다 |
...
| 대안 | 채택 안 한 이유 |
| --- | --- |
| v1에 함께 도입 | 지금은 **효과를 측정할 대상이 없습니다** — 화면이 비어 있어 무엇이 얼마나 빨라지는지 근거를 만들 수 없습니다. 시작 경로(로그인·세션 판정·그래프 결정)가 v1 개발 내내 바뀌므로 지금 뜬 프로파일은 출시 전에 낡습니다. 테스터 옵트인 기한이 절대 기한으로 고정된 일정에([`android-v1-release-plan.md §AR-1`](../planning/android-v1-release-plan.md)) 기능이 아닌 최적화로 리스크를 더할 이유도 없습니다 |
...
**출시 일정·마일스톤·선행 작업은 [`android-v1-release-plan.md`](../planning/android-v1-release-plan.md)가 소유합니다.** 일정은 실행 상황에 따라 수시로 조정되는 반면 이 문서는 결정이 바뀔 때만 갱신되므로, 변경 주기를 분리했습니다. 이 문서는 **무엇을 어떤 기준으로 만드는지**(계약·구현 기준·검수 기준)만 정의하고, **언제 어떤 순서로 만드는지**는 계획 문서가 정의합니다.

일정 문서가 이 문서를 참조하는 지점은 다음과 같습니다 — 주차별 작업 항목이 각 절의 구현 기준을, W3·W4 완료 조건이 §3-3-3 매트릭스와 위 테스트 범위를, W4 완료 조건이 아래 Definition of Done을 가리킵니다.
...
- [ ] 테스터 인원·기간 요건을 충족했습니다([`android-v1-release-plan.md §AR-1`](../planning/android-v1-release-plan.md) 개인 개발자 계정 요건).
...
- **문서 동기화** — 이 문서의 계약에 영향을 주는 변경은 해당 절 갱신을 같은 Pull Request에 포함합니다. v1 범위가 바뀌면 [`android-v1-release-plan.md`](../planning/android-v1-release-plan.md)의 주차 배정도 함께 확인합니다. 개발 환경 설정 절차와 필요한 로컬 키 목록은 안드로이드 레포 `README`가 소유합니다.
