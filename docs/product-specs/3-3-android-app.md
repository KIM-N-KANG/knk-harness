...
**누적 턴 수는 표지 뱃지가 아니라 메타 줄이 말하고, 제작일이 그 옆에 붙습니다 — 앱 전용 차이.** 표지가 커지며 우하단 뱃지가 그림을 가리므로, 채팅 목록 카드처럼 오른쪽 끝 메타 줄에 누적 턴 수와 제작일을 둡니다. 제작일은 목록 응답 `createdAt`의 KST 기준 날짜(`YYYY-MM-DD` — [`3-1-client.md §3-1-7` 날짜 표시 기준](./3-1-client.md#날짜-표시-기준))이고 형식이 다르거나 값이 없으면 그 칩만 빠집니다 — 스토리 상세의 생성일과 같은 변환을 씁니다. 웹 카드에는 없는 표시입니다.
...
| FE-SCREEN-003 | 스토리 좋아요(KNK-1211) | Phase 2 | 확정 | 구현 | **스토리 상세 필수** | 구현 | 상세 하단 CTA 왼쪽의 좋아요 아이콘 버튼(`POST`·`DELETE /stories/{storyId}/like`)과 목록·상세 표지의 좋아요 수 뱃지 구현(§3-3-3 스토리 목록·스토리 상세). **`isOwner`가 참인 상세에는 버튼을 두지 않음.** 204 성공 뒤에만 상태와 수를 옮기고 실패는 토스트로 알림. **게스트 로그인 유도 시트는 비적용**(§3-3-1) | §3-3-3 |
