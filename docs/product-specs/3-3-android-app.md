...
| 단계 인디케이터 | 헤더 아래 가로로 긴 얇은 막대 3분절. 완료·현재 단계는 주 동작 색으로 채우고 나머지는 무채색 | 웹은 원형 노드 3개(완료 체크·현재 테두리·미도달 숫자)와 연결선 |
...
| FE-SCREEN-002 | 생성 퍼널 기본 | MVP | 확정 | 구현 | 간편 제작 필수 | 부분 구현 | 키워드 단계 골격(FAB 진입·헤더·인디케이터·카테고리 탭·CTA)만 구현, 키워드 입력·API 연동 잔여. 이탈 가드의 앱 대응 결정 필요 | §3-3-3 · §3-3-5 |
...
| --- | --- | --- | --- | --- | --- |
| FLOW-002 | 스토리 생성 | 002 · 005 | 결정 필요 | 부분 구현 | 이탈 가드·백그라운드 복귀 대응 |
