...
- **토큰 세션** — Keystore 암호화 저장, 선제 재발급(만료 여유 60초), 전역 단일 비행, 실패 분기 3종(401·403·네트워크), 로그아웃 시 세션 폐기와 `device_id` 재발급.
- **홈·채팅 목록** — 회원 서재 API 연동, 카드·썸네일·빈 상태·오류 상태, 하단 3탭과 탭 상태 보존, 당겨서 새로고침.
- **빌드 구성** — 빌드 타입별 `API_BASE_URL` 주입, 시크릿 주입, R8·리소스 축소, 백업 비활성, 평문 통신 금지.
