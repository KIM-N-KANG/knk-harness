...
**셸을 두르는 목적지는 탭 넷뿐입니다**(`MUST`). 스토리 상세·제작 퍼널·채팅 화면·공용 웹 문서·마이 하위 목적지(친구 초대·이프 내역·피드백·회원 탈퇴)는 헤더도 하단 탭도 없는 전체 화면이며, 웹의 `(story)`·`(chat)`·`(legal)`·`(my)` 그룹에 대응합니다. 목적지가 늘어날 때의 판정 기준은 **하단 탭으로 직접 갈 수 있는 목적지인가**이고, 그 밖의 목적지는 전체 화면입니다.
...
**이프 카드와 출석 보상** — 잔액·출석 여부의 정본은 프로필(`GET /auth/me`의 `creditBalance`·`attendedToday`)이고 화면 진입 시 한 번 다시 읽습니다. 출석 체크는 `POST /users/me/credits/attendance`(멱등 — 오늘 이미 받았으면 `rewarded=false` 200)이며 결과를 토스트로 안내합니다: 지급 "출석 체크 보상으로 {n} 이프를 받았어요" · 이미 출석 "오늘은 이미 출석 체크했어요" · 실패 "출석 체크에 실패했어요"(웹 토스트 문구와 동일). **지급 결과를 잔액에 직접 더하지 않고 프로필 재조회로 반영합니다**(`MUST`) — 잔액 정본을 화면이 복제하지 않습니다. `attendedToday`면 버튼은 "출석 완료" 비활성입니다. **출석 버튼 왼쪽에는 이프 내역으로 가는 "내역" 버튼을 둡니다** — 주 동작은 출석이라 내역은 배경·테두리만 있는 보조 버튼이고, 두 버튼의 높이·모서리·안쪽 여백은 같습니다(나란히 놓인 버튼의 무게 차이는 색으로만 냅니다).

**이프 내역(FE-SCREEN-008 이프 내역)** — 구성·문구·데이터는 공통 계약을 따르고, 아래는 앱에서만 정해지는 부분입니다. 잔액 상자는 친구 초대의 내 코드 카드와 **같은 상자**(중립 배경·카드 모서리·16dp 안쪽 여백)이고 그 아래 32dp 간격 뒤에 목록이 옵니다. **목록은 화면을 채우는 지연 목록이고 마지막 항목이 보이면 다음 커서를 잇습니다**(`MUST`) — "더 보기" 버튼을 두지 않습니다. 잔액은 화면이 보일 때 프로필을 다시 읽어 그립니다(위 이프 카드와 같은 규칙 — 잔액 정본을 화면이 복제하지 않습니다). 사유 라벨과 "삭제된 스토리"·빈 상태 문구는 `:core:ui` 문자열 리소스가 소유합니다.

**당겨서 새로고침은 빈 목록·조회 실패에서도 당길 수 있습니다**(2026-09-01 결정, 앱 전용). 홈·내 스토리·채팅 세 목록은 그 두 상태에서 당김을 막고 화면 복귀 갱신에 맡기지만(위 스토리 목록·채팅 목록 절), 이프 내역은 갈래가 다릅니다 — 목록이 비어도 위에 잔액 상자가 남아 **당길 대상이 있고**, 이프는 채팅·제작에서 계속 줄어들어 "지금 서버에 무엇이 있는지"를 화면을 나가지 않고 맞출 수단이 필요합니다. 당김은 잔액(프로필)과 첫 페이지를 함께 다시 읽고, 골격을 다시 깔지 않으며, 실패해도 보고 있던 목록을 지우지 않고 "새로고침하지 못했어요" 토스트로 알립니다(세 목록과 같은 문구·같은 처리). **명시적 요청이라 잔액 재조회의 5초 간격은 보지 않습니다.** 웹은 이 자리를 당김이 아니라 버튼으로 채울 수 있어 공통 계약으로 올리지 않았습니다.

**이프 내역은 구성 변경에서 이미 불러온 페이지와 스크롤 위치를 잃지 않습니다**(`MUST`, §3-3-5 구성 변경) — 커서 목록을 처음부터 다시 읽으면 사용자가 보던 자리가 사라지고, 같은 구간을 두 번 요청하게 됩니다.
...
**친구 초대(FE-SCREEN-008 친구 초대)** — 레이아웃·문구·간격은 웹(`manyak-web/src/features/my/invite`)을 그대로 옮깁니다. 내 코드 카드(조회 중 골격 · 실패 시 "초대 코드를 불러오지 못했어요"와 다시 시도하기 · 이번 달 보상 진행) · 복사·공유 버튼 쌍 · 받은 코드 등록 · 이용 안내 네 줄 순서입니다. 코드와 보상 진행은 `GET /users/me/invite` 이고 월 상한은 정책 수치라 응답이 싣는 값을 그대로 씁니다(화면이 갖지 않습니다). 등록은 `POST /users/me/invite/redeem` 이며 **신규 가입 안내 시트와 같은 실패 문구 계약을 공유합니다**(위 초대 코드 안내 문단).
...
| FE-SCREEN-008 | 이프 내역(KNK-1081) | Phase 2 | 확정 | 구현 | **이프 내역 필수** | 구현 | 마이 이프 카드의 "내역" 진입 버튼과 화면(잔액 상자 + 32dp 아래 커서 목록·사유 라벨·부호·만료일 표기·골격·빈 상태·재시도·당겨서 새로고침) 구현(§3-3-3 마이 · 이프 내역). 빈 목록·실패에서도 당길 수 있는 것이 세 목록과 갈리는 앱 전용 결정. 목록 끝에서 다음 커서를 잇고 다음 페이지 실패는 그 자리 재시도 버튼으로 받음. 구성 변경에서 불러온 페이지와 스크롤 위치 유지 확인. **앱이 웹보다 먼저 구현** | §3-3-3 · §3-3-5 |
