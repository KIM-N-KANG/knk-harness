...
| FE-SCREEN-013 | 제작·내 스토리 목록 | Phase 1 | 확정 | 구현 | 제작 탭 진입점 필수 · 목록 범위 결정 필요 | 부분 구현 | 제작 진입점·FAB·이어서 만들기 배너에 더해 내 스토리 목록(제목·한 줄 소개·장르 `+N` 뱃지 카드, 골격·재시도·빈 상태)과 카드 더보기 삭제 구현(§3-3-3 스토리 목록). 카드 삭제 진입은 앱 전용 — 웹은 상세 헤더. 화면 복귀 시 조용한 목록 갱신 구현. 페이징·당겨서 새로고침과 카드 탭 상세 진입 잔여 | §3-3-3 · §3-3-5 |
