...
**완성 직후 채팅 진입은 채팅 화면 구현까지 보류합니다.** 3-1 계약은 완성 후 채팅을 만들어 바로 채팅 화면으로 진입하지만, 앱은 채팅 화면(FE-SCREEN-005)이 미착수라 지금은 완성 토스트("스토리가 완성되었어요")를 띄우고 퍼널을 닫아 홈으로 복귀합니다. 채팅 화면이 구현되면 채팅 생성 호출과 채팅 화면 진입으로 교체합니다.
...
| FE-SCREEN-002 | 생성 퍼널 기본 | MVP | 확정 | 구현 | 간편 제작 필수 | 부분 구현 | 퍼널 3단계와 서버 연동 구현 — 태그 조회·스토리라인 생성·재생성·평가·스토리 완성 API, 생성·완성 로딩(타자기 문구·지연 힌트)과 실패 복귀 포함. 완성 직후 채팅 진입은 채팅 화면(005) 구현과 함께 잔여(지금은 완성 토스트 후 홈 복귀 — §3-3-3), 이탈 가드의 앱 대응 결정 필요 | §3-3-3 · §3-3-5 |
...
| --- | --- | --- | --- | --- | --- |
| FLOW-002 | 스토리 생성 | 002 · 005 | 결정 필요 | 부분 구현 | 완성 직후 채팅 진입(005 미착수 — §3-3-3) · 이탈 가드·백그라운드 복귀 대응 |
