...
| 로컬 저장(본인 리소스 캐시) | **Room** | 목록과 채팅 턴을 방·항목 단위로 조회하고 오래된 것부터 제거·전삭제해야 합니다. 목적은 오프라인 지원이 아니라 **TTI 단축**입니다(§3-3-4 로컬 캐시) |
...
graph TD
    ...
    subgraph DATA["Data 계층 (:core:data)"]
        ...
        DB["Room<br/>본인 리소스 캐시<br/>서재·채팅 목록 · 완료된 턴"]
    end
    C -->|Intent| VM
...
- **앱 전용 추가 항목** — 로그인 화면(앱 전용), 당겨서 새로고침(홈·채팅 목록), **로컬 캐시**(회원 서재·채팅 목록·채팅 방의 완료된 턴 — TTI 단축, §3-3-4).
...
**캐시 대상은 화면이 아니라 데이터의 출처로 정합니다**(`MUST`). 화면 기준으로 쓰면 그 화면에 새 데이터가 붙는 순간 규칙이 조용히 틀리기 때문입니다.

| 항목 | 규칙 |
| --- | --- |
| 대상 | **본인 소유 리소스**(`/users/me/*`)의 목록과 완료된 콘텐츠뿐입니다(`MUST`) — 회원 서재 스토리 목록(FE-SCREEN-001), 채팅 목록(FE-SCREEN-004), 채팅 방의 완료된 턴(FE-SCREEN-005) |
| 비대상 | **남이 만든 콘텐츠·공개 목록(스토리 피드), 크레딧 잔액, 스토리 상세**는 캐시하지 않습니다(`MUST NOT`) |
| 읽기 | 화면 진입 시 캐시를 **즉시 그리고** 동시에 조회를 보냅니다(`MUST`). 응답이 오면 교체합니다 |
| 쓰기 — 목록 | 조회가 성공하면 응답을 기록합니다(`MUST`) |
...
- **스토리 상세** — 본 엔딩 배지(`reachedEndings`)가 플레이로 변하는데, 캐시된 상세를 그리면 방금 본 엔딩의 배지가 빠진 채 표시됩니다. 상세는 목록에서 한 단계 들어간 곳이라 앱이 이미 warm이고 단건 조회라 가벼워 **TTI 이득은 작고 틀릴 거리는 있습니다.**
...
| 캐시 없음 · 조회 중 (첫 실행, 크레딧 잔액) | **스켈레톤** — 최종 레이아웃과 같은 크기를 차지합니다 |
| 캐시 없음 · 조회 실패 | 오류 상태 + 재시도 |

**스켈레톤은 화면 전체가 아니라 필드 단위로 적용합니다**(`MUST`). 홈에서 스토리 목록은 캐시에서 실제 카드를 그리고, 같은 화면의 크레딧 잔액만 스켈레톤이 자리를 지킵니다. 스피너나 빈칸을 쓰지 않는 이유는 **레이아웃 시프트**입니다 — 최종 크기와 다르면 값이 도착하는 순간 주변 요소가 밀립니다.
...
| 로컬 캐시 | 캐시 대상 판정(본인 리소스만), 쓰기 시점(목록은 조회 성공 시 · 채팅 턴은 `completed` 후 재조회 결과만), 서버 응답 통째 교체, 404 시 항목 삭제, 방 보관 상한 초과 시 오래된 방 제거, 로그아웃 전삭제(§3-3-4) |
...
- [ ] **캐시가 있는 홈·채팅 목록·채팅 방에 다시 들어가면 빈 화면이나 스피너가 보이지 않습니다**(TTI 검수 — §3-3-4). 갱신 중 표시가 함께 보이고, 홈의 크레딧 잔액은 스켈레톤이 자리를 지켰다가 값으로 교체되며 레이아웃이 흔들리지 않습니다.
