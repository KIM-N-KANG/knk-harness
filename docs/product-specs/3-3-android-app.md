...
- 전송 버튼 왼쪽 `spacing.compact`(8) 자리에 3-1 계약대로 턴 비용 `20 이프`를 `typography.body-small`·`textSubtle`로 표시합니다. 두 입력 모드와 세 전송 아이콘 상태가 같은 툴바를 쓰므로 자리도 그대로입니다. 앱은 로그인 필수라 게스트 비노출 분기는 없습니다.
...
**무료 충전 탭** — 중립 배경 상자에 "매일 출석하고 / 매일 700 이프 받으세요"를 두 줄 제목으로 두고, 그 아래 가로 전폭 강조색 버튼 "출석 하기"를 둡니다. 버튼 아래에는 작은 보조색 문구 "매일 오전 00시에 초기화돼요"와 "보상으로 받은 이프는 적립일로부터 30일 동안 사용할 수 있어요"를 오른쪽 정렬로 붙입니다. 상자에서 32dp 아래에는 **마이와 같은 친구 초대 진입 줄**(라벨 + 강조색 보조 문구 "하고 2,000 이프 받기")을 둬 무료 수단 둘을 한 자리에 모읍니다 — 마이의 메뉴 줄 컴포넌트를 그대로 재사용하고 새로 그리지 않습니다.
...
**친구 초대(FE-SCREEN-008 친구 초대)** — 마이와 이프 충전의 진입 줄은 "친구 초대" 라벨 아래에 작은 강조색 보조 문구 "하고 2,000 이프 받기"를 붙여 라벨과 이어 한 문장으로 읽히게 합니다(KNK-1093). 화면 자체의 레이아웃·문구·간격은 웹(`manyak-web/src/features/my/invite`)을 그대로 옮깁니다. 내 코드 카드(조회 중 골격 · 실패 시 "초대 코드를 불러오지 못했어요"와 다시 시도하기 · 이번 달 보상 진행) · 복사·공유 버튼 쌍 · 받은 코드 등록 · 이용 안내 네 줄 순서입니다. 코드와 보상 진행은 `GET /users/me/invite` 이고 월 상한은 정책 수치라 응답이 싣는 값을 그대로 씁니다(화면이 갖지 않습니다). 등록은 `POST /users/me/invite/redeem` 이며 **신규 가입 안내 시트와 같은 실패 문구 계약을 공유합니다**(위 초대 코드 안내 문단).

**결정 — 카카오 공유 대신 안드로이드 공유 시트를 씁니다(2026-08-31)**

- **배경.** 웹은 카카오 JS SDK 로 공유 카드(제목 "초대 코드 등록하고 2,000 이프 받기 🎁" · 설명 "초대 코드: {코드}")를 띄웁니다. 앱이 같은 카드를 띄우려면 카카오 공유 SDK 를 새로 실어야 합니다 — 지금 앱에 든 카카오 의존성은 로그인용(`v2-user`) 하나뿐입니다.
...
**서비스 안내(FE-SCREEN-011)** — 별도 네이티브 화면·모듈을 만들지 않고 약관·개인정보처리방침과 같은 웹 문서 화면(`:feature:legal`)으로 `{WEB_BASE_URL}/about`을 엽니다. 본문 정본이 웹 한 곳이라 이프 정책 수치가 갈라질 수 없습니다 — FE-SCREEN-010과 같은 논거입니다. 웹이 새 브라우저 탭으로 여는 것에 맞춰 메뉴 트레일링에 외부 열림 아이콘을 둡니다. 경로는 `/about` 으로 확정됐고(KNK-1043 — `dev` 에 병합된 [`3-1-client.md`](./3-1-client.md) FE-SCREEN-011), **배포만 남았습니다** — 배포 전까지 앱에서는 404 페이지가 보입니다. **웹 잔여** — 앱 WebView는 미로그인 상태로 페이지를 열므로, "게스트 이용 안내(웹 전용)" 섹션을 앱 노출에서 숨기는 판정(3-1 검수 기준)은 웹이 소유합니다.
