...
마이 탭 화면입니다([`3-1-client.md`](./3-1-client.md) FE-SCREEN-008 마이 페이지 회원 상태 구성). 앱은 회원만 진입하므로 게스트 로그인 버튼·이관 안내는 비적용입니다(§3-3-1). 섹션 구성은 웹과 같습니다 — 프로필 헤더 · 이프 카드 · 이벤트(친구 초대) · 화면(테마 변경) · 기타 · 계정(로그아웃·회원 탈퇴). **기타 섹션의 항목과 순서는 앱이 갈립니다** — 피드백 · 서비스 안내 · 오픈소스 라이선스 · 버전 정보 순이고, 웹은 서비스 안내가 피드백 위이며 뒤 둘이 없습니다. 오픈소스 라이선스와 버전 정보는 배포 형태가 있는 앱에만 필요한 항목이라 웹에 대응 화면이 없습니다.
...
- **앱 전용 항목(공통 계약 없음)** — 오픈소스 라이선스 고지·버전 정보(§3-3-3 마이), 푸시 토큰 등록·알림 권한(§3-3-4 푸시 토큰 등록). 웹에 대응 화면이 없어 위 표에 행을 두지 않고, 푸시는 아래 흐름 매트릭스 FLOW-010으로 추적합니다.
...
| FLOW-010 | 푸시 알림 | — (앱 전용) · 008 알림 설정 | **토큰 등록 → 수신·탭 진입 → 수신 동의 설정** | 부분 구현 | 토큰 등록·로그아웃 삭제·권한 요청은 KNK-1133 구현(§3-3-4 푸시 토큰 등록 — `notification` 모듈, dev 에뮬레이터에서 로그인·재시작 `PUT` 204·로그아웃 `DELETE` 204·권한 거부 확인), 수신 표시·채널·탭 진입은 KNK-1134(이전 계정 알림 차단 조건은 아래 확정, 수신자 검증 방식은 서버 합의 필요), 수신 동의 설정 화면은 KNK-1135 — 뒤 둘의 상세 스펙은 미작성 |
...
**알림 권한(Android 13+ `POST_NOTIFICATIONS`).** 메인 그래프가 `회원` 상태로 처음 그려질 때 프로세스당 한 번 시스템 권한 요청을 띄웁니다. 사전 설명 화면과 거부 뒤 재요청 UI는 두지 않습니다 — 시스템이 두 번 거부 뒤 자동 거부하므로 앱이 횟수를 저장할 필요가 없고, 이후 재진입은 알림 설정 화면(KNK-1135)의 시스템 설정 이동이 맡습니다. API 32 이하는 런타임 권한이 없어 요청하지 않습니다. **권한 거부는 등록·세션·화면 어느 것에도 영향을 주지 않습니다**(`MUST`).
