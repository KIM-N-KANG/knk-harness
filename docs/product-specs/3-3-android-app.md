...
| FLOW-010 | 푸시 알림 | — (앱 전용) · 008 알림 설정 | **토큰 등록 → 수신·탭 진입 → 수신 동의 설정** | 미착수 | 토큰 등록·권한 요청은 KNK-1133(§3-3-4 푸시 토큰 등록, 계획 확정), 수신 표시·채널·탭 진입은 KNK-1134(이전 계정 알림 차단 조건은 아래 확정, 수신자 검증 방식은 서버 합의 필요), 수신 동의 설정 화면은 KNK-1135 — 뒤 둘의 상세 스펙은 미작성 |
