...
§3-3-3  내비게이션·화면 구조 대응       (그래프·메인 탭 셸·제작 퍼널 구조·화면 전환·푸시 탭 진입 확정 · App Link 작성 예정)
...
| 402 응답의 게스트 분기(로그인 유도 다이얼로그) | 앱 사용자는 전원 회원이라 이프 부족 분기만 남습니다 |
| 게스트 체험 한도 선차단(로컬 카운터) | 위와 같음 |
| 채팅 상세 403(게스트-회원 교차 접근) 전용 안내 | 앱은 내 채팅 목록과 내 스토리 상세로만 방에 들어가고 임의 `chatId` 딥링크가 없어 도달할 수 없습니다. 일반 조회 실패 화면으로 처리합니다 |
...
- API 연동 방식과 인증 세션(토큰 보관·재발급·푸시 토큰 등록)
...
| -------------------------------------------------------------- | ----------------------------------------------------------------- |
| URL 경로·히스토리(`push`/`replace`)                            | 인증·메인 그래프 분리와 라우트 규칙·화면 전환 확정. 딥링크는 결정 필요 (§3-3-3) |
...
| 카카오톡 공유(웹: 카카오 JS SDK)                               | 안드로이드 공유 시트로 대체 — 확정 (§3-3-3 마이 · 친구 초대)      |
...
| 푸시 | **Firebase Cloud Messaging** | 서버 발송 모듈이 FCM HTTP v1이고([`4-backend.md §4-3-5`](./4-backend.md#4-3-api-계약)) Crashlytics와 같은 Firebase 프로젝트·BOM을 이미 쓰고 있어 추가 배선이 없습니다. 토큰 등록·삭제와 권한 요청 시점은 §3-3-4 푸시 토큰 등록이 소유합니다 |
...
- **잃는 것 — 딥링크 선언.** 목적지별 딥링크 선언 API가 없습니다. App Link를 도입할 때 URI를 `NavKey` 목록으로 바꾸는 매핑을 `:navigation`이 직접 소유해야 하고, 그 매핑 규칙은 딥링크 처리 방식을 확정할 때 함께 정합니다(§3-3-3).

### 계층과 책임

기능은 필요한 `entity`·`domain`·`data`·`presentation` 패키지를 소유합니다. 계층·공통 코드·의존 방향의 상세 규칙은 [Android 모듈 아키텍처 §3–4](../planning/android-module-architecture.md)가 소유합니다. 단순 위임 UseCase나 빈 계층은 만들지 않습니다.

Repository 계약과 구현은 분리합니다. ViewModel은 기능 또는 common의 domain 계약을 사용하고, 구현은 소유 기능의 data에 둡니다. 같은 Gradle 모듈 안의 패키지이므로 컴파일 가시성만으로 격리되지 않으며 Kotlin PSI 기반 경계 검사와 리뷰로 강제합니다. 화면은 상태를 그리고 Intent를 발행하며, reducer는 순수 함수이고 데이터 계층은 UI 문구를 모릅니다.

**결정 기록 — Clean Architecture를 전면 도입하지 않습니다(2026-08-22)**

- **배경.** 계층을 나누기로 한 이상 UseCase 계층과 계층별 모델 이중화까지 갈지가 갈림길입니다. 이 제품은 **도메인 규칙(이프 차감·체험 한도·소유권·멱등·엔딩 판정)이 전부 백엔드에 있습니다**([`4-backend.md`](./4-backend.md)).
...
기본 Hilt 컴포넌트는 앱 수명과 화면 수명(`SingletonComponent` · `ViewModelComponent`)이며, 제작 퍼널 공유 상태에는 기존 `ActivityRetainedComponent`를 사용합니다. **로그인 세션 수명의 커스텀 컴포넌트를 만들지 않습니다**(`MUST NOT`).
...
`SessionTerminationCoordinator`와 사용자 귀속 저장소의 중앙 등록은 composition root인 `app`이 소유합니다. 화면은 auth/domain의 세션 계약을 호출하고, app이 auth의 토큰·공급자, my의 프로필·초대 안내, create의 진행 레코드와 analytics의 식별 해제를 조합합니다. 종료 뒤에는 이동 명령 대신 세션 상태를 발행해 루트가 그래프를 교체합니다.
...
문자열은 실제 소유 모듈의 리소스에 둡니다. 기능 전용은 기능, 시각적 공용은 designsystem, 공통 오류·상태는 common, 앱 셸은 app입니다. 상세 소유 기준은 [모듈 아키텍처 §8](../planning/android-module-architecture.md#8-리소스분석빌드-경계)을 따릅니다.

DomainError·DomainResult는 common/domain이 소유하고, network/data의 apiCall이 네트워크·HTTP·파싱 실패를 공통 오류로 변환합니다. 기능 data는 필요한 업무 변환을 수행하며 common/presentation의 DomainErrorMessages와 각 화면이 문구를 결정합니다. domain/data는 화면 이동이나 문구를 요청하지 않으며, 루트의 인증 그래프 전환은 중앙 세션 상태를 따릅니다.

#### 오류 변환과 재시도

- **공통 통신 오류 변환 지점은 `:network`의 data입니다**(`MUST`). 네트워크·HTTP·파싱 실패를 여기서 오류 타입으로 바꾸고, 상위 계층은 예외가 아니라 결과 타입으로 받습니다.
...
모든 화면은 `:common`의 **`MviViewModel<I, S, E, F>` 베이스 클래스를 상속합니다**(`MUST`). 네 요소는 다음과 같습니다.
...
[`3-1-client.md §3-1-3`](./3-1-client.md#3-1-3-화면별-스펙)의 화면 인벤토리(FE-SCREEN-001~013)와 흐름 인덱스(FLOW-001~009)를 안드로이드 내비게이션 구조로 대응합니다. 그래프 구조·인증 게이트·메인 탭 셸·화면 전환은 아래에서 확정합니다. 외부 진입은 푸시 알림 탭만 확정됐고(§3-3-4 알림 수신·탭 진입 — `:navigation`의 `PushEntry`가 라우트 매핑을 소유), App Link·공유 URL은 FE-SCREEN-012 결정과 함께 다룹니다.
...
- **라우트 정의는 `:navigation` 한 곳에 등록합니다**(`MUST`). 화면 기능 모듈끼리 직접 참조하지 않으므로(§3-3-2) 이동은 이 모듈을 거칩니다.
...
백스택 조작이 화면에 어떻게 보이는지의 규칙입니다. 웹은 브라우저가 전환을 소유해 `push`/`replace` 표에 모양이 없으므로 이 절은 앱 전용입니다. 규칙은 **전환의 모양이 백스택 조작의 종류를 말해 준다**는 한 가지 원칙에서 나옵니다 — 쌓이면 밀려 들어오고, 걷히면 밀려 나가고, 고르면 바뀝니다.

| 백스택 조작 | 전환 | 적용 자리 |
| --- | --- | --- |
| 스택 위에 쌓기(`push`) | 새 화면이 오른쪽에서 폭 전체를 밀고 들어오고, 아래 화면은 폭의 1/4만 왼쪽으로 물러나며 어두워짐 | 상세·채팅방·마이 하위·공용 문서·퍼널 단계 사이 |
| 스택에서 걷기(`pop`·뒤로가기) | 위 규칙의 정확한 역방향 — 나가는 화면이 오른쪽으로 밀려 나가고 아래 화면이 왼쪽에서 제자리로 돌아옴 | 같음 |
| 예측형 뒤로가기(제스처) | `pop`과 같은 변환을 손가락 진행도로 되감음 | 같음 |
| 탭 고르기 | 교차 페이드 | 메인 탭 셸의 탭 전환과 탭 밑으로 떨어지는 뒤로가기 |
| 제작 퍼널 진입·이탈 | 퍼널이 아래에서 올라와 탭을 덮고, 덮인 탭은 제자리에서 어두워짐. 이탈은 역방향 | 제작 탭·초안 카드 → 퍼널, 퍼널 → 제작 탭 |

**쌓기와 걷기는 수평 밀기이고 서로 정확한 역방향입니다**(`MUST`). 뒤로가기가 진입의 되감기로 보여야 어느 화면 위에 있었는지 읽힙니다. **움직이는 화면은 불투명합니다**(`MUST`) — 밀기에 페이드를 섞으면 겹치는 동안 아래 화면이 비쳐 두 장이 한 장으로 뭉개집니다. 이를 위해 목적지마다 바탕색을 깔며, 덮이는 화면만 어두워졌다가 되살아납니다. 아래 화면이 폭 전체가 아니라 일부만 물러나는 것은 두 화면이 같은 속도로 흐르면 한 장의 띠처럼 보여 위아래가 사라지기 때문입니다. **예측형 뒤로가기는 같은 변환을 씁니다**(`MUST`) — 제스처를 놓기 전과 후에 모양이 갈리면 손가락을 따라오던 화면이 마지막에 다른 곳으로 튑니다. 라이브러리 기본값(축소·페이드)은 이 이유로 쓰지 않습니다.

**탭 전환은 방향이 없는 교차 페이드입니다**(`MUST`). 탭 전환은 이동이 아니라 어느 백스택을 그릴지 고르는 일이므로(위 메인 탭 셸) 밀기를 쓰면 없는 이력을 있는 것처럼 말하게 됩니다. 홈이 아닌 탭에서 뒤로가기로 홈에 떨어지는 것도 탭 전환이라 같은 페이드입니다.

**제작 퍼널은 밖에서 볼 때 한 덩어리로 덮이고 걷힙니다**(`MUST`). 퍼널은 닫기(X)로 이탈하는 모달 성격의 흐름이라(§3-3-5 이탈 가드) 옆으로 밀려 들어오는 상세와 다른 축을 씁니다. 세로 축은 "이 위에 잠시 덮인 것"을 말하고, 닫기가 아래로 내려가는 것과 짝이 맞습니다. **퍼널 단계 사이의 이동은 다른 스택 화면과 같은 수평 밀기입니다** — 키워드 → 스토리라인 교체와 스토리라인 ↔ 추가 정보 쌓기·걷기 모두 그렇습니다. 어느 축인지는 전환의 반대편 화면이 퍼널 단계인지로 가르며, 재개 진입처럼 단계를 한꺼번에 쌓아도 맨 위 단계 하나만 그려지므로 같은 규칙이 맞습니다.

**전환 길이는 이 문서가 정하지 않습니다.** 밀기와 페이드의 시간은 앱 레포 `DESIGN.md` 모션 토큰이 소유합니다.

**결정 기록 — 뒤로가기를 교차 페이드에서 수평 밀기로 바꿉니다(2026-09-03)**

- **배경.** 멘토 검수(KNK-1180)에서 뒤로가기 전환이 이동 방향을 말해 주지 않는다는 지적이 나왔습니다. 그때까지는 쌓기·걷기·탭 전환이 모두 같은 150ms 교차 페이드였고, 상세에서 채팅방으로 갔다가 돌아올 때 어느 쪽이 위였는지 화면이 말하지 않았습니다.

| 대안 | 채택 안 한 이유 |
| --- | --- |
| 교차 페이드 유지, 길이만 조정 | 방향이 없는 것이 문제라 길이로는 해결되지 않습니다 |
| 라이브러리 기본 예측형 전환(축소·페이드)을 일반 뒤로가기에도 적용 | 축소는 "위에 떠 있던 것이 사라진다"만 말하고 어디로 돌아가는지는 말하지 않습니다. 제스처 없이 버튼으로 눌렀을 때도 같은 모양이라 제스처 피드백이라는 본래 의미가 흐려집니다 |
| 퍼널도 수평 밀기로 통일 | 퍼널은 닫기(X)로 이탈하는 덮개라 옆 화면으로 밀려 들어오면 상세와 같은 층위로 읽힙니다. 닫기가 아래로 내려가야 X 버튼과 짝이 맞습니다 |

- **영향.** 메인·인증 백스택의 기본 전환이 밀기로 바뀌고 탭 셸만 페이드를 유지합니다. 퍼널 세 단계는 목적지별 전환 메타데이터로 축을 고릅니다. 앱 `DESIGN.md`에 밀기 길이 토큰이 추가됩니다.
...
**셸을 두르는 목적지는 탭 넷뿐입니다**(`MUST`). 스토리 상세·제작 퍼널·채팅 화면·공용 웹 문서·마이 하위 목적지(친구 초대·이프 충전·피드백·회원 탈퇴)는 헤더도 하단 탭도 없는 전체 화면이며, 웹의 `(story)`·`(chat)`·`(legal)`·`(my)` 그룹에 대응합니다. 목적지가 늘어날 때의 판정 기준은 **하단 탭으로 직접 갈 수 있는 목적지인가**이고, 그 밖의 목적지는 전체 화면입니다.
...
**백스택 조작은 연타에 견딥니다**(`MUST`). 화면 이동 콜백은 ViewModel 의 진행 중 잠금을 거치지 않고 백스택을 바로 고치므로, 카드나 버튼을 빠르게 두 번 누르면 같은 목적지가 두 번 쌓이고 뒤로가기를 두 번 누르면 백스택이 비어 화면 호스트가 죽습니다. 이를 콜백마다 막지 않고 백스택 조작 수단 자체가 거릅니다 — **쌓기는 같은 키가 이미 스택에 있으면 무시하고, 걷기는 마지막 하나(셸 또는 인증 시작 목적지)를 남깁니다**. 같은 키가 스택에 두 번 있어야 하는 흐름은 없습니다.

**비용이 있는 요청은 응답 직후 0.5초 동안 같은 탭을 버립니다**(`MUST`). 요청 중 잠금만으로는 응답이 온 직후의 연타가 같은 요청을 곧바로 다시 보냅니다. 스토리 좋아요(좋아요·취소가 번갈아 전송), 채팅 턴 재생성과 스토리라인 재생성(AI 호출과 이프 차감)이 대상이며, 화면은 응답 결과를 즉시 반영하고 버튼도 잠그지 않되 그 사이의 탭만 무시합니다. 이어쓰기 전송처럼 매번 새 입력을 싣는 요청은 대상이 아닙니다.
...
**탭 화면이 사는 모듈은 §3-3-2 모듈 구조를 따릅니다** — 홈은 `:home`, 채팅 목록은 `:chat`, 제작은 `:studio`, 마이는 `:my`이고 셸 자체는 `:app`이 소유합니다. `:designsystem`는 라우트를 알 수 없으므로(§3-3-2 의존 방향) 헤더와 탭 바를 값·콜백만 받는 표현 컴포넌트로 두고, 목적지와 탭의 대응은 `:app`이 맺습니다.
...
- **모듈 이름은 `:studio`입니다.** `:create`는 이미 FE-SCREEN-002 생성 퍼널이 쓰고 있고, feature 모듈은 화면 단위라는 §3-3-2 규칙상 한 이름을 두 화면이 나눠 가질 수 없습니다. 퍼널 쪽 이름을 바꾸는 대안은 이미 그 모듈에 쌓인 코드 전체의 패키지·네임스페이스를 건드리는 데 비해 얻는 것이 이름 하나뿐이라 택하지 않았습니다.
...
**홈 카드의 표지 뱃지는 좋아요 수·누적 턴 수 두 개이고 왼쪽부터 나란히 놓입니다.** 좋아요 수는 아웃라인 하트 아이콘과 함께 그리고, 0 이나 필드 누락도 0 으로 표시하며 천 단위 콤마를 씁니다 — 상세 히어로와 같은 표지 컴포넌트가 두 뱃지를 함께 그립니다(아래 스토리 상세 절).

**내 스토리 카드는 표지 뱃지가 아니라 메타 줄이 좋아요 수와 누적 턴 수를 말하고, 제작일이 그 옆에 붙습니다 — 앱 전용 차이.** 표지가 커지며 우하단 뱃지가 그림을 가리므로, 채팅 목록 카드처럼 오른쪽 끝 메타 줄에 좋아요 수·누적 턴 수·제작일을 이 순서로 둡니다. 좋아요 수는 아웃라인 하트 아이콘과 함께 턴 수 왼쪽에 놓고 0 이나 필드 누락도 0 으로 표시합니다 — 웹 카드의 메타 순서와 같습니다. 제작일은 목록 응답 `createdAt`의 KST 기준 날짜(`YYYY-MM-DD` — [`3-1-client.md §3-1-7` 날짜 표시 기준](./3-1-client.md#날짜-표시-기준))이고 형식이 다르거나 값이 없으면 그 칩만 빠집니다 — 스토리 상세의 생성일과 같은 변환을 씁니다. 웹 카드에는 없는 표시입니다.
...
- **영향.** 메뉴 항목은 당시 "삭제하기" 하나였습니다. 오리지널 카드에는 메뉴를 두지 않습니다 — 내 소유가 아니라 관리할 동작이 없습니다. 상세 헤더의 삭제는 아래 스토리 상세 절 결정 기록이 다룹니다.

**결정 기록 수정 — 카드 메뉴를 옵션 다이얼로그로 바꾸고 길게 누르기로도 엽니다(2026-09-03)**

- **배경.** 멘토 검수(KNK-1180)에서 목록 카드를 길게 눌렀을 때 아무 반응이 없는 것이 지적됐습니다. 위 2026-08-27 결정은 "이 앱에 길게 누르기 문법이 아직 없다"는 이유로 길게 누르기를 채택하지 않았는데, 이번에 채팅 카드와 내 스토리 카드에 같은 문법을 함께 들이므로 그 근거가 사라집니다.
- **결정.** 내 스토리 카드와 채팅 카드는 **제목 줄 오른쪽 더보기 버튼과 길게 누르기로 옵션 다이얼로그**를 엽니다(`MUST`). 발견 가능한 진입점 하나는 화면에 드러나 있어야 하고, 길게 누르기는 그 지름길입니다. 다이얼로그 상단에는 회색 상자 안에 그 카드와 같은 정보를 표지·서체·간격·뱃지 한 단계씩 작게 그린 미리보기를 두고, 그 아래 8dp 간격으로 **"신고하기"와 "삭제하기"** 두 항목이 있습니다. 신고 대상은 그 카드의 스토리(채팅 카드는 참조 스토리 `storyId`)이고 시트·전송 절차는 헤더 메뉴의 신고와 같습니다(아래 매트릭스 스토리 신고 행). "신고하기"·"삭제하기"를 누르면 옵션 다이얼로그가 먼저 닫히고 신고 시트나 확인 다이얼로그가 열립니다 — 겹쳐 뜨지 않습니다.
- **팝업이 아니라 다이얼로그인 이유.** 카드 위 팝업은 앵커가 손가락 아래라 열리는 순간 카드 내용을 가립니다. 길게 누르기로 여는 자리는 앵커가 정해져 있지 않아 팝업이 어디에 뜰지 예측되지 않습니다. 가운데 다이얼로그는 항상 같은 자리에 뜨고 어느 카드의 옵션인지 미리보기로 밝힙니다. 바텀 시트도 검토했으나 항목 둘에 미리보기 하나뿐인 내용에 시트 높이가 남아 다이얼로그로 정했습니다.
- **영향.** 다이얼로그와 더보기 버튼 컴포넌트는 `:designsystem`가 소유해 두 목록이 같은 것을 씁니다. 오리지널 카드에는 여전히 아무 진입도 없습니다. 채팅 목록의 삭제 진입에 관한 아래 채팅 목록 절의 `MUST NOT`은 이 결정으로 대체됩니다.
...
| 썸네일 히어로 | 3:4 비율. `thumbnailUrl`(원본)이 있으면 이미지, 없으면 placeholder 아이콘. 우하단에 좋아요 수(`likeCount`)·누적 턴 수 뱃지를 왼쪽부터 나란히(천 단위 콤마, 누락 값은 0). 목록 카드와 같은 표지 컴포넌트를 쓰되 축소본이 아니라 원본을 받음. **좌우 여백과 모서리 없이 화면 폭을 꽉 채우고 상태바·헤더 뒤까지 올라가며**, 아래 경계는 옅은 선 한 줄. 본문의 나머지 항목만 좌우 여백을 가짐 | 표지를 화면 폭 전체로 까는 것은 앱 전용 |
| 이미지 뷰어 | 이미지가 있을 때만 히어로를 누를 수 있고, 전체 화면 오버레이로 열림(아래 이미지 뷰어). 핀치·더블탭으로 확대 | 웹은 라이트박스. 뒤로가기 흡수 수단이 플랫폼별로 갈림. **확대는 앱 전용** |
| 제목·한 줄 소개 | 항상 표시 | 같음 |
...
| 주변 인물 | `characters[]` 의 이름과 이미지. 이미지는 채팅방 인물 이미지와 같은 컴포넌트(아래 주변 인물 문단) | 3-1 이 정한 구성 그대로 |
...
| 제작자·생성일 | 본문 마지막에 딸린 메타 정보라 화면 폭을 채우는 옅은 바탕 하나 위에 두 줄로 둠. 제작자는 `author.nickname`, 생성일은 `createdAt` 의 KST 기준 날짜 | 안쪽 여백·줄 간격 16dp 는 3-1 공통 계약. 바탕을 화면 폭까지 늘리는 것은 앱 전용 |
| 하단 CTA | 본문 위에 떠 있는 "채팅 시작하기" 버튼과 버튼 위쪽 본문 페이드. **응답 `isOwner`가 거짓인 상세에만** 그 왼쪽에 같은 높이(48dp)의 정사각 좋아요 아이콘 버튼을 16dp 띄워 둠 — 배경 없이 하트의 모양과 색만으로 상태를 말함(기본 아웃라인 + `{colors.text}`, 누른 뒤 filled + `{colors.text-danger}`) | 좋아요 버튼 배치는 웹과 같음. **게스트 로그인 유도 시트는 비적용** — 앱은 로그인 필수라 누를 수 있는 사용자가 곧 회원임(§3-3-1) |


**좋아요는 성공한 뒤에야 상태와 수를 옮깁니다**(`MUST`). `POST /stories/{storyId}/like`·`DELETE /stories/{storyId}/like` 는 204 로 끝나고 갱신된 수를 주지 않으므로, 화면이 들고 있던 수를 한 칸 올리거나 내리고(0 아래로 내려가지 않음) 다음 상세 조회가 서버 값으로 맞춥니다. **미리 옮겼다가 실패로 되돌리지 않습니다** — 눌린 하트가 깜빡였다 풀리는 편이 잠깐 늦게 칠해지는 것보다 나쁩니다. 요청 중에는 버튼을 잠가 중복 전송을 막고, **응답이 온 뒤에도 0.5초 동안은 탭을 버립니다** — 하트는 응답 즉시 바뀌지만 그 직후의 연타가 좋아요와 취소를 번갈아 서버로 보내는 것을 막기 위해서입니다. 실패하면 상태와 수를 그대로 둔 채 토스트("좋아요 변경에 실패했어요")로 알립니다. 접근성 이름은 상태에 따라 "좋아요"·"좋아요 취소"입니다.
...
**뷰어 안에서는 핀치와 더블탭으로 확대합니다**(`MUST`). 핀치는 1배에서 5배 사이로 배율을 바꾸고 확대된 상태에서는 한 손가락 드래그로 이동하되, 이미지가 화면 밖으로 빠져나가 검은 바탕만 남지 않도록 이동 범위를 뷰포트 안으로 자릅니다. 더블탭은 누른 지점을 중심으로 2.5배로 확대하고 다시 더블탭하면 원래 크기로 돌아옵니다. 확대 중에도 한 번 탭·닫기·뒤로가기는 그대로 뷰어를 닫습니다. **확대 배율과 위치는 뷰어가 열려 있는 동안만 삽니다** — 닫거나 화면이 재생성되면 원래 크기로 돌아가며, 다시 열었을 때 어디를 보고 있었는지 이어 줄 이유가 없어 열림 여부와 달리 ViewModel 에 올리지 않습니다. 웹 라이트박스에는 확대가 없습니다.
...
**주변 인물은 이름 하나에 이미지 한 장입니다.** 섹션 라벨은 주요 내용과 같은 단계이고, **인물 이름은 시작 상황의 갈래 제목과 같은 `bodyLargeStrong`** 입니다 — 무엇이 한 묶음인지 읽는 규칙을 섹션마다 새로 만들지 않습니다. 간격도 같은 자리에서 가져옵니다: 인물 사이는 갈래 사이와 같은 `section`(24dp), 이름과 이미지 사이는 갈래 안과 같은 `gutter`(16dp)입니다.

- **이미지가 없는 인물도 이름은 남깁니다**(`MUST`). 이미지 생성 실패는 스토리를 막지 않는 서버 계약이라([`4-backend.md §4-3-1`](./4-backend.md)) 이름만 있는 인물이 정상 응답이고, 그 인물을 통째로 빼면 등장인물 수가 화면에서 어긋납니다.
- **이미지는 채팅방과 같은 컴포넌트입니다.** 비율·모서리·테두리·맞춤·로드 실패 처리는 아래 인물 이미지 절의 표시 규칙 그대로이고, **URL 허용 검사도 같은 함수를 씁니다.** 두 번째 사용처가 생겼으므로 컴포넌트와 검사를 `:designsystem` 로 올렸습니다(§3-3-2 모듈 구조의 승격 규칙). 상세의 URL 은 AI 본문 마커가 아니라 서버 필드에서 오지만 검사를 건너뛰지 않습니다 — 두 경로가 같은 허용 목록을 쓰지 않으면 한쪽만 조용히 넓어집니다.
...
- **완성 직후 진입과 갈리는 이유(KNK-1222 이전 기록).** 퍼널 완성 직후 채팅 진입은 퍼널 단계를 걷어내고 `[메인 탭, 채팅방]`을 만듭니다(아래 생성 퍼널 절). 그 단계들은 한 번 쓰고 끝나는 자리라 돌아갈 대상이 아니지만, 상세는 언제든 다시 열 수 있는 상시 목적지입니다. 두 경로의 모양이 다른 것은 걷어내는 대상의 성격이 다르기 때문입니다.
...
**상세 응답의 `isOwner`가 참인 스토리는 헤더 메뉴에 "삭제하기"를 둡니다.** 그 밖의 상세에는 "신고하기"만 있습니다. 소유 판정은 서버의 몫이며(아래 2026-09-03 결정 기록), 삭제 절차·다이얼로그·토스트 문구는 제작 탭 카드와 같고 성공하면 진입한 목록으로 돌아갑니다. 돌아온 목록은 화면 복귀 갱신이 맞춥니다.
...
- **영향.** 당시 상세 헤더 오른쪽은 비어 있었고, 이후 스토리 신고(KNK-1111)로 "신고하기"만 담은 메뉴가 생겼습니다. 삭제는 아래 결정 기록으로 다시 엽니다.

**결정 기록 수정 — 상세 응답의 `isOwner`로 삭제 항목을 엽니다(2026-09-03)**

- **배경.** 멘토 검수(KNK-1180)에서 내가 만든 스토리의 상세에 삭제가 없는 것이 지적됐습니다. 위 2026-08-27 기록이 기다리던 서버 소유 여부는 KNK-1016·1018(2026-08-29)로 이미 구현돼 있었고([`4-backend.md §4-3-1`](./4-backend.md) — `author.nickname` 실 전달과 상세 `isOwner`), 문서만 `Phase 2 · 계획`으로 남아 있었습니다.
- **결정.** **상세는 응답의 `isOwner`가 참일 때만 헤더 메뉴에 "삭제하기"를 더합니다**(`MUST`). 클라이언트는 소유 여부를 따로 추정하지 않습니다 — `author.id`는 여전히 null이라 비교할 값이 없고, 진입 자리(제작 탭 카드)를 라우트에 실어 판정하는 대안은 서버 판정이 있는 이상 같은 사실을 두 곳에서 말하게 됩니다.
- **영향.** 삭제 성공은 상세를 걷어내고 진입한 목록으로 돌아가며, 목록은 복귀 갱신으로 맞춥니다. 홈 오리지널 상세는 `isOwner`가 거짓이라 신고만 남습니다. "수정" 항목은 여전히 미구현(FE-SCREEN-009)입니다.
...
- **영향.** 단계 간 공유 상태(생성 ID·선택 스토리라인)는 목적지 키에 식별자만 싣는 라우트 규칙을 따라야 하므로, 구체 보관 위치는 구현 시 `:create` 안에서 정합니다. 최초에는 시스템·헤더 뒤로가기를 곧 백스택 pop 으로 보고 추가 정보 → 스토리라인 복귀에 사용했지만, 이 동작은 아래 2026-08-26 닫기 결정으로 대체됐습니다.
...
**퍼널 화면의 chrome은 각 단계 화면이 공용 컴포넌트로 그립니다.** 셸이 없으므로 헤더·인디케이터를 화면이 소유하되, 단계마다 다시 만들지 않고 공용 컴포넌트를 씁니다. 퍼널 단계가 모두 `:create` 안에 있으므로 사용처가 그 모듈에 머무는 동안에는 컴포넌트도 그 모듈이 소유하고, 다른 화면이 쓰게 되면 `:designsystem`로 올립니다(§3-3-2 모듈 구조의 승격 규칙 그대로).
...
- **적용 범위.** 키워드·스토리라인·추가 정보 세 목적지의 헤더와 시스템 뒤로가기는 모두 퍼널 이탈입니다. 퍼널 진입·단계 이동·이탈·완성 제출 후 제작 탭 복귀는 별도 방향 전환을 두지 않고 앱의 기존 화면 교차 페이드를 사용합니다.
...
| 임시 저장 | 닫기 왼쪽에 높이 40dp 회색 버튼(`button-neutral` + 1dp `border` 테두리 — 퍼널 하단 "이전" 버튼과 같은 형태). 기본 라벨 "임시 저장", 쓰기 중에는 라벨 자리에 스피너를 겹침, 성공 뒤 2초 동안 `background-brand-subtle` 배경과 `text-brand` 아이콘·라벨(테두리 없음)로 16dp 체크 아이콘과 "임시 저장됨"을 보인 뒤 기본으로 복귀. 쓰기 중과 완료 표시 중에는 잠금. 마지막 저장 이후 바뀐 것이 없거나 진행 중 요청 레코드가 슬롯을 쥐고 있어도 잠금(응답을 못 받은 완성 실패 뒤는 예외 — §3-3-5 저장 시점) | 웹에는 아직 같은 버튼이 없음 |
...
**스토리라인 단계의 "선택한 키워드 보기"는 드로어가 아니라 모달 바텀 시트입니다.** 푸터 위·좌·우 여백 없이 프레임을 채우는 높이 40dp 의 각진 중립색 띠(`backgroundNeutral` 배경 · `textSubtle` 라벨)를 두고 "다시 만들기"·"선택하기" 행과 8dp 를 띄웁니다 — 띠의 형태는 [`3-1-client.md §3-1-4`](./3-1-client.md#3-1-4-스토리-생성-퍼널)의 웹 계약 그대로이고, 여는 대상만 앱의 시트 문법(신규 가입 초대 코드 시트와 같은 레이아웃)으로 바꿉니다. 이 단계가 키워드 목적지를 대체해 뒤로 돌아가 확인할 수 없으므로 시트는 읽기 전용이며, 고치는 수단은 "다시 만들기"뿐입니다. 내용은 "선택한 키워드들이에요 / 이 키워드들을 이용해서 스토리라인 세 개를 만들었어요" 아래에 장르 · 주인공 (나) · 주변 인물 순서로 24dp 간격의 묶음을 놓고, 키워드는 키워드 단계의 선택 상태 칩과 같은 모양으로 그립니다. 주변 인물이 둘 이상이면 키워드 단계와 같은 "주변 인물 N" 순번 라벨을 붙이고, 키워드를 하나도 고르지 않은 묶음은 내지 않습니다.

**키워드 이름은 제공 태그 목록으로 풉니다.** 생성 명령에는 태그 ID 만 실리고 재개 진입은 키워드 화면을 거치지 않아 이름표가 없습니다. 그래서 생성 스토어가 퍼널 수명 동안 태그 목록을 한 번만 들고(키워드 화면이 이미 받아 뒀으면 그 목록을 넘겨받아 조회를 생략), 시트를 열 때 ID 를 이름으로 바꿉니다. 조회가 실패하면 시트에는 "키워드를 불러오지 못했어요"와 다시 시도만 남습니다 — 직접 입력한 키워드만 부분으로 보이면 무엇이 빠졌는지 알 수 없습니다. 생성 명령조차 없는 상태(복원할 레코드 없이 결과가 사라진 경우)에서는 띠 자체를 그리지 않습니다.

**추가 정보 단계의 선택 스토리라인 박스는 첫 프레임부터 더보기·접기 토글을 그립니다.** 본문은 기본 한 줄로 접혀 있지만, overflow 측정 뒤 토글을 늦게 삽입하지 않아 화면 진입 직후 박스 높이가 늘어나지 않습니다. 하단 "다시 선택하기"는 입력한 자유 텍스트나 선택한 추천 정보가 없으면 바로 스토리라인 단계로 돌아갑니다. 하나라도 있으면 "스토리라인을 바꿀까요? / 입력한 추가 정보는 모두 사라져요" 다이얼로그를 표시하고, "닫기"는 현재 화면을 유지하며 "다시 선택하기"는 추가 정보·추천 선택·선택 스토리라인 순번을 지운 뒤 돌아갑니다. 파괴 동작 버튼은 `backgroundDangerSubtle` 배경과 `textDanger` 글자를 사용합니다. CTA 행 바로 위에는 3-1 계약대로 화면 폭을 채우는 `sizes.input` 높이의 `backgroundNeutral` 비용 행을 두어 왼쪽에 `textSubtle`로 "스토리 완성 비용", 오른쪽에 `text` 색의 `typography.body-medium-strong`로 `N 이프`를 표시하고(N은 서버 정책의 `storyCreationCost` — §3-3-4 이프 정책 수치 패칭), CTA 행과는 `spacing.compact` 간격을 둡니다. 이 행은 푸터의 일부라 IME가 열리면 CTA와 함께 콘텐츠로 돌아가 숨습니다.
...
**단계 간 공유 상태는 `:create`의 생성 스토어가 담습니다.** 위 결정 기록이 구현 시점으로 미룬 보관 위치의 결과입니다. 키워드 화면이 시작한 스토리라인 생성을 뒤 단계 화면들이 관찰하도록, Activity 보존 범위의 스토어가 생성 상태(진행·결과·실패)와 재생성 체인(직전 `requestId`)을 담고 각 단계 ViewModel이 읽습니다. 라우트에는 규칙대로 선택 스토리라인 순번만 실립니다. 스토리라인·완성 요청의 `requestId` 멱등 계약(실패 재시도·같은 페이로드 재시도의 재사용)은 3-1 그대로 따르고, **요청 영속·복구 조회·임시 저장도 구현되어 있습니다** — 프로세스 재시작·재개 진입으로 스토어가 비면 진행 레코드에서 복원해 복구 폴링에 합류합니다(§3-3-5 간편 제작 라이프사이클).

**완성 제출은 제작 탭으로 돌아갑니다(KNK-1222, 2026-09-08 — 아래 §3-3-5 KNK-1222 절이 정본).** 완성 버튼은 요청을 영속하고 앱 스코프 실행자에 넘긴 뒤 퍼널을 걷어내고 제작 탭을 선택하며, 서버 응답을 기다리거나 채팅을 만들지 않습니다. 결과는 제작 탭의 완성 중 카드가 받습니다. 아래 문단은 그 이전(PR #5 기준) 동작 기록입니다.

**(이전) 완성 직후 채팅 진입은 3-1 계약대로 채팅 생성 → 채팅방 진입이었습니다.** 완성 성공 시 완성 토스트("스토리가 완성되었어요")를 띄우고 `POST /chats {storyId}`로 채팅을 만들어 채팅방 목적지로 진입합니다(시작 설정 미지정 — 서버가 첫 시작 설정으로 폴백). 웹의 채팅 화면 `replace`는 **퍼널 단계를 모두 걷어내고 채팅방을 쌓는 것**으로 대응해 백스택이 `[메인 탭, 채팅방]`이 되고, 채팅방에서 뒤로가기는 퍼널 진입 전 선택돼 있던 제작 탭으로 복귀하고, 그때 내 스토리 목록이 다시 조회돼 방금 만든 스토리가 반영됩니다(위 스토리 목록 절) — [`3-1-client.md §3-1-5`](./3-1-client.md#3-1-5-채팅-플레이와-sse-스트리밍)의 "헤더 back은 `/chats`"는 진입 이력이 없을 수 있는 웹 대비이므로 앱은 백스택 pop이 그 역할을 대신합니다. 스토리 완성은 성공했는데 채팅 생성이 실패하면 추가 정보 화면으로 복귀해 완성 실패와 같은 토스트로 알리고, 재시도는 스토리 완성을 건너뛰고 채팅 생성만 재호출해 스토리를 중복 생성하지 않습니다(3-1 완성 재시도 분기). 채팅방 라우트에는 규칙대로 `chatId`만 실리고 화면이 `GET /chats/{chatId}`로 다시 얻습니다 — 웹의 상세 prefetch 대응입니다. 채팅방 화면은 상세 조회 렌더(제목·프롤로그·턴 이력)에 더해 입력 컴포저·SSE 턴 진행까지 구현돼 있습니다(아래 채팅방 절·매트릭스 005).

**완성 실패는 토스트로 알립니다**(`MUST`). 사유(이프 부족·그 밖의 실패)는 문구로만 갈리고, 복귀 지점은 언제나 입력 화면이며 재시도는 "스토리 완성하기"를 다시 누르는 것입니다. 채팅방의 이프 부족 안내와 같은 표현이라 같은 사유를 화면마다 다르게 알리지 않습니다(2026-08-30 결정 — CTA 위 인라인 문구에서 교체).

**완성 402는 이프 부족 문구까지만 대응합니다.** 앱은 로그인 필수라 402는 회원 이프 부족뿐입니다(§3-3-1 — 게스트 한도 분기 비적용). 웹의 이프 획득 유도 다이얼로그는 앱에 이프 화면·획득 수단이 없어 붙일 대상이 없으므로, 완성 실패 토스트를 "이프가 부족해요"로 구분해 안내하는 데서 멈추고 유도 UI는 이프 기능 도입과 함께 정합니다.
...
| 스토리 제목 | 1줄, 넘치면 말줄임. **참조 스토리가 삭제돼 제목이 비면 그 자리에 옅은 글자색으로 "삭제된 스토리예요"** (KNK-1093 — 웹 선행 KNK-1092) | 같음 |
...
**채팅 카드는 제목 줄 오른쪽 더보기 버튼과 길게 누르기로 신고·삭제 옵션 다이얼로그를 엽니다**(`MUST`, 2026-09-03 — 위 스토리 목록 절 결정 기록). 다이얼로그의 항목·절차·문구는 채팅방 옵션 메뉴와 같고(아래 채팅방 절), 신고 대상은 그 카드가 참조하는 스토리(`ChatSummary.storyId`)입니다. 삭제 성공은 재조회 없이 목록에서 그 카드만 뺍니다. 채팅방에서 삭제하고 돌아온 목록은 화면 복귀 갱신이 맞추므로, 목록이 삭제 사실을 따로 전달받는 경로는 여전히 만들지 않습니다. 종전 "채팅 삭제 진입점은 목록에 두지 않는다"(`MUST NOT`)는 이 결정으로 대체됩니다 — 그 근거였던 "채팅방이 이미 상시 목적지"라는 사실은 그대로지만, 길게 누르기가 목록 카드의 공통 문법이 되면서 채팅 카드만 빠질 이유가 없어졌습니다.
...
**화면은 `:chat`이 소유합니다.** 채팅 탭과 채팅방이 같은 모듈이고 모듈 이름은 탭 이름과 맞춥니다(§3-3-2 모듈 구조). 채팅 요약 모델과 목록 조회는 `chat/domain`의 `ChatRepository`에 더해 채팅 생성·상세와 한 계약에 둡니다.
...
- **영향.** `ChatRoomScreen`의 `UserBubble`·`ChatTranscript` 간격을 이 절 기준으로 고칩니다. 채팅 공유 열람(FE-SCREEN-012)도 같은 렌더를 써야 하므로 밴드 표현을 `:chat` 안에 두되 두 번째 사용처가 생길 때 `:designsystem`로 올립니다(§3-3-2 코드 규칙).
...
이 규칙은 `:designsystem`의 `parseTextSegments`·`storyAnnotatedString`에 이미 같은 정규식과 같은 스타일 규칙으로 구현돼 있습니다. **채팅방은 그것을 그대로 쓰고 별도 파서를 만들지 않습니다**(`MUST NOT`). 선택지 문구도 같은 파서를 통과시킵니다.
...
**URL 허용 검사는 렌더 직전에 한 번 더 합니다**(`MUST`). `https` · 사용자·비밀번호·포트 없음 · 호스트가 `cdn.manyak.app`·`dev-cdn.manyak.app` 중 하나 · 경로가 `/characters/generated/` 또는 `/characters/originals/`로 시작하고 그보다 길 것. 사용자 생성 스토리는 `generated`, 오리지널 스토리는 `originals` 경로를 씁니다(KNK-1055·1075). 하나라도 어긋나면 이미지 조각을 만들지 않습니다 — 저장 본문에서는 마커 줄이 평문으로 남으므로, 경로 하나가 빠지면 오리지널 스토리 채팅에 `[[URL]]` 글자가 그대로 보입니다.

**같은 검사와 아래 표시 규칙을 스토리 상세의 주변 인물 섹션도 씁니다**(위 스토리 상세 절). 컴포넌트와 검사 함수는 `:designsystem` 에 있고, 채팅은 이벤트·마커에서 얻은 URL 을, 상세는 서버 필드(`characters[].imageUrl`)를 넘깁니다.
...
**기본은 블럭 모드입니다**(`MUST`). 웹 기본값도 블럭이고, 선택은 계정이 아니라 **기기 단위**로 남깁니다 — 웹은 `localStorage`, 앱은 `chat/data`의 DataStore Preferences입니다. 저장에 실패해도 현재 세션의 선택은 유지하고 기본값으로 되돌리지 않습니다.
...
- **잠긴 입력창을 누르면 토스트로 이유를 알립니다**(2026-09-03 결정 — 멘토 검수 KNK-1180, 앱 전용). 문구는 "응답을 만드는 중이에요. 끝나면 입력할 수 있어요"이고 일반·블럭 모드의 입력 칸 모두에 적용됩니다. 잠긴 칸은 터치를 받지 않아 눌러도 아무 반응이 없었는데, 비활성 색만으로는 왜 안 되는지와 언제 되는지가 드러나지 않았습니다. 추가·삭제·전송 버튼은 비활성 표시가 곧 잠금이라 따로 알리지 않습니다.
- 전송 버튼 아이콘은 세 갈래입니다.
...
- 전송 버튼 왼쪽 `spacing.compact`(8) 자리에 3-1 계약대로 턴 비용 `N 이프`를 `typography.body-small`·`textSubtle`로 표시합니다(N은 서버 정책의 `chatTurnCost` — §3-3-4 이프 정책 수치 패칭). 두 입력 모드와 세 전송 아이콘 상태가 같은 툴바를 쓰므로 자리도 그대로입니다. 앱은 로그인 필수라 게스트 비노출 분기는 없습니다.
...
**선택지 생성은 응답 스트림과 분리된 별도 요청입니다**(`MUST`). `POST /chats/{chatId}/turns/{turnId}/choices`는 동기 JSON이고 이프를 쓰지 않습니다.
...
**추천 입력 토글은 기본 on이고 기기 단위로 저장합니다.** 입력 모드와 같은 저장소를 쓰고, 켜져 있으면 툴바 아이콘을 `colors.textBrand`로 물들여 상태를 드러냅니다. 끄면 선택지를 그리지도, 생성하지도 않습니다. **스위치 연타는 마지막 값으로 한 번만 생성합니다** — 스위치와 저장은 탭마다 바로 바꾸되, 켜졌을 때의 선택지 생성 요청은 손이 멈추고 0.5초 뒤에 한 번 보냅니다. 켜기·끄기를 오가는 동안 생성 요청이 그때마다 나가지 않게 하기 위해서입니다.
...
**전송 즉시 낙관적으로 그립니다** — 사용자 밴드를 붙이고 스트리밍 블록을 엽니다. 첫 표시 가능 이벤트(`token`·`character_image`) 전까지는 "작성 중" 자리표시자를 `colors.textSubtlest`로 두고, 웹과 같이 글자 위로 옅은 띠가 왼쪽에서 오른쪽으로 2초 주기로 지나가는 시머를 얹습니다 — 밝은 배경에서는 띠가 글자색의 20% 알파로 배경 쪽으로 옅어지고(웹 라이트 규칙 — 하이라이트가 별도 색이 아니라 `currentColor`의 알파 20%) **어두운 배경에서는 `colors.text`까지 밝아집니다**(어두운 배경에서 옅어지는 띠는 배경에 묻혀 보이지 않습니다). 낭독기에는 문구 대신 답변을 작성 중이라는 상태 설명을 라이브 리전으로 알립니다. 토큰이 오기 시작하면 누적해 부분 렌더합니다. **누적된 본문은 글자 단위 타자기로 드러냅니다**(앱 전용) — 앱은 토큰을 프레임 예산으로 배칭해 상태에 덩이로 쌓이는데, 도착분을 그대로 그리면 웹(토큰 도착 즉시 반영이라 글자 단위로 보임)과 달리 뭉텅뭉텅 나타납니다. 공개 속도는 청크 간격보다 긴 시정수로 밀린 분량을 이어 비우고, 새 배치가 끊기면 빠르게 소화해 확정 교체 시점에 한꺼번에 나타나는 점프가 없습니다.
...
**`error`·네트워크 실패는 전송하며 비운 입력을 컴포저로 되돌립니다**(`MUST`). 턴이 열리지 못한 실패라 사용자가 쓴 문장을 없앨 이유가 없고, 이프 부족(402)처럼 다시 누르는 것이 유일한 복구인 실패에서 문장을 다시 쓰게 만들면 안 됩니다. 되돌릴 때 채우기로 넣어 둔 추천 원문 기억도 함께 돌아가 출처 판정이 어긋나지 않습니다. **EOF로 끊긴 실패는 되돌리지 않습니다**(`MUST NOT`) — 서버가 이미 턴을 저장했을 수 있어 다시 읽은 목록과 컴포저에 같은 문장이 겹칩니다. 재생성 실패는 컴포저를 비운 적이 없어 되돌릴 것이 없습니다.
...
| 402 | 이프 부족 안내, 기존 본문 유지 |

#### 스크롤

- 진입 스크롤은 턴이 있으면 마지막 메시지, 턴이 0개면 프롤로그가 보이는 상단입니다.
- **전송한 턴을 뷰포트 상단에 앵커합니다** — 조각이 늘어도 읽던 자리가 끌려 내려가지 않습니다. 웹과 같이 목록 아래에 여백을 깔아 올라갈 자리를 만들고, 스트리밍이 끝나면 **사용자가 위로 스크롤해 생긴 여유만큼만** 그 여백을 줄여 화면을 움직이지 않고 회수합니다.
- **스트리밍 중 사용자가 스크롤을 옮겨도 앵커를 되잡지 않습니다**(`MUST NOT`). 목록이 픽셀이 아니라 (인덱스, 오프셋)으로 위치를 잡아 조각이 붙어도 앵커가 밀리지 않으므로, 웹이 재앵커 루프를 끊으려고 두는 제스처 감지가 앱에는 필요 없습니다. 되잡는 자리는 **키보드 등으로 뷰포트 높이가 바뀔 때** 하나뿐이고, 그때는 깔아 둔 자리가 한 화면에 모자라져 목록이 앵커를 놓습니다.
- **구성 변경도 앵커를 다시 잡지 않습니다**(`MUST NOT`). 깔아 둔 여백 높이와 "이 스트림은 이미 앵커했다"는 표식, 그 기준이 된 뷰포트 높이는 재생성을 견뎌야 합니다 — 목록만 (인덱스, 오프셋)으로 복원되고 여백이 사라지면 복원한 위치가 콘텐츠 끝에 걸려 전송 턴이 상단에서 밀립니다. 되잡기는 위 규칙 그대로 **복원한 기준 높이와 실제 뷰포트 높이가 다를 때만** 돕니다 — 회전은 이 조건에 걸려 다시 잡히고, 높이가 그대로인 다크 모드는 복원한 자리에 남습니다.
...
헤더는 높이 56dp에 좌우 `compact`(8), 왼쪽 뒤로가기 · 가운데 스토리 제목 한 줄 말줄임 · 오른쪽 옵션 메뉴(⋮)입니다. **웹 헤더에 있는 공유 버튼은 앱에 두지 않습니다**(아래 앱이 옮기지 않는 것).

**채팅방의 삭제 진입점은 헤더 옵션 메뉴입니다**(목록 카드의 길게 누르기 시트와 같은 항목 — 위 채팅 목록 절). 항목은 파괴적 표현의 "삭제하기" 하나이고, 확인 다이얼로그 "채팅을 삭제할까요?"를 거칩니다. 요청 중에는 확정 버튼을 잠급니다. `DELETE /chats/{chatId}`가 204면 "채팅이 삭제되었어요", **404도 성공으로 취급하고**(이미 지워진 채팅), 실패는 "채팅 삭제에 실패했어요"입니다.
...
- **채팅 공유(발급·열람)** — 웹은 헤더 공유 버튼으로 공유 URL을 만들고 그 URL을 여는 화면(FE-SCREEN-012)을 따로 두지만, **앱은 지금 범위에서 둘 다 만들지 않습니다**(2026-08-29 결정). 공유 URL을 앱 딥링크로 열지 웹으로 넘길지가 정해지지 않았고, 그 결정 없이 발급만 붙이면 앱에서 만든 링크를 앱이 열지 못합니다. 다시 다룰 때는 KNK-872와 함께 판단합니다.
...
마이 탭 화면입니다([`3-1-client.md`](./3-1-client.md) FE-SCREEN-008 마이 페이지 회원 상태 구성). 앱은 회원만 진입하므로 게스트 로그인 버튼·이관 안내는 비적용입니다(§3-3-1). 섹션 구성은 웹을 따르되 알림 섹션이 하나 더 있습니다 — 프로필 헤더 · 이프 카드 · 이벤트(친구 초대) · 화면(테마 변경) · **알림(알림 설정, 앱 전용 — KNK-1135)** · 기타 · 계정(로그아웃·회원 탈퇴). **기타 섹션의 항목과 순서는 앱이 갈립니다** — 피드백 · 서비스 안내 · 오픈소스 라이선스 · 버전 정보 순이고, 웹은 서비스 안내가 피드백 위이며 뒤 둘이 없습니다. 오픈소스 라이선스와 버전 정보는 배포 형태가 있는 앱에만 필요한 항목이라 웹에 대응 화면이 없습니다.

**프로필 헤더** — 아바타(56dp 원형)·닉네임·계정 연동 칩 행입니다. 아바타는 `GET /auth/me`의 인라인 썸네일(`profileThumbnailBase64`)을 우선 그리고, 없으면 `profileImageUrl`, 그것도 없으면 빈 원입니다. 칩 행은 공통 계약 FE-SCREEN-008을 그대로 따릅니다 — 연동된 provider는 실선 칩, 미연동 provider는 같은 모양의 점선 버튼("카카오 연동하기")이고, 누르면 확인 다이얼로그가 먼저 뜹니다(§3-3-4 계정 연동). **칩 치수는 웹 배지와 같습니다**(높이 24dp·로고 12dp) — 같은 정보를 두 플랫폼에서 나란히 보는 자리라 크기가 갈리면 다른 것으로 읽힙니다. `linkedProviders`가 비어 있으면(아직 실데이터가 아님) 행 자체를 그리지 않아 연동 버튼 둘만 뜨는 오해를 막습니다. 웹의 로딩 스켈레톤은 앱에 두지 않습니다 — 프로필이 로컬에 캐시돼 있어 첫 페인트에 이미 값이 있습니다.

**이프 카드**(KNK-1093 — 웹 선행 KNK-1092) — 잔액의 정본은 프로필(`GET /auth/me`의 `creditBalance`)이고 화면 진입 시 한 번 다시 읽습니다. **카드에는 잔액 오른쪽에 이프 충전으로 가는 강조색 버튼 "충전" 하나만 둡니다** — 이프를 얻는 수단은 모두 이프 충전 화면이 소유하므로 마이에는 그 화면으로 가는 입구만 남깁니다. 출석 체크 버튼과 "내역" 버튼은 카드에서 빼고 이프 충전으로 옮깁니다(아래 이프 충전 절).

**출석 보상** — 동작 계약은 그대로이고 자리만 이프 충전의 무료 충전 탭으로 옮깁니다. 출석 체크는 `POST /users/me/credits/attendance`(멱등 — 오늘 이미 받았으면 `rewarded=false` 200)이며 결과를 토스트로 안내합니다: 지급 "출석 체크 보상으로 {n} 이프를 받았어요" · 이미 출석 "오늘은 이미 출석 체크했어요" · 실패 "출석 체크에 실패했어요"(웹 토스트 문구와 동일). **지급 결과를 잔액에 직접 더하지 않고 프로필 재조회로 반영합니다**(`MUST`) — 잔액 정본을 화면이 복제하지 않습니다. `attendedToday`(프로필)면 버튼은 "출석 완료" 비활성입니다.

**이프 충전(FE-SCREEN-008 이프 충전)**(KNK-1093 — 웹 선행 KNK-1092) — 구성·문구·데이터는 공통 계약을 따르고, 아래는 앱에서만 정해지는 부분입니다. 기존 이프 내역 화면을 두 탭으로 넓힌 것이라 **새 목적지를 만들지 않고 같은 목적지를 고칩니다** — 진입 버튼만 "내역"에서 "충전"으로 바뀝니다.

**잔액 상자와 탭 줄은 화면 상단에 고정하고 탭 내용만 스크롤합니다**(`MUST`) — 잔액은 어느 탭에서 무엇을 하든 계속 보여야 하는 값이고, 스크롤을 따라 올려 보내면 출석하러 들어온 사용자가 지금 얼마인지 모르는 채로 버튼을 누릅니다. 탭은 무료 충전 · 내역 둘이고 기본값은 무료 충전이며, **퍼널 카테고리 탭과 같은 `SecondaryTabRow` 밑줄 탭**을 씁니다(표시선·리플 규칙은 앱 레포 `DESIGN.md`가 소유). 선택한 탭은 저장하지 않습니다 — 진입 경로가 마이의 "충전" 하나뿐이라 복원할 맥락이 없고, 구성 변경에서만 유지하면 됩니다(§3-3-5).

**잔액 상자** — 중립 배경·카드 모서리·16dp 안쪽 여백이고 "내 이프" 라벨이 위, 잔액이 그 아래 오른쪽에 한 단계 큰 글자로 옵니다. 잔액은 화면이 보일 때 프로필을 다시 읽어 그립니다(위 이프 카드와 같은 규칙 — 잔액 정본을 화면이 복제하지 않습니다).

**무료 충전 탭** — 중립 배경 상자에 "매일 출석하고 / 매일 N 이프 받으세요"를 두 줄 제목으로 두고(N은 서버 정책의 `attendanceReward` — §3-3-4 이프 정책 수치 패칭), 그 아래 가로 전폭 강조색 버튼 "출석 하기"를 둡니다. 버튼 아래에는 작은 보조색 문구 "매일 오전 00시에 초기화돼요"와 "보상으로 받은 이프는 적립일로부터 30일 동안 사용할 수 있어요"를 오른쪽 정렬로 붙입니다. 상자에서 32dp 아래에는 **마이와 같은 친구 초대 진입 줄**(라벨 + 강조색 보조 문구 "하고 N 이프 받기" — N은 `inviteReward`)을 둬 무료 수단 둘을 한 자리에 모읍니다 — 마이의 메뉴 줄 컴포넌트를 그대로 재사용하고 새로 그리지 않습니다.

**내역 탭** — 기존 이프 내역 목록을 그대로 옮깁니다. **화면을 채우는 지연 목록이고 마지막 항목이 보이면 다음 커서를 잇습니다**(`MUST`) — "더 보기" 버튼을 두지 않습니다. 목록 줄의 위아래 여백은 8dp이고 탭 내용의 위아래 여백도 8dp입니다(무료 충전 탭만 위 16dp). 사유 라벨과 "삭제된 스토리"·빈 상태 문구는 `:my` 문자열 리소스가 소유합니다.

**무료 충전 탭도 당겨서 새로고침할 수 있고, 당기면 프로필만 다시 읽습니다**(2026-09-03 결정 — 멘토 검수 KNK-1180, 앱 전용). 출석 여부는 자정에 서버에서 초기화되는데 화면을 켜 둔 채 자정을 넘기면 프로필이 어제 값이라 버튼이 "출석 완료"로 잠겨 있고, 화면을 나갔다 돌아오는 것 말고는 맞출 수단이 없었습니다. 당김은 `GET /auth/me` 하나만 다시 읽어 출석 여부와 잔액을 갱신하고(내역은 이 탭이 그리지 않아 읽지 않음), 내용이 화면보다 짧아도 당길 수 있으며, 실패는 내역 탭과 같은 "새로고침하지 못했어요" 토스트로 알립니다. 잔액 재조회의 5초 간격은 명시적 요청이라 보지 않습니다. 두 탭의 당김 표시자는 따로 돕니다 — 무료 충전 탭을 당겼는데 내역 탭 표시자가 도는 일이 없습니다.

**내역 탭의 당겨서 새로고침은 빈 목록·조회 실패에서도 당길 수 있습니다**(2026-09-01 결정, 앱 전용). 홈·내 스토리·채팅 세 목록은 그 두 상태에서 당김을 막고 화면 복귀 갱신에 맡기지만(위 스토리 목록·채팅 목록 절), 이프 충전의 내역 탭은 갈래가 다릅니다 — 목록이 비어도 위에 잔액 상자가 남아 **당길 대상이 있고**, 이프는 채팅·제작에서 계속 줄어들어 "지금 서버에 무엇이 있는지"를 화면을 나가지 않고 맞출 수단이 필요합니다. 당김은 잔액(프로필)과 첫 페이지를 함께 다시 읽고, 골격을 다시 깔지 않으며, 실패해도 보고 있던 목록을 지우지 않고 "새로고침하지 못했어요" 토스트로 알립니다(세 목록과 같은 문구·같은 처리). **명시적 요청이라 잔액 재조회의 5초 간격은 보지 않습니다.** 웹은 이 자리를 당김이 아니라 버튼으로 채울 수 있어 공통 계약으로 올리지 않았습니다.

**이프 충전은 구성 변경에서 선택한 탭과 내역 탭이 이미 불러온 페이지·스크롤 위치를 잃지 않습니다**(`MUST`, §3-3-5 구성 변경) — 커서 목록을 처음부터 다시 읽으면 사용자가 보던 자리가 사라지고, 같은 구간을 두 번 요청하게 됩니다.

**테마 변경** — 공통 계약(FE-SCREEN-008 테마 변경)대로 탭할 때마다 시스템 설정 → 라이트 모드 → 다크 모드 순환이고, 아이콘(모니터·해·달)과 현재 값 문구가 함께 바뀌며 즉시 적용됩니다. 선택값은 기기 DataStore에 저장하고 **사용자 귀속이 아니라 로그아웃 정리 대상이 아닙니다**(§3-3-4 로그아웃 절차 무관). 테마는 루트 컴포저블에서 한 번 적용하고 시스템 바 아이콘 밝기도 함께 맞춥니다 — edge-to-edge 기본값은 시스템 다크 모드를 따르므로, 앱이 테마를 강제하면 상태 바 시계·배터리가 배경에 묻히기 때문입니다.

**알림 설정(앱 전용)** — `Phase 3 · 구현`(KNK-1135). 화면 섹션 아래 **알림 섹션의 "알림 설정" 항목**(벨 아이콘·오른쪽 화살표)이 진입점이고, 셸 없이 마이 위에 쌓이는 전체 화면입니다(`detail-header` "알림 설정"). 웹에 대응 화면이 없어 공통 계약이 없고, 서버 계약은 [`4-backend.md §4-3-5`](./4-backend.md#4-3-api-계약) 푸시 수신 동의(`GET`·`PUT /users/me/push-settings`)가 소유합니다(dev Swagger 2026-09-08 대조). 코드는 `notification` 모듈의 `settings` 패키지가 소유합니다 — 수신 동의는 알림 기능의 일부이고, `my`에 두면 알림을 모르는 모듈이 알림 API를 갖게 됩니다([모듈 아키텍처 §5](../planning/android-module-architecture.md)).

- **토글 세 개, 순서는 서버 정책 표와 같습니다** — 서비스 알림 · 광고 알림 · 야간 광고 허용. 각 행은 라벨과 한 줄 설명(서비스: 스토리 완성 등 내 활동 결과, 광고: 이벤트·혜택 소식과 출석 리마인드, 야간: 밤 9시~아침 8시에도 광고 알림 수신)을 갖고, **행 전체가 토글 대상**입니다(체크박스 규칙과 같음 — 스위치 자체는 따로 누르는 대상이 아니고 접근성 이름도 행이 읽힙니다).
- **야간 토글은 광고 동의가 꺼져 있으면 비활성입니다**(`MUST`). 광고를 끄면 같은 요청에서 야간도 함께 `false`로 보냅니다(서버도 광고 철회 시 둘을 함께 지우므로 결과가 같고, 야간만 참인 요청은 400 `NIGHT_PUSH_REQUIRES_MARKETING`이라 보내지 않습니다). "야간을 켤 때 광고도 함께 켜기"는 택하지 않았습니다 — 야간은 별도 옵트인이라 한 탭으로 두 동의를 받는 셈이 됩니다.
- **저장은 토글 즉시 `PUT`이고 낙관 갱신입니다.** 탭한 순간 화면이 바뀌고, 성공 응답(갱신 후 상태)으로 상태를 교체합니다. 실패하면 **마지막으로 확인된 서버 값으로 되돌리고** 토스트 "알림 설정을 저장하지 못했어요"를 띄웁니다. 응답이 400 `NIGHT_PUSH_REQUIRES_MARKETING`이면 앱이 들고 있던 값이 서버와 어긋난 것이므로 되돌린 뒤 다시 조회합니다. 저장 버튼은 두지 않습니다 — 설정 셋을 모아서 확정할 이유가 없고, 되돌리기 규칙 하나로 실패가 덮입니다. 요청은 화면 ViewModel의 Intent 큐에서 순서대로 처리되어 앞 `PUT`이 끝나기 전의 탭도 버려지지 않습니다.
- **기기 알림이 꺼져 있으면 토글 위에 안내 배너를 둡니다** — 제작 탭 이어서 만들기 배너와 같은 중립 배경(`background-neutral`·`card` 모서리) 한 줄에 "기기 알림 설정이 꺼져 있어요"와 글자 버튼 "알림 켜기"를 둡니다. 버튼은 강조색이 아니라 본문 색입니다 — 켜라고 재촉하는 자리가 아니라 상태를 알리는 자리이고, 경고 배너를 쓰지 않는 것도 같은 이유입니다. 판정은 `areNotificationsEnabled`로, Android 13+ 권한 거부와 그 이하의 앱 알림 끔을 한 규칙으로 덮습니다. **버튼은 시스템 권한 다이얼로그를 먼저 띄웁니다** — 사용자가 켜려고 누른 자리라 §3-3-4의 "앱이 먼저 되묻지 않는다"와 어긋나지 않습니다. 두 번 거부해 시스템이 더 묻지 않는 상태에서는 다이얼로그 없이 거부 결과만 즉시 돌아오므로(요청 전후 모두 `shouldShowRequestPermissionRationale`가 거짓) 그때만 앱 알림 설정(`ACTION_APP_NOTIFICATION_SETTINGS`)으로 보내고, 다이얼로그에서 방금 거부한 사용자를 설정으로 밀어 넣지 않습니다. 런타임 권한이 없는 API 32 이하는 바로 설정으로 갑니다. 이 화면이 §3-3-4 푸시 토큰 등록의 "거부 뒤 재진입" 경로입니다. 배너는 허용 결과를 받거나 화면이 `RESUME`될 때마다 다시 판정합니다(설정에서 돌아오면 사라짐). **토글은 기기 알림 여부와 무관하게 조작할 수 있습니다** — 서버 동의와 기기 표시 허용은 별개 값입니다.
- **로딩·오류.** 라벨은 고정값이라 행을 그대로 그리고 스위치 자리에만 골격을 깝니다. 조회 실패는 본문 전체를 실패 자리(문구 + 다시 시도)로 바꾸며, 403(정지 계정)은 공통 정지 문구를 씁니다. 상태는 ViewModel이 들고 있어 구성 변경에서 토글 값과 진행 중 요청이 유지됩니다. 화면 관측 이벤트는 두지 않습니다(§3-3-6 화면 목록에 추가하지 않음).

**신규 가입 초대 코드 안내** — 첫 로그인이 신규 가입이면(`isNewUser`) 초대 코드 입력 바텀 시트를 띄웁니다. **로그인 화면이 아니라 회원 그래프 위에 얹습니다**(`MUST`) — 로그인 성공과 동시에 인증 백스택이 통째로 사라져(§3-3-3 내비게이션 구조) 로그인 화면에 두면 안내도 함께 걷힙니다. 그래서 `isNewUser` 를 그 자리에서 쓰지 않고 **사용자가 확인할 때까지 유지되는 앱 스코프 지속 상태**로 남깁니다(§3-3-2 UI 상태 모델 — 화면이 사라져도 관찰돼야 하는 결과는 `UiEffect` 가 아닙니다). 표시는 사용자 귀속이라 세션 종료 정리에 참여합니다(§3-3-4 로그아웃 절차).

닫기·스크림 탭·뒤로가기·끌어내리기가 모두 같은 처리(표시 소비)이고, 등록 중에는 넷을 모두 막습니다. **표시 지우기가 실패해도 시트는 닫습니다** — 화면 상태로도 닫아 두어 그 자리에서 다시 뜨지 않게 하고, 실패의 결과는 다음 실행에서 한 번 더 뜨는 것뿐입니다. 건너뛰어도 자격(계정당 1회)은 서버가 들고 있습니다.

**표현은 다이얼로그가 아니라 바텀 시트이고 버튼 배치도 웹과 갈립니다.** 공통 계약(FE-SCREEN-008 신규 가입 온보딩)은 "나중에 하기"·"등록하기" 두 버튼을 **같은 폭**으로 두지만, 앱은 등록하기를 전체 폭 주 버튼으로 두고 **그 아래 한 단 작은 "닫기"** 를 놓습니다 — 같은 폭으로 나란히 두면 둘의 무게가 같아 보여 이 시트에서 무엇이 다음 행동인지 흐려집니다. 문구도 혜택을 제목으로 올리고 안심 문구를 설명으로 내렸습니다(제목 "초대 코드를 등록하면 N 이프를 받을 수 있어요" — N은 서버 정책의 `inviteReward`(§3-3-4 이프 정책 수치 패칭) · 설명 "지금은 건너뛰고 나중에 등록해도 돼요") — 웹처럼 조건절을 겹쳐 쓰면 핵심이 문장 끝까지 밀립니다. 시트의 색·여백·모서리는 `DESIGN.md` 의 `sheet` 컴포넌트가 소유합니다.

등록은 `POST /users/me/invite/redeem` 이고 실패 사유를 입력창 아래 인라인으로 구분합니다 — 빈 값 · 400·404 "코드를 다시 확인해주세요" · 409 `INVITE_SELF_CODE` · 409 `INVITE_ALREADY_REDEEMED` (문구는 공통 계약 FE-SCREEN-008 과 같음). 성공은 토스트 뒤 **프로필 재조회로 잔액을 반영합니다**(`MUST`, 위 이프 카드와 같은 이유). **입력창에 자동 초점을 주지 않습니다**(`MUST NOT`) — 진입과 동시에 뜨는 시트라 키보드가 안내와 버튼을 가립니다(웹 KNK-715 와 같은 판단). 자판은 `Ascii` 로 유도하고 입력값에서 영문·숫자만 남깁니다 — 한글 IME 가 먼저 뜨는 기기가 있어 웹의 공백 제거·대문자화만으로는 코드에 없는 문자가 통과합니다(앱 전용 보강).

**가입 보상에는 전용 안내를 두지 않습니다**(2026-08-30 결정). 가입 시 지급되는 이프는 서버가 계정 생성과 함께 적립하므로, 앱은 이프 카드의 잔액으로만 드러냅니다 — 전용 안내를 따로 띄우면 잔액과 두 곳에서 같은 사실을 말하게 되고 중복 노출을 막을 상태를 하나 더 들어야 합니다. **신규 가입 직후 띄우는 것은 초대 코드 안내 하나뿐이며 웹도 같습니다.** 사용자가 직접 한 행동(출석·초대 코드 등록)의 결과만 토스트로 알립니다.

**친구 초대(FE-SCREEN-008 친구 초대)** — 마이와 이프 충전의 진입 줄은 "친구 초대" 라벨 아래에 작은 강조색 보조 문구 "하고 N 이프 받기"를 붙여 라벨과 이어 한 문장으로 읽히게 합니다(KNK-1093·1096 — N은 서버 정책의 `inviteReward`, §3-3-4 이프 정책 수치 패칭). 화면 자체의 레이아웃·문구·간격은 웹(`manyak-web/src/features/my/invite`)을 그대로 옮깁니다. 내 코드 카드(조회 중 골격 · 실패 시 "초대 코드를 불러오지 못했어요"와 다시 시도하기 · 이번 달 보상 진행) · 복사·공유 버튼 쌍 · 받은 코드 등록 · 이용 안내 네 줄 순서입니다. 코드와 보상 진행은 `GET /users/me/invite` 이고 월 상한은 정책 수치라 응답이 싣는 값을 그대로 씁니다(화면이 갖지 않습니다). 등록은 `POST /users/me/invite/redeem` 이며 **신규 가입 안내 시트와 같은 실패 문구 계약을 공유합니다**(위 초대 코드 안내 문단).

**결정 — 카카오 공유 대신 안드로이드 공유 시트를 씁니다(2026-08-31)**

- **배경.** 웹은 카카오 JS SDK 로 공유 카드(제목 "초대 코드 등록하고 N 이프 받기 🎁" — N은 서버 정책의 `inviteReward` · 설명 "초대 코드: {코드}")를 띄웁니다. 앱이 같은 카드를 띄우려면 카카오 공유 SDK 를 새로 실어야 합니다 — 지금 앱에 든 카카오 의존성은 로그인용(`v2-user`) 하나뿐입니다.
- **결정.** `Intent.ACTION_SEND` 시스템 공유 시트로 보냅니다. 카카오톡을 포함해 기기에 있는 앱을 사용자가 고르고, 공유 SDK 를 싣지 않습니다. 버튼 라벨도 "카카오톡 공유하기"가 아니라 **"공유하기"** 입니다 — 시트가 여러 앱을 내놓는데 한 앱 이름을 적으면 거짓말이 됩니다.
- **문구.** 웹 카드와 같은 제목·코드에 웹 홈 주소(`{WEB_BASE_URL}`)를 붙인 평문입니다. 아직 스토어 링크가 없어 웹으로 보내고, 웹이 앱과 같은 계정으로 이어지므로 초대받은 쪽이 어디서 열어도 코드를 등록할 수 있습니다.
- **영향.** 공유 카드의 썸네일·버튼은 앱에서 사라집니다. 스토어 배포 뒤 링크를 바꿀 때도 이 문단만 고치면 됩니다.

**복사 확인 토스트는 Android 13 미만에서만 띄웁니다**(앱 전용) — 13 부터는 시스템이 클립보드 복사 확인을 직접 띄워, 앱 토스트를 함께 내면 같은 말을 두 번 하게 됩니다.

**피드백(FE-SCREEN-006)** — 레이아웃·문구는 웹(`manyak-web/src/features/my/feedback`)과 같습니다. 본문(필수, 500자)과 답변 이메일(선택, 320자), 하단 고정 CTA 구성이며 성공 "소중한 피드백을 보내주셔서 감사해요" · 실패 "피드백 전송에 실패했어요" 토스트도 같습니다. 전송은 `POST /feedbacks` 이고 `platform: ANDROID` 고정에 더해 **`appVersion` 도 함께 보냅니다**(웹은 보내지 않습니다) — 어느 버전에서 온 피드백인지가 앱에서는 재현의 출발점입니다. 값은 `:app` 이 `BuildConfig` 에서 주입합니다. **전송에 실패하면 입력을 비우지 않습니다**(`MUST`) — 쓴 글이 사라지면 다시 쓸 방법이 없습니다. IME 가 열리면 CTA 를 접어 입력창이 키보드 위로 올라오게 합니다(제작 퍼널과 같은 처리 — §3-3-5).

**이메일 형식은 앱이 직접 검증합니다**(2026-08-31 결정). 웹은 `input[type=email]` 의 브라우저 기본 검증이 이 자리를 맡지만 앱에는 대응이 없어, 형식이 어긋난 주소가 그대로 서버까지 가서 `400` 으로 돌아왔고 화면에는 "피드백 전송에 실패했어요" 토스트만 떠 고칠 곳이 이메일 한 칸이라는 것을 알 수 없었습니다. 보내기 전에 형식(공백 없는 `로컬@도메인.최상위`)을 보고 그 칸 아래에서 "이메일 형식을 확인해주세요"로 안내합니다. 브라우저 검증과 같은 수준까지만 좁힙니다 — 실제로 쓰이는 주소를 앱이 막는 쪽이 서버가 거절하는 쪽보다 나쁩니다. 비어 있으면 선택 입력이므로 검사하지 않습니다.

**회원 탈퇴(FE-SCREEN-008 회원 탈퇴)** — 확인 네 항목·문구·CTA 상태는 웹([`3-1-client.md`](./3-1-client.md) FE-SCREEN-008 회원 탈퇴)과 같습니다. 체크박스만이 아니라 **줄 전체가 토글**이라 문구를 눌러도 켜지고, 최소 터치 타깃도 그 줄이 확보합니다. 넷을 모두 켜야 위험색 CTA 가 열립니다.

**탈퇴는 서버 삭제가 성공한 뒤에만 세션 종료를 시작합니다**(`MUST`). 순서가 반대면 삭제 요청에 붙일 토큰이 없습니다. 종료는 로그아웃과 **같은 흐름**을 탑니다(§3-3-4 로그아웃 절차) — 정리 범위가 경로마다 갈리면 안 되기 때문이고, 이미 무효해진 refresh 토큰의 서버 로그아웃이 실패해도 로컬 정리는 계속됩니다(3-1 계약과 같음). **삭제가 실패하면 세션을 그대로 둡니다** — 계정이 남았는데 기기에서만 로그아웃되면 안 됩니다. 성공을 따로 알리지 않습니다 — 세션 종료가 이 화면을 걷어내고 루트가 인증 그래프로 옮겨 가는 것이 결과입니다(웹의 게스트 마이 복귀에 대응).

**오픈소스 라이선스·버전 정보(앱 전용)** — 웹에 대응 화면이 없는 기타 섹션 항목 둘입니다. 버전 정보는 이동 없이 오른쪽에 값만 세웁니다.

**결정 — 오픈소스 고지 목록을 손으로 적지 않습니다(2026-08-31)**

- **배경.** 처음에는 주요 의존성을 손으로 적었고 전부 Apache-2.0 으로 표기했습니다. 실제 의존성 그래프를 수집해 보니 라이선스가 넷이었습니다 — Apache-2.0 외에 Google Play services·googleid 의 Android SDK License 와 DataStore 안 protobuf 의 BSD-3-Clause 가 있었습니다. 손으로 유지할 성격의 목록이 아닙니다.
- **결정.** AboutLibraries Gradle 플러그인이 빌드 때 `:app` 의 의존성 그래프에서 수집한 것을 읽습니다. 화면은 라이선스별로 묶어 이름을 나열합니다.
- **UI 는 앱이 직접 그립니다.** 라이브러리가 주는 Compose 화면은 M3 토큰을 직접 읽어 `DESIGN.md` 와 결이 다르고 한국어 문자열도 없습니다. 데이터(로더·모델)만 씁니다.
- **번들 폰트는 수동 항목입니다.** Pretendard·MaruBuri 는 Gradle 의존성이 아니라 함께 싣는 파일이라 자동 수집에 잡히지 않아 수집 설정에 항목을 따로 둡니다.
- **전문은 앱이 옮겨 적지 않습니다.** 수집된 본문이 있으면 앱 안에서 펼치고, 없으면 원문 주소를 엽니다. 전문은 한 글자도 바뀌면 안 됩니다.
- **비용.** 목록 자산이 APK 에 들어가고(현재 압축 후 13KB), 플러그인이 빌드 때 SPDX 전문을 받아옵니다 — 네트워크가 막힌 빌드에서는 `offlineMode` 로 끄되 전문이 빠집니다.

**계정 섹션 — 로그아웃은 기본색, 회원 탈퇴가 위험색입니다**(2026-08-30 결정). 앱은 로그아웃을 `{colors.text}` 기본색으로 두고 그 아래 회원 탈퇴 항목을 `{colors.text-danger}`로 둡니다 — 위험색은 되돌릴 수 없는 동작(탈퇴)에만 남깁니다. 로그아웃 진행 중에는 라벨을 "로그아웃하는 중"으로 바꾸고 스피너를 표시하며 항목을 잠급니다. **앱이 선행한 배치였고 공통 계약이 뒤따라 같은 배치를 담았습니다**(KNK-1052 — [`3-1-client.md`](./3-1-client.md) FE-SCREEN-008 마이 페이지 회원 상태 구성).

**서비스 안내(FE-SCREEN-011)** — 별도 네이티브 화면·모듈을 만들지 않고 약관·개인정보처리방침과 같은 웹 문서 화면(`:legal`)으로 `{WEB_BASE_URL}/about`을 엽니다. 본문 정본이 웹 한 곳이라 앱이 옮겨 적은 문구가 웹과 갈라질 수 없습니다 — FE-SCREEN-010과 같은 논거입니다. **이 화면은 더 이상 이프 수치를 고지하지 않습니다**(KNK-1095) — 수치를 모아 보여주는 자리를 두면 각 사용 지점과 이중으로 관리해야 해서 절 자체가 없어졌고, 수치는 각 소모·적립 지점이 서버 정책에서 직접 읽습니다(§3-3-4 이프 정책 수치 패칭). 웹이 새 브라우저 탭으로 여는 것에 맞춰 메뉴 트레일링에 외부 열림 아이콘을 둡니다. 경로는 `/about` 으로 확정됐고(KNK-1043 — `dev` 에 병합된 [`3-1-client.md`](./3-1-client.md) FE-SCREEN-011), **배포만 남았습니다** — 배포 전까지 앱에서는 404 페이지가 보입니다. **웹 잔여** — 앱 WebView는 미로그인 상태로 페이지를 열므로, "게스트 이용 안내(웹 전용)" 섹션을 앱 노출에서 숨기는 판정(3-1 검수 기준)은 웹이 소유합니다.

**결정 기록 — 웹 문서 화면의 경로 이탈 감지를 제거했습니다(2026-08-30)**

- **배경.** 초기 구현은 WebView의 방문 이력 콜백으로 SPA 경로 이탈을 감지해 화면을 닫았습니다 — 웹 문서 페이지의 헤더 뒤로가기가 SPA 이동이라, 문서 화면 안에 다른 제품 페이지가 열리는 것을 막기 위해서였습니다.
- **결정.** 웹이 약관·개인정보처리방침 페이지에서 뒤로가기 진입 UI를 제거하면서 감지할 이탈 경로가 사라져 앱 쪽 감지를 제거했습니다. 화면 닫기는 시스템 뒤로가기 하나이고, 같은 호스트 밖으로의 이동 차단은 유지합니다.
- **영향.** 문서 화면 상태에서 경로 추적이 빠지고, 같은 호스트 안 SPA 이동(예: 서비스 안내 → 약관 링크)은 화면 안에서 허용됩니다. 문서 헤더의 마냑 로고 홈 링크도 같은 호스트라 화면 안에서 열립니다 — 앱 웹뷰 노출에서 이를 조정할지는 웹 소유의 미결입니다.

### 플랫폼 구현 상태 매트릭스

> KNK-1197은 아래 제품 기능의 구현·미구현 상태를 변경하지 않습니다. 모듈 이전 완료와 최종 통합 검증 범위는 [Android 실행 기록](../../../manyak-android/docs/plans/module-reorganization.md)에 별도로 남깁니다.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| FE-SCREEN-001 | 홈 오리지널 스토리 목록 | Phase 1 | 확정 | 구현 | 홈 탭 진입점 필수 · 목록 범위 결정 필요 | 부분 구현 | 오리지널 목록 조회와 카드(ORIGINAL 태그·턴 수 뱃지·`@` 를 붙인 제작자 닉네임)·지연 골격·재시도·빈 목록 미노출·당겨서 새로고침 구현(§3-3-3 스토리 목록). 카드 탭 상세 진입 구현(§3-3-3 스토리 상세). 게스트 노출 분기는 비적용(§3-3-1) | §3-3-3 · §3-3-4 |
| FE-SCREEN-013 | 제작·내 스토리 목록 | Phase 1 | 확정 | 구현 | 제작 탭 진입점 필수 · 목록 범위 결정 필요 | 부분 구현 | 제작 진입점·FAB·이어서 만들기 배너에 더해 내 스토리 목록(표지 좌측 가로 행 카드 — 128dp 고정 표지·제목 2줄·한 줄 소개 2줄·장르 `+N` 뱃지·좋아요 수와 누적 턴 수와 제작일 메타 줄, 골격·재시도·빈 상태)과 제목 줄 더보기 삭제 구현(§3-3-3 스토리 목록). 행 카드 구성·섹션 제목 없음·제작일 표시는 앱 전용 차이. 카드 삭제 진입은 앱 전용 — 웹은 상세 헤더. 화면 복귀 시 조용한 목록 갱신·당겨서 새로고침 구현. 카드 탭 상세 진입 구현. 페이징 잔여. KNK-1222(2026-09-08) 구현 — 배너를 초안 카드로 교체, 완성 중·완료·실패 요청 카드 다중 표시, 탭 복귀·당겨서 새로고침의 요청별 상태 조회와 실제 카드 전환, 로컬 카드가 있으면 완성 스토리 0개·목록 실패에서도 목록·새로고침 유지(§3-3-5 KNK-1222 절). | §3-3-3 · §3-3-5 |
| FE-SCREEN-002 | 생성 퍼널 기본 | MVP | 확정 | 구현 | 간편 제작 필수 | 구현 | 퍼널 3단계와 서버 연동 구현 — 태그 조회·스토리라인 생성·재생성·평가·스토리 완성 API, 생성·완성 로딩(타자기 문구·지연 힌트)과 실패 복귀(완성 실패는 사유별 토스트), 닫기(X) 헤더·덮개 진입·이탈 전환(§3-3-3 화면 전환)·추가 정보 재선택 경고·완성 비용 행, 스토리라인 단계의 선택 키워드 시트, 이탈 가드(§3-3-5) 포함. 구성 변경에서 세 단계 입력·선택, 키워드 추가 시트, 스토리라인 읽던 위치, 생성 지연 안내 경과가 유지됨(§3-3-5 구성 변경). KNK-1222(2026-09-08) 구현 — 완성 버튼은 요청 영속 성공 후 제작 탭으로 복귀하고 완성 로딩 화면·자동 채팅 생성·채팅방 진입을 제거, 영속 실패는 미전송·입력 유지·토스트, 연타는 요청 하나(§3-3-5 KNK-1222 절). | §3-3-3 · §3-3-5 |
| FE-SCREEN-002 | 백그라운드 생성 복귀·제작 임시 저장 | Phase 1 | 확정 | 구현 | 간편 제작 필수 | 구현 | requestId 영속(Room 단일 행)·복구 3초 폴링·`STORY_DRAFT`·`KEYWORD_DRAFT` 구현. 저장 시점은 헤더 임시 저장 버튼과 Activity `onStop`(구성 변경 제외 — §3-3-5) 두 경로이고, 미저장 이탈에는 경고 다이얼로그가 붙습니다. 제작 탭 배너·재개 다이얼로그 구현(재개 다이얼로그는 "닫기·새로 만들기" 두 갈래이고 이어가기는 배너가 소유 — §3-3-5). 앱 전용 차이 — 임시 저장 토스트와 카드 닫기(X)는 두지 않음(§3-3-5). KNK-1222(2026-09-08) 구현 — 편집 슬롯 1개 + `story_completion_request` requestId별 테이블(DB v2 보존 마이그레이션, 파괴적 폴백 제거), 앱 스코프 완성 실행자와 `SessionGate` 커밋, 두 저장소의 로그아웃 정리, 완성 복구는 탭 복귀·새로고침 조회로 전환(§3-3-5 KNK-1222 절). | §3-3-5 |
...
| FE-SCREEN-003 | 스토리 상세 기본 | MVP | 확정 | 구현 | **스토리 상세 필수** | 구현 | 목적지·본문 구성·지연 골격·인라인 오류·채팅 시작(`POST /chats`) 구현(§3-3-3 스토리 상세). 채팅 진입은 웹 `replace`와 달리 상세를 남겨 뒤로가기가 상세로 복귀하고, 상세는 화면이 보일 때마다 조용히 다시 읽음. 404는 재시도 없는 안내로 구분. 구성 변경에서는 진행 중인 채팅 시작 잠금과 읽지 않은 실패 안내를 걷지 않음(§3-3-5 구성 변경). 헤더 오른쪽 더보기 메뉴에는 신고하기가 있고, **응답 `isOwner`가 참인 상세에만 삭제하기가 더해짐**(§3-3-3 스토리 상세 2026-09-03 결정 기록) | §3-3-3 |
...
| FE-SCREEN-003 | 스토리 좋아요(KNK-1211) | Phase 2 | 확정 | 구현 | **스토리 상세 필수** | 구현 | 상세 하단 CTA 왼쪽의 좋아요 아이콘 버튼(`POST`·`DELETE /stories/{storyId}/like`)과 홈 카드·상세 히어로 표지의 좋아요 수 뱃지, 내 스토리 카드 메타 줄의 좋아요 수 칩 구현(§3-3-3 스토리 목록·스토리 상세). **`isOwner`가 참인 상세에는 버튼을 두지 않음.** 204 성공 뒤에만 상태와 수를 옮기고 실패는 토스트로 알림. **게스트 로그인 유도 시트는 비적용**(§3-3-1) | §3-3-3 |
...
| FE-SCREEN-003·005 | 스토리 신고 | Phase 2 | 확정 | 미구현 | **스토어 심사 필수** | 구현 | 두 화면 모두 헤더 오른쪽 더보기 메뉴의 "신고하기"로 같은 시트를 염(사유 3개·상세 500자). **앱 전용 진입 추가(KNK-1180)** — 내 스토리 카드·채팅 카드의 옵션 다이얼로그(더보기·길게 누르기)에도 "신고하기"가 있어 같은 시트를 열며, 채팅 카드의 대상은 `ChatSummary.storyId`. 메뉴 컴포넌트는 `:designsystem` 가 소유해 채팅방과 상세가 같은 팝업을 씀. 채팅방에서 열어도 대상은 그 채팅이 참조하는 스토리(`ChatDetail.storyId`). 전송은 `POST /stories/{storyId}/reports`이고 성공하면 시트를 닫고 실패하면 입력을 남김. 시트·상태·전송 절차는 `:report` 가 소유해 두 화면이 공유하고, 각 ViewModel 은 상태 한 덩어리와 Intent 한 갈래만 가짐. Google Play 생성형 AI 정책의 인앱 신고 요건을 겸함. 앱이 웹보다 먼저 구현 | §3-3-3 |
| FE-SCREEN-003 | 주변 인물 목록(KNK-1058) | Phase 2 | 확정 | 구현 | **스토리 상세 필수** | 구현 | 상세 응답 `characters[]` 의 이름과 이미지를 주요 내용 아래 섹션으로 그림. 이미지는 채팅 인물 이미지와 같은 컴포넌트·URL 허용 검사이고 둘 다 `:designsystem` 로 승격. 이미지가 없는 인물은 이름만 남김(§3-3-3 스토리 상세 — 주변 인물) | §3-3-3 |
| FE-SCREEN-003 | 제작자·생성일 메타 블록(KNK-1079) | Phase 2 | 확정 | 구현 | **스토리 상세 필수** | 구현 | 본문 끝 바탕 하나에 제작자(`author.nickname`)와 생성일 두 줄. 안쪽 여백·줄 간격 16dp 는 3-1 공통 계약이고, 바탕을 화면 폭까지 늘리는 것만 앱 전용. `author.id` 는 여전히 placeholder 라 소유 여부 판정에는 쓰지 않음(위 헤더 옵션 메뉴 결정 기록) | §3-3-3 |
| FE-SCREEN-004 | 채팅 목록·이어가기 기본 | MVP | 확정 | 구현 | **채팅 목록·이어가기 필수** | 구현 | 목록 조회·카드·지연 골격·재시도·빈 상태·화면 복귀 갱신·당겨서 새로고침과 카드 탭 채팅방 진입 구현(§3-3-3 채팅 목록). 데이터 소스는 회원 서재(`GET /users/me/chats`) 하나이고 웹의 게스트 로컬 ID 배치 조회는 비적용(§3-3-1). **빈 상태를 스토리 보유 여부로 가르지 않고 제작 탭으로 탭을 바꾸는 것이 앱 전용 차이**. 삭제는 웹과 같이 채팅방 옵션 메뉴 하나라 목록에 두지 않음(FE-SCREEN-005 구현). 페이징은 서버 계약에 커서·오프셋이 없어 웹과 공통으로 없으며, 열리면 내 스토리 목록과 함께 붙임 | §3-3-3 |
...
| FE-SCREEN-005 | 채팅 기본(SSE 스트리밍·선택지·삭제) | MVP | 확정 | 구현 | **채팅 플레이 필수** | 구현 | 채팅 생성·상세 조회·채팅방 진입·조회 렌더(제목·프롤로그·턴 이력)에 더해 입력 컴포저(블럭·일반 두 모드)·SSE 턴 진행(낙관적 렌더·"작성 중" 시머·글자 단위 타자기·전송 턴 상단 앵커·원자적 확정 교체)·추천 입력·1회성 힌트·턴 비용 표시·이프 부족(402) 안내·실패 시 입력 복원·삭제까지 구현 — 구조·색·간격·파싱 규칙은 §3-3-3 채팅방 절이 정본. **사용자 입력을 오른쪽 정렬 말풍선이 아니라 전체 폭 밴드로 그리는 것이 앱의 확정 사항**(웹과 같음 — 먼저 만든 말풍선은 교체 대상). Enter 전송·헤더 show/hide·게스트 분기는 비적용 | §3-3-2 · §3-3-3 |
| FE-SCREEN-005 | 응답 재생성(US-6-10) | Phase 1 | 확정 | 구현 | **채팅 플레이 필수** | 구현 | 표시 조건 넷(마지막 턴·`id`·`aiOutput`·엔딩 아님)과 `error`·EOF·409·402 네 실패 분기의 복구까지 구현(§3-3-3 채팅방) | §3-3-3 |
| FE-SCREEN-005 | 주요 사건 방향 선택지 | Phase 1 | 확정 | 구현 | **채팅 플레이 필수** | 구현 | 생성이 응답 스트림과 분리된 별도 요청이고 렌더는 상세 재조회의 `turns[].choices`. 즉시 전송·채우기 두 동작, 채우기 시 초안 덮어쓰기 확인, `userSource`·`sourceTurnId`·`choiceOrder` 전달까지 구현(§3-3-3 채팅방) | §3-3-3 |
| FE-SCREEN-005 | 채팅 공유 발급 | Phase 1 | 확정 | 구현 | **비적용** | 비적용 | 공유 URL을 앱 딥링크로 열지 웹으로 넘길지가 정해지지 않아 발급만 붙이면 앱이 만든 링크를 앱이 열지 못합니다. 헤더에서 공유 버튼을 빼고 열람(012)과 함께 다시 판단합니다(§3-3-3 앱이 옮기지 않는 것) | §3-3-3 |
...
| FE-SCREEN-005 | 채팅 인물 이미지(US-6-11) | Phase 2 | 확정 | 구현 | **채팅 플레이 필수** | 구현 | 스트리밍 `character_image` 이벤트와 저장 본문의 URL 마커 두 경로를 한 조각 목록으로 환원. 마커 인정 조건 셋·허용 CDN 검사·4:3·로드 실패 시 자리 제거·이미지 앞 간격 가산까지 구현(§3-3-3 채팅방) | §3-3-3 |
| FE-SCREEN-005 | 엔딩 도달 턴 배지(US-6-13) | Phase 1 | 확정 | 구현 | **채팅 플레이 필수** | 구현 | 웹 구현 상태를 `미구현`에서 `구현`으로 정정(웹 채팅 화면이 `reachedEnding` 배지를 그림 — [`3-1-client.md §3-1-5`](./3-1-client.md#3-1-5-채팅-플레이와-sse-스트리밍)와 일치). 앱은 값 유무로만 렌더하고 자리·색은 §3-3-3 채팅방 | §3-3-3 |
| FE-SCREEN-005 | 엔딩 도달 후 계속 진행(US-6-14) | Phase 1 | 확정 | 구현 | **채팅 플레이 필수** | 구현 | 입력 잠금 조건이 스트리밍 여부 하나뿐이라 충족(§3-3-3 채팅방 전송 조건과 잠금) | §3-3-3 |
| FE-SCREEN-006 | 피드백 | MVP | 확정 | 구현 | 결정 필요 | 구현 | 본문·선택 이메일 폼과 전송 구현(§3-3-3 마이 · 피드백). `platform: ANDROID` 고정에 더해 `appVersion` 을 함께 보내는 것과 이메일 형식을 앱이 직접 검증하는 것이 앱 전용 차이 — 웹은 브라우저 기본 검증이 그 자리를 맡음. 전송 실패 시 입력 유지 | §3-3-1 · §3-3-3 |
| FE-SCREEN-007 | 온보딩 | MVP | 확정 | 구현 | **비적용** | — | 로그인 화면이 첫 실행 경험을 대체(§3-3-1) | §3-3-1 |
| FE-SCREEN-008 | 인증·마이 기본 | Phase 1 | 확정 | 구현 | **인증 출시 필수** | 부분 구현 | 로그인·토큰 보관·재발급·로그아웃에 더해 마이 화면 구성과 신규 가입 초대 코드 안내, **친구 초대 화면**(코드 조회·복사·공유·받은 코드 등록·이용 안내)과 **회원 탈퇴**(확인 네 항목 → `DELETE /users/me` → 로그아웃과 같은 종료 흐름), **계정 연동**(연동 칩·점선 연동 버튼 → 확인 다이얼로그 → 현재 제공자 재인증 → `POST /auth/links/reauth` → 대상 제공자 인증 → `POST /auth/links/{provider}` → 프로필 재조회, 409 두 종류를 토스트와 다이얼로그로 구분) 구현(§3-3-3 마이 · §3-3-4 계정 연동). 로그아웃 기본색·회원 탈퇴 위험색 배치는 앱이 선행. 카카오 공유는 시스템 공유 시트로 대체(앱 전용 확정). **402 다이얼로그가 잔여**. 이관·게스트 분기는 비적용(§3-3-1) | §3-3-3 · §3-3-4 |
| FE-SCREEN-008 | 이프 내역(KNK-1081) | Phase 2 | 확정 | 구현 | **이프 내역 필수** | 구현 | 마이 이프 카드의 "내역" 진입 버튼과 화면(잔액 상자 + 32dp 아래 커서 목록·사유 라벨·부호·만료일 표기·골격·빈 상태·재시도·당겨서 새로고침) 구현(§3-3-3 마이 · 이프 충전). 빈 목록·실패에서도 당길 수 있는 것이 세 목록과 갈리는 앱 전용 결정. 목록 끝에서 다음 커서를 잇고 다음 페이지 실패는 그 자리 재시도 버튼으로 받음. 구성 변경에서 불러온 페이지와 스크롤 위치 유지 확인. **앱이 웹보다 먼저 구현** | §3-3-3 · §3-3-5 |
| FE-SCREEN-008 | 이프 충전 개편(KNK-1091) | Phase 2 | 확정 | 구현 | **이프 충전 개편 필수** | 구현 | 웹 선행(KNK-1092) 뒤 앱 반영(KNK-1093). 마이 이프 카드를 "충전" 버튼 하나로 줄이고 출석을 이프 충전으로 옮겼으며, 같은 목적지를 무료 충전·내역 두 탭으로 넓혀 잔액 상자·탭 줄을 고정했습니다. 무료 충전 탭에 출석 상자와 친구 초대 진입 줄을 두고 그 줄에 보조 문구를 붙였습니다(§3-3-3 마이 · 이프 충전). 무료 충전 탭 당겨서 새로고침(프로필만 재조회 — KNK-1180) 구현. 새 API 없음 — 기존 프로필·출석·내역 호출을 그대로 씁니다. 탭 줄은 퍼널 카테고리 탭과 같은 `SecondaryTabRow` 밑줄 탭입니다 | §3-3-3 |
| FE-SCREEN-004 | 삭제된 스토리 채팅 카드(KNK-1091) | Phase 2 | 확정 | 구현 | **채팅 목록 필수** | 구현 | 웹 선행(KNK-1092) 뒤 앱 반영(KNK-1093). `storyTitle`이 빈 채팅을 목록에서 버리지 않고 제목 자리에 "삭제된 스토리예요"를 옅은 글자색으로 표시(§3-3-3 채팅 목록) | §3-3-3 |
| FE-SCREEN-008 | 이프 정책 수치 서버 연동(KNK-1096) | Phase 2 | 확정 | 구현 | 결정 필요 | 구현 | 웹 선행(KNK-1095) 뒤 앱 반영. 화면에 박아 둔 이프 수치를 지우고 `GET /credits/policies` 값을 렌더 — 완성 비용·턴 비용·출석 제목·친구 초대 보조 문구·초대 화면 안내·초대 안내 시트 제목 여섯 자리와 등록 성공 토스트·공유 문구. 앱 수명 싱글턴이 한 번 읽어 `CompositionLocal` 로 내리고, 응답 전·실패에는 자리표시 숫자를 조회 중 목록과 같은 맥박으로 그립니다(§3-3-4 이프 정책 수치 패칭). **웹이 서비스 안내의 수치 고지 절을 삭제해 앱이 옮길 대상이 없어졌습니다** — 이전의 "소모량 사전 고지 — 서비스 안내 정책 수치" 행을 이 행이 대체합니다 | §3-3-4 |
...
| FE-SCREEN-011 | 서비스 안내 | Phase 1 | 확정 | 구현 | 결정 필요 | 구현 | **본문을 복제하지 않고 약관과 같은 웹 문서 화면으로 `/about`을 엶**(§3-3-3 마이). 앱은 회원만 진입. 경로는 `/about` 으로 확정(KNK-1043)됐고 웹 배포만 남음 — 배포 전에는 앱에서 404. 게스트 이용 안내 섹션의 앱 노출 숨김 판정도 웹 잔여 | §3-3-3 |
| FE-SCREEN-012 | 채팅 공유 열람 | Phase 1 | 확정 | 구현 | **비적용** | 비적용 | 발급과 한 쌍이라 함께 뺐습니다(위 FE-SCREEN-005 채팅 공유 발급). 공유 URL을 앱 딥링크로 열지 웹으로 열지의 결정이 먼저입니다 | §3-3-3 |
...
- **앱 전용 항목(공통 계약 없음)** — 오픈소스 라이선스 고지·버전 정보·알림 설정(§3-3-3 마이), 푸시 토큰 등록·알림 권한(§3-3-4 푸시 토큰 등록). 웹에 대응 화면이 없어 위 표에 행을 두지 않고, 푸시는 아래 흐름 매트릭스 FLOW-010으로 추적합니다.
...
| --- | --- | --- | --- | --- | --- |
| FLOW-002 | 스토리 생성 | 013 · 002 · 005 | **퍼널 완성 → 채팅 플레이 필수** | 구현 | 퍼널 완성 제출 → 제작 탭 완성 중 카드 → 완성 스토리 카드/완성 알림 → 상세 "채팅 시작하기" → 채팅방 플레이(005). KNK-1222(2026-09-08)로 완성 직후 자동 채팅 진입은 없음 |
| FLOW-002A | 일반 제작 | 002 · 003 · 009 | 결정 필요 | 미착수 | 웹도 미구현 — 클라이언트 공통 잔여 |
| FLOW-002B | 스토리 수정 | 003 · 009 | 결정 필요 | 미착수 | 웹도 미구현 — 클라이언트 공통 잔여 |
| FLOW-003 | 탐색 → 플레이 시작 | 001 · 003 · 005 | **목록 → 상세 → 채팅 시작 필수** | 구현 | 목록 → 상세 → 채팅 시작 → 채팅방 플레이(005 입력 컴포저·SSE 턴 진행)까지 전 구간 동작 |
| FLOW-004 | 채팅 플레이(스트리밍) | 005 | **채팅 플레이 필수** | 구현 | SSE 클라이언트는 okhttp-sse(§3-3-2). 이벤트 파싱·낙관적 렌더·원자적 확정 교체·중단 처리·재생성·추천 입력까지 §3-3-3 채팅방대로 구현 |
...
| FLOW-006 | 스토리·채팅 삭제 | 003 · 004 · 005 | 결정 필요 | 구현 | 내 스토리 삭제는 제작 탭 카드 옵션 다이얼로그(더보기·길게 누르기)와 **`isOwner`가 참인 상세의 헤더 메뉴** 두 경로(§3-3-3 스토리 목록·스토리 상세 2026-09-03 결정 기록). **채팅 삭제는 채팅방 옵션 메뉴와 채팅 카드 옵션 다이얼로그** 두 경로(§3-3-3 채팅 목록). 확인 다이얼로그·204·404 처리·삭제 후 채팅 탭 복귀까지 §3-3-3 채팅방대로 구현 |
| FLOW-007 | 피드백 제출 | 006 | 결정 필요 | 구현 | 마이 → 피드백 → 전송까지 동작. `platform: ANDROID` 와 `appVersion` 을 함께 보내고, 이메일 형식은 보내기 전에 앱이 검증(§3-3-3 마이 · 피드백) |
| FLOW-008 | 잘못된 경로 접근 | 999 Not Found · 001 | 결정 필요 | 미착수 | URL 개념이 없어 딥링크 실패 처리로 대응 — 방식 결정 필요 |
| FLOW-009 | 서비스 정보 열람 | 008 · 010 · 011 | 결정 필요 | 구현 | 마이에서 서비스 안내(011)를 웹 문서 화면으로 열고, 약관·개인정보처리방침(010)은 로그인·서비스 안내 본문에서 동작. `/about` 경로는 확정(KNK-1043)이고 웹 배포 전에는 서비스 안내에 404가 보임 |
| FLOW-010 | 푸시 알림 | — (앱 전용) · 008 알림 설정 | **토큰 등록 → 수신·탭 진입 → 수신 동의 설정** | 구현 | 토큰 등록·로그아웃 삭제·권한 요청은 KNK-1133(§3-3-4 푸시 토큰 등록). 수신 동의 설정 화면은 KNK-1135(§3-3-3 마이 · 알림 설정). **수신 표시·채널·탭 진입은 KNK-1134 구현**(§3-3-4 알림 수신·탭 진입 — `service`·`marketing` 채널, `recipientId` 검증, `PushEntry` 라우트 매핑, 미로그인 보류 → 로그인 뒤 복원, 종료 정리 `cancelAll`). 서버 공통 키 `recipientId`는 KNK-1220 구현(4-backend 푸시 발송 모듈) |
...
- **권위 있는 출처는 `GET /auth/me`** 입니다. 프로필 구현은 `my/profile/data`가 소유하고 화면은 그 하나만 관찰합니다 — 화면마다 `/auth/me`를 부르지 않습니다(`MUST NOT`).
- **조회 시점은 정해진 목록입니다** — 로그인 직후, 앱 시작 시 토큰이 있을 때, 계정 연동 성공 후, 마이 화면이 보일 때, 잔액을 바꾸는 요청(출석 보상·초대 코드 입력)이 성공한 직후. 그 밖의 화면은 캐시된 프로필만 관찰합니다.
- **마이 화면은 보일 때마다 다시 읽습니다**(`MUST`). 이프는 채팅 턴·스토리 완성으로 줄어들고 그 화면들은 프로필을 갱신하지 않으므로, 화면을 처음 만들 때 한 번만 읽으면 탭을 오가는 동안 낡은 잔액이 남습니다. 화면 재진입은 구성 변경으로도 일어나므로 직전 조회로부터 짧은 간격(현재 5초) 안의 재요청은 건너뜁니다.
...
- **`status == SUSPENDED` 응답은 갱신 성공으로 처리하지 않습니다**(`MUST NOT`). 캐시에 남기지 않고 세션 종료 사유로 올립니다 — 곧 지울 데이터를 새로 쓸 이유가 없고, 캐시에 남으면 정리 실패 시 정지 계정의 프로필이 다음 사용자에게 보입니다. 위 조회 시점 중 **어디서 확인되든 같은 종료 절차**를 탑니다(`MUST`).
...
| 경계 | 토큰은 `auth/data`와 `network`의 토큰 접근 계약 밖으로 나가지 않습니다(`MUST`). ViewModel과 화면은 토큰을 보지 않고 세션 상태만 봅니다 |
...
- **영향.** 소형 암호화 계층이 `auth/data`의 저장소 구현에 붙습니다. Keystore 키가 손상된 기기에서는 복호화가 실패하므로 위 재로그인 경로가 반드시 있어야 합니다.
...
- **사용자가 누른 로그아웃은 1단계 장벽을 세우기 전에 등록기를 먼저 닫고 이 기기의 푸시 토큰 삭제를 한 번 시도합니다.** 새 저널 단계나 토큰 스냅숏을 추가하지 않고, 실패해도 절차를 막지 않습니다. 서버가 밀어낸 종료와 프로세스 재시작으로 재개한 정리에서는 삭제를 시도하지 않습니다. 순서와 남은 토큰의 한계는 아래 푸시 토큰 등록 계약을 따릅니다.
...
- **앱은 FCM 토큰을 영속 저장하지 않습니다**(`MUST NOT`). FCM SDK가 설치본의 토큰을 보관하므로 등록·삭제 모두 그 시점에 `getToken()`으로 읽고, 호출에 필요한 동안만 메모리에 둡니다. 마지막 등록값 비교도 두지 않습니다 — 서버 등록이 멱등 upsert라 매번 보내도 됩니다. 별도 사용자 귀속 저장소는 추가하지 않습니다.
- **오프라인·5xx 등록 실패는 사용자에게 알리지 않고 재시도 큐를 두지 않습니다.** 다음 `회원` 전이(앱 재시작)나 `onNewToken`이 다시 보냅니다. 해당 전이가 없으면 미등록 상태가 계속될 수 있으며, 연결 복구만으로 등록이 보장되지는 않습니다. 예상 가능한 네트워크 실패이므로 non-fatal을 기록하지 않습니다(§3-3-6). 취소는 실패로 삼키지 않고 전파합니다.
- **401은 공통 인증 클라이언트의 재발급·재요청 경로를 따릅니다. 403은 등록기가 처리합니다.** 현재 `AuthInterceptor`는 401만 처리하고, 일반 API의 403은 `DomainError.AccountSuspended`로 변환할 뿐 자동으로 세션을 종료하지 않습니다. 푸시 등록 API의 403은 Swagger상 정지 계정이므로 등록기가 `SessionEndSignal`에 `ACCOUNT_SUSPENDED`를 보냅니다. 이 신호는 **세션 관문의 `commit` 블록 안에서** 보냅니다 — 관문이 커밋 잠금 안에서 요청 시작 때의 세대를 다시 검사하므로, 이전 세대의 늦은 403은 신호를 내지 못하고 버려집니다. 신호 계약 자체는 바꾸지 않습니다(프로필 조회의 정지 처리와 같은 통로). 인증 작업 안에서 종료 완료를 기다리지 않습니다. 연동 API 등 다른 경로의 403 처리는 바꾸지 않습니다.
- **등록은 세션 관문의 인증 작업으로 실행합니다**(`MUST`). `회원` 관찰과 `onNewToken`은 같은 등록기로 합류하고, 등록기는 요청을 직렬화합니다. 실행 직전에 회원 상태·세션 세대를 다시 확인하여 이전 세대의 대기 요청을 버립니다. 로그아웃은 **등록기를 먼저 닫아 새 등록을 막은 뒤** 진행 중인 등록을 취소·합류하고 삭제를 시도합니다(아래 순서). 단, 앱의 취소·합류는 서버 트랜잭션의 취소를 보장하지 않습니다. 이미 서버에 도착한 `PUT`이 `DELETE` 뒤에 완료되어 토큰을 다시 남길 수 있으므로, 삭제 성공을 이전 계정 알림 차단의 보증으로 사용하지 않습니다.
- 토큰 값을 로그·Crashlytics·분석 이벤트에 싣지 않습니다(`MUST NOT`, §3-3-6). 알림 권한 여부와 무관하게 등록합니다 — 권한이 없으면 OS가 표시를 막을 뿐 토큰은 유효합니다.

**로그아웃 때 삭제.** 중앙 세션 종료 조정자가 아래 순서를 소유하며, 전부 위 절차의 1단계 장벽 **앞**에서 일어납니다. 장벽은 등록과 삭제를 구분하지 않고 인증 작업을 통째로 막으므로, 등록만 먼저 닫는 별도 차단이 필요합니다.

1. 사용자 요청 로그아웃이면 **등록기를 닫습니다.** 닫힌 등록기는 `회원` 관찰과 `onNewToken`에서 새 등록을 시작하지 않고, 진행 중인 등록을 취소한 뒤 합류합니다. 등록기는 정리가 끝나 세션이 `미로그인`으로 발행된 뒤 다시 열립니다.
2. `getToken()`으로 읽은 FCM 토큰을 본문에 넣어 `DELETE /users/me/push-tokens`를 **일반 인증 클라이언트로 세션 관문의 인증 작업 안에서** 한 번 시도합니다. 만료된 access의 선제 재발급과 401 뒤 재발급 1회·재요청 1회는 인터셉터의 기존 규칙을 그대로 따릅니다 — access TTL이 30분이라 오래 뒀다가 누른 로그아웃에서는 저장된 access가 이미 만료된 경우가 보통이고, 재발급 없이 보내면 그 경우마다 삭제가 빠집니다. **FCM 토큰 조회와 삭제를 합쳐 최대 3초만 기다립니다.** 조회 실패·시간 초과·HTTP 실패 모두 뒤 절차를 막지 않으며, 이 호출의 401·403에서는 별도 종료 신호를 보내지 않습니다(어차피 종료 중).
3. 이어서 1단계 장벽부터 기존 절차를 그대로 진행합니다.

새 저널 단계·토큰 스냅숏·종료 전용 호출 경로는 추가하지 않습니다. 프로세스 재시작으로 재개한 정리는 장벽 뒤에서 시작하므로 푸시 삭제를 건너뜁니다. 서버가 밀어낸 종료(재발급 401·403·정지·토큰 손상)도 삭제를 시도하지 않습니다. 탈퇴는 서버가 회원 토큰을 전부 지우므로 앱이 별도 삭제를 부르지 않습니다.

**이전 계정 알림 차단 — KNK-1134 필수 완료 조건.** 오프라인·5xx·인증 만료·프로세스 종료·늦은 `PUT`으로 토큰이 남을 수 있습니다. 같은 토큰을 다음 회원이 등록하면 소유자가 옮겨가지만, 그 등록이 실패하거나 이전 회원 대상으로 이미 발송한 메시지가 늦게 도착할 수도 있습니다. 따라서 현재 세션이 `회원`이라는 검사나 토큰 등록 성공만으로 수신자를 판정하지 않습니다.

- 서버의 data 전용 메시지를 앱이 표시하기 전에 **현재 회원이 그 알림의 수신자인지 확인해야 합니다**(`MUST`). 미로그인·복원 중·종료 중이거나 수신자를 확인할 수 없으면 알림을 표시하지 않고 보류 메시지를 다음 계정에 넘기지 않습니다. 표시 직전에도 같은 세션인지 확인합니다.
- 로그아웃·세션 무효화·탈퇴에서 이미 표시한 계정 알림과 대기 중인 표시 작업을 정리합니다. 알림 탭 진입도 수신자·대상 접근 권한을 다시 검증합니다. 표시·정리·탭 진입의 구체 배선은 KNK-1134가 소유합니다.
- **수신자 판별 계약은 아래 알림 수신·탭 진입 절이 소유합니다**(KNK-1134) — 서버 발송 모듈이 모든 페이로드에 싣는 공통 키 `recipientId`([`4-backend.md §4-3-5`](./4-backend.md#4-3-api-계약) 푸시 발송 모듈, KNK-1220)를 프로필 캐시와 비교하고, 없거나 다르면 버립니다.

**알림 권한(Android 13+ `POST_NOTIFICATIONS`).** 메인 그래프가 `회원` 상태로 처음 그려질 때 **설치당 한 번** 시스템 권한 요청을 띄웁니다(KNK-1135, 2026-09-08 — KNK-1133의 "프로세스당 한 번"을 바꿨습니다. 프로세스 변수만으로는 앱을 재실행할 때마다 시스템이 자동 거부하기 전까지 되물어 사용자의 결정을 다시 묻는 셈이었습니다). "물었음" 표시는 테마 설정과 같은 **기기 귀속 DataStore**에 두어 로그아웃 정리 대상이 아니며, 이미 로그인된 채 업데이트를 받은 회원도 첫 실행에서 한 번은 묻습니다. 이미 허용된 상태여도 표시를 남겨 나중에 사용자가 시스템에서 끄더라도 되묻지 않습니다. 사전 설명 화면과 거부 뒤 재요청 UI는 두지 않습니다 — 이후 재진입은 알림 설정 화면(KNK-1135)의 기기 알림 배너가 시스템 설정으로 보냅니다. API 32 이하는 런타임 권한이 없어 요청하지 않습니다. **권한 거부는 등록·세션·화면 어느 것에도 영향을 주지 않습니다**(`MUST`).

**결정 기록 — 토큰 삭제는 등록기 차단 뒤·세션 장벽 앞의 최선 노력 호출, 계정 격리는 수신 시 검증(2026-09-08, KNK-1133)**

- **배경.** 등록을 막지 않고 삭제하면 `onNewToken`이 새 등록을 시작할 수 있고, 세션 장벽을 먼저 세우면 삭제 호출이 쓸 access 토큰을 재발급할 수 없습니다. 어느 쪽이든 이미 서버에 도착한 등록을 앱이 취소할 수는 없습니다. 삭제와 수신자 검증은 서로 다른 보장입니다.

| 대안 | 채택 안 한 이유 |
| --- | --- |
| 등록기를 닫지 않고 진행 중인 등록만 취소하고 삭제한다 | 취소와 삭제 사이에 `onNewToken`이 새 등록을 시작할 수 있고, 서버에서 늦게 완료되는 등록도 막지 못합니다 |
| 세션 장벽을 먼저 세우고 스냅숏한 access 토큰으로 종료 전용 호출을 보낸다 | 장벽 뒤에는 재발급이 없어 만료된 access(TTL 30분)로는 삭제가 401로 빠집니다. 오래 뒀다가 누른 로그아웃이 바로 그 경우라 흔한 경로에서 토큰이 남고, 수동 Bearer 경로를 따로 배선해야 합니다 |
| 새 저널 단계로 삭제 성공까지 재시도하며 로그아웃을 막는다 | 오프라인·인증 만료에서도 로컬 로그아웃은 되어야 합니다. 삭제 재시도만으로 이미 발송된 메시지의 계정 격리를 보장할 수도 없습니다 |
| 토큰 상한·다음 로그인·`UNREGISTERED` 정리만 믿는다 | 다음 로그인이 없으면 소유자 이전이 없고, 10대 상한은 만료 정책이 아니며, 유효한 FCM 토큰에는 `UNREGISTERED`가 오지 않습니다 |

- **영향.** 토큰 잔존과 불필요한 서버 발송 가능성은 남습니다. 이를 서버가 자동으로 흡수한다고 보장하지 않습니다. 원격 삭제는 제한 시간 안에서 시도하고, 이전 계정 알림의 표시·탭 진입 차단은 위 수신자 검증 계약으로 보장합니다. KNK-1134의 서버 합의와 검증이 끝나기 전까지 알림 표시는 미완료입니다.

**결정 기록 — 권한 요청 시점은 메인 그래프 첫 진입(2026-09-08, KNK-1133)**

| 대안 | 채택 안 한 이유 |
| --- | --- |
| 스토리 완성 로딩 화면("완성되면 알려드릴게요")에서 요청한다 | 알림의 가치가 가장 분명한 시점이라 허용률은 높겠지만, 제작 퍼널 화면이 권한 흐름을 소유하게 되고 출석 리마인드처럼 퍼널 밖 알림은 그 화면을 거치지 않은 사용자에게 닿지 않습니다 |
| 알림 설정 화면(KNK-1135)에서만 요청한다 | 설정에 들어가지 않는 대다수가 영영 알림을 받지 못합니다 |

- **영향.** 허용률이 낮게 나오면 첫 대안으로 옮깁니다. 요청 위치는 루트 컴포저블 한 곳이라 옮기는 비용이 작습니다.

**검증.** KNK-1133은 `push-registration`·`permission`·`auth-session`·`session-cleanup` 위험으로 변경 경로를 확인합니다.

- 로그인 직후·저장 세션으로 앱 재시작·`onNewToken`에서 등록하고, 두 등록 계기가 겹쳐도 직렬 실행합니다. 권한 거부는 등록·화면에 영향을 주지 않습니다. 오프라인 복원 시 실패한 등록은 온라인 상태로 앱을 재시작하면 보정됩니다.
- 삭제 요청을 지연시킨 동안 `onNewToken`을 발생시켜도 등록기가 닫힌 뒤에는 새 `PUT`이 출발하지 않아야 합니다. 취소한 `PUT`의 서버 처리를 `DELETE` 뒤에 완료시키는 경우는 서버 토큰 잔존이 가능한 것으로 확인하며, 삭제 성공만으로 차단을 검증했다고 판단하지 않습니다.
- FCM 조회 지연·오프라인·DELETE 5xx를 주입해도 푸시 정리는 최대 3초 후 빠지고 기존 로그아웃이 계속돼야 합니다. 저장된 access가 만료된 상태에서 로그아웃하면 재발급 뒤 `DELETE`가 나가야 하고, 삭제 호출의 401·403이 중복 종료를 만들지 않아야 합니다. 각 종료 단계에서 프로세스를 종료한 뒤 재개해도 새 등록 없이 기존 로컬 정리를 마쳐야 합니다.
- 등록 403은 현재 세션의 정지 종료로 이어지고, 이전 세대의 지연된 403은 관문 `commit`에서 버려져 새 세션을 종료하지 않아야 합니다.
- KNK-1134 완료 시에는 삭제 실패 후 미로그인 수신, A 로그아웃 후 B 로그인과 등록 실패, A 대상 메시지의 지연 수신, 알림 표시와 로그아웃의 경합, 기존 알림 탭을 확인합니다. 어떤 경우에도 A 알림이 B에게 표시되거나 A의 화면으로 진입하지 않아야 합니다. 수신자를 확인할 수 없는 메시지도 표시하지 않습니다.

Google Play 서비스가 있는 에뮬레이터 이미지가 필요합니다. debug 빌드도 같은 Firebase 프로젝트로 토큰을 받고 dev 서버에 등록하며, dev의 FCM 발송 배선은 끝났지만(KNK-1181) prod는 미배선이라 실제 수신 확인은 dev로 합니다.

### 알림 수신·탭 진입 — `Phase 3 · 구현`(KNK-1134)

서버의 data 전용 메시지([`4-backend.md §4-3-5`](./4-backend.md#4-3-api-계약) 푸시 발송 모듈)를 앱이 받아 시스템 알림으로 표시하고, 탭하면 해당 화면으로 들어가는 계약입니다. 2026-09-08 결정 기록입니다. 코드는 `notification` 모듈(수신·검증·표시·정리), `:navigation`(페이로드 → 라우트 매핑), `app`(진입 인텐트 해석·보류·소비)이 나눠 갖습니다.

**수신자 검증**(`MUST`) — 위 "이전 계정 알림 차단" 조건의 구체 계약입니다.

- 서버 발송 모듈이 모든 페이로드에 싣는 공통 키 `recipientId`(수신 회원 `public_id`, [`4-backend.md §4-3-5`](./4-backend.md#4-3-api-계약) 푸시 발송 모듈 — KNK-1220)를 프로필 캐시(`GET /auth/me`의 `id`, §3-3-4 프로필 조회와 캐시)와 비교합니다. **`recipientId`가 없거나 다르면 표시하지 않습니다.** 키가 빠진 메시지는 버려지므로 서버 회귀가 나도 이전 계정 알림이 새지는 않습니다.
- 수신 시 세션이 `미확정`이면 확정될 때까지 기다리고(프로세스 콜드 스타트·종료 정리 중), `회원`이 아니면 버립니다. 프로필 캐시가 아직 없으면(로그인 직후 첫 조회 전) 도착할 때까지 기다립니다. 대기는 통틀어 5초를 넘기지 않으며 넘기면 버립니다 — FCM 서비스 콜백의 시간 예산 안이어야 합니다.
- 표시 직전에 세션이 여전히 `회원`인지 다시 봅니다. 그 뒤 로그아웃과 `notify`가 밀리초 단위로 겹치는 창은 아래 정리 단계의 `cancelAll`이 덮습니다.
- 보류 중인 표시 작업은 세션이 `회원` 아닌 값으로 확정되는 순간 폐기됩니다. 다음 계정에 넘기지 않습니다.

**채널.** 사용자가 기기에서 따로 끌 단위는 알림 설정 화면의 토글 단위와 같습니다(§3-3-3 마이 · 알림 설정).

| 채널 ID | 이름·설명 | 중요도 | 잠금화면 | 담는 것 |
| --- | --- | --- | --- | --- |
| `service` | 서비스 알림 — 설정 화면 문자열 재사용 | HIGH(헤드업) | PRIVATE — 잠긴 화면에는 "스토리가 완성됐어요"만 보이고 스토리 제목은 숨김 | `STORY_COMPLETED`, 앱이 모르는 `type` |
| `marketing` | 광고 알림 — 설정 화면 문자열 재사용 | DEFAULT | PUBLIC — 문구가 광고라 숨길 것이 없음 | `ATTENDANCE_REMINDER`, `PROMOTION` |

- 채널은 Application 시작 때 멱등으로 만듭니다(이미 있으면 그대로). 채널을 지우거나 이름을 바꾸지 않습니다 — 사용자가 채널에 내린 설정이 사라집니다.
- `com.google.firebase.messaging.default_notification_channel_id`를 `service`로 선언합니다. Firebase 콘솔 테스트처럼 `notification` 필드가 있는 메시지를 SDK가 직접 띄울 때 쓰이는 값이며, 서버의 정식 메시지는 이 경로를 타지 않습니다.
- **기기 채널 설정과 서버 수신 동의는 별개입니다.** 채널을 꺼도 서버는 계속 보내고 OS가 표시만 막습니다. 설정 화면의 기기 알림 배너가 이미 이 차이를 안내합니다.

**표시 규칙.** data 전용 메시지는 포그라운드·백그라운드 모두 앱 코드가 받으므로 **두 상태에서 같은 시스템 알림**을 띄웁니다. 앱 안 배너·스낵바는 두지 않습니다 — 스토리 완성은 퍼널 로딩 화면이 자체 결과 화면으로 넘어가므로 포그라운드 중복 안내가 필요 없고, 광고성은 앱 안에서 가로막을 이유가 없습니다.

| `type` | 제목 | 본문 | 알림 ID 기준 |
| --- | --- | --- | --- |
| `STORY_COMPLETED` | "스토리가 완성됐어요" | 페이로드 `title`(스토리 제목) | `storyId` |
| `ATTENDANCE_REMINDER` | 페이로드 `title`(서버가 `(광고)` 부착) | 페이로드 `body` | `date` |
| `PROMOTION` | 페이로드 `title` | 페이로드 `body` | `campaignId` |
| 그 외 | 페이로드 `title` | 페이로드 `body` | 없음 |

- 알림 ID는 `type`과 대상 식별자로 만들어 **같은 대상의 재발송은 이전 알림을 교체**합니다. 제목이 없는 메시지는 표시하지 않습니다.
- 기기 알림 권한이 없으면 조용히 버립니다(권한 요청은 위 알림 권한 절). 자동 취소(`autoCancel`)이며, 알림 자체에는 페이로드 식별자만 싣습니다.
- 알림 아이콘은 디자인 시스템의 종 아이콘(`ic_bell`)입니다. 단색 브랜드 마크가 생기면 바꿉니다.

**탭 진입 → 라우트**(`MUST`). 알림·App Link·공유 인텐트가 내부 라우트로 바뀌는 자리는 `:navigation`의 `PushEntry` 하나입니다(§3-3-2 결정 기록 — Navigation 3에는 딥링크 선언 API가 없어 매핑을 직접 소유). 알림은 `type`·대상 식별자·`recipientId` 세 값만 인텐트 extra로 실어 `MainActivity`(`singleTop`, `CLEAR_TOP`)를 명시적으로 엽니다.

| `type` | 목적지 | 비고 |
| --- | --- | --- |
| `STORY_COMPLETED` | `StoryDetailRoute(storyId)` | 상세가 다시 조회하므로 접근 권한은 그 화면의 조회 결과가 정합니다 |
| `ATTENDANCE_REMINDER` | `MyCreditChargeRoute` | 출석은 이프 충전의 무료 충전 탭입니다(§3-3-3 마이) |
| `PROMOTION` · 알 수 없는 `type` · 식별자 누락 | 홈(셸) | 캠페인 상세 화면이 없고, 모르는 타입은 안전한 도착지로 |

- **진입은 셸까지 걷어낸 뒤 목적지 하나만 쌓습니다.** 퍼널·채팅방 위에 상세를 겹쳐 쌓지 않으며, 뒤로가기는 현재 탭으로 돌아옵니다. 같은 목적지가 이미 맨 위면 다시 쌓지 않습니다.
- **앱이 없는 상태와 실행 중 상태가 같은 경로로 수렴합니다**(`MUST`). `onCreate`(첫 생성)와 `onNewIntent` 모두 extra를 루트 ViewModel의 **보류 진입**에 넣고, 메인 그래프가 보류를 소비합니다. 프로세스 재생성으로 다시 받은 원래 인텐트는 해석하지 않습니다(저장 상태에 백스택과 보류가 있음).
- **미로그인이면 로그인 뒤 목적지를 복원합니다.** 보류 진입은 `SavedStateHandle`에 있어 로그인 화면에서 외부 제공자로 나갔다가 프로세스가 죽어도 남습니다. 세션이 `회원`이고 프로필 캐시가 있을 때만 해석하며, **`recipientId`가 프로필 `id`와 다르면 홈으로 보냅니다** — 탭 시점의 수신자 재검증입니다. 웹의 로그인 `callbackUrl`(KNK-928)과 같은 "보류 → 로그인 뒤 복귀, 검증 실패는 기본 목적지" 규칙이고, 값 형태만 라우트 키입니다.
- 진입 데이터는 다른 세션 종료 정리 대상이 아닙니다 — 보류는 Activity 저장 상태에만 있고 다음 로그인의 프로필로 재검증되므로 이전 계정 화면으로 들어갈 수 없습니다.

**세션 종료 정리.** 표시된 알림은 사용자 귀속 데이터입니다. 알림 트레이를 `UserScopedStore`로 등록해 **종료 정리의 사용자 데이터 삭제 단계에서 `cancelAll`**합니다(§3-3-4 로그아웃 절차). 로그아웃·탈퇴·서버 강제 종료·프로세스 재개·재시도가 모두 이 단계를 지나므로 경로를 따로 두지 않습니다. 새 저널 단계나 `notification/domain` 전용 계약은 추가하지 않습니다 — 기존 정리 계약이 같은 보장을 이미 줍니다.

**범위 밖.** App Link(https 도메인 링크)와 공유 URL 열기는 FE-SCREEN-012 결정과 함께 다룹니다. 매핑 자리는 `PushEntry`와 같은 `:navigation`입니다. 앱 안 알림 목록 화면은 없습니다.

**결정 기록 — 수신자는 페이로드 `recipientId`로 판별하고, 정리는 기존 사용자 귀속 저장소 계약으로(2026-09-08, KNK-1134)**

| 대안 | 채택 안 한 이유 |
| --- | --- |
| 서버가 토큰 소유자 이전·삭제로 격리를 보장한다고 보고 앱은 `회원` 여부만 본다 | 등록 실패·늦은 `PUT`·이미 발송된 메시지의 지연 도착을 막지 못합니다(위 이전 계정 알림 차단) |
| 앱이 마지막 등록 시점의 회원 id를 토큰과 함께 저장해 비교한다 | 토큰을 영속 저장하지 않는 규칙(`MUST NOT`)과 충돌하고, 서버가 누구에게 보냈는지는 여전히 모릅니다 |
| 서버가 `type`별 대상 식별자(`storyId` 등)의 소유자를 앱이 조회해 판정한다 | 광고성 알림에는 회원 귀속 식별자가 없고, 표시 전 네트워크 조회는 오프라인·시간 예산에서 깨집니다 |
| `notification/domain`에 별도 정리 계약을 두고 조정자가 직접 부른다 | `UserScopedStore` 등록만으로 같은 단계·같은 재개·재시도를 얻습니다. 계약을 하나 더 두면 두 정리 목록이 생깁니다 |
| 포그라운드에서는 시스템 알림 대신 앱 안 배너를 띄운다 | 두 표시 경로를 유지해야 하고, 퍼널 로딩 화면이 이미 완성 결과로 넘어가므로 얻는 것이 없습니다 |

- **영향.** `recipientId`가 없는 메시지는 앱이 버립니다. 새 시나리오는 `type`만 더하면 홈 진입으로 안전하게 표시되고, 전용 목적지가 필요하면 `PushEntry` 한 곳만 고칩니다.

**검증.** `notification-delivery`·`external-entry`·`session-cleanup` 위험으로 확인합니다.

- 유닛: `recipientId` 없음·불일치는 표시하지 않음, 미로그인·미확정 세션은 대기 뒤 폐기, 알 수 없는 `type`은 홈, 보류 진입은 미로그인 동안 유지되고 `회원`+프로필에서 해석되며 수신자 불일치는 홈.
- 에뮬레이터(Play 서비스 이미지): dev 서버 스토리 완성 푸시로 백그라운드·포그라운드 표시, 앱 종료 상태에서 탭 → 상세 진입, 로그아웃 뒤 알림 사라짐, A 로그아웃 → B 로그인 뒤 A 대상 메시지 미표시. **Firebase 콘솔의 테스트 메시지는 `notification` 타입이라 SDK가 직접 띄우며 이 절의 data 경로를 검증하지 못합니다** — 채널 배정만 확인됩니다.
- **확인 결과(2026-09-08, emulator-5554 · API 37 Play 이미지).** 유닛 11건 통과. `dumpsys notification`에 `service`(HIGH)·`marketing`(DEFAULT) 채널 생성. `am start` extra 주입으로 실행 중(`onNewIntent`)·종료 상태 모두 상세 진입, 수신자 불일치·알 수 없는 `type`·식별자 누락은 홈. Firebase 콘솔 테스트 메시지에 맞춤 데이터를 실어 **포그라운드**에서 수신 → `service` 채널·PRIVATE·"스토리가 완성됐어요"/스토리 제목으로 표시, 백그라운드에서 탭 → 상세. 알림 2건 상태에서 로그아웃 → 0건. A 로그아웃 뒤 A 대상 메시지 → 미표시, A 대상 보류 진입을 둔 채 B 로그인 → 홈, B 상태에서 A 대상 메시지 → 미표시, A 재로그인 뒤 보류 진입 → 상세. 서버 `recipientId` 반영(KNK-1220, dev 배포) 뒤 **정식 data 경로**도 확인 — 간편 제작 완성 요청 후 앱을 백그라운드로 두면 약 60~70초 뒤 스토리 완성 푸시가 `service` 채널·PRIVATE로 표시되고, 프로세스를 없앤 뒤(`am kill`) 탭하면 새 스토리 상세로 진입. 서비스 알림 토글이 꺼져 있으면 서버가 발송을 건너뛰므로 검증 전 설정을 확인해야 합니다.
...
- **링크 코드는 두 호출 사이에만 존재하는 단명 값입니다.** `auth/data` 안에 두고 UI 상태나 화면 간 인자로 흘리지 않습니다(`MUST NOT`).
- 웹이 필요로 했던 연동 전용 제공자 변형과 중계 화면은 앱에 필요하지 않습니다 — 콜백 URL 개념이 없어 로그인 경로와 연동 경로가 구조적으로 분리되기 때문입니다.
- **재인증에 쓸 ID 토큰은 캐시가 아니라 새로 발급받습니다**(`MUST`). Credential Manager는 유효기간이 남은 ID 토큰을 캐시에서 그대로 돌려줍니다 — **31분 전에 발급된 토큰을 재사용하는 것을 실측했습니다**(2026-08-31). 로그인은 `iat`를 보지 않아 통과하지만 재인증은 10분 기준에 걸려 403 `REAUTH_FAILED`가 되고, 화면에는 일반 실패로만 보입니다. 요청에 **nonce를 실어** 캐시를 우회합니다 — nonce는 토큰 클레임에 실려야 하므로 캐시된 토큰으로는 만족할 수 없고 새 `iat`로 재발급됩니다(같은 조건에서 `age=0s` · 201 확인). 서버는 이 클레임을 검사하지 않습니다. 제공자 어댑터는 로그인용(캐시 허용)과 연동용(재발급 강제) 두 경로를 나눠 노출하고, **연동은 재인증과 대상 인증 모두 재발급 경로를 씁니다** — 대상 토큰도 캐시에서 오면 만료된 토큰을 제출하게 됩니다.
- **재인증 본문의 provider는 경로 파라미터와 표기가 다릅니다** — `POST /auth/links/{provider}`의 경로는 소문자(`kakao`)지만, `POST /auth/links/reauth`의 본문 `provider`는 서버 enum 이름(`GOOGLE`·`KAKAO`)입니다. 소문자로 보내면 400이고 화면에는 일반 실패로만 보여 원인을 찾기 어렵습니다(2026-08-31 실측).
- **연동 경로의 403을 정지 계정으로 접지 않습니다**(`MUST NOT`). 공용 응답 매핑은 403을 `AccountSuspended`로 접는데, 이 경로에서 그러면 재인증 실패가 정지 안내로 둔갑하고 409 두 종류도 구분할 수 없습니다. 연동 호출만 상태·사유 코드를 그대로 올리는 매핑을 씁니다(401만 세션 문제로 남깁니다).
- **연동 실패로 로그인 세션이 풀리지 않아야 합니다**(`MUST`, 위 재발급 실패 분기).
- **확인 다이얼로그의 이탈 버튼은 "닫기"입니다** — 공통 계약의 "나중에 하기"와 다릅니다. 앱은 삭제·이탈·경고 다이얼로그의 보조 버튼을 모두 "닫기"로 쓰고 있어(스토리 삭제·채팅 삭제·작성 내용 교체·추가 정보 재선택), 이 다이얼로그만 다른 표기를 쓰면 같은 자리의 버튼이 화면마다 달라집니다. 주 버튼("연동하기")과 문구 세 줄은 공통 계약 그대로입니다.
...
### 이프 정책 수치 패칭 — `Phase 2 · 구현`(KNK-1096)

수치 출처와 표시 규칙은 [`3-1-client.md §3-1-7` 이프 정책 수치 표시](./3-1-client.md#이프-정책-수치-표시)가 소유하고, 웹 구현은 [`3-2-web-app.md`](./3-2-web-app.md) §3-2-2 이프 정책 수치 패칭입니다. 이 절은 앱이 그 계약을 어떻게 충족하는지만 적습니다.
...
- **응답 전·실패에는 자리표시 숫자를 쉬머와 함께 그립니다**(`MUST`). 대체 수치를 앱에 두지 않습니다 — 앱이 들고 있는 값은 서버가 정책을 바꾸는 순간 거짓이 되고, 스토어 배포 주기가 웹보다 길어 고치는 데도 더 오래 걸립니다. 문구 골격은 유지한 채 숫자 자리만 자리표시로 두어 값이 도착할 때 줄 높이가 바뀌지 않게 합니다.
- **공유 시트 문구는 그 순간의 값을 한 번 읽습니다.** 문자열을 만들어 `Intent` 에 실어 보내는 자리라 쉬머를 얹을 대상이 없습니다. 화면을 띄운 뒤 누르는 흐름이라 그때는 값이 도착해 있습니다(웹과 같은 판단 — 3-1 §3-1-7).

- **조회는 앱 수명 싱글턴이 한 번 합니다.** 저장소가 `StateFlow` 로 값을 들고 루트가 시작할 때 한 번 읽습니다 — 수치를 쓰는 화면이 세 기능 모듈(`:create`·`:chat`·`:my`)에 걸쳐 있어 화면마다 부르면 같은 값을 여러 번 읽습니다. 화면에는 테마 토큰과 같은 방식으로 `CompositionLocal` 로 내립니다 — 화면이 조회를 시작하지 않고 값만 읽으므로 화면 상태 기계에 넣을 것이 없고, 다섯 화면의 MVI 배선을 늘리지 않습니다.
- **조회 실패를 화면에 알리지 않습니다**(`MUST NOT`). 들고 있던 값을 지우지 않고 못 받은 자리는 자리표시로 남습니다 — 수치는 화면의 주된 정보가 아니라 보조 안내라 오류 안내를 띄울 자리가 아닙니다. 다시 읽을 계기는 앱 재실행이며, 자리표시를 오래 보는 사례가 확인되면 화면 복귀 갱신에 얹습니다.
- **천 단위 구분자는 한국어로 고정합니다.** 문구가 한국어 하나뿐이라 기기 로케일에 따라 숫자만 다른 규칙으로 끊기면 같은 문장 안에서 어긋납니다.

### 아직 결정하지 않은 것 — `결정 필요`

- **카카오 재인증의 토큰 신선도.** 구글은 nonce로 확정했습니다(위 §3-3-4 계정 연동 — 31분 된 캐시 토큰에서 시작해 새 `iat` 발급을 확인). 카카오는 `login*`이 저장 토큰을 돌려주는 API가 아니라 매번 인가를 거쳐 발급한다는 근거로 추가 조치 없이 두었지만, **같은 방식(기준보다 오래된 토큰에서 시작해 새 `iat` 확인)으로 실측하지 못했습니다** — 카카오로 로그인한 계정이 필요합니다. 실측 뒤 필요하면 `Prompt.LOGIN` 같은 강제 재인증 수단을 더하고 이 항목을 닫습니다.
...
### KNK-1222 제작 카드와 다중 완성 진행

- 결정일: 2026-09-08 사용자 요청 · 구현 2026-09-08
- 티켓: [KNK-1222](https://kimandkang.atlassian.net/browse/KNK-1222)
- 상태: **구현·에뮬레이터 검증 완료**. 실패 카드 문구·동작과 완료 분석 이벤트는 아래 "확정 실패와 미결정"의 잠정 구현이며 제품 확인이 남아 있습니다.
- 적용: Android FE-SCREEN-013 제작 목록, FE-SCREEN-002 간편 제작 완성 흐름. 웹의 배너·완성 흐름은 변경하지 않습니다.
- 구현 결정·검증 기록: [Android 계획](../../../manyak-android/docs/plans/studio-creation-cards.md).

**기존 규칙과의 관계.** 이 절이 Android의 이어서 만들기 배너, 완성 로딩 화면, 완성 후 자동 채팅 생성·진입, 임시 저장과 완성 요청의 단일 Room 행 공유를 대체합니다. 아래 라이프사이클 절과 §3-3-3의 완성 진입 설명은 이 절 기준으로 정리했습니다. 스토리라인 생성 로딩·선택과 편집 단계의 임시 저장은 그대로입니다.

**상태별 카드.** 배너를 제거하고 기존 내 스토리와 같은 1열 가로 카드에 초안·완성 중 요청을 표시합니다. 표지 폭 128dp, 3:4 비율, 행 패딩과 정보 배치는 기존 카드 기준입니다.

| 항목 | 편집 중 초안 | 완성 중 요청 | 완성된 스토리 |
| --- | --- | --- | --- |
| 개수 | 편집 가능한 초안 최대 1개 | 여러 개 허용 | 기존 목록 |
| 썸네일 | 회색 바탕 중앙에 더 진한 회색 마냑 캐릭터 심벌(`text-disabled`) | 회색 바탕에 회전하는 스피너 | 실제 표지/기존 placeholder |
| 제목 | 회색 `만들고 있는 스토리` | `스토리를 완성 중이에요` | 실제 제목 |
| 소개 | `임시 저장한 내용부터 이어서 만들 수 있어요`(2026-09-08 사용자 결정 — 처음 안은 없음) | `스토리가 완성되면 알림으로 알려드려요` | 기존 소개 |
| 태그·좋아요·채팅 횟수·날짜·지표 아이콘 | 없음 | 없음 | 기존 표시 |
| 하단 지표 자리 | `이어서 만들기` 버튼, 보이는 높이 40dp | 버튼 없음 | 기존 지표 |
| 더보기 | `삭제하기`만 | 버튼 자체 없음 | 기존 신고·삭제 |
| 클릭 | 이어서 만들기로 저장된 단계 복원 | 재개·상세·옵션 동작 없음 | 기존 상세 진입 |

캐릭터 심벌에 워드마크를 포함하지 않습니다. `이어서 만들기` 버튼의 40dp는 시각 높이이며 최소 터치 영역은 접근성 기준을 유지합니다. 문구는 지정한 문장 그대로 사용하되 큰 글자·좁은 화면에서는 줄바꿈을 허용합니다. 회색·크기·여백은 디자인 토큰으로 제공하며 스피너 상태는 접근성으로도 안내합니다.

**제작과 완료.** 유효한 입력에서 완성 버튼을 누르면 복구 가능한 요청을 영속화하고 전송 실행자에 인계한 뒤 **완성 응답을 기다리지 않고 제작 탭으로 이동**합니다. 완성 전용 로딩 목적지로 가지 않으며 자동 채팅 생성·자동 채팅방 진입도 하지 않습니다. 저장 실패 시 입력을 남기고 해당 화면에서 오류를 안내합니다. 제출 연타는 같은 요청 하나로 처리합니다. 퍼널 백스택을 제거하고 제작 탭을 선택해 뒤로가기로 제출한 편집 화면이 되살아나지 않게 합니다.

**동시 진행과 삭제.** 초안 1개 + 완성 중 N개 + 기존 스토리가 동시에 있을 수 있습니다. 완성 요청을 제출하면 편집 슬롯이 비어 다음 스토리를 만들 수 있습니다. 완성 중 요청만 있을 때 새 제작을 막거나 폐기 확인을 하지 않습니다. 편집 초안이 있을 때만 기존 새로 만들기 확인을 사용하며, 확정하더라도 그 초안만 삭제합니다. 초안 카드 더보기의 삭제도 확인 후 해당 초안만 지우며 서버 스토리 삭제 API를 호출하지 않습니다. 삭제 실패 시 카드와 입력을 남깁니다. 완성 중 요청에는 삭제·신고·재개 버튼을 제공하지 않습니다.

**새로고침과 알림.** 제작 탭 복귀 및 당겨서 새로고침으로 각 요청의 상태와 내 스토리 목록을 갱신합니다. 완성된 스토리가 0개여도 진행 카드가 있으면 스크롤과 당겨서 새로고침이 가능합니다. 목록 조회가 실패해도 로컬 초안·진행 카드와 새 제작 진입을 유지합니다. 요청별 실패를 격리하고 완료된 요청은 실제 storyId의 일반 카드로 전환합니다. 같은 스토리의 중복 카드나 목록 반영 지연에 따른 결과 유실을 막습니다. 다른 스토리 편집 중 결과가 오더라도 그 편집 화면을 강제로 이동시키지 않습니다. 기존 완성 푸시와 알림 탭 상세 진입을 재사용하며, 알림 권한·수신 설정이 꺼져 있어도 새로고침으로 완료를 확인할 수 있어야 합니다. 완성 중 카드가 제작 탭에 보이는 동안만 5초 간격으로 요청 상태를 조회하고(2026-09-09 사용자 결정 — 완성이 끝나도 탭을 당기기 전까지 스피너가 남는 것을 없애기 위해), 탭을 벗어나거나 미확정 요청이 없어지면 멈춥니다. 상시 폴링과 새 알림 권한 흐름은 추가하지 않습니다.

**저장·작업 수명.** 편집 슬롯은 하나로 유지하고 완성 요청은 requestId별로 별도 보존합니다. 제출 스냅숏 저장과 해당 편집 슬롯 해제는 원자적이어야 합니다. 완료·실패 콜백, 초안 삭제, 새로 만들기, 지연 임시 저장이 다른 요청이나 새 초안을 삭제·덮어쓰지 않아야 합니다. 화면 수명과 완성 전송을 분리하되 기존 세션 종료 장벽·사용자 데이터 정리 계약을 따릅니다. 로그아웃 후 늦은 결과를 거절하고 다른 계정으로 요청을 재전송하지 않습니다. 앱 재실행 후에는 보존된 requestId로 상태를 조회하고 미접수 요청은 기존 멱등 계약 안에서 복구합니다. 앱 데이터 삭제 이후 진행 요청을 복원하거나 다른 기기의 진행 목록을 동기화하는 기능은 포함하지 않습니다.

기존 Room v1의 초안·키워드·스토리라인 입력과 완성 요청은 **보존 마이그레이션** 대상입니다. 이 변경에는 기존 파괴적 fallback 규칙을 적용하지 않습니다. 서버에서 다시 받을 수 없는 입력이나 복구 키를 자동 폐기하면 안 됩니다.

**구현 형태(2026-09-08).** DB v2는 편집 슬롯 `pending_story_creation`(단일 행)을 유지하고 `story_completion_request`(`requestId` 기본 키, 명령·생성 결과·진행 JSON, `submittedAt`, `status=PENDING|COMPLETED|FAILED`, `storyId`·`storyTitle`)를 더합니다. v1→v2 마이그레이션은 `STORY_COMPLETION` 행을 해석해 요청 행(PENDING)으로 옮기고 원본을 지우며, 해석할 수 없는 행은 원본을 남긴 채 무시합니다. 파괴적 폴백은 제거했습니다. 제출은 DAO 트랜잭션 하나에서 요청 행 삽입과 **같은 생성 결과를 담은 초안 행** 삭제를 처리하고, 영속 실패면 전송하지 않습니다. 전송·복구는 `:create`의 앱 스코프 실행자가 `SessionGate.withAuthWork/commit` 안에서 하며 requestId 단위로만 합류합니다 — POST 성공은 COMPLETED, 네트워크 오류·401/403은 미확정 유지, 그 밖의 상태 코드(409 포함)는 복구 GET으로 판정하되 GET 404(서버가 모르는 요청)만 FAILED로 확정합니다. 제작 탭 복귀·당겨서 새로고침과 완성 중 카드가 보이는 동안의 5초 주기 조회(`StudioViewModel.drivePendingCompletionPolling`, STARTED 게이트)가 미확정 요청을 GET으로 조회하고, GET 404는 접수 전 끊긴 요청으로 보아 같은 명령을 재전송합니다. 완료된 요청은 내 스토리 목록에 같은 `storyId`가 실리면 행을 지우고, 아직 없으면 목록을 한 번 더 읽는 동안 실제 제목의 완료 카드로 남깁니다. 두 테이블은 `UserScopedStore` 정리에 넣지 않습니다(아래 계정별 보존). 제작 탭은 `:common`의 `CreationProgressAccess`(초안 요약·요청 요약 흐름·초안 폐기·요청 새로고침/재시도/삭제)만 봅니다.

**계정별 보존(2026-09-08 사용자 결정).** 완성 중 로그아웃했다가 같은 계정으로 돌아오면 완성 알림이 오는데 카드가 사라져 있어, 로그아웃 정리에서 두 테이블을 지우지 않고 **행마다 소유 회원 공개 ID(`ownerId`)로 격리**합니다(DB v3, `ALTER TABLE … ADD COLUMN ownerId`). 스토어는 로그인한 회원(`UserProfileRepository.profile.id`)의 행만 읽고 쓰며, 회원을 모르면(로그아웃·프로필 미도착) 읽기는 비어 있고 쓰기는 실패합니다. 이전 버전 행은 빈 소유자로 남았다가 다음 로그인 회원이 넘겨받습니다 — v2까지는 로그아웃이 행을 전부 지웠으므로 남은 행은 그 회원의 것입니다. 다른 회원에게는 보이지 않으니 공용 기기 노출 규칙은 유지되며, 로그아웃 뒤 늦은 응답 거절(`SessionGate`)도 그대로입니다. 한계: 편집 슬롯은 기기당 한 행이라 다른 회원이 새 초안을 쓰면 이전 회원의 초안은 덮이고, 탈퇴한 회원의 행은 보이지 않는 채 남습니다. 서버에 닿지 않은 요청은 재로그인 뒤 새로고침에서 GET 404 → 재전송으로 이어집니다.

**서버 계약 근거.** 2026-09-08 dev Swagger의 `POST /api/v1/stories/simple`는 최종 결과 `201`이며 접수 응답 `202`를 가정하지 않습니다. `GET /api/v1/stories/simple/creation-requests/{requestId}`는 `PENDING/COMPLETED/FAILED` 및 nullable `result`를 제공하고, `404`는 요청 없음 또는 요청자 소유 아님입니다. 내 스토리 목록은 `GET /api/v1/users/me/stories`를 사용합니다. 요청 전체 조회 API를 새로 가정하지 않습니다. 필드·오류 계약은 [dev Swagger](https://dev-api.manyak.app/swagger-ui/index.html)와 서버 코드가 정본입니다.

**확정 실패와 미결정.** 응답 유실·네트워크 오류를 확정 실패로 낮추지 않습니다. 확정 실패도 입력을 보존하며, 다른 초안이 생겼다고 그 초안을 자동 덮어쓰거나 편집 초안을 두 개로 늘리지 않습니다. 다음 두 사항은 **잠정 구현**이며 제품·분석 확인 뒤 확정합니다.

- 실패 카드(잠정): 회색 표지에 심벌, 제목 "스토리를 완성하지 못했어요", 소개 "다시 시도하거나 삭제할 수 있어요", 하단 40dp "다시 시도하기"(같은 requestId 재전송), 더보기는 "삭제하기"만. 편집 복귀는 두지 않았습니다. 402 이프 부족도 이 카드로 합류하며 사유별 문구는 없습니다.
- `client_storyCreate_completed`는 `chat_id` 필수라 자동 채팅 제거 뒤 수집하지 않습니다(가짜 ID를 넣지 않음). `storyCompletion_requested`·`completeError_shown(stage=story)`은 유지합니다. 분석 계약에서 `chat_id`를 선택으로 바꾸면 완료 콜백에서 수집을 되살립니다.

**완료 검수(2026-09-08 에뮬레이터, dev 서버).** v1 완성 행 업그레이드 → PENDING 카드 → 온라인 새로고침에서 GET 404 재전송 → 실제 카드 전환, 초안 1개+완성 중 1개+완성 스토리 동시 표시, 완성 중 2개(오프라인 제출 2건 → 온라인 새로고침에서 동시 전송·각각 완료), 회전·프로세스 재실행 후 카드 유지, 완성 스토리 목록 실패 상태에서도 카드와 새로고침 유지, 새로 만들기 확인이 초안만 지우고 요청 보존, 초안 더보기 삭제, 로그아웃 후 두 테이블 비움과 재로그인, 완성 푸시 수신·탭 상세 진입, 다크 모드를 확인했습니다. 확정 실패 카드와 영속 실패는 실제 서버로 재현하지 못해 유닛 테스트로만 확인했습니다.
...
**요청 영속 — Room 단일 행(KNK-1222 이전 기록. 현재는 편집 슬롯 + `story_completion_request` 두 테이블이며 마이그레이션은 보존형입니다 — 위 KNK-1222 절).** 생성·완성 요청 전에 `requestId`·단계·퍼널 컨텍스트를 Room 테이블 `pending_story_creation`의 단일 행(`id` 고정 0)에 저장합니다(웹 `manyak:pending-creation-request` 키의 앱 대응). 임시 저장(`STORY_DRAFT`)과 키워드 임시 저장(`KEYWORD_DRAFT`)도 같은 행을 씁니다. `id`·`stage`만 정규화하고 `generationCommand`·`completionCommand`·`generation`·`progress`·`keywordSnapshot`은 nullable JSON 문자열 컬럼에 담습니다 — 슬롯이 하나라 조인할 대상이 없어 중첩 리스트까지 테이블로 쪼갤 이유가 없습니다. 스키마는 export하되 마이그레이션은 파괴적 폴백을 씁니다(진행 레코드는 재생성 가능한 스냅숏이라 해석 불가 시 폐기가 안전하며, 이는 기존 레코드 정리 규칙과 같습니다). 레코드 스토어는 사용자 귀속 저장소 정리 계약(`UserScopedStore` 멀티바인딩 — §3-3-4 로그아웃 절차)에 참여합니다.
...
**퍼널 경고 다이얼로그의 버튼 배치와 라벨은 셋이 같습니다.** 미저장 이탈·소실·"다시 선택하기" 경고 모두 만들던 내용을 버리는 동작을 오른쪽에 `backgroundDangerSubtle` 배경·`textDanger` 글자로, 되돌아가는 동작을 왼쪽 텍스트 버튼 "닫기"로 둡니다 — 경고마다 자리·색·말이 다르면 어느 쪽이 무엇을 버리는지 매번 다시 읽어야 합니다(소실 경고의 "계속 만들기"를 "닫기"로 맞춘 이유입니다). 스토리 삭제 확인(§3-3-3)과 제작 재개 다이얼로그도 같은 표기를 씁니다. 이탈 시 별도 임시 저장 토스트는 표시하지 않습니다.

**닫기는 상태와 무관하게 늘 확인 다이얼로그를 거칩니다**(2026-09-03 결정 — 멘토 검수 KNK-1180). 종전에는 이미 저장된 스냅숏만 남은 이탈을 경고 없이 내보냈는데, 헤더 닫기(X)를 눌렀을 때 어떤 때는 묻고 어떤 때는 바로 나가면 사용자가 방금 무엇을 잃었는지 확신할 수 없고, 실수로 누른 닫기에서 돌아올 길이 없었습니다. 그래서 세 번째 경고 **닫기 확인**("스토리 만들기를 그만둘까요? / 만들던 내용은 제작 탭의 이어서 만들기에서 계속할 수 있어요", 왼쪽 "닫기" · 오른쪽 "나가기")을 두어, 저장분이나 진행 중 레코드만 남은 이탈은 이 확인을 거칩니다. 판정 순서는 미저장 편집 → 저장분·진행 중 레코드 → 아무것도 없음이며 세 경고는 잃는 대상이 달라 문구를 합치지 않습니다. 키워드 단계에서 입력이 없는 채로 닫는 경우도 소실 경고를 거칩니다.
...
**응답을 못 받은 완성 실패 뒤에도 저장은 열어 둡니다**(`MUST`). 네트워크 실패는 복구 조회에 쓸 `requestId` 때문에 완성 레코드를 슬롯에 그대로 두는데, 저장까지 잠가 두면 실패 뒤에 고친 입력이 디스크에 닿지 못합니다. 다만 이 구간의 저장은 단계를 `STORY_DRAFT`로 낮추지 않고 **완성 레코드의 진행만 갈아 끼웁니다**(`MUST`) — 초안으로 덮으면 재진입이 복구 폴링에 합류하지 못해, 응답만 유실됐을 뿐 서버가 완성해 둔 스토리를 되찾지 못합니다. 상태 코드로 거절된 실패(402 포함)는 종전대로 레코드를 `STORY_DRAFT`로 되돌립니다.
...
**초안 카드(이전 이어서 만들기 배너)에 닫기(X)를 두지 않습니다 — 앱 전용 차이.** 3-1은 배너의 닫기가 레코드를 폐기한다고 정하지만, 앱은 그 버튼을 두지 않고 카드 더보기의 "삭제하기"가 확인을 거쳐 초안만 지웁니다(KNK-1222). 확인 없이 만들던 스토리를 버리는 파괴적 동작인데, 같은 일을 하는 FAB 진입 다이얼로그의 "새로 만들기"는 한 번 묻기 때문입니다. 되돌릴 수 없는 폐기는 확인이 있는 경로 하나로 모읍니다. 재개는 레코드를 지우지 않고, "새로 만들기" 확정·초안 카드 삭제·입력을 모두 지운 키워드 초안·완성 제출(요청 테이블로 이동)·복구할 생성 결과가 없는 확정 서버 실패·로그아웃에서 정리합니다. 새 생성은 같은 단일 행을 최신 단계로 덮어씁니다.

- **완성 로딩 화면은 없습니다(KNK-1222).** 완성 제출 뒤 퍼널은 곧바로 닫히고, 같은 requestId 재전송이 409로 거절되면 실행자가 복구 조회로 판정합니다(3-1 완성 재시도 멱등 조항 — 생성·완성 공통).

**복구 폴링 — 화면 수명 게이트, 백그라운드 작업 수단 불채택.** 스토리라인 생성의 복구 조회(`GET /stories/simple/creation-requests/{requestId}`, 3초 주기)는 복원된 단계 화면이 STARTED 인 동안만 수집되는 Flow 로 돌립니다. 완성 요청의 복구 조회는 제작 탭 복귀·당겨서 새로고침과, 완성 중 카드가 보이는 동안의 5초 주기 조회입니다(KNK-1222) — 3-1의 "백그라운드 중지·복귀 재개"를 라이프사이클 수집으로 충족합니다. WorkManager·Foreground Service 는 채택하지 않습니다(§3-3-2 미결 해소) — 서버가 연결과 무관하게 생성을 완주하므로 앱이 백그라운드에서 요청을 완주시킬 이유가 없고, 결과 반영은 어차피 화면 복귀 시점입니다.

**백스택 복원 — 재개·복구 진입은 단계 체인 전체를 쌓습니다.** 제작 탭 배너·재개 다이얼로그에서 퍼널로 들어갈 때 레코드의 단계까지 `[스토리라인(, 추가 정보)]` 라우트를 한 번에 쌓아, 복원 후에도 백스택 구조(§3-3-3 — 스토리라인이 키워드를 대체)가 유지되게 합니다. 재개 지점은 레코드가 정합니다 — `KEYWORD_DRAFT`는 `[키워드]`, `STORYLINE_GENERATION`과 선택 순번이 없는 `STORY_DRAFT`는 `[스토리라인]`, 선택 순번이 있는 `STORY_DRAFT`는 `[스토리라인, 추가 정보]`입니다(`STORY_COMPLETION`은 KNK-1222로 편집 슬롯에서 사라졌습니다). 프로세스 재시작은 Navigation 3 의 백스택 직렬화가 라우트를 복원하고, 비어 있는 생성 스토어(Idle)를 단계 ViewModel 이 감지해 레코드 복원·폴링에 합류합니다 — §3-3-3 이 보류로 남겼던 "프로세스 재시작 시 실패 화면 합류"를 대체합니다.
...
1. 앱 시작 시 중앙 식별자 저장소에서 `device_id`를 읽거나 먼저 생성합니다. `:analytics`는 Amplitude에 그 값을 `setDeviceId`로 주입하기 전에는 이벤트를 발행하지 않습니다. API 헤더는 같은 저장소의 값을 읽으므로 SDK 생성값과 경합하지 않습니다.
...
| StudioScreen(스튜디오 탭)                     | `storyList` (`section=created`) | 위와 같음 + `CreateStory` → `createButton_clicked(source)` · `OpenCardOptions(Story)` → `storyOptions_opened` · `ConfirmDelete(Story)` → `story_deleted` · 초안 카드·재개 다이얼로그는 `storyCreate_*`(`continueBanner_*`) 이름 유지, 완성 요청 카드는 이벤트 없음 |
...
**발화 위치 원칙** — 이벤트는 ViewModel의 부수효과 단계에서 보냅니다(`reduce`는 순수 함수라 발화하지 않습니다). 화면 진입 `viewed`만 컴포저블의 `LaunchedEffect(Unit)`에서 보내되, 구성 변경 재실행을 막기 위해 ViewModel이 `viewedReported` 플래그를 들거나 `ScreenShown` Intent가 있는 화면은 그 Intent에서 보냅니다. 노출(`impressed`)은 `:analytics`의 `Modifier.trackImpression`이 §6-4-3 기준(50% · 1초 · 30초 중복 제거)을 구현합니다.

**P0 Crashlytics breadcrumb — `구현`(KNK-1179).** `:analytics`의 `AmplitudeAnalytics`가 분석 이벤트를 발행하는 그 자리에서 `CrashReporter`로 이름을 넘겨 `FirebaseCrashlytics.log()`에 남깁니다. 화면이 같은 사실을 두 번 알리지 않게 하려는 것입니다. `screen_name`만 지속 custom key로 두고 `story_id`·`chat_id`·`creation_id`는 breadcrumb 줄에만 싣습니다 — 지속 key로 두면 그 화면을 떠난 뒤 난 크래시에도 옛 식별자가 붙어 무관한 리포트를 그 스토리·채팅 탓으로 읽게 됩니다. 나머지 프로퍼티는 생략합니다. breadcrumb은 `device_id` 주입 대기열을 타지 않습니다 — 식별자가 붙기 전에 죽어도 직전 행동은 리포트에 남아야 합니다.

**API non-fatal `recordException`은 미배선입니다.** 공통 통신 오류 변환 지점인 `:network`의 `apiCall`이 최상위 함수라 주입 지점이 없고, OkHttp 인터셉터로 옮기려면 5xx를 어떤 예외로 합성해 이슈를 묶을지와 취소된 호출을 어떻게 거를지를 함께 정해야 합니다. `DomainError.Server`가 이미 `requestId`를 들고 있어 붙일 자리는 남아 있습니다.
...
| 7 | `thumbnailUrlSm`이 없는 스토리의 채팅 카드에 placeholder가 그려지고 카드 높이가 다른 카드와 같습니다. 카드의 더보기 버튼이나 길게 누르기로 신고하기·삭제하기 다이얼로그가 열리고, 삭제는 확인을 거쳐 그 카드만 목록에서 빠집니다 | 표지 유무로 높이가 달라지면 목록이 들쭉날쭉해집니다. 다이얼로그의 항목·절차는 채팅방 옵션 메뉴와 같아야 진입점이 둘이어도 동작이 하나로 읽힙니다 |
| 8 | 참조 스토리를 지운 뒤 그 채팅이 목록에 **남아 있고** 제목 자리에 옅은 글자색으로 "삭제된 스토리예요"가 나옵니다. 카드를 누르면 채팅방이 그대로 열립니다 | 제목이 비었다고 항목을 버리면 사용자가 이어가던 채팅에 닿을 길이 없어지고, 빈 제목을 그대로 그리면 무엇의 채팅인지 알 수 없는 줄이 남습니다(KNK-1093) |
...
| 8 | 채팅방에서 턴을 전송하고 **응답을 받는 중에** 화면을 돌리거나 다크 모드를 토글하면 전송 턴이 상단 앵커를 유지하고, 스트리밍 중 스크롤을 옮겨 뒀다면 그 자리가 유지됩니다 | 앵커가 깔아 둔 여백과 표식이 재생성을 견디지 않으면 복원한 위치가 콘텐츠 끝에 걸려 전송 턴이 아래로 밀립니다(§3-3-3 채팅방 스크롤) |

1~8 은 모두 화면 동작이라 메인 탭 셸 검수 항목과 같은 제약을 받습니다 — 계측 테스트가 CI 게이트에 붙기 전까지 수동 확인 대상입니다. 3의 잠금 유지 판정만 진행 중 요청이 있는 상태에서 화면 표시 인텐트를 다시 보내는 ViewModel 단위 테스트로 떼어 볼 수 있습니다. **프로세스 종료 복원은 이 표가 다루지 않습니다** — "활동 유지 안 함"으로 확인하는 백스택·라우트 복원은 메인 탭 셸 8과 스토리 상세 9가 담당합니다.
