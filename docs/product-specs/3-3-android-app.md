...
| 로컬 저장(채팅 캐시) | **Room** | 방이 여러 개이고 방마다 턴이 수십 개라 방 단위 조회·오래된 방 제거·전삭제가 필요합니다. 목적은 오프라인 지원이 아니라 **TTI 단축**입니다(§3-3-4 채팅 로컬 캐시) |
...
**도입하지 않는 것과 그 이유** — WorkManager·Foreground Service(서버 복구 계약과 FCM 알림이 안전망 — §3-3-5), 범용 응답 캐시 계층(채팅 캐시는 예외 — §3-3-4), 원격 Feature Flag(§3-3-7), **MVI 프레임워크**(직접 구현 — 아래), **Gradle 규약 플러그인**(v1 미도입 — 아래 모듈 구조).
...
`:core:data` 내부는 관심사별로 패키지를 나눕니다 — `api`(Retrofit 인터페이스·응답 모델), `sse`(스트리밍 수신), `datastore`(Preferences·암호화 토큰·퍼널 슬롯), `database`(Room 엔티티·DAO — 채팅 캐시, §3-3-4), `repository`(구현), `di`(Hilt 모듈), `interceptor`(식별 헤더·인증·로깅).
...
- **앱 전용 추가 항목** — 로그인 화면(앱 전용), 당겨서 새로고침(홈·채팅 목록), **채팅 로컬 캐시**(채팅 목록·방 — TTI 단축, §3-3-4).
...
**목표는 오프라인 지원이 아니라 TTI(Time to Interact, 화면 진입부터 콘텐츠를 볼 수 있을 때까지의 시간) 단축입니다.** 채팅 방에 다시 들어갔을 때 스피너 대신 **마지막으로 본 대화를 즉시 그리고**, 서버 응답이 도착하면 조용히 맞춥니다. 연결이 나쁠 때 캐시가 보이는 것은 부산물이며, **오프라인 동작을 보장하지 않습니다**(§3-3-5 디바이스 지원 범위).

| 항목 | 규칙 |
| --- | --- |
| 대상 | **채팅 목록(FE-SCREEN-004)과 채팅 방의 완료된 턴(FE-SCREEN-005)** 뿐입니다(`MUST`). 방만 캐시하면 오프라인에서 도달할 경로가 없어 목록이 함께 들어갑니다 |
| 비대상 | 크레딧 잔액·스토리 목록·스토리 상세는 캐시하지 않습니다(`MUST NOT`) |
| 읽기 | 화면 진입 시 캐시를 **즉시 그리고** 동시에 조회를 보냅니다(`MUST`). 응답이 오면 교체합니다 |
| 쓰기 | SSE `completed` 후 **상세 재조회 결과만** 기록합니다(`MUST`). 토큰 단위로 쓰지 않습니다(`MUST NOT`) |
| 교체 | 서버 응답이 오면 **통째로 교체**합니다(`MUST`). 캐시는 정본이 아니라 첫 프레임용이므로 병합하지 않습니다 |
| 404 | 해당 캐시를 삭제하고 목록으로 되돌립니다(`MUST`). 삭제 성공 시 처리(§3-3-3 화면 전환 규칙)에 합류합니다 |
| 보관 | **최근 진입한 방 N개**만 유지합니다(초기값 30). 기간 상한을 두지 않습니다 — 오래 안 들어간 방은 캐시해도 쓰이지 않습니다 |
| 로그아웃 | 캐시를 **전부 삭제**합니다(`MUST`). `device_id`를 새로 발급하는데 대화가 남으면 다음 사용자에게 노출됩니다 |
| 암호화 | 적용하지 않습니다(아래 결정 기록) |
| 스키마 | 테이블은 화면이 아니라 **도메인(방·턴) 단위**로 정의하고 UI 모델을 저장하지 않습니다(`MUST`). 화면이 바뀌어도 캐시가 살아남게 하려는 것입니다 |

**갱신 중임을 사용자에게 표시합니다**(`MUST`). 캐시를 먼저 그리면 웹에서 만든 최신 채팅이 잠시 보이지 않고, 느린 연결에서는 그 구간이 길어집니다. 표시가 없으면 사용자가 "내 채팅이 사라졌다"고 오해합니다.
...
- **암호화하지 않는 이유.** 토큰에는 Keystore 암호화를 적용하지만(위 인증 토큰 보관) 채팅 캐시에는 적용하지 않습니다. **토큰은 탈취해 다른 기기에서 쓸 수 있지만 채팅 캐시는 그 기기 안에서만 의미가 있고**, 기기를 물리적으로 확보한 공격자는 세션이 살아 있으므로 앱을 열어 같은 내용을 봅니다 — 암호화의 실익이 작습니다. 자동 백업도 비활성화되어 있어(§3-3-7) 백업 유출 경로가 없습니다. 재검토 조건은 민감 콘텐츠 정책이 생기거나 기기 공유가 제품 요구가 될 때입니다.
...
- **채팅을 제외하면** 응답 캐시 계층을 두지 않습니다(채팅 캐시는 위 채팅 로컬 캐시). 재조회 규칙은 셋이며, 캐시가 있는 화면도 이 규칙을 그대로 따릅니다 — 캐시는 조회를 대체하지 않고 **응답을 기다리는 동안 그릴 내용**을 제공할 뿐입니다.
...
| 네트워크 | 오프라인 미지원 | 연결이 없으면 각 화면의 오류 상태와 재시도로 처리합니다. **채팅 로컬 캐시(§3-3-4)는 이 결정을 바꾸지 않습니다** — 목적이 TTI 단축이고, 연결이 없을 때 캐시가 보이는 것은 부산물이라 오프라인 동작을 보장하지 않습니다 |
...
| 채팅 캐시 | 쓰기 시점(`completed` 후 재조회 결과만), 서버 응답 통째 교체, 404 시 삭제, 보관 상한 초과 시 오래된 방 제거, 로그아웃 전삭제(§3-3-4) |
...
| TTI 계측 | v1 미도입. 채팅 캐시의 효과는 **검수 항목으로 육안 확인**합니다(아래 Definition of Done). 구간별 수치 계측은 후속 과제입니다([`roadmap.md §R-5`](../planning/roadmap.md)) |
...
- **채팅 로컬 캐시** — Room 배선(엔티티·DAO), 진입 시 캐시 선표시와 응답 교체, `completed` 후 기록, 갱신 중 표시, 보관 상한과 로그아웃 전삭제(§3-3-4).
...
- [ ] **캐시가 있는 채팅 방에 다시 들어가면 빈 화면이나 스피너가 보이지 않습니다**(TTI 검수 — §3-3-4). 갱신 중 표시가 함께 보입니다.
