...
| FE-SCREEN-008 | 이프 내역(KNK-1081) | Phase 2 | 확정 | 미구현 | **이프 내역 필수** | 구현 | 마이 이프 카드의 "내역" 진입 버튼과 화면(잔액 상자 + 32dp 아래 커서 목록·사유 라벨·부호·만료일 표기·골격·빈 상태·재시도) 구현(§3-3-3 마이 · 이프 내역). 목록 끝에서 다음 커서를 잇고 다음 페이지 실패는 그 자리 재시도 버튼으로 받음. 구성 변경에서 불러온 페이지와 스크롤 위치 유지 확인. **앱이 웹보다 먼저 구현** | §3-3-3 · §3-3-5 |
