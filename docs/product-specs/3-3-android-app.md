...
§3-3-5  디바이스 지원·접근성·라이프사이클 (퍼널 이탈 가드·백그라운드 복구 확정 · 디바이스 지원·접근성 작성 예정)
...
FE-SCREEN-001(홈 오리지널 스토리 목록)·FE-SCREEN-013(제작·내 스토리 목록)의 안드로이드 구조입니다. 카드 구성·문구·API 계약의 정본은 [`3-1-client.md §3-1-3`](./3-1-client.md#3-1-3-화면별-스펙)이고, 이 절은 앱이 그 계약을 어떻게 충족하는지와 웹과의 차이만 정합니다. 두 목록 모두 셸 아래 2열 카드 그리드이며, 섹션 제목("오리지널 스토리"·"내가 만든 스토리")은 목록의 첫 행으로 들어가 콘텐츠와 함께 스크롤됩니다. **두 목록 모두 카드 탭(상세 진입)은 FE-SCREEN-003과 함께 구현합니다** — 현재 카드는 표시 전용입니다.

**목록은 탭 진입 시 한 번 조회합니다.** 홈은 인증 없는 `GET /stories/originals`(등록순), 제작은 인증 경로 `GET /users/me/stories`(생성 최신순)이며 서버가 주는 순서를 그대로 씁니다. 조회 중에는 카드와 같은 구조의 골격을 지연 판정 후 표시하고, 실패는 문구와 재시도 버튼으로 복구합니다. 빈 목록은 홈에서는 섹션 자체를 그리지 않고, 제작에서는 안내 문구("아직 만든 스토리가 없어요")와 FAB가 만들기 유도를 맡습니다. **페이징·당겨서 새로고침은 잔여입니다** — 내 스토리는 서버 기본 상한(100건)까지만 보이고, 제작 완료 뒤 탭 복귀 시의 목록 갱신 정책은 완성 흐름과 함께 정합니다(`결정 필요`).
...
**내 스토리 삭제는 카드 더보기 메뉴에서 진입합니다** — 웹에 없는 앱 전용 진입입니다. 카드 표지 우상단 더보기 버튼 → "삭제하기" 항목 → 확인 다이얼로그 → `DELETE /stories/{storyId}` 순서이고, 다이얼로그 제목·설명·버튼과 결과 토스트 문구는 웹 스토리 옵션 메뉴의 계약을 그대로 씁니다. 이미 삭제된 스토리(404)는 성공으로 간주하고, 성공 시 재조회 없이 목록에서 대상만 제거합니다. 삭제 진행 중에는 다이얼로그 버튼을 잠그고 확인 버튼 라벨을 스피너로 바꿉니다.
...
- **영향.** 메뉴 항목은 현재 "삭제하기" 하나이고, 상세가 생기면 웹처럼 상세 헤더에도 같은 메뉴를 둡니다. 오리지널 카드에는 메뉴를 두지 않습니다 — 내 소유가 아니라 관리할 동작이 없습니다.
...
| 임시 저장 상태 | 닫기 왼쪽에 pill 뱃지. 쓰기 대기·진행 중은 연한 회색 배경·회색 글자의 "임시 저장중", 영속 성공 후에는 연한 브랜드 배경·브랜드 글자의 "임시 저장됨". 저장할 내용이 없거나 쓰기에 실패하면 숨김 | 웹에는 아직 같은 뱃지가 없음 |
...
**완성 직후 채팅 진입은 3-1 계약대로 채팅 생성 → 채팅방 진입입니다.** 완성 성공 시 완성 토스트("스토리가 완성되었어요")를 띄우고 `POST /chats {storyId}`로 채팅을 만들어 채팅방 목적지로 진입합니다(시작 설정 미지정 — 서버가 첫 시작 설정으로 폴백). 웹의 채팅 화면 `replace`는 **퍼널 단계를 모두 걷어내고 채팅방을 쌓는 것**으로 대응해 백스택이 `[메인 탭, 채팅방]`이 되고, 채팅방에서 뒤로가기는 퍼널 진입 전 선택돼 있던 제작 탭으로 복귀합니다 — [`3-1-client.md §3-1-5`](./3-1-client.md#3-1-5-채팅-플레이와-sse-스트리밍)의 "헤더 back은 `/chats`"는 진입 이력이 없을 수 있는 웹 대비이므로 앱은 백스택 pop이 그 역할을 대신합니다. 스토리 완성은 성공했는데 채팅 생성이 실패하면 추가 정보 화면의 인라인 오류로 복귀하고, 재시도는 스토리 완성을 건너뛰고 채팅 생성만 재호출해 스토리를 중복 생성하지 않습니다(3-1 완성 재시도 분기). 채팅방 라우트에는 규칙대로 `chatId`만 실리고 화면이 `GET /chats/{chatId}`로 다시 얻습니다 — 웹의 상세 prefetch 대응입니다. 채팅방 화면 자체는 상세 조회 렌더(제목·프롤로그·턴 이력)까지 구현했고 입력 컴포저·SSE 턴 진행은 잔여입니다(아래 매트릭스 005).
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| FE-SCREEN-001 | 홈 오리지널 스토리 목록 | Phase 1 | 확정 | 구현 | 홈 탭 진입점 필수 · 목록 범위 결정 필요 | 부분 구현 | 오리지널 목록 조회와 카드(ORIGINAL 태그·턴 수 뱃지)·지연 골격·재시도·빈 목록 미노출 구현(§3-3-3 스토리 목록). 카드 탭 상세 진입은 003과 함께 구현하고, 게스트 노출 분기는 비적용(§3-3-1) | §3-3-3 · §3-3-4 |
| FE-SCREEN-013 | 제작·내 스토리 목록 | Phase 1 | 확정 | 구현 | 제작 탭 진입점 필수 · 목록 범위 결정 필요 | 부분 구현 | 제작 진입점·FAB·이어서 만들기 배너에 더해 내 스토리 목록(제목·한 줄 소개·장르 `+N` 뱃지 카드, 골격·재시도·빈 상태)과 카드 더보기 삭제 구현(§3-3-3 스토리 목록). 카드 삭제 진입은 앱 전용 — 웹은 상세 헤더. 페이징·새로고침과 카드 탭 상세 진입 잔여 | §3-3-3 · §3-3-5 |
| FE-SCREEN-002 | 생성 퍼널 기본 | MVP | 확정 | 구현 | 간편 제작 필수 | 구현 | 퍼널 3단계와 서버 연동 구현 — 태그 조회·스토리라인 생성·재생성·평가·스토리 완성 API, 생성·완성 로딩(타자기 문구·지연 힌트)과 실패 복귀, 닫기(X) 헤더·기본 교차 페이드·추가 정보 재선택 경고, 완성 직후 채팅 생성·채팅방 진입(§3-3-3), 이탈 가드(§3-3-5) 포함 | §3-3-3 · §3-3-5 |
| FE-SCREEN-002 | 백그라운드 생성 복귀·제작 임시 저장 | Phase 1 | 확정 | 구현 | 간편 제작 필수 | 구현 | requestId 영속(Room 단일 행)·복구 3초 폴링·`STORY_DRAFT`·`KEYWORD_DRAFT` 300ms 자동 저장·명시적 이탈 즉시 저장·저장 상태 뱃지·제작 탭 배너·재개 다이얼로그 구현. 앱 전용 차이 — 임시 저장 토스트와 배너 닫기(X)는 두지 않음(§3-3-5) | §3-3-5 |
...
| FE-SCREEN-005 | 채팅 기본(SSE 스트리밍·선택지·삭제) | MVP | 확정 | 구현 | 결정 필요 | 부분 구현 | 채팅 생성·상세 조회와 채팅방 진입·조회 렌더(제목·프롤로그·턴 이력)만 구현(§3-3-3). 입력 컴포저·SSE 턴 진행·선택지·삭제 잔여 — SSE 클라이언트 확정(okhttp-sse — §3-3-2) | §3-3-2 · §3-3-3 |
...
| --- | --- | --- | --- | --- | --- |
| FLOW-002 | 스토리 생성 | 013 · 002 · 005 | 결정 필요 | 부분 구현 | 채팅 플레이(005 입력 컴포저·SSE 턴 진행) 잔여 — §3-3-3 |
| FLOW-002A | 일반 제작 | 002 · 003 · 009 | 결정 필요 | 미착수 | 웹도 미구현 — 클라이언트 공통 잔여 |
| FLOW-002B | 스토리 수정 | 003 · 009 | 결정 필요 | 미착수 | 웹도 미구현 — 클라이언트 공통 잔여 |
| FLOW-003 | 탐색 → 플레이 시작 | 001 · 003 · 005 | 결정 필요 | 부분 구현 | 홈 오리지널 목록까지 동작. 카드 탭 상세 진입(003)과 플레이(005) 잔여 |
| FLOW-004 | 채팅 플레이(스트리밍) | 005 | 결정 필요 | 미착수 | SSE 클라이언트(§3-3-2) |
| FLOW-005 | 채팅 이어가기 | 004 · 005 | 결정 필요 | 미착수 | — |
| FLOW-006 | 스토리·채팅 삭제 | 003 · 004 · 005 | 결정 필요 | 부분 구현 | 내 스토리 삭제는 제작 탭 카드 더보기 메뉴로 구현(§3-3-3 스토리 목록 — 앱 전용 진입, 웹은 003 상세 헤더). 상세·채팅 쪽 삭제 잔여 |
...
## 3-3-5. 디바이스 지원·접근성·라이프사이클 — `라이프사이클 확정 · 디바이스 지원·접근성 작성 예정`

지원 OS 최소 버전(빌드 기준 `minSdk` 24 — 제품 지원 선언은 별도 결정), 화면 크기 대응, 접근성(TalkBack·포커스·터치 타깃)은 작성 예정입니다. 앱 라이프사이클 — 간편 제작 퍼널의 이탈 가드·백그라운드 복구 대응은 아래에서 확정합니다.
...
**이탈 가드 — 이탈 지점은 퍼널 3개 목적지 전부입니다.** 헤더 닫기와 시스템 뒤로가기는 같은 이탈 처리로 합류합니다. 입력이 있는 키워드 단계와 생성 결과가 있는 스토리라인·추가 정보 단계는 이미 자동 저장하고, 명시적 이탈 시 아직 대기 중인 쓰기를 즉시 반영한 뒤 나갑니다. 진행 중 레코드(`STORYLINE_GENERATION`·`STORY_COMPLETION`)가 있으면 이를 유지하고 클라이언트 대기만 취소합니다 — 서버는 끝까지 진행합니다. 자동 저장 뱃지가 보존 여부를 계속 알려 주므로 이탈 시 별도 임시 저장 토스트는 표시하지 않습니다. 생성 실패처럼 보존할 결과가 없으면 **소실 경고 다이얼로그**("스토리를 그만 만들까요? 지금 나가면 만들고 있는 내용이 사라져요")를 표시하고 확정 시 이탈합니다. 입력이 전혀 없는 키워드 단계는 저장·경고 없이 이탈합니다.
...
**자동 저장 — 마지막 변경 300ms 뒤, 명시적 이탈은 즉시.** 키워드 입력, 스토리라인 생성 성공·활성 탭·선택, 추가 정보 자유 입력·추천 선택이 바뀌면 직전 예약을 취소하고 마지막 변경 300ms 뒤 현재 스냅숏을 저장합니다. 헤더 닫기·시스템 뒤로가기는 디바운스가 끝나지 않았어도 최신 상태를 즉시 저장합니다. 스토리라인 생성·완성 요청 레코드는 API 호출 **전에 즉시** 저장하고, 생성 성공 결과도 즉시 `STORY_DRAFT`로 바꿉니다. 스토리라인·추가 정보의 지연 초안 쓰기와 요청 단계 쓰기는 mutex·revision 순서로 직렬화하고, 키워드 단계는 생성 시작 전에 예약된 자동 저장을 취소·합류합니다. 따라서 오래된 초안이 최신 진행 중 요청을 덮지 못합니다.

**저장 상태 — 헤더 뱃지가 쓰기 결과를 반영합니다.** 저장할 변경이 생긴 순간부터 실제 쓰기가 끝날 때까지 "임시 저장중"을 유지하고, 성공하면 "임시 저장됨"으로 바꿉니다(색상은 §3-3-3 헤더 표). 쓸 내용이 없거나 쓰기에 실패하면 뱃지를 숨깁니다. 복원 직후에는 디스크에 있는 스냅숏과 화면이 같으므로 "임시 저장됨"으로 시작하며, 복원 자체는 레코드를 소비하지 않습니다. 이후 편집이 다시 300ms 저장 흐름을 시작합니다.

**키워드 단계 임시 저장 — 앱 전용 확장.** 3-1은 키워드 단계를 저장 범위 밖에 두지만, 앱은 키워드 입력(선택 장르·커스텀 장르·주인공과 주변 인물의 이름·성별·특징)을 `KEYWORD_DRAFT` 스테이지로 저장해 재개할 수 있게 합니다. 저장은 입력이 하나라도 있을 때만 하며 진입 시 놓인 빈 주변 인물 섹션은 입력으로 세지 않습니다. 입력을 모두 지우면 기존 `KEYWORD_DRAFT`도 삭제합니다. 활성 카테고리 탭은 저장하지 않아 재개 시 항상 첫 탭에서 시작합니다. 진행 중 레코드(`STORYLINE_GENERATION`·`STORY_COMPLETION`)와 생성 결과 초안은 키워드 스냅숏보다 우선하므로 덮지 않습니다.
...
- **임시 저장 재료** — 선택 스토리라인·활성 스토리라인 탭·추가 정보 입력·추천 선택은 Activity 보존 범위의 생성 스토어(§3-3-3)가 미러로 보관하고 변경 후 300ms마다 `STORY_DRAFT`에 반영합니다. 프로세스가 디바운스 300ms 안에 강제 종료되면 마지막 미반영 변경만 유실될 수 있으며, 그 전에 성공한 스냅숏은 앱 재실행 뒤 복원됩니다.
