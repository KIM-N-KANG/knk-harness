...
[`3-1-client.md §3-1-3`](./3-1-client.md#3-1-3-화면별-스펙)의 화면 인벤토리(FE-SCREEN-001~013)와 흐름 인덱스(FLOW-001~009)를 안드로이드 내비게이션 구조로 대응합니다. 그래프 구조·인증 게이트·메인 탭 셸·화면 전환은 아래에서 확정하고, 딥링크 처리 방식은 `작성 예정`입니다.
...
| FLOW-010 | 푸시 알림 | — (앱 전용) · 008 알림 설정 | **토큰 등록 → 수신·탭 진입 → 수신 동의 설정** | 부분 구현 | 토큰 등록·로그아웃 삭제·권한 요청은 KNK-1133 구현(§3-3-4 푸시 토큰 등록 — `notification` 모듈, dev 에뮬레이터에서 로그인·재시작 `PUT` 204·로그아웃 `DELETE` 204·권한 거부 확인). **수신 동의 설정 화면은 KNK-1135 구현**(§3-3-3 마이 · 알림 설정 — 마이 알림 섹션 진입, 토글 셋 즉시 `PUT`·실패 되돌리기, 야간은 광고 꺼짐이면 비활성, 기기 알림 꺼짐 배너 → 시스템 설정, dev 에뮬레이터에서 조회·갱신 200·되돌리기·회전 유지 확인). 수신 표시·채널·탭 진입은 KNK-1134(이전 계정 알림 차단 조건은 아래 확정, 수신자 검증 방식은 서버 합의 필요) — 상세 스펙 미작성 |
...
- **수신자 검증 방식은 서버 합의가 필요하며 아직 확정하지 않았습니다.** 현재 서버 페이로드에 모든 시나리오 공통 수신자 식별자가 있다고 가정하지 않습니다. KNK-1134에서 수신자를 판별할 계약과 미지원 페이로드 처리 방식을 서버와 확정하고 검증하기 전에는 알림 표시를 출시하지 않습니다. KNK-1133의 토큰 등록 완료와 알림 수신 기능 완료를 구분합니다. 이 문서는 새 API 필드를 확정하지 않습니다.
