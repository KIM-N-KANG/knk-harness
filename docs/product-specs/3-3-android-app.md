...
| 이력      | v0.5 골격을 KNK-772 기술 선택 인터뷰(2026-08-03) 결과로 본문 확정. v0.7 교차 리뷰 반영(2026-08-04) — 테스트 트랙 용어를 비공개 테스트로 통일, 재조회 규칙의 복귀 판정 명확화, 예외 표·흐름 표를 3-1 정본과 정렬, `minSdk` 26 정렬을 W1 목표에 명시. v0.8 아키텍처 리뷰 반영(2026-08-04) — 결과 타입·오류 모델을 `core/domain`으로 이동, core 간 의존 규칙 추가, 앱 수준 세션 상태 전파 경로·반응형 재발급·회전 저장 순서·SSE 무수신 상한(150초)·sealed 스트림 이벤트 정의. v0.9 명명 정리(2026-08-04) — "로그인 게이트"를 "로그인 화면(앱 전용)"으로 개칭, 패키지 `feature/gate` → `feature/login`. v0.10(2026-08-04) — `feature`를 화면 단위에서 도메인 그룹형(`story`·`chat` 하위에 화면 배치)으로 재편, 모듈 승격 단위를 도메인으로 명시. **v1.0 멘토 리뷰 반영(2026-08-04)** — 결정 4건을 뒤집었습니다: ① UI 상태 관리를 Orbit에서 **직접 구현 MVI**로(3요소를 `UiState`·`Intent`·`ReducerEvent`로 재정의, 사이드이펙트를 앱 스코프 헬퍼로 분리), ② 단일 모듈에서 **처음부터 멀티 모듈 12개**로(문자열 리소스를 `:core:ui`에 중앙화), ③ 크래시 수집을 Sentry에서 **Firebase Crashlytics**로(릴리스 R8 활성화 포함), ④ 폰 세로 고정을 해제하고 **전 기기 회전 지원**으로. 범위 변경 1건 — **FCM 생성 완료 알림을 v1에 편입**(서버 계약은 [`4-backend.md §4-3-12`](./4-backend.md) `계획`). 미반영 리뷰 항목은 Room 표현 정정·SSE 정책 요약 표·토큰 만료 이중 시계로, 후속 판에서 다룹니다. **v1.1(2026-08-04)** — App Link 표기를 정합화했습니다. 시점 표현 없이 "선언하지 않습니다"로 적혀 영구 결정처럼 읽히던 4곳을 "v1 미선언 · Phase 3 도입"으로 바꾸고([`3-1-client.md`](./3-1-client.md) 2곳도 동일), Phase 3 딥링크 도입에 대비한 이음매 3건(목적지 보류·라우트 단일 등록처·백스택 방침)을 §3-3-3에 기록했습니다. **v1.2(2026-08-04)** — 참고 오픈소스 아키텍처(`AI-SW-Maestro-OSS/26-mobile-eng-class`) 대조 리뷰를 반영했습니다. ① `compose_stability.conf` 등록 규칙 추가 — `:core:domain`이 `kotlin-jvm` 모듈이라 그 값 객체가 unstable로 취급되어 `ImmutableList` 규칙만으로는 리컴포지션 스킵이 성립하지 않는 문제를 막습니다. ② 앱 스코프 헬퍼 2종의 **인터페이스를 `:core:domain`으로 이동**하고 구현만 `:core:ui`·`:core:navigation`에 남겼습니다 — 교차 참조 금지 규칙과 오류 경로 3단계(`:core:common` 판정 헬퍼)가 충돌해 UI 아래 계층이 안내를 띄울 수 없던 문제를 해소하며, 문자열 리소스 id 대신 오류 타입을 넘기는 제약과 라우트 마커 규칙이 함께 붙습니다. 헬퍼 발행의 `trySend` 유실 정책도 명시했습니다. 미반영 리뷰 항목은 `reduce` 순수성 근거 보강·필수 테스트 항목 분리·명시적 백킹 필드 주의·Baseline Profile 도입 판단으로, 후속 판에서 다룹니다. **v1.3(2026-08-04)** — 앞 판의 미반영 항목 중 셋을 반영했습니다. ① `reduce` 순수성의 근거를 편의에서 **정확성**으로 교체 — `MutableStateFlow.update`가 비교 후 교체 실패 시 재계산하므로 `reduce`가 여러 번 실행될 수 있고, 소비형 필드 해제도 `StateFlow`가 같은 값을 재방출하지 않는 성질 때문에 성립 조건임을 명시. ② 필수 단위 테스트의 "복잡 화면의 상태 전이" 1행을 **`reduce` 상태 전이**와 **소비형 필드 해제** 2행으로 분리 — `Intent` 기준 서술이 코루틴·가짜 구현을 요구해 `ReducerEvent` 분리의 이득을 회수하지 못했습니다. ③ 명시적 백킹 필드(`field =`) 금지를 명시 — 참고 프로젝트 코드를 그대로 옮기면 실험 단계 컴파일러 옵션이 필요합니다. 남은 항목은 **Baseline Profile 도입 판단** 하나입니다. **v1.4(2026-08-04)** — Baseline Profile 판단을 §3-3-7에 결정 기록으로 남겼습니다. **v1 미도입**이며 도입 시점은 출시 후 첫 안정화 구간입니다 — 지금은 효과를 측정할 대상이 없고 시작 경로가 개발 내내 바뀌며 M1이 절대 기한이기 때문입니다. v1에서는 `testTag` 부착만 `SHOULD`로 두고, 후속 과제는 [`roadmap.md §R-5`](../planning/roadmap.md)가 추적합니다. 이 결정이 §3-3-6의 성능 추적 미도입과 별건임을 양쪽에 명시했습니다(런타임 수집 대 빌드 시점 최적화). 이로써 **참고 아키텍처 대조 리뷰(v1.2)의 미반영 항목이 모두 해소**됐고, v1.0 멘토 리뷰의 미반영 3건(Room 표현 정정·SSE 정책 요약 표·토큰 만료 이중 시계)이 남습니다. **v1.5(2026-08-04)** — 토큰 만료 판정을 **이중 시계**로 교정했습니다. 단조 시계(`elapsedRealtime`) 단독 앵커는 **재부팅 시 0으로 초기화**되어 경과 시간을 실제보다 작게 계산하고, 그 결과 만료 토큰이 유효로 판정돼 스펙이 막으려던 사고(익명 통과·주인 없는 콘텐츠)가 그대로 재현됩니다. 단조·벽시계 두 앵커를 함께 저장하고 **경과값이 큰 쪽을 채택**해 재부팅·시계 조작 양쪽에서 안전한 방향으로 틀리게 했으며, 앵커 영속화와 앵커 부재 시 규칙도 정의했습니다. 필수 테스트에 재부팅 케이스를 추가했습니다. **v1.6(2026-08-04)** — SSE 종료 정책을 §3-3-2에 요약 표로 모았습니다. 세 문서에 흩어져 있던 상한 3종(AI 무진행 60초 · 백엔드 전체 상한 120초 · 앱 무수신 150초)과 그 순서 불변식, 이벤트별 종료 처리를 한자리에 배치했습니다. **백엔드 전체 상한 초과가 `error`가 아니라 EOF로 도착한다**는 사실이 긴 문단 속에 묻혀 있어 표에 명시했습니다. 중복된 계약 목록은 요약 표로 흡수했습니다(새 결정 없음, 정보 재배치). **v2.0 멘토 리뷰 반영 — Room 도입으로 결정 전환(2026-08-04)** — v1.0까지의 "Room 미도입"을 뒤집었습니다. 이전 근거("관계형 데이터가 없다")는 **데이터의 모양만 보고 사용성을 보지 않은 판단**이었습니다. 목표를 오프라인 지원이 아니라 **TTI(Time to Interact) 단축**으로 다시 잡고, **채팅 목록·방에 한해 cache-first + sync**를 도입했습니다 — 진입 시 캐시를 즉시 그리고 응답이 오면 교체합니다. 채팅만 고른 이유는 **완료된 턴이 서버에서도 변하지 않아 캐시가 틀릴 여지가 구조적으로 없기** 때문이며, 계속 변하는 서재 목록·크레딧 잔액은 캐시하지 않습니다. 쓰기는 `completed` 후 재조회 결과만, 교체는 통째로, 로그아웃 시 전삭제이며 암호화는 적용하지 않습니다(근거는 §3-3-4 결정 기록). 이 캐시는 네트워크 호출을 추가하지 않아 공통 계약의 "자동 재조회 없음"과 충돌하지 않고, §3-3-5의 "오프라인 미지원"도 유지됩니다. 스크롤 성능을 위한 `LazyColumn` `key` 규칙, 필수 테스트 1종(8종으로), TTI 검수 항목, W2 작업 항목을 함께 추가했으며 확장·계측은 [`roadmap.md §R-5`](../planning/roadmap.md)가 추적합니다. **이로써 v1.0 멘토 리뷰의 미반영 항목이 모두 해소됐습니다**. **v2.1(2026-08-04)** — 캐시 범위와 표시 규칙을 정리했습니다. ① **회원 서재 스토리 목록을 캐시 대상에 추가**했습니다 — v2.0이 홈을 제외한 근거는 "크레딧 잔액을 캐시하면 안 된다"와 "서재가 낡으면 오해를 만든다"였는데, 잔액과 목록은 **다른 필드라 정책을 따로 둘 수 있고** 서재는 본인이 만든 것만 담겨 피드처럼 매번 바뀌지 않습니다. ② **캐시 대상을 화면이 아니라 출처로 재정의**했습니다(`/users/me/*` 본인 소유 리소스). 화면 기준이면 그 화면에 새 데이터가 붙을 때 규칙이 조용히 틀리는데, 스토리 피드가 앱 출시와 **같은 Phase 2**라 실제 위험입니다 — 출처 기준이면 피드가 홈에 붙어도 자동 제외됩니다. ③ **표시 규칙 3종**을 명시했습니다(캐시 있음→실제 내용+갱신 표시 / 캐시 없음→**필드 단위 스켈레톤** / 실패→오류). 스켈레톤은 레이아웃 시프트를 막기 위한 것이며 화면 전체가 아니라 필드에 적용합니다. 스토리 상세는 계속 제외합니다 — 엔딩 배지가 플레이로 변하는 데 반해 TTI 이득이 작습니다. 범위 확대에 맞춰 쓰기·보관 규칙을 목록과 채팅 턴으로 나눴고(목록은 조회 성공 시 기록·최신 1벌, 채팅 턴은 `completed` 후 기록·최근 방 30개), W1 목표의 토큰 문구를 v1.5의 이중 시계와 정렬했습니다. **v2.2(2026-08-04)** — **스토리 상세를 캐시 대상에 편입**했습니다. v2.1이 상세를 제외한 근거 중 "TTI 이득이 작다"는 약했습니다 — 목록을 캐시해 카드를 즉시 그려 놓고 그 카드를 누르면 스피너가 뜨면 경험이 어중간해지고, 홈 → 상세 → 채팅은 가장 흔한 경로(FLOW-003)입니다. 대신 조건 둘을 붙였습니다. ① **소유 판정** — 상세는 `GET /stories/{storyId}`라 인증이 선택이고 남의 스토리도 같은 경로로 오므로 출처만으로 판정할 수 없습니다. **캐시된 서재 목록에 그 `storyId`가 있을 때만** 캐시하며, 피드가 생겨도 남의 스토리는 자동으로 빠집니다. ② **본 엔딩 배지는 캐시에서 제외**하고 스켈레톤으로 둡니다 — 크레딧 잔액과 같은 처리라 새 메커니즘이 필요 없습니다. 서재 목록을 갱신할 때 목록에 없는 스토리의 상세 캐시를 삭제해 소유 판정과 캐시 내용이 어긋나지 않게 했습니다. **v2.3 정합성 리뷰 반영(2026-08-05)** — 캐시 범위를 세 판에 걸쳐 넓히면서 앞판 근거가 남아 생긴 모순과, 정의되지 않은 채 남아 있던 절차를 정리했습니다. ① **Room 결정 기록이 v2.0 상태로 화석화**돼 있었습니다 — 제목이 "채팅을 캐시"였고, 대안 표의 "홈·스토리 상세까지 캐시(채택 안 함)"와 영향의 "홈·스토리 상세로의 확장은 v1 범위 밖"이 v2.1·v2.2가 확정한 대상 표·W1·W2·DoD와 정면으로 충돌했습니다. 제목을 "본인 소유 리소스"로 고치고, 대안 표는 **지금도 비대상인 것**(크레딧 잔액·남의 콘텐츠)으로 교체했으며, 확대 경위와 그 근거를 별도 항목으로 남겼습니다. ② **로그아웃 절차를 §3-3-4에 신설**했습니다 — FCM 해제·세션 폐기·토큰 삭제·캐시 전삭제·`device_id` 재발급이 다섯 곳에 흩어져 있고 순서 제약이 없었는데, 해제 API가 인증 필수이고 토큰이 `device_id`에 귀속되므로 순서가 실제로 강제됩니다. 순서 5단계와 1·2 실패 시에도 3~5를 진행하는 규칙을 못박고, 그 결과 남는 잔여 토큰 행은 **서버가 등록 시 같은 `fcm_token`의 다른 행을 지우는 규칙**으로 닫았습니다([`4-backend.md §4-3-12`](./4-backend.md)). ③ **회원 탈퇴 앱 구현을 W3에 배정**했습니다 — 8/26 구현으로 적혀 있었으나 W4는 검수·제출 전용이라 개발 시간이 없었습니다. ④ **절대 기한을 M1 배포에서 8/14 옵트인 완료로 교정**했습니다 — 유지 기간의 기산점은 배포가 아니라 옵트인이고, 8/28 M2까지 정확히 14일이라 버퍼가 없다는 사실을 명시했습니다. ⑤ 코드 리뷰 체크리스트의 3요소가 Orbit 시절 표기(사이드이펙트)로 남아 금지 규칙과 어긋나던 것, W1에 붙어 있던 "보관 상한"이 상한 없는 목록 캐시를 가리키던 것(상한 30은 W2 채팅 방)을 바로잡았습니다. 교차 문서로는 [`3-1-client.md`](./3-1-client.md)의 법적 문서 "공유 정본 결정 필요" 잔존 2곳과 공유 다이얼로그 확정 버튼 문구(앱은 "공유하기")를 정렬했고, [`roadmap.md`](../planning/roadmap.md)의 깨진 §3-3-7 앵커를 고쳤습니다 |
...
| 로컬 저장(설정·토큰) | **DataStore**(Preferences + 커스텀 직렬화) | 토큰·`device_id`·기기 플래그·퍼널 슬롯은 키-값이거나 단일 레코드라 질의가 필요 없습니다 |
| 로컬 저장(본인 리소스 캐시) | **Room** | 목록·상세·채팅 턴을 항목 단위로 조회·교체·삭제하고, 채팅 방은 오래된 것부터 제거하며 로그아웃 시 전삭제해야 합니다. 목적은 오프라인 지원이 아니라 **TTI 단축**입니다(§3-3-4 로컬 캐시) |
...
**도입하지 않는 것과 그 이유** — WorkManager·Foreground Service(서버 복구 계약과 FCM 알림이 안전망 — §3-3-5), 범용 응답 캐시 계층(본인 리소스 캐시는 예외 — §3-3-4), 원격 Feature Flag(§3-3-7), **MVI 프레임워크**(직접 구현 — 아래), **Gradle 규약 플러그인**(v1 미도입 — 아래 모듈 구조).
...
graph TD
    ...
    subgraph DATA["Data 계층 (:core:data)"]
        ...
        DB["Room<br/>본인 리소스 캐시<br/>서재·채팅 목록 · 내 스토리 상세 · 완료된 턴"]
    end
    C -->|Intent| VM
...
include(":core:data")         // Repository 구현 · Retrofit API · SSE · DataStore · Room 캐시 · 인터셉터 · DI 모듈
...
`:core:data` 내부는 관심사별로 패키지를 나눕니다 — `api`(Retrofit 인터페이스·응답 모델), `sse`(스트리밍 수신), `datastore`(Preferences·암호화 토큰·퍼널 슬롯), `database`(Room 엔티티·DAO — 본인 리소스 캐시, §3-3-4), `repository`(구현), `di`(Hilt 모듈), `interceptor`(식별 헤더·인증·로깅).
...
- 리스트는 `LazyColumn`·`LazyRow`의 `key`에 **안정적인 식별자**(턴 ID·스토리 ID)를 지정합니다(`MUST`). `key`가 없으면 목록이 갱신될 때 항목이 통째로 재구성되어 스크롤이 튀고 프레임이 끊깁니다. 캐시를 서버 응답으로 교체하는 채팅 방(§3-3-4)에서 특히 드러납니다.
...
값이 세 문서에 흩어져 있어 한자리에 모읍니다. **원천은 [`4-backend.md §4-3-3`](./4-backend.md)과 [`3-1-client.md §3-1-5`](./3-1-client.md#3-1-5-채팅-플레이와-sse-스트리밍)이며, 이 표는 옮겨 적은 것입니다.**

| 무엇이 자르나 | 값 | 앱에 보이는 모습 | 앱의 처리 |
| --- | --- | --- | --- |
| AI 무진행 (백엔드↔AI, 이벤트 간) | 60초 | `error` 이벤트(`AI_STREAM_FAILED`) | 실패 확정 → 상세 재조회 |
| SSE 전체 상한 (백엔드→앱, `SseEmitter`) | 120초 | **이벤트 없이 스트림 종료 = EOF** | EOF 분기 → 실패 확정 → 상세 재조회 |
| 무수신 상한 (앱 `readTimeout`) | 150초 | 로컬에서 스트림 종료 | EOF와 같은 분기 |
| 전체 상한 (앱) | **없음** | — | 위 두 층에 위임 |

**세 숫자의 순서가 불변식입니다 — `60 < 120 < 150`.** 앱의 무수신 상한은 백엔드 두 상한의 최댓값(120초)을 여유 있게 넘어야 정상 스트림에서 발화하지 않습니다. 백엔드 상한은 조정된 적이 있으므로(160→120초, KNK-645) **그 값이 바뀌면 150초도 재도출합니다**(`MUST`).

**전체 상한 초과가 `error`가 아니라 EOF로 온다는 점이 특히 중요합니다.** 백엔드는 초과 시 진행 중인 AI 호출을 취소하고 이벤트 없이 스트림을 닫습니다(`onTimeout` → `complete`). 앱에서는 `completed`·`error` 없이 끊긴 것과 구분되지 않으므로 EOF 분기 하나가 두 경우를 함께 처리합니다.

정상·예외 종료의 처리는 다음과 같습니다.

| 사건 | 처리 | 강도 |
| --- | --- | --- |
| `completed` | 누적 텍스트를 버리고 상세를 재조회해 서버 확정본을 표시 | `MUST` |
| `error` | 실패 확정 → 상세 재조회 | `MUST` |
| EOF(`completed`·`error` 없이 종료) | 실패 확정 → 상세 재조회 | `MUST` |
| 사용자 취소 | 조용히 종료. **실패로 확정하지 않습니다** | `MUST` |
| 402 | 스트림이 열리기 전 동기 응답이므로 402 공통 분기로 합류 | `MUST` |
...
- 이벤트별 종료 처리는 위 **SSE 종료 정책 요약**이 정본입니다.
...
- **앱 전용 추가 항목** — 로그인 화면(앱 전용), 당겨서 새로고침(홈·채팅 목록), **로컬 캐시**(회원 서재·채팅 목록·내 스토리 상세·채팅 방의 완료된 턴 — TTI 단축, §3-3-4).
...
| 규칙 | 내용 | 지키지 않으면 |
| --- | --- | --- |
| **선제 재발급**(`MUST`) | 요청 직전 access 토큰의 만료 시각을 확인해 임박(여유 60초)하면 먼저 `POST /auth/token/refresh`를 완료하고 본 요청을 보냅니다. 만료 판정 규칙은 아래 **이중 시계**를 따릅니다 | 만료 토큰이 익명으로 통과해 회원의 생성물이 주인 없는 콘텐츠로 저장되고, 본인 리소스 접근이 403이 됩니다 |
| **이중 시계 만료 판정**(`MUST`) | 로그인·재발급 응답의 `expiresIn`을 수신 시점의 **단조 시계(`elapsedRealtime`)와 벽시계(`currentTimeMillis`) 양쪽에 앵커**해 토큰과 함께 저장하고, 판정 시 두 경과값 중 **큰 값을 채택**합니다 | 한쪽 시계만 쓰면 위 행의 사고가 그대로 재현됩니다(아래 결정 기록) |
| **단조 시계 단독 사용 금지**(`MUST NOT`) | `elapsedRealtime`만으로 판정하지 않습니다 | `elapsedRealtime`은 부팅 후 경과라 **재부팅하면 0으로 돌아갑니다.** 저장된 앵커보다 현재 값이 커지는 순간부터 경과 시간이 실제보다 **작게** 계산되어, 만료된 토큰이 유효로 판정됩니다 |
...
**결정 기록 — 만료 판정에 시계 두 개를 함께 저장(2026-08-04)**

- **배경.** 이전 판은 단조 시계(`elapsedRealtime`) 단독 앵커였고, 근거는 "시계가 틀어진 기기에서 오판하지 않기 위해"였습니다. 의도는 맞지만 **재부팅을 고려하지 않았습니다.** `elapsedRealtime`은 부팅 후 경과 시간이라 재부팅하면 0부터 다시 셉니다. 부팅 후 시간이 저장된 앵커보다 커지면 재부팅 사실이 드러나지 않은 채 경과 시간이 실제보다 작게 계산되고, **만료 토큰이 유효로 판정됩니다.** 앵커를 저장소에 두는지, 프로세스 재시작 후 어떻게 판정하는지도 정의돼 있지 않았습니다.
...
| 시계를 미래로 조작 | 정확 | 실제보다 큼 | 벽시계 채택 → 불필요한 재발급 1회 |

- **영향.** 두 앵커를 토큰과 같은 저장소에 함께 기록합니다(`MUST`). 마지막 행만 손해인데 **불필요한 재발급은 요청 한 번**이고, 반대 방향(만료 토큰을 유효로 판정)은 계정 귀속이 깨집니다 — 틀릴 때 안전한 쪽으로 틀리는 규칙입니다. 프로세스 재시작 후에도 저장된 앵커로 같은 판정을 하며, 앵커가 없으면 만료로 간주해 선제 재발급합니다(`MUST`).
...
| 식별자 | 앱 매핑 |
| --- | --- |
| `device_id` | **앱이 첫 실행 시 생성한 UUID가 정본입니다.** DataStore에 보관하고, API 헤더와 분석 SDK가 모두 이 값을 사용합니다. 로그아웃 시 새로 발급하며, **재발급은 로그아웃 정리의 마지막 단계입니다**(아래 로그아웃 절차) |
...
게스트 서재·사용량 카운터·온보딩 플래그는 앱에 없습니다(§3-3-1).

### 로컬 캐시 — TTI 단축

**목표는 오프라인 지원이 아니라 TTI(Time to Interact, 화면 진입부터 콘텐츠를 볼 수 있을 때까지의 시간) 단축입니다.** 화면에 다시 들어갔을 때 스피너 대신 **마지막으로 본 내용을 즉시 그리고**, 서버 응답이 도착하면 조용히 맞춥니다. 연결이 나쁠 때 캐시가 보이는 것은 부산물이며, **오프라인 동작을 보장하지 않습니다**(§3-3-5 디바이스 지원 범위).

**캐시 대상은 화면이 아니라 데이터의 출처와 소유로 정합니다**(`MUST`). 화면 기준으로 쓰면 그 화면에 새 데이터가 붙는 순간 규칙이 조용히 틀리기 때문입니다.

| 항목 | 규칙 |
| --- | --- |
| 대상 | **본인 소유 리소스**뿐입니다(`MUST`) — 회원 서재 스토리 목록(FE-SCREEN-001), **내 서재에 있는 스토리의 상세**(FE-SCREEN-003), 채팅 목록(FE-SCREEN-004), 채팅 방의 완료된 턴(FE-SCREEN-005) |
| 소유 판정 | 출처가 `/users/me/*`이면 그 자체로 본인 것입니다. **스토리 상세(`GET /stories/{storyId}`)는 출처로 판정할 수 없으므로**(인증 선택·남의 스토리도 같은 경로) **캐시된 서재 목록에 그 `storyId`가 있을 때만** 캐시합니다(`MUST`). 판정은 로컬에서 끝납니다 |
| 비대상 | **남이 만든 콘텐츠·공개 목록(스토리 피드), 크레딧 잔액, 본 엔딩 배지**는 캐시하지 않습니다(`MUST NOT`) |
| 읽기 | 화면 진입 시 캐시를 **즉시 그리고** 동시에 조회를 보냅니다(`MUST`). 응답이 오면 교체합니다 |
| 쓰기 — 목록·상세 | 조회가 성공하면 응답을 기록합니다(`MUST`). 상세는 소유 판정을 통과한 경우에만 기록합니다 |
| 쓰기 — 채팅 턴 | SSE `completed` 후 **상세 재조회 결과만** 기록합니다(`MUST`). 토큰 단위로 쓰지 않습니다(`MUST NOT`) — 스트리밍 중 프로세스가 죽어 그 턴이 캐시에 없어도 재진입 시 서버 응답이 채웁니다 |
| 교체 | 서버 응답이 오면 **통째로 교체**합니다(`MUST`). 캐시는 정본이 아니라 첫 프레임용이므로 병합하지 않습니다 |
| 404 | 해당 항목의 캐시를 삭제합니다(`MUST`). 상세·방 화면에서 받았다면 목록으로 되돌리며, 삭제 성공 시 처리(§3-3-3 화면 전환 규칙)에 합류합니다 |
| 보관 — 목록 | 목록별로 최신 1벌만 둡니다. 상한이 필요 없습니다 |
| 보관 — 상세 | **서재 목록을 갱신할 때 목록에 없는 스토리의 상세 캐시를 삭제합니다**(`MUST`). 웹에서 지운 스토리의 상세가 남지 않고, 소유 판정과 캐시 내용이 어긋나지 않습니다 |
| 보관 — 채팅 턴 | **최근 진입한 방 N개**만 유지합니다(초기값 30). 기간 상한을 두지 않습니다 — 오래 안 들어간 방은 캐시해도 쓰이지 않습니다 |
| 로그아웃 | 캐시를 **전부 삭제**합니다(`MUST`). `device_id`를 새로 발급하는데 데이터가 남으면 다음 사용자에게 노출됩니다. 삭제 시점은 아래 **로그아웃 절차** 4단계입니다 |
| 암호화 | 적용하지 않습니다(아래 결정 기록) |
| 스키마 | 테이블은 화면이 아니라 **도메인(스토리·채팅방·턴) 단위**로 정의하고 UI 모델을 저장하지 않습니다(`MUST`). 화면이 바뀌어도 캐시가 살아남게 하려는 것입니다 |

비대상의 이유는 각각 다릅니다.

- **남이 만든 콘텐츠·공개 목록** — 매번 바뀌는 데이터라 낡은 값이 의미가 없습니다. 스토리 피드는 [`roadmap.md §R-4`](../planning/roadmap.md) Phase 2 범위이며 앱 v1 화면 매트릭스에는 없지만, **홈에 섹션으로 붙더라도 출처가 다르므로 위 규칙이 자동으로 제외합니다.**
- **크레딧 잔액** — 낡은 값을 믿고 생성을 시도하면 402를 맞습니다. 화면 표시는 아래 표시 규칙을 따릅니다.
- **본 엔딩 배지(`reachedEndings`)** — 플레이로 변하는 값이라 캐시하면 방금 도달한 엔딩의 배지가 빠진 채 표시됩니다. **상세의 나머지 필드는 캐시하고 배지만 뺍니다** — 크레딧 잔액과 같은 처리입니다(아래 표시 규칙). 웹에서 제목·소개를 수정한 경우는 잠깐 낡은 값이 보였다가 교체되며, 목록의 낡음과 같은 등급으로 수용합니다.

**본인 소유 목록 중 채팅 목록은 도달 경로이기도 합니다.** 방만 캐시하면 연결이 나쁠 때 캐시한 방을 열 방법이 없습니다.

#### 표시 규칙

캐시 유무와 조회 결과로 세 가지가 갈립니다(`MUST`). 화면마다 다시 판단하지 않습니다.

| 상황 | 표시 |
| --- | --- |
| 캐시 있음 · 갱신 중 | **실제 내용** + 갱신 중 표시(얇은 진행 표시줄) |
| 캐시 없음 · 조회 중 (첫 실행, 크레딧 잔액, 본 엔딩 배지) | **스켈레톤** — 최종 레이아웃과 같은 크기를 차지합니다 |
| 캐시 없음 · 조회 실패 | 오류 상태 + 재시도 |

**스켈레톤은 화면 전체가 아니라 필드 단위로 적용합니다**(`MUST`). 홈에서 스토리 목록은 캐시에서 실제 카드를 그리고 같은 화면의 크레딧 잔액만 스켈레톤이 자리를 지키며, 스토리 상세도 제목·소개·썸네일은 캐시에서 그리고 본 엔딩 배지만 스켈레톤으로 둡니다. 스피너나 빈칸을 쓰지 않는 이유는 **레이아웃 시프트**입니다 — 최종 크기와 다르면 값이 도착하는 순간 주변 요소가 밀립니다.

**갱신 중임을 사용자에게 표시합니다**(`MUST`). 캐시를 먼저 그리면 다른 기기(웹)에서 만든 최신 항목이 잠시 보이지 않고, 느린 연결에서는 그 구간이 길어집니다. 표시가 없으면 사용자가 "내 것이 사라졌다"고 오해합니다.

**이 캐시는 네트워크 호출을 추가하지 않습니다.** 화면 진입 시 조회는 원래 하던 것이고(아래 데이터 소스와 재조회 규칙 1), 바뀌는 것은 응답을 기다리는 동안 화면에 그리는 내용뿐입니다. 따라서 공통 계약의 "자동 재조회 없음"과 충돌하지 않습니다.

**오프라인에서도 입력창을 잠그지 않습니다**(`MUST NOT`). 안드로이드의 연결 상태는 신호가 약한 구간에서도 "연결됨"으로 보고되어 잠금 판정의 근거가 될 수 없습니다. 전송이 실패하면 **입력한 텍스트를 유지**해 재시도할 수 있게 합니다(`MUST`). 입력 잠금은 기존대로 스트리밍 여부로만 판정합니다(공통 계약 FE-SCREEN-005).

**결정 기록 — Room을 도입하고 본인 소유 리소스를 로컬 캐시(2026-08-04 재결정 · 2026-08-05 범위 확대)**

- **배경.** v1.0까지는 "관계형 데이터가 없고 오프라인을 지원하지 않는다"는 이유로 Room을 도입하지 않았습니다. 리뷰에서 두 가지가 드러났습니다. 첫째, **그 근거는 데이터의 모양만 본 것이고 사용성을 보지 않았습니다** — Room은 조인이 필요할 때 쓰는 도구가 아니라 로컬에 데이터를 두고 질의할 때 쓰는 도구입니다. 둘째, 진짜 판단해야 할 것은 "관계형이냐"가 아니라 **"연결이 나쁠 때 사용자가 무엇을 보는가"**였습니다. 목표를 오프라인 지원이 아닌 **TTI 단축**으로 다시 잡자 답이 정해졌습니다.

| 대안 | 채택 안 한 이유 |
| --- | --- |
| 미도입 유지(v1.0 결정) | 진입할 때마다 스피너를 봅니다. 카카오톡 같은 채팅 경험에서 로딩은 서비스가 느리다는 인상으로 직결되고, 그 인상은 이탈로 이어집니다 |
| 오프라인 지원을 목표로 전면 캐시 | 며칠 된 캐시의 유효성, 전송 큐잉, 충돌 해소, 완전성을 모두 감당해야 합니다. 크레딧·삭제·엔딩 판정이 전부 서버에 있어 **큐잉한 전송은 나중에 실패할 수 있고**, 그 실패 처리가 캐시 자체보다 비쌉니다 |
| 크레딧 잔액·남이 만든 콘텐츠까지 캐시 | 잔액은 낡은 값을 믿고 생성을 시도하면 402를 맞고, 남의 콘텐츠·공개 목록은 계속 바뀌어 낡은 값이 의미가 없습니다. 둘 다 지금도 비대상입니다(위 비대상 이유) |
| DataStore + 직렬화로 해결 | 방이 여러 개이고 방마다 턴이 수십 개입니다. 키-값 저장소에 통째로 직렬화하면 **방 하나를 열려고 전체를 역직렬화**해야 하고, 방 단위 조회·오래된 방 제거·전삭제가 모두 수동이 됩니다. 이것이 Room이 필요한 이유입니다 |

- **암호화하지 않는 이유.** 토큰에는 Keystore 암호화를 적용하지만(위 인증 토큰 보관) 로컬 캐시에는 적용하지 않습니다. **토큰은 탈취해 다른 기기에서 쓸 수 있지만 캐시는 그 기기 안에서만 의미가 있고**, 기기를 물리적으로 확보한 공격자는 세션이 살아 있으므로 앱을 열어 같은 내용을 봅니다 — 암호화의 실익이 작습니다. 자동 백업도 비활성화되어 있어(§3-3-7) 백업 유출 경로가 없습니다. 재검토 조건은 민감 콘텐츠 정책이 생기거나 기기 공유가 제품 요구가 될 때입니다.

- **영향.** `:core:data`에 Room 계층이 생기고 `:core:domain`에 캐시 동작을 포함한 Repository 계약이 추가됩니다. TTI를 수치로 재는 계측은 v1 범위 밖이며 후속 과제로 [`roadmap.md §R-5`](../planning/roadmap.md)가 추적합니다 — v1은 검수 항목으로 확인합니다(§3-3-7).

- **범위 확대(2026-08-05).** 초판은 **채팅 목록·방만** 캐시하고 홈과 스토리 상세를 제외했습니다. 그 근거 둘이 모두 약했습니다. ① "서재 목록과 크레딧 잔액은 계속 변한다" — **잔액과 목록을 한 덩어리로 본 것이 오판**입니다. 둘은 다른 필드라 정책을 따로 둘 수 있고(잔액만 스켈레톤 — 위 표시 규칙), 서재는 본인이 만든 것만 담겨 피드처럼 매번 바뀌지 않습니다. ② "상세는 TTI 이득이 작다" — 목록을 캐시해 카드를 즉시 그려 놓고 **그 카드를 누르면 스피너가 뜨면** 경험이 어중간해지고, 홈 → 상세 → 채팅이 가장 흔한 경로(FLOW-003)입니다. 그래서 **대상을 화면이 아니라 출처·소유로 재정의**했고(위 대상 표), 상세에는 조건 둘이 붙습니다 — 캐시된 서재 목록에 있는 `storyId`만 캐시하는 **소유 판정**과, 플레이로 변하는 **본 엔딩 배지 제외**입니다. 확대 후에도 캐시가 틀릴 여지는 열려 있지 않습니다 — 목록·상세는 통째로 교체되고 서재 갱신 시 사라진 상세를 지우므로 낡음이 남지 않습니다.

### 로그아웃 절차

로그아웃은 다섯 가지를 정리하는데(FCM 토큰 해제 · 서버 세션 폐기 · 로컬 토큰 삭제 · 로컬 데이터 삭제 · `device_id` 재발급) 각 항목이 문서 여기저기에 흩어져 있어 **순서를 계약으로 못박습니다**(`MUST`). 순서가 틀리면 각 항목이 막으려던 사고가 그대로 남습니다.

| 순서 | 작업 | 이 순서여야 하는 이유 |
| --- | --- | --- |
| 1 | `DELETE /users/me/devices`로 FCM 토큰 해제 | **인증 필수 경로**(401 — [`4-backend.md §4-3-12`](./4-backend.md))라 로컬 토큰을 지운 뒤에는 호출할 수 없습니다. 서버가 토큰을 `(user_id, device_id_hash)`로 보관하므로 `device_id` 재발급 뒤에 보내면 지우려던 행을 찾지 못합니다 |
| 2 | `POST /auth/logout`으로 서버 세션 폐기 | 본문에 실을 refresh 토큰이 아직 로컬에 있어야 합니다([`4-backend.md §4-3-5`](./4-backend.md)) |
| 3 | 로컬 토큰과 **만료 앵커 2종** 삭제 | 앵커를 남기면 다음 세션의 판정에 옛 값이 섞입니다(위 이중 시계) |
| 4 | 로컬 캐시·퍼널 슬롯 전삭제 | 남으면 다음 사용자에게 이전 회원의 내용이 보입니다(위 로컬 캐시 · §3-3-5 퍼널 슬롯) |
| 5 | `device_id` 재발급과 분석 SDK 사용자 식별자 해제 | 앞 네 단계가 모두 옛 `device_id`·옛 세션을 전제로 합니다(§3-3-6) |

**그래프 전환은 다섯 단계를 모두 마친 뒤에 일어납니다**(`MUST`). 앱 수준 세션 상태 `Flow`(§3-3-3)에 로그아웃을 발행하는 시점이 5 이후여야 합니다 — 토큰 삭제(3)가 곧바로 상태 발행이 되도록 배선하면 4·5가 끝나기 전에 로그인 화면으로 넘어가 캐시 삭제와 겹칩니다. **재발급 실패로 인한 세션 폐기(위 재발급 실패 표)도 같은 절차를 탑니다** — 사용자가 누른 로그아웃과 서버가 밀어낸 로그아웃이 정리 범위가 다르면 안 되기 때문이며, 다만 그 경로는 이미 세션이 죽어 1·2가 실패합니다.

**1·2가 실패해도 3~5는 그대로 진행합니다**(`MUST`). 사용자가 로그아웃을 눌렀는데 네트워크 때문에 로그인 상태로 남는 것이 더 나쁘고, 오프라인에서도 로그아웃은 되어야 합니다. 실패는 사용자에게 알리지 않고 경고 로그만 남깁니다.

그 결과 **1이 실패하면 서버에 이전 회원 귀속 FCM 토큰 행이 남습니다.** 이 잔여는 앱이 아니라 서버가 정리합니다 — 다음 사용자가 같은 기기에서 로그인해 토큰을 등록할 때 서버가 같은 `fcm_token`을 가진 다른 행을 함께 삭제하는 것이 최종 방어선입니다([`4-backend.md §4-3-12`](./4-backend.md) 토큰 등록·해제). 앱은 해제를 정상 경로로 수행하되 이 실패를 재시도하지 않습니다.
...
- **본인 리소스 캐시를 제외하면** 응답 캐시 계층을 두지 않습니다(위 로컬 캐시). 재조회 규칙은 셋이며, 캐시가 있는 화면도 이 규칙을 그대로 따릅니다 — 캐시는 조회를 대체하지 않고 **응답을 기다리는 동안 그릴 내용**을 제공할 뿐입니다.
...
| 대상 | 앱 구현 |
| --- | --- |
| 채팅 공유 발급(FE-SCREEN-005) | 발급 API 호출 후 **OS 공유 시트**로 링크를 전달합니다. 확인 다이얼로그와 발급 계약은 공통 계약을 따르되, 공통 계약이 확정 버튼 문구를 플랫폼 소유로 두므로 앱은 **"공유하기"**(진행 중 접근 이름 "공유 준비 중")를 씁니다 — 웹의 "링크 복사하기"를 그대로 쓰면 실제 결과(공유 시트)와 어긋납니다. 복사 완료 토스트는 표시하지 않습니다 |
...
- **일정** — 8/15 결정, 앱 구현은 **W3(8/19~8/25)**입니다(§3-3-7 마일스톤). W4는 검수·제출 전용이라 개발 시간이 없으므로, 결정이 8/15를 넘기면 최소 대응(웹 탈퇴 요청 페이지 + 앱 진입점)으로 고정합니다.
...
| 네트워크 | 오프라인 미지원 | 연결이 없으면 각 화면의 오류 상태와 재시도로 처리합니다. **로컬 캐시(§3-3-4)는 이 결정을 바꾸지 않습니다** — 목적이 TTI 단축이고, 연결이 없을 때 캐시가 보이는 것은 부산물이라 오프라인 동작을 보장하지 않습니다 |
...
| 토큰 등록 | 로그인 성공 직후와 `onNewToken`에서 서버에 등록합니다. **로그아웃 시 해제하며, 해제가 정리의 첫 단계입니다**(`MUST`, §3-3-4 로그아웃 절차) — 인증 필수 경로라 토큰을 지운 뒤에는 호출할 수 없고, `device_id` 재발급 후에는 서버가 지울 행을 찾지 못합니다. 남으면 이전 회원 귀속 토큰이 다음 사용자 기기로 발송됩니다 |
...
| 항목 | 결정 |
| --- | --- |
| 수집 범위 | **크래시와 ANR만** 활성화합니다. 성능 추적(Firebase Performance)은 도입하지 않습니다 — 지금 그 데이터를 볼 사용처가 없고, 품질 지표는 Play Console의 앱 활성 지표로 시작합니다. 재검토 조건은 성능 문제가 실제 조사 대상이 될 때입니다. 이 결정은 **런타임 수집**에 대한 것이며, 빌드 시점 최적화인 Baseline Profile은 별건으로 §3-3-7이 판단합니다 |
...
| 필수 단위 테스트 대상 | 확인 내용 |
| --- | --- |
| 토큰 재발급 | 만료 임박 판정(**이중 시계 — 재부팅·시계 조작·앵커 없음 3종 포함**), 단일 비행(병렬 요청에서 재발급 1회), 반응형 401 재시도 1회, 실패 분기 3종 |
...
| 로컬 캐시 | 캐시 대상 판정(**상세의 소유 판정 — 서재에 없는 `storyId`는 캐시하지 않음** 포함), 쓰기 시점(목록·상세는 조회 성공 시 · 채팅 턴은 `completed` 후 재조회 결과만), 서버 응답 통째 교체, 404 시 항목 삭제, 서재 갱신 시 사라진 스토리의 상세 캐시 삭제, 방 보관 상한 초과 시 오래된 방 제거, 로그아웃 전삭제(§3-3-4) |
...
| 시작 성능(Baseline Profile) | v1 미도입. 도입 시점은 출시 후 첫 안정화 구간입니다(아래 결정 기록) |
| TTI 계측 | v1 미도입. 로컬 캐시의 효과는 **검수 항목으로 육안 확인**합니다(아래 Definition of Done). 구간별 수치 계측은 후속 과제입니다([`roadmap.md §R-5`](../planning/roadmap.md)) |
| 품질 수치 | Google Play의 나쁜 동작 임계값(사용자 체감 크래시율·ANR율)을 출시 후 기준으로 삼습니다 |

**결정 기록 — Baseline Profile을 v1에 도입하지 않음(2026-08-04)**

- **배경.** Baseline Profile은 시작 경로에서 실행되는 코드 목록을 빌드 시점에 APK로 함께 배포해, 설치 직후부터 그 코드가 미리 컴파일된 상태가 되게 하는 장치입니다. **런타임 수집이 없어 §3-3-6의 성능 추적 미도입 결정과 충돌하지 않습니다** — 사용자 데이터를 보내지 않고 수집 백엔드도 필요 없습니다. 앱의 시작 경로에 **암호화 저장소 복호화와 세션 판정**이 들어 있어(§3-3-3) 효과를 볼 조건 자체는 갖췄습니다.

| 대안 | 채택 안 한 이유 |
| --- | --- |
| v1에 함께 도입 | 지금은 **효과를 측정할 대상이 없습니다** — 화면이 비어 있어 무엇이 얼마나 빨라지는지 근거를 만들 수 없습니다. 시작 경로(로그인·세션 판정·그래프 결정)가 v1 개발 내내 바뀌므로 지금 뜬 프로파일은 출시 전에 낡습니다. 테스터 옵트인 기한이 절대 기한으로 고정된 일정에(아래 출시 일정) 기능이 아닌 최적화로 리스크를 더할 이유도 없습니다 |
| 영구 미도입 | 시작 시간은 Play Console의 품질 지표이고, 도입하면 **시작 시간을 숫자로 재는 수단**이 함께 따라옵니다. 지금은 육안 판단뿐입니다 |

- **영향.** v1에서는 모듈도 시나리오도 만들지 않습니다. 다만 **핵심 화면의 주요 요소에 `testTag`를 붙입니다**(`SHOULD`) — 도입 시 시나리오가 이 태그로 요소를 찾고, UI 테스트에도 그대로 쓰입니다. 나중에 소급해서 다는 것보다 화면을 만들며 다는 편이 쌉니다. 재검토 시점은 v1 출시 후 첫 안정화 구간이며, 후속 과제로 [`roadmap.md §R-5`](../planning/roadmap.md)가 추적합니다. 도입 시 시나리오는 화면의 `testTag`에 의존하므로, **태그를 지우면 시나리오가 조용히 깨진다**는 점을 그때 유지보수 조건으로 함께 기록합니다.
...
**개인 개발자 계정 요건** — 정식 출시 전에 비공개 테스트에서 일정 인원의 테스터를 연속 기간 유지해야 합니다(최신 수치는 Play Console 안내를 직접 확인). 이 요건은 코드 완성도와 무관하게 달력 시간을 소비하므로, 비공개 테스트 배포(**M1**)를 개발 완료보다 앞당겨 배치합니다. **기산점은 트랙 배포가 아니라 테스터 옵트인 완료입니다** — 배포만 해 두고 테스터가 참여하지 않으면 하루도 쌓이지 않습니다. 따라서 절대 기한으로 관리하는 대상은 M1 배포가 아니라 **8/14 옵트인 완료**이며(아래 선행 작업), 여기가 밀리면 정식 출시 신청(M2)도 같은 기간만큼 밀립니다. M1 빌드는 완성본이 아니어도 됩니다 — 요건은 테스터 유지이지 기능 완성이 아니므로, 로그인과 기본 루프만 동작하면 배포하고 이후 빌드를 같은 트랙에 계속 올립니다.

**이 일정에는 버퍼가 없습니다.** 8/14 옵트인 완료에서 M2(8/28 심사 등록)까지가 **정확히 14일**이라, 통상적인 "연속 14일" 요건을 이론적 최솟값으로 채우는 계획입니다. 옵트인이 하루라도 밀리거나 유지 기간 중 테스터 수가 요건 아래로 떨어지면 M2가 그만큼 밀립니다. 대응은 둘입니다 — 테스터를 **요건 인원보다 여유 있게** 모집해 이탈을 흡수하고(8/8 명단), 요건 인원·기간 수치를 명단 확정 시점에 Play Console 안내로 **한 번 더 실측**합니다. 그래도 밀리면 위 개발 조건대로 범위가 아니라 출시일을 조정합니다.
...
- **홈·채팅 목록** — 회원 서재 API 연동, 카드·썸네일·빈 상태·오류 상태, 하단 3탭과 탭 상태 보존, 당겨서 새로고침. **Room 배선(엔티티·DAO)과 목록 캐시** — 진입 시 캐시 선표시와 응답 교체, 갱신 중 표시, 잔액 스켈레톤, 목록별 최신 1벌 유지(상한 없음), 로그아웃 전삭제(§3-3-4).
...
- **회원 탈퇴 진입점**(§3-3-4 `TBD`) — 8/15에 확정되는 B26 결정을 **이 주차에 구현합니다.** 최소 대응(웹 탈퇴 요청 페이지를 인앱 브라우저로 열기)이면 마이 메뉴 한 줄이고, 셀프서브 탈퇴를 택하면 확인 다이얼로그·API 연동·탈퇴 후 세션 정리(§3-3-4 로그아웃 절차 재사용)까지 이 주차 범위입니다. **W4에는 개발 시간이 없으므로**(검수·스토어 자산·제출 전용) 결정이 8/15를 넘기면 최소 대응으로 고정합니다.
...
| 테스터 모집과 옵트인 | 8/8 명단(요건 인원 + 이탈 여유) · **8/14 옵트인 완료 — 절대 기한** | 유지 기간의 기산이 시작되지 않아 M2가 같은 기간만큼 밀립니다. 8/28 M2까지 정확히 14일이라 버퍼가 없습니다(위 개인 개발자 계정 요건) |
| 회원 탈퇴 대응 방식 결정([`4-backend.md §4-8`](./4-backend.md) B26) | **8/15 결정 · W3(~8/25) 구현** | 계정 삭제 요건 미충족은 심사 거절 사유입니다. W4는 검수·제출 전용이라 개발 시간이 없으므로, 결정이 8/15를 넘기면 최소 대응으로 고정합니다 |
...
- [ ] 위 필수 단위 테스트 8종이 존재하고 통과합니다.
- [ ] **캐시가 있는 홈·스토리 상세·채팅 목록·채팅 방에 다시 들어가면 빈 화면이나 스피너가 보이지 않습니다**(TTI 검수 — §3-3-4). 갱신 중 표시가 함께 보이고, 홈의 크레딧 잔액과 상세의 본 엔딩 배지는 스켈레톤이 자리를 지켰다가 값으로 교체되며 레이아웃이 흔들리지 않습니다.
...
- **코드 리뷰 체크리스트** — ① 응답 모델의 필드와 nullable 여부를 [`4-backend.md`](./4-backend.md) 계약과 대조 ② 사용자 노출 문자열은 리소스로 ③ 로그·크래시 리포트에 원문·토큰·공유 식별자 금지 ④ 앱 스코프 사용은 허용 3종 외에는 사유 필요 ⑤ 화면은 `UiState`·`Intent`·`ReducerEvent` 3요소 준수, 화면별 `SideEffect` 채널 금지·1회성 사건은 앱 스코프 헬퍼 또는 소비형 필드(§3-3-2) ⑥ `:feature:*` 모듈 간 직접 참조 금지 ⑦ 새 의존성 추가는 결정 기록 동반.
