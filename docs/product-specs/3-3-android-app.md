...
| FE-SCREEN-008 | 이프 정책 수치 서버 연동(KNK-1096) | Phase 2 | 확정 | 구현 | 결정 필요 | 미착수 | 웹 선행(KNK-1095) 뒤 앱 반영. 화면에 박아 둔 이프 수치를 지우고 `GET /credits/policies` 값을 렌더합니다 — 완성 비용·턴 비용·출석 제목·친구 초대 보조 문구·초대 안내 시트 제목·공유 문구 여섯 자리. 응답 전·실패에는 자리표시 숫자와 쉬머로 그립니다(§3-3-4 이프 정책 수치 패칭). **웹이 서비스 안내의 수치 고지 절을 삭제해 앱이 옮길 대상이 없어졌습니다** — 이전의 "소모량 사전 고지 — 서비스 안내 정책 수치" 행을 이 행이 대체합니다 | §3-3-4 |
...
### 이프 정책 수치 패칭 — `Phase 2 · 미착수`(KNK-1096)
...
**결정 필요 — 보관 위치와 수명.** 조회 결과를 어디에 얼마나 들고 있을지(앱 수명 싱글턴 · 화면 진입마다 재조회 · 프로세스 재시작 시 재조회)는 정하지 않았습니다. 수치를 쓰는 화면이 세 기능 모듈(`:feature:create`·`:feature:chat`·`:feature:my`)에 걸쳐 있어 화면마다 따로 부르면 같은 값을 여러 번 읽습니다.
