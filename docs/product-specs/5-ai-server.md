...
| 기준 코드 | `../manyak-ai` `dev` `a0b19e6071b3`(2026-08-28, Python 3.11 · FastAPI). 컴파일 인물 이미지(KNK-414)·채팅 인물 이미지(KNK-982)·화자 라벨 감지 전환(KNK-1002)이 모두 이 `dev`에 병합돼 단일 SHA로 대조했습니다. |
...
> **진행 상태(2026-08-28).** 아래 ①(화자 라벨 감지)과 ②의 "대사 줄 위 별도 줄·빈 줄" 부분은 manyak-ai PR #95(`a0b19e6071b3`)로 구현돼 위 본문에 반영했습니다. ②의 마커에서 인물 이름 빼기(`[[URL]]`)는 KNK-1025로, ③(이미지 이름 계약)은 KNK-1026(채팅 요청·이벤트·완료)과 KNK-1027(컴파일 응답)로 AI 서버 구현을 마쳐 위 본문과 §5-3-3에 반영했습니다. 백엔드(`story_characters.image_name`·채팅 요청에 싣기)와 프론트(`[[URL]]` 줄 렌더)는 각자 티켓으로 맞춥니다. 아래 "어떻게" 1~8은 모두 구현분입니다. ③의 모양은 백엔드 리뷰(2026-08-28)를 받아 확정했습니다 — 컴파일 응답 `name`은 인물 이름으로 두고 `image_name`을 더합니다(아래 "왜"·"왜 그 방법").
