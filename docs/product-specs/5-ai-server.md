...
| 기준 코드 | `../manyak-ai` `dev` 브랜치 `deb93c43c4dd`(2026-08-28, Python 3.11 · FastAPI). 컴파일 인물 이미지(KNK-414)·채팅 인물 이미지(KNK-982)·화자 라벨 감지 전환(KNK-1002)·마커 `[[URL]]`과 이미지 이름 계약(KNK-1014)이 모두 이 `dev`에 병합돼 단일 SHA로 대조했습니다. |
...
6. 인물 카드의 성별과 외형 6필드로 인물별 이미지를 최대 5개까지 동시에 생성합니다(KNK-414). 한 인물의 이미지 생성이 실패해도 다른 인물 생성과 위 1~5단계의 스토리 결과는 그대로 200으로 반환합니다. 스토리 썸네일 한 장도 이 단계에서 인물 이미지와 동시에 만들 계획입니다(`Phase 2 · 계획`, KNK-1047 — 아래 "스토리 썸네일 생성").
...
graph TD
    ...
    Render --> Image["인물별 이미지 병렬 생성<br/>+ 썸네일 1장(계획, KNK-1047)<br/>(실패해도 계속)"]
    Image --> OK[200 응답]
...
**스토리 썸네일 생성 — `Phase 2 · 계획`(KNK-1047).**
...
**왜 그 방법.** 실패 표현을 "항상 객체"로 하나로 둔 것은, 백엔드가 성공·실패 두 갈래만 보면 되게 하기 위해서입니다(인물 이미지의 빈 배열은 항목별 결과를 만들 수 없을 때의 표현인데, 썸네일은 한 장이라 그 제약이 없습니다). 표지 인물을 백엔드가 아니라 AI 서버가 고르는 것은 컴파일 안에서 만들므로 백엔드가 되돌려 보낼 재료가 없기 때문입니다. 인물 외형이 전부 빈 스토리에서는 지금 이미지 단계가 0초인데 썸네일 때문에 최악 약 180초가 새로 붙을 수 있습니다(A18과 같은 원인). 드문 경우라 이번엔 상한을 두지 않고 A18 후속에서 함께 다룹니다. 컴파일이 없는 일반 제작에는 AI 썸네일이 생기지 않습니다.
...
`prompt/` 아래 11개 템플릿이 전부이고, 썸네일 템플릿 1개가 계획돼 있습니다(D4). 버전 값은 각 파일 frontmatter가 SSOT이므로 이 표에 싣지 않습니다.
...
| `prompt/image/THUMBNAIL-IMAGE-TEMPLATE.md` | 컴파일 썸네일 `Phase 2 · 계획`(KNK-1047) | 단독 — 이미지 모델용, 장르와 인물 0~2명 치환 | `THUMBNAIL_IMAGE` |
...
| `IMAGE_SIZE` | `1024x768` | 인물 이미지 크기(가로 4:3). 썸네일은 이 값과 무관하게 768x1024 고정(`Phase 2 · 계획`, KNK-1047) |
...
| 관측 | 모든 응답에 meta 동봉, `prompt_versions` 키 구성(채팅 턴=7키, 선택지=1키, 스토리라인=1키, 컴파일=컴파일 템플릿+`CHARACTER_IMAGE` 2키, 썸네일 구현 후 `THUMBNAIL_IMAGE` 포함 3키), Sentry 원문 비수집. 애플리케이션·접근 로그는 stdout 한 줄 JSON이고 요청 식별자를 같은 필드로 포함하며, 성공한 헬스체크만 제외하고 실패한 헬스체크·다른 접근 로그는 보존 |
...
| 컴파일 썸네일 `Phase 2 · 계획` | `thumbnail_image`가 항상 객체(null 없음), 세로 3:4(768x1024) WebP, 표지 인물은 외형이 채워진 앞 1~2명이고 그런 인물이 없어도 장르만으로 생성, 썸네일 실패에도 컴파일 200과 인물 이미지 유지, 실패가 Sentry에 원문 없이 기록됨, `meta.prompt_versions`에 `THUMBNAIL_IMAGE` 포함 |
...
| A18 | 이미지 시간 제한이 전체 예산이 아님 | `IMAGE_TIMEOUT=60`은 이미지 한 장의 전체 제한이 아니라 OpenAI SDK 한 번 시도의 제한입니다. SDK가 시간 초과도 최대 2회 자동 재시도하므로 한 장이 약 180초 이상 걸릴 수 있고, 최대 5명 병렬 호출(썸네일 구현 후에는 5명 + 썸네일 1장)의 컴파일 완료도 가장 늦은 호출만큼 밀립니다. 썸네일은 인물 외형이 전부 빈 스토리에서도 호출되므로, 지금 이미지 단계가 0초인 그 경우에도 같은 지연이 새로 붙을 수 있습니다. 현재는 이미지 생성 실패를 컴파일 200으로 흡수하지만, 호출이 끝날 때까지는 응답 자체가 늦어집니다. 텍스트 LLM의 A12와 같은 종류지만 이미지 어댑터는 별도 통로라 따로 추적합니다 | `계획` — 후속 티켓에서 호출부 전체를 하나의 시간 예산으로 묶어 남은 SDK 재시도를 취소합니다(썸네일 호출도 같은 예산에 포함). 현재는 동작을 바꾸지 않고 시도당 제한이라는 의미를 문서·환경 변수 설명에 명시했습니다 |
