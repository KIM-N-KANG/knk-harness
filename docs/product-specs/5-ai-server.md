...
| 기준 코드 | `../manyak-ai` `dev` 브랜치 `d471761c9be1` (2026-07-29, Python 3.11 · FastAPI). LLM 호출 3층 재구성(D13 — KNK-667, PR #67)과 두 번째 어댑터·채팅 자리 차단(KNK-675, PR #69)이 dev에 머지되어 반영됐습니다. 모델 4개 등록과 등록부 확장, Anthropic 자동 계측 제거(KNK-703, `8e84a34030dd`)는 **dev 머지 대기 상태로 선반영**했습니다. 운영 릴리스는 `v0.2.2`(main `4624fc2`) |
