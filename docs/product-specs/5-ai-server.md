...
| 기준 코드 | `../manyak-ai` `dev` 브랜치 `62f203bf0fe8` (2026-07-29, Python 3.11 · FastAPI). LLM 호출 3층 재구성(D13 — KNK-667, PR #67), 두 번째 어댑터·채팅 자리 차단(KNK-675, PR #69), 모델 4개 등록과 등록부 확장·Anthropic 자동 계측 제거(KNK-703, PR #70)가 모두 dev에 머지되어 반영됐습니다. 운영 릴리스는 `v0.2.2`(main `4624fc2`) |
...
| A14 | 스냅샷이 있는 모델을 별칭으로 등록해 둠 | 공급자가 스냅샷(특정 시점 버전에 못 박은 이름)을 내는 모델은 **그 이름으로 등록하고 모델 이름 변수에도 그 이름을 적기로** 정했습니다(2026-07-29, [§5-4-3](#5-4-3-모델과-파라미터)). 아직 등록부는 `gpt-5.4-mini`를 별칭으로 올려 두었고, 스냅샷 `gpt-5.4-mini-2026-03-17`은 값으로 적어만 두고 호출에 쓰지 않습니다. **지금 걸리는 자리는 없습니다** — 실제로 고른 세 모델은 모두 DeepSeek이고 이 공급자는 스냅샷을 아예 발행하지 않습니다(2026-07-29 확인). `claude-sonnet-5`는 이름 자체가 스냅샷이라 이미 규칙에 맞고, GPT-5.6 둘은 스냅샷이 없습니다 | `계획` — 이 모델을 실제로 고를 때(A7) 등록 이름을 스냅샷으로 바꿉니다. 그때 은퇴 날짜도 함께 정합니다 — 스냅샷은 은퇴하면 404가 되고, 등록되지 않은 모델은 기동에서 막히므로(D13) 은퇴일이 지나면 배포가 실패합니다 |
