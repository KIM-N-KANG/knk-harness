...
`Phase 1 · 계획` 상태입니다. 계약은 확정됐고 구현은 남았습니다.
...
| KNK-420 | D11 | 채팅 런타임에 주요 사건·엔딩 반영 — 계약 확장·판정 호출·배선 | 구현 |
| KNK-422 | D11 | 컴파일 프롬프트에 로어북 반영 | 구현 |
| KNK-465 | D11 | 컴파일 엔딩 계약을 이름 기반·달성 조건 스펙에 맞춰 재작업 | 구현 |

> KNK-413·417·420·422·465는 이 문서의 기준 코드 SHA **이후** `dev`에 머지된 구현입니다. 본문의 해당 기능 서술과 `Phase 1 · 계획` 라벨에는 아직 반영되지 않았으며, 문서 현행화는 KNK-517이 담당합니다.
...
- 주요 사건·엔딩 판정 호출(D11)도 채팅 턴 내부의 LLM 호출이지만, 신규 feature 값을 만들지 않고 관측을 `chat_response`로 합산합니다. feature 값 신설은 관측 카탈로그 개정이 필요한 사안이라 보류했습니다([`6-analytics.md`](./6-analytics.md)).
...
이 흐름의 분기를 그림으로 보면 다음과 같습니다. 특히 실패가 502로 끝나는 블록과 빈 배열 200으로 끝나는 블록의 차이가 드러납니다(주요 사건·엔딩 분기는 `Phase 1 · 계획`):
...
graph TD
    ...
    Refill --> Re{"그래도 비어 있나?"}
    Re -->|아니오| Render
    Re -->|"예 — 필수 4필드·시작 설정·추천 입력"| F502[502 실패]
    Re -.->|"예 — 주요 사건·엔딩 블록"| Empty["해당 블록만 빈 배열로 두고 200<br/>(프롬프트 점검 신호 로깅)"]
```
...
- **주인공 이름 반영.** 세분 JSON의 주인공 이름(시작 설정의 이름 `name`과 동일 값)을 주인공 통글(`user_role_setting`)의 `## 호칭`에 명시 포함해, 채팅이 주인공을 이름으로 일관 지칭하게 합니다. 이름 불일치·미반영은 평가 지표 `setting_consistency`로 감시합니다([§5-6](#5-6-운영과-관측)).
- 엔딩 최소 턴 수(`min_turns`)는 LLM이 이야기 밀도에 맞춰 제안하고, 허용 범위를 벗어난 값은 코드가 경계값으로 잘라 맞춥니다(클램프). 범위 수치는 구현 시 실측으로 확정합니다(`계획`).
- **실패 처리.** 주요 사건·엔딩 블록은 부분 재호출(refill, 최대 2회) 후에도 온전하지 않으면(엔딩은 해피·노말·배드 3개가 채워지지 않으면 불완전) **해당 블록만 빈 배열로 두고 200을 반환**합니다. 부가 요소 때문에 스토리 본체 생성까지 실패시키지 않기 위해서입니다(선택지 폴백과 같은 원칙). 따라서 컴파일의 엔딩 목록(`story_endings`)은 0개 또는 3개입니다. 백엔드 저장은 시작 설정당 최대 10개까지 허용합니다([`4-backend.md §4-3-8`](./4-backend.md)). 빈 블록 응답은 프롬프트 점검 신호로 로깅합니다. 필수 4필드·시작 설정의 502 정책은 기존과 같습니다.
...
sequenceDiagram
    ...
    alt 본문 생성 성공
        ...
        AI->>AI: 화자 볼드 정규화(D7)
        AI->>LLM: 선택지 호출 — JSON 모드(§5-3-5)
        LLM-->>AI: 선택지(3개 보장은 D7)
        AI-->>BE: completed — aiOutput·choices 3개·합산 meta
    else 본문 스트림 실패
...
**주요 사건·엔딩 진행 — `Phase 1 · 계획`.** D11이 정한 분담의 와이어 계약입니다. 백엔드의 재료 구성·상태 저장 규칙은 [`4-backend.md §4-3-10`](./4-backend.md)이 소유합니다.
...
- **엔딩 판정.** 요청에 엔딩 후보가 실려 있으면 각 후보의 달성 조건을 정성 판정합니다. 달성 조건은 목적과 거쳐온 주요 사건을 한 문장으로 서술한 자연어 하드 조건이며, 특정 사건 경유를 강제하지 않습니다. 거쳐온 사건 목록은 판정의 맥락으로만 참고합니다. 충족한 후보가 있으면 현재 맥락과 에필로그(출력 가이드)를 반영한 엔딩 응답을 본문으로 생성하고, 도달한 엔딩의 이름을 완료 메타에 싣습니다. 미충족이면 억지 수렴 없이 일반 턴으로 진행합니다. 최소 턴 수 판정과 채팅당 최초 1회 가드는 백엔드가 담당해 요청에서 이미 걸러져 옵니다.
...
- **구성 기준은 주요 사건 기반입니다(`Phase 1 · 계획`).** 선택지 3개를 ① 현재 향하는 목표 사건 방향 1 + ② 아직 향하지 않은(미완결·비목표) 주요 사건 방향 1 + ③ 사용자 입력 맥락 기반 1로 구성합니다(D11이 정한 대로 강제 수렴 대신 선택지로만 방향 제안). 주요 사건이 없는 스토리, 또는 모든 사건이 완결된 채팅은 기존 다양성 규칙으로 폴백합니다. 선택지 템플릿은 이를 위해 주요 사건·목표 사건·거쳐온 사건을 치환 재료로 함께 받습니다.
...
3개 보장 흐름(D7)은 이렇습니다. 첫 호출이 부족하면 "기존 제외, 모자란 개수만" 방식으로 누적 재호출합니다(최대 2회). 그래도 부족하면 중립 행동 3종 폴백으로 채워 정확히 3개를 만들고, 3개를 넘으면 앞에서부터 3개만 남깁니다. 관측은 chat 로깅 메타(`meta`)에 합산됩니다. 토큰 수는 본문과 선택지의 합으로 싣고, 재호출 횟수(`retry_count`)에는 선택지 재호출 횟수를, 프롬프트 버전(`prompt_versions`)에는 6레이어와 선택지 버전을 함께 싣습니다.
...
`Phase 1 · 계획`으로 슬롯이 늘어납니다. 주요 사건·엔딩 진행(D11)을 위해 STORY 레이어에 `{{main_events}}` · `{{target_main_event}}` · `{{endings}}` 슬롯이 추가됩니다(재료는 [§5-3-4](#5-3-4-채팅-턴)의 요청 추가 필드). 선택지 템플릿도 주요 사건 재료를 함께 받습니다([§5-3-5](#5-3-5-선택지-생성-내부-호출)). 슬롯 이름·배치의 최종 확정은 manyak-ai `spec/chat/` 정본이 소유합니다.
...
- 채팅 본문의 출력 토큰 상한(`max_tokens`)은 분량 고정(약 600자, [§5-3-4](#5-3-4-채팅-턴)) 도입 시 초과 방지용으로 함께 확정하기로 했으나, 분량 고정 규칙만 먼저 반영되고 상한은 미확정으로 남았습니다. 수치는 실측으로만 정합니다(D9). 이 간극은 [§5-7](#5-7-검수-체크리스트) A8이 추적합니다. LLM 모델 변경·성능 비교(같은 표 A7)도 D9 절차를 따르며, 결정 전까지 현재 값을 유지합니다.
...
- 읽힌 버전은 모든 응답 meta의 프롬프트 버전 목록(`prompt_versions`)에 실립니다(D10). chat은 6레이어 + `NEXT_ACTIONS`의 다중 키 dict이고, story는 해당 템플릿 1키 dict입니다. 백엔드는 이를 JSONB로 적재합니다.
- 프롬프트·명세의 변경 이력은 git이 담당합니다. manyak-ai 레포의 pre-commit 훅이 브랜치당 1회 버전을 자동으로 올립니다(로컬 전용).
...
graph LR
    ...
    L2 -->|복구| OK
    L2 -->|잔존| L3["3차 — 폴백·흡수<br/>선택지 폴백 · 사건·엔딩 빈 배열 · 판정 null"]
    L3 -->|복구| OK
    L3 -->|잔존| Split{어느 경로인가?}
...
- 주요 사건·엔딩 판정 호출(D11) 실패도 같은 흡수 원칙을 따라 위 표에 도달하지 않습니다. 실패하면 판정 메타 3필드를 null로 돌려보내고 턴은 정상 완료합니다. 다만 판정 결과는 선택지처럼 폴백으로 지어낼 수 있는 값이 아니라서, 재호출 없이 null로만 흡수합니다([§5-3-4](#5-3-4-채팅-턴)).
...
| A6 | 주요 사건·엔딩 진행 규칙 | 팀 결정(2026-07-04)의 계약·프롬프트 규칙 정의 완료 — D11(판정은 AI · 상태는 백엔드 · 강제 수렴 금지), 요청·응답 확장([§5-3-4](#5-3-4-채팅-턴)), 컴파일 확장([§5-3-3](#5-3-3-스토리-컴파일)), 선택지 구성([§5-3-5](#5-3-5-선택지-생성-내부-호출)), 분량 600자. 설계 결정 항목도 확정 — `main_events`는 매 턴 전체 전달, `key_sentence` 판정 주체는 AI, 목표 사건 상태는 백엔드 저장([`4-backend.md §4-3-10`](./4-backend.md)), 복수 매칭은 개연성 최고 1건, 이탈 턴은 카운터를 늘리지 않고 유지. 구현(프롬프트·스키마·조립)은 남음 | `Phase 1 · 계획` — 구현 티켓 KNK-413·417·420에서 반영, 평가는 KNK-299([§5-6](#5-6-운영과-관측)) |
