...
**구현.** [§5-3-5](#5-3-5-선택지-생성-내부-호출)에 있습니다. 개수 보장은 D7이 담당합니다. `Phase 1 · 계획`(KNK-622) — 2026-07-20 팀 합의로 분리 호출을 전용 엔드포인트로 승격합니다(분리 원칙은 유지·강화, [§5-3-5](#5-3-5-선택지-생성-내부-호출)).
...
- 컴파일(`POST /story/compile`)도 같은 재료(`background_tags` · `supporting_characters[]`)를 받습니다. 프롬프트 반영(태그 귀속 규칙의 배경 축 확장 포함)은 구현 시점에 확정합니다.
...
role 변환: 계약·저장은 대문자(`USER`/`ASSISTANT`), LLM 호출 직전 소문자 변환은 AI 내부 구현입니다([`0-glossary.md §0-4`](./0-glossary.md)). `Phase 1 · 계획`(KNK-622) — 위 흐름의 선택지 호출은 전용 엔드포인트로 분리 예정입니다([§5-3-5](#5-3-5-선택지-생성-내부-호출)).
...
**선택지 전용 엔드포인트 분리 — `Phase 1 · 계획`(KNK-622).** 2026-07-20 팀 합의로 선택지 생성을 `/chat/turns` 내부 2번째 호출에서 전용 엔드포인트로 분리합니다. 요청 flag로 선택지 동시 생성 여부를 판단하는 방식은 검토 후 폐기했고(추천 입력을 끈 채 본문을 받은 뒤 다시 켜는 경로에 결국 전용 API가 필요), 선택지는 항상 별도 API로 생성합니다. 토글 on/off는 프론트엔드가 이 API를 호출하는지로 자연스럽게 처리됩니다([`3-frontend.md §3-6`](./3-frontend.md)). 구현 전까지 위 내부 호출 계약이 유효합니다.

- **엔드포인트(확정 제안)** — `POST /chat/choices`(동기 REST, JSON 응답). 기존 선택지 생성 함수(generate_choices)를 재사용하고 엔드포인트만 추가합니다.
- **요청** — 채팅 턴 요청([§5-3-4](#5-3-4-채팅-턴)의 전체 재료 — 통글 4필드·시작 설정·주요 사건·목표 사건·거쳐온 사건·엔딩·History·summary·user_input)에 `ai_output`(방금 생성된 본문)을 더한 스키마입니다. AI는 무상태(D2)라 첫 요청의 재료를 들고 있지 않으므로, 백엔드가 턴 요청과 같은 조립 로직으로 재료를 다시 실어 보냅니다([`4-backend.md §4-3-3`](./4-backend.md)). 단 `history`는 메인 턴 요청과 동일한 스냅샷(이번 턴 제외)이어야 합니다 — 이번 턴을 포함해 조립하면 방금 장면이 `history`와 `ai_output`에 중복으로 실립니다(절단은 백엔드 소유).
- **응답** — `{choices: string[3], meta}`. 3개 보장(누적 재호출 최대 2회 + 폴백)과 주요 사건 기반 구성 규칙은 그대로이며, `meta`는 이 호출의 토큰·`retry_count`만 담습니다(본문·판정과의 합산 폐지).
- **`/chat/turns` 변경** — 선택지 호출을 제거하고 판정 호출(D12)은 유지합니다(판정은 토글 대상이 아니고 백엔드가 상태로 저장하는 값). `completed`의 meta에서 선택지 몫(토큰·`retry_count`)이 빠지고, `choices` 필드는 계약 파괴를 피해 항상 빈 배열로 유지합니다(필드 제거는 추후 SSE 구조 개편 시 함께).
- **후순위** — `completed`를 본문만 담아 즉시 발행하고 판정을 후속 이벤트로 빼는 SSE 구조 변경(`token → completed → judgement`)은 판정이 매 턴 실행되기 시작하면 재논의합니다. 현재는 판정 재료가 없는 턴이 많아 판정 스킵(D12)으로 대기가 거의 없습니다.
