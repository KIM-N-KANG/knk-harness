...
**인물 단위 입력 반영(`Phase 1 · 계획`, KNK-834).** 주변 인물 세트(KNK-621)를 주인공까지 넓힌 2026-08-16 합의분입니다. 인물을 특징 태그 묶음이 아니라 인물 단위(이름·성별·특징)로 입력받아, 사용자가 정한 인물이 그대로 이야기에 등장하게 합니다. 스토리라인·컴파일 두 요청에 함께 적용되며, 구현 전까지 위 현행 계약이 유효합니다.
...
- **성별 칸 추가(`Phase 1 · 계획`, KNK-834).** 인물 단위 입력으로 성별이 사용자 선택값이 되므로, 서술에 녹여 흐리게 두지 않고 통글에 명시 칸을 둡니다. 인물 카드에 `### 성별`, 주인공 통글에 `## 성별` 항목이 추가됩니다. 통글은 문자열 하나로 프롬프트에 그대로 끼워지므로 백엔드 저장·채팅 계약은 바뀌지 않습니다.
