...
- **요청** — 채팅 턴 요청([§5-3-4](#5-3-4-채팅-턴)의 전체 재료 — 통글 4필드·시작 설정·주요 사건·목표 사건·거쳐온 사건·엔딩·History·summary·user_input)에 `ai_output`(방금 생성된 본문)을 더한 스키마입니다. AI는 무상태(D2)라 첫 요청의 재료를 들고 있지 않으므로, 백엔드가 턴 요청과 같은 조립 로직으로 재료를 다시 실어 보냅니다([`4-backend.md §4-3-3`](./4-backend.md)).
