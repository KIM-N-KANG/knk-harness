...
| 기준 코드 | `../manyak-ai` `dev` 브랜치 `5ebde4977191` (2026-07-21, Python 3.11 · FastAPI) |
...
**구현.** [§5-3-5](#5-3-5-선택지-생성-별도-api)에 있습니다. 개수 보장은 D7이 담당합니다. **AI 서버만 구현 완료**이며(KNK-625), 백엔드 호출 전환과 프론트엔드 토글은 미착수입니다(KNK-622 — [`4-backend.md §4-3-3`](./4-backend.md) · [`3-frontend.md §3-6`](./3-frontend.md), [§5-7](#5-7-검수-체크리스트) A11).
...
- `choice_generation`은 전용 엔드포인트입니다(D6, KNK-625 — AI 서버만 구현 완료). 이전에는 채팅 턴 내부의 2번째 LLM 호출이라 관측도 `chat_response`에 합산됐지만, API가 분리되면서 응답 메타도 이 호출의 몫만 담습니다([§5-3-5](#5-3-5-선택지-생성-별도-api)). 백엔드가 이를 `ai_call_logs`의 별도 행으로 적재하는 것은 `Phase 1 · 계획`입니다(KNK-622 — [`4-backend.md §4-7`](./4-backend.md) · [`6-analytics.md`](./6-analytics.md)).
...
선택지는 `completed` 이후 별도 API로 요청하는 계약이며, **백엔드·프론트엔드 반영은 미착수라 현재 트래픽에는 이 호출이 없습니다**(`Phase 1 · 계획`, KNK-622 — [§5-7](#5-7-검수-체크리스트) A11). 배선이 끝나면 흐름은 다음과 같습니다. 본문이 실패한 턴에는 호출하지 않습니다(이어붙일 본문이 없습니다).
...
sequenceDiagram
    ...
    Note over FE,LLM: Phase 1 · 계획 — 백엔드·프론트 미착수(A11)
...
**구현.** 전용 엔드포인트 `POST /chat/choices`(동기 REST, JSON 응답)입니다. `Phase 1 · 구현`은 AI 서버 한정이며, 백엔드가 이 API를 호출하도록 전환하는 작업은 미착수입니다(KNK-622 — [§5-7](#5-7-검수-체크리스트) A11). 백엔드가 턴 완료·저장 후 호출하는 계약이며, 요청에 실려 온 방금 본문(`ai_output`)과 재료를 독립 프롬프트(선택지 템플릿, 6레이어 조립에 들어가지 않음)에 치환해 JSON 모드로 호출합니다.
...
관측은 이 호출의 몫만 담습니다. `meta`는 다른 응답과 같은 구성(모델·프롬프트 버전·공급자·입출력 토큰·`retry_count` — [§5-6](#5-6-운영과-관측))이며, `retry_count`는 선택지 재호출 횟수(0~2), `prompt_versions`는 선택지 버전 키 하나입니다. 토큰은 **성공한 호출분만** 합산되고, 예외로 끝난 시도의 usage는 회수하지 못해 빠집니다([§5-6](#5-6-운영과-관측)). 본문·판정과의 합산은 폐지됐고, 백엔드가 이를 `ai_call_logs`의 `choice_generation` 행으로 적재하는 것은 `Phase 1 · 계획`입니다(KNK-622).
...
| 선택지 생성 | US-6-4(선택지 활용) · US-6-12(주요 사건 방향 제안, [§5-3-5](#5-3-5-선택지-생성-별도-api)). 전용 API 분리는 AI만 구현되고 3자 배선은 `Phase 1 · 계획`([§5-7](#5-7-검수-체크리스트) A11) |
...
| A11 | 선택지 전용 API의 3자 배선 미완 | AI 서버는 선택지를 `POST /chat/choices`로 분리 완료(KNK-625, `dev` 머지)했으나 백엔드 호출 전환·프론트엔드 트리거는 미착수입니다(KNK-622). 그 사이 AI만 릴리스되면 `completed.choices`가 빈 배열이라 백엔드가 저장할 선택지를 받지 못해 화면에서 선택지가 사라집니다 — A2와 같은 3자 릴리스 정렬 사안. 더해 타임아웃이 역전돼 있습니다: 백엔드 `CHOICE_GENERATION` 30초([`4-backend.md §4-7`](./4-backend.md)) < AI 60초/호출(누적 재호출 시 그 배수, [§5-4-3](#5-4-3-모델과-파라미터)) — A1과 같은 유형 | `계획` — 백엔드·프론트 반영 후 3자 릴리스 순서 확정(AI 선행 릴리스 금지). 타임아웃 매트릭스는 A1과 함께 정렬 |
