...
| 기준 코드 | `../manyak-ai` `dev` `a0b19e6071b3`(2026-08-28, Python 3.11 · FastAPI). 컴파일 인물 이미지(KNK-414)·채팅 인물 이미지(KNK-982)·인물명 라벨 감지 전환(KNK-1002)이 모두 이 `dev`에 병합돼 단일 SHA로 대조했습니다. |
...
| `Phase 2 · 구현` | Phase 2 범위. AI 서버 구현 완료 | 주인공 이름·성별 주입(KNK-838, [§5-3-3](#5-3-3-스토리-컴파일)). 성별 칸 추가(KNK-838). 인물별 이미지 생성(KNK-414, [§5-3-3](#5-3-3-스토리-컴파일)) — 컴파일 시 외형 필드로 이미지를 만들어 base64로 반환. 채팅 인물 이미지 출력(KNK-982, [§5-3-4](#5-3-4-채팅-턴)) — 인물의 대사마다 실시간 이미지 이벤트와 저장 마커를 만듦. 인물명 라벨 감지로 전환(KNK-1002, [§5-3-4](#5-3-4-채팅-턴)) — LLM 태그 대신 AI 서버가 `인물명:` 줄을 찾아 이미지를 붙이고, 저장 마커는 대사 줄 위 별도 줄에 둠 |
| `Phase 2 · 계획` | Phase 2 범위. 미구현, 방향만 합의됨 | 요약(`summary`) 생성(백엔드 소유) — AI의 수신 계약은 구현되어 빈 문자열로 동작 중. 인물 단위 입력(KNK-834, [§5-3-2](#5-3-2-스토리라인-생성)). 스토리 썸네일 AI 생성 — 별도 엔드포인트로 컴파일 후 비동기 호출. 채팅 인물 이미지 계약 정리(KNK-1002 후속, [§5-3-4](#5-3-4-채팅-턴) 변경 계획) — 저장 마커를 `[[URL]]`로 줄이고 이미지 이름(`image_name`/`imageName`)을 계약에 더함 |
...
sequenceDiagram
    ...
    alt 본문 생성 성공
        loop 생성이 끝날 때까지
            LLM-->>AI: 토큰 델타
            alt 줄 머리 인물명 라벨(`인물명:`) 감지 — 매핑에 있는 이름(KNK-1002)
...
**채팅 인물 이미지 — `Phase 2 · 구현`(상위 KNK-982, 요청 계약 KNK-989 · 저장과 재입력 정리 KNK-992 · 인물명 라벨 감지 전환 KNK-1002: 파서 KNK-1005 · 프롬프트 KNK-1006 · 옛 태그 처리 제거 KNK-1007).**

**무엇.** 채팅 답변에서 인물이 말하는 순간마다 그 인물의 이미지를 보여줍니다. 한 턴에서 여러 인물이 말할 수 있고, 같은 인물이 지문을 사이에 두고 다시 말할 수도 있습니다. 이미지가 저장된 인물의 대사에는 매번 이미지 이벤트를 하나씩 만들며, 이미지가 없는 단역·배경 인물도 대사는 할 수 있지만 이미지 이벤트는 만들지 않습니다. 이미지를 붙일 자리는 **AI 서버가 출력에서 직접 찾은 `인물명:` 줄(인물명 라벨 — 용어집의 "화자 라벨"과 같은 뜻)**이고, LLM에게 이미지 관련 표시를 따로 쓰게 하지 않습니다(KNK-1002).
...
- **감지 기준.** 줄 맨 앞(들여쓰기 허용)에서 볼드 기호(`**`)를 걷어낸 뒤 남은 글자가 채팅 요청 `character_images[].name`과 **글자 그대로 같고** 바로 콜론(`:`)이 따르면 인물명 라벨입니다. 이름 비교는 앞뒤 공백만 제거하고 대소문자·별칭 보정은 하지 않습니다. 볼드 기호와 이름 사이, 이름과 콜론 사이 공백은 최대 2칸까지 인정합니다. 이름이 겹치면(`세린`·`세린아`) 더 긴 이름을 먼저 대조합니다.
- **실시간 처리.** 줄이 시작될 때마다 인물명 라벨인지 확정될 때까지 글자를 잠시 모읍니다. 등록된 어떤 이름과도 앞글자가 맞지 않는 줄과 `*`로 시작하는 지문 줄은 그 글자에서 바로 내보내므로 체감 지연은 없습니다. 모으는 상한은 들여쓰기를 뺀 41자(볼드 기호 4 + 이름 30 + 공백 6 + 콜론 1)이며, 상한을 넘으면 라벨이 아닌 것으로 보고 모아둔 원문을 내보냅니다. 인물명 라벨이면 `character_image` 이벤트(`{name, imageUrl}`)를 먼저 보내고, 볼드를 뗀 `인물명:`을 `token`으로 내보냅니다. 실시간 `token`에는 마커가 들어가지 않습니다.
...
- **저장 마커.** 턴이 끝나면 완료 본문에서 볼드를 뗀 뒤, 위와 같은 규칙으로 인물명 라벨을 다시 찾아 그 대사 줄 **위에** `[[인물이름:URL]]` 한 줄을 넣고 마커와 대사 사이에 빈 줄을 하나 둡니다(프론트가 마커 줄을 통째로 이미지로 바꾸기 쉽게 — 프론트 요청). 대사 줄의 들여쓰기는 그대로 남습니다. 같은 순서로 `completed.characterImages[]`(`{name, imageUrl}`)를 만듭니다. 실시간과 저장이 같은 정규식을 쓰므로 정상 입력에서 이벤트 개수와 마커 개수가 같으며, 이를 테스트로 고정합니다(극단 입력의 예외는 A22).
...
**왜 그 방법.** LLM이 URL을 직접 출력하는 방식은 쓰지 않았습니다. URL이 길어 매 턴 입력 토큰을 늘리고, LLM이 한 글자만 바꿔도 이미지가 깨지기 때문입니다. LLM에게 짧은 내부 태그를 쓰게 하는 방식(KNK-982)은 실측에서 태그 누락이 재현돼 버렸습니다 — 프롬프트를 더 세게 쓰는 안도 검토했지만, 누락은 확률적으로 재발하고 발견도 사용자 화면에서만 됩니다. 인물명 라벨을 기준으로 삼으면 LLM이 지켜야 할 규칙이 하나 줄고, 이미지 대상 여부를 LLM이 판단할 필요가 없어 CHARACTER 프롬프트도 짧아집니다. 저장할 때는 `[[인물이름:URL]]`로 바꿉니다. 이름은 어떤 인물인지 알려주고, URL은 나중에 과거 턴을 열 때 현재 인물 테이블을 다시 조회하지 않아도 당시 표시한 이미지를 그대로 복원하게 합니다. `aiOutput`의 마커는 이미지가 들어갈 **본문 위치**를 보존하고, `characterImages[]`는 완료 시점에 실제 표시된 **순서와 횟수**를 구조화해 전달합니다. 다음 LLM 입력에서만 이 둘을 제거하는 이유는 URL과 내부 문법을 모델이 따라 쓰게 하지 않으면서도 저장·재조회 정보는 잃지 않기 위해서입니다.

**변경 계획 — 인물명 라벨 감지로 전환과 이미지 계약 정리(`Phase 2 · 계획`, KNK-1002 구현 · KNK-1003 스펙).**

> **진행 상태(2026-08-28).** 아래 ①(인물명 라벨 감지)과 ②의 "대사 줄 위 별도 줄·빈 줄" 부분은 manyak-ai PR #95(`a0b19e6071b3`)로 구현돼 위 본문에 반영했습니다. **남은 것**은 ②의 마커에서 인물 이름 빼기(`[[URL]]`)와 ③(이미지 이름 계약)이며, 백엔드·프론트와 함께 후속 PR에서 바꿉니다. 아래 "어떻게" 1~5·7·8은 구현 완료분, 6은 남은 계약입니다.

**무엇.** 세 가지를 함께 바꿉니다. ① 이미지를 띄우는 근거를 LLM이 출력하는 `[character:인물명]` 태그에서 **AI 서버가 직접 찾는 인물명 라벨(`인물명:`)**로 바꿉니다. LLM에는 태그를 쓰라고 시키지 않고, 이미지 보유 인물 이름 목록도 더는 알려주지 않습니다. ② 저장 마커를 `[[인물이름:URL]]`에서 **`[[URL]]`**로 바꾸고, 대사 줄 바로 앞이 아니라 **대사 줄 위에 빈 줄을 두고 별도 줄**에 둡니다. ③ 컴파일 응답 `character_images[].name`을 인물 이름이 아니라 **이미지 이름**으로 바꾸고, 채팅 요청 `character_images[]`와 `completed.characterImages[]`에 `image_name`을 더합니다. 이미지 이름은 AI가 완성해서 주는 문자열이며, 지금은 인물당 한 장이라 `인물이름_기본`입니다.

**왜.** ①은 개발 서버 실측에서 LLM이 몇 턴 뒤부터 태그를 빼먹어 이미지가 뜨지 않았기 때문입니다. 태그는 형식 계약이고, 형식 계약은 프롬프트가 아니라 코드가 담보합니다(D7). 이미지에 필요한 재료는 백엔드가 보내는 이름·URL 매핑과 LLM이 매 대사에 붙이는 인물명 라벨뿐이며 둘 다 AI 서버가 이미 갖고 있습니다. ②는 프론트 요청입니다. 마커가 대사와 같은 줄에 붙어 있으면 이미지 요소를 끼워 넣기 불편하고, 마커 줄이 따로 있으면 그 줄을 통째로 이미지로 바꾸면 됩니다. 인물 이름을 마커에서 뺀 것은 URL의 파일 이름에 이미지 이름이 들어가 중복이고, 인물 이름은 `characterImages[]`가 따로 주기 때문입니다. ③은 백엔드 요청입니다. 한 인물에 이미지가 여러 장(표정·배경·상황) 생길 때 인물 이름만으로는 어느 장인지 구분할 수 없어, 이미지마다 설명이 담긴 이름이 필요합니다. 표정처럼 고정 칸으로 나누면 구분 기준이 늘 때마다 칸과 백엔드 조립 로직이 같이 바뀌므로, AI가 완성한 자유 문자열 하나로 받기로 했습니다(2026-08-27 백엔드·프론트 합의). 운영(`main`, v0.2.6)에는 태그 방식이 배포된 적이 없어 옛 형식이 저장된 대화는 없습니다.

**어떻게.**

1. **감지 기준.** 줄 맨 앞(앞 공백 허용)에서 볼드 기호(`**`)를 걷어낸 뒤 남은 글자가 채팅 요청 `character_images[].name`과 **정확히 같고** 바로 콜론(`:`)이 따르면 인물명 라벨입니다. 이름 비교는 앞뒤 공백만 제거하고 대소문자·별칭 보정은 하지 않습니다. 라벨 안 공백(볼드 기호와 이름 사이, 이름과 콜론 사이)은 최대 2칸까지 인정합니다. `*`로 시작하는 지문 줄과 등록된 어떤 이름과도 앞글자가 맞지 않는 줄은 그 글자에서 탈락합니다.
2. **실시간 처리.** 줄이 시작될 때마다 인물명 라벨인지 확정될 때까지 글자를 잠시 모읍니다. 모으는 상한은 들여쓰기를 뺀 길이로 세며, 가장 긴 라벨(볼드 기호 4 + 이름 30 + 공백 6 + 콜론 1 = 41자)과 같습니다. 인물명 라벨이면 `character_image` 이벤트(`{name, imageName, imageUrl}`)를 먼저 보내고, 볼드를 뗀 `인물명:`을 `token`으로 내보냅니다. 아니면 모아둔 원문을 그대로 `token`으로 내보냅니다. 실시간 `token`엔 마커가 들어가지 않습니다.
...
4. **저장 마커.** 턴이 끝나면 완료 본문에서 볼드를 뗀 뒤, 1과 같은 규칙으로 인물명 라벨을 찾아 그 줄 위에 `[[URL]]` 한 줄과 빈 줄을 넣고, 같은 순서로 `completed.characterImages[]`(`{name, imageName, imageUrl}`)를 만듭니다. 실시간과 저장이 한 규칙을 쓰므로 이벤트 개수와 마커 개수가 항상 같으며, 이를 테스트로 고정합니다. 저장본 예:
...
6. **요청·응답 필드.** 채팅 요청 `character_images[]` 항목은 `{name, image_name, image_url}`입니다. `name`은 인물 이름이며 대사 줄과 맞추는 키라 **반드시 인물 이름**이어야 합니다. `image_name`은 이미지 이름이고 지금은 고르는 데 쓰지 않으며, 백엔드가 아직 보내지 않으면 빈 값으로 두고 AI가 인물 이름으로 대신 채웁니다. 컴파일 응답 `character_images[].name`은 이미지 이름(`인물이름_기본`)이며, 백엔드가 여기에 uuid를 붙여 파일 이름을 짓습니다. 인물에 어떻게 연결할지는 백엔드가 정합니다.
