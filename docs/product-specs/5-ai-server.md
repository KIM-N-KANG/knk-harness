...
| # | 항목 | 내용 | 계획 |
| --- | --- | --- | --- |
| A1 | storylines 타임아웃 역전 | **해소** — 백엔드 storyline read timeout이 90초로 조정돼(`StoryAiClient.kt:178`) AI 내부 LLM 타임아웃(90초, `story_llm.py:57`)과 정렬, 30~90초 구간 유실이 사라졌습니다. compile은 백엔드 180초·AI 여유 상한으로 함께 정렬([`4-backend.md §4-7`](./4-backend.md) 프록시 표) | 해소(2026-07-21 코드 대조 확인, KNK-642 스펙 반영) |
| A2 | 와이어 필드 정렬 3자 릴리스 | AI(KNK-371)·백엔드(KNK-372)·웹(KNK-373) 모두 용어집 정렬(`storyline` · `additional_info` · `recommended_infos`)을 각자 `dev`에 반영 완료(2026-07-21 코드 대조 — 구 `story`·`extra_info` 소멸). 남은 조각은 세 레포 동시 prod 릴리스뿐 — [`4-backend.md §4-8`](./4-backend.md) B2의 짝 | 3자 동시 릴리스로 종결 |
