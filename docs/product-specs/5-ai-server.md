...
| `Phase 2 · 계획` | Phase 2 범위. 미구현, 방향만 합의됨 | 요약(`summary`) 생성(백엔드 소유) — AI의 수신 계약은 구현되어 빈 문자열로 동작 중. 인물 단위 입력(KNK-834, [§5-3-2](#5-3-2-스토리라인-생성)). 스토리 썸네일 AI 생성 — 별도 엔드포인트로 컴파일 후 비동기 호출. 채팅 인물 이미지 계약 정리(KNK-1014, [§5-3-4](#5-3-4-채팅-턴) 변경 계획) — 이미지 이름(`image_name`/`imageName`)을 계약에 더함 |
...
| `character_images[]` | 객체 배열 | 저장된 인물 이미지 매핑(`Phase 2 · 구현`, KNK-982). 항목은 `name`(인물 이름) + `image_url`(이미지 URL). AI 서버는 배열 개수에 상한을 두지 않고 인물 수는 백엔드가 관리합니다. 백엔드가 DB에서 이미지가 있는 인물만 조회해 채워 보냅니다. AI는 이름도 URL도 LLM 프롬프트에 넣지 않고, 출력의 `인물명:` 라벨과 대조하는 데만 씁니다(KNK-1002). 현재 두 값은 일반 문자열이라 빈 값도 스키마를 통과하고, 같은 이름이 여러 번 오면 마지막 항목이 쓰이는 제한이 있습니다(A19). 변경 계획(KNK-1014): 항목에 `image_name`을 더합니다(아래 변경 계획) |
...
| `character_image` | `{"name": string, "imageUrl": string}` | 인물 이미지(`Phase 2 · 구현`, KNK-982·KNK-1002). 이미지가 있는 인물의 `인물명:` 라벨을 줄 머리에서 감지할 때마다 그 라벨 글자보다 먼저 전송. 같은 인물이 다시 말하면 같은 URL로 다시 전송. 변경 계획: `imageName`을 더함(아래 변경 계획) |
| `ping` | `{}` | 판정을 기다리는 동안만 10초 간격으로 나가는 빈 신호(KNK-750). 백엔드의 이벤트 간 상한 시계를 되돌리는 것이 유일한 목적이라 담는 값이 없음 |
| `completed` | `{"aiOutput": string, "choices": [], "characterImages": [...], "meta": {...}}` | 턴 완료. camelCase 표기. `aiOutput`은 본문과 저장 마커를 포함 — 이미지가 표시된 대사 줄 **위에** `[[URL]]` 한 줄을 두고 마커와 대사 사이에 빈 줄 하나를 둠(KNK-1002). `characterImages`는 실제로 표시한 순서대로 `{name, imageUrl}`를 담으며 같은 인물의 반복도 그대로 중복 기록. `choices`는 **항상 빈 배열**(선택지는 별도 API — D6). 변경 계획: 마커에서 인물 이름을 빼 `[[URL]]`로 줄이고 `characterImages` 항목에 `imageName`을 더함(아래 변경 계획) |
...
- **저장 마커.** 턴이 끝나면 완료 본문에서 볼드를 뗀 뒤, 위와 같은 규칙으로 화자 라벨을 다시 찾아 그 대사 줄 **위에** `[[URL]]` 한 줄을 넣고 마커와 대사 사이에 빈 줄을 하나 둡니다(프론트가 마커 줄을 통째로 이미지로 바꾸기 쉽게 — 프론트 요청). 마커에 인물 이름은 넣지 않습니다(KNK-1025) — URL 파일명과 `characterImages[]`에 이미 있어 겹칩니다. 대사 줄의 들여쓰기는 그대로 남습니다. 같은 순서로 `completed.characterImages[]`(`{name, imageUrl}`)를 만듭니다. 실시간과 저장이 같은 정규식을 쓰므로 정상 입력에서 이벤트 개수와 마커 개수가 같으며, 이를 테스트로 고정합니다(극단 입력의 예외는 A22).
...
> **진행 상태(2026-08-28).** 아래 ①(화자 라벨 감지)과 ②의 "대사 줄 위 별도 줄·빈 줄" 부분은 manyak-ai PR #95(`a0b19e6071b3`)로 구현돼 위 본문에 반영했습니다. ②의 마커에서 인물 이름 빼기(`[[URL]]`)는 KNK-1025로 구현해 위 본문에 반영했습니다. **남은 것**은 ③(이미지 이름 계약)이며, AI 서버는 KNK-1014(KNK-1026·1027)로, 백엔드·프론트는 각자 티켓으로 바꿉니다. 아래 "어떻게" 1~5·7·8은 구현 완료분, 6은 남은 계약입니다. ③의 모양은 백엔드 리뷰(2026-08-28)를 받아 확정했습니다 — 컴파일 응답 `name`은 인물 이름으로 두고 `image_name`을 더합니다(아래 "왜"·"왜 그 방법").
...
| A21 | 채팅 인물 이미지가 LLM 태그 출력에 의존 | 이미지 이벤트·저장 마커의 근거가 LLM이 쓰는 `[character:인물명]` 태그라, LLM이 태그를 빼먹으면 이미지가 조용히 빠집니다. 개발 서버 실측에서 몇 턴 뒤부터 누락이 재현됐습니다. 형식 계약을 프롬프트에 맡긴 것이라 D7과도 어긋납니다. 운영에는 이 방식이 배포되지 않았습니다. 같은 자리에서 마커 형식(`[[인물이름:URL]]`, 대사 옆)과 이미지 이름 계약(컴파일 `name`이 인물 이름)도 백엔드·프론트 요청으로 함께 바꾸기로 했습니다 | `부분 해소`(2026-08-28, manyak-ai PR #95 `a0b19e6071b3`) — 태그 의존은 해소됐습니다. AI 서버가 `인물명:` 줄을 직접 찾아 이미지를 만들고, 마커를 대사 줄 위 별도 줄(빈 줄 포함)에 둡니다([§5-3-4](#5-3-4-채팅-턴), KNK-1002). 마커에서 인물 이름을 빼 `[[URL]]`로 줄이는 것은 KNK-1025로 구현했습니다. 남은 것은 컴파일 `character_images[]`에 `image_name` 더하기(`name`은 인물 이름 유지), 채팅 요청에 `image_name`·이벤트·완료 응답에 `imageName` 더하기이며 AI 서버는 KNK-1014(KNK-1026·1027)로, 백엔드·프론트는 각자 티켓으로 바꿉니다([§5-3-4](#5-3-4-채팅-턴) 변경 계획, 모양은 2026-08-28 백엔드 리뷰로 확정). `4-backend.md`·`3-1-client.md`·`0-glossary.md`에는 변경 계획으로 적혀 있습니다(KNK-1003) |
