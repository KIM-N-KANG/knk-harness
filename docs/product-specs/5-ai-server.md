...
**그 기다림이 실제로 턴을 죽였습니다.** 판정 구간에 SSE 프레임이 하나도 나가지 않아, 백엔드의 이벤트 간 상한이 그 침묵을 세다가 정상 진행 중인 턴을 끊었습니다(KNK-748). 결정은 그대로 두고 귀결만 손봤습니다 — 기다리는 동안 `ping`을 내보내 그 시계를 되돌리고([§5-3-4](#5-3-4-채팅-턴)), 판정에 주는 시간을 이 턴에 남은 시간으로 산정해 본문과 판정이 합쳐 백엔드의 전체 상한을 넘기지 않게 합니다(KNK-750). 판정을 API로 빼는 대신 턴 안에 두기로 한 이상, **기다림을 없애는 것이 아니라 기다림을 안전하게 만드는 것**이 이 결정에 딸린 책임입니다.
...
AI가 발행하는 SSE 이벤트는 4개입니다(시작 이벤트 `started`는 백엔드 발행, 채팅·턴 식별자 `chatId`·`turnId`도 백엔드 부착):
...
**`ping`은 백엔드가 몰라도 동작합니다.** 백엔드의 이벤트 간 상한(`RestChatTurnAiClient`의 `Flux.timeout`)은 프레임을 하나라도 받으면 처음부터 다시 세는데, 판정 구간에 아무 프레임도 나가지 않으면 그 시계가 정상 진행 중인 턴을 끊습니다(KNK-748 — 사용자는 본문을 다 읽은 뒤에 에러를 맞았습니다). 백엔드는 모르는 이벤트 이름을 조용히 넘기므로 **처리는 안 되지만 시계는 초기화**되고, 그래서 백엔드를 고치지 않고 AI만 배포해도 동작합니다. 다만 `data:` 줄 자체는 반드시 실어 보냅니다 — 주석만 있는 프레임은 디코더가 항목으로 만들지 않아 시계가 돌아가지 않습니다.
...
- **판정을 못 돌린 턴.** 판정에 쓸 시간이 남지 않았거나 시간 상한에 걸려 끊긴 턴은 `targetMainEvent`에 **요청으로 받은 목표를 그대로 되돌려 싣습니다**(진행 카운터도 받은 값 그대로 — 판정이 없었으니 올릴 근거가 없습니다). 3필드를 모두 null로 보내면 백엔드가 그것을 목표 해제로 읽어 사용자가 쌓아온 진행을 지우기 때문입니다([`4-backend.md §4-3-10`](./4-backend.md)). 이 시간 상한은 공급자가 아니라 **AI 서버가 스스로 건 것**이라(본문이 길어진 턴에는 판정에 남은 시간만 줍니다) 그 때문에 난 시간 초과로 진행 상태를 바꾸는 것은 앞뒤가 맞지 않습니다. 완결 사건·엔딩 이름은 판정만 알 수 있는 값이라 계속 null입니다. **시간 상한이 아닌 판정 실패**(빈 응답·깨진 JSON·전송 오류)는 종전대로 3필드 null이며, 그때 진행이 지워지는 문제는 백엔드 가드로 다룹니다([§5-7](#5-7-검수-체크리스트) A16).
...
**판정만 상한의 뜻이 다릅니다.** 다른 경로의 타임아웃은 SDK에 넘기는 값이라 **요청 한 번당** 상한이고, SDK가 시간 초과도 재시도하므로 실제 대기는 그 세 배까지 늘어납니다(A12). 판정은 호출 전체를 코드로 묶어 재시도를 포함한 상한으로 만들었습니다(KNK-749). 여기에 더해 **이 턴에 남은 시간**(백엔드의 전체 상한 120초에서 본문에 쓴 시간과 안전 여유를 뺀 값)과 비교해 작은 쪽을 씁니다 — 본문이 길어진 턴에 판정에 60초를 통째로 주면 둘을 합쳐 백엔드 상한을 넘겨 턴이 죽기 때문입니다. 남은 시간이 없으면 아예 부르지 않습니다.
...
| A7 | LLM 모델 변경·성능 비교 | 퀄리티 개선 범위로 모델 후보 비교가 로드맵 Phase 1 항목("LLM 모델 변경")으로 남아 있으나 실측 전이라 미결정. 전담 Jira 티켓은 아직 없음. **후보는 등록부에 올라왔습니다**(KNK-703) — GPT 3종과 Claude 1종을 가격·한도까지 함께 적어 두었고, 이제 세 모델 이름 변수만 바꾸면 그 모델로 호출됩니다(채팅 자리의 Anthropic만 예외 — A13). 남은 것은 실측입니다. 실측 때 [§5-4-3](#5-4-3-모델과-파라미터) 표에서 문서로 확인되지 않은 두 칸(GPT 3종의 `temperature` 미수용, GPT-5.6 둘의 추론 강도 허용 목록)과 등록 이름의 스냅샷 전환(A14)도 함께 처리합니다 | `계획` — AI 실험 환경 구성(KNK-457) 완료 후 D9 절차(벤치 실측·근거 기록, [§5-6](#5-6-운영과-관측)의 평가 지표 활용)로 비교하고 전환 여부 결정. 결정 전까지 현재 모델 유지 |
