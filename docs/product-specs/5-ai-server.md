...
| 기준 코드 | `../manyak-ai` `dev` 브랜치 `7abfdd5cd6f2`(2026-08-08, Python 3.11 · FastAPI)입니다. KNK-803의 Terra 컴파일 전환과 KNK-813의 공급자 키 형식 검사가 반영됐습니다. 운영 릴리스는 `v0.2.4`(main `34e1346`)이며, 운영 컴파일이 `openai`·`gpt-5.6-terra`로 성공하는 것까지 확인했습니다. |
...
| `Phase 2 · 계획` | Phase 2 범위. 미구현, 방향만 합의됨 | 요약(`summary`) 생성(백엔드 소유) — AI의 수신 계약은 구현되어 빈 문자열로 동작 중. 인물 단위 입력(KNK-834, [§5-3-2](#5-3-2-스토리라인-생성))과 성별 칸([§5-3-3](#5-3-3-스토리-컴파일)) |
...
| LLM 모델 | 등록된 모델은 6개입니다(KNK-703) — deepseek-v4-pro · deepseek-v4-flash · gpt-5.6-terra · gpt-5.6-luna · gpt-5.4-mini · claude-sonnet-5. 이 중 **실제로 고른 것은 둘**입니다: gpt-5.6-terra(컴파일) · deepseek-v4-flash(스토리라인·채팅·선택지·판정) — 근거는 D9. 나머지 넷은 대체 후보로 등록돼 있습니다 |
...
**키워드 단계 개편 반영 — `Phase 2 · 계획`(KNK-621).** 2026-07-20 팀 결정([`3-1-client.md §3-1-4`](./3-1-client.md))의 AI 계약 영향분입니다. 구현 전까지 위 현행 계약이 유효합니다.
...
**인물 단위 입력 반영(`Phase 2 · 계획`, KNK-834).** 주변 인물 세트(KNK-621)를 주인공까지 넓힌 2026-08-16 합의분입니다. 인물을 특징 태그 묶음이 아니라 인물 단위(이름·성별·특징)로 입력받아, 사용자가 정한 인물이 그대로 이야기에 등장하게 합니다. 스토리라인·컴파일 두 요청에 함께 적용되며, 구현 전까지 위 현행 계약이 유효합니다.

- `protagonist_tags` 대신 주인공 세트 `protagonist`(`{name, gender("MALE"·"FEMALE"·null), features[]}`)를 받습니다. 이름·성별은 선택이며 비면(null) LLM이 지어냅니다. 특징은 인물당 최대 3개이고 개수 강제는 백엔드가 맡습니다(주변 인물 세트의 `features[]`도 동일).
- 사용자가 채운 인물은 스토리라인 3편 모두와 컴파일 인물 카드에 전원 등장시키고, 인물 카드 5명 한도에서 남는 자리만 LLM이 채웁니다. 이름은 입력값을 그대로 씁니다. 사용자 인물이 빠지거나 이름이 바뀐 응답은 유효하지 않은 응답으로 보고 invalid 재호출 대상에 넣습니다(KNK-312와 같은 정책).
...
- **주인공 이름 반영 — `Phase 1 · 계획`.** 세분 JSON의 주인공 이름(시작 설정의 이름 `name`과 동일 값)을 주인공 통글(`user_role_setting`)의 `## 호칭`에 명시 포함해, 채팅이 주인공을 이름으로 일관 지칭하게 하는 설계입니다. 현재 구현은 호칭을 LLM 생성(추가 정보 반영)에 맡길 뿐 이름을 강제 주입하지 않아 미구현입니다. 이름 불일치·미반영은 평가 지표 `setting_consistency`로 감시합니다([§5-6](#5-6-운영과-관측)). 인물 단위 입력([§5-3-2](#5-3-2-스토리라인-생성) KNK-834)이 구현되면 사용자가 입력한 주인공 이름이 정본이 됩니다. LLM 출력에 맡기지 않고 코드가 주인공 통글의 이름을 입력값으로 덮어써 담보합니다(장르 덮어쓰기와 같은 원칙, D7).
- **성별 칸 추가(`Phase 2 · 계획`, KNK-834).** 인물 단위 입력으로 성별이 사용자 선택값이 되므로, 서술에 녹여 흐리게 두지 않고 통글에 명시 칸을 둡니다. 인물 카드에 `### 성별`, 주인공 통글에 `## 성별` 항목이 추가됩니다. 통글은 문자열 하나로 프롬프트에 그대로 끼워지므로 백엔드 저장·채팅 계약은 바뀌지 않습니다.
...
- 다만 **모델 등록부에 있는 이름만** 넣을 수 있습니다(D13). 지금 등록된 이름은 여섯입니다 — `deepseek-v4-pro` · `deepseek-v4-flash` · `gpt-5.6-terra` · `gpt-5.6-luna` · `gpt-5.4-mini` · `claude-sonnet-5`. 미등록 모델이거나, 그 모델의 공급자 키가 비어 있거나 형식이 잘못됐거나, **그 자리가 막아 둔 공급자를 쓰는 모델이면** 기동에서 실패합니다. 지금 막혀 있는 것은 `CHAT_MODEL`의 Anthropic 하나입니다(사유는 D13).
...
| `provider` | `deepseek` · `openai` · `anthropic` | 모델 등록부가 모델 이름을 해석한 값(D13). 컴파일은 `openai`, 스토리라인·채팅·선택지·판정은 `deepseek`이 실립니다. 아래 세 모델 이름 변수를 바꾸면 값도 함께 바뀝니다. 호출 전에 확정하므로 실패·폴백 경로에서도 비지 않음 |
...
| `OPENAI_API_KEY` | (빈 값) | GPT 모델을 고를 때 필요. 기본 컴파일 모델이 Terra이므로 기본 구성에서는 필수입니다. 선택된 키가 비었거나 개행·앞뒤 공백·비 ASCII·공백·제어문자를 포함하면 기동에서 실패하고, 오류에는 키 원문을 남기지 않습니다(D13) |
...
| `STORY_COMPILE_MODEL` | `gpt-5.6-terra` | 컴파일용 모델 |
