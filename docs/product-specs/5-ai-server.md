...
- 다만 **모델 등록부에 있는 이름만** 넣을 수 있습니다(D13). 지금 등록된 이름은 여덟입니다 — `deepseek-v4-pro` · `deepseek-v4-flash` · `gpt-5.6-terra` · `gpt-5.6-luna` · `gpt-5.4-mini` · `claude-sonnet-5` · `gemini-3.6-flash` · `gemini-3.7-flash`. 미등록 모델이거나, 그 모델의 공급자 키가 비어 있거나 형식이 잘못됐거나, **그 자리가 막아 둔 공급자를 쓰는 모델이면** 기동에서 실패합니다. 지금 막혀 있는 것은 `CHAT_MODEL`의 Anthropic 하나입니다(사유는 D13).
