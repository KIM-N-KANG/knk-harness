...
| 기준 코드 | `../manyak-ai` `dev` 브랜치 `d471761c9be1` (2026-07-29, Python 3.11 · FastAPI). LLM 호출 3층 재구성(D13 — KNK-667, PR #67)과 두 번째 어댑터·채팅 자리 차단(KNK-675, PR #69)이 dev에 머지되어 반영됐습니다. 모델 4개 등록과 등록부 확장(KNK-703, `c4eee2e67cf1`)은 **dev 머지 대기 상태로 선반영**했습니다. 운영 릴리스는 `v0.2.2`(main `4624fc2`) |
...
| LLM 연동 | 공통 통로 → 모델 등록부 → 공급자 어댑터(D13). 공급자는 셋(DeepSeek · OpenAI · Anthropic)이고 어댑터는 둘입니다 — OpenAI SDK(`AsyncOpenAI`, DeepSeek과 GPT를 함께 담당)와 Anthropic SDK(`AsyncAnthropic`) |
| LLM 모델 | 등록된 모델은 6개입니다(KNK-703) — deepseek-v4-pro · deepseek-v4-flash · gpt-5.6-terra · gpt-5.6-luna · gpt-5.4-mini · claude-sonnet-5. 이 중 **실제로 고른 것은 DeepSeek 둘**입니다: deepseek-v4-pro(컴파일) · deepseek-v4-flash(스토리라인·채팅·선택지·판정) — 근거는 D9. 나머지 넷은 모델 후보 비교([§5-7](#5-7-검수-체크리스트) A7)를 위해 등록만 해 둔 상태입니다 |
...
- 다만 **모델 등록부에 있는 이름만** 넣을 수 있습니다(D13). 지금 등록된 이름은 여섯입니다 — `deepseek-v4-pro` · `deepseek-v4-flash` · `gpt-5.6-terra` · `gpt-5.6-luna` · `gpt-5.4-mini` · `claude-sonnet-5`. 미등록 모델이거나, 그 모델의 공급자 키가 비어 있거나, **그 자리가 막아 둔 공급자를 쓰는 모델이면** 기동에서 실패합니다. 지금 막혀 있는 것은 `CHAT_MODEL`의 Anthropic 하나입니다(사유는 D13).
- 위 표의 값 말고도 **모델마다 딸린 정보가 등록부에 함께 있습니다**(D13) — 가격, 문맥 창과 최대 출력 한도, 추론 강도와 허용 목록, 구조화 출력을 받는 방식, 고정 이름, 그리고 그 값들을 언제 어느 공식 문서로 확인했는지입니다. 어댑터는 호출 전에 이 값들과 요청을 맞춰 봅니다(D13). 막는 시점과 범위는 이렇습니다 — **추론 강도**가 그 모델의 허용 목록에 없으면 **기동에서** 걸리고(양쪽 어댑터), 요청의 **최대 출력 초과**는 호출을 조립할 때 걸립니다(양쪽 어댑터). **구조화 출력**은 OpenAI SDK 어댑터만 막고, Anthropic SDK 어댑터는 막지 않고 그 인자를 빼고 경고만 남깁니다([§5-7](#5-7-검수-체크리스트) A13). 막힌 것이 어떤 오류로 올라가는지는 [§5-5](#5-5-오류와-실패-코드)에 있습니다. **가격은 아직 쓰이는 곳이 없습니다** — 모델 후보 비교(A7)에서 쓰기 위해 미리 적어 둔 값입니다.
...
- **호출 전에 막히는 것은 이 표에 오지 않습니다.** 어댑터가 요청을 등록부의 모델 정보와 맞춰 보고 거르는 것들(최대 출력 초과, 그 모델이 받지 않는 추론 강도, 그리고 OpenAI SDK 어댑터에서는 지원하지 않는 구조화 출력 — D13)은 공급자 장애가 아니라 우리 설정·요청의 문제라, `provider_*`가 아니라 **설정 오류**로 올라갑니다. 502로 접으면 "모델을 잘못 골랐다"가 공급자 장애로 위장되기 때문입니다.
...
| `provider` | `deepseek` · `openai` · `anthropic` | 모델 등록부가 모델 이름을 해석한 값(D13). 지금 고른 모델이 DeepSeek뿐이라 실제로 실리는 값은 `deepseek`이고, 아래 세 모델 이름 변수를 바꾸면 나머지 값이 나옵니다. 호출을 시작하기 전에 확정하므로 실패·폴백 경로에서도 비지 않음 |
...
| `OPENAI_API_KEY` | (빈 값) | GPT 모델을 고를 때만 필요 — 기동 검사는 선택된 모델의 공급자 키만 봅니다(D13) |
| `OPENAI_API_URL` | (미설정 = SDK 기본 주소) | 값을 적으면 `http://` 또는 `https://`와 호스트가 있어야 함. 빈 문자열로 두지 않습니다(아래 Anthropic 주소와 같은 규칙) |
| `ANTHROPIC_API_KEY` | (빈 값) | Claude 모델을 고를 때만 필요 — 기동 검사는 선택된 모델의 공급자 키만 봅니다(D13) |
...
| A7 | LLM 모델 변경·성능 비교 | 퀄리티 개선 범위로 모델 후보 비교가 로드맵 Phase 1 항목("LLM 모델 변경")으로 남아 있으나 실측 전이라 미결정. 전담 Jira 티켓은 아직 없음. **후보는 등록부에 올라왔습니다**(KNK-703) — GPT 3종과 Claude 1종을 가격·한도까지 함께 적어 두었고, 이제 세 모델 이름 변수만 바꾸면 그 모델로 호출됩니다(채팅 자리의 Anthropic만 예외 — A13). 남은 것은 실측입니다 | `계획` — AI 실험 환경 구성(KNK-457) 완료 후 D9 절차(벤치 실측·근거 기록, [§5-6](#5-6-운영과-관측)의 평가 지표 활용)로 비교하고 전환 여부 결정. 결정 전까지 현재 모델 유지 |
...
| A13 | 채팅을 Anthropic 공급자로 열지 못함 | 어댑터는 단발 호출과 조각 흘리기를 모두 구현했고 모델(`claude-sonnet-5`)도 등록부에 올라왔지만(KNK-703), `CHAT_MODEL`에 이 공급자를 쓰는 것은 등록부가 기동에서 막습니다(D13). 여는 데 필요한 것이 넷입니다. ① **지시문 배치** — 뒤쪽 둘(Depth·PHI)이 최상위 지시문 칸에 들어가지 못해 버려집니다. 이 회사도 대화 목록 안에 지시문 줄을 넣는 길을 지원하지만(첫 줄이 아니어야 하고, 사용자 발화 뒤에 와야 하며, 마지막이거나 뒤에 assistant 턴이 와야 함) 모델마다 지원 여부가 갈립니다. ② **`max_tokens`** — 이 회사 요청의 필수 항목인데 채팅 본문 호출은 이 값을 싣지 않아, 그대로 열면 턴마다 400입니다(A8이 추적하는 상한 미확정과 맞물림). 값을 넣을 때는 등록부에 적힌 그 모델의 최대 출력을 넘지 않아야 합니다 — 넘으면 어댑터가 호출 전에 막습니다(KNK-703). 이 회사의 최신 모델은 추론이 기본으로 켜져 있고 이 한도가 추론과 본문을 합쳐 자르므로, 다른 경로의 값을 그대로 가져다 쓰면 본문이 잘립니다. ③ **안전 실측** — 지시문을 옮긴 뒤 안전 규칙이 실제로 지켜지는지 확인. ④ **호출 기록** — 이 공급자를 Langfuse에 남기는 것은 바깥에서 만든 자동 계측에 기대는데, 그 계측은 기록을 "끝까지 읽었을 때"와 "읽다가 오류났을 때"만 끝냅니다. **사용자가 답변 도중 취소하면 기록이 끝나지 않아, 그 턴이 기록에서 빠지고 그때까지 받은 글을 프로세스가 끝날 때까지 붙들고 있습니다**(어댑터가 부르는 스트림 닫기를 그 계측이 가로채지 않아 원본 SDK로 그대로 넘어갑니다 — 패키지 소스로 확인). 오류가 나지 않아 드러나지 않으므로, 채팅을 여는 티켓이 기록 방식을 Langfuse 직접 기록으로 바꿔야 합니다. 조각으로 받는 자리가 채팅뿐이라 지금은 이 경로에 닿지 않고, 한 번에 답을 받는 컴파일·스토리라인·선택지·판정은 호출 함수 안에서 기록을 반드시 끝내므로 영향이 없습니다. 함께 알려진 제약으로, 이 회사는 JSON 강제에 스키마 전체를 요구하는데 통로의 요청에 그것을 담을 칸이 없어 어댑터가 그 인자를 빼고 보냅니다 — 막지 않고 경고만 남기므로(OpenAI SDK 어댑터와 다릅니다, D13) 형식 준수가 프롬프트 지시에만 기대게 되고, 이 공급자로 story·선택지·판정을 열 때도 별도 검토가 필요합니다 | `계획` — 채팅을 여는 티켓이 넷을 모두 처리한 뒤 등록부의 금지 한 줄을 지웁니다 |
