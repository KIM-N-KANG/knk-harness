...
**지금 하지 않는 것.** 한 인물에 이미지 여러 장(표정별)을 받아 대사마다 어느 장을 띄울지 고르는 것. 간편 제작과 오리지널 스토리 모두 **인물당 한 장**으로 갑니다(2026-08-28 결정). 이유는 간편 제작에서 한 장조차 안정적으로 만들어지고 채팅 화면에 뜨는지 아직 검증되지 않아, 그 위에 고르는 로직을 얹는 것이 이르기 때문입니다. `image_name`은 여러 장이 되더라도 장마다 이름이 필요한 건 같아 지금 넣어도 버려지지 않으며, "추가"라 하위호환도 깨지 않습니다. 여러 장이 필요해지면 고르는 주체(LLM이 대사마다 표정을 표시 / 코드가 별도 판정)·요청 모양(인물당 이미지 목록)·`story_characters`의 인물:이미지 1:N 분리를 그때 한 묶음으로 정합니다.
