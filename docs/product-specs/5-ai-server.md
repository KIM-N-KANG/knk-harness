...
| `completed` | `{"aiOutput": string, "choices": [], "characterImages": [...], "meta": {...}}` | 턴 완료. camelCase 표기. `aiOutput`은 본문과 저장 마커를 포함 — 이미지가 표시된 대사 줄 **위에** `[[인물이름:URL]]` 한 줄을 두고 마커와 대사 사이에 빈 줄 하나를 둠(KNK-1002). `characterImages`는 실제로 표시한 순서대로 `{name, imageUrl}`를 담으며 같은 인물의 반복도 그대로 중복 기록. `choices`는 **항상 빈 배열**(선택지는 별도 API — D6). 변경 계획: 마커에서 인물 이름을 빼 `[[URL]]`로 줄이고 `characterImages` 항목에 `imageName`을 더함(아래 변경 계획) |
...
- **저장 마커.** 턴이 끝나면 완료 본문에서 볼드를 뗀 뒤, 위와 같은 규칙으로 화자 라벨을 다시 찾아 그 대사 줄 **위에** `[[인물이름:URL]]` 한 줄을 넣고 마커와 대사 사이에 빈 줄을 하나 둡니다(프론트가 마커 줄을 통째로 이미지로 바꾸기 쉽게 — 프론트 요청). 대사 줄의 들여쓰기는 그대로 남습니다. 같은 순서로 `completed.characterImages[]`(`{name, imageUrl}`)를 만듭니다. 실시간과 저장이 같은 정규식을 쓰므로 정상 입력에서 이벤트 개수와 마커 개수가 같으며, 이를 테스트로 고정합니다(극단 입력의 예외는 A22).
- **다음 LLM 입력 정리.** 마커와 그 뒤 줄바꿈(최대 2개)을 **복사본에서만** 함께 지웁니다(마커만 지우면 빈 줄이 남습니다). 채팅 턴은 `history[]`의 USER·ASSISTANT 항목 모두에서, 선택지 생성은 `history[]` 전체와 이번 `ai_output`에서, 판정은 이번 `ai_output`에서만 지웁니다. 줄바꿈 없이 대사 옆에 붙은 옛 모양 마커(개발 서버 기록)도 같은 규칙으로 지워집니다. 옛 `[character:이름]` 태그는 더 지우지 않습니다(KNK-1007) — LLM이 태그를 쓰지 않으므로 남은 것은 개발 서버 옛 기록뿐이고, 운영에는 태그 방식이 배포된 적이 없습니다. DB에 저장한 `aiOutput`은 바꾸지 않습니다.
...
4. 턴이 끝나면 같은 규칙으로 라벨을 다시 찾아 그 줄 위에 `[[인물이름:URL]]` 저장 마커를 넣어 `completed.aiOutput`에 담습니다. 같은 순서와 횟수로 `completed.characterImages[]`도 만듭니다.
...
> **진행 상태(2026-08-28).** 아래 ①(화자 라벨 감지)과 ②의 "대사 줄 위 별도 줄·빈 줄" 부분은 manyak-ai PR #95(`a0b19e6071b3`)로 구현돼 위 본문에 반영했습니다. **남은 것**은 ②의 마커에서 인물 이름 빼기(`[[URL]]`)와 ③(이미지 이름 계약)이며, AI 서버는 KNK-1014로, 백엔드·프론트는 각자 티켓으로 바꿉니다. 아래 "어떻게" 1~5·7·8은 구현 완료분, 6은 남은 계약입니다. ③의 모양은 백엔드 리뷰(2026-08-28)를 받아 확정했습니다 — 컴파일 응답 `name`은 인물 이름으로 두고 `image_name`을 더합니다(아래 "왜"·"왜 그 방법").
