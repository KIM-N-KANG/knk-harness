...
| 기준 코드 | `../manyak-ai` `dev` 브랜치 `dcce0c7d9507`(2026-09-02, Python 3.11 · FastAPI). 스토리라인 이름 검증 소진 시 결과 반환(KNK-1102)까지 이 `dev`에 병합된 상태를 대조했습니다. |
...
| `Phase 1 · 구현` | Phase 1 범위. AI 서버 구현 완료. 백엔드가 판정 재료를 실어 보내는 런타임 연결도 완료([`4-backend.md §4-3-10`](./4-backend.md) 소관) | 주요 사건·엔딩 진행(D11·D12, [§5-3-3](#5-3-3-스토리-컴파일)·[§5-3-4](#5-3-4-채팅-턴)·[§5-3-5](#5-3-5-선택지-생성-별도-api)). 채팅 분량 고정(700~1000자, [§5-3-4](#5-3-4-채팅-턴)). 로어북 반영([§5-3-3](#5-3-3-스토리-컴파일)). LLM 트레이싱(Langfuse) — 프로덕션 활성, 구조화 입력·생성 결과 연결 구현([§5-6](#5-6-운영과-관측)) |
| `Phase 1 · 계획` | Phase 1 범위. 미구현, 방향만 합의됨(계약 확정 여부는 각 항목·어긋난 곳 표가 기록) | 태그 귀속 강화([§5-3-2](#5-3-2-스토리라인-생성)). 주인공 이름 반영([§5-3-3](#5-3-3-스토리-컴파일)). 품질 평가와 자가개선 루프([§5-6](#5-6-운영과-관측)). AI 응답 재생성은 채팅 턴 계약의 재호출로 처리해 AI 스키마 무변경([§5-3-4](#5-3-4-채팅-턴)). 일반 제작·스토리 수정은 AI 미사용으로 판정되어 이 문서 변경 없음([`4-backend.md §4-3-8·§4-3-9`](./4-backend.md)). 썸네일은 이 판정이 뒤집혀 컴파일 응답에 표지를 싣습니다(KNK-1047 — `Phase 2 · 구현`, [§5-3-3](#5-3-3-스토리-컴파일)) |
| `Phase 2 · 구현` | Phase 2 범위. AI 서버 구현 완료 | 주인공 이름·성별 주입(KNK-838, [§5-3-3](#5-3-3-스토리-컴파일)). 성별 칸 추가(KNK-838). 인물별 이미지 생성(KNK-414, [§5-3-3](#5-3-3-스토리-컴파일)) — 컴파일 시 외형 필드로 이미지를 만들어 base64로 반환. 채팅 인물 이미지 출력(KNK-982, [§5-3-4](#5-3-4-채팅-턴)) — 인물의 대사마다 실시간 이미지 이벤트와 저장 마커를 만듦. 화자 라벨 감지로 전환(KNK-1002, [§5-3-4](#5-3-4-채팅-턴)) — LLM 태그 대신 AI 서버가 `인물명:` 줄을 찾아 이미지를 붙이고, 저장 마커는 대사 줄 위 별도 줄에 둠. 저장 마커에서 인물 이름 제거(KNK-1025, [§5-3-4](#5-3-4-채팅-턴)) — 마커는 `[[URL]]`만 담음. 인물 이미지에 이미지 이름 추가(KNK-1026·1027, [§5-3-3](#5-3-3-스토리-컴파일)·[§5-3-4](#5-3-4-채팅-턴)) — 컴파일 응답 `image_name`, 채팅 요청 `image_name`, 이벤트·완료 `imageName`. 스토리 썸네일 AI 생성(KNK-1047, [§5-3-3](#5-3-3-스토리-컴파일)) — 컴파일 안에서 인물 이미지와 동시에 만들어 컴파일 응답에 함께 실어 보냄. 이전 계획(별도 엔드포인트, 컴파일 뒤 비동기 호출)은 2026-08-30 팀 협의로 폐기 |
| `Phase 2 · 계획` | Phase 2 범위. 미구현, 방향만 합의됨 | 요약(`summary`) 생성(백엔드 소유) — AI의 수신 계약은 구현되어 빈 문자열로 동작 중. 인물 단위 입력(KNK-834, [§5-3-2](#5-3-2-스토리라인-생성)) |
...
LLM은 JSON 모드로 호출하고, 코드펜스로 감싼 응답도 방어적으로 파싱합니다(D7). 그래도 유효하지 않은 응답이면 같은 요청을 최대 2회 재호출합니다(KNK-312). 유효하지 않은 응답에는 두 종류가 있습니다 — 파싱 자체가 깨지는 경우(깨진 JSON·빈 응답·비객체)와, 파싱은 됐지만 stories 계약을 어긴 경우(3편이 아님·항목 스키마 불일치·추천 3개가 아님·본문이 비어 있음)입니다. 후자를 검증하지 않으면 응답 조립 단계에서 500으로 터집니다(실측: 계약 위반이 파싱을 통과해 500). 실측된 주 원인이 본문 속 대사 인용부호를 JSON 이스케이프 없이 출력하는 확률적 실수라 재호출로 대부분 회복되며, 백엔드 대기 한도(90초) 안에 응답하도록 전체 60초 상한을 두고, 공급자 오류(타임아웃·레이트리밋)는 재호출하지 않습니다. 그래도 실패하면 502로 응답합니다([§5-5](#5-5-오류와-실패-코드)). 표의 3편·3개라는 개수와 항목 스키마는 코드가 검증하고, id는 등장 순서대로 1·2·3으로 정규화합니다(위 문제와 결정 참조).

**인물 등장 검증과 부분 재호출 — `Phase 2 · 구현`(KNK-833·KNK-840).** 사용자가 이름 지은 주변 인물은 3편 모두에 등장해야 합니다. 전체 재호출(KNK-312)을 통과한 뒤, 이름이 빠진 편만 골라 그 편의 흐름을 유지하면서 빠진 인물을 넣도록 다시 받습니다(부분 재호출, 최대 2회). 세 편을 통째로 다시 사지 않아 잘 나온 편을 보존하고 대기 시간이 줄어듭니다(실측: 12초 → 5초). 부분 재호출이 깨진 편을 데려오면 병합 전 원본으로 되돌리고 다시 시도합니다. `retry_count`는 전체 재호출(0~2) + 부분 재호출(0~2) 합산이고, 토큰도 합산합니다.

**재호출을 다 써도 이름이 빠져 있으면 502가 아니라 결과를 그대로 반환하고 Sentry에 경고로만 기록합니다 — `Phase 2 · 구현`(KNK-1102).** 이전 판까지는 2회 뒤에도 남으면 502로 막았습니다. 바꾼 이유는 운영 사례입니다. 2026-09-01에 한 사용자가 스토리라인 생성을 7번 눌러 7번 모두 502를 받았고, 그 요청은 세 편 모두에서 이름이 빠져 부분 재호출 2회로도 채워지지 않았습니다. 어떤 입력은 다시 불러도 같은 결과가 나오므로, 502로 막으면 그 사용자에게는 몇 번을 눌러도 풀리지 않는 영구 실패가 됩니다. 이름 미등장은 품질 흠이지 못 쓸 결과가 아닙니다 — 세 편의 줄거리는 온전하고, 완성본에서 인물 등장은 컴파일의 인물 카드 검증([§5-3-3](#5-3-3-스토리-컴파일), KNK-833)이 따로 보증합니다. 그래서 이 검증은 D7의 "코드가 담보하는 계약"이 아니라 재호출로 최선을 다하되 끝내 안 되면 내주는 품질 항목으로 분류합니다. 502를 유지하는 안을 버린 이유는, 502가 막는 것이 못 쓸 결과가 아니라 사용자의 다음 시도 자체였기 때문입니다. 기록은 오류가 아니라 경고 수준으로 Sentry에 보내 아침 요약에서 빈도를 보고, 특정 입력에서 반복되면 프롬프트를 고칠 근거로 씁니다. 완화 뒤에도 빈 본문은 내보내지 않습니다 — 재호출이 스키마만 맞고 본문이 빈 편을 데려오면 계약 위반으로 걸러 원본으로 되돌립니다(위 stories 계약의 "본문이 비어 있지 않음").
...
6. 인물 카드의 성별과 외형 6필드로 인물별 이미지를 최대 5개까지 동시에 생성합니다(KNK-414). 한 인물의 이미지 생성이 실패해도 다른 인물 생성과 위 1~5단계의 스토리 결과는 그대로 200으로 반환합니다. 스토리 썸네일 한 장도 이 단계에서 인물 이미지와 동시에 만듭니다(KNK-1047 — 아래 "스토리 썸네일 생성").
...
graph TD
    ...
    Render --> Image["인물별 이미지 병렬 생성<br/>+ 썸네일 1장(KNK-1047)<br/>(실패해도 계속)"]
    Image --> OK[200 응답]
...
| `character_images[]` | 객체 배열 (0~5개) | 인물별 이미지 `{name, image_name, image_base64, content_type, error}`(KNK-414·KNK-1027). `name`은 인물 이름(백엔드가 외형·인물과 연결하는 키), `image_name`은 이미지 한 장의 이름으로 성공·실패 항목 모두에 있으며 지금은 인물당 한 장이라 `인물이름_기본`. 백엔드는 `image_name`에 uuid를 붙여 파일명으로 쓰고 `story_character_images.image_name`에 저장(KNK-1126 — `story_characters`에 이름 컬럼을 두지 않고 인물:이미지 1:N 테이블로 감). 성공과 실패 모두 현재 `content_type: "image/webp"`를 가지므로 성공 여부를 이 필드로 판단하면 안 됨. 성공은 `image_base64`가 문자열인 항목이고, 실패는 `image_base64: null`과 사유 코드(`timeout`, `rate_limited`, `rejected`, `appearance_missing`, `generation_failed`)가 있는 항목. 정상 처리에서는 인물마다 성공 또는 실패 항목이 하나씩 생기며, 이미지 생성 로직 전체가 예상하지 못한 오류로 중단되면 빈 배열. 백엔드는 `name`으로 외형·이미지를 연결하고, 성공 항목만 base64를 디코딩해 S3에 올린 뒤 `content_type`을 Content-Type으로 설정 |
| `thumbnail_image` | 객체 | 스토리 표지 한 장(KNK-1047) `{image_name, image_base64, content_type, error}`. 항상 객체이며 null이 아님. `image_name`은 `썸네일_기본`(스토리당 한 장이라 상수). 성공은 `image_base64`가 문자열, 실패는 `image_base64: null`과 사유 코드(`timeout`, `rate_limited`, `rejected`, `generation_failed`) — 썸네일 생성 로직 자체가 예상하지 못한 오류로 중단된 경우도 `generation_failed`. `content_type`은 성공·실패 모두 `image/webp`. 인물 항목과 달리 `name`이 없으므로 백엔드는 이 항목을 인물 이미지 연결(`name` 매칭)에 넣지 않음. 저장·표시는 [`4-backend.md §4-3-9`](./4-backend.md) |
...
**스토리 썸네일 생성 — `Phase 2 · 구현`(KNK-1047).**

**무엇.** 스토리 컴파일 때 스토리 표지(썸네일) 한 장을 만들어 `thumbnail_image`로 반환합니다. 세로 3:4(768x1024) WebP이며, 외형이 채워진 주변 인물 1~2명과 장르 분위기를 담습니다. 제목 글자는 넣지 않습니다.

**왜.** 지금 썸네일은 팀이 미리 만든 이미지를 장르로 골라 붙이므로 스토리 내용과 무관합니다([`4-backend.md §4-3-9`](./4-backend.md)). 표지에 그 스토리의 인물과 분위기가 담겨야 목록에서 이야기를 구분할 수 있습니다(US-2-7). 별도 엔드포인트를 두지 않는 이유는 재료(장르 태그와 인물 외형)가 이미 컴파일 안에 있고, 백엔드가 컴파일 뒤에 한 번 더 부르는 왕복을 없앨 수 있기 때문입니다.

**어떻게.** 다음 순서로 처리합니다.

1. 컴파일 LLM이 인물 외형을 채우고 나면, 인물별 이미지 생성과 **동시에** 썸네일 생성을 시작합니다. 컴파일 전체 대기 시간은 가장 오래 걸리는 한 장만큼만 늘어나며, 공급자 동시 호출은 최대 6건(인물 5 + 썸네일 1)입니다.
2. 외형 6필드가 모두 채워진 인물 중 인물 카드 순서 앞 1~2명을 표지 인물로 고릅니다. 그런 인물이 없으면 장르만으로 장면 표지를 만듭니다(건너뛰지 않음). 주인공(`user_role_setting`)은 외형 필드가 없어 표지에 넣지 않습니다.
3. 장르 태그와 표지 인물의 성별·외형을 썸네일 전용 프롬프트(`prompt/image/THUMBNAIL-IMAGE-TEMPLATE.md`)에 넣고, 인물 이미지와 같은 모델(`IMAGE_MODEL`)·화질(`IMAGE_QUALITY`)로 768x1024 한 장을 받습니다. 크기는 `IMAGE_SIZE`와 무관하게 고정입니다. 프롬프트 버전은 `meta.prompt_versions.THUMBNAIL_IMAGE`에 기록합니다.
4. 성공하면 WebP를 base64로 바꿔 싣습니다. 실패하면 `image_base64: null`과 사유 코드를 싣고 컴파일은 200을 유지합니다. 썸네일 실패는 인물 이미지와 스토리 본체에 영향을 주지 않으며, 백엔드는 실패 시 기존 프리셋 썸네일을 그대로 씁니다. 실패는 Sentry의 `thumbnail_image_generation` 기능으로 기록하되 인물 이름과 프롬프트 원문은 보내지 않습니다.

**왜 그 방법.** 실패 표현을 "항상 객체"로 하나로 둔 것은, 백엔드가 성공·실패 두 갈래만 보면 되게 하기 위해서입니다(인물 이미지의 빈 배열은 항목별 결과를 만들 수 없을 때의 표현인데, 썸네일은 한 장이라 그 제약이 없습니다). 표지 인물을 백엔드가 아니라 AI 서버가 고르는 것은 컴파일 안에서 만들므로 백엔드가 되돌려 보낼 재료가 없기 때문입니다. 인물 외형이 전부 빈 스토리에서는 인물 이미지 호출이 없는데 썸네일 때문에 최악 약 180초가 새로 붙을 수 있습니다(A18과 같은 원인). 드문 경우라 이번엔 상한을 두지 않고 A18 후속에서 함께 다룹니다. 컴파일이 없는 일반 제작에는 AI 썸네일이 생기지 않습니다.
...
| `character_images[]` | 객체 배열 | 저장된 인물 이미지 매핑(`Phase 2 · 구현`, KNK-982). 항목은 `{name, image_name, image_url}` — `name`(인물 이름), `image_name`(이미지 한 장의 이름, KNK-1026 — 칸 없음·빈 문자열·null 모두 통과하며 지금은 고르는 데 쓰지 않고 받은 값을 그대로 `imageName`에 돌려줌), `image_url`(이미지 URL). AI 서버는 배열 개수에 상한을 두지 않고 인물 수는 백엔드가 관리합니다. 백엔드가 DB에서 이미지가 있는 인물만 조회해 채워 보냅니다. AI는 이름도 URL도 LLM 프롬프트에 넣지 않고, 출력의 `인물명:` 라벨과 대조하는 데만 씁니다(KNK-1002). 현재 두 값은 일반 문자열이라 빈 값도 스키마를 통과하고, 같은 이름이 여러 번 오면 마지막 항목이 쓰이는 제한이 있습니다(A19) |
...
| `character_image` | `{"name": string, "imageName": string, "imageUrl": string}` | 인물 이미지(`Phase 2 · 구현`, KNK-982·KNK-1002·KNK-1026). 이미지가 있는 인물의 `인물명:` 라벨을 줄 머리에서 감지할 때마다 그 라벨 글자보다 먼저 전송. 같은 인물이 다시 말하면 같은 URL로 다시 전송. `imageName`은 요청 `image_name`을 그대로 돌려준 값이며 요청이 비어 있으면 빈 문자열 |
| `ping` | `{}` | 판정을 기다리는 동안만 10초 간격으로 나가는 빈 신호(KNK-750). 백엔드의 이벤트 간 상한 시계를 되돌리는 것이 유일한 목적이라 담는 값이 없음 |
| `completed` | `{"aiOutput": string, "choices": [], "characterImages": [...], "meta": {...}}` | 턴 완료. camelCase 표기. `aiOutput`은 본문과 저장 마커를 포함 — 이미지가 표시된 대사 줄 **위에** `[[URL]]` 한 줄을 두고 마커와 대사 사이에 빈 줄 하나를 둠(KNK-1002). `characterImages`는 실제로 표시한 순서대로 `{name, imageName, imageUrl}`(이벤트와 같은 모양, KNK-1026)를 담으며 같은 인물의 반복도 그대로 중복 기록. `choices`는 **항상 빈 배열**(선택지는 별도 API — D6) |
...
sequenceDiagram
    ...
    alt 본문 생성 성공
        loop 생성이 끝날 때까지
            LLM-->>AI: 토큰 델타
            alt 줄 머리 화자 라벨(`인물명:`) 감지 — 매핑에 있는 이름(KNK-1002)
                AI-->>BE: character_image 이벤트
                AI-->>BE: token 이벤트 — 볼드를 뗀 라벨 글자
            else 일반 텍스트
                AI-->>BE: token 이벤트 — 받은 그대로 전달
            end
        end
        AI->>AI: 같은 규칙으로 라벨을 다시 찾아 그 줄 위에 저장 마커 삽입(D7)
...
**채팅 인물 이미지 — `Phase 2 · 구현`(상위 KNK-982, 요청 계약 KNK-989 · 저장과 재입력 정리 KNK-992 · 화자 라벨 감지 전환 KNK-1002: 파서 KNK-1005 · 프롬프트 KNK-1006 · 옛 태그 처리 제거 KNK-1007 · 줄임 이름 매칭 KNK-1062).**

**무엇.** 채팅 답변에서 인물이 말하는 순간마다 그 인물의 이미지를 보여줍니다. 한 턴에서 여러 인물이 말할 수 있고, 같은 인물이 지문을 사이에 두고 다시 말할 수도 있습니다. 이미지가 저장된 인물의 대사에는 매번 이미지 이벤트를 하나씩 만들며, 이미지가 없는 단역·배경 인물도 대사는 할 수 있지만 이미지 이벤트는 만들지 않습니다. 이미지를 붙일 자리는 **AI 서버가 출력에서 직접 찾은 `인물명:` 줄(화자 라벨, [`0-glossary.md §0-6`](./0-glossary.md))**이고, LLM에게 이미지 관련 표시를 따로 쓰게 하지 않습니다(KNK-1002).

**왜.** 답변 시작 전에 이번 턴의 화자를 하나 고르면, 뒤에서 다른 인물이 말하거나 첫 인물이 다시 말하는 장면을 표현할 수 없습니다. 반대로 인물이 말한 위치를 알 수 있는 표시를 본문에 남기면, 스트리밍 중에는 대사와 함께 이미지를 즉시 보여주고 저장 후에는 같은 위치에 이미지를 복원할 수 있습니다. 따라서 “이번 턴에 누가 등장했는가”가 아니라 **대사 한 줄이 실제로 출력됐는가**를 기준으로 삼습니다. 처음(KNK-982)에는 LLM이 대사 앞에 `[character:인물명]` 태그를 쓰게 하고 서버가 그 태그를 이미지로 바꿨는데, 개발 서버 실측에서 몇 턴 뒤부터 LLM이 태그를 빼먹어 이미지가 조용히 빠졌습니다. 태그는 형식 계약이고 형식 계약은 프롬프트가 아니라 코드가 담보해야 하므로(D7), 이미지의 근거를 LLM이 어차피 매 대사에 쓰는 `인물명:` 라벨로 옮겼습니다(KNK-1002, A21). 라벨로 옮긴 뒤에도 이름 자체가 어긋나는 문제가 남았습니다 — 등록 이름은 `지한결`·`카시안 발데르크` 같은 정식 이름인데 LLM은 대사 라벨에 성을 뗀 이름이나 풀네임의 한 조각을 씁니다(개발 서버 실측: `한결` 46회 대 `지한결` 4회, 학원물 스토리 응답 22건 중 이미지 마커 저장 1건). 이름을 설정 글자 그대로 쓰라는 프롬프트 지시가 이미 있는데도 이 비율이므로, 태그 때와 같은 결론으로 **코드가 줄임 이름을 알아보게** 했습니다(KNK-1062).

**어떻게.** 발화 판정과 라벨 감지는 다음 규칙을 따릅니다.

- 매 답변에는 주변 인물이나 단역·배경 인물의 대사가 최소 한 줄 있어야 합니다(CHARACTER 프롬프트 운용 원칙).
- “말했다”의 기준은 별도 줄의 `인물명: 대사`입니다. CORE 프롬프트가 모든 대사를 이 형식의 별도 줄로 쓰게 하고, 이름은 CHARACTER 설정에 적힌 글자 그대로(호칭·별칭·수식어 없이, 볼드·따옴표·괄호 없이) 쓰게 합니다. 지문에 인물 이름만 나오거나 행동만 묘사된 경우는 이 정의상 발화가 아닙니다.
- **감지 기준.** 줄 맨 앞(들여쓰기 허용)에서 볼드 기호(`**`)를 걷어낸 뒤 남은 글자가 채팅 요청 `character_images[].name` **또는 그 이름에서 파생한 줄임 이름**과 글자가 같고 바로 콜론(`:`)이 따르면 화자 라벨입니다(KNK-1062). 이름 비교는 앞뒤 공백만 제거하고 대소문자 보정은 하지 않습니다. 볼드 기호와 이름 사이, 이름과 콜론 사이 공백은 최대 2칸까지 인정합니다. 이름이 겹치면(`세린`·`세린아`) 더 긴 이름을 먼저 대조합니다.
- **줄임 이름 파생.** 공백 있는 이름은 두 글자 이상 조각마다(`카시안 발데르크` → `카시안`·`발데르크`), 공백 없는 한글 세 글자 이름은 성을 뗀 두 글자(`지한결` → `한결`)를 줄임 이름으로 만듭니다. 줄임 이름이 다른 인물의 정식 이름과 같으면 만들지 않고(정식 이름 우선), 두 인물이 같은 줄임 이름을 만들면 누구인지 모호하므로 버립니다. 이벤트와 `characterImages[]`의 `name`은 줄임 이름으로 잡혀도 정식 이름입니다. 백엔드가 별칭 목록을 계약에 실어 보내는 방법도 있었지만, 파생 재료(정식 이름)를 서버가 이미 갖고 있고 계약을 바꾸면 백엔드·프론트·이 문서 세 곳이 함께 움직여야 하므로, 라벨을 읽는 AI 서버가 인정 규칙을 갖습니다. 알려진 한계: ① 두 글자 성(남궁 등)의 세 글자 이름은 잘못 잘려 줄임 이름이 무용지물이 됩니다 — 이미지가 안 붙던 종전과 같은 결과입니다. ② 이미지 없는 동명이인(예: 줄임 이름과 같은 이름의 다른 인물)이 스토리에 있으면 요청에 오지 않아 구분할 수 없고, 그 인물의 대사에 엉뚱한 이미지가 붙을 수 있습니다. 둘 다 그 글자가 대사 줄머리에 라벨로 나타나야만 발동해 발생 확률이 낮아 감수하고, 문제가 실제로 나타나면 전체 인물 이름 전달(계약 확장)로 풉니다.
- **실시간 처리.** 줄이 시작될 때마다 화자 라벨인지 확정될 때까지 글자를 잠시 모읍니다. 등록된 어떤 이름과도 앞글자가 맞지 않는 줄과 `*`로 시작하는 지문 줄은 그 글자에서 바로 내보내므로 체감 지연은 없습니다. 모으는 상한은 들여쓰기를 뺀 41자(볼드 기호 4 + 이름 30 + 공백 6 + 콜론 1)이며, 상한을 넘으면 라벨이 아닌 것으로 보고 모아둔 원문을 내보냅니다. 화자 라벨이면 `character_image` 이벤트(`{name, imageUrl}`)를 먼저 보내고, 볼드를 뗀 `인물명:`을 `token`으로 내보냅니다. 실시간 `token`에는 마커가 들어가지 않습니다.
- **볼드 정리.** `**세린:**`·`**세린**:` 같은 볼드 라벨은 실시간 `token`에서도 평문 `세린:`으로 고쳐 내보냅니다. 정리 정규식은 이름을 30자까지 잡고 공백을 줄 안(`[ \t]`, 최대 2칸)으로 한정해, 줄바꿈을 건너 다음 줄의 콜론까지 한 라벨로 묶지 않습니다.
- **저장 마커.** 턴이 끝나면 완료 본문에서 볼드를 뗀 뒤, 위와 같은 규칙으로 화자 라벨을 다시 찾아 그 대사 줄 **위에** `[[URL]]` 한 줄을 넣고 마커와 대사 사이에 빈 줄을 하나 둡니다(프론트가 마커 줄을 통째로 이미지로 바꾸기 쉽게 — 프론트 요청). 마커에 인물 이름은 넣지 않습니다(KNK-1025) — URL 파일명과 `characterImages[]`에 이미 있어 겹칩니다. 대사 줄의 들여쓰기는 그대로 남습니다. 같은 순서로 `completed.characterImages[]`(`{name, imageName, imageUrl}` — 이벤트와 한 함수로 만들어 모양이 같음, KNK-1026)를 만듭니다. 실시간과 저장이 같은 정규식을 쓰므로 정상 입력에서 이벤트 개수와 마커 개수가 같으며, 이를 테스트로 고정합니다(극단 입력의 예외는 A22).
- **다음 LLM 입력 정리.** 마커와 그 뒤 줄바꿈(최대 2개)을 **복사본에서만** 함께 지웁니다(마커만 지우면 빈 줄이 남습니다). 채팅 턴은 `history[]`의 USER·ASSISTANT 항목 모두에서, 선택지 생성은 `history[]` 전체와 이번 `ai_output`에서, 판정은 이번 `ai_output`에서만 지웁니다. 겹대괄호 안의 모양은 따지지 않으므로 옛 모양 `[[인물이름:URL]]`(개발 서버 기록, 대사 옆에 붙은 것 포함)도 같은 규칙으로 지워집니다(KNK-1025). 옛 `[character:이름]` 태그는 더 지우지 않습니다(KNK-1007) — LLM이 태그를 쓰지 않으므로 남은 것은 개발 서버 옛 기록뿐이고, 운영에는 태그 방식이 배포된 적이 없습니다. DB에 저장한 `aiOutput`은 바꾸지 않습니다.
- **프롬프트.** CORE는 대사 형식만 책임집니다 — 태그 규칙과 응답 전체 예시는 지우고, 이름을 설정 글자 그대로 쓰라는 규칙과 화자 라벨의 좋은 예 1개·나쁜 예 7개(볼드·따옴표·괄호 지문·호칭·글자 다름·역할어 2종)를 두었습니다. CHARACTER는 이미지 보유 인물 목록 슬롯과 "이미지 태그 대상" 절을 지웠습니다. 어느 인물이 이미지를 가졌는지는 프롬프트가 알지 못하며, 이미지 여부 판단은 전부 코드가 합니다.
...
예를 들어 세린과 레이에게 이미지가 있고, 세린이 두 번 말하면 LLM 원문은 다음과 같습니다. 이미지 관련 표시는 없습니다.
...
1. 백엔드가 `character_images[]`에 이미지가 저장된 인물의 `{name, image_name, image_url}`을 실어 보냅니다. AI 서버는 배열 개수를 제한하지 않으며 인물 수는 백엔드가 관리합니다.
2. AI 서버는 이 매핑을 LLM 프롬프트에 넣지 않습니다. 이름도 URL도 서버 메모리의 이름-URL 표로만 보관합니다.
3. LLM이 본문을 스트리밍하면 AI 서버는 줄 머리를 잠시 모아 `인물명:` 라벨인지 확인합니다. 매핑에 있는 이름이면 `{name, imageUrl}`을 담은 `character_image` 이벤트를 보낸 뒤 라벨 글자를 `token`으로 내보냅니다. 위 예시는 세린 → 레이 → 세린 순서로 이벤트 3건이 나갑니다.
4. 턴이 끝나면 같은 규칙으로 라벨을 다시 찾아 그 줄 위에 `[[URL]]` 저장 마커를 넣어 `completed.aiOutput`에 담습니다. 같은 순서와 횟수로 `completed.characterImages[]`도 만듭니다.
5. 이후 LLM 입력을 만들 때는 복사본에서만 저장 마커와 뒤 줄바꿈을 지웁니다(위 "다음 LLM 입력 정리").
...
{
  "characterImages": [
    { "name": "세린", "imageName": "세린_기본", "imageUrl": "https://cdn.example.com/serin.webp" },
    { "name": "레이", "imageName": "레이_기본", "imageUrl": "https://cdn.example.com/rei.webp" },
    { "name": "세린", "imageName": "세린_기본", "imageUrl": "https://cdn.example.com/serin.webp" }
  ]
}
```
...
라벨로 보지 않은 줄은 본문을 임의로 지우지 않고 글자 그대로 내보냅니다.

| 상황 | 처리 |
| --- | --- |
| 줄 머리 `인물명:`의 이름이 `character_images[]`에 있음 | `character_image` 이벤트를 먼저 보내고 라벨 글자를 `token`으로 전송. 완료 본문에서는 그 줄 위에 저장 마커 삽입 |
| 매핑에 없는 인물의 `인물명:` | 이미지 이벤트 없이 대사 줄을 화면과 저장 본문에 그대로 둠 |
| `**인물명:**`·`**인물명**:` 볼드 라벨 | 평문 `인물명:`으로 고쳐 내보내고, 매핑에 있으면 위와 같이 이미지 처리 |
| 이름은 맞지만 `image_url`이 빈 매핑 | 빈 URL의 이미지 이벤트와 `[[인물이름:]]` 마커가 만들어짐(마커가 `[[URL]]`로 바뀌면 `[[]]`, KNK-1025). 이 빈 마커는 현재 제거 정규식에도 잡히지 않아 다음 LLM 입력에 남을 수 있음(A19) |
| 줄 머리가 41자를 넘도록 콜론이 안 나옴 | 라벨이 아닌 것으로 보고 모아둔 원문을 `token`과 저장 본문에 그대로 남김 |
| 지문에 이름만 나옴(`*세린이 웃는다*`) | 발화가 아니므로 이미지 없음 |
...
매핑에 없는 인물이 말해도 AI 서버가 그 자리에서 이미지를 만들거나 과거 턴을 나중에 보충하지는 않습니다. 공유 채팅도 AI 서버에서는 별도 경로가 아닙니다. AI 서버는 공유 여부를 받지 않고 마커가 든 `aiOutput`을 그대로 반환하며, 공유 응답에서 `characterImages`를 제외하고 마커를 화면에 숨기는 일은 백엔드·클라이언트가 맡습니다.

**왜 그 방법.** LLM이 URL을 직접 출력하는 방식은 쓰지 않았습니다. URL이 길어 매 턴 입력 토큰을 늘리고, LLM이 한 글자만 바꿔도 이미지가 깨지기 때문입니다. LLM에게 짧은 내부 태그를 쓰게 하는 방식(KNK-982)은 실측에서 태그 누락이 재현돼 버렸습니다 — 프롬프트를 더 세게 쓰는 안도 검토했지만, 누락은 확률적으로 재발하고 발견도 사용자 화면에서만 됩니다. 화자 라벨을 기준으로 삼으면 LLM이 지켜야 할 규칙이 하나 줄고, 이미지 대상 여부를 LLM이 판단할 필요가 없어 CHARACTER 프롬프트도 짧아집니다. 저장할 때는 `[[URL]]`로 바꿉니다. 인물 이름은 URL 파일명과 `characterImages[]`에 있어 마커에 넣지 않고(KNK-1025), URL은 나중에 과거 턴을 열 때 현재 인물 테이블을 다시 조회하지 않아도 당시 표시한 이미지를 그대로 복원하게 합니다. `aiOutput`의 마커는 이미지가 들어갈 **본문 위치**를 보존하고, `characterImages[]`는 완료 시점에 실제 표시된 **순서와 횟수**를 구조화해 전달합니다. 다음 LLM 입력에서만 이 둘을 제거하는 이유는 URL과 내부 문법을 모델이 따라 쓰게 하지 않으면서도 저장·재조회 정보는 잃지 않기 위해서입니다.

**변경 계획 — 화자 라벨 감지로 전환과 이미지 계약 정리(`Phase 2 · 계획`, KNK-1002 구현 · KNK-1003 스펙).**

> **진행 상태(2026-08-28).** 아래 ①(화자 라벨 감지)과 ②의 "대사 줄 위 별도 줄·빈 줄" 부분은 manyak-ai PR #95(`a0b19e6071b3`)로 구현돼 위 본문에 반영했습니다. ②의 마커에서 인물 이름 빼기(`[[URL]]`)는 KNK-1025로, ③(이미지 이름 계약)은 KNK-1026(채팅 요청·이벤트·완료)과 KNK-1027(컴파일 응답)로 AI 서버 구현을 마쳐(manyak-ai PR #96, `dev` `deb93c43c4dd`) 위 본문과 §5-3-3에 반영했습니다. 백엔드(`story_characters.image_name`·채팅 요청에 싣기)와 프론트(`[[URL]]` 줄 렌더)는 각자 티켓으로 맞춥니다. 아래 "어떻게" 1~8은 모두 구현분입니다. ③의 모양은 백엔드 리뷰(2026-08-28)를 받아 확정했습니다 — 컴파일 응답 `name`은 인물 이름으로 두고 `image_name`을 더합니다(아래 "왜"·"왜 그 방법").

**무엇.** 세 가지를 함께 바꿉니다. ① 이미지를 띄우는 근거를 LLM이 출력하는 `[character:인물명]` 태그에서 **AI 서버가 직접 찾는 화자 라벨(`인물명:`)**로 바꿉니다. LLM에는 태그를 쓰라고 시키지 않고, 이미지 보유 인물 이름 목록도 더는 알려주지 않습니다. ② 저장 마커를 `[[인물이름:URL]]`에서 **`[[URL]]`**로 바꾸고, 대사 줄 바로 앞이 아니라 **대사 줄 위에 빈 줄을 두고 별도 줄**에 둡니다. ③ 이미지에 이름을 붙입니다. 컴파일 응답 `character_images[]`는 `name`(인물 이름)을 그대로 두고 **`image_name`을 더하며**, 채팅 요청 `character_images[]`에 `image_name`을, `character_image` 이벤트와 `completed.characterImages[]`에 `imageName`을 더합니다. 이미지 이름은 AI가 완성해서 주는 문자열이며, 지금은 인물당 한 장이라 `인물이름_기본`입니다.

**왜.** ①은 개발 서버 실측에서 LLM이 몇 턴 뒤부터 태그를 빼먹어 이미지가 뜨지 않았기 때문입니다. 태그는 형식 계약이고, 형식 계약은 프롬프트가 아니라 코드가 담보합니다(D7). 이미지에 필요한 재료는 백엔드가 보내는 이름·URL 매핑과 LLM이 매 대사에 붙이는 화자 라벨뿐이며 둘 다 AI 서버가 이미 갖고 있습니다. ②는 프론트 요청입니다. 마커가 대사와 같은 줄에 붙어 있으면 이미지 요소를 끼워 넣기 불편하고, 마커 줄이 따로 있으면 그 줄을 통째로 이미지로 바꾸면 됩니다. 인물 이름을 마커에서 뺀 것은 URL의 파일 이름에 이미지 이름이 들어가 중복이고, 인물 이름은 `characterImages[]`가 따로 주기 때문입니다. ③은 백엔드 요청입니다. 한 인물에 이미지가 여러 장(표정·배경·상황) 생길 때 인물 이름만으로는 어느 장인지 구분할 수 없어, 이미지마다 설명이 담긴 이름이 필요합니다. 표정처럼 고정 칸으로 나누면 구분 기준이 늘 때마다 칸과 백엔드 조립 로직이 같이 바뀌므로, AI가 완성한 자유 문자열 하나로 받기로 했습니다(2026-08-27 백엔드·프론트 합의). 컴파일 응답의 `name`을 이미지 이름으로 **바꾸지 않고 더하는** 이유는, 라벨 감지가 인물 이름을 글자 그대로 대조하므로 인물 이름이 계약에 따로 있어야 하기 때문입니다 — `세린_기본` 같은 합성 문자열에서 인물 이름을 쪼개 내면 이름에 `_`가 들어갈 때 애매하고, 형식을 문자열에 맡기지 않는다는 D7과도 어긋납니다(2026-08-28 백엔드 리뷰). 운영(`main`, v0.2.6)에는 태그 방식이 배포된 적이 없어 옛 형식이 저장된 대화는 없습니다.

**어떻게.**

1. **감지 기준.** 줄 맨 앞(앞 공백 허용)에서 볼드 기호(`**`)를 걷어낸 뒤 남은 글자가 채팅 요청 `character_images[].name` **또는 그 이름에서 파생한 줄임 이름**과 글자가 같고 바로 콜론(`:`)이 따르면 화자 라벨입니다(KNK-1062, 파생·충돌 규칙과 그 이유는 위 채팅 인물 이미지 단위의 "줄임 이름 파생"). 이름 비교는 앞뒤 공백만 제거하고 대소문자 보정은 하지 않습니다. 라벨 안 공백(볼드 기호와 이름 사이, 이름과 콜론 사이)은 최대 2칸까지 인정합니다. `*`로 시작하는 지문 줄과 등록된 어떤 이름·줄임 이름과도 앞글자가 맞지 않는 줄은 그 글자에서 탈락합니다.
2. **실시간 처리.** 줄이 시작될 때마다 화자 라벨인지 확정될 때까지 글자를 잠시 모읍니다. 모으는 상한은 들여쓰기를 뺀 길이로 세며, 가장 긴 라벨(볼드 기호 4 + 이름 30 + 공백 6 + 콜론 1 = 41자)과 같습니다. 화자 라벨이면 `character_image` 이벤트(`{name, imageName, imageUrl}`)를 먼저 보내고, 볼드를 뗀 `인물명:`을 `token`으로 내보냅니다. 아니면 모아둔 원문을 그대로 `token`으로 내보냅니다. 실시간 `token`엔 마커가 들어가지 않습니다.
3. **볼드 정리 시점.** 완료 본문뿐 아니라 실시간 `token`에서도 화자 볼드를 뗍니다. 줄 앞을 모았다가 내보내므로 추가 지연이 없고, 실시간 화면과 저장 본문이 같아집니다. 볼드 정리 정규식은 이름을 30자(이름 상한)까지 잡고, 공백을 줄 안(`[ \t]`, 최대 2칸)으로 한정합니다 — 줄바꿈을 건너 다음 줄의 콜론까지 한 라벨로 묶던 옛 동작은 스트리밍 파서와 어긋났습니다.
4. **저장 마커.** 턴이 끝나면 완료 본문에서 볼드를 뗀 뒤, 1과 같은 규칙으로 화자 라벨을 찾아 그 줄 위에 `[[URL]]` 한 줄과 빈 줄을 넣고, 같은 순서로 `completed.characterImages[]`(`{name, imageName, imageUrl}`)를 만듭니다. 실시간과 저장이 한 규칙을 쓰므로 이벤트 개수와 마커 개수가 항상 같으며, 이를 테스트로 고정합니다. 저장본 예:
...
5. **다음 LLM 입력 정리.** 마커와 그 뒤 줄바꿈(최대 2개)을 복사본에서 함께 지웁니다. 마커만 지우면 빈 줄이 남습니다. 옛 모양 `[[이름:URL]]`(개발 서버 기록)도 같은 규칙으로 지워집니다. 스트리밍 태그 파서와 `[character:인물명]` 태그 제거 처리는 없앱니다.
6. **요청·응답 필드.** 채팅 요청 `character_images[]` 항목은 `{name, image_name, image_url}`입니다. `name`은 인물 이름이며 대사 줄과 맞추는 키라 **반드시 인물 이름**이어야 합니다. `image_name`은 이미지 이름이고 지금은 고르는 데 쓰지 않으며 받은 값을 그대로 `imageName`에 돌려줍니다. 백엔드가 비워 보내면(칸 없음·빈 문자열·null) 빈 값 그대로 내보냅니다 — 없는 값을 인물 이름으로 채워 있는 척하지 않고, `null`도 거부하지 않습니다(컬럼을 새로 만들 때 기존 행이 null로 올 수 있으므로). 같은 `name`이 두 번 오면 종전처럼 마지막 항목이 쓰입니다(A19). 컴파일 응답 `character_images[]`는 `name`(인물 이름)을 그대로 두고 `image_name`(`인물이름_기본`)을 더합니다. 백엔드는 `image_name`에 uuid를 붙여 파일 이름을 짓고, 인물 연결은 지금처럼 `name`으로 합니다.
7. **프롬프트.** CORE에서 태그 규칙과 응답 전체 예시를 지우고, 이름을 설정에 적힌 글자 그대로 쓰라는 규칙과 화자 라벨의 좋은 예·나쁜 예를 넣습니다. CHARACTER에서 `character_image_names` 슬롯과 "인물 이미지 태그 대상" 절을 지웁니다. 매 답변에 최소 한 인물이 말하게 하는 규칙은 남깁니다.
8. **검증.** 유닛 테스트는 볼드 라벨, 30자 이름, 같은 인물 반복, 이미지 없는 인물, 지문만 있는 줄, 조각으로 나뉘어 오는 델타, 실시간과 저장의 개수 일치를 다룹니다. 실측은 인물 1명·5명으로 각 30턴을 이어 간 자동 실측으로 확인했습니다(2026-08-27, 마커 형식 변경 전): 이미지 인물의 대사 줄 수와 이벤트·마커 수가 60턴 전부 같았고, 이미지가 뜬 턴은 29/30·30/30이었습니다(빠진 1턴은 모델이 대사를 아예 쓰지 않은 턴).

**변경 계획 — 인물당 여러 장 중 문맥 선택(`Phase 3 · 계획`, KNK-1199, 2026-09-05 결정).** 백엔드 KNK-1126으로 사용자가 인물 이미지를 여러 장 올리게 되면서 채팅 요청 `character_images[]`에 같은 `name`의 항목이 여러 개(각각 `image_name` = `{인물이름}_{표정|상황}`) 실립니다. AI 서버는 이름-이미지 표를 이름당 목록으로 바꾸고, 대사 문맥에 맞는 `image_name`을 골라 이벤트·마커에 싣습니다. 판단이 서지 않으면 `_기본`(없으면 첫 장)으로 폴백합니다. 구현 전까지는 아래 종전 결정(마지막 항목 사용, A19)이 그대로입니다.

**종전 결정(2026-08-28).** 한 인물에 이미지 여러 장(표정별)을 받아 대사마다 어느 장을 띄울지 고르는 것은 하지 않았습니다. 간편 제작과 오리지널 스토리 모두 **인물당 한 장**으로 갔습니다. 이유는 간편 제작에서 한 장조차 안정적으로 만들어지고 채팅 화면에 뜨는지 아직 검증되지 않아, 그 위에 고르는 로직을 얹는 것이 이르기 때문입니다. `image_name`은 여러 장이 되더라도 장마다 이름이 필요한 건 같아 지금 넣어도 버려지지 않으며, "추가"라 하위호환도 깨지 않습니다. 여러 장이 필요해지면 고르는 주체(LLM이 대사마다 표정을 표시 / 코드가 별도 판정)·요청 모양(인물당 이미지 목록)·`story_characters`의 인물:이미지 1:N 분리를 그때 한 묶음으로 정합니다.

**왜 그 방법.** 프롬프트를 더 세게 쓰는 안은 버렸습니다. 태그 누락은 확률적으로 재발하고, 발견도 사용자 화면에서만 됩니다. LLM이 URL을 직접 쓰는 안은 기존 결정대로 쓰지 않습니다. 화자 라벨을 기준으로 삼으면 LLM이 지켜야 할 규칙이 하나 줄고, 이미지 대상 여부를 LLM이 판단할 필요가 없어 CHARACTER 프롬프트도 짧아집니다. 이미지 이름을 `name`·`expression`처럼 칸으로 나눠 백엔드가 조립하는 안은, 구분 기준이 늘 때마다 두 쪽이 같이 바뀌어야 해 버렸습니다. 마커에 인물 이름·이미지 이름·URL을 모두 넣는 안은 정보가 URL과 겹쳐 버렸습니다. 컴파일 응답 `name`을 이미지 이름으로 **바꾸는** 안은 백엔드가 인물 이름을 합성 문자열에서 쪼개야 해 버렸습니다(위 "왜"). 채팅 요청을 지금 인물당 이미지 목록 모양으로 바꾸는 안은, 여러 장 자체를 하지 않기로 해 필요가 없어 두지 않았습니다. 구현 뒤에는 이 절의 태그 서술·예시·시퀀스 다이어그램·검수표와 §5-3-3 컴파일 이미지 응답을 이 계획 기준으로 다시 씁니다.
...
- **분량.** 턴당 본문은 700~1000자를 목표로 합니다(채팅 분량 고정, 엔딩 응답 포함). 처음엔 450~650자였고 KNK-1002(2026-08-27)에서 사용자 결정으로 올렸습니다 — 한 장면을 상황·행동·대사·여운으로 충분히 펼치기에 짧다는 판단이었습니다. 다만 같은 날 실측 60턴에서 평균 517자·618자로 모델이 새 범위를 따르지 않았고, 이 미준수는 별도 티켓으로 다룹니다. 분량은 프롬프트가 담보하고, 준수 여부는 `output_char_count` 로깅([`6-analytics.md §6-6-11`](./6-analytics.md))과 평가 지표 `length_adherence`([§5-6](#5-6-운영과-관측))로 실측합니다.

`필드` 진행 카운터 → 요청 `progress_turns` / 응답(`completed` 메타) `progressTurns`

**AI 응답 재생성 — `Phase 1 · 계획`.** 재생성(US-6-10)은 이 계약의 재호출로 처리하며 **AI 서버 변경이 없습니다.** 완전 stateless(D2)의 귀결로 AI는 재생성을 구분할 수 없고 구분할 필요도 없습니다. 백엔드가 마지막 턴의 USER·ASSISTANT 메시지 쌍을 History에서 제외하고(1..N-1턴) 같은 사용자 입력(`user_input`)을 다시 보내면, AI에게는 일반 턴 요청과 형태가 동일합니다(이번 턴 입력은 History가 아니라 `user_input`으로만 전달). 요청·응답 스키마, SSE 이벤트, 오류 처리, 관측 meta 모두 무변경이고 `feature`도 `chat_response`를 유지합니다. 같은 입력의 재호출이라 유사한 본문이 다시 나올 수 있으며 Phase 1은 이를 수용합니다(샘플링 변동에 의존하며, 직전 본문 회피 지시 슬롯 추가는 품질 실측 후 검토). 재생성 트리거·이프·저장 교체 규칙은 [`4-backend.md §4-3-9`](./4-backend.md)가 소유합니다. 채팅 배경 이미지 표시는 백엔드의 결정적 매칭이라 AI가 관여하지 않습니다(같은 절). 스토리 표지는 더 이상 여기 해당하지 않습니다 — 컴파일이 생성해 응답에 싣고(KNK-1047, [§5-3-3](#5-3-3-스토리-컴파일)) 백엔드가 저장·노출합니다(KNK-1069).
...
`prompt/` 아래 12개 템플릿이 전부입니다(D4). 버전 값은 각 파일 frontmatter가 SSOT이므로 이 표에 싣지 않습니다.
...
| `prompt/image/CHARACTER-IMAGE-TEMPLATE.md` | 컴파일 인물 이미지(KNK-414) | 단독 — 이미지 모델용, `<character>` 블록만 치환 | `CHARACTER_IMAGE` |
| `prompt/image/THUMBNAIL-IMAGE-TEMPLATE.md` | 컴파일 썸네일(KNK-1047) | 단독 — 이미지 모델용, 장르와 인물 0~2명 치환 | `THUMBNAIL_IMAGE` |
...
- 채팅 본문의 출력 토큰 상한(`max_tokens`)은 분량 고정(700~1000자, [§5-3-4](#5-3-4-채팅-턴)) 도입 시 초과 방지용으로 함께 확정하기로 했으나, 분량 고정 규칙만 먼저 반영되고 상한은 미확정으로 남았습니다. 수치는 실측으로만 정합니다(D9). 이 어긋난 곳은 [§5-7](#5-7-검수-체크리스트) A8이 추적합니다. 이번 컴파일 모델 비교는 A7에서 해소했으며, 이후 다른 모델로 바꿀 때도 D9 절차를 다시 따릅니다.
...
| `invalid_ai_response` | 빈 응답, JSON 파싱 실패, 비객체 응답, 스토리라인 계약 위반(3편 아님·항목 스키마 불일치·추천 3개 아님·본문 비어 있음), 재호출 후에도 필수 필드 누락. 스토리라인 인물 이름 미등장은 재호출 소진 뒤에도 502가 아니라 200으로 내보내고 Sentry 경고만 남깁니다(KNK-1102, [§5-3-2](#5-3-2-스토리라인-생성)) | 502 | LLM이 올바른 형식의 응답을 반환하지 않았습니다. (refill 소진 케이스만 별도 문구: 재호출 후에도 컴파일 결과에 필수 필드가 비어 있습니다.) |
...
| `IMAGE_MODEL` | `gpt-image-2-2026-04-21` | 컴파일 인물 이미지·썸네일 공용(스냅샷 고정). OpenAI API 키를 재사용 |
| `IMAGE_QUALITY` | `low` | 이미지 화질(`low` / `medium` / `high`) |
| `IMAGE_SIZE` | `1024x768` | 인물 이미지 크기(가로 4:3). 썸네일은 이 값과 무관하게 768x1024 고정(KNK-1047) |
...
| `length_adherence` | 채팅 | 턴당 700~1000자 분량 준수([§5-3-4](#5-3-4-채팅-턴)) |
...
| 관측 | 모든 응답에 meta 동봉, `prompt_versions` 키 구성(채팅 턴=7키, 선택지=1키, 스토리라인=1키, 컴파일=컴파일 템플릿+`CHARACTER_IMAGE`+`THUMBNAIL_IMAGE` 3키), Sentry 원문 비수집. 애플리케이션·접근 로그는 stdout 한 줄 JSON이고 요청 식별자를 같은 필드로 포함하며, 성공한 헬스체크만 제외하고 실패한 헬스체크·다른 접근 로그는 보존 |
...
| 채팅 인물 이미지 `Phase 2 · 구현` | 이미지 보유 인물의 모든 `인물명:` 대사 줄마다 라벨 글자보다 먼저 `character_image` 이벤트, 여러 인물과 같은 인물 재발화 모두 반복, `**인물명:**` 볼드 라벨은 평문으로 고쳐 `token`에 내보냄, 실시간 `token`에 마커 없음, 완료 본문에 대사 줄 위 별도 줄(뒤에 빈 줄)로 `[[URL]]` 마커가 이벤트와 같은 개수·순서로, `characterImages`도 같은 순서, 매핑에 없는 인물의 대사는 글자 그대로. 채팅 History는 USER·ASSISTANT 모두, 선택지는 History와 `ai_output`, 판정은 `ai_output` 복사본에서 마커와 뒤 줄바꿈 제거, 옛 `[character:이름]` 태그는 지우지 않음. 빈 URL·중복 이름 제한은 A19, 극단 입력의 실시간·저장 어긋남은 A22로 별도 추적 |
| 컴파일 썸네일 `Phase 2 · 구현` | `thumbnail_image`가 항상 객체(null 없음), 세로 3:4(768x1024) WebP, 표지 인물은 외형이 채워진 앞 1~2명이고 그런 인물이 없어도 장르만으로 생성, 썸네일 실패에도 컴파일 200과 인물 이미지 유지, 실패가 Sentry에 원문 없이 기록됨, `meta.prompt_versions`에 `THUMBNAIL_IMAGE` 포함 |
| 주인공 이름 반영 `Phase 1 · 계획` | 주인공 이름이 `user_role_setting`의 호칭에 포함되고 채팅이 이름으로 일관 지칭 — 벤치 `setting_consistency`로 검증 |
| 채팅 분량 `Phase 1 · 구현` | 턴당 본문 700~1000자 준수 — `output_char_count` 분포와 벤치 `length_adherence`로 실측 |
...
| A21 | 채팅 인물 이미지가 LLM 태그 출력에 의존 | 이미지 이벤트·저장 마커의 근거가 LLM이 쓰는 `[character:인물명]` 태그라, LLM이 태그를 빼먹으면 이미지가 조용히 빠집니다. 개발 서버 실측에서 몇 턴 뒤부터 누락이 재현됐습니다. 형식 계약을 프롬프트에 맡긴 것이라 D7과도 어긋납니다. 운영에는 이 방식이 배포되지 않았습니다. 같은 자리에서 마커 형식(`[[인물이름:URL]]`, 대사 옆)과 이미지 이름 계약(컴파일 `name`이 인물 이름)도 백엔드·프론트 요청으로 함께 바꾸기로 했습니다 | `해소`(2026-08-28) — 태그 의존은 manyak-ai PR #95(`a0b19e6071b3`, KNK-1002)로 해소됐습니다. AI 서버가 `인물명:` 줄을 직접 찾아 이미지를 만들고, 마커를 대사 줄 위 별도 줄(빈 줄 포함)에 둡니다([§5-3-4](#5-3-4-채팅-턴)). 함께 바꾸기로 한 계약도 AI 서버 쪽은 KNK-1014(manyak-ai PR #96 `deb93c43c4dd`)로 마쳤습니다 — 마커 `[[URL]]`(KNK-1025), 채팅 요청 `image_name`·이벤트/완료 `imageName`(KNK-1026), 컴파일 `character_images[].image_name`(KNK-1027, `name`은 인물 이름 유지 — 모양은 2026-08-28 백엔드 리뷰로 확정). 백엔드(`story_characters.image_name` 컬럼·채팅 요청에 싣기)와 프론트(`[[URL]]` 줄 렌더)는 각자 티켓으로 맞춥니다(`4-backend.md`·`3-1-client.md`·`0-glossary.md`, KNK-1003) |
