...
| 기준 코드 | `../manyak-ai` `dev` 브랜치 `a502c81aa0df`(2026-08-31, Python 3.11 · FastAPI). 컴파일 표지 썸네일 생성(KNK-1047)까지 이 `dev`에 병합된 상태를 대조했습니다. |
...
**채팅 인물 이미지 — `Phase 2 · 구현`(상위 KNK-982, 요청 계약 KNK-989 · 저장과 재입력 정리 KNK-992 · 화자 라벨 감지 전환 KNK-1002: 파서 KNK-1005 · 프롬프트 KNK-1006 · 옛 태그 처리 제거 KNK-1007).**
...
- **감지 기준.** 줄 맨 앞(들여쓰기 허용)에서 볼드 기호(`**`)를 걷어낸 뒤 남은 글자가 채팅 요청 `character_images[].name`과 **글자 그대로 같고** 바로 콜론(`:`)이 따르면 화자 라벨입니다. 이름 비교는 앞뒤 공백만 제거하고 대소문자·별칭 보정은 하지 않습니다. 볼드 기호와 이름 사이, 이름과 콜론 사이 공백은 최대 2칸까지 인정합니다. 이름이 겹치면(`세린`·`세린아`) 더 긴 이름을 먼저 대조합니다.
...
1. **감지 기준.** 줄 맨 앞(앞 공백 허용)에서 볼드 기호(`**`)를 걷어낸 뒤 남은 글자가 채팅 요청 `character_images[].name`과 **정확히 같고** 바로 콜론(`:`)이 따르면 화자 라벨입니다. 이름 비교는 앞뒤 공백만 제거하고 대소문자·별칭 보정은 하지 않습니다. 라벨 안 공백(볼드 기호와 이름 사이, 이름과 콜론 사이)은 최대 2칸까지 인정합니다. `*`로 시작하는 지문 줄과 등록된 어떤 이름과도 앞글자가 맞지 않는 줄은 그 글자에서 탈락합니다.
