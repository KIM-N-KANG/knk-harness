...
| `Phase 2 · 구현` | Phase 2 범위. AI 서버 구현 완료 | 주인공 이름·성별 주입(KNK-838, [§5-3-3](#5-3-3-스토리-컴파일)). 성별 칸 추가(KNK-838). 인물별 이미지 생성(KNK-414, [§5-3-3](#5-3-3-스토리-컴파일)) — 컴파일 시 외형 필드로 이미지를 만들어 base64로 반환 |
| `Phase 2 · 계획` | Phase 2 범위. 미구현, 방향만 합의됨 | 요약(`summary`) 생성(백엔드 소유) — AI의 수신 계약은 구현되어 빈 문자열로 동작 중. 인물 단위 입력(KNK-834, [§5-3-2](#5-3-2-스토리라인-생성)). 채팅 인물 이미지 출력(KNK-981, [§5-3-4](#5-3-4-채팅-턴)) — 스트리밍 중 태그 치환으로 인물 이미지 전달. 스토리 썸네일 AI 생성(KNK-982) — 별도 엔드포인트로 컴파일 후 비동기 호출 |
...
3. 빈 필수 필드를 탐지합니다. 공백만·빈 배열·null도 빈 값으로 판정하며, 추천 입력(`suggested_inputs`)은 정확히 3개가 필수입니다. 사용자가 이름 지은 주변 인물이 인물 카드에 없으면 카드 블록도 빈 필드와 같이 재호출 대상에 넣습니다(KNK-833).
4. 빈 필드가 속한 **블록만** 부분 재호출(refill)로 다시 채웁니다(최대 2회, 토큰은 합산). 재호출 뒤에도 장르와 주인공을 다시 덮어씁니다. 그래도 비면 502로 실패합니다.
5. 스키마를 파싱하고(실패 시 502) 통글 4필드로 렌더링합니다.
6. 인물 카드의 외형 필드로 인물별 이미지를 병렬 생성합니다(KNK-414). 외형 필드가 비어 프롬프트를 조립할 수 없는 인물은 건너뜁니다. 이미지 생성이 실패해도 위 1~5단계의 결과는 그대로 200으로 반환합니다.
...
graph TD
    ...
    Inject --> Check{"빈 필수 필드 있나?<br/>(공백만·빈 배열·null·인물 카드 누락 포함)"}
...
| `character_images[]` | 객체 배열 (0~5개) | 인물별 이미지 `{name, image_base64, content_type, error}`(KNK-414). 성공한 인물은 `image_base64`에 WebP base64 문자열과 `content_type: "image/webp"`, 실패한 인물은 `error`에 사유 코드(`timeout`, `rate_limited`, `rejected`, `appearance_missing`, `generation_failed`). 백엔드는 `image_base64`를 디코딩해 S3에 올리고 `content_type`을 Content-Type으로 설정. 빈 배열은 인물이 0명이거나 이미지 생성 로직 자체가 실패한 경우. `character_appearances`는 인물 전원, `character_images`는 외형이 있는 인물만 포함하므로 두 배열의 길이가 다를 수 있으며 `name`으로 매칭 |

- **인물별 이미지 생성 — `Phase 2 · 구현`(KNK-414).** 컴파일이 성공하면 LLM이 채운 인물 카드의 외형 필드(age·body·face·hair·outfit·visual_identity)로 인물별 이미지를 병렬 생성합니다. 이미지 생성은 컴파일의 부가물이라, 실패해도 컴파일 자체는 200을 반환합니다. 이미지가 필요한 이유는 채팅 플레이에서 인물이 말할 때 해당 인물의 이미지를 화면에 보여주기 위해서입니다. 컴파일 시점에 한 번 만들어 두면 이후 채팅에서 매번 생성하지 않아도 됩니다. 외형 필드는 통글에는 싣지 않지만, `character_appearances` 배열로 백엔드에 별도 전달합니다. 백엔드는 이를 DB에 저장해 썸네일 생성·인물 이미지 재생성에 활용합니다. 이미지는 WebP 바이너리를 base64로 인코딩해 응답에 실어 보내고, 백엔드가 디코딩해 S3에 저장합니다. 이미지를 직접 S3에 올리지 않고 base64로 넘기는 이유는 AI 서버가 저장소를 모르는 stateless 구조를 유지하기 위해서입니다(D2).
...
| `character_images[]` | 객체 배열 | 인물 이미지 매핑(`Phase 2 · 계획`, KNK-981). `name`(인물 이름) + `image_url`(이미지 URL). 백엔드가 DB에서 조회해 채워 보냄. 이 매핑이 있어야 AI 서버가 태그를 이미지 이벤트로 치환할 수 있음 |

AI가 발행하는 SSE 이벤트는 5개입니다(시작 이벤트 `started`는 백엔드 발행, 채팅·턴 식별자 `chatId`·`turnId`도 백엔드 부착. `character_image`는 `Phase 2 · 계획`으로 미구현):
...
sequenceDiagram
    ...
    alt 본문 생성 성공
        loop 생성이 끝날 때까지
            LLM-->>AI: 토큰 델타
            alt [character:이름] 태그 감지(KNK-981)
                AI->>AI: 매핑에서 URL 조회
...
**채팅 인물 이미지 — `Phase 2 · 계획`(KNK-981).**

**무엇.** 채팅 답변에 주변 인물이 등장하면, 그 인물의 이미지를 스트리밍 도중에 `character_image` 이벤트로 전달합니다. 턴당 최대 1건입니다.

**왜.** 컴파일 때 생성한 인물 이미지(KNK-414)를 채팅에서도 보여주면 몰입감이 높아집니다. 인물이 말하거나 등장하는 시점에 바로 이미지가 떠야 하므로, `completed`까지 기다리지 않고 스트리밍 중에 전달합니다.

**어떻게.** 태그 치환 방식입니다.

1. 백엔드가 채팅 요청의 `character_images[]`에 인물 이름과 이미지 URL 매핑을 실어 보냅니다.
2. AI 서버가 채팅 프롬프트에 인물 이름 목록과 태그 출력 규칙을 추가합니다. LLM은 URL을 모르고, 인물이 등장하면 `[character:이름]` 태그만 출력합니다.
...
**왜 그 방법.** URL 직접 출력 방식(LLM이 프롬프트에 받은 URL을 본문에 직접 쓰는 방식)을 기각했습니다. 이유는 두 가지입니다. ① LLM이 URL을 한 글자라도 틀리면 이미지가 깨집니다. ② 매 턴마다 URL 5개가 입력 토큰으로 들어가 100턴이면 500번 반복됩니다. 태그 방식은 LLM이 인물 이름만 맞추면 되고, URL 조회는 AI 서버가 요청 처리 중 변수로 만든 매핑에서 즉시 처리합니다.

| 상황 | 처리 |
| --- | --- |
| 매핑에 없는 인물 태그 | 태그 삭제, 이미지 없이 본문만 전달 |
| 인물이 등장하지 않는 답변 | 이미지 없는 정상 응답 |
| 인물이 2명 이상 등장 | 첫 번째만 채택 |
| 태그 깨짐 (`]` 안 닫힘) | 30자 초과 시 버퍼 텍스트를 `token`으로 그대로 전달 |
...
| `GEMINI_API_KEY` | (빈 값) | Gemini 모델을 고를 때 필요(KNK-951). 기본 컴파일 모델이 Terra인 동안에는 없어도 됩니다 |
...
| `IMAGE_TIMEOUT` | `60.0` | 이미지 1장 생성 제한 시간(초) |
