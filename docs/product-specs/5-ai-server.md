...
6. **요청·응답 필드.** 채팅 요청 `character_images[]` 항목은 `{name, image_name, image_url}`입니다. `name`은 인물 이름이며 대사 줄과 맞추는 키라 **반드시 인물 이름**이어야 합니다. `image_name`은 이미지 이름이고 지금은 고르는 데 쓰지 않으며, 비어 있으면 AI가 인물 이름으로 대신 채웁니다. 같은 `name`이 두 번 오면 종전처럼 마지막 항목이 쓰입니다(A19). 컴파일 응답 `character_images[]`는 `name`(인물 이름)을 그대로 두고 `image_name`(`인물이름_기본`)을 더합니다. 백엔드는 `image_name`에 uuid를 붙여 파일 이름을 짓고, 인물 연결은 지금처럼 `name`으로 합니다.
