...
| 기준 코드 | `../manyak-ai` `dev` 브랜치 `604b68ab060d` (2026-07-22, Python 3.11 · FastAPI). 운영 릴리스는 `v0.2.1`(main `4bc9c1a`)로 dev와 동일 내용입니다 |
...
| `Phase 1 · 구현` | Phase 1 범위. AI 서버 구현 완료. 백엔드가 판정 재료를 실어 보내는 런타임 배선도 완료([`4-backend.md §4-3-10`](./4-backend.md) 소관) | 주요 사건·엔딩 진행(D11·D12, [§5-3-3](#5-3-3-스토리-컴파일)·[§5-3-4](#5-3-4-채팅-턴)·[§5-3-5](#5-3-5-선택지-생성-별도-api)). 채팅 분량 고정(450~650자, [§5-3-4](#5-3-4-채팅-턴)). 로어북 반영([§5-3-3](#5-3-3-스토리-컴파일)). LLM 트레이싱(Langfuse) — 관측 그릇만 구현이고 프로덕션은 미활성([§5-6](#5-6-운영과-관측)) |
...
| 관측 | Sentry SDK(오류 수집) · Langfuse(LLM 호출 트레이싱, prod 미활성) — Sentry는 키 미설정 시, Langfuse는 키·JP·prod 미충족 시 no-op |
...
| `LANGFUSE_PUBLIC_KEY` · `LANGFUSE_SECRET_KEY` | (빈 값 = no-op) | prod에서만 채움(켜기는 KNK-654) |
...
> 관측 그릇(트레이스 묶기·차원 부착, KNK-624)과 켜기 전 안전장치(활성화 가드·채팅 트레이스 장르 태그 제거·실패 격리, KNK-652)는 **운영에 배포 완료**입니다(AI `v0.2.1`, 2026-07-22). 인프라 배선(KNK-653, manyak-terraform)은 **머지 완료·`terraform apply` 대기**라 운영 EC2에는 아직 반영되지 않았습니다. 그래서 **프로덕션은 여전히 미활성**입니다 — 키가 Secrets Manager에 없고, 있어도 배선 전이라 컨테이너로 전달되지 않습니다. 켜기(키 발급·주입·검증)는 KNK-654이고, apply와 백엔드 키워드 단계 개편(KNK-621) 배포가 선행입니다(장르 정제 — [`7-deployment.md §7-9`](./7-deployment.md)). 턴·스토리 식별자 수신은 백엔드 협의 대상입니다.
...
- 분석 차원을 함께 싣습니다 — 저카디널리티 태그(사전 정의 장르)와 metadata(프롬프트 버전 맵·`retry_count`·`request_id`). 장르 태그는 **스토리 제작 트레이스(스토리라인·컴파일)에만** 싣습니다 — 채팅 턴·선택지 트레이스의 장르 태그는 KNK-652에서 제거했습니다(채팅 트레이스에 장르가 필요한지는 후속). **PII 경계**: tags·metadata에는 구조화된 장르·버전만 싣고 사용자 자유입력(주인공·조연 커스텀 태그·`user_input`·`additional_info`)은 넣지 않습니다. 다만 이 자유입력은 프롬프트의 일부라 **Langfuse 요청 원문에는 저장됩니다** — 색인 차원(tags·metadata)으로 올리지 않을 뿐입니다.
