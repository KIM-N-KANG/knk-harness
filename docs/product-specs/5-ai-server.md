...
- `background_tags`(`string[]`) 추가 — 배경 키워드(장르와 함께 세계관 그룹, 각 1~2개).
- `supporting_tags` 대신 주변 인물 세트 `supporting_characters[]`(최대 5)를 받습니다. 각 항목 `{name, gender, features}`는 전부 선택이며, 빈 항목은 스토리라인·컴파일이 자동 생성합니다. 인물 카드 1~5명 상한([§5-3-3](#5-3-3-스토리-컴파일))과 정렬됩니다.
- 컴파일(`POST /story/compile`)도 같은 재료(`background_tags` · `supporting_characters[]`)를 받습니다. 프롬프트 반영(태그 귀속 규칙의 배경 축 확장 포함)과 필드 스키마는 구현 시점에 확정합니다.
...
- **`/chat/turns` 변경** — 선택지 호출을 제거하고 판정 호출(D12)은 유지합니다(판정은 토글 대상이 아니고 백엔드가 상태로 저장하는 값). `completed`의 meta에서 선택지 몫(토큰·`retry_count`)이 빠지고, `choices` 필드를 빈 배열로 유지할지 제거할지는 프론트엔드와 확정합니다(미정).
