...
| 기준 코드 | `../manyak-ai` `dev` 브랜치 `73ab29c9630d` (2026-07-09, Python 3.11 · FastAPI) |
...
| `Phase 1 · 구현` | Phase 1 범위. AI 서버 구현 완료. 백엔드가 판정 재료를 실어 보내면 동작하며, 그 런타임 배선은 [`4-backend.md §4-3-10`](./4-backend.md) 소관 | 주요 사건·엔딩 진행(D11·D12, [§5-3-3](#5-3-3-스토리-컴파일)·[§5-3-4](#5-3-4-채팅-턴)·[§5-3-5](#5-3-5-선택지-생성-내부-호출)). 채팅 분량 고정(450~650자, [§5-3-4](#5-3-4-채팅-턴)). 로어북 반영([§5-3-3](#5-3-3-스토리-컴파일)) |
...
`Phase 1 · 구현`입니다. AI 서버 쪽 구현은 끝났고, 백엔드가 판정 재료를 요청에 실어 보내는 런타임 배선이 남았습니다([`4-backend.md §4-3-10`](./4-backend.md) 소관).
...
**주요 사건·엔딩 진행 — `Phase 1 · 구현`.** D11이 정한 분담의 와이어 계약입니다. AI 서버 쪽 구현은 끝났고, 백엔드가 판정 재료를 요청에 싣기 전까지 판정 호출은 건너뜁니다(재료가 없으면 호출하지 않아 현행 트래픽에 비용·지연이 없습니다 — D12). 백엔드의 재료 구성·상태 저장 규칙은 [`4-backend.md §4-3-10`](./4-backend.md)이 소유합니다.
...
Sentry 캡처 항목(AN-4-8): 태그 `feature` · `provider` · `model` · `error_code`, 컨텍스트 `prompt_versions` · `retry_count` · `latency_ms`. chat SSE는 HTTP 200이라 미들웨어가 실패를 못 잡으므로 스트림 내부에서 직접 캡처합니다. 다만 컨텍스트 3종을 모두 채우는 것은 story 경로뿐입니다. chat 경로(본문·선택지·판정)는 현재 `prompt_versions`만 싣고, 소요 시간(`latency_ms`) 측정 자체가 없어 계약에 못 미칩니다([§5-7](#5-7-검수-체크리스트) A9).
...
| A6 | 주요 사건·엔딩 진행 규칙 | **해소** — AI 서버 구현이 KNK-413·417·420·422·465로 `dev`에 머지 완료(판정 호출은 D12로 등록). 남은 조각은 백엔드 런타임 배선([`4-backend.md §4-8`](./4-backend.md) B5)으로, 이 문서 범위 밖 | 해소(2026-07-10, KNK-517 스펙 반영). 평가는 KNK-299([§5-6](#5-6-운영과-관측)) |
...
| A9 | chat 실패 캡처의 관측 컨텍스트 누락 | 관측 계약(AN-4-8)은 실패 캡처에 `retry_count` · `latency_ms`를 요구하지만, chat 경로(본문·선택지·판정)의 Sentry 캡처는 `prompt_versions`만 싣습니다. chat 경로는 소요 시간 측정 자체가 없습니다. 캡처 함수는 세 값을 모두 받도록 준비돼 있어 호출부 배선만 남은 상태입니다([§5-6](#5-6-운영과-관측)) | `계획` — manyak-ai 코드 수정(전담 티켓 미발번) |
