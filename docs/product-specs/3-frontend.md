...
**진입점** — 상단 헤더 우측 프로필 아이콘. 게스트는 아이콘 → `/login`, 회원은 아이콘 → 계정 시트(닉네임·프로필 이미지 표시, 로그아웃 버튼). 게스트/회원 판정은 [§3-8](#3-8-api-연동에러-처리-계약) 토큰 세션의 모드 판정 소스를 따릅니다.
...
| 채팅 화면 | `POST /chats/{id}/turns/stream` | 전송·선택지 | 토큰 누적 → 완료 시 refetch | 실패 토스트 |
