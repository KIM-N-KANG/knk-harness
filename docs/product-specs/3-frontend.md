...
- **리뷰·QA** — 각 화면·절의 검수 기준과 §3-12 e2e 매핑, §3-13 간극 표(G2~G7)에서 시작해 본문을 역참조합니다.
...
| 기준 코드 | `../manyak-web` `dev` 브랜치 `330ab4156192` (2026-08-02, Next.js 16 App Router, React 19) |
...
| `Phase 1 · 구현` | Phase 1 범위. 코드 반영 완료            | Google·Kakao 로그인·계정 연동·로그아웃·BFF 토큰 세션·자동 마이그레이션·회원 서재·마이 페이지·이관 1회 안내·약관 동의 고지·초대 코드 공유·입력·신규 가입 온보딩·크레딧 잔액·출석·소모량 고지·체험 한도 선차단·402 다이얼로그(FE-SCREEN-008, [§3-8](#3-8-api-연동에러-처리-계약))·서비스이용약관·개인정보처리방침 페이지(FE-SCREEN-010)·서비스 안내 페이지(FE-SCREEN-011)·상세 썸네일 히어로·이미지 뷰어·시작 설정 선택(FE-SCREEN-003)·AI 응답 재생성([§3-6](#3-6-채팅-플레이와-sse-스트리밍))·인앱 브라우저 감지·탈출([§3-10](#3-10-반응형접근성브라우저-지원))·채팅 공유 발급·열람(FE-SCREEN-012, [§3-6](#3-6-채팅-플레이와-sse-스트리밍))   |
...
경로는 MVP 6개 화면 라우트와 1개 프록시 라우트에 `/login`·`/my`·`/my/invite`·`/my/link/continue`·`/terms`·`/privacy`(구현 완료), `Phase 1 · 계획` 2개(`/stories/new/general`, `/stories/[id]/edit`), `계획` 1개(`/my/about`)를 더해 구성됩니다. 여기에 화면이 아닌 검색 크롤러용 메타데이터 라우트 2개(`/robots.txt`·`/sitemap.xml`)가 있습니다. 회원가입 별도 경로·대시보드·설정·관리자 경로는 없습니다. 동적 세그먼트 `[id]`는 문자열입니다.
...
**카카오 로그인 버튼** (`Phase 1 · 구현`, KNK-721·KNK-728) — Google 버튼 위에 카카오 로그인 버튼("카카오로 시작하기")을 세로로 둡니다. 두 버튼은 같은 공통 시작 함수([§3-10](#3-10-반응형접근성브라우저-지원))를 provider 인자만 달리해 호출하며, 로그인 성공 이후 처리(토큰 보관·자동 마이그레이션·완료 피드백·신규 가입 온보딩)는 provider와 무관하게 동일합니다. 버튼 시각 사양은 카카오 디자인 가이드를 따릅니다.
...
- 채운 뒤 내용을 전부 지우고 새로 써도 `edited_choice`입니다. 추천을 본 뒤의 입력이라 순수 `typed`와 구분하는 편이 선호 분석에 유용하고, "고쳐 쓴 것"의 극단으로 두는 편이 규칙이 단순합니다. 이는 KNK-757의 현행 구현 기준이며, KNK-762 전환 뒤에는 [§3-11](#3-11-관측-연동) 계약대로 입력을 완전히 비운 시점에 선택지 연결을 지우고 이후 새 입력을 `typed`로 보냅니다.
...
- **무엇.** 프론트엔드는 시작 추천 입력과 턴 선택지가 실제로 화면에 표시되고, 사용자가 채우기·바로 전송·무작위 전송·직접 입력 중 무엇을 선택했는지 백엔드에 전달합니다. 기존 `parentCreationId`와 `userSource` 전달은 구현됐고(KNK-755·KNK-757), 선택지 고유 ID와 노출·선택·반영 추적은 KNK-762 계획입니다.
...
- **어떻게.** 백엔드는 시작 추천 입력과 턴 선택지를 `{choiceId, choiceOrder, text}` 객체로 함께 제공합니다. 기존 `suggestedInputs: string[]`·`choices: string[]`는 프론트 전환이 끝날 때까지 유지합니다. 프론트는 시작 추천 입력을 `START_SUGGESTION`, 턴 선택지를 `TURN_CHOICE`로 상태에 보존하고 다음 규칙을 적용합니다.
  ...
  아직 정하지 않은 프론트 구현 세부사항은 manyak-web 레포의 선택지 연동 티켓이 결정합니다. 대상은 `selectionAttemptId`의 생성 형식·수명, 노출 감지 방식, 비동기 기록 재시도와 화면 상태 보존 방식입니다. `inputAttemptId`의 생성 주체·형식·수명과 API 경로·필드 자료형·필수 여부·오류 응답은 manyak-server와 manyak-web의 계약 티켓에서 확정합니다. 이 구현 세부사항은 위에서 정한 행동 시점과 성공·실패 판정 규칙을 바꾸지 않습니다.
- **왜 이 방법.** 고유 ID는 문장 비교보다 정확하고, `PRESENTED`·`SELECTED`·다음 턴 저장 결과를 분리하면 “볼 수 있었음”, “눌렀음”, “실제로 반영됨”을 섞지 않습니다. 프론트는 화면과 클릭처럼 자신만 아는 사실만 보내고, 소유 관계·현재 활성 선택지·최종 저장 성공은 백엔드가 검증합니다. 행동 기록을 채팅 전송과 분리하면 관측 장애가 사용자 기능을 막지 않습니다. score 이름과 해석은 [`6-analytics.md §6-6-12`](./6-analytics.md), 서버 검증과 발행은 [`4-backend.md §4-7`](./4-backend.md)이 소유합니다.
...
| G7  | Langfuse 선택지 행동 연결 | `parentCreationId`·`userSource` 전달은 구현됐지만, 선택지 고유 ID 수신과 노출·선택 행동 전달은 미구현([§3-11](#3-11-관측-연동)) | `Phase 1 · 계획`(KNK-762) — 백엔드 객체 응답·상호작용 API와 함께 구현하고 선택지 연결 상태·409 복구·결과 미확정 경로를 E2E로 검증 |
