...
| `Phase 1 · 계획` | Phase 1 범위. 미구현, 방향만 합의됨 | 로그인·계정·크레딧 UI(FE-SCREEN-008, [§3-7](#3-7-게스트-데이터-모델)·[§3-8](#3-8-api-연동에러-처리-계약)). 일반 제작·스토리 수정(FE-SCREEN-009, [§3-9](#3-9-입력폼-검증-규칙)). AI 응답 재생성·채팅 이미지([§3-6](#3-6-채팅-플레이와-sse-스트리밍))·썸네일 표시(FE-SCREEN-001·003) |
...
| 관련 API | `GET /stories/{storyId}`, `POST /chats` |
| 관련 US | US-4-1 ~ US-4-4 |
...
| 관련 US | US-6-1 ~ US-6-8, US-6-10 · US-6-11 `Phase 1 · 계획` |
...
**폼 구성** — 섹션 아코디언 구조. 필수: 기본 정보(제목·한 줄 소개·장르), 스토리 설정(세계관·등장인물·주인공·규칙 — 섹션별 텍스트 입력을 통글 마크다운으로 조립해 전송), 시작 설정(이름·프롤로그·시작 상황), 추천 입력 3개. 선택: 주요 사건(최대 10 — 이름·설명·키 문장), 엔딩(넣으려면 3종 세트 — 해피·노말·배드 각 1), 이미지(최대 10 — 이름·트리거 문구·업로드 또는 프리셋 선택, 썸네일 지정. 프리셋 목록은 `GET /images/presets`). 주요 사건·엔딩은 KNK-444 확정 전까지 저장·수정 폼 왕복용 초안이며 채팅 런타임 반영은 이 화면의 검수 대상이 아닙니다. 이미지는 썸네일·채팅 이미지 표시([`4-backend.md §4-3-9`](./4-backend.md))로 런타임에 반영되며, 등록한 트리거 문구가 채팅 이미지 매칭에 쓰입니다. 게스트는 이미지 업로드가 비활성화되고 프리셋만 선택할 수 있습니다.
...
**재생성 버튼** — 마지막 턴의 AI 버블 하단에 "다시 생성" 버튼을 둡니다(US-6-10). 마지막 턴에만 표시하고, 스트리밍 중에는 전송 잠금과 같은 조건으로 비활성화합니다.
...
**재생성 실패 처리** — `error` 이벤트·네트워크 실패 시 기존 본문·선택지를 복원하고 실패 토스트를 표시합니다(서버도 성공 시에만 교체하므로 refetch 결과와 일치). 402는 턴 진행과 동일하게 크레딧 부족(회원)·체험 종료(게스트) 다이얼로그를 띄우고 기존 본문을 유지합니다. 409(이미 새 턴이 추가된 낡은 화면)는 상세 refetch로 최신 상태를 반영합니다.
...
- `Phase 1` 다시 생성 시 마지막 AI 응답이 새 본문으로 교체되고 사용자 입력 버블은 유지돼야 합니다. 실패하면 기존 본문이 복원돼야 합니다. (US-6-10)
...
| 재생성 `Phase 1 · 계획` | `POST /chats/{id}/turns/regenerate/stream` | 마지막 AI 버블 "다시 생성" | 토큰 누적 → 완료 시 refetch | `error`=본문 복원 + 토스트, 402=크레딧/체험 다이얼로그, 409=refetch([§3-6](#3-6-채팅-플레이와-sse-스트리밍)) |
