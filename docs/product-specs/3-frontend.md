...
MVP 본문은 로그인 없는 전원 게스트 기준입니다. Phase 1 로그인·더보기 페이지·BFF 토큰 세션·회원 서재·크레딧 잔액과 출석·소모량 고지·체험 한도 처리는 `Phase 1 · 구현`입니다. 일반 제작·스토리 수정 흐름은 `Phase 1 · 계획` 라벨로 본문에 포함됩니다(FE-SCREEN-008·009, 토큰 세션은 [§3-8](#3-8-api-연동에러-처리-계약)).
...
| `Phase 1 · 구현` | Phase 1 범위. 코드 반영 완료            | Google 로그인·로그아웃·BFF 토큰 세션·자동 마이그레이션·회원 서재·더보기 페이지·이관 1회 안내·약관 동의 고지·초대 코드 공유·입력·신규 가입 온보딩·크레딧 잔액·출석·소모량 고지·체험 한도 선차단·402 다이얼로그(FE-SCREEN-008, [§3-8](#3-8-api-연동에러-처리-계약))·서비스이용약관·개인정보처리방침 페이지(FE-SCREEN-010)·서비스 안내 페이지(FE-SCREEN-011)·상세 썸네일 히어로·이미지 뷰어·시작 설정 선택(FE-SCREEN-003)·AI 응답 재생성([§3-6](#3-6-채팅-플레이와-sse-스트리밍))·인앱 브라우저 감지·탈출([§3-10](#3-10-반응형접근성브라우저-지원)) |
| `Phase 1 · 계획` | Phase 1 범위. 미구현, 방향만 합의됨     | 일반 제작·스토리 수정(FE-SCREEN-009, [§3-9](#3-9-입력폼-검증-규칙)). 채팅 이미지·엔딩 도달 표시([§3-6](#3-6-채팅-플레이와-sse-스트리밍))·목록 카드 썸네일(FE-SCREEN-001·004)·본 엔딩 표시(FE-SCREEN-003). Safari 렌더링·애니메이션·입력 UX 개선([§3-10](#3-10-반응형접근성브라우저-지원))                                                                                                                                                                                                                                                        |
...
| `/more/about` `Phase 1 · 구현`          | FE-SCREEN-011 | `(more)`    | 서비스 안내               | 뒤로가기 헤더(탭 없음) |
...
**노출 조건** — 비로그인 세션(`unauthenticated`)에서, 저장된 스토리와 채팅이 모두 0이고(`ids != null && length === 0`), 온보딩을 본 적이 없을 때 최초 1회만 표시합니다. 세 조건을 모두 만족하면 약 0.3초 지연 후 엽니다. SSR·미하이드레이션 상태에서는 열리지 않습니다.

**화면 구성·동작** — 전체 화면 다이얼로그입니다. 닫기 버튼·배경 클릭·ESC로 닫히지 않으며, 유일한 출구는 CTA입니다. 제목 "키워드 몇 개로, 나만의 스토리 완성"과 한 줄 설명("복잡한 설정 없이 스토리를 만들고 AI와 채팅하며 나만의 전개를 이어가보세요"), CTA만 두어 첫 진입 장벽을 최소화합니다. 게스트 한도·저장 범위 고지는 이 다이얼로그에서 다루지 않고 서비스 안내 화면(FE-SCREEN-011)과 생성·채팅 화면의 크레딧 안내 팝오버가 담당합니다.
...
**목적** — 게스트를 회원으로 전환하고, 기기에 쌓은 서재를 계정으로 이관합니다. 더보기 페이지는 로그인 진입점과 회원 상태(프로필·크레딧·로그아웃), 친구 초대, 피드백, 서비스 안내(FE-SCREEN-011)를 모아두는 하단 탭 화면입니다.

**진입점** — 하단 탭 "더보기"(`/more`). 게스트는 로그인 버튼(→ `/login`)을, 회원은 프로필·로그아웃을 표시합니다. 게스트/회원 판정은 [§3-8](#3-8-api-연동에러-처리-계약) 토큰 세션의 모드 판정 소스를 따릅니다.

**더보기 페이지 회원 상태 구성** — 게스트는 로그인 버튼을 표시합니다. 회원은 닉네임·프로필 이미지·크레딧 잔액·출석 체크·친구 초대·로그아웃을 표시합니다. 기타 섹션의 피드백과 서비스 안내(FE-SCREEN-011)는 게스트와 회원 모두 진입할 수 있습니다.
...
**진입점** — `/login` Google 로그인 버튼 하단 동의 고지의 밑줄 링크(서비스이용약관 → `/terms`, 개인정보처리방침 → `/privacy` — [FE-SCREEN-008](#3-4-화면별-스펙))와 서비스 안내 페이지(FE-SCREEN-011)의 약관 및 정책 링크에서 진입합니다. 로그인 전 게스트도 접근할 수 있으며 라우트 가드는 없습니다([§3-3](#3-3-라우팅레이아웃공통-셸)). 뒤로가기 헤더로 진입 이전 화면으로 복귀합니다.
...
**콘텐츠 소스** — 정본은 웹 레포 `src/features/legal/content/terms-content.ts`·`privacy-content.ts`이며 페이지가 이를 렌더합니다(초기의 `docs/legal/*.md` 마크다운 초안은 TS 모듈로 정본을 일원화하며 삭제 — KNK-616). 본문은 일반적인 모바일 서비스 표준 항목으로 구성합니다 — 서비스이용약관: 목적·정의·약관 게시와 개정·이용계약·서비스 제공과 변경·이용자 의무·게시물의 관리·서비스 이용 제한·면책·분쟁 해결 등. 개인정보처리방침: 수집 항목·수집 방법·이용 목적·보유 및 이용 기간·제3자 제공·처리 위탁·국외 이전·이용자와 법정대리인의 권리·파기 절차·안전성 확보 조치·쿠키·행태정보의 수집 및 맞춤형 광고(Meta 픽셀 — v1.1, KNK-616)·개인정보 보호책임자와 문의처 등. 개정 시 시행일·버전과 부칙의 개정 이력을 함께 갱신합니다. **사업자 정보·문의처·수집 항목 확정값 등은 법무 검토가 필요합니다.**
...
- **탭 구조** — 첫 탭 "장르"를 "세계관"으로 바꾸고, 세계관 탭 안을 "장르"와 "배경" 두 키워드 그룹으로 나눕니다. 탭 순서는 세계관 → 주인공 → 주변 인물입니다.
- **장르·배경 선택 규칙** — 각 그룹에서 1~2개를 선택합니다(기존 장르 최대 3 대체). 두 그룹 모두 커스텀 키워드를 추가할 수 없으며(서비스 제공 키워드만 선택) "키워드 추가" 트리거를 노출하지 않습니다. 진행 조건의 장르 1개 이상 필수는 유지하고, 배경의 최소 선택 필수 여부는 미정입니다.
- **주변 인물 세트 입력** — 주변 인물 특징 키워드 선택을 인물 세트 입력으로 바꿉니다. 세트는 이름(직접 입력)·성별(선택)·특징으로 구성하며 세 항목 전부 선택 항목입니다 — 비워 두면 AI가 자동 생성합니다. 세트 1개가 주변 인물 1명이고, 항목을 채우지 않은 세트도 인원으로 집계됩니다(빈 세트 3개 = 주변 인물 3명). 최대 5세트이며, 성별 값 집합과 특징 입력 방식(태그 선택·자유 입력)은 미정입니다.
...
### 백그라운드 생성 복귀 — `Phase 1 · 계획`(KNK-623)

모바일에서 스토리라인 생성·완성(스토리 생성) 대기 중 다른 앱으로 전환했다가 돌아와도 진행 상태를 이어서 확인할 수 있어야 합니다(2026-07-20 팀 결정 — 방향만 확정). 서버는 생성을 백그라운드로 계속 진행하고([`4-backend.md §4-3-2`](./4-backend.md)), 프론트엔드는 복귀 시 진행 중·완료·실패 상태를 확인해 화면을 복원합니다. 현재 생성 요청은 열린 HTTP 응답을 기다리는 동기 구조라 앱 전환으로 연결이 끊기면 결과를 잃습니다. 구체 구현 방안(상태 조회·복귀 UI 등)은 모색 중이며 미정입니다.
...
- **추천 입력 토글 — `Phase 1 · 계획`(KNK-622)** — 선택지(추천 입력) 생성을 응답 스트림에서 분리합니다(2026-07-20 팀 합의 — [`4-backend.md §4-3-3`](./4-backend.md)). 인풋 근처에 추천 입력 on/off 토글 버튼을 둡니다. 토글 on이면 `completed` 수신 직후 선택지 생성 API(`POST /chats/{id}/turns/{turnId}/choices`)를 호출해 도착한 3개를 마지막 턴의 선택지로 표시합니다(본문 확정과 선택지 도착 사이의 지연은 수용 — 본문이 먼저 확정되는 체감 개선이 목적). 토글 off면 호출하지 않고, off 상태로 턴이 끝난 뒤 토글을 켜면 같은 API로 마지막 턴의 선택지만 생성합니다. flag 없이 "API를 부르는가"로 on/off를 처리합니다. `completed`의 `choices` 필드 처리(빈 배열 유지 vs 제거)와 토글 기본값·선택지 로딩 표시는 미정입니다.
...
온보딩 다이얼로그의 알파 고지("이 브라우저에만 저장")가 이 한계를 사용자에게 알립니다. 로그인 직후 자동 마이그레이션이 게스트 서재를 회원 서재로 이관합니다(`Phase 1 · 구현` — [FE-SCREEN-008](#3-4-화면별-스펙)).
...
| 크레딧·초대 `Phase 1 · 구현` | `GET /users/me/credits` · `POST /users/me/credits/attendance` · `GET /users/me/invite` · `POST /users/me/invite/redeem`(KNK-567) | 더보기 페이지 진입·버튼 클릭·초대 코드 제출                                           | 잔액·적립 반영                            | 실패 토스트(초대 코드 입력은 사유별 안내 — FE-SCREEN-008)                                                                                           |
...
- `Phase 1 · 계획`(KNK-621) — 키워드 단계 개편이 구현되면 장르·배경은 각 1~2개(커스텀 추가 불가), 주변 인물은 세트 입력(최대 5)으로 바뀝니다([§3-5](#3-5-스토리-생성-퍼널) 키워드 단계 개편).
...
| `chats/chat-room`                         | 프롤로그·빈 입력의 무작위 추천 전송·스트리밍·선택지·실패·설정 드로어 삭제·응답 재생성 교체와 실패 복원 | US-5-3, US-6-1·2·3·4·8·10 |
...
| `more/login-page`                         | Google 로그인·이관 안내·약관 링크·로그인 로딩                                                          | US-9-1·2                  |
| `more/more-page`                          | 더보기 게스트·회원 상태·헤더                                                                           | US-9-1·5, US-10-1·2       |
| `more/invite`                             | 초대 코드 조회·복사·공유·입력 오류·등록 스피너·신규 가입 온보딩                                        | US-10-2                   |
| `visual/*-visual`                         | 계정·채팅·약관·더보기·온보딩·스토리의 안정된 정적 상태                                                 | 각 화면의 연결 US         |
