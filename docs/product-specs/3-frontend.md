...
| `/onboarding`                           | FE-SCREEN-007 | `(onboarding)` | 온보딩                 | 없음(헤더·탭 없음)     |
...
- **`(main)` 레이아웃** — 상단 헤더, 스크롤 콘텐츠 영역, 하단 네비게이션, 온보딩 진입 게이트를 렌더합니다.
...
| 항목    | 값                                                      |
| ------- | ------------------------------------------------------- |
| 경로    | `/onboarding`. 진입 게이트는 `(main)` 레이아웃에 마운트 |
| 관련 US | US-2-5, US-8-3                                          |

**목적** — 처음 온 사용자에게 시작 방법을 안내하고 첫 스토리 생성으로 유도합니다.

**진입 조건** — 비로그인 세션(`unauthenticated`)에서, 저장된 스토리와 채팅이 모두 0이고(`ids != null && length === 0`), 온보딩을 본 적이 없을 때 `(main)` 라우트(`/`·`/chats`·`/more`) 진입을 `/onboarding`으로 주소를 교체해 이동시킵니다. 판정에 필요한 값이 로컬 스토리지에 있어 서버에서는 알 수 없으므로 클라이언트에서 이동하며, SSR·미하이드레이션 상태에서는 판정을 보류합니다. 조건을 만족하지 않는 상태로 `/onboarding`에 직접 접근하면 홈으로 되돌립니다.

**화면 구성·동작** — 전체 화면 페이지입니다. 두 줄 제목("키워드 몇 개로," / "나만의 스토리 완성")과 한 줄 설명("복잡한 설정 없이 스토리를 만들고 AI와 채팅하며 나만의 전개를 이어가보세요"), 그 아래 스토리 만들기 과정을 보여주는 미리보기 콘텐츠, 하단에 버튼 두 개를 둡니다. 게스트 한도·저장 범위 고지는 이 화면에서 다루지 않고 서비스 안내 화면(FE-SCREEN-011)과 생성·채팅 화면의 크레딧 안내 팝오버가 담당합니다.

**사용자 액션** — 두 버튼 모두 온보딩 열람을 저장하고 주소를 교체해 이동하므로, 뒤로가기로 온보딩에 돌아올 수 없습니다.

- "첫 스토리 만들기"(오른쪽) → `/stories/new`로 이동
- "나중에 하기"(왼쪽) → 홈(`/`)으로 이동

**히스토리 동선** — 온보딩이 주소를 교체하며 이동하므로 스토리 생성 퍼널에는 돌아갈 앱 내 히스토리가 없습니다. 온보딩을 거쳐 퍼널에 진입한 경우 키워드 스텝의 뒤로가기는 사이트를 벗어나지 않고 홈으로 이동합니다.
