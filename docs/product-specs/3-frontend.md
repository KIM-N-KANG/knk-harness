...
| `Phase 1 · 계획` | Phase 1 범위. 미구현, 방향만 합의됨 | 크레딧 UI(잔액·출석체크·초대, FE-SCREEN-008, [§3-7](#3-7-게스트-데이터-모델)·[§3-8](#3-8-api-연동에러-처리-계약)). 일반 제작·스토리 수정(FE-SCREEN-009, [§3-9](#3-9-입력폼-검증-규칙)). AI 응답 재생성·채팅 이미지·엔딩 도달 표시([§3-6](#3-6-채팅-플레이와-sse-스트리밍))·목록 카드 썸네일(FE-SCREEN-001·004)·본 엔딩 표시(FE-SCREEN-003). Safari 렌더링·애니메이션·입력 UX 개선([§3-10](#3-10-반응형접근성브라우저-지원)) |
