...
MVP 본문은 로그인 없는 전원 게스트 기준입니다. Phase 1 로그인·마이 페이지 뼈대·BFF 토큰 세션은 `Phase 1 · 구현`으로 구현이 완료되었고, 크레딧·일반 제작·스토리 수정 흐름은 `Phase 1 · 계획` 라벨로 본문에 포함됩니다(FE-SCREEN-008·009, 토큰 세션은 [§3-8](#3-8-api-연동에러-처리-계약)).
...
| `Phase 1 · 구현` | Phase 1 범위. 코드 반영 완료 | Google 로그인·로그아웃·BFF 토큰 세션·자동 마이그레이션·마이 페이지 뼈대·이관 1회 안내·약관 동의 고지(FE-SCREEN-008, [§3-8](#3-8-api-연동에러-처리-계약))·서비스이용약관·개인정보처리방침 페이지(FE-SCREEN-010)·상세 썸네일 히어로·이미지 뷰어·시작 설정 선택(FE-SCREEN-003) |
| `Phase 1 · 계획` | Phase 1 범위. 미구현, 방향만 합의됨 | 크레딧 UI(잔액·출석체크·초대, FE-SCREEN-008, [§3-7](#3-7-게스트-데이터-모델)·[§3-8](#3-8-api-연동에러-처리-계약)). 일반 제작·스토리 수정(FE-SCREEN-009, [§3-9](#3-9-입력폼-검증-규칙)). AI 응답 재생성·채팅 이미지·엔딩 도달 표시([§3-6](#3-6-채팅-플레이와-sse-스트리밍))·목록 카드 썸네일(FE-SCREEN-001·004)·본 엔딩 표시(FE-SCREEN-003). Safari 렌더링·애니메이션·입력 UX 개선([§3-10](#3-10-반응형접근성브라우저-지원)). 초대 방식 개편 — 코드 공유·입력·신규 가입 온보딩(FE-SCREEN-008, KNK-567)과 인앱 브라우저 감지·탈출([§3-10](#3-10-반응형접근성브라우저-지원)) |
...
경로는 MVP 6개 화면 라우트와 1개 프록시 라우트에 `/login`·`/my`·`/terms`·`/privacy`(구현 완료)와 `Phase 1 · 계획` 2개(`/stories/new/general`, `/stories/[id]/edit`)를 더해 구성됩니다. 회원가입 별도 경로·대시보드·설정·관리자 경로는 없습니다. 동적 세그먼트 `[id]`는 문자열입니다.
...
오버레이 화면(별도 라우트 없음): 온보딩 다이얼로그(FE-SCREEN-007, `(main)`에 마운트), 옵션 메뉴+삭제 확인 다이얼로그(스토리 상세), 썸네일 이미지 뷰어(FE-SCREEN-003), 채팅 설정 드로어+채팅 삭제 확인 다이얼로그(FE-SCREEN-005, [§3-6](#3-6-채팅-플레이와-sse-스트리밍)), 키워드 추가 다이얼로그, 선택 키워드 드로어, 생성 이탈 확인 다이얼로그.

### 라우팅 규칙

- **MVP는 접근 제어가 없습니다.** 모든 경로는 인증 없이 접근 가능하고 가드·미들웨어·리다이렉트가 없습니다. `/login`·`/my`는 게스트·회원 모두 접근 가능하며(회원이 `/login`에 진입해도 차단하지 않음, `/my`는 회원이면 프로필·로그아웃을, 게스트면 로그인 버튼을 표시), 경로 가드는 두지 않습니다. 로그인 성공 후 홈으로 이동하는 것은 화면 내 이동이지 라우트 가드가 아닙니다.
- **라우트 그룹은 권한이 아니라 chrome을 가릅니다.** `(main)`은 상단 헤더와 하단 탭을 두르는 탭 화면, `(story)`·`(chat)`·`(auth)`·`(my)`·`(legal)`은 헤더·하단탭이 없는(또는 뒤로가기 헤더만 있는) 전체 화면입니다.
...
- **상단 헤더** — `(main)` 화면에서만 로고와 현재 섹션 라벨을 표시합니다. 스크롤 여부와 무관하게 보더 없이 렌더합니다. 로그인 진입점은 헤더에 두지 않고 마이 페이지 탭 내부 상태로 구성합니다: 게스트는 로그인 버튼(→ `/login`), 회원은 프로필·로그아웃을 표시합니다. 게스트/회원 판정 소스는 [§3-8](#3-8-api-연동에러-처리-계약)입니다.
- **하단 네비게이션** — `(main)` 그룹에서만 노출하는 고정 3탭입니다(filled 스타일 아이콘 + 라벨). 피드백은 하단 탭에서 빠지고 마이 페이지 내 항목(`/my/feedback`)으로 이동했습니다.
...
- 하단 탭으로 스토리·채팅·마이를 오갈 수 있어야 합니다. (US-8-1, e2e `smoke/navigation`)
...
| 액션 | 트리거 | 결과 |
| --- | --- | --- |
| 채팅 시작 | 하단 CTA "채팅 시작하기" | `POST /chats {storyId, startSettingId}` → 채팅 ID 저장 → 상세 prefetch → `/chats/[id]`로 `replace`. 진행 중 버튼 비활성화·"채팅을 시작하는 중...". `startSettingId`는 Select에서 선택한 시작 설정의 ID이며, ID가 없는 설정이 선택됐거나 생략되면 서버가 첫 번째 시작 설정을 사용합니다([`4-backend.md §4-3-3`](./4-backend.md)) |
...
**화면 구성 (채팅 카드)** — 좌측 3:4 썸네일 자리(placeholder 아이콘 — `Phase 1 · 계획`: 배치 응답에 썸네일 필드 추가 대기, 스토리 카드와 동일 크롭), 스토리 제목, 마지막 장면 미리보기, 턴 수, 마지막 활동 시각(상대 시간). 카드 전체가 채팅 화면 링크이며 옵션 메뉴는 없습니다 — 삭제 진입점은 채팅 화면의 설정 드로어입니다([§3-6](#3-6-채팅-플레이와-sse-스트리밍)). `lastStoryPreview`가 빈 문자열이면 "채팅을 시작하고 이야기를 이어가 보세요" fallback을 표시합니다. `lastStoryPreview`가 `null`인 채팅은 목록 변환 단계에서 제외되어 카드로 나타나지 않습니다.
...
- 목록 카드에는 삭제 메뉴가 없어야 하고, 채팅 화면의 설정 드로어에서 삭제하면 완료 안내 후 이 목록으로 돌아가야 합니다. (US-5-3, e2e `chats/chat-room`)
...
| 항목 | 값 |
| --- | --- |
| 라우트 | `/login`(`(auth)`, 뒤로가기 헤더) · `/my`(마이 페이지, `(main)`, 헤더 + 하단탭) |
| 관련 US | US-9-1 ~ 9-5, US-10-1 ~ 10-5 |

**목적** — 게스트를 회원으로 전환하고, 기기에 쌓은 서재를 계정으로 이관합니다. 마이 페이지는 로그인 진입점과 회원 상태(프로필·크레딧·로그아웃)를 모아두는 하단 탭 화면입니다.

**진입점** — 하단 탭 "마이 페이지"(`/my`). 게스트는 로그인 버튼(→ `/login`)을, 회원은 프로필·로그아웃을 표시합니다. 게스트/회원 판정은 [§3-8](#3-8-api-연동에러-처리-계약) 토큰 세션의 모드 판정 소스를 따릅니다.

**마이 페이지 회원 상태 구성** — 게스트는 로그인 버튼만 표시합니다(`Phase 1 · 구현`). 회원은 닉네임·프로필 이미지와 로그아웃 버튼을 표시합니다(`Phase 1 · 구현`). `Phase 1 · 계획` — 회원 상태에 크레딧 잔액(`GET /users/me/credits`), 출석체크 버튼(`POST /users/me/credits/attendance` — 성공 시 "크레딧 250개를 받았어요" 토스트, 이미 받았으면 버튼 비활성), 친구 초대 메뉴를 추가로 구성합니다. `Phase 1 · 구현` — 친구 초대 메뉴는 전용 페이지 `/my/invite`로 이동합니다. 페이지는 내 초대 코드(`GET /users/me/invite` — 진입 시 조회, 최초 조회 시 서버가 지연 발급)와 카카오톡 공유 버튼(Kakao JS SDK, `NEXT_PUBLIC_KAKAO_JS_KEY` 미설정 시 비활성)을 표시하며, 게스트 진입 시 `/login`으로 보냅니다. `Phase 1 · 계획`(KNK-567 개편 — [`4-backend.md §4-3-7`](./4-backend.md) 결정 기록) — 링크 공유를 코드 공유·입력으로 바꿉니다: ① **코드 복사 버튼** — 기존 링크 복사 버튼을 대체해 초대 코드 문자열을 복사합니다(복사 완료 토스트 — 진입 시 프리페치한 캐시값을 동기 복사해 iOS 사파리 클립보드 제약을 피함). ② **카카오톡 공유 내용** — 메시지 본문 텍스트에 초대 코드를 싣고, 버튼은 서비스 홈으로 이동합니다. 카카오 메시지 템플릿의 버튼은 URL 열기만 가능하고 클립보드 복사는 불가하므로 수신자는 본문에서 코드를 복사합니다. ③ **초대 코드 입력란** — 같은 페이지에 상시 배치합니다. 입력값은 trim·자동 대문자 변환 후 `POST /users/me/invite/redeem`으로 제출하고, 성공 시 "크레딧 500개를 받았어요" 토스트, 실패는 사유별로 안내합니다(404 없는 코드 → "코드를 다시 확인해 주세요", 409 `INVITE_SELF_CODE` → "내 코드는 입력할 수 없어요", 409 `INVITE_ALREADY_REDEEMED` → "이미 초대 코드를 입력했어요"). ④ **이용 안내 문구** — "친구가 내 초대 코드를 입력하면 나와 친구 모두 500 크레딧"으로 바꿉니다. 회원가입 보상 500 크레딧은 가입 성공 후 서버가 자동 적립하므로 별도 버튼을 두지 않습니다. `Phase 1 · 계획`(B12) — 보상 크레딧은 적립 후 30일 유효하며, 잔액은 만료분을 제외해 서버가 계산한 값을 그대로 표시합니다([`4-backend.md §4-3-7`](./4-backend.md)).

**소모량 사전 고지** `Phase 1 · 계획` (US-10-3) — 회원에게는 스토리라인 생성 무료, 스토리 생성 20 크레딧, 채팅 1턴 10 크레딧을 표시합니다. 게스트에게는 같은 위치에 남은 체험 횟수를 표시합니다: 스토리라인 생성·재생성 5회, 스토리 생성 1회, 모든 채팅방 합산 채팅 5회. 스토리 제작 완료 CTA와 채팅 전송 버튼 근처에는 해당 작업의 값만 표시하고, 스토리라인 단계의 "다시 만들기"에는 남은 스토리라인 생성 횟수를 표시합니다. `Phase 1 · 계획`(B13) — 디바이스의 체험 횟수가 남은 회원은 크레딧 대신 체험 횟수를 먼저 차감하므로([`4-backend.md §4-3-7`](./4-backend.md)), 소모량 고지는 이 2단 판정(체험 잔여 → 크레딧)과 어긋나지 않아야 합니다.
...
**화면 구성·동작** (`Phase 1 · 구현`) — `/login`은 Google 로그인 버튼을 단일 CTA로 두고, 버튼 주변에 두 가지 고지를 함께 표시합니다:

- **이관 1회 안내** — "로그인하면 이 기기에서 만든 스토리·채팅이 계정으로 자동 이관됩니다. 이관은 계정당 한 번만 가능하며, 다음 로그인부터는 이 기기의 스토리·채팅이 이관되지 않습니다." 게스트에게만 실질적 의미가 있으나 `/login`은 로그인 전 단일 CTA이므로 상시 노출합니다. 근거는 이관 1회 잠금 계약([`4-backend.md §4-3-5`](./4-backend.md) B15 — 최초 성공 이관 시 `users.migrated_at`을 기록해 이후 호출은 `migrationClosed: true`로 닫힘)입니다.
- **약관 동의 고지** — Google 로그인 버튼 하단에 "로그인 시 <u>서비스이용약관</u> 및 <u>개인정보처리방침</u>에 동의하는 것으로 간주합니다."를 표시하고, 밑줄 링크로 각각 `/terms`·`/privacy`(FE-SCREEN-010)로 이동합니다. 별도 동의 체크박스는 없으며 로그인 버튼 클릭을 동의로 간주합니다(묵시적 동의).

로그인 성공 시 순서대로:

1. **토큰 보관** — access·refresh 토큰을 BFF가 httpOnly 쿠키로 보관하고 백엔드 요청에 주입합니다([§3-8](#3-8-api-연동에러-처리-계약) 토큰 세션). 브라우저 JS에 토큰을 노출하지 않습니다. `Phase 1 · 계획`(KNK-567) — 초대 URL 라우트(`/invite/[code]`)·초대 코드 httpOnly 쿠키(24시간 어트리뷰션 윈도우)·로그인 요청의 `inviteCode` 전송은 초대 방식 개편으로 폐기합니다. 라우트는 리다이렉트 없이 완전 삭제합니다(링크 방식 실사용자가 없어 유포된 URL이 없음). 초대 보상 적립은 `/my/invite`의 코드 입력으로 이동합니다([`4-backend.md §4-3-7`](./4-backend.md) 결정 기록).
...
`Phase 1 · 계획`(KNK-567) — **신규 가입 온보딩(초대 코드 입력)**: 신규 가입의 첫 로그인 완료 직후 "초대 코드가 있나요?" 스텝을 표시합니다. 코드 입력란과 건너뛰기 버튼으로 구성하고, 제출·피드백 규칙은 `/my/invite` 입력란과 동일합니다(같은 `POST /users/me/invite/redeem`). 건너뛰어도 자격(계정당 평생 1회)은 유지되어 이후 `/my/invite`에서 입력할 수 있습니다. 기존 회원에게는 별도 고지 없이 `/my/invite` 입력란만 제공합니다. 신규 가입 판정 신호(로그인 응답의 신규 가입 여부 등)는 서버 정렬([`4-backend.md §4-8`](./4-backend.md) B22)과 함께 확정합니다.
...
**회원 모드 데이터 소스** `Phase 1 · 계획` — 스토리·채팅 목록을 `GET /users/me/stories` · `GET /users/me/chats`로 전환합니다(현재는 로그인 후에도 게스트 배치 조회를 그대로 사용). 전환 후 목록의 낙관적 삭제는 게스트와 동일한 규칙이되 롤백 대상이 로컬 배열이 아니라 서버 목록 캐시입니다(재조회로 복원). 게스트 배치 조회([§3-7](#3-7-게스트-데이터-모델))는 게스트 전용으로 유지합니다.

**로그아웃** (`Phase 1 · 구현`) — 마이 페이지의 로그아웃은 `POST /auth/logout` 후 세션 쿠키를 폐기하고 게스트 모드로 복귀합니다. 로컬 배열은 로그인 시 비워졌으므로 빈 서재에서 새로 시작합니다. 분석 식별자는 [`6-analytics.md §6-2`](./6-analytics.md)에 따라 재설정합니다(공용 기기 보호).
...
- `Phase 1 · 계획` 회원 상태에서 만든 스토리·채팅은 로컬 목록이 아니라 서버 목록에 나타나야 합니다(데이터 소스 전환 후).
- `Phase 1 · 계획` 회원 서재 조회가 401이면 재발급 후 성공하거나, 재발급 실패 시 재로그인을 안내해야 합니다.
- `Phase 1 · 계획` 잔액 부족 회원의 소모 요청은 크레딧 부족 다이얼로그를, 한도 소진 게스트는 로그인 다이얼로그를 봐야 하며 입력값이 유실되지 않아야 합니다. (US-10-4, US-10-5)
...
**진입점** — `/login` Google 로그인 버튼 하단 동의 고지의 밑줄 링크(서비스이용약관 → `/terms`, 개인정보처리방침 → `/privacy` — [FE-SCREEN-008](#3-4-화면별-스펙)). 로그인 전 게스트도 접근할 수 있으며 라우트 가드는 없습니다([§3-3](#3-3-라우팅레이아웃공통-셸)). 뒤로가기 헤더로 진입 이전 화면(`/login`)으로 복귀합니다.

**화면 구성** — 뒤로가기 헤더 + 문서 제목 + 정적 본문(길어서 세로 스크롤). 본문 최상단에 **시행일**과 **버전**을 표기해 개정 이력을 드러냅니다. 별도 API·상태 없이 클라이언트 정적 렌더입니다.

**콘텐츠 소스** — 표준 초안을 웹 레포 `docs/legal/terms.md`·`docs/legal/privacy.md`에 두고 페이지가 이를 렌더합니다. 초안은 일반적인 모바일 서비스 표준 항목으로 구성합니다 — 서비스이용약관: 목적·정의·약관 게시와 개정·이용계약·서비스 제공과 변경·이용자 의무·게시물의 관리·서비스 이용 제한·면책·분쟁 해결 등. 개인정보처리방침: 수집 항목·수집 방법·이용 목적·보유 및 이용 기간·제3자 제공·처리 위탁·이용자와 법정대리인의 권리·파기 절차·안전성 확보 조치·개인정보 보호책임자와 문의처 등. **사업자 정보·문의처·수집 항목 확정값 등은 placeholder이며 법무 검토가 필요합니다.**
...
- **생성** — `POST /stories/simple/storylines {selectedTagIds, customTags}`. 요청과 동시에 스토리라인 선택 단계로 전환해 로딩을 그 자리에 표시합니다. 응답은 스토리라인 3개와 생성 ID(`simpleCreationId`)를 포함합니다. 로딩 중에는 타자기형 문구와 15·30초 시점 힌트를 표시합니다. `Phase 1 · 계획` — 회원은 무료이고, 게스트는 디바이스 ID별 스토리라인 생성·재생성 합산 5회 한도를 사용합니다.
...
- **영향.** 이벤트 파싱과 스트림 종료 감지를 직접 구현합니다. `completed`·`error` 없이 EOF로 끝나는 종료의 처리도 클라이언트 책임이 되며, 현재 미처리라 스트리밍 상태가 멈춥니다([§3-13](#3-13-mvp-제외-범위와-스펙-구현-간극) G5).
...
- **하단 이동 버튼** — 스크롤이 하단에 있지 않으면 "맨 아래로 이동" 버튼을 표시하고, 스트리밍 중에는 진행 스피너(비활성)로 대체합니다.
...
헤더 우측의 설정 버튼이 우측 드로어를 엽니다. 드로어는 입력 모드 선택과 최하단의 "채팅 삭제하기" 버튼으로 구성합니다.

- **삭제 흐름** — "채팅 삭제하기" → 드로어가 닫히고 확인 다이얼로그("채팅을 삭제할까요?") 표시 → 확정 시 낙관적 삭제([§3-7](#3-7-게스트-데이터-모델)과 동일 규칙: 204 완료 토스트, 404 무음 성공) 후 `/chats`로 `replace`합니다.
- 채팅 목록(FE-SCREEN-004)에는 삭제 진입점이 없으며 이 드로어가 유일한 삭제 경로입니다.
...
- 설정 드로어 최하단에서 채팅을 삭제하면 완료 안내 후 채팅 목록으로 돌아가야 합니다. (US-5-3, e2e `chats/chat-room`)
...
- 브라우저 데이터 요청과 BFF→백엔드 인증 요청의 타임아웃은 요청마다 180초입니다. 단, 채팅 스트리밍은 이 경로를 우회하므로 클라이언트 타임아웃이 없습니다([§3-6](#3-6-채팅-플레이와-sse-스트리밍)).
...
| 삭제(공통) | `DELETE /stories/{id}` · `DELETE /chats/{id}` | 스토리=상세 옵션 메뉴, 채팅=설정 드로어([§3-6](#3-6-채팅-플레이와-sse-스트리밍)) 확인 | 204 완료 토스트 후 목록으로 `replace` | 404=무음 성공(로컬 제거 유지), 그 외 롤백 + 토스트 |
| 로그인 `Phase 1 · 구현` | `POST /auth/login/google` → `POST /auth/migrate` | Google 로그인 성공 직후 | 이관 결과 반영 후 홈 이동 + 토스트 | 로그인 실패 토스트, 이관 실패는 유효 ID 1회 재제출 |
...
| 크레딧 `Phase 1 · 계획` | `GET /users/me/credits` · `POST /users/me/credits/attendance` · `GET /users/me/invite` · `POST /users/me/invite/redeem`(KNK-567) | 마이 페이지 진입·버튼 클릭·초대 코드 제출 | 잔액·적립 반영 | 실패 토스트(초대 코드 입력은 사유별 안내 — FE-SCREEN-008) |
...
- 서버 원본 메시지·스택은 사용자에게 그대로 노출하지 않습니다. MVP는 인증 만료 개념이 없어 401 전용 리다이렉트를 두지 않습니다. `Phase 1 · 구현` — 회원 세션의 401은 아래 토큰 세션 규칙(BFF 선제 재발급)이 처리하므로 화면 레벨 리다이렉트는 여전히 두지 않습니다. 재발급까지 실패하면 게스트 모드로 되돌립니다. `Phase 1 · 계획` — 회원 전용 목록(서버 정본 전환 후)에서 재발급까지 실패하면 재로그인을 안내합니다.

### 토큰 세션 (BFF) — `Phase 1 · 구현`

- 웹 구현은 NextAuth(Google OAuth 리다이렉트 플로우) + BFF 프록시 자체 httpOnly 쿠키를 함께 쓰는 하이브리드입니다. 선제 재발급은 BFF 프록시가 수행합니다.
...
- **회원 요청의 익명 전달 차단** — 회원이던 세션이 인증을 잃은 요청은 백엔드로 전달하지 않고 프록시가 즉시 응답합니다. 익명으로 전달하면 회원의 요청(특히 쓰기)이 게스트 콘텐츠로 잘못 귀속되기 때문입니다.
  ...
  - **일시 강등**(네트워크·5xx로 이번 요청의 토큰 미확보, 기존 access 토큰도 없음) — 세션을 보존한 채 `503`으로 응답해 재시도에 맡깁니다.
...
이 앱의 입력은 피드백·키워드·추가 정보·채팅 입력입니다. 로그인은 Google OAuth 버튼 하나여서 검증 대상 입력 폼이 없습니다(`Phase 1 · 구현`, FE-SCREEN-008).
...
### 인앱 브라우저 감지·탈출 — `Phase 1 · 계획`(KNK-567)

카카오톡 인앱 브라우저는 Google OAuth를 차단해 로그인·가입이 불가능하고, 인앱 브라우저의 저장소는 실제 브라우저와 격리되어 거기서 쌓은 게스트 데이터가 고립됩니다([`4-backend.md §4-3-7`](./4-backend.md) 결정 기록). 루트 레이아웃의 클라이언트 컴포넌트가 `navigator.userAgent`로 인앱 브라우저를 감지해 대응합니다(SSR 가드 필수 — `typeof window` 확인 후 실행).

| 인앱 | UA 패턴 | 대응 |
| --- | --- | --- |
| 카카오톡 | `KAKAOTALK` | 자동 탈출 — 현재 URL(쿼리 포함)을 인코딩해 `kakaotalk://web/openExternal?url=...`을 호출해 기본 브라우저에서 열고, 이어서 `kakaotalk://inappbrowser/close`로 인앱 창을 닫습니다. 호출 후 1~2초 뒤에도 페이지가 살아 있으면 실패로 간접 판정하고 안내 배너를 표시합니다 |
| 인스타그램 | `Instagram` | 안내 배너만(탈출 스킴 없음) |
| 쓰레드 | `Barcelona`(앱 코드네임 — 실기기 검증 필요) | 안내 배너만(탈출 스킴 없음) |

- **전역 적용** — 감지 시 모든 페이지에서 즉시 실행합니다. 게스트 데이터가 격리 저장소에 쌓이기 전에 실제 브라우저로 옮기는 것이 목적이므로 진입 페이지를 가리지 않습니다.
- **안내 배너** — 세 인앱이 공유하는 단일 컴포넌트로 두고, 문구만 앱별 메뉴 위치에 맞춰 분기합니다(예: "우측 하단 ⋯ 메뉴 → 다른 브라우저로 열기").
- **비공식 스킴 리스크** — 카카오톡 탈출 스킴은 카카오가 문서로 보장하지 않는 진입점이라 앱 업데이트로 깨질 수 있습니다. 실패 배너가 항상 받쳐야 하며, 아래 검수 기준의 실기기 확인을 매 릴리스 수행합니다.
- **루프 없음** — 외부 브라우저에는 인앱 UA가 없으므로 탈출 후 재감지가 발생하지 않습니다.

**검수 기준** — 카카오톡 실기기에서 공유 메시지의 링크를 열면 기본 브라우저로 전환돼야 합니다. 스킴이 동작하지 않는 환경에서는 안내 배너가 표시돼야 합니다. 인스타그램·쓰레드 인앱에서는 자동 전환 없이 안내 배너가 표시돼야 합니다. 일반 브라우저에서는 감지 로직이 어떤 UI도 표시하지 않아야 합니다.
...
| `stories/story-detail` | 상세 표시(썸네일·턴 수·생성일)·썸네일 뷰어·시작 상황 선택·채팅 시작·삭제·재시도 | US-4-1·2·3·4 |
...
| `chats/chat-room` | 프롤로그·스트리밍·선택지·실패·설정 드로어 삭제 | US-5-3, US-6-1·2·3·4·8 |
...
| # | 간극 | 현황 | 방향 |
| --- | --- | --- | --- |
| G1 | 인증 | Google 로그인·`/me`·refresh·logout·자동 마이그레이션은 `Phase 1 · 구현`으로 도입 완료(FE-SCREEN-008). 크레딧 UI·회원 서재 데이터 소스 전환·초대 방식 개편(코드 입력, KNK-567)은 아직 `Phase 1 · 계획` | 잔여 항목은 §3-4·§3-8 로드맵대로 진행. 초대 개편은 서버 정렬([`4-backend.md §4-8`](./4-backend.md) B22)과 동시 배포 |
...
- **회원 서재 데이터 소스 전환** — 로그인·자동 마이그레이션은 도입 완료(FE-SCREEN-008). 남은 과제는 회원 모드 목록을 `GET /users/me/stories`·`/chats`로 전환하는 것과 크레딧 UI·초대 방식 개편(코드 입력, KNK-567)입니다.
