...
- **문서 제목(브라우저 탭)은 `제목 • 마냑`으로 통일합니다.** 루트 레이아웃이 제목 템플릿을 선언하므로 각 화면은 자기 제목만 선언하면 되고, 제목을 선언하지 않은 화면은 서비스명만 표시합니다. 채팅방과 공유 열람은 스토리 제목을 씁니다. 다만 채팅방은 로그인 세션·게스트 식별자가 있어야 상세를 읽을 수 있어 서버 메타데이터로 제목을 만들 수 없으므로, 클라이언트에서 데이터를 받은 뒤 제목을 덮어씁니다 — 제목이 비어 있는 동안에는 손대지 않아 서버가 렌더한 기본 제목이 남습니다. 공유 열람은 서버가 공유본을 직접 읽을 수 있어 메타데이터 단계에서 제목이 확정됩니다.
...
**링크 미리보기** — 서버가 공유본을 읽어 제목에 스토리 제목, 설명에 프롤로그 발췌를 담은 오픈그래프 메타데이터를 만듭니다. 조회에 실패하면 제목·설명 없이 색인 차단만 담은 기본값을 돌려주고 페이지는 그대로 띄웁니다 — 미리보기가 비는 것이 페이지가 뜨지 않는 것보다 낫기 때문입니다.
