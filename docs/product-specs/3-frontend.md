...
- **연동 시작 확인 다이얼로그** — 연동 버튼을 누르면 곧바로 OAuth로 보내지 않고 확인 다이얼로그를 먼저 띄웁니다. 재인증이 선행되므로 **대상 provider가 아니라 현재 로그인한 provider의 인증 창이 먼저** 뜨는데, 예고가 없으면 사용자가 흐름을 오해합니다. 또 연동은 해제할 수 없어 비가역 확인이 필요합니다. 다이얼로그는 혜택(어느 계정으로 로그인해도 같은 스토리·채팅·크레딧)·재인증 순서 예고·해제 불가 고지를 각각 한 줄로 나눠 담고, "나중에 하기"/"연동하기" 2버튼(신규 가입 온보딩과 같은 배치·라벨)을 둡니다. 명시적 이탈 버튼이 있으므로 닫기(X) 버튼은 두지 않습니다(초대 코드 다이얼로그와 같은 규칙, KNK-715).
