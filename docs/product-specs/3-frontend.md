...
| `Phase 1 · 구현` | Phase 1 범위. 코드 반영 완료 | Google 로그인·로그아웃·BFF 토큰 세션·자동 마이그레이션·마이 페이지 뼈대·이관 1회 안내·약관 동의 고지(FE-SCREEN-008, [§3-8](#3-8-api-연동에러-처리-계약))·서비스이용약관·개인정보처리방침 페이지(FE-SCREEN-010) |
| `Phase 1 · 계획` | Phase 1 범위. 미구현, 방향만 합의됨 | 크레딧 UI(잔액·출석체크·초대, FE-SCREEN-008, [§3-7](#3-7-게스트-데이터-모델)·[§3-8](#3-8-api-연동에러-처리-계약)). 일반 제작·스토리 수정(FE-SCREEN-009, [§3-9](#3-9-입력폼-검증-규칙)). AI 응답 재생성·채팅 이미지·엔딩 도달 표시([§3-6](#3-6-채팅-플레이와-sse-스트리밍))·썸네일·본 엔딩 표시(FE-SCREEN-001·003). Safari 렌더링·애니메이션·입력 UX 개선([§3-10](#3-10-반응형접근성브라우저-지원)) |
...
- **상단 헤더** — `(main)` 화면에서만 로고와 현재 섹션 라벨을 표시합니다. 스크롤 후에만 하단 보더를 표시합니다. 로그인 진입점은 헤더에 두지 않고 마이 페이지 탭 내부 상태로 구성합니다: 게스트는 로그인 버튼(→ `/login`), 회원은 프로필·로그아웃을 표시합니다. 게스트/회원 판정 소스는 [§3-8](#3-8-api-연동에러-처리-계약)입니다.
- **하단 네비게이션** — `(main)` 그룹에서만 노출하는 고정 3탭입니다. 피드백은 하단 탭에서 빠지고 마이 페이지 내 항목(`/my/feedback`)으로 이동했습니다.
...
- 하단 탭으로 내 이야기·채팅·마이 페이지를 오갈 수 있어야 합니다. (US-8-1, e2e `smoke/navigation`)
...
| --- | --- | --- |
| 제목 | `title` | 항상 |
| 썸네일 `Phase 1 · 계획` | `thumbnailUrl`(목록·상세 통일 — 현행 와이어 필드는 `coverImageUrl`, [`4-backend.md §4-8`](./4-backend.md) B14) | 값이 있을 때. 로드 실패 시 이미지 영역 제거 |
| 장르 배지 | 장르 태그 | 항상 |
...
| 액션 | 트리거 | 결과 |
| --- | --- | --- |
| 채팅 시작 | 하단 CTA "채팅 시작하기" | `POST /chats {storyId}` → 채팅 ID 저장 → 상세 prefetch → `/chats/[id]`로 `replace`. 진행 중 버튼 비활성화·"채팅을 시작하는 중...". `Phase 1 · 계획`(B14) — 시작 설정이 여러 개인 스토리는 시작 설정을 선택해 `startSettingId`로 전달합니다(미지정 시 첫 번째 — [`4-backend.md §4-3-3`](./4-backend.md)) |
| 뒤로 | 헤더 back | 히스토리 뒤로 |
...
**마이 페이지 회원 상태 구성** — 게스트는 로그인 버튼만 표시합니다(`Phase 1 · 구현`). 회원은 닉네임·프로필 이미지와 로그아웃 버튼을 표시합니다(`Phase 1 · 구현`). `Phase 1 · 계획` — 회원 상태에 크레딧 잔액(`GET /users/me/credits`), 출석체크 버튼(`POST /users/me/credits/attendance` — 성공 시 "크레딧 250개를 받았어요" 토스트, 이미 받았으면 버튼 비활성), 친구 초대 메뉴를 추가로 구성합니다. `Phase 1 · 구현` — 친구 초대 메뉴는 전용 페이지 `/my/invite`로 이동합니다. 페이지는 내 초대 코드(`GET /users/me/invite` — 진입 시 조회, 최초 조회 시 서버가 지연 발급), 카카오톡 공유 버튼(Kakao JS SDK, `NEXT_PUBLIC_KAKAO_JS_KEY` 미설정 시 비활성), 링크 복사 버튼(복사 완료 토스트 — 진입 시 프리페치한 캐시값을 동기 복사해 iOS 사파리 클립보드 제약을 피함), "친구가 내 초대 링크로 가입하면 나와 친구 모두 500 크레딧" 등 이용 안내로 구성하며, 게스트 진입 시 `/login`으로 보냅니다. 회원가입 보상 500 크레딧은 가입 성공 후 서버가 자동 적립하므로 별도 버튼을 두지 않습니다. `Phase 1 · 계획`(B12) — 보상 크레딧은 적립 후 30일 유효하며, 잔액은 만료분을 제외해 서버가 계산한 값을 그대로 표시합니다([`4-backend.md §4-3-7`](./4-backend.md)).
...
1. **토큰 보관** — access·refresh 토큰을 BFF가 httpOnly 쿠키로 보관하고 백엔드 요청에 주입합니다([§3-8](#3-8-api-연동에러-처리-계약) 토큰 세션). 브라우저 JS에 토큰을 노출하지 않습니다. `Phase 1 · 구현` — 초대 URL(`/invite/[code]`)로 진입하면 라우트 핸들러가 초대 코드를 httpOnly 쿠키(수명 24시간 = 초대 어트리뷰션 윈도우, [`4-backend.md §4-3-7`](./4-backend.md))에 저장한 뒤 `/login`으로 보내고(이미 로그인된 사용자는 쿠키 없이 홈으로), 로그인 처리 시 쿠키의 코드를 `POST /auth/login/google`의 `inviteCode`로 함께 전송합니다. 무효 코드 판정은 서버 몫이므로 프론트는 형식 검증만 하고 항상 전달하며, 쿠키는 세션 수립이 끝까지 성공했을 때만 삭제합니다(실패 시 다음 시도에서 재전달).
...
| 크레딧 `Phase 1 · 계획` | `GET /users/me/credits` · `POST /users/me/credits/attendance` · `GET /users/me/invite` | 마이 페이지 진입·버튼 클릭 | 잔액·적립 반영 | 실패 토스트 |
...
| # | 간극 | 현황 | 방향 |
| --- | --- | --- | --- |
| G1 | 인증 | Google 로그인·`/me`·refresh·logout·자동 마이그레이션은 `Phase 1 · 구현`으로 도입 완료(FE-SCREEN-008). 크레딧 UI·회원 서재 데이터 소스 전환·초대 코드 전달은 아직 `Phase 1 · 계획` | 잔여 항목은 §3-4·§3-8 로드맵대로 진행 |
...
- **회원 서재 데이터 소스 전환** — 로그인·자동 마이그레이션은 도입 완료(FE-SCREEN-008). 남은 과제는 회원 모드 목록을 `GET /users/me/stories`·`/chats`로 전환하는 것과 크레딧 UI·초대 코드 전달입니다.
