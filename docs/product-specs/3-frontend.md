...
MVP 본문은 로그인 없는 전원 게스트 기준입니다. Phase 1 로그인·더보기 페이지·BFF 토큰 세션은 `Phase 1 · 구현`으로 구현이 완료되었고, 크레딧·일반 제작·스토리 수정 흐름은 `Phase 1 · 계획` 라벨로 본문에 포함됩니다(FE-SCREEN-008·009, 토큰 세션은 [§3-8](#3-8-api-연동에러-처리-계약)).
...
| `Phase 1 · 구현` | Phase 1 범위. 코드 반영 완료 | Google 로그인·로그아웃·BFF 토큰 세션·자동 마이그레이션·더보기 페이지·이관 1회 안내·약관 동의 고지·초대 코드 공유·입력·신규 가입 온보딩(FE-SCREEN-008, [§3-8](#3-8-api-연동에러-처리-계약))·서비스이용약관·개인정보처리방침 페이지(FE-SCREEN-010)·상세 썸네일 히어로·이미지 뷰어·시작 설정 선택(FE-SCREEN-003) |
| `Phase 1 · 계획` | Phase 1 범위. 미구현, 방향만 합의됨 | 크레딧 정책 고도화(FE-SCREEN-008, [§3-7](#3-7-게스트-데이터-모델)·[§3-8](#3-8-api-연동에러-처리-계약)). 일반 제작·스토리 수정(FE-SCREEN-009, [§3-9](#3-9-입력폼-검증-규칙)). AI 응답 재생성·채팅 이미지·엔딩 도달 표시([§3-6](#3-6-채팅-플레이와-sse-스트리밍))·목록 카드 썸네일(FE-SCREEN-001·004)·본 엔딩 표시(FE-SCREEN-003). Safari 렌더링·애니메이션·입력 UX 개선([§3-10](#3-10-반응형접근성브라우저-지원))과 인앱 브라우저 감지·탈출([§3-10](#3-10-반응형접근성브라우저-지원)) |
...
경로는 MVP 6개 화면 라우트와 1개 프록시 라우트에 `/login`·`/more`·`/more/invite`·`/terms`·`/privacy`(구현 완료)와 `Phase 1 · 계획` 2개(`/stories/new/general`, `/stories/[id]/edit`)를 더해 구성됩니다. 회원가입 별도 경로·대시보드·설정·관리자 경로는 없습니다. 동적 세그먼트 `[id]`는 문자열입니다.
...
| `/more/feedback` | FE-SCREEN-006 | `(more)` | 피드백 | 뒤로가기 헤더(탭 없음) |
| `*` | FE-SCREEN-999 | — | Not Found | 최소 셸 |
| `/api/[...path]` | — | — | BFF 프록시(라우트 핸들러) | — |
| `/login` | FE-SCREEN-008 | `(auth)` | 로그인 | 뒤로가기 헤더(탭 없음) |
| `/more` | FE-SCREEN-008 | `(main)` | 더보기 페이지 | 헤더 + 하단탭 |
| `/more/invite` | FE-SCREEN-008 | `(more)` | 친구 초대 | 뒤로가기 헤더(탭 없음) |
...
- **기본 화면은 접근 제어가 없습니다.** `/login`·`/more`는 게스트·회원 모두 접근 가능하며(회원이 `/login`에 진입해도 차단하지 않음, `/more`는 회원이면 프로필·로그아웃을, 게스트면 로그인 버튼을 표시), 경로 가드는 두지 않습니다. `/more/invite`는 회원 전용 화면이라 게스트를 `/login`으로 보냅니다. 로그인 성공 후 홈으로 이동하는 것은 화면 내 이동이지 공통 라우트 가드가 아닙니다.
- **레거시 더보기 경로** — `/my`, `/my/invite`, `/my/feedback`은 각각 `/more`, `/more/invite`, `/more/feedback`으로 리다이렉트합니다.
- **라우트 그룹은 권한이 아니라 chrome을 가릅니다.** `(main)`은 상단 헤더와 하단 탭을 두르는 탭 화면, `(story)`·`(chat)`·`(auth)`·`(more)`·`(legal)`은 헤더·하단탭이 없는(또는 뒤로가기 헤더만 있는) 전체 화면입니다. `(my)`는 레거시 `/my*` 리다이렉트만 소유합니다.
...
- **상단 헤더** — `(main)` 화면에서만 로고와 현재 섹션 라벨(`홈`·`채팅`·`더보기`)을 표시합니다. 스크롤 여부와 무관하게 보더 없이 렌더합니다. 로그인 진입점은 헤더에 두지 않고 더보기 탭 내부 상태로 구성합니다: 게스트는 로그인 버튼(→ `/login`), 회원은 프로필·로그아웃을 표시합니다. 게스트/회원 판정 소스는 [§3-8](#3-8-api-연동에러-처리-계약)입니다.
- **하단 네비게이션** — `(main)` 그룹에서만 노출하는 고정 3탭입니다. 화면에는 24px filled 아이콘만 표시합니다(홈=집, 채팅=말풍선, 더보기=가로 점 3개). 각 링크의 접근 가능한 이름은 `홈`·`채팅`·`더보기`로 제공하며, 링크 상하 패딩은 각각 16px이고 하단 안전 영역을 별도로 더합니다. 피드백은 하단 탭에서 빠지고 더보기 페이지 내 항목(`/more/feedback`)으로 이동했습니다.
...
- 하단 탭의 아이콘과 접근 가능한 이름으로 홈·채팅·더보기를 오갈 수 있어야 합니다. (US-8-1, e2e `smoke/navigation`)
- `(story)`·`(chat)`·`(auth)`·`(more)` 화면에는 하단 탭과 상단 헤더가 나타나지 않아야 합니다.
...
| 항목 | 값 |
| --- | --- |
| 경로 | `/more/feedback` (`(more)` 그룹, 뒤로가기 헤더) |
...
### FE-SCREEN-008. 로그인·더보기 페이지 — `Phase 1 · 구현`

| 항목 | 값 |
| --- | --- |
| 라우트 | `/login`(`(auth)`, 뒤로가기 헤더) · `/more`(더보기 페이지, `(main)`, 헤더 + 하단탭) · `/more/invite`(`(more)`, 뒤로가기 헤더) |
| 관련 US | US-9-1 ~ 9-5, US-10-1 ~ 10-5 |

**목적** — 게스트를 회원으로 전환하고, 기기에 쌓은 서재를 계정으로 이관합니다. 더보기 페이지는 로그인 진입점과 회원 상태(프로필·크레딧·로그아웃), 친구 초대, 피드백을 모아두는 하단 탭 화면입니다.

**진입점** — 하단 탭 "더보기"(`/more`). 게스트는 로그인 버튼(→ `/login`)을, 회원은 프로필·로그아웃을 표시합니다. 게스트/회원 판정은 [§3-8](#3-8-api-연동에러-처리-계약) 토큰 세션의 모드 판정 소스를 따릅니다.

**더보기 페이지 회원 상태 구성** — 게스트는 로그인 버튼을 표시합니다. 회원은 닉네임·프로필 이미지·크레딧 잔액·출석 체크·친구 초대·로그아웃을 표시합니다. 피드백은 게스트와 회원 모두 진입할 수 있습니다.

**친구 초대** (`Phase 1 · 구현`, KNK-567) — 더보기의 친구 초대 메뉴는 `/more/invite`로 이동하며, 게스트 진입 시 `/login`으로 보냅니다.
...
- **등록 중 상태** — 입력창과 등록 버튼을 비활성화하고 버튼 문구 공간을 유지한 중앙 스피너를 표시합니다. 신규 가입 온보딩에서는 등록·건너뛰기 버튼을 모두 잠급니다.
...
- **회원(잔액 부족)** — "크레딧이 부족해요" 다이얼로그: 출석체크·친구 초대로 받는 방법을 안내하고 더보기 페이지로 유도합니다. (US-10-4)
...
1. **토큰 보관** — access·refresh 토큰을 BFF가 httpOnly 쿠키로 보관하고 백엔드 요청에 주입합니다([§3-8](#3-8-api-연동에러-처리-계약) 토큰 세션). 브라우저 JS에 토큰을 노출하지 않습니다. 초대 URL 라우트(`/invite/[code]`)·초대 코드 httpOnly 쿠키·로그인 요청의 `inviteCode` 전송은 폐기했고, 초대 보상 적립은 `/more/invite`의 코드 입력이 소유합니다([`4-backend.md §4-3-7`](./4-backend.md) 결정 기록).
...
**신규 가입 온보딩(초대 코드 입력)** (`Phase 1 · 구현`) — 신규 가입의 첫 로그인 완료 직후 "초대 코드가 있나요?" 다이얼로그를 표시합니다. 코드 입력란과 건너뛰기 버튼으로 구성하고, 제출·피드백 규칙은 `/more/invite` 입력란과 동일합니다. 건너뛰어도 자격(계정당 평생 1회)은 유지되어 이후 `/more/invite`에서 입력할 수 있습니다. 입력 성공 후 세션 플래그 저장이 끝날 때까지 등록 스피너와 입력 잠금을 유지하며, 저장 실패 시 보상은 다시 요청하지 않고 플래그 저장만 재시도합니다.
...
**로그아웃** (`Phase 1 · 구현`) — 더보기 페이지의 로그아웃은 `POST /auth/logout` 후 세션 쿠키를 폐기하고 게스트 모드로 복귀합니다. 로컬 배열은 로그인 시 비워졌으므로 빈 서재에서 새로 시작합니다. 분석 식별자는 [`6-analytics.md §6-2`](./6-analytics.md)에 따라 재설정합니다(공용 기기 보호).
...
- 더보기 페이지 헤더와 하단 네비게이션은 `더보기`라는 접근 가능한 이름을 사용하고, 레거시 `/my*` 경로는 대응하는 `/more*` 경로로 이동해야 합니다.
- 초대 코드 등록 중에는 버튼 문구 대신 중앙 스피너가 표시되고 입력이 잠겨야 합니다. 신규 가입 온보딩에서는 등록·건너뛰기 버튼이 함께 잠겨야 합니다.
...
- **생성** — `POST /stories/simple/storylines {selectedTagIds, customTags}`. 클릭 직후 "스토리라인 만들기" 버튼을 비활성화하고 문구 공간을 유지한 중앙 스피너를 표시하며, 동시에 스토리라인 선택 단계로 전환해 로딩 화면을 렌더합니다. 응답은 스토리라인 3개와 생성 ID(`simpleCreationId`)를 포함합니다. 로딩 중에는 타자기형 문구와 15·30초 시점 힌트를 표시합니다. `Phase 1 · 계획` — 회원은 무료이고, 게스트는 디바이스 ID별 스토리라인 생성·재생성 합산 5회 한도를 사용합니다.
...
- **입력 모드** — 일반 텍스트 모드와 블럭 모드(상황·대사)를 제공합니다. 두 모드는 같은 전송·추천 입력 규칙을 사용합니다.
...
- **입력이 없을 때** — 현재 추천 입력이 1개 이상이면 전송 버튼을 filled 재생 아이콘으로 표시하고 활성화합니다. 누르면 현재 추천 입력 중 하나를 무작위로 골라 즉시 전송합니다. 추천 입력도 없으면 같은 아이콘을 유지한 채 버튼을 비활성화합니다. 이 동작에는 별도 스피너를 표시하지 않으며, 요청이 시작되면 기존 스트리밍 잠금 규칙을 적용합니다.
...
| 계정 `Phase 1 · 구현` | `GET /auth/me` | 모드 판정·더보기 페이지 회원 상태 표시 | 회원 프로필 표시 | 401=게스트 모드 |
| 로그아웃 `Phase 1 · 구현` | `POST /auth/logout` | 더보기 페이지 로그아웃 | 세션 폐기·게스트 복귀 | 실패해도 로컬 세션은 폐기 |
| 회원 서재 `Phase 1 · 계획` | `GET /users/me/stories` · `GET /users/me/chats` | 회원 모드 목록 진입 | 서버 정본 표시 | 401=재발급 후 재시도, 그 외 재시도 상태 |
| 크레딧·초대 | `GET /users/me/credits` · `POST /users/me/credits/attendance` `Phase 1 · 계획` · `GET /users/me/invite` · `POST /users/me/invite/redeem` `Phase 1 · 구현`(KNK-567) | 더보기 페이지 진입·버튼 클릭·초대 코드 제출 | 잔액·적립 반영 | 실패 토스트(초대 코드 입력은 사유별 안내 — FE-SCREEN-008) |
...
- **모드 판정 소스** — 브라우저 JS는 토큰을 볼 수 없으므로, BFF가 세션 쿠키 유무를 담은 값을 SSR에 전달하거나 `GET /auth/me`(회원이면 200, 게스트면 401)로 판정합니다. 이 값이 더보기 페이지 회원 상태·데이터 소스 전환·분석 `is_logged_in`의 단일 기준입니다.
...
| --- | --- | --- |
| `smoke/navigation` | 아이콘 전용 하단 탭 이동·접근 가능한 이름·홈/채팅/더보기 헤더 | US-8-1 |
...
| `chats/chat-room` | 프롤로그·빈 입력의 무작위 추천 전송·입력 아이콘 전환·스트리밍·선택지·실패·설정 드로어 삭제 | US-5-3, US-6-1·2·3·4·8 |
...
| `my/login-page` | Google 로그인·이관 안내·약관 링크·로그인 로딩 | US-9-1·2 |
| `my/my-page` | 더보기 게스트·회원 상태·헤더·레거시 `/my` 리다이렉트 | US-9-1·5, US-10-1·2 |
...
| # | 간극 | 현황 | 방향 |
| --- | --- | --- | --- |
| G1 | 인증 | Google 로그인·`/me`·refresh·logout·자동 마이그레이션과 초대 코드 공유·입력은 `Phase 1 · 구현`으로 도입 완료(FE-SCREEN-008). 크레딧 정책 고도화와 회원 서재 데이터 소스 전환은 아직 `Phase 1 · 계획` | 잔여 항목은 §3-4·§3-8 로드맵대로 진행 |
...
- **회원 서재 데이터 소스 전환** — 로그인·자동 마이그레이션과 초대 코드 공유·입력은 도입 완료했습니다(FE-SCREEN-008). 남은 과제는 회원 모드 목록을 `GET /users/me/stories`·`/chats`로 전환하고 크레딧 정책 고도화를 마치는 것입니다.
