...
**화면 구성 (채팅 카드)** — 스토리 제목, 마지막 장면 미리보기, 턴 수, 마지막 활동 시각(상대 시간). 카드 전체가 채팅방 링크이며 우상단에 삭제 옵션 메뉴를 둡니다. `lastStoryPreview`가 빈 문자열이면 "채팅을 시작하고 이야기를 이어가 보세요" fallback을 표시합니다. `lastStoryPreview`가 `null`인 채팅은 목록 변환 단계에서 제외되어 카드로 나타나지 않습니다.
...
추가 정보(`additionalInfos`)는 `[선택한 추천 태그 텍스트, ...공백 제거한 자유 텍스트]`로 구성하며 **서버·zod 검증 상한은 총 3개**입니다. 현재 프론트는 자유 입력만 최대 10개로 안내해 추천 태그와 합산 시 3개를 초과할 수 있습니다([§3-13](#3-13-mvp-제외-범위와-스펙-구현-간극) G7). 완성은 두 경로로 나뉩니다.
...
- 프론트엔드는 `X-Manyak-Request-Id`를 생성·주입하지 않으며, 인증 토큰(`Authorization`)도 붙이지 않습니다.
...
- 추천 태그 토글 + 자유 텍스트 행(자유 텍스트 UI 최대 10개, 각 100자). 공백 제거 후 비어 있지 않은 값만 전송합니다.
- 전송 페이로드 `additionalInfos`(추천 태그 + 자유 텍스트)의 **서버·zod 검증 상한은 총 3개**입니다. 프론트 합산 차단은 미구현이라 초과 가능([§3-13](#3-13-mvp-제외-범위와-스펙-구현-간극) G7).
...
| G7 | 추가 정보 개수 | 서버·zod는 `additionalInfos` 총 3개(max)인데, 프론트는 자유 입력만 최대 10개로 안내하고 추천 태그까지 합산 전송 → 3개 초과 시 서버 검증 실패 가능 | 프론트에서 합산 3개로 제한 또는 안내 정정 |
