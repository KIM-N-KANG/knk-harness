...
| 이탈 시도(생성 결과 있음) `Phase 1 · 구현` | 확인 다이얼로그("나중에 이어서 만들까요? 지금까지 만든 내용은 임시 저장돼요") + 확정 시 진행 상태 임시 저장(아래 제작 임시 저장)                                          |
| 이탈 시도(키워드 이후·저장할 결과 없음) | 확인 다이얼로그("스토리를 그만 만들까요? 지금 나가면 만들고 있는 내용이 사라져요")                                                                                        |

- 이탈 가드는 키워드 단계를 제외한 모든 단계에서 활성입니다.
- 다이얼로그 문구는 이탈해도 내용이 남는지로 갈립니다. 임시 저장되거나 진행 중 요청이 복구 대상으로 남으면 "나중에" 안내를, 복원할 결과가 없으면(스토리라인 생성 실패 등) 기존 소실 경고를 표시합니다. `beforeunload` 경로는 브라우저 기본 확인창이라 문구를 제어하지 않습니다.
...
- **정리 규칙** — 서버가 상태 코드로 응답한 실패(HTTP 오류)는 레코드를 즉시 제거하고, 응답 자체를 받지 못한 네트워크 오류(백그라운드 전환·타임아웃)만 보존해 복구 대상으로 남깁니다. 원 응답과 복구 조회가 경합하면 레코드 제거를 선점한 쪽만 결과를 반영해 부수효과(카운터 증가·채팅 생성)가 이중 실행되지 않습니다.
...
사용자가 직접 이탈해도 만들던 스토리를 잃지 않게 합니다(2026-07-22 결정). 기존에는 백그라운드 전환처럼 **응답을 못 받은 경우**에만 이어서 만들 수 있었고, 뒤로가기로 나가면 선택한 스토리라인과 입력이 모두 사라졌습니다. 이탈을 확정하면 퍼널 진행 상태를 백그라운드 복구와 **같은 슬롯**(`manyak:pending-creation-request`)에 `STORY_DRAFT` 스테이지로 저장합니다. 슬롯이 하나이므로 임시 저장본은 항상 한 개이고, 새 생성을 시작하면 자연히 덮어씁니다.
...
- **범위 밖** — 키워드 단계 저장, 새로고침·탭 닫기(`beforeunload`) 저장, 서버 저장·기기 간 동기화, 만료(TTL)는 이번 범위가 아닙니다. 임시 저장본은 재개로 소비하거나 배너 닫기로 폐기하거나 새 생성으로 덮어쓰는 세 경로로만 정리됩니다.
...
- 스토리라인 생성 결과가 있는 상태에서 이탈을 확정하면 임시 저장 안내 다이얼로그가 뜨고, 홈 배너의 "이어서 만들기"로 저장된 단계가 복원돼야 합니다. (e2e `stories/story-create-draft`)
