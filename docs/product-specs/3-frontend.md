...
| `Phase 1 · 구현` | Phase 1 범위. 코드 반영 완료 | Google 로그인·로그아웃·BFF 토큰 세션·자동 마이그레이션·더보기 페이지·이관 1회 안내·약관 동의 고지·초대 코드 공유·입력·신규 가입 온보딩(FE-SCREEN-008, [§3-8](#3-8-api-연동에러-처리-계약))·서비스이용약관·개인정보처리방침 페이지(FE-SCREEN-010)·상세 썸네일 히어로·이미지 뷰어·시작 설정 선택(FE-SCREEN-003)·인앱 브라우저 감지·탈출([§3-10](#3-10-반응형접근성브라우저-지원)) |
| `Phase 1 · 계획` | Phase 1 범위. 미구현, 방향만 합의됨 | 크레딧 정책 고도화(FE-SCREEN-008, [§3-7](#3-7-게스트-데이터-모델)·[§3-8](#3-8-api-연동에러-처리-계약)). 일반 제작·스토리 수정(FE-SCREEN-009, [§3-9](#3-9-입력폼-검증-규칙)). AI 응답 재생성·채팅 이미지·엔딩 도달 표시([§3-6](#3-6-채팅-플레이와-sse-스트리밍))·목록 카드 썸네일(FE-SCREEN-001·004)·본 엔딩 표시(FE-SCREEN-003). Safari 렌더링·애니메이션·입력 UX 개선([§3-10](#3-10-반응형접근성브라우저-지원)) |
| `계획` | Phase 미배정. 미구현, 방향만 합의됨 | 서비스 안내 페이지(FE-SCREEN-011) |
...
경로는 MVP 6개 화면 라우트와 1개 프록시 라우트에 `/login`·`/more`·`/more/invite`·`/terms`·`/privacy`(구현 완료), `Phase 1 · 계획` 2개(`/stories/new/general`, `/stories/[id]/edit`), `계획` 1개(`/more/about`)를 더해 구성됩니다. 회원가입 별도 경로·대시보드·설정·관리자 경로는 없습니다. 동적 세그먼트 `[id]`는 문자열입니다.
...
| `/more/about` `계획` | FE-SCREEN-011 | `(more)` | 서비스 안내 | 뒤로가기 헤더(탭 없음) |
...
| FLOW-009 `계획` | 서비스 정보 열람 | 008 더보기 | 010 약관·개인정보 | 011 서비스 안내 | — |
...
**목적** — 게스트를 회원으로 전환하고, 기기에 쌓은 서재를 계정으로 이관합니다. 더보기 페이지는 로그인 진입점과 회원 상태(프로필·크레딧·로그아웃), 친구 초대, 피드백, 서비스 안내(`계획`, FE-SCREEN-011)를 모아두는 하단 탭 화면입니다.

**진입점** — 하단 탭 "더보기"(`/more`). 게스트는 로그인 버튼(→ `/login`)을, 회원은 프로필·로그아웃을 표시합니다. 게스트/회원 판정은 [§3-8](#3-8-api-연동에러-처리-계약) 토큰 세션의 모드 판정 소스를 따릅니다.

**더보기 페이지 회원 상태 구성** — 게스트는 로그인 버튼을 표시합니다. 회원은 닉네임·프로필 이미지·크레딧 잔액·출석 체크·친구 초대·로그아웃을 표시합니다. 기타 섹션의 피드백과 서비스 안내(`계획`, FE-SCREEN-011)는 게스트와 회원 모두 진입할 수 있습니다.
...
**진입점** — `/login` Google 로그인 버튼 하단 동의 고지의 밑줄 링크(서비스이용약관 → `/terms`, 개인정보처리방침 → `/privacy` — [FE-SCREEN-008](#3-4-화면별-스펙)). `계획` — 서비스 안내 페이지(FE-SCREEN-011)의 약관 및 정책 링크에서도 진입합니다. 로그인 전 게스트도 접근할 수 있으며 라우트 가드는 없습니다([§3-3](#3-3-라우팅레이아웃공통-셸)). 뒤로가기 헤더로 진입 이전 화면(`/login`)으로 복귀합니다.
...
### FE-SCREEN-011. 서비스 안내 — `계획`

| 항목 | 값 |
| --- | --- |
| 라우트 | `/more/about`, `(more)` 그룹(뒤로가기 헤더, 탭 없음) |
| 관련 US | — |

**목적** — 서비스 이용에 필요한 정책 정보(크레딧, 게스트 데이터, AI 콘텐츠 특성, 탈퇴·문의)를 약관보다 쉬운 말로 모아 보여주고, 서비스이용약관·개인정보처리방침(FE-SCREEN-010)으로의 상시 진입점을 제공합니다. 현재 두 문서의 진입점이 로그인 화면뿐이라 로그인 후에는 다시 볼 경로가 없습니다 — 개인정보처리방침은 이용자가 쉽게 확인할 수 있는 곳에 상시 공개해야 하므로(개인정보 보호법 제30조) 더보기에 상시 진입점을 마련합니다.

**진입점** — 더보기 페이지(FE-SCREEN-008) 기타 섹션의 "서비스 안내" 메뉴(피드백 위). 게스트·회원 모두 접근할 수 있으며 라우트 가드는 없습니다.
...
4. **회원 탈퇴·문의** — 탈퇴는 문의처(이메일) 요청으로 처리됨을 안내하고, 서비스 개선 의견은 피드백(`/more/feedback`)으로 유도.
...
- 더보기 기타 섹션의 서비스 안내 메뉴로 진입할 수 있어야 합니다(게스트·회원 동일).
