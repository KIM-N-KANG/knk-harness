...
모바일에서 스토리라인 생성·완성(스토리 생성) 대기 중 다른 앱으로 전환했다가 돌아와도 진행 상태를 이어서 확인할 수 있어야 합니다(2026-07-20 팀 결정). 서버는 연결이 끊겨도 생성·저장을 끝까지 진행하므로([`4-backend.md §4-3-2`](./4-backend.md) 복구 계약), 프론트엔드는 복구 조회로 결과를 되찾습니다(확정 제안). 생성 요청 전에 UUID `requestId`를 만들어 요청 본문에 싣고 단계 정보와 함께 `localStorage`에 저장합니다. 복귀(`visibilitychange` 등)나 퍼널 재진입 시 미정리 `requestId`가 있으면 `GET /stories/simple/creation-requests/{requestId}`로 상태를 확인해 `PENDING`이면 해당 단계 로딩 화면을 복원하고 폴링, `COMPLETED`면 결과 화면을 복원, `FAILED`면 기존 실패 처리로 합류합니다. 완료·실패를 화면에 반영한 뒤 저장한 `requestId`를 제거합니다. 폴링 주기와 이탈 가드([§3-5](#3-5-스토리-생성-퍼널))의 상호작용은 구현 시 확정합니다.
...
- **추천 입력 토글 — `Phase 1 · 계획`(KNK-622)** — 선택지(추천 입력) 생성을 응답 스트림에서 분리합니다(2026-07-20 팀 합의 — [`4-backend.md §4-3-3`](./4-backend.md)). 인풋 근처에 추천 입력 on/off 토글 버튼을 둡니다. 토글 on이면 `completed` 수신 직후 선택지 생성 API(`POST /chats/{id}/turns/{turnId}/choices`)를 호출해 도착한 3개를 마지막 턴의 선택지로 표시합니다(본문 확정과 선택지 도착 사이의 지연은 수용 — 본문이 먼저 확정되는 체감 개선이 목적). 토글 off면 호출하지 않고, off 상태로 턴이 끝난 뒤 토글을 켜면 같은 API로 마지막 턴의 선택지만 생성합니다. flag 없이 "API를 부르는가"로 on/off를 처리합니다. 토글 기본값은 on이고 변경값은 `localStorage`에 기기 단위로 기억합니다. 선택지 생성은 무료이며 마지막 턴만·턴당 1회 멱등 계약입니다([`4-backend.md §4-3-3`](./4-backend.md)). `completed`의 `choices`는 항상 빈 배열로 오므로 렌더 소스로 쓰지 않고, 선택지 API 응답의 3개를 직접 렌더합니다(선택지를 위한 추가 상세 refetch 불필요 — 재진입 시에는 상세 `turns[].choices`가 소스). 선택지 로딩은 선택지 자리의 스켈레톤 칩 3개로 표시하고, 도착하면 교체하며 실패하면 조용히 제거합니다(전용 재시도 UI 없음 — 직접 입력이 가능하고, 토글 off→on이 재요청 경로입니다).
