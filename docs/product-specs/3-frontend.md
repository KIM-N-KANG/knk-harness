...
**소모량 사전 고지** `Phase 1 · 구현` (US-10-3) — 스토리 제작 헤더는 회원에게 완성 시 20크레딧 차감을, 게스트에게 제작 1회 한도를 안내합니다. 채팅 헤더는 회원에게 전송·재생성 1회당 10크레딧 차감을, 게스트에게 모든 채팅방 합산 5회 한도를 안내합니다. 서비스 안내(FE-SCREEN-011)는 스토리라인 생성·재생성 5회, 스토리 제작 1회, 채팅 5회의 전체 체험 한도를 함께 표시합니다. 정책 수치의 정본은 [`4-backend.md §4-3-7`](./4-backend.md)입니다.
