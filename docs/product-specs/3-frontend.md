...
**소모량 사전 고지** `Phase 1 · 계획` (US-10-3) — 회원에게는 스토리라인 생성 무료, 스토리 생성 20 크레딧, 채팅 1턴 10 크레딧을 표시합니다. 게스트에게는 같은 위치에 남은 체험 횟수를 표시합니다: 스토리라인 생성·재생성 10회, 스토리 생성 3회, 모든 채팅방 합산 채팅 15회. 스토리 제작 완료 CTA와 채팅 전송 버튼 근처에는 해당 작업의 값만 표시하고, 스토리라인 단계의 "다시 만들기"에는 남은 스토리라인 생성 횟수를 표시합니다. `Phase 1 · 계획`(B13) — 디바이스의 체험 횟수가 남은 회원은 크레딧 대신 체험 횟수를 먼저 차감하므로([`4-backend.md §4-3-7`](./4-backend.md)), 소모량 고지는 이 2단 판정(체험 잔여 → 크레딧)과 어긋나지 않아야 합니다.
