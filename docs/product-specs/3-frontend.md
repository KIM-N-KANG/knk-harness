...
| `Phase 1 · 계획` | Phase 1 범위. 미구현, 방향만 합의됨 | 로그인·계정·크레딧 UI(FE-SCREEN-008, [§3-7](#3-7-게스트-데이터-모델)·[§3-8](#3-8-api-연동에러-처리-계약)). 일반 제작·스토리 수정(FE-SCREEN-009, [§3-9](#3-9-입력폼-검증-규칙)). 재생성·이미지 표시는 후속 반영에서 추가 |
...
| 관련 US | US-2-1, US-2-3, US-2-4, US-2-6 |

**목적** — 이 브라우저에서 만든 스토리를 카드 목록으로 보고, 상세로 진입하거나 새로 만들거나 삭제합니다.

**진입 조건** — 앱의 기본 진입 화면입니다. 최초 방문자에게는 온보딩 다이얼로그(FE-SCREEN-007)가 위에 표시됩니다.

**화면 구성** — 스토리 카드 목록(썸네일 자리 3:4, 제목 1줄, 장르 배지 초과 시 `+N`, 한 줄 소개 1줄, 생성일)과 우하단 생성 FAB(스크롤 시 라벨 접힘), 카드별 삭제 옵션 메뉴로 구성합니다.
...
| --- | --- |
| 경로 | `/chats/[id]` (`id`: 문자열) |
| 관련 API | `GET /chats/{id}`, `POST /chats/{id}/turns/stream`(SSE) |
| 관련 US | US-6-1 ~ US-6-8 |
...
**폼 구성** — 섹션 아코디언 구조. 필수: 기본 정보(제목·한 줄 소개·장르), 스토리 설정(세계관·등장인물·주인공·규칙 — 섹션별 텍스트 입력을 통글 마크다운으로 조립해 전송), 시작 설정(이름·프롤로그·시작 상황), 추천 입력 3개. 선택: 주요 사건(최대 10 — 이름·설명·키 문장), 엔딩(넣으려면 3종 세트 — 해피·노말·배드 각 1), 이미지(최대 10 — 이름·트리거 문구·업로드 또는 프리셋 선택, 썸네일 지정). 주요 사건·엔딩·이미지는 KNK-444·이미지 출력 스펙 확정 전까지 저장·수정 폼 왕복용 초안이며, 채팅 런타임 반영은 이 화면의 검수 대상이 아닙니다. 게스트는 이미지 업로드가 비활성화되고 프리셋만 선택할 수 있습니다.
