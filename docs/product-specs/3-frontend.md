...
현재 본문은 로그인 없는 전원 게스트 기준입니다. 인증·권한 화면과 흐름은 이 문서 범위이지만 아직 본문에 없으며, Phase 1 스펙 반영에서 `Phase 1 · 계획` 라벨로 추가합니다.
...
- 스토리 탐색, 좋아요·댓글·공유, 전문가 모드, 관리자 (MVP 제외, [§3-13](#3-13-mvp-제외-범위와-스펙-구현-간극)). 로그인·크레딧·일반 제작은 Phase 1 스펙 반영에서 이 문서에 추가
...
| `Phase 1 · 계획` | Phase 1 범위. 미구현, 방향만 합의됨 | 로그인·크레딧·일반 제작·스토리 수정·재생성·이미지 화면과 흐름 — 아직 본문 사용처 없음, Phase 1 스펙 반영에서 추가 |
