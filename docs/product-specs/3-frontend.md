...
- **이어서 만들기 배너** ⚠️ *스펙 밖 신규 UI — 팀 공유 필요(2026-07-21 구현)* — 미정리 복구 레코드가 있으면 홈(스토리 목록) 상단에 "만들고 있는/완성 중인 스토리가 있어요" 배너를 표시합니다. 레코드 존재만 확인하며(서버 조회 없음) "이어서 만들기" 탭 시 퍼널로 이동해 위 복구 흐름에 합류하고, 닫기(X)는 레코드를 폐기해 다음 진입은 새 생성부터 시작합니다.
