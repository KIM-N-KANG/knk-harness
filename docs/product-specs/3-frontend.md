...
| `Phase 1 · 구현` | Phase 1 범위. 코드 반영 완료 | Google 로그인·로그아웃·BFF 토큰 세션·자동 마이그레이션·마이 페이지 뼈대(FE-SCREEN-008, [§3-8](#3-8-api-연동에러-처리-계약)) |
...
경로는 MVP 6개 화면 라우트와 1개 프록시 라우트에 `/login`·`/my`(구현 완료)와 `Phase 1 · 계획` 2개(`/stories/new/general`, `/stories/[id]/edit`)를 더해 구성됩니다. 회원가입 별도 경로·대시보드·설정·관리자 경로는 없습니다. 동적 세그먼트 `[id]`는 문자열입니다.
...
- **라우트 그룹은 권한이 아니라 chrome을 가릅니다.** `(main)`은 상단 헤더와 하단 탭을 두르는 탭 화면, `(story)`·`(chat)`·`(auth)`·`(my)`는 헤더·하단탭이 없는(또는 뒤로가기 헤더만 있는) 전체 화면입니다.
...
**마이 페이지 회원 상태 구성** — 게스트는 로그인 버튼만 표시합니다(`Phase 1 · 구현`). 회원은 닉네임·프로필 이미지와 로그아웃 버튼을 표시합니다(`Phase 1 · 구현`). `Phase 1 · 계획` — 회원 상태에 크레딧 잔액(`GET /users/me/credits`), 출석체크 버튼(`POST /users/me/credits/attendance` — 성공 시 "크레딧 250개를 받았어요" 토스트, 이미 받았으면 버튼 비활성), 초대 링크 복사 버튼(`GET /users/me/invite` — 복사 완료 토스트와 "친구가 이 링크로 가입하면 둘 다 500 크레딧을 받아요. 초대 보상은 계정별 월 10회까지 받을 수 있어요" 안내)을 추가로 구성합니다. 회원가입 보상 500 크레딧은 가입 성공 후 서버가 자동 적립하므로 별도 버튼을 두지 않습니다.
...
**화면 구성·동작** (`Phase 1 · 구현`) — `/login`은 Google 로그인 버튼 단일 CTA. 로그인 성공 시 순서대로:

1. **토큰 보관** — access·refresh 토큰을 BFF가 httpOnly 쿠키로 보관하고 백엔드 요청에 주입합니다([§3-8](#3-8-api-연동에러-처리-계약) 토큰 세션). 브라우저 JS에 토큰을 노출하지 않습니다. `Phase 1 · 계획` — 초대 URL로 진입한 경우 URL의 초대 코드를 `POST /auth/login/google`의 `inviteCode`로 함께 전송합니다(현재는 전달하지 않음).
