...
- **영향.** Auth.js가 프로바이더 id로 환경 변수를 추론하므로(`AUTH_LINK_GOOGLE_ID`는 존재하지 않음) 변형에는 자격증명을 명시합니다. 콜백 URL이 하나 더 생겨 콘솔 등록이 배포 선행 조건이 됩니다. 연동 콜백에서는 jwt 콜백이 세션 클레임을 갱신하지 않고, 로그인 세션 없이 연동 콜백에 도달하면 로그인 자체를 실패시켜 사용자 귀속 없는 세션 발급을 막습니다.
