...
- **연동 시작 확인 다이얼로그** — 연동 버튼을 누르면 곧바로 OAuth로 보내지 않고 확인 다이얼로그를 먼저 띄웁니다. 재인증이 선행되므로 **대상 provider가 아니라 현재 로그인한 provider의 인증 창이 먼저** 뜨는데, 예고가 없으면 사용자가 흐름을 오해합니다. 또 연동은 해제할 수 없어 비가역 확인이 필요합니다. 다이얼로그는 혜택("어느 계정으로 로그인해도 같은 크레딧과 서재")·재인증 순서 예고·해제 불가 고지를 담고, 취소/연동하기 2버튼(다른 다이얼로그와 같은 배치)을 둡니다.
