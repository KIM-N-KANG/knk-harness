...
2. **공통 로그인 진입점** — 모든 Google 로그인 CTA(로그인 화면·게스트 한도 다이얼로그)는 공통 시작 함수를 거칩니다. 인앱이 아니면 기존 로그인 흐름(FE-SCREEN-008) 그대로입니다. 인앱에서는 홈 헤더·더보기의 로그인 버튼도 `/login`을 거치지 않고 공통 시작 함수로 직행합니다 — 인앱의 `/login`은 Google 버튼 하나뿐이라 전환 안내로 이어지는 중간 단계일 뿐이기 때문입니다(복귀 경로는 버튼을 누른 현재 화면).
