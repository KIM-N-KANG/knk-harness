...
**마이 페이지 회원 상태 구성** — 게스트는 로그인 버튼만 표시합니다(`Phase 1 · 구현`). 회원은 닉네임·프로필 이미지와 로그아웃 버튼을 표시합니다(`Phase 1 · 구현`). `Phase 1 · 계획` — 회원 상태에 크레딧 잔액(`GET /users/me/credits`), 출석체크 버튼(`POST /users/me/credits/attendance` — 성공 시 "크레딧 250개를 받았어요" 토스트, 이미 받았으면 버튼 비활성), 초대 링크 복사 버튼(`GET /users/me/invite` — 복사 완료 토스트와 "친구가 이 링크로 가입하면 둘 다 500 크레딧을 받아요. 초대 보상은 계정별 월 10회까지 받을 수 있어요" 안내)을 추가로 구성합니다. 회원가입 보상 500 크레딧은 가입 성공 후 서버가 자동 적립하므로 별도 버튼을 두지 않습니다. `Phase 1 · 계획`(B12) — 보상 크레딧은 적립 후 30일 유효하며, 잔액은 만료분을 제외해 서버가 계산한 값을 그대로 표시합니다([`4-backend.md §4-3-7`](./4-backend.md)).
