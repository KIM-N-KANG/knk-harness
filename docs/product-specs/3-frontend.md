...
MVP 본문은 로그인 없는 전원 게스트 기준이고, Phase 1 로그인·계정 흐름은 `Phase 1 · 계획` 라벨로 본문에 포함됩니다(FE-SCREEN-008, 토큰 세션은 [§3-8](#3-8-api-연동에러-처리-계약)). 크레딧·권한 강화 등 후속 항목은 각 서브태스크에서 추가합니다.
...
- 스토리 탐색, 좋아요·댓글·공유, 전문가 모드, 관리자 (MVP 제외, [§3-13](#3-13-mvp-제외-범위와-스펙-구현-간극)). 크레딧·일반 제작은 후속 서브태스크에서 이 문서에 추가
...
| `Phase 1 · 계획` | Phase 1 범위. 미구현, 방향만 합의됨 | 로그인·계정(FE-SCREEN-008, [§3-7](#3-7-게스트-데이터-모델)·[§3-8](#3-8-api-연동에러-처리-계약)). 크레딧·일반 제작·스토리 수정·재생성·이미지는 후속 반영에서 추가 |
...
경로는 MVP 6개 화면 라우트와 1개 프록시 라우트에 `Phase 1 · 계획` `/login`을 더해 구성됩니다. 회원가입 별도 경로·대시보드·설정·관리자 경로는 없습니다. 동적 세그먼트 `[id]`는 문자열입니다.
