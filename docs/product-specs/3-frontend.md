...
이 문서는 마냑 웹 프론트엔드가 구현하고 QA가 검수해야 할 계약을 정의합니다. 화면, 사용자 흐름, 화면 상태, API 연동, 오류 처리, 검수 기준을 다룹니다.
...
MVP는 로그인 없는 전원 게스트로 동작합니다. 인증·권한·세션 처리는 범위에 없습니다.
...
| 5 | 채팅 SSE 스트리밍 | AI 응답을 토큰 단위로 스트리밍합니다. | [§3-6](#3-6-채팅-플레이와-sse-스트리밍) |
...
경로는 6개 화면 라우트와 1개 프록시 라우트로 구성됩니다. 로그인·회원가입·대시보드·설정·관리자 경로는 없습니다. 동적 세그먼트 `[id]`는 문자열입니다.
...
| `/chats/[id]` | FE-SCREEN-005 | `(chat)` | 채팅방 | 없음(자체 헤더) |
...
| 상세 CTA → 채팅방 | "채팅 시작하기" | `replace` | 뒤로가기 재진입으로 채팅 중복 생성 방지 |
| 퍼널 완료 → 채팅방 | 스토리·채팅 생성 완료 | `replace` | 동상 |
| 채팅방 뒤로 | 헤더 back | `push('/chats')` | 진입 경로와 무관하게 채팅 목록으로 복귀 |
...
- **상단 헤더** — `(main)` 화면에서만 로고와 현재 섹션 라벨을 표시합니다. 스크롤 후에만 하단 보더를 표시합니다.
...
각 화면은 목적·진입 조건·구성·사용자 액션·API·상태·검수 기준으로 정의합니다. 스토리 생성 퍼널(FE-SCREEN-002)과 채팅방(FE-SCREEN-005)은 복잡도가 높아 여기서는 최소 항목만 두고 각각 [§3-5](#3-5-스토리-생성-퍼널), [§3-6](#3-6-채팅-플레이와-sse-스트리밍)에서 정의합니다. 화면은 모두 P0입니다.
...
| FLOW-002 | 스토리 생성 | 002 퍼널 | 005 채팅방 | 001 홈 | US-3 |
| FLOW-003 | 탐색 → 플레이 시작 | 001 홈 | 005 채팅방 | 003 상세 | US-2-1·2, US-4-1·2 |
| FLOW-004 | 채팅 플레이(스트리밍) | 005 채팅방 | 005 채팅방 | — | US-6 |
| FLOW-005 | 채팅 이어가기 | 004 채팅목록 | 005 채팅방 | — | US-5-1·2 |
...
**화면 구성** — 스토리 카드 목록(커버 자리 3:4, 제목 1줄, 장르 배지 초과 시 `+N`, 한 줄 소개 1줄, 생성일)과 우하단 생성 FAB(스크롤 시 라벨 접힘), 카드별 삭제 옵션 메뉴로 구성합니다.
...
| 삭제 | 옵션 메뉴 → 확인 | 낙관적 삭제 후 완료 토스트([§3-7](#3-7-게스트-데이터-모델)) |
...
**목적** — 키워드 선택과 AI 스토리라인 생성을 거쳐 스토리를 만들고, 완성 직후 바로 채팅방으로 진입합니다. 단계·검증·이탈 가드·재시도는 [§3-5](#3-5-스토리-생성-퍼널)에서 정의합니다.
...
**화면 구성 (채팅 카드)** — 스토리 제목, 마지막 장면 미리보기(`lastStoryPreview`, 없으면 "채팅을 시작하고 이야기를 이어가 보세요"), 턴 수, 마지막 활동 시각(상대 시간). 카드 전체가 채팅방 링크이며 우상단에 삭제 옵션 메뉴를 둡니다.
...
1. **최초 완성** — `POST /stories/simple {simpleCreationId, storylineId, additionalInfos}` → 성공 시 스토리 ID를 로컬에 저장 → `POST /chats {storyId}` → 성공 시 채팅 ID 저장·완료 이벤트·상세 prefetch·완료 토스트 후 채팅방으로 `replace`.
...
- 채팅방은 `GET /chats/{id}`로 스토리 제목·프롤로그·턴 목록·추천 입력을 조회합니다.
...
- **요청** — `POST /api/v1/chats/{id}/turns/stream`, 헤더 `Accept: text/event-stream` + `X-Manyak-*` 식별자, 본문 `{userInput}`(최대 3000자).
...
- **주의**: 스트리밍 경로는 공용 요청 계층을 우회하므로 120초 클라이언트 타임아웃이 적용되지 않습니다([§3-13](#3-13-mvp-제외-범위와-스펙-구현-간극) G5).
...
- 스토리: `POST /api/v1/stories/batch {storyIds}`, 채팅: `POST /api/v1/chats/batch {chatIds}`. ID가 비어 있으면 요청하지 않습니다.
...
온보딩 다이얼로그의 알파 고지("이 브라우저에만 저장")가 이 한계를 사용자에게 알립니다. 향후 로그인 도입 시 서재 이관 전략이 필요합니다([§3-13](#3-13-mvp-제외-범위와-스펙-구현-간극)).
...
- 요청에 인증 토큰(`Authorization`)은 붙지 않습니다.
...
- 서버 원본 메시지·스택은 사용자에게 그대로 노출하지 않습니다. 인증 만료 개념이 없으므로 401 전용 리다이렉트 처리는 하지 않습니다.
