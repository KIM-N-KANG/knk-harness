...
**계정 시트 구성** `Phase 1 · 계획` — 닉네임·프로필 이미지, 크레딧 잔액(`GET /users/me/credits`), 출석체크 버튼(`POST /users/me/credits/attendance` — 성공 시 "크레딧 250개를 받았어요" 토스트, 이미 받았으면 버튼 비활성), 초대 링크 복사 버튼(`GET /users/me/invite` — 복사 완료 토스트와 "친구가 이 링크로 가입하면 둘 다 500 크레딧을 받아요" 안내), 로그아웃 버튼. 회원가입 보상 500 크레딧은 가입 성공 후 서버가 자동 적립하므로 별도 버튼을 두지 않습니다.
