...
| 이탈 시도(내용 보존 — 임시 저장 또는 진행 중 복구) `Phase 1 · 구현` | 다이얼로그 없이 즉시 이탈. 진행 상태를 임시 저장(아래 제작 임시 저장)하거나 진행 중 슬롯을 유지하고 토스트("만들던 스토리를 임시 저장했어요")로 알립니다                  |
...
- 스토리라인 생성 결과가 있는 상태에서 뒤로가기로 나가면 다이얼로그 없이 즉시 이탈하며 "만들던 스토리를 임시 저장했어요" 토스트가 뜨고, 홈 배너의 "이어서 만들기"로 저장된 단계가 복원돼야 합니다. (e2e `stories/story-create-draft`)
