...
- 스토리라인 생성 재요청이 재생성(다시 만들기)일 때 원본 `creation_id`를 함께 보내면 백엔드가 `X-Manyak-Parent-Creation-Id`로 AI에 forward합니다. 보내지 않으면 헤더가 생략되고 서버는 신규 진행으로 취급합니다 — 스토리라인 재생성은 전용 API가 없어 서버가 원본과의 연결을 스스로 알 수 없기 때문입니다([`4-backend.md §4-7`](./4-backend.md)).
