...
**화면 구성·동작** — 전체 화면 페이지입니다. 두 줄 제목("뭘 쓸지 고민 말고" / "키워드부터 골라보세요")과 한 줄 설명("나머지는 AI와 함께 만들어가면 돼요"), 그 아래 스토리 만들기 과정을 보여주는 미리보기 콘텐츠, 하단에 버튼 두 개를 둡니다. 게스트 한도·저장 범위 고지는 이 화면에서 다루지 않고 서비스 안내 화면(FE-SCREEN-011)과 생성·채팅 화면의 크레딧 안내 팝오버가 담당합니다.
