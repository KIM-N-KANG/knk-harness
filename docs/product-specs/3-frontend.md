...
| 메인 진입 → 온보딩      | 신규 방문자 판정      | `replace`           | 뒤로가기 재진입으로 조회 중복 방지      |
...
**화면 구성·동작** — 전체 화면 페이지입니다. 두 줄 제목("눈을 떠보니" / "스토리 속 주인공이 되었다")과 한 줄 설명("AI와 함께 나만의 다음 장면을 만들어보세요"), 그 아래 스토리 만들기 과정을 보여주는 미리보기 콘텐츠, 하단에 버튼 두 개를 둡니다. 게스트 한도·저장 범위 고지는 이 화면에서 다루지 않고 서비스 안내 화면(FE-SCREEN-011)과 생성·채팅 화면의 크레딧 안내 팝오버가 담당합니다.
