...
**트레이스 분석 차원.** AI가 트레이스에 싣는 저카디널리티 차원은 장르 태그·프롬프트 버전 맵·`retry_count`입니다([`5-ai-server.md §5-6`](./5-ai-server.md)). 장르는 **사전 정의 장르만** 태그로 답니다 — 커스텀 장르 입력(`customTags` category `GENRE`)이 섞이면 카디널리티가 폭발하는데, 백엔드 키워드 단계 개편(KNK-621)이 커스텀 장르 입력을 400으로 차단해 `genre_tags`가 사전 정의만 남으므로 별도 구분 전달은 필요 없습니다. 이 차원은 저장된 트레이스에 소급 부착되지 않으므로, 관측 켜기는 KNK-621 배포 이후여야 초기 데이터부터 장르로 분해됩니다([`7-deployment.md §7-9`](./7-deployment.md)). 장르는 스토리 제작 시점에만 선택되므로 사전 정의 장르 태그는 **스토리 제작 트레이스에만** 답니다 — 채팅 트레이스 장르 태그는 KNK-652에서 제거했고(AI `v0.2.1` 배포 완료 — [`5-ai-server.md §5-6`](./5-ai-server.md)), 채팅 트레이스에 장르가 필요한지는 후속(필요 여부와 매 턴 forward 비용을 함께 판단)입니다.
