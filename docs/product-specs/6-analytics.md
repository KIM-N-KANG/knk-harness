...
**트레이스 분석 차원.** AI가 트레이스에 싣는 저카디널리티 차원은 장르 태그·프롬프트 버전 맵·`retry_count`입니다([`5-ai-server.md §5-6`](./5-ai-server.md)). 장르는 **사전 정의 장르만** 태그로 답니다 — 커스텀 장르 입력(`customTags` category `GENRE`)이 섞이면 카디널리티가 폭발하므로, 백엔드가 사전 정의 장르를 구분해 전달하고 AI가 그것만 답니다(태깅 코드 반영은 KNK-640 후속). 이 차원은 저장된 트레이스에 소급 부착되지 않으므로, 관측을 켜기 전에 배선돼야 초기 데이터부터 장르로 분해됩니다.
