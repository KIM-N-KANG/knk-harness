...
**배경.** Sentry는 오류를 모아 두지만 사람이 대시보드를 열어야 압니다. 슬랙 제보([`slack-report.md`](../templates/slack-report.md))도 사람이 요청해야 움직입니다. 방아쇠가 사람이라, 아무도 열어 보지 않은 날은 장애가 있었는지조차 모릅니다.
...
| 영향 인원 | 조회 창 기준 값(`count_unique(user)`)만 씁니다. 이슈 목록이 주는 사용자 수는 누적이라 매일 부풀려집니다 |
| 0건인 날 | 침묵하지 않고 "오류 없음"을 보냅니다 |

- **오류 0건인 날에도 보내는 이유**: 매일 무언가 오면 파이프라인이 끊긴 날을 사람이 알아챕니다. 그래서 죽음 감시를 따로 두지 않습니다.
- **조회 실패와 오류 없음을 구분합니다.** 판정 근거는 조회 호출의 성패입니다. 호출이 실패하면(HTTP 400·타임아웃) 요약 대신 조회 실패 사실을 보내고, 호출이 성공했는데 결과가 비면 "오류 없음"으로 봅니다. 장애가 난 날을 조용한 날로 보고하는 것이 이 파이프라인의 가장 나쁜 실패입니다.
- **제품 지표로 쓰지 않습니다.** Sentry 이벤트 수를 지표 계산에 사용하지 않는 원칙(§6-6-1)은 그대로입니다.
- **백엔드는 영향 인원이 0으로 나옵니다.** AI·서버는 사용자 컨텍스트를 붙이지 않기 때문입니다(§6-6-8·§6-6-10 대비). 사용자 수로 정렬하거나 임계값을 걸면 백엔드 오류가 매일 바닥에 깔립니다. 그래서 이 요약은 심각도가 아니라 서비스별로 나눕니다.

**원문 비수집 유지(§6-7).** Sentry 이슈 상세 응답에는 프롬프트 원문·지역변수·`device_id_hash`가 들어 있습니다. **요약은 이슈 상세를 열지 않습니다.** 이슈 이름·발생 횟수·마지막 발생 시각·영향 인원은 집계 조회로만 받습니다. 상세를 열고 "슬랙에 옮기지 않는다"고만 정하면, 옮기지 않아도 그 내용이 요약을 만드는 에이전트의 실행 기록에 남습니다. 슬랙 메시지에도 사용자 입력 원문·채팅 원문·식별자를 싣지 않습니다.
