...
| Langfuse         | AI LLM 트레이싱      | 정상·실패 LLM 호출의 프롬프트·응답 원문·토큰·지연, 선호 신호 결합 — AI 프로덕션 전용(`Phase 1 · 계획`) |
...
### 6-6-12. AI LLM 트레이싱과 선호 신호 (Langfuse) — `Phase 1 · 계획`
...
- **선호 신호는 Langfuse score로 붙입니다(백엔드 직접 전송).** 이는 관측·선호 분석용이며, 품질 평가·자가개선 루프는 이 원문을 쓰지 않습니다(§6-6-11 — 벤치 합성 데이터로 해결).
...
**트레이스 분석 차원.** AI가 트레이스에 싣는 저카디널리티 차원은 장르 태그·프롬프트 버전 맵·`retry_count`입니다([`5-ai-server.md §5-6`](./5-ai-server.md)). 장르는 **사전 정의 장르만** 태그로 답니다 — 커스텀 장르 입력(`customTags` category `GENRE`)이 섞이면 카디널리티가 폭발하는데, 백엔드 키워드 단계 개편(KNK-621)이 커스텀 장르 입력을 400으로 차단해 `genre_tags`가 사전 정의만 남으므로 별도 구분 전달은 필요 없습니다. 이 차원은 저장된 트레이스에 소급 부착되지 않으므로, 관측 켜기는 KNK-621 배포 이후여야 초기 데이터부터 장르로 분해됩니다([`7-deployment.md §7-9`](./7-deployment.md)). 장르는 스토리 제작 시점에만 선택되므로 사전 정의 장르 태그는 **스토리 제작 트레이스에만** 답니다 — 채팅 트레이스 장르 태그 제거는 AI 코드 반영 예정이고([`5-ai-server.md §5-6`](./5-ai-server.md)), 채팅 트레이스에 장르가 필요한지는 후속(필요 여부와 매 턴 forward 비용을 함께 판단)입니다.
...
**AI LLM 트레이싱(Langfuse) 예외 — `Phase 1 · 계획`.** 위 표의 "저장 금지"는 운영 로그·Sentry·`ai_call_logs`·분석 이벤트에 대한 규칙입니다. AI 서버의 LLM 트레이싱(Langfuse)은 이 원칙의 **명시적 예외**로, 정상·실패 LLM 호출의 프롬프트·응답 원문을 별도 관측 저장소(Langfuse Cloud)에 남깁니다. 위 문단이 요구하는 "별도 보안 정책과 제한된 저장소"에 해당하며, 확정된 조건은 다음과 같습니다:

- **저장 리전**: 일본(JP) — HOST `https://jp.cloud.langfuse.com`. **HOST를 지정하지 않으면 코드 기본값(JP 아님)으로 전송되므로** 운영에서 반드시 이 값으로 덮어씁니다([`5-ai-server.md §5-6`](./5-ai-server.md)).
- **접근·삭제·권한 책임**: AI 담당자로 한정.
- **보존 기간**: 무기한(별도 삭제 정책 수립 전까지 — 기간을 정하면 이 값을 갱신).
- **범위**: 프로덕션 전용(실험·로컬 제외).
- **강제는 후속**: 현재 코드는 키 유무만 보고 켜지므로, prod·JP가 아니면 켜지지 않게 막는 것은 코드 후속 과제입니다(체크리스트만으로 막지 않음).

사용자 식별은 원본이 아니라 기기 해시(`device_id_hash`)로만 싣습니다. 사용자 자유입력(`user_input`·`additional_info`·커스텀 태그)은 tags·metadata 같은 색인 차원에는 넣지 않지만, **LLM 프롬프트의 일부라 Langfuse 요청 원문에는 저장됩니다** — 색인 차원으로 올리지 않을 뿐입니다. 평가·자가개선 루프는 이 예외를 쓰지 않고 여전히 운영 원문을 수집하지 않습니다(§6-6-11). 구현은 [`5-ai-server.md §5-6`](./5-ai-server.md), 선호 신호 결합은 §6-6-12입니다.
...
| Langfuse 원문 예외 | Langfuse가 §6-7 확정 조건(JP 리전·AI 담당자 한정 접근·prod 전용)대로 동작하는지, tags·metadata에 사용자 자유입력이 색인되지 않는지 확인합니다(`Phase 1 · 계획`).                             |
