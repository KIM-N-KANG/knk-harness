...
**로그인 진행 중 상태** (`Phase 1 · 구현`, KNK-760) — 소셜 로그인 CTA(`/login`·외부 핸드오프 랜딩·게스트 한도 다이얼로그 공통)는 버튼 탭 직후 탭한 버튼을 문구 공간을 유지한 중앙 스피너로 바꾸고 두 provider 버튼을 모두 비활성화합니다. OAuth 리다이렉트 전 공백에서의 중복 탭(클릭 계측 중복 적재·핸드오프 이중 생성)과 provider 교차 탭을 막기 위해서입니다. 공통 시작 함수는 페이지 이탈 시작 여부(`redirected`/`failed`)를 반환하며, 이탈 없이 실패하면(핸드오프 생성 실패, signIn 준비 실패) 실패 토스트와 함께 잠금을 풀어 재시도를 허용합니다. OAuth 화면에서 뒤로가기로 복귀하면 bfcache가 잠금 상태까지 복원할 수 있으므로 `pageshow`(persisted)에서 상태를 리셋합니다. 게스트 한도 다이얼로그는 진행 중 바깥 터치·Escape 닫기를 막습니다(진행 중 이탈로 상태가 꼬이지 않게 — 계정 연동 확인 다이얼로그와 같은 규칙).

로그인 성공 시 순서대로:

1. **토큰 보관** — access·refresh 토큰을 BFF가 httpOnly 쿠키로 보관하고 백엔드 요청에 주입합니다([3-2-web-app.md §3-2-4](./3-2-web-app.md#3-2-4-bff-프록시토큰-세션) 토큰 세션). 브라우저 JS에 토큰을 노출하지 않습니다. 초대 URL 라우트(`/invite/[code]`)·초대 코드 httpOnly 쿠키·로그인 요청의 `inviteCode` 전송은 폐기했고, 초대 보상 적립은 `/my/invite`의 코드 입력이 소유합니다([`4-backend.md §4-3-7`](./4-backend.md) 결정 기록).
2. **자동 마이그레이션** — `localStorage`의 스토리·채팅 ID 배열([§3-1-6](#3-1-6-게스트-데이터-모델))을 `POST /auth/migrate`로 제출합니다. 사용자 확인 없이 실행합니다.
...
- **플로우** — 확인 → 현재 provider 재인증 OAuth → `POST /auth/links/reauth`로 링크 코드 발급(httpOnly 쿠키 보관) → 중계 화면(`/my/link/continue?target=`)이 대상 provider OAuth 시작 → `POST /auth/links/{provider}`(`X-Manyak-Link-Code` 헤더) → 성공 시 `GET /auth/me` 재조회로 Chip 갱신 + 성공 토스트. 두 OAuth는 NextAuth의 연동 전용 프로바이더 변형(`link-google`·`link-kakao`)을 써서 로그인 경로(백엔드 로그인 호출·세션 교체)를 타지 않습니다. 단계 판정은 클라이언트 상태가 아니라 콜백 provider의 `linkedProviders` 포함 여부로 서버가 하므로, 중단된 이전 시도의 잔여 쿠키에도 자기 교정됩니다. 현재 provider가 Google이면 재인증 OAuth에 세션의 이메일을 `login_hint`로 실어, 구글 세션이 살아 있을 때 계정 선택 화면 없이 자동 통과시킵니다(KNK-760) — 재인증 대상은 현재 계정 하나로 고정이라 힌트가 선택을 뺏지 않고, 다른 계정을 골라 403 `REAUTH_FAILED`가 나는 사고도 줄입니다. 재인증 요구 자체(2단계 프로토콜)는 백엔드 계약이므로 유지합니다.
...
- **인앱 브라우저 차단** — 연동은 재인증·연동 두 단계 모두 OAuth라 Google 인증이 반드시 끼는데 인앱 WebView는 Google이 막습니다. 인앱에서 연동 버튼을 누르면 플로우를 시작하지 않고 "계정 연동은 외부 브라우저에서만 할 수 있어요" 토스트로 안내합니다(로그인의 핸드오프 경로는 재사용하지 않습니다 — 연동은 기존 세션 유지가 전제).
...
게스트 모드의 "내 스토리·채팅"은 서버의 사용자 소유 목록이 아니라 **브라우저가 보관한 공개 ID 배열**로 구성합니다. 회원 모드는 [FE-SCREEN-008](#3-1-3-화면별-스펙)의 서버 정본 목록을 사용합니다. 사용자가 보는 목록의 동작을 좌우하므로 게스트 저장 구조를 계약으로 정의합니다. 식별자 정책은 [`6-analytics.md §6-2`](./6-analytics.md)를 따릅니다.
