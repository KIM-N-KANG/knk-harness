...
각 항목은 조건, 입력, 출력, 화면 상태, 예외 처리, 완료 기준을 구체값으로 적습니다. 플랫폼별 기술 환경·라우팅·구현 세부는 플랫폼 문서([`3-2-web-app.md`](./3-2-web-app.md)·[`3-3-mobile-app.md`](./3-3-mobile-app.md))가 소유합니다.
...
- **구현자** — 담당 화면의 §3-1-3 스펙 → 관련 상세 절(§3-1-4~§3-1-8) → §3-1-7 API 연동·에러 처리 순서로 보고, 담당 플랫폼 문서([`3-2-web-app.md`](./3-2-web-app.md)·[`3-3-mobile-app.md`](./3-3-mobile-app.md))로 구현 세부를 확인합니다. 각 결정 지점의 "결정 기록"(배경·대안·영향)이 왜 이 계약인지의 근거입니다.
- **리뷰·QA** — 각 화면·절의 검수 기준과 §3-1-9 간극 표(G2~G6), 플랫폼 문서의 테스트 절에서 시작해 본문을 역참조합니다.

### 플랫폼 표기 원칙

이 문서는 웹 구현을 기준으로 검증된 계약을 서술하므로, 본문에 웹 메커니즘(경로·`localStorage`·`replace`/`push` 히스토리·`beforeunload` 등)이 예시로 등장합니다. 이때 **계약(무엇을 보장해야 하는가)은 플랫폼 공통**이고, 그 메커니즘 자체는 웹 구현([`3-2-web-app.md`](./3-2-web-app.md))의 소유입니다. 모바일 앱은 같은 계약을 플랫폼 수단(내비게이션 스택·로컬 저장소·라이프사이클 콜백 등)으로 충족하며, 대응 규칙은 [`3-3-mobile-app.md`](./3-3-mobile-app.md)가 정의합니다.
...
| [`3-3-mobile-app.md`](./3-3-mobile-app.md) | 안드로이드 앱 플랫폼 구현      | 이 문서의 계약을 모바일이 충족하는 방식    |
