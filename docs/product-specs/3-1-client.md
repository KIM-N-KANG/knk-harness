...
| #   | 특징               | 요지                                                                                      | 상세                                          |
| --- | ------------------ | ----------------------------------------------------------------------------------------- | --------------------------------------------- |
| 1   | 전원 게스트(익명)  | 로그인·회원가입·권한이 없습니다. 사용자 식별은 Amplitude `device_id`뿐입니다.             | [§3-1-9](#3-1-9-mvp-제외-범위와-스펙-구현-간극) |
| 2   | BFF 프록시         | 브라우저는 백엔드를 직접 호출하지 않고 `/api/*`가 `API_BASE_URL`로 전달합니다.            | [3-2-web-app.md §3-2-4](./3-2-web-app.md#3-2-4-bff-프록시토큰-세션) |
| 3   | 게스트 데이터 모델 | "내 스토리·채팅"은 `localStorage`의 공개 ID 배열을 `POST /…/batch`로 재조회해 구성합니다. | [§3-1-6](#3-1-6-게스트-데이터-모델)               |
| 4   | 생성 퍼널          | `/stories/new` 단일 라우트에서 로컬 상태로 4단계를 전환합니다.                            | [§3-1-4](#3-1-4-스토리-생성-퍼널)                 |
| 5   | 채팅 SSE 스트리밍  | AI 출력을 토큰 단위로 스트리밍합니다.                                                     | [§3-1-5](#3-1-5-채팅-플레이와-sse-스트리밍)       |
