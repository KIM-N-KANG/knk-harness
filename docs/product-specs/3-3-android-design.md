...
[공통 스펙](./3-1-client-spec.md)과 [Android 스펙](./3-3-android-spec.md)을 충족하는 현재 구조입니다. 선택 이유는 [Android ADR](./3-3-android-adr.md), 일회성 변경 단계는 구현 저장소의 기능 계획, 반영·검증·배포 증거는 [추적](../planning/client-tracking.md)이 소유합니다.
...
| 분류 | 선택 |
| --- | --- |
| UI 상태 관리 | **직접 구현 MVI**(`MviViewModel` 베이스 클래스) |
| 의존성 주입 | **Hilt** |
| 내비게이션 | **Navigation 3**(`NavKey` 백스택) |
| HTTP·직렬화 | **Retrofit + OkHttp + kotlinx.serialization** |
| SSE | **okhttp-sse** |
| 로컬 저장(설정·토큰) | **DataStore** |
| 이미지 로딩 | **Coil** |
| 소셜 로그인 | **Credential Manager**(구글) · **Kakao SDK v2**(카카오) |
...
**백엔드는 직접 호출합니다.** 웹의 BFF 프록시에 대응하는 중간 계층을 두지 않습니다 — 웹이 프록시를 둔 이유는 브라우저 JS에 토큰을 노출하지 않기 위해서인데([`3-2-web-design.md §3-2-4`](3-2-web-design.md#3-2-4-bff-프록시토큰-세션)), 앱은 토큰을 데이터 계층 안에 가둘 수 있어 같은 목적을 프록시 없이 달성합니다(§3-3-4).
...
에서 기능·기반을 최상위 Gradle 모듈로 이전했습니다. 상세 정본은 [Android 모듈 아키텍처](../planning/android-module-architecture.md), 이동 순서·커밋·실행 검증은 [Android 실행 기록](../../../manyak-android/docs/plans/module-reorganization.md)입니다. 하네스 `docs/-improve-folder-structure`와 Android `refactor/-improve-folder-structure`의 대응 변경이며 병합·배포 완료를 뜻하지 않습니다.

런타임 모듈은 `app`, `common`, `auth`, `network`, `analytics`, `navigation`, `designsystem`, `report`, `home`, `chat`, `studio`, `story`, `create`, `my`, `login`, `legal`이고, 부터 화면 기능 `notification`(푸시 토큰·알림 수신·수신 동의, §3-3-4 푸시 토큰 등록)이 더해집니다. 공통 빌드 설정은 included build `build-logic`으로 관리합니다. 이전 core/feature 모듈과 화면 루트 패키지 강제 규칙은 적용하지 않습니다.
...
`SessionTerminationCoordinator`와 사용자 귀속 저장소의 중앙 등록은 composition root인 `app`이 소유합니다. 화면은 auth/domain의 세션 계약을 호출하고, app이 auth의 토큰·공급자, my의 프로필·초대 안내, notification의 표시 알림 정리와 analytics의 식별 해제를 조합합니다. 제작의 보존 데이터는 아래 ownerId·SessionGate 규칙으로 격리합니다. 종료 뒤에는 이동 명령 대신 세션 상태를 발행해 루트가 그래프를 교체합니다.
...
- 읽기 요청은 공통 계약대로 자동 재시도·복귀 자동 재조회 없이 오류를 상태로 반환하고, 화면이 인라인으로 처리합니다([`3-1-client-spec.md §3-1-7`](3-1-client-spec.md#3-1-7-api-연동에러-처리-계약)).
...
[`3-1-client-spec.md §3-1-3`](3-1-client-spec.md#3-1-3-화면별-스펙)의 화면 인벤토리(FE-SCREEN-001~013)와 흐름 인덱스(FLOW-001~009)를 안드로이드 내비게이션 구조로 대응합니다. 그래프 구조·인증 게이트·메인 탭 셸·화면 전환은 아래에서 확정합니다. 외부 진입은 푸시 알림 탭만 확정됐고(§3-3-4 알림 수신·탭 진입 — `:navigation`의 `PushEntry`가 라우트 매핑을 소유), App Link·공유 URL은 FE-SCREEN-012 결정과 함께 다룹니다.

### 로그인 화면 (앱 전용)

앱은 로그인 필수이므로(§3-3-1) **첫 실행 경험이 로그인 화면입니다.** 이 화면은 3-1의 화면 인벤토리에 없는 **앱 전용 화면**이며, 웹 `/login`의 계약([`3-1-client-spec.md`](3-1-client-spec.md#3-1-3-화면별-스펙) FE-SCREEN-008) 중 다음만 가져옵니다.
...
| --- | --- |
| 탭 순서·이름 | 홈 · 채팅 · 제작 · 마이 |
| 헤더 구성 | 홈은 마냑 로고 락업만, 나머지 탭은 현재 섹션 이름만 표시 — 브랜드가 홈의 이름을 대신하고, 로고는 지금 어디인지 알려 주지 않습니다 |
| 탭 아이콘 | 선택은 filled, 비선택은 outline |
| 탭 라벨 | 아이콘 아래에 이름을 표시 |
| 뒤로가기 | 탭 목적지에서 시작 탭(홈)으로, 홈에서 앱 종료 |
...
**내 스토리 카드는 표지 뱃지가 아니라 메타 줄이 좋아요 수와 누적 턴 수를 말하고, 제작일이 그 옆에 붙습니다 — 앱 전용 차이.** 표지가 커지며 우하단 뱃지가 그림을 가리므로, 채팅 목록 카드처럼 오른쪽 끝 메타 줄에 좋아요 수·누적 턴 수·제작일을 이 순서로 둡니다. 좋아요 수는 아웃라인 하트 아이콘과 함께 턴 수 왼쪽에 놓고 0 이나 필드 누락도 0 으로 표시합니다 — 웹 카드의 메타 순서와 같습니다. 제작일은 목록 응답 `createdAt`의 KST 기준 날짜(`YYYY-MM-DD` — [`3-1-client-spec.md §3-1-7` 날짜 표시 기준](3-1-client-spec.md#날짜-표시-기준))이고 형식이 다르거나 값이 없으면 그 칩만 빠집니다 — 스토리 상세의 생성일과 같은 변환을 씁니다. 웹 카드에는 없는 표시입니다.
...
FE-SCREEN-003(스토리 상세)의 안드로이드 구조입니다. 본문 항목·문구·API 계약의 정본은 [`3-1-client-spec.md §3-1-3`](3-1-client-spec.md#3-1-3-화면별-스펙)이고, 이 절은 앱이 그 계약을 어떤 화면 구조로 충족하는지와 웹과의 차이만 정합니다. 상세는 셸을 두르지 않는 전체 화면이며(위 메인 탭 셸), 홈·제작 두 목록의 카드 탭이 유일한 진입점입니다.
...
| 요소 | 앱 |
| --- | --- |
| 헤더 | 왼쪽 뒤로가기 아이콘 버튼 하나. 본문 위에 얹혀 있어 표지 위에서는 배경 없이 흰 아이콘으로 시작하고, 표지가 헤더 뒤로 지나간 만큼 배경이 차올라 표지가 끝나는 지점에서 제 배경과 아이콘 색을 되찾음. 본문 제목이 헤더 뒤로 다 지나가면 헤더가 제목을 이어받음 |
| 썸네일 히어로 | 3:4 비율. `thumbnailUrl`(원본)이 있으면 이미지, 없으면 placeholder 아이콘. 우하단에 좋아요 수(`likeCount`)·누적 턴 수 뱃지를 왼쪽부터 나란히(천 단위 콤마, 누락 값은 0). 목록 카드와 같은 표지 컴포넌트를 쓰되 축소본이 아니라 원본을 받음. **좌우 여백과 모서리 없이 화면 폭을 꽉 채우고 상태바·헤더 뒤까지 올라가며**, 아래 경계는 옅은 선 한 줄. 본문의 나머지 항목만 좌우 여백을 가짐 |
| 이미지 뷰어 | 이미지가 있을 때만 히어로를 누를 수 있고, 전체 화면 오버레이로 열림(아래 이미지 뷰어). 핀치·더블탭으로 확대 |
| 제목·한 줄 소개 | 항상 표시 |
| 장르 뱃지 | 접지 않고 줄바꿈으로 모두 표시 |
| 본 엔딩 뱃지 | `reachedEndings`가 비어 있지 않을 때 엔딩 이름 뱃지 |
| 주요 내용 | `description`이 있을 때 |
| 주변 인물 | `characters[]` 의 이름과 이미지. 이미지는 채팅방 인물 이미지와 같은 컴포넌트(아래 주변 인물 문단) |
| 채팅 시작 상황 | 상황 이름·상황 설명·엔딩 세 갈래. 이름은 드롭다운 셀렉트로 고르고 설명과 엔딩이 따라 바뀜(아래 시작 설정 선택·엔딩 목록). 프롤로그·추천 입력은 상세에 표시하지 않고 채팅 화면이 노출 |
| 제작자·생성일 | 본문 마지막에 딸린 메타 정보라 화면 폭을 채우는 옅은 바탕 하나 위에 두 줄로 둠. 제작자는 `author.nickname`, 생성일은 `createdAt` 의 KST 기준 날짜 |
| 하단 CTA | 본문 위에 떠 있는 "채팅 시작하기" 버튼과 버튼 위쪽 본문 페이드. **응답 `isOwner`가 거짓인 상세에만** 그 왼쪽에 같은 높이(48dp)의 정사각 좋아요 아이콘 버튼을 16dp 띄워 둠 — 배경 없이 하트의 모양과 색만으로 상태를 말함(기본 아웃라인 + `{colors.text}`, 누른 뒤 filled + `{colors.text-danger}`) |
...
**채팅 시작은 상세 위에 채팅방을 쌓습니다**(`MUST`). CTA를 누르면 `POST /chats {storyId, startSettingId}`로 채팅을 만들고 채팅방 목적지를 push해 백스택이 `[메인 탭, 상세, 채팅방]`이 됩니다. **채팅방에서 뒤로가기는 상세로 돌아오고, 한 번 더 누르면 진입한 탭으로 나옵니다**(`MUST`) — 웹 `replace`와 갈리는 앱 전용 차이입니다. **진행 중에는 버튼 크기와 접근성 이름을 유지한 채 라벨 자리를 스피너로 바꾸고 잠급니다**(`MUST`) — 퍼널 헤더 임시 저장 버튼과 같은 표현입니다. 실패는 CTA 위 인라인 문구로 알리고 버튼을 되돌려 다시 누를 수 있게 합니다.

### 스토리 생성 퍼널 구조 — 간편 제작

FE-SCREEN-002(간편 제작)의 안드로이드 구조입니다. 단계 구성·키워드 선택 규칙·검증·문구·API 계약의 정본은 [`3-1-client-spec.md §3-1-4`](3-1-client-spec.md#3-1-4-스토리-생성-퍼널)이고, 이 절은 앱이 그 계약을 어떤 화면 구조로 충족하는지만 정합니다. 퍼널 화면은 셸을 두르지 않는 전체 화면입니다(위 메인 탭 셸).

**단계마다 별도 목적지를 둡니다.** 키워드·스토리라인·추가 정보는 각각 NavKey 목적지입니다. 스토리라인이 키워드를 replace하고 추가 정보가 그 위에 쌓입니다. 완성 요청 영속 후에는 퍼널을 닫으며 완료 로딩 목적지를 두지 않습니다. 키워드 카테고리 탭은 화면 로컬 상태입니다.
...
**단계 간 공유 상태는 `:create`의 생성 스토어가 담습니다.** 키워드 화면이 시작한 스토리라인 생성을 뒤 단계 화면들이 관찰하도록, Activity 보존 범위의 스토어가 생성 상태(진행·결과·실패)와 재생성 체인(직전 `requestId`)을 담고 각 단계 ViewModel이 읽습니다. 라우트에는 규칙대로 선택 스토리라인 순번만 실립니다. 스토리라인·완성 요청의 `requestId` 멱등 계약(실패 재시도·같은 페이로드 재시도의 재사용)은 3-1 그대로 따르고, **요청 영속·복구 조회·임시 저장도 구현되어 있습니다** — 프로세스 재시작·재개 진입으로 스토어가 비면 진행 레코드에서 복원해 복구 폴링에 합류합니다(§3-3-5 간편 제작 라이프사이클).

**완성 제출은 제작 탭으로 돌아갑니다.** 완성 버튼은 요청을 영속하고 앱 스코프 실행자에 넘긴 뒤 퍼널을 걷어내고 제작 탭을 선택하며, 서버 응답을 기다리거나 채팅을 만들지 않습니다. 결과는 제작 탭의 완성 중 카드가 받습니다. 아래 문단은 그 이전(PR #5 기준) 동작 기록입니다.

### 채팅 목록 — 이어가기

FE-SCREEN-004(채팅 목록)의 안드로이드 구조입니다. 카드 항목·문구·상태 계약의 정본은 [`3-1-client-spec.md §3-1-3`](3-1-client-spec.md#3-1-3-화면별-스펙)이고, 이 절은 앱이 그 계약을 어떤 화면 구조로 충족하는지와 웹과의 차이만 정합니다. 채팅 목록은 셸을 두르는 탭 목적지 넷 중 하나이며(위 메인 탭 셸), 카드 탭이 이어가기의 유일한 진입점입니다.
...
**회원 채팅 목록 변환.** 서버의 항목 순서를 유지하고 빈 미리보기와 삭제된 참조 스토리는 정상 항목으로 취급합니다. DTO 변환은 기본값과 표시용 fallback을 제공하며 빈 문자열만으로 채팅을 제거하지 않습니다. 사용자 문구와 수용 기준은 Android 스펙을 따릅니다.
...
| 요소 | 앱 |
| --- | --- |
| 표지 | 왼쪽에 3:4 축소 썸네일(`thumbnailUrlSm`, 폭 52dp). 없으면 placeholder 아이콘. 목록 카드와 같은 표지 컴포넌트를 폭만 줄여 쓰고 옅은 테두리도 그대로(위 스토리 목록 절). **표지 위 턴 수 뱃지는 얹지 않습니다** — 이 카드에서는 아래 메타 행이 그 값을 맡고, 이만한 표지 위에서는 뱃지가 표지를 거의 다 덮습니다 |
| 스토리 제목 | 1줄, 넘치면 말줄임. **참조 스토리가 삭제돼 제목이 비면 그 자리에 옅은 글자색으로 "삭제된 스토리예요"**  |
| 마지막 장면 미리보기 | 1줄, 넘치면 말줄임. `lastStoryPreview`가 빈 문자열이면 "채팅을 시작하고 이야기를 이어가 보세요"를 옅은 글자색으로 |
| 메타 | 오른쪽 정렬 한 행에 턴 수(말풍선 아이콘)와 마지막 활동 시각(달력 아이콘) |
| 옵션 메뉴 | 더보기·길게 누르기로 카드 옵션 다이얼로그 표시 |
| 진입 | 카드 전체가 채팅방 진입 |
...
**페이징은 서버 계약이 열릴 때까지 두지 않습니다.** `GET /users/me/chats`는 `limit`(`[1, 100]`으로 clamp)만 받고 커서도 오프셋도 없습니다 — 은 페이지네이션 없이 상한 100을 유지하고 초과분은 스토리 피드 설계와 함께 확장합니다([`4-backend.md §4-3-5`](./4-backend.md)). 앱은 `limit`을 싣지 않고 서버 기본값을 쓰며, 채팅이 100건을 넘으면 최근 100건만 보입니다. 내 스토리 목록과 같은 잔여이고, 서버가 확장할 때 두 목록을 함께 붙입니다. **더 받을 것이 있는 것처럼 보이는 하단 로딩 자리표시자를 미리 두지 않습니다**(`MUST NOT`) — 끝난 목록에 끝나지 않은 표시를 그리는 셈입니다.
...
FE-SCREEN-005(채팅 화면)의 안드로이드 구조입니다. 턴 모델·SSE 이벤트·입력 규칙의 정본은 [`3-1-client-spec.md §3-1-5`](3-1-client-spec.md#3-1-5-채팅-플레이와-sse-스트리밍)이고, 이 절은 **웹 현행 구현을 읽어 확인한 값과 규칙을 앱이 어떤 구조로 충족하는지**와 웹과의 차이만 정합니다. 채팅방 진입과 상세 조회 렌더는 이미 구현했고(위 스토리 상세·생성 퍼널 절), 이 절은 그 위에 얹는 입력·스트리밍·선택지 범위를 다룹니다.
...
**URL 허용 검사는 렌더 직전에 한 번 더 합니다**(`MUST`). `https` · 사용자·비밀번호·포트 없음 · 호스트가 `cdn.manyak.app`·`dev-cdn.manyak.app` 중 하나 · 경로가 `/characters/generated/` 또는 `/characters/originals/`로 시작하고 그보다 길 것. 사용자 생성 스토리는 `generated`, 오리지널 스토리는 `originals` 경로를 씁니다. 하나라도 어긋나면 이미지 조각을 만들지 않습니다 — 저장 본문에서는 마커 줄이 평문으로 남으므로, 경로 하나가 빠지면 오리지널 스토리 채팅에 `[[URL]]` 글자가 그대로 보입니다.
...
인물 이미지와 배경 이미지 마커는 별개입니다. 배경 이미지의 승인·적용 상태는 [추적 문서](../planning/client-tracking.md#계약-확인-항목)를 확인합니다.
...
- **잠긴 입력창을 누르면 토스트로 이유를 알립니다**. 문구는 "응답을 만드는 중이에요. 끝나면 입력할 수 있어요"이고 일반·블럭 모드의 입력 칸 모두에 적용됩니다. 잠긴 칸은 터치를 받지 않아 눌러도 아무 반응이 없었는데, 비활성 색만으로는 왜 안 되는지와 언제 되는지가 드러나지 않았습니다. 추가·삭제·전송 버튼은 비활성 표시가 곧 잠금이라 따로 알리지 않습니다.
- 전송 버튼 아이콘은 세 갈래입니다.
...
종료 처리는  범위와 같습니다 — 방을 떠나면 조용히 중단, `error`·네트워크 실패는 안내 후 낙관적 밴드 제거, `completed`·`error` 없이 끝난 EOF는 실패로 확정하고 상세를 다시 읽습니다. 자동 재시도·자동 재연결은 없습니다.
...
- **채팅 공유(발급·열람)** — 웹은 헤더 공유 버튼으로 공유 URL을 만들고 그 URL을 여는 화면(FE-SCREEN-012)을 따로 두지만, **앱은 지금 범위에서 둘 다 만들지 않습니다**. 공유 URL을 앱 딥링크로 열지 웹으로 넘길지가 정해지지 않았고, 그 결정 없이 발급만 붙이면 앱에서 만든 링크를 앱이 열지 못합니다. 다시 다룰 때는 와 함께 판단합니다.
...
마이 탭 화면입니다([`3-1-client-spec.md`](3-1-client-spec.md) FE-SCREEN-008 마이 페이지 회원 상태 구성). 앱은 회원만 진입하므로 게스트 로그인 버튼·이관 안내는 비적용입니다(§3-3-1). 섹션 구성은 웹을 따르되 알림 섹션이 하나 더 있습니다 — 프로필 헤더 · 이프 카드 · 이벤트(친구 초대) · 화면(테마 변경) · **알림** · 기타 · 계정(로그아웃·회원 탈퇴). **기타 섹션의 항목과 순서는 앱이 갈립니다** — 피드백 · 서비스 안내 · 오픈소스 라이선스 · 버전 정보 순이고, 웹은 서비스 안내가 피드백 위이며 뒤 둘이 없습니다. 오픈소스 라이선스와 버전 정보는 배포 형태가 있는 앱에만 필요한 항목이라 웹에 대응 화면이 없습니다.
...
**이프 카드** — 잔액의 정본은 프로필(`GET /auth/me`의 `creditBalance`)이고 화면 진입 시 한 번 다시 읽습니다. **카드에는 잔액 오른쪽에 이프 충전으로 가는 강조색 버튼 "충전" 하나만 둡니다** — 이프를 얻는 수단은 모두 이프 충전 화면이 소유하므로 마이에는 그 화면으로 가는 입구만 남깁니다. 출석 체크 버튼과 "내역" 버튼은 카드에서 빼고 이프 충전으로 옮깁니다(아래 이프 충전 절).
...
**이프 충전(FE-SCREEN-008 이프 충전)** — 구성·문구·데이터는 공통 계약을 따르고, 아래는 앱에서만 정해지는 부분입니다. 기존 이프 내역 화면을 두 탭으로 넓힌 것이라 **새 목적지를 만들지 않고 같은 목적지를 고칩니다** — 진입 버튼만 "내역"에서 "충전"으로 바뀝니다.
...
**알림 설정(앱 전용)** — . 화면 섹션 아래 **알림 섹션의 "알림 설정" 항목**(벨 아이콘·오른쪽 화살표)이 진입점이고, 셸 없이 마이 위에 쌓이는 전체 화면입니다(`detail-header` "알림 설정"). 웹에 대응 화면이 없어 공통 계약이 없고, 서버 계약은 [`4-backend.md §4-3-5`](./4-backend.md#4-3-api-계약) 푸시 수신 동의(`GET`·`PUT /users/me/push-settings`)가 소유합니다. 코드는 `notification` 모듈의 `settings` 패키지가 소유합니다 — 수신 동의는 알림 기능의 일부이고, `my`에 두면 알림을 모르는 모듈이 알림 API를 갖게 됩니다([모듈 아키텍처 §5](../planning/android-module-architecture.md)).
...
등록은 `POST /users/me/invite/redeem` 이고 실패 사유를 입력창 아래 인라인으로 구분합니다 — 빈 값 · 400·404 "코드를 다시 확인해주세요" · 409 `INVITE_SELF_CODE` · 409 `INVITE_ALREADY_REDEEMED` (문구는 공통 계약 FE-SCREEN-008 과 같음). 성공은 토스트 뒤 **프로필 재조회로 잔액을 반영합니다**(`MUST`, 위 이프 카드와 같은 이유). **입력창에 자동 초점을 주지 않습니다**(`MUST NOT`) — 진입과 동시에 뜨는 시트라 키보드가 안내와 버튼을 가립니다. 자판은 `Ascii` 로 유도하고 입력값에서 영문·숫자만 남깁니다 — 한글 IME 가 먼저 뜨는 기기가 있어 웹의 공백 제거·대문자화만으로는 코드에 없는 문자가 통과합니다(앱 전용 보강).

**가입 보상에는 전용 안내를 두지 않습니다**. 가입 시 지급되는 이프는 서버가 계정 생성과 함께 적립하므로, 앱은 이프 카드의 잔액으로만 드러냅니다 — 전용 안내를 따로 띄우면 잔액과 두 곳에서 같은 사실을 말하게 되고 중복 노출을 막을 상태를 하나 더 들어야 합니다. **신규 가입 직후 띄우는 것은 초대 코드 안내 하나뿐이며 웹도 같습니다.** 사용자가 직접 한 행동(출석·초대 코드 등록)의 결과만 토스트로 알립니다.

**친구 초대(FE-SCREEN-008 친구 초대)** — 마이와 이프 충전의 진입 줄은 "친구 초대" 라벨 아래에 작은 강조색 보조 문구 "하고 N 이프 받기"를 붙여 라벨과 이어 한 문장으로 읽히게 합니다. 화면 자체의 레이아웃·문구·간격은 웹(`manyak-web/src/features/my/invite`)을 그대로 옮깁니다. 내 코드 카드(조회 중 골격 · 실패 시 "초대 코드를 불러오지 못했어요"와 다시 시도하기 · 이번 달 보상 진행) · 복사·공유 버튼 쌍 · 받은 코드 등록 · 이용 안내 네 줄 순서입니다. 코드와 보상 진행은 `GET /users/me/invite` 이고 월 상한은 정책 수치라 응답이 싣는 값을 그대로 씁니다(화면이 갖지 않습니다). 등록은 `POST /users/me/invite/redeem` 이며 **신규 가입 안내 시트와 같은 실패 문구 계약을 공유합니다**(위 초대 코드 안내 문단).
...
**이메일 형식은 앱이 직접 검증합니다**. 웹은 `input[type=email]` 의 브라우저 기본 검증이 이 자리를 맡지만 앱에는 대응이 없어, 형식이 어긋난 주소가 그대로 서버까지 가서 `400` 으로 돌아왔고 화면에는 "피드백 전송에 실패했어요" 토스트만 떠 고칠 곳이 이메일 한 칸이라는 것을 알 수 없었습니다. 보내기 전에 형식(공백 없는 `로컬@도메인.최상위`)을 보고 그 칸 아래에서 "이메일 형식을 확인해주세요"로 안내합니다. 브라우저 검증과 같은 수준까지만 좁힙니다 — 실제로 쓰이는 주소를 앱이 막는 쪽이 서버가 거절하는 쪽보다 나쁩니다. 비어 있으면 선택 입력이므로 검사하지 않습니다.

**회원 탈퇴(FE-SCREEN-008 회원 탈퇴)** — 확인 네 항목·문구·CTA 상태는 웹([`3-1-client-spec.md`](3-1-client-spec.md) FE-SCREEN-008 회원 탈퇴)과 같습니다. 체크박스만이 아니라 **줄 전체가 토글**이라 문구를 눌러도 켜지고, 최소 터치 타깃도 그 줄이 확보합니다. 넷을 모두 켜야 위험색 CTA 가 열립니다.
...
**계정 섹션 — 로그아웃은 기본색, 회원 탈퇴가 위험색입니다**. 앱은 로그아웃을 `{colors.text}` 기본색으로 두고 그 아래 회원 탈퇴 항목을 `{colors.text-danger}`로 둡니다 — 위험색은 되돌릴 수 없는 동작(탈퇴)에만 남깁니다. 로그아웃 진행 중에는 라벨을 "로그아웃하는 중"으로 바꾸고 스피너를 표시하며 항목을 잠급니다. **앱이 선행한 배치였고 공통 계약이 뒤따라 같은 배치를 담았습니다**( — [`3-1-client-spec.md`](3-1-client-spec.md) FE-SCREEN-008 마이 페이지 회원 상태 구성).

**서비스 안내(FE-SCREEN-011)** — 별도 네이티브 화면·모듈을 만들지 않고 약관·개인정보처리방침과 같은 웹 문서 화면(`:legal`)으로 `{WEB_BASE_URL}/about`을 엽니다. 본문 정본이 웹 한 곳이라 앱이 옮겨 적은 문구가 웹과 갈라질 수 없습니다 — FE-SCREEN-010과 같은 논거입니다. **이 화면은 더 이상 이프 수치를 고지하지 않습니다** — 수치를 모아 보여주는 자리를 두면 각 사용 지점과 이중으로 관리해야 해서 절 자체가 없어졌고, 수치는 각 소모·적립 지점이 서버 정책에서 직접 읽습니다(§3-3-4 이프 정책 수치 패칭). 웹이 새 브라우저 탭으로 여는 것에 맞춰 메뉴 트레일링에 외부 열림 아이콘을 둡니다. 경로는 `/about` 으로 확정됐고( — `dev` 에 병합된 [`3-1-client-spec.md`](3-1-client-spec.md) FE-SCREEN-011), **배포만 남았습니다** — 배포 전까지 앱에서는 404 페이지가 보입니다. **웹 잔여** — 앱 WebView는 미로그인 상태로 페이지를 열므로, "게스트 이용 안내(웹 전용)" 섹션을 앱 노출에서 숨기는 판정(3-1 검수 기준)은 웹이 소유합니다.

## 3-3-4. API 연동·인증 세션

API 사용 계약([`3-1-client-spec.md §3-1-7`](3-1-client-spec.md#3-1-7-api-연동에러-처리-계약))을 따릅니다. 백엔드는 직접 호출하며 중간 계층을 두지 않습니다(§3-3-2). 논리적 식별자(`device_id`·`session_id`·`user_id`)의 의미·타입과 `X-Manyak-*` 헤더 계약의 정본은 [`6-analytics.md §6-2`](./6-analytics.md)이고, 이 절은 **앱의 매핑과 인증 세션 동작**을 확정합니다.
...
- **계정 연동 경로의 403·409는 세션 만료로 처리하지 않습니다**(`MUST NOT`, [`3-1-client-spec.md`](3-1-client-spec.md#3-1-3-화면별-스펙) FE-SCREEN-008). 연동 실패로 로그인 세션이 풀리면 안 됩니다. 재발급은 401에만 반응하므로 자연히 만족하지만, 인터셉터에서 403을 일괄 처리하지 않습니다.
...
| 4 | UserScopedStore 목록의 프로필·초대 안내·표시 알림을 정리합니다. 제작 두 테이블은 삭제 대상이 아니며 ownerId로 격리합니다 | 남으면 다음 사용자에게 이전 회원의 내용이 보입니다 |
...
### 푸시 토큰 등록
앱이 FCM 등록 토큰을 발급받아 서버에 맡기고, 로그아웃 때 이 기기의 토큰을 지우는 계약입니다. 서버 계약(`PUT`·`DELETE /users/me/push-tokens`, 멱등 upsert·회원당 기기 10대·같은 토큰 재등록 시 소유자 이전)은 [`4-backend.md §4-3-5`](./4-backend.md#4-3-api-계약) 디바이스 푸시 토큰이 소유합니다. 알림 표시·채널·탭 진입은 , 수신 동의 설정 화면(`push-settings`)은 가 소유합니다. 가 반드시 만족해야 할 이전 계정 알림 차단 조건은 이 절에 둡니다. 엔드포인트·본문·상태 코드는 [dev Swagger](https://dev-api.manyak.app/v3/api-docs)로 대조했습니다.

**스택과 소유.** Firebase Cloud Messaging은 Crashlytics와 같은 BOM·프로젝트를 사용합니다. `notification`은 FirebaseMessagingService·등록기·토큰 API·권한 요청·수신·표시·수신 동의를 소유합니다. app은 등록기 종료·토큰 삭제 계약을 호출하고 권한 요청 컴포저블을 조립합니다. google-services 플러그인은 app에 둡니다. 모듈 의존 규칙은 [모듈 아키텍처](../planning/android-module-architecture.md)가 소유합니다.
...
**이전 계정 알림 차단 —  필수 완료 조건.** 오프라인·5xx·인증 만료·프로세스 종료·늦은 `PUT`으로 토큰이 남을 수 있습니다. 같은 토큰을 다음 회원이 등록하면 소유자가 옮겨가지만, 그 등록이 실패하거나 이전 회원 대상으로 이미 발송한 메시지가 늦게 도착할 수도 있습니다. 따라서 현재 세션이 `회원`이라는 검사나 토큰 등록 성공만으로 수신자를 판정하지 않습니다.
...
- 로그아웃·세션 무효화·탈퇴에서 이미 표시한 계정 알림과 대기 중인 표시 작업을 정리합니다. 알림 탭 진입도 수신자·대상 접근 권한을 다시 검증합니다. 표시·정리·탭 진입의 구체 배선은 가 소유합니다.
- **수신자 판별 계약은 아래 알림 수신·탭 진입 절이 소유합니다** — 서버 발송 모듈이 모든 페이로드에 싣는 공통 키 `recipientId`([`4-backend.md §4-3-5`](./4-backend.md#4-3-api-계약) 푸시 발송 모듈)를 프로필 캐시와 비교하고, 없거나 다르면 버립니다.

**알림 권한(Android 13+ `POST_NOTIFICATIONS`).** 메인 그래프가 `회원` 상태로 처음 그려질 때 **설치당 한 번** 시스템 권한 요청을 띄웁니다. "물었음" 표시는 테마 설정과 같은 **기기 귀속 DataStore**에 두어 로그아웃 정리 대상이 아니며, 이미 로그인된 채 업데이트를 받은 회원도 첫 실행에서 한 번은 묻습니다. 이미 허용된 상태여도 표시를 남겨 나중에 사용자가 시스템에서 끄더라도 되묻지 않습니다. 사전 설명 화면과 거부 뒤 재요청 UI는 두지 않습니다 — 이후 재진입은 알림 설정 화면의 기기 알림 배너가 시스템 설정으로 보냅니다. API 32 이하는 런타임 권한이 없어 요청하지 않습니다. **권한 거부는 등록·세션·화면 어느 것에도 영향을 주지 않습니다**(`MUST`).

### 알림 수신·탭 진입
서버의 data 전용 메시지([`4-backend.md §4-3-5`](./4-backend.md#4-3-api-계약) 푸시 발송 모듈)를 앱이 받아 시스템 알림으로 표시하고, 탭하면 해당 화면으로 들어가는 계약입니다. 코드는 `notification` 모듈(수신·검증·표시·정리), `:navigation`(페이로드 → 라우트 매핑), `app`(진입 인텐트 해석·보류·소비)이 나눠 갖습니다.

**수신자 검증**(`MUST`) — 위 "이전 계정 알림 차단" 조건의 구체 계약입니다.

- 서버 발송 모듈이 모든 페이로드에 싣는 공통 키 `recipientId`(수신 회원 `public_id`, [`4-backend.md §4-3-5`](./4-backend.md#4-3-api-계약) 푸시 발송 모듈)를 프로필 캐시(`GET /auth/me`의 `id`, §3-3-4 프로필 조회와 캐시)와 비교합니다. **`recipientId`가 없거나 다르면 표시하지 않습니다.** 키가 빠진 메시지는 버려지므로 서버 회귀가 나도 이전 계정 알림이 새지는 않습니다.
...
**표시 규칙.** data 전용 메시지는 포그라운드·백그라운드 모두 앱 코드가 받으므로 **두 상태에서 같은 시스템 알림**을 띄웁니다. 앱 안 배너·스낵바는 두지 않습니다.
...
**탭 진입 → 라우트**(`MUST`). 알림·App Link·공유 인텐트가 내부 라우트로 바뀌는 자리는 `:navigation`의 `PushEntry` 하나입니다(내비게이션 구조 참조). 알림은 `type`·대상 식별자·`recipientId` 세 값만 인텐트 extra로 실어 `MainActivity`(`singleTop`, `CLEAR_TOP`)를 명시적으로 엽니다.
...
- **미로그인이면 로그인 뒤 목적지를 복원합니다.** 보류 진입은 `SavedStateHandle`에 있어 로그인 화면에서 외부 제공자로 나갔다가 프로세스가 죽어도 남습니다. 세션이 `회원`이고 프로필 캐시가 있을 때만 해석하며, **`recipientId`가 프로필 `id`와 다르면 홈으로 보냅니다** — 탭 시점의 수신자 재검증입니다. 웹의 로그인 `callbackUrl`과 같은 "보류 → 로그인 뒤 복귀, 검증 실패는 기본 목적지" 규칙이고, 값 형태만 라우트 키입니다.
...
- **재인증에 쓸 ID 토큰은 캐시가 아니라 새로 발급받습니다**(`MUST`). Credential Manager는 유효기간이 남은 ID 토큰을 캐시에서 그대로 돌려줍니다 — **31분 전에 발급된 토큰을 재사용하는 것을 실측했습니다**. 로그인은 `iat`를 보지 않아 통과하지만 재인증은 10분 기준에 걸려 403 `REAUTH_FAILED`가 되고, 화면에는 일반 실패로만 보입니다. 요청에 **nonce를 실어** 캐시를 우회합니다 — nonce는 토큰 클레임에 실려야 하므로 캐시된 토큰으로는 만족할 수 없고 새 `iat`로 재발급됩니다(같은 조건에서 `age=0s` · 201 확인). 서버는 이 클레임을 검사하지 않습니다. 제공자 어댑터는 로그인용(캐시 허용)과 연동용(재발급 강제) 두 경로를 나눠 노출하고, **연동은 재인증과 대상 인증 모두 재발급 경로를 씁니다** — 대상 토큰도 캐시에서 오면 만료된 토큰을 제출하게 됩니다.
- **재인증 본문의 provider는 경로 파라미터와 표기가 다릅니다** — `POST /auth/links/{provider}`의 경로는 소문자(`kakao`)지만, `POST /auth/links/reauth`의 본문 `provider`는 서버 enum 이름(`GOOGLE`·`KAKAO`)입니다. 소문자로 보내면 400이고 화면에는 일반 실패로만 보여 원인을 찾기 어렵습니다.
...
### 이프 정책 수치 패칭
수치 출처와 표시 규칙은 [`3-1-client-spec.md §3-1-7` 이프 정책 수치 표시](3-1-client-spec.md#이프-정책-수치-표시)가 소유하고, 웹 구현은 [`3-2-web-design.md`](3-2-web-design.md) §3-2-2 이프 정책 수치 패칭입니다. 이 절은 앱이 그 계약을 어떻게 충족하는지만 적습니다.
...
현재 빌드 기준은 `minSdk` 24입니다. 제품 지원 선언·전체 접근성·최대 글자 크기와 가로 레이아웃의 미확정 범위는 [추적](../planning/client-tracking.md#계약-확인-항목)이 소유합니다.
...
방향·다크 모드·글자 크기·표시 크기·로케일·창 크기가 바뀌면 Activity 가 재생성되고 컴포지션이 통째로 다시 만들어집니다. **앱은 재생성을 그대로 수용하고 상태 복원만 책임집니다.**
...
**화면 전체 상태를 `SavedStateHandle`에 일괄 저장하지 않습니다.** 루트의 알림 보류 진입은 `SavedStateHandle`로 복원합니다. 제작 입력은 Room, 화면 재조회는 라우트 식별자가 맡습니다. ViewModel 상태는 구성 변경까지만 보장하고, 프로세스 종료 복원은 이미 있는 두 경로가 맡습니다 — 라우트 키에 실린 식별자로 화면이 다시 조회하고(§3-3-3 라우트 규칙), 제작 퍼널 입력은 진행 레코드가 복구합니다. 화면 로컬 상태의 기본은 `remember` 이고, **잃으면 사용자가 다시 입력해야 하는 값만** `rememberSaveable` 로 올립니다.
...
- 제출은 요청 행 삽입과 같은 생성 결과의 편집 행 삭제를 DAO 트랜잭션 하나로 처리합니다. 저장 실패면 전송하지 않습니다.
- 앱 스코프 `StoryCompletionExecutor`가 `SessionGate.withAuthWork/commit` 안에서 requestId별로만 합류합니다. 서로 다른 요청을 한 전역 순서로 직렬화하지 않습니다.
- POST 성공은 COMPLETED, 네트워크·401/403은 미확정으로 유지합니다. 그 밖의 응답(409 포함)은 복구 GET으로 판정하고 GET 404만 FAILED로 확정합니다. 사용자 새로고침에서 미접수 요청의 GET 404는 같은 명령 재전송으로 복구합니다.
- `StudioViewModel.drivePendingCompletionPolling`은 STARTED와 완성 중 카드 노출 조건에서만 5초 간격으로 조회합니다. 스토리라인 복구의 3초 간격과 별개입니다.
- 완료 요청은 서버 목록에 같은 `storyId`가 있으면 지우고, 없으면 목록을 한 번 더 조회하는 동안 실제 제목의 완료 카드로 유지합니다.
- v1→v2는 기존 `STORY_COMPLETION`을 PENDING 요청 행으로 옮깁니다. 해석하지 못한 행은 원본을 보존합니다. v2→v3는 빈 `ownerId`를 추가하며 이전 버전의 소유자 없는 행은 다음 회원에게 귀속합니다. 파괴적 fallback을 사용하지 않습니다.
- 실패 카드의 현재 렌더·행동과 완료 분석 이벤트 공백은 [추적의 잠정 구현](../planning/client-tracking.md#계약-확인-항목)에 기록합니다. 이 설계가 잠정 동작의 제품 승인을 대신하지 않습니다.
...
- 중복 저장은 ViewModel·생성 스토어가 직접 가진 쓰기 상태와 마지막 스냅숏으로 판정합니다. 같은 스냅숏이면 디스크 쓰기는 건너뛰고 저장 피드백만 제공합니다. 채널로 늦게 반영되는 UiState를 중복 방지 장벽으로 쓰지 않습니다.
- 키워드는 `KEYWORD_DRAFT`, 생성 요청은 `STORYLINE_GENERATION`, 결과 편집은 `STORY_DRAFT`입니다. 완성 제출은 별도 요청 테이블로 이동하며 편집 슬롯의 `STORY_COMPLETION`은 사용하지 않습니다.
- 초안 재개는 행을 소비하지 않습니다. 새 제작·초안 삭제는 대상 초안만 처리합니다. 로그아웃은 두 테이블을 삭제하지 않고 ownerId와 세션 세대로 격리합니다.
- 재개 스택은 키워드 초안이면 `[키워드]`, 생성 중 또는 선택 전 결과이면 `[스토리라인]`, 선택된 결과이면 `[스토리라인, 추가 정보]`입니다. 프로세스 재시작 시 비어 있는 생성 스토어를 단계 ViewModel이 감지해 Room 복원·조회에 합류합니다.
- 별도 재개 의도 저장소는 없습니다. 레코드가 남아 있는 진입은 재개, 새 제작은 초안 폐기 확인 뒤 진입입니다.
- 스토리라인 복구 Flow는 해당 화면이 STARTED일 때만 3초 주기로 수집합니다. 완성 결과 조회는 위 제작 카드의 수명을 따릅니다. WorkManager·Foreground Service를 사용하지 않습니다.
...
5. Crashlytics ANR은 API 30+만, NDK 수집은 미도입입니다. API 24~29 ANR은 Android vitals를 별도 확인하되 완전한 대체로 간주하지 않습니다([Android ADR](./3-3-android-adr.md)).
...
**P0 Crashlytics breadcrumb** `:analytics`의 `AmplitudeAnalytics`가 분석 이벤트를 발행하는 그 자리에서 `CrashReporter`로 이름을 넘겨 `FirebaseCrashlytics.log`에 남깁니다. 화면이 같은 사실을 두 번 알리지 않게 하려는 것입니다. `screen_name`만 지속 custom key로 두고 `story_id`·`chat_id`·`creation_id`는 breadcrumb 줄에만 싣습니다 — 지속 key로 두면 그 화면을 떠난 뒤 난 크래시에도 옛 식별자가 붙어 무관한 리포트를 그 스토리·채팅 탓으로 읽게 됩니다. 나머지 프로퍼티는 생략합니다. breadcrumb은 `device_id` 주입 대기열을 타지 않습니다 — 식별자가 붙기 전에 죽어도 직전 행동은 리포트에 남아야 합니다.

API non-fatal의 적용·검증 범위는 [추적 문서](../planning/client-tracking.md#계약-확인-항목)를 확인합니다.
...
사용자 수용 기준은 Android 스펙과 공통 스펙이 정본입니다. 아래 항목은 해당 계약을 확인하는 절차이며 정책을 독립적으로 변경하지 않습니다. CI·수동·릴리스 검증의 수행 여부는 추적 문서와 기능 계획에서 확인합니다.
...
인증·세션 계약을 아래 절차로 검증합니다. 실행 결과는 기능 계획 또는 PR에서 연결합니다.
...
자동화·수동 수행 범위와 계측 테스트의 CI 연결 여부는 [추적](../planning/client-tracking.md)과 [Android 검증 정책](../../../manyak-android/docs/plans/_project.md)을 확인합니다.
...
| 8 | 참조 스토리를 지운 뒤 그 채팅이 목록에 **남아 있고** 제목 자리에 옅은 글자색으로 "삭제된 스토리예요"가 나옵니다. 카드를 누르면 채팅방이 그대로 열립니다 | 제목이 비었다고 항목을 버리면 사용자가 이어가던 채팅에 닿을 길이 없어지고, 빈 제목을 그대로 그리면 무엇의 채팅인지 알 수 없는 줄이 남습니다 |
