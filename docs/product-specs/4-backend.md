...
- **모든 페이로드에 `recipientId` — `제안 · 서버 합의 대기`(KNK-1134, 2026-09-08).** `sendToUser`가 시나리오 데이터에 `recipientId` = 수신 회원의 `public_id`(`GET /auth/me`의 `id`와 같은 값)를 **한 곳에서** 덧붙입니다. 이미 `sendToUser`가 회원 행을 읽어 상태를 확인하므로 추가 조회 없이 `data + ("recipientId" to user.publicId.toString())`이면 됩니다. 앱은 이 값을 프로필 캐시의 `id`와 비교해 현재 회원이 수신자일 때만 표시하고, **값이 없거나 다르면 표시하지 않습니다**([`3-3-android-app.md §3-3-4`](./3-3-android-app.md#3-3-4-api-연동인증-세션) 알림 수신·탭 진입). 필요한 이유: 로그아웃 뒤 남은 토큰(오프라인·늦은 `PUT`)이나 이전 회원 대상으로 이미 발송된 메시지가 다음 로그인 회원 기기에 도착할 수 있고, 토큰 소유자 이전·삭제 성공만으로는 이를 막지 못합니다. 시나리오 키(`type` 등)와 충돌하지 않는 예약 키이며, 시나리오 구현은 이 키를 직접 싣지 않습니다. **서버 팀 합의 뒤 `Phase 3 · 구현`으로 바꾸고, 합의 전에는 앱의 알림 표시가 출시 불가 상태입니다**(앱은 `recipientId` 없는 메시지를 전부 버립니다).
