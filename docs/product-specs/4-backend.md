...
| `Phase 1 · 계획` | Phase 1 범위. 미구현, 방향만 합의됨 | 가입 프로필 이미지 랜덤 배정([§4-5](#4-5-인증과-권한)). 썸네일·채팅 이미지 삽입([§4-3-9](#4-3-api-계약)). 주요 사건·엔딩 런타임 반영과 도달 메타([§4-3-10](#4-3-api-계약)). 로어북 런타임 반영([§4-3-6](#4-3-api-계약)). 2026-07-07 결정분 — 재생성 이력 보존·보상 크레딧 만료·게스트 한도 회원 공유·시작 설정 복수화·와이어 개편·이관 1회 제한·게스트-회원 교차 접근 차단·체험 한도 축소([§4-8](#4-8-검수-체크리스트) B8·B10~B16). 2026-07-08 결정분 — 세션 부트스트랩 응답 확장·정지 계정 집행([§4-8](#4-8-검수-체크리스트) B17·B20) |
...
**`POST /chats`** — 요청 `{storyId: string}`. 스토리가 없으면 404. `Phase 1 · 계획`(B16) — `user_id`가 NULL인 스토리로는 익명(게스트) 요청만 채팅을 생성할 수 있고, 인증된 회원은 403입니다([§4-5](#4-5-인증과-권한)). `Phase 1 · 계획`(B14) — 요청에 `startSettingId`(선택, 미지정 시 첫 번째 시작 설정)를 받습니다([§4-3-8](#4-3-api-계약) 시작 설정 복수화). 응답(201):
...
**`GET /chats/{chatId}`** — 응답(`ChatDetailResponse`): `{id, storyId, storyTitle, prologue, turns[], suggestedInputs}`. `Phase 1 · 계획`(B16) — 채팅 상세는 플레이 기록이므로 소유권 규칙을 적용합니다: 소유 채팅은 소유자만, `user_id`가 NULL인 채팅은 익명(게스트) 요청만 조회할 수 있고 위반은 403입니다([§4-5](#4-5-인증과-권한)).
...
- **계정당 이관 1회 — `Phase 1 · 계획`(B15).** 요청에서 1건 이상 `MIGRATED`가 발생하면 그 시점에 계정을 잠급니다(`users.migrated_at` 기록 — [§4-4](#4-4-데이터-모델)). 성공 0건인 호출(빈 배열, 전부 `CONFLICT`·`NOT_FOUND` 등)은 잠그지 않으므로 이관 기회가 소진되지 않습니다.
- **동시 호출 직렬화 — `Phase 1 · 계획`(B15).** 같은 계정의 이관 호출은 `users` 행 잠금 또는 `migrated_at IS NULL` 조건부 갱신으로 직렬화합니다. 두 요청이 경합해도(중복 탭·재시도) 잠금·이관 결과는 순차 실행과 같아야 하며, 잠금 이후 진입한 호출은 닫힌 응답을 받습니다 — 직렬화가 없으면 동시 호출 2건이 모두 "잠기지 않은 계정"으로 판정되어 1회 제한이 뚫립니다.
- **닫힌 계정의 재호출 — `Phase 1 · 계획`(B15).** 잠긴 계정의 호출은 오류가 아니라 정상 흐름입니다(프론트엔드가 매 로그인마다 자동 호출). 서버는 제출 항목을 평가하지 않고 `migrationClosed: true`와 빈 결과로 200을 반환하며, 프론트엔드는 조용히 무시합니다.
...
- **영향.** `users`에 잠금 컬럼(`migrated_at`)이 추가되고 응답에 `migrationClosed` 필드가 추가됩니다. 프론트엔드 자동 호출 흐름은 변경 없이 유지됩니다. 교차 접근 차단(B16)·체험 한도 축소(B8)와 함께 파밍 경로를 닫습니다.
...
- 기준: `X-Manyak-Device-Id`별 누적 카운터 3종입니다. `storyline_generation`은 스토리라인 생성·재생성 합산 5회, `story_creation`은 스토리 간편 제작(컴파일) 1회, `chat_turn`은 모든 채팅방의 채팅 턴·AI 응답 재생성 합산 5회입니다. 일반 제작 등록은 제외합니다. 수치 축소는 `Phase 1 · 계획`입니다(현행 구현 기본값 10·3·15 — [§4-8](#4-8-검수-체크리스트) B8). 축소는 카운터 리셋 없이 적용합니다 — 이미 새 한도 이상을 쓴 기기는 즉시 한도 소진 상태입니다.
...
| 대안 | 채택 안 한 이유 |
| --- | --- |
| 기존 수치 유지(스토리라인 10 · 스토리 3 · 채팅 턴 15 — 2026-07-06 확정치, KNK-477) — 현행 구현 | 파밍 1건으로 확보되는 무료 사용량이 커서 이관·차단 강화 후에도 어뷰징 유인이 남습니다 |
| 축소와 함께 기존 카운터 리셋 | 리셋은 모든 기존 게스트에게 한도를 재지급해 축소 목적과 상충하고, 별도 마이그레이션 작업만 늘립니다 |

- **영향.** 서버 한도 기본값 수치 변경이 필요합니다. 카운터를 리셋하지 않으므로 기존 게스트 중 새 한도 이상 사용자는 즉시 402를 받습니다(수용 — 팀 인지 필요).
...
- 소유권: [§4-5](#4-5-인증과-권한) 규칙을 따릅니다. 회원 소유 스토리는 소유자만 수정할 수 있습니다. `user_id`가 NULL인 게스트 스토리는 `GET /stories/{storyId}/edit`와 `PATCH` 모두 익명(게스트) 요청만 허용하고 인증된 회원은 403입니다(`Phase 1 · 계획` B16). 서버는 게스트끼리는 구분할 수 없으므로, 프론트엔드가 로컬 서재에 해당 `storyId`가 있을 때만 수정 진입점을 표시합니다.
...
- 저장: `completed` 전에 새 AI 출력·선택지를 마지막 턴의 **활성본**으로 저장합니다(`Phase 1 · 계획` — 버전 보관 스키마, B11). 이전 출력·선택지는 버전 이력으로 남고, 상세 조회·SSE `completed`는 활성본만 싣습니다. 저장 트랜잭션에서 해당 턴이 여전히 마지막 턴인지 재확인하고, 아니면(스트림 중 새 턴 추가·동시 재생성 경합) 결과를 폐기하고 `error` 이벤트로 종료합니다 — 환불은 실패 규칙을 따릅니다. `turn_number`·사용자 입력은 변하지 않습니다.
...
- **영향.** 메시지 버전 보관 스키마로의 서버 전환이 필요합니다(B11). 사용자에게는 활성본만 노출되므로 프론트엔드 계약은 변하지 않습니다.
...
| 그룹 | 테이블 | 역할 |
| --- | --- | --- |
| 사용자 | `users` | 계정. `public_id`(UUID) · `nickname` · `profile_image_url`(nullable) · `profile_thumbnail_base64`(nullable, 목록·미리보기용 저해상도) · `status`. `Phase 1 · 계획` 컬럼 추가 — `migrated_at`(timestamp nullable) — 게스트 이관 1회 잠금. 이관 성공 시 기록([§4-3-5](#4-3-api-계약) B15) |
...
| NULL 리소스(`user_id` NULL) — 턴 진행·재생성·수정·NULL 스토리로 채팅 생성(`POST /chats`) | `Phase 1 · 계획`(B16) | 익명(게스트) 요청만 허용. 인증된 회원은 `403` |
| 채팅 열람(`GET /chats/{chatId}` · `POST /chats/batch`) | `Phase 1 · 계획`(B16) | 플레이 기록이므로 위 두 규칙을 동일 적용 — 소유 채팅은 소유자만, NULL 채팅은 게스트만. 상세는 위반 시 403, 배치는 열람 불가 항목을 오류 없이 제외([§4-3-3](#4-3-api-계약)) |
| `DELETE /stories/{storyId}` · `DELETE /chats/{chatId}` | `계획` | 위 두 규칙을 동일 적용해야 하나 아직 소유권 검증이 없습니다(익명·회원 무검증 삭제 잔존 — [§4-8](#4-8-검수-체크리스트) B9·B16) |
...
| 403 | `FORBIDDEN` | `Phase 1 · 구현(일부)` 소유자가 있는 리소스에 대한 타인·익명의 변경 시도, `Phase 1 · 계획`(B16) 인증된 회원의 NULL 소유 리소스 접근(플레이·변경·채팅 생성·채팅 상세 조회) — [§4-5](#4-5-인증과-권한). 삭제 계열은 미적용(B9) |
...
| B8 | 게스트 한도 우회·수치 축소 | 체험 한도가 디바이스 ID 헤더 기준이라 헤더 변조·기기 변경으로 우회 가능. 계약 수치는 5·1·5([§4-3-7](#4-3-api-계약))인데 현행 구현 기본값은 10·3·15로 축소 미반영. 디바이스 ID 회전은 Redis 카운터 키를 만료 없이 무한히 늘림 | 한도 구조는 `Phase 1 · 구현`, 수치 축소는 `Phase 1 · 계획`(결정 2026-07-07 — 카운터 리셋 없이 적용). 우회는 수용하고 관측(호출량 급증 알림·카운터 키 증가 추이)으로 추적, 강화(rate limit 등)는 후속 결정 |
...
| B11 | 재생성 이력 보존 | 계약은 이전 출력 보존 + 활성본 표시([§4-3-9](#4-3-api-계약)) — 재생성 원인 분석 목적. 현행 구현은 마지막 턴 교체(이전 본문 미보관)라 전환 필요 | `Phase 1 · 계획`(결정 2026-07-07) — 메시지 버전 보관 스키마로 서버 전환 |
...
| B15 | 게스트 이관 1회 제한 | 계약은 계정당 이관 1회(1건 이상 `MIGRATED` 시 잠금)·닫힌 계정의 `migrationClosed: true` 응답([§4-3-5](#4-3-api-계약)). 현행 구현은 효과 멱등·재호출 허용(2회차 `ALREADY_OWNED`)이라 전환 필요 | `Phase 1 · 계획`(결정 2026-07-07) — `users.migrated_at` 컬럼 추가와 잠금 게이트로 서버 전환 |
| B16 | 게스트-회원 교차 접근 차단 | 계약은 NULL 소유 리소스의 플레이·변경·채팅 생성·채팅 상세 조회를 게스트 전용으로 제한(회원 403)하고, 소유 채팅 상세 조회를 소유자 전용으로 강화하며, 채팅 배치 조회는 열람 불가 항목을 제외([§4-5](#4-5-인증과-권한)·[§4-3-3](#4-3-api-계약)). 현행 구현은 NULL 리소스에 익명·회원 모두 허용, 채팅 상세·배치 조회는 소유권 검증 없음 | `Phase 1 · 계획`(결정 2026-07-07, 배치 필터는 2026-07-08 추가) — 소유권 검증 확장으로 서버 전환, 프론트엔드 403 안내 후속 정렬 |
