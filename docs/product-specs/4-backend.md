...
- **리뷰·QA** — §4-8 검수 체크리스트와 간극 표(B1~B16)에서 시작해 본문을 역참조합니다.
...
| `Phase 1 · 계획` | Phase 1 범위. 미구현, 방향만 합의됨 | 가입 프로필 이미지 랜덤 배정([§4-5](#4-5-인증과-권한)). 썸네일·채팅 이미지 삽입([§4-3-9](#4-3-api-계약)). 주요 사건·엔딩 런타임 반영과 도달 메타([§4-3-10](#4-3-api-계약)). 로어북 런타임 반영([§4-3-6](#4-3-api-계약)). 2026-07-07 결정분 — 재생성 이력 보존·보상 크레딧 만료·게스트 한도 회원 공유·시작 설정 복수화·와이어 개편·이관 1회 제한·게스트-회원 교차 접근 차단·체험 한도 축소([§4-8](#4-8-검수-체크리스트) B8·B10~B16) |
...
| 채팅 상세 조회(`GET /chats/{chatId}`) | `Phase 1 · 계획`(B16) | 플레이 기록이므로 위 두 규칙을 동일 적용 — 소유 채팅은 소유자만, NULL 채팅은 게스트만 |
...
- `Phase 1` 소유권 검증: `user_id`가 설정된 스토리·채팅은 소유자만 삭제·턴 진행·채팅 상세 조회가 가능하고, 타인·익명 요청에 403을 반환해야 합니다. `user_id`가 NULL인 리소스는 익명(게스트) 요청만 허용되고, 인증된 회원의 턴 진행·재생성·채팅 상세 조회·채팅 생성·수정·삭제는 403이어야 합니다(B16). 스토리 공개 조회(`GET /stories/{storyId}`·배치 카드 조회)는 소유권과 무관하게 동작해야 합니다.
...
| B8 | 게스트 한도 우회·수치 축소 | 체험 한도가 디바이스 ID 헤더 기준이라 헤더 변조·기기 변경으로 우회 가능. 계약 수치는 5·1·5([§4-3-7](#4-3-api-계약))인데 현행 구현 기본값은 10·3·15로 축소 미반영 | 한도 구조는 `Phase 1 · 구현`, 수치 축소는 `Phase 1 · 계획`(결정 2026-07-07 — 카운터 리셋 없이 적용). 우회는 수용하고 관측(호출량 급증 알림)으로 추적, 강화(rate limit 등)는 후속 결정 |
...
| B16 | 게스트-회원 교차 접근 차단 | 계약은 NULL 소유 리소스의 플레이·변경·채팅 생성·채팅 상세 조회를 게스트 전용으로 제한(회원 403)하고 소유 채팅 상세 조회를 소유자 전용으로 강화([§4-5](#4-5-인증과-권한)). 현행 구현은 NULL 리소스에 익명·회원 모두 허용, 채팅 상세 조회는 소유권 검증 없음 | `Phase 1 · 계획`(결정 2026-07-07) — 소유권 검증 확장으로 서버 전환, 프론트엔드 403 안내 후속 정렬 |
