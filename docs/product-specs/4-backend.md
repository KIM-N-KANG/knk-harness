...
- **대상.** `ACTIVE` ∩ `marketing_push_agreed_at IS NOT NULL` ∩ 등록 토큰 보유. 회원마다 **발송 직전에 다시 읽어** `canReceiveMarketingPush(now)`를 재확인합니다(출석 리마인드와 같은 이유 — 회차 도중 철회 반영). **야간(21:00~08:00 KST)에 예약된 캠페인은 거부·연기하지 않고 야간 동의자에게만** 나갑니다 — 정책이 "야간은 별도 동의"이지 "야간 금지"가 아니고, 야간 동의자 대상 캠페인이 있을 수 있습니다. 건너뛴 수는 `skipped_count`로 남습니다.
