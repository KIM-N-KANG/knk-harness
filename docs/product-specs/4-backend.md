...
- **초대 어트리뷰션 윈도우** — 피초대자가 초대 URL로 진입한 뒤 **24시간** 안에 가입해야 초대로 인정합니다. 윈도우 집행 주체는 웹 프론트엔드입니다 — 초대 코드를 httpOnly 쿠키로 보관하고, 쿠키 수명(24시간)이 만료되면 로그인 요청에 `inviteCode`를 싣지 않습니다([`3-frontend.md`](./3-frontend.md) 소유). 서버는 윈도우를 별도 검증하지 않고 제출된 `inviteCode`를 신뢰합니다 — 변조로 윈도우를 넘겨 제출해도 자기 초대 금지·월 10회 상한이 남용을 제한하며, 잔여 리스크는 Sybil 파밍과 같은 수준으로 수용합니다(B21). 24시간은 구현 기본값이며 팀 확정 전입니다(미확정 — 값 변경은 프론트 쿠키 `maxAge` 상수 수정만으로 반영).
...
- 판정: 게스트 요청은 Redis 카운터로 한도를 확인하고, 한도 소진 시 `402`를 반환합니다. 게스트의 체험 한도 대상 요청은 device 헤더를 필수로 강화하며, 헤더가 없으면 400을 반환합니다(`details`에 누락 필드 명시 — 현행 best-effort에서 변경, [§4-8](#4-8-검수-체크리스트) B8).
...
| 대상 | 상태 | 규칙 |
| --- | --- | --- |
| 소유 리소스(`user_id` NOT NULL) — 턴 진행·재생성·수정(`GET /stories/{storyId}/edit` · `PATCH`)·스토리라인 평가 | `구현` | 요청자 `user_id`와 일치할 때만 허용. 불일치·미인증이면 `403` |
...
| 403 | `FORBIDDEN` | `Phase 1 · 구현` 소유자가 있는 리소스에 대한 타인·익명의 변경·삭제 시도(B9), 인증된 회원의 NULL 소유 리소스 접근(플레이·변경·삭제·채팅 생성·채팅 상세 조회, B16) — [§4-5](#4-5-인증과-권한). 채팅 배치 조회의 열람 필터만 미적용(B16) |
...
| B7 | 가입 프로필 발급 재정의 | 닉네임 랜덤 발급(형용사+명사)은 구현 완료. 프로필 이미지는 여전히 Google `picture`를 저장 — 프리셋 랜덤 배정으로 교체 필요([§4-5](#4-5-인증과-권한)). 교체 전까지 공개 스토리 `author.profileImageUrl`([§4-3-1](#4-3-api-계약))로 회원의 실제 Google 사진이 무인증 조회에 노출됨 | 닉네임 `Phase 1 · 구현`, 프로필 이미지 `Phase 1 · 계획`. 교체 전 임시 완화(공개 응답의 `profileImageUrl` 마스킹)는 교체 일정과 함께 판단 |
