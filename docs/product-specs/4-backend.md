...
- **초대 보상** — 초대자가 `inviteUrl`을 공유하고, 피초대자가 그 URL에서 회원가입을 완료해야 합니다. 프론트엔드는 URL의 초대 코드를 `POST /auth/login/google` 요청의 선택 필드 `inviteCode`로 전달합니다. 조건을 만족하면 **초대자와 피초대자 양쪽에 각각 500 크레딧**을 적립합니다. 자기 자신의 코드 제출, 이미 가입된 계정의 코드 제출은 무시합니다(오류 아님).
...
- `Phase 1` 크레딧: 가입 보상은 500 크레딧이어야 합니다. 출석 적립은 같은 KST 날짜에 2회 요청 시 첫 번째만 250 크레딧을 적립하고 두 번째는 `rewarded: false`(잔액 불변)여야 합니다. 초대 적립은 초대 URL에서 가입한 (초대자, 피초대자) 쌍으로 양쪽 500 크레딧씩 1회만 발생해야 합니다.
