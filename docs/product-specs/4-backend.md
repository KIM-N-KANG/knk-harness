...
| 알림 | `POST /users/me/devices` | 푸시 토큰 등록·갱신(기기 단위 멱등) | 204 | 400·401 | 필수 | 계획 |
| 알림 | `DELETE /users/me/devices` | 푸시 토큰 해제(로그아웃, 멱등) | 204 | 400·401 | 필수 | 계획 |
...
안드로이드 앱의 백그라운드 완료 알림입니다([`3-3-android-app.md §3-3-5`](./3-3-android-app.md) 백그라운드 알림). 웹은 대상이 아닙니다(웹 푸시는 별도 결정).

**발송 대상은 간편 제작 완성(`POST /stories/simple`) 하나뿐입니다.** 채팅 턴·재생성은 대상이 아닙니다 — `completed` 발행 전에 끊기면 턴이 저장되지 않고 크레딧이 환불되어([§4-3-7](#4-3-api-계약)) 알릴 결과 자체가 없기 때문입니다. 간편 제작은 반대로 연결이 끊겨도 서버가 완주해 결과가 DB에 남으므로([§4-3-2](#4-3-api-계약) 복구 계약) 알릴 대상이 존재합니다.

#### 토큰 등록·해제

**`POST /users/me/devices`** — 요청 `{token}`(FCM 등록 토큰), 응답 204.

- **토큰은 `X-Manyak-Device-Id`에 귀속합니다**(`MUST`). `(user_id, device_id_hash)` 유니크로 upsert하며 같은 기기의 토큰 갱신은 기존 행을 덮어씁니다. 헤더가 없으면 400입니다.
- 기기 귀속이 필요한 이유는 **발송 범위** 때문입니다. 생성을 요청한 기기에만 보내야 하며, 같은 계정의 다른 기기로 "네 스토리가 완성됐다"가 가는 것은 의도된 동작이 아닙니다.
- **같은 `fcm_token`을 가진 다른 행은 등록 시 함께 삭제합니다**(`MUST`). 물리 기기 하나의 FCM 토큰은 한 시점에 한 계정에만 속합니다. 해제(`DELETE`)가 정상 경로지만 **오프라인 로그아웃이나 네트워크 실패로 실패할 수 있고**([`3-3-android-app.md §3-3-4`](./3-3-android-app.md#3-3-4-api-연동인증-세션) 로그아웃 절차), 그 경우 이전 회원 귀속 행이 남아 다음 사용자의 기기로 발송됩니다. 이 규칙이 최종 방어선입니다.
- 클라이언트는 로그인 성공 직후와 FCM 토큰 회전(`onNewToken`) 시 호출합니다.

**`DELETE /users/me/devices`** — 요청 `{token}`, 응답 204(멱등). 해당 행이 없어도 성공합니다. `POST /auth/logout`과 같은 본문 기반 폐기 패턴입니다.

- 로그아웃 시 클라이언트가 `device_id`를 재발급하므로([`3-3-android-app.md §3-3-4`](./3-3-android-app.md)), 해제하지 않으면 이전 회원에게 귀속된 토큰 행이 남아 **다음 사용자의 기기로 발송**될 수 있습니다.

#### 발송 트리거

간편 제작 완성의 **저장 트랜잭션 커밋 후**(`AFTER_COMMIT`) 비동기(`@Async`)로 1회 발송합니다. 피드백 Slack 알림과 같은 패턴입니다([§4-3-4](#4-3-api-계약)).

- **연결 상태와 무관하게 항상 발송합니다**(`MUST`). 서버는 클라이언트 연결 여부를 판정할 수 없습니다 — 동기 블로킹 구조에서 응답 쓰기 실패는 서비스 밖에서 발생하기 때문입니다([§4-3-2](#4-3-api-계약)). 포그라운드 중복 표시는 **앱이 억제**합니다.
- **`requestId`당 1회만 발송합니다**(`MUST`). 복구 멱등으로 `COMPLETED` 상태를 재조회·재POST해도 추가 발송하지 않습니다([§4-3-2](#4-3-api-계약) 멱등 겸용).
- 발송 대상은 **요청 헤더의 `device_id`에 등록된 토큰**입니다. 등록된 토큰이 없으면 발송을 생략합니다(오류 아님).
- 회원 요청만 대상입니다. 게스트는 토큰을 귀속할 주체가 없습니다.

#### 페이로드

`notification`과 `data`를 함께 싣습니다. 백그라운드에서는 시스템이 표시하고, 포그라운드에서는 앱이 `onMessageReceived`로 받아 억제합니다.

| 필드 | 값 |
| --- | --- |
| `notification.title`·`body` | **고정 문구**를 사용합니다(`MUST`) |
| `data.type` | `story_creation_completed` |
| `data.storyId` | 완성된 스토리의 **공개 ID** |

- **사용자 입력 원문과 스토리 제목·본문을 알림 문구에 싣지 않습니다**(`MUST`). 잠금화면·알림 센터에 그대로 노출되는 표면이고 FCM 전송 구간에도 남습니다. 랜딩에 필요한 것은 식별자뿐입니다.
- 내부 Long PK를 싣지 않습니다(식별자 정책 — [§4-4](#4-4-데이터-모델)).

#### 실패 처리

- **발송 실패가 생성 성공(201)을 뒤집지 않습니다**(`MUST`). 커밋 후 비동기이므로 롤백 대상이 아닙니다.
- FCM이 `UNREGISTERED`·`INVALID_ARGUMENT`를 반환하면 해당 토큰 행을 삭제합니다. 그 밖의 오류는 로그만 남기고 재시도하지 않습니다 — 알림은 유실을 허용하며 앱의 복구 조회가 안전망입니다.
- 자격 증명 미설정 시 발송만 생략하고 기능은 정상 동작합니다(피드백 webhook URL 미설정과 동일 처리).

#### 데이터 모델

`user_devices` — `{id, user_id, device_id_hash, fcm_token, created_at, updated_at}`.

- `(user_id, device_id_hash)` 유니크. `device_id`는 원본이 아니라 **SHA-256 해시**를 저장합니다(게스트 체험 카운터 키와 동일 원칙 — [§4-7](#4-7-운영과-관측)). `fcm_token`에도 인덱스를 둡니다 — 등록 시 같은 토큰의 다른 행을 지우는 규칙(위 토큰 등록·해제)이 매 등록마다 이 컬럼으로 조회하기 때문입니다.
- `fcm_token`은 로그·분석 이벤트·Sentry에 싣지 않습니다(`MUST`).

#### 환경 변수

| 이름 | 필수 | 설명 |
| --- | --- | --- |
| `MANYAK_FCM_CREDENTIALS` | 아니오 | FCM Admin SDK 서비스 계정. 미설정 시 발송 생략 |
...
- `계획` 푸시 알림([§4-3-12](#4-3-api-계약)): 같은 `requestId`의 복구 조회·재POST가 추가 발송을 만들지 않아야 합니다(1회 멱등). 로그아웃 후 같은 기기의 재로그인이 이전 토큰 행을 남기지 않아야 합니다 — **해제 API를 건너뛴 경우에도** 같은 `fcm_token`으로 재등록하면 이전 회원 행이 사라져야 합니다(등록 시 동일 토큰 행 삭제). 등록된 토큰이 없는 요청의 생성이 정상 201이어야 하고, 발송 실패가 201을 뒤집지 않아야 합니다. 알림 문구에 스토리 제목·사용자 입력이 포함되지 않아야 합니다.
...
| B26 | 회원 탈퇴(계정 삭제) 부재 | 탈퇴 API·화면이 없고, 서비스 안내가 "탈퇴는 문의처(이메일) 요청으로 처리"만 안내합니다([`3-1-client.md §3-1-3`](./3-1-client.md) FE-SCREEN-011). 웹 단독 시기에는 수용됐지만, 안드로이드 앱이 로그인 필수로 확정되면서(2026-08-03 팀 결정 — [`3-3-android-app.md`](./3-3-android-app.md)) Google Play User Data 정책의 계정 삭제 요건이 적용됩니다 — 계정 생성이 있는 앱은 앱 내 삭제 진입점과 웹 삭제 요청 URL(Data safety 폼 제출)을 요구하며, 미충족은 심사 거절·업데이트 차단 사유입니다 | **Android 출시 전 결정 필수** — ① 최소 대응: 웹 탈퇴 요청 페이지(접수 후 운영 처리) + 앱 진입점 연결, ② 셀프서브 탈퇴 API + 앱 내 탈퇴 메뉴(삭제 범위 — `users`·크레딧·초대 그래프·생성 콘텐츠 처리를 소프트 삭제 정책([§4-4](#4-4-데이터-모델))과 정합시켜 설계)를 비교 결정. 정확한 정책 요건(이메일 문의 방식 허용 여부 포함)은 Play 최신 정책 문서로 검증 후 확정 |
