...
| `chatCount` | number | `Phase 1 · 계획`(결정 2026-07-07) — `turnCount`(누적 진행 턴 수 = 사용자 입력 횟수)로 교체합니다. 채팅 개수 집계(현행 와이어 값)는 눌러본 횟수에 가까워 실제 플레이량 지표로 제외([§4-8](#4-8-검수-체크리스트) B14) |
...
- **보상 크레딧 유효기간·차감 순서 — `Phase 1 · 계획`(결정 2026-07-07, B12)** — 보상 적립(`SIGNUP_REWARD` · `INVITE_REWARD` · `ATTENDANCE_REWARD`)은 적립 시점부터 30일 유효하며 만료분은 잔액에서 제외합니다. 차감은 유효기간이 있는 크레딧을 무기한 크레딧(`PURCHASE` — Phase 3)보다 먼저, 그중 남은 기간이 짧은(먼저 적립된) 것부터 소진합니다(FIFO) — 사용자가 손해 보지 않는 유일한 소진 순서입니다(무기한분부터 쓰면 보상분이 만료로 증발).
...
- **배경.** 보상 재화가 무기한 누적되면 잔액이 쌓인 휴면 사용자에게 재방문 유인이 없고 미사용 잔액이 부채처럼 쌓입니다. 30일 만료가 소진·재방문 주기를 만듭니다.
...
- **회원도 게스트 체험 횟수를 공유합니다 — `Phase 1 · 계획`(결정 2026-07-07, B13)** — 디바이스의 체험 횟수가 남아 있으면 회원 요청도 크레딧 대신 체험 횟수를 먼저 차감해 무료로 처리합니다. 회원가입 시 디바이스 ID 기반 남은 횟수를 계정에 동기화합니다. 회원 요청의 소모 판정은 2단(체험 잔여 → 크레딧)입니다.
...
- 저장: `completed` 전에 새 AI 출력·선택지를 마지막 턴의 **활성본으로 확정**합니다. 기존 출력·선택지는 덮어쓰거나 삭제하지 않고 보존하며, 상세 조회·SSE `completed`는 활성본만 싣고 이전 출력은 화면에 노출하지 않습니다(보존 스키마는 `Phase 1 · 계획` — [§4-8](#4-8-검수-체크리스트) B11). 확정 트랜잭션에서 해당 턴이 여전히 마지막 턴인지 재확인하고, 아니면(스트림 중 새 턴 추가·동시 재생성 경합) 결과를 폐기하고 `error` 이벤트로 종료합니다 — 환불은 실패 규칙을 따릅니다. `turn_number`·사용자 입력은 변하지 않습니다.
