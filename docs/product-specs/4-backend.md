...
`Phase 1 · 구현`(KNK-623, V48·V49) — 생성 요청 복구입니다. 모바일에서 스토리라인 생성·스토리 완성 대기 중 앱 전환으로 연결이 끊겨도 서버가 생성을 계속 진행하고, 프론트엔드가 복귀 시 진행 상태·결과를 조회할 수 있습니다(2026-07-20 팀 결정, [`3-frontend.md §3-5`](./3-frontend.md)). 두 생성 호출 모두 servlet 워커 스레드의 동기 블로킹 구조(RestClient)라, 클라이언트 연결이 끊겨도 서버는 AI 호출·저장 트랜잭션을 끝까지 진행하고 응답 쓰기에서만 실패합니다 — 결과는 DB에 남지만 클라이언트가 식별자를 받지 못해 되찾을 수 없던 것이 문제였습니다(응답 쓰기 실패는 서비스 밖에서 발생하므로 크레딧·게스트 카운터도 성공 경로로 정산 — 저장 결과와 정합). 따라서 동기 흐름은 유지하고 복구 경로만 추가했습니다(전면 비동기화·폴링 전환은 하지 않음).
...
| 같은 이메일 발견 시 안내 노출 | 해당 이메일의 가입 여부를 알려주는 열거 오라클이 됩니다. 현재 인증은 계정 존재 여부를 일괄 401로 감추고 있어(위 흐름 2) 기조가 어긋납니다. 안내가 필요하면 조회 없는 정적 문구로 둡니다([`3-frontend.md`](./3-frontend.md) FE-SCREEN-008) |
...
계정 연동(로그인된 세션에서 다른 provider를 같은 `user_id`에 추가)은 갈라진 계정을 사후에 합치는 것보다 훨씬 싸므로, 계정 통합 대신 이 방식을 제공합니다(아래 절). 표기는 "계정 연동"으로 통일합니다([`0-glossary.md §0-3-4`](./0-glossary.md) — "계정 연결"은 지양, KNK-737).

### 계정 연동 — `Phase 1 · 구현`(KNK-739)

로그인된 세션에서 다른 provider를 같은 `user_id`에 추가합니다. 연동해 두면 그 provider로 로그인해도 같은 계정(같은 크레딧·서재)으로 들어오므로, **계정이 갈라지기 전에 막는 유일한 수단**입니다(갈라진 뒤에 합치는 merge는 미도입 — 위 결정 기록). **연동 해제는 제공하지 않습니다**(아래 결정 기록).
...
**재인증이 선행됩니다.** 연동은 계정에 로그인 수단을 영구히 추가하는 작업이라 현재 세션만으로는 부족합니다 — 세션을 주운 사람(공용 기기에 남은 세션 포함)이 자기 소셜 계정을 붙이면 세션을 정리해도 영구 접근권이 남습니다. **이미 연동된 provider**의 ID 토큰으로 소유를 재확인해야 하며, 그 토큰의 `sub`가 요청자의 해당 provider 연동 행과 일치해야 합니다. 재인증 토큰은 **발급된 지 10분 이내**여야 하고(`manyak.auth.link.reauth-max-age`, 시계 오차 60초 허용) `iat`가 없으면 거부합니다(fail-closed). 실패는 사유를 구분하지 않고 일괄 403 `REAUTH_FAILED`입니다 — 어떤 소셜 계정이 그 회원에게 붙어 있는지 노출하지 않기 위해서입니다.

**링크 코드는 일회용 불투명 코드입니다.** Redis `link_auth:{codeHash}`에 **TTL 5분**(`manyak.auth.link.code-ttl`)으로 보관하며, 키는 코드 원문이 아니라 SHA-256 해시이고 값은 소유자 `user_id`입니다(로그인 핸드오프의 보관 패턴 재사용). 코드는 **URL이 아니라 `X-Manyak-Link-Code` 헤더로만** 주고받습니다 — 서버가 모든 요청 URI를 구조화 로그·Sentry breadcrumb에 남기기 때문입니다. **성공했을 때만 1회 소비**하고, 403·409로 실패하면 미소비로 남겨 만료 전까지 재인증 없이 재시도할 수 있습니다(핸드오프의 "실패 시 미소비 유지"와 같은 결). 없음·만료·이미 소비됨·타인 소유는 모두 403 `REAUTH_FAILED`입니다.
...
- **자격증명류 실패에 401을 쓰지 않습니다.** 이 경로는 세션 자체가 유효한 상태이므로, 401을 내면 클라이언트 인터셉터가 "세션 만료 → 재발급·로그아웃"으로 오해해 연동 실수 한 번에 로그아웃됩니다. 401은 계정 자체가 사라진 경우(부재·`DELETED`)에만 씁니다.
- **`DELETED`는 401, `SUSPENDED`는 403입니다.** 정지 판정 유틸(`isActiveAccessAllowed`)이 `SUSPENDED`만 막고 `DELETED`를 통과시키므로, 연동은 사용자 행에서 상태를 직접 확인합니다 — 탈퇴 계정에 로그인 수단이 붙으면 되살아납니다.
- **같은 소셜 계정 재요청도 409입니다**(멱등 성공으로 흘리지 않습니다). 최종 상태는 어느 쪽이든 같고, 클라이언트는 409를 받아도 `GET /auth/me`로 현재 상태를 정확히 표시할 수 있습니다.
