...
- **서버 인증.** 태스크 역할 IAM 매핑(SigV4)입니다 — FireLens 로그 쓰기와 같은 방식([`7-deployment.md`](./7-deployment.md)). OpenSearch 세분 접근 제어에 역할 `manyak-search-{env}`(`stories-{env}*` 한정 `indices_all`, `cluster_composite_ops`)를 만들고 태스크 롤 ARN을 backend role로 매핑합니다. 인덱스 존재 확인(`indices:admin/exists`)·매핑 조회가 `crud` 밖이므로 환경별 검색 인덱스에 `indices_all`을 부여합니다. 시크릿이 없고, dev IAM 정책(`es:ESHttp*`)은 FireLens가 이미 부여했습니다. **운영은 FireLens 운영 연동(KNK-857)이 아직이라 태스크 역할에 이 권한이 없습니다** — 검색의 운영 릴리스는 857과 함께 하거나 같은 statement를 운영 모듈에 따로 넣습니다(`manyak-terraform`). 클라이언트는 `opensearch-java` + `AwsSdk2Transport`.
...
- **테스트.** 통합 테스트는 클라이언트를 대체해 질의 JSON·필터·커서·400을 고정하고, 실제 질의·필터·`search_after`는 Testcontainers OpenSearch(2.19)로 별도 클래스 한 건만 둡니다(컨텍스트 예산). 공식 이미지에는 `analysis-nori`가 없어 그 테스트는 `standard` 분석기로 돌리며, 형태소 분석 자체는 dev 도메인에서 `_analyze`로 실측합니다. `turnCount`는 매 턴 갱신하지 않고 그 스토리의 다음 색인·재색인 때 따라옵니다.
