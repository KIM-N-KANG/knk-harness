...
채팅 기능 확장 2종의 계약입니다: 마지막 AI 응답 재생성(US-6-10)과 이미지 표시 — 스토리 썸네일(US-2-7)·채팅 이미지(US-6-11). 이미지는 썸네일·배경은 팀이 사전 제작한 프리셋 자산이고, 인물 이미지는 컴파일 시 AI가 직접 생성합니다(KNK-414). 채팅에서는 **AI가 응답 생성 중 적절한 위치에 이미지 마커를 삽입**합니다. 이미지 계약은 2026-07-10 팀 합의로 확정됐고(아래 결정 기록), **썸네일 트랙은 서버 구현 완료**(V45·46·KNK-548 — 자동 연결·반응형 변형·응답 배선), **인물 이미지 AI 생성은 AI 서버 구현 완료**(KNK-414 — 백엔드 base64 수신·S3 업로드는 KNK-966으로 잔여), **채팅 이미지 마커 트랙은 서버·AI 서버·프론트엔드 모두 구현이 남아 있습니다**([§4-8](#4-8-검수-체크리스트) B10).
