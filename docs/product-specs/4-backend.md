...
| `aiOutput` | string | AI 출력 전문. `Phase 2 · 계획` — 인물 이미지는 스트리밍 중 `character_image` SSE 이벤트로 전달되며 본문에는 포함되지 않음(KNK-981, [§4-3-9](#4-3-api-계약)) |
| `choices` | string[] | 선택지 |
...
- 이벤트는 위 5종이며(`character_image`는 `Phase 2 · 계획`으로 미구현), heartbeat(주기 ping)는 없습니다. 페이로드는 이벤트별 DTO의 JSON 직렬화입니다.
...
채팅 기능 확장 2종의 계약입니다: 마지막 AI 응답 재생성(US-6-10)과 이미지 표시 — 스토리 썸네일(US-2-7)·채팅 이미지(US-6-11). 이미지는 썸네일·배경은 팀이 사전 제작한 프리셋 자산이고, 인물 이미지는 컴파일 시 AI가 직접 생성합니다(KNK-414). 채팅에서 배경은 **AI가 본문 안에 마커(`[[image:<imageKey>]]`)를 삽입**하는 방식이고, 인물 이미지는 **AI 서버가 스트리밍 중 태그를 감지해 `character_image` SSE 이벤트로 전달**하는 방식입니다(KNK-981 — [`5-ai-server.md §5-3-4`](./5-ai-server.md)). 이미지 계약은 2026-07-10 팀 합의로 확정됐고(아래 결정 기록), **썸네일 트랙은 서버 구현 완료**(V45·46·KNK-548 — 자동 연결·반응형 변형·응답 배선), **인물 이미지 AI 생성은 AI 서버 구현 완료**(KNK-414 — 백엔드 base64 수신·S3 업로드는 KNK-966으로 잔여), **채팅 인물 이미지 전달은 태그 치환 방식으로 설계 확정**(KNK-981, `Phase 2 · 계획` — 서버·AI 서버·프론트엔드 구현 미착수), **배경 마커 트랙도 구현이 남아 있습니다**([§4-8](#4-8-검수-체크리스트) B10).
...
**채팅 인물 이미지 전달 — 태그 치환 방식(`Phase 2 · 계획`, KNK-981).**

**무엇.** 채팅에서 인물이 등장하면 그 인물의 이미지를 스트리밍 도중에 `character_image` SSE 이벤트로 전달합니다. 배경 이미지의 마커 방식(`[[image:<imageKey>]]` → `completed` 동봉)과 다릅니다.

**왜.** 배경 마커는 `completed`에서 검증하므로 턴이 끝나야 이미지가 보입니다. 인물 이미지는 인물이 등장하는 시점에 바로 보여야 몰입에 유리하므로, 스트리밍 중에 전달합니다.
...
아래 검증·저장·스트리밍 규칙은 **배경 마커 방식**(`[[image:<imageKey>]]` + `completed` 동봉 `images[]`)의 상세이며, 배경 마커 트랙은 미구현 보류입니다. **인물 이미지는 이 규칙의 적용 대상이 아닙니다** — 인물은 `character_image` SSE 이벤트로 전달되며 본문에 마커가 포함되지 않습니다(위 "채팅 인물 이미지 전달" 절).
...
| B10 | 이미지 표시 구현 | **썸네일 트랙은 서버 구현 완료**(V45·46, 2026-07-21 코드 대조 확인) — `stories.thumbnail_image_key` 컬럼, 등록 시 `StoryThumbnailLinker`로 자동 연결(간편·일반 제작 공통), 프리셋 시드, 상세·목록·채팅 응답 `thumbnailUrl`/`thumbnailUrlSm` 조합. **인물 이미지 — AI 서버 구현 완료**(KNK-414) — 컴파일 응답의 `character_images[]`에 인물별 WebP base64가 실림([`5-ai-server.md §5-3-3`](./5-ai-server.md)). **백엔드 잔여**: 컴파일 응답 DTO에 `character_images` 추가, base64 디코딩 → S3 업로드, URL을 `story_characters`에 저장(KNK-966). **채팅 이미지 마커 트랙은 미구현** — 배경 후보 배정·`completed.images[]`·마커 렌더는 서버·AI 서버·프론트엔드 모두 남아 있음 | `Phase 2 · 구현`(인물 이미지 AI 생성) + `Phase 1 · 계획`(채팅 마커·배경 트랙). 인물 이미지 백엔드 잔여(KNK-966): base64 수신 → S3 업로드 → DB URL 저장. 공통 후속: [`6-analytics.md`](./6-analytics.md) `image_key` 재정렬, 프론트엔드 이미지 렌더·CDN 허용 호스트([`7-deployment.md §7-4`](./7-deployment.md)는 운영 시드 전용으로 정리 완료) |
