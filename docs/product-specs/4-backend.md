...
- **리뷰·QA** — §4-8 검수 체크리스트와 간극 표(B1~B14)에서 시작해 본문을 역참조합니다.
...
- **보상 크레딧 유효기간·차감 순서 — `Phase 1 · 계획`(결정 2026-07-07)** — 보상 적립(`SIGNUP_REWARD` · `INVITE_REWARD` · `ATTENDANCE_REWARD`)은 적립 시점부터 30일 유효하며 만료분은 잔액에서 제외합니다. 차감은 유효기간이 있는 크레딧을 무기한 크레딧(`PURCHASE` — Phase 3)보다 먼저, 그중 남은 기간이 짧은(먼저 적립된) 것부터 소진합니다(FIFO). 현행 구현은 단일 잔액 캐시로 만료·순서를 표현할 수 없어 적립 로트별 잔여 추적(원장 `expires_at` + 로트 소진 기록)으로의 서버 변경이 필요합니다([§4-8](#4-8-검수-체크리스트) B12).
  - 결정 기록(2026-07-07, B12) — **배경**: 보상 재화가 무기한 누적되면 잔액이 쌓인 휴면 사용자에게 재방문 유인이 없고 미사용 잔액이 부채처럼 쌓입니다. 30일 만료는 소진·재방문 주기를 만듭니다. **대안**: 무기한·단일 잔액(원안 — 구현 단순)과 유료 크레딧(`PURCHASE`)까지 만료시키는 안은 각각 재방문 유인 부재·결제 재화 신뢰 훼손으로 제외했습니다. **영향**: 만료 임박(선입) 우선 차감이 사용자가 손해 보지 않는 유일한 소진 순서이고(무기한분부터 쓰면 보상분이 만료로 증발), 로트별 잔여 추적으로의 서버 전환이 필요합니다.
...
- **회원도 게스트 체험 횟수를 공유합니다 — `Phase 1 · 계획`(결정 2026-07-07, 기존 "별개 카운터" 정책 대체)** — 디바이스의 체험 횟수가 남아 있으면 회원 요청도 크레딧 대신 체험 횟수를 먼저 차감해 무료로 처리합니다. 회원가입 시 디바이스 ID 기반 남은 횟수를 계정에 동기화합니다. 현행 구현은 회원=크레딧, 게스트=디바이스 카운터로 분리되어 있어 서버 변경이 필요합니다([§4-8](#4-8-검수-체크리스트) B13).
  - 결정 기록(2026-07-07, B13) — **배경**: 체험 횟수가 남은 게스트가 가입하는 순간 남은 무료 체험이 사라지면 **가입이 손해가 되는 역인센티브**가 생깁니다. **대안**: 별개 카운터(원안 — KNK-477. "크레딧은 회원 전용 재화" 정의와 정합하고 판정이 단순)는 이 역인센티브 때문에 대체했습니다. **영향**: 전환 직후에도 남은 체험분을 무료로 이어 쓰게 되어 가입 마찰이 없어지고, 비용은 회원 요청의 소모 판정이 2단(체험 잔여 → 크레딧)이 되는 복잡도입니다.
...
- 이력 보존 결정 기록(2026-07-07, B11) — **배경**: 재생성은 곧 "기존 출력이 마음에 안 들었다"는 품질 신호라서, 이전 출력 ↔ 새 출력 쌍이 재생성 원인 분석과 AI 품질 평가 데이터의 원천입니다. **대안**: 교체 유지(원안 — KNK-406에서 "Phase 1은 버전 이력을 두지 않는다"로 명시. 스키마가 단순하지만 이 데이터를 저장 시점에 파괴)는 채택하지 않았습니다. **영향**: 분석 가치를 위해 보존 비용(메시지 버전 보관 스키마 전환)을 받아들입니다.
