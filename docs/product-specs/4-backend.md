...
| `GET /profile-presets` | 없음 | 200 `[{ "key": string, "imageUrl": string, "thumbnailBase64": string? }]` — 명사 키 오름차순, 40종 |

**닉네임 규칙**

- 앞뒤 공백을 지운 뒤 **2~20자**. 허용 문자는 완성형 한글·영문·숫자·공백이며, 연속 공백·자모 단독·특수문자·이모지는 400입니다. `"nickname": null`은 "지우기"가 아니라 400입니다(닉네임은 필수값이라 지울 수 없음).
