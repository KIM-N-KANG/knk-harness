...
- **주변 인물 세트** — 주변 인물 특징 태그 대신 주변 인물 세트 `supportingCharacters[]`(최대 5)를 받습니다. 각 세트 `{name(≤30자), gender("MALE"·"FEMALE"·null), features[](세트당 최대 3, 각 30자 — 제공 태그 + 직접 입력)}`는 전부 선택 항목이며 빈 항목은 AI가 자동 생성합니다. `features` 직접 입력분을 기존 태그 시스템(`CUSTOM` 태그·세션 태그 저장)에 연결할지, 세트 구조로만 저장할지는 미정입니다. AI 요청 필드 구성은 [`5-ai-server.md §5-3-2`](./5-ai-server.md)가 소유합니다.
...
`Phase 1 · 계획`(KNK-623) — 모바일에서 스토리라인 생성·스토리 완성 대기 중 앱 전환으로 연결이 끊겨도 서버가 생성을 계속 진행하고, 프론트엔드가 복귀 시 진행 상태·결과를 조회할 수 있어야 합니다(2026-07-20 팀 결정 — 방향만 확정, [`3-frontend.md §3-5`](./3-frontend.md)). 현행 계약은 동기 응답이라 연결이 끊기면 클라이언트가 결과를 받을 방법이 없습니다. 비동기화 범위·진행 상태 조회 API·완료 결과 전달 방식 등 구체 구현 방안은 모색 중이며 미정입니다.
