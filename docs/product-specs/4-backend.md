...
**③ 정해진 장르만 남게 하기 — 키워드 단계 개편(KNK-621)에 얹힘.** 장르는 현재 커스텀 입력이 섞여 오지만(`customTags` category `GENRE`, [§4-3-2](#4-3-api-계약)), 키워드 단계 개편(KNK-621, [§4-3-2](#4-3-api-계약))이 장르·배경 커스텀 입력을 400으로 차단하므로 개편 배포 후에는 `genre_tags`가 사전 정의만 남습니다 — **별도 구분 전달이 필요 없습니다**(커스텀 장르가 태그로 새면 카디널리티·PII 문제인데, 입력 단계에서 원천 차단됨). Langfuse 켜기는 KNK-621 배포 이후로 순서를 잡습니다(장르 차원은 소급 부착 불가 — [`7-deployment.md §7-9`](./7-deployment.md)). 장르는 스토리 제작 시점에만 선택되므로 사전 정의 장르 태그는 **스토리 제작 트레이스에만** 답니다(채팅 트레이스 장르 태그는 KNK-652에서 제거, AI `v0.2.1` 배포 완료 — [`5-ai-server.md §5-6`](./5-ai-server.md)). 채팅 트레이스에도 장르가 필요한지, 필요하면 매 채팅 턴에 장르를 실어 나르는 비용을 어떻게 감당할지는 후속에서 정합니다.
