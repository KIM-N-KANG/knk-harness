...
| 엔드포인트 | 요청 | 응답 |
| --- | --- | --- |
| `POST /auth/login/google` | `{idToken}` — 구 `inviteCode?` 필드는 폐기 완료(`Phase 1 · 구현` KNK-567, [§4-3-7](#4-3-api-계약)) | `TokenResponse` |
...
**③ 정해진 장르만 남게 하기 — 키워드 단계 개편(KNK-621)에 얹힘.** 장르는 현재 커스텀 입력이 섞여 오지만(`customTags` category `GENRE`, [§4-3-2](#4-3-api-계약)), 키워드 단계 개편(KNK-621, [§4-3-2](#4-3-api-계약))이 커스텀 입력을 400으로 막으면 `genre_tags`에 사전 정의만 남으므로 **별도 구분 전달이 필요 없습니다**. 다만 이는 개편의 차단 범위에 달린 전제라, **`customTags`의 `GENRE` 카테고리 커스텀 입력을 400으로 막는 것이 KNK-621의 수용조건입니다** — 기준 코드는 아직 `GENRE` 커스텀을 받고 있고, 개편이 키워드·배경만 막고 장르를 통과시키면 사용자 자유입력이 그대로 `genre_tags`로 흘러 트레이스 태그의 카디널리티·PII 문제가 됩니다. Langfuse 켜기는 KNK-621 배포 이후로 순서를 잡고, 켜기 전 점검에서 이 조건 충족을 확인합니다(장르 차원은 소급 부착 불가 — [`7-deployment.md §7-9`](./7-deployment.md)). 장르는 스토리 제작 시점에만 선택되므로 사전 정의 장르 태그는 **스토리 제작 트레이스에만** 답니다(채팅 트레이스 장르 태그는 KNK-652에서 제거, AI `v0.2.1` 배포 완료 — [`5-ai-server.md §5-6`](./5-ai-server.md)). 채팅 트레이스에도 장르가 필요한지, 필요하면 매 채팅 턴에 장르를 실어 나르는 비용을 어떻게 감당할지는 후속에서 정합니다.
