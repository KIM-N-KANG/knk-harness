...
- 채팅: 본문(`/chat/turns`)과 선택지(`/chat/choices`) 두 호출에 **같은 연결 식별자**를 실어 나중에 한 턴으로 이어봅니다.
- 스토리: 멱등용 `requestId`는 **단계 간 재사용이 409로 막히므로**(`StoryCreationRequestRecorder` — 스토리라인↔완성 재사용 시 "다른 단계에서 사용된 요청 ID" 오류) 재사용하지 않고, 제작 세션에 저장하는 **별도 연결 식별자**를 만들어 단계마다 실어 보냅니다.
...
**③ 정해진 장르를 구분해 전달.** 장르는 커스텀 입력이 섞여 오므로(`customTags` category `GENRE`, [§4-3-2](#4-3-api-계약)), 백엔드가 **사전 정의 장르만 구분**(예: 사전 정의 태그 ID)해 전달합니다. AI는 그 구분을 근거로 사전 정의 장르만 트레이스 태그로 답니다(커스텀은 원문에만 남고 태그로는 올리지 않음 — 카디널리티·PII 방지). AI 태깅 코드 반영은 KNK-640 후속입니다.
