...
- `(user_id, device_id_hash)` 유니크. `device_id`는 원본이 아니라 **SHA-256 해시**를 저장합니다(게스트 체험 카운터 키와 동일 원칙 — [§4-7](#4-7-운영과-관측)).
...
- `계획` 푸시 알림([§4-3-12](#4-3-api-계약)): 같은 `requestId`의 복구 조회·재POST가 추가 발송을 만들지 않아야 합니다(1회 멱등). 로그아웃 후 같은 기기의 재로그인이 이전 토큰 행을 남기지 않아야 합니다. 등록된 토큰이 없는 요청의 생성이 정상 201이어야 하고, 발송 실패가 201을 뒤집지 않아야 합니다. 알림 문구에 스토리 제목·사용자 입력이 포함되지 않아야 합니다.
