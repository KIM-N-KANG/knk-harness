...
| `X-Manyak-Parent-Creation-Id` | 프론트가 재생성 요청에 실어 보내는 이전 `creation_id`. **어떤 값을 보낼지(직전 값 체인인지, 같은 시도 묶음의 최초 루트 값인지)는 미확정** — 아래 참고 | 스토리라인 생성(재생성일 때만, 값 없으면 생략) |
