...
- **리뷰·QA** — §4-8 검수 체크리스트와 간극 표(B1~B21)에서 시작해 본문을 역참조합니다.
...
쿼리 `limit`(기본 100). 1 미만·100 초과·비정수 값은 100으로 clamp합니다(400 아님). Phase 1은 페이지네이션 없이 상한 100을 유지하고, 초과분 처리는 Phase 2 스토리 피드 설계와 함께 확장합니다. 소프트 삭제된 항목은 제외합니다. 정렬(생성 최신순·최근 활동순)의 동일 시각 tie는 내부 PK 내림차순을 2차 키로 확정합니다(서버 `OrderByCreatedAtDescIdDesc` 관례).
