...
구현 이력: draft 플로우는 실제로 먼저 구현됐다가(KNK-401·402·460 — 초안 생성·탭별 PUT 자동 저장·발행 시 공개 범위 동시 선택) 2026-07-06 결정으로 제거됐습니다(KNK-464 — **하네스 스펙을 단일 정본으로 삼고 코드를 스펙에 맞춘다**). draft 전용으로 추가됐던 스펙 외 컬럼(`story_start_settings.opening_scene`·`first_ai_message`, V32)도 함께 제거했습니다(V34).
...
| `visibility` | 선택, 기본 `PRIVATE` | 공개 범위(`PUBLIC` · `PRIVATE`). 기본값이 PRIVATE인 이유: 제작자가 명시적으로 공개하기 전까지 비공개가 안전 |
...
- 크레딧·체험 한도: 채팅 턴과 동일합니다([§4-3-7](#4-3-api-계약)) — 회원은 10 크레딧 선차감, 게스트는 `chat_turn` 카운터 1턴 집계, 잔액 부족·한도 소진은 동기 402. 재생성 횟수 제한은 따로 두지 않습니다(소모와 게스트 전체 채팅 턴 한도 15회가 반복을 제어). 404·403·409 검증은 선차감보다 앞서 수행합니다 — 실패가 확정된 요청에 차감·환불 왕복을 만들지 않기 위해서입니다(KNK-406).
...
- 이력 보존 결정 배경: 원안(교체·이전 본문 미보관 — KNK-406에서 "Phase 1은 버전 이력을 두지 않는다"로 명시)은 스키마가 단순하다는 이유로 택했지만, 재생성은 곧 "기존 출력이 마음에 안 들었다"는 품질 신호라서 이전 출력 ↔ 새 출력 쌍이 재생성 원인 분석과 AI 품질 평가 데이터의 원천이 됩니다. 교체 방식은 이 데이터를 저장 시점에 파괴하므로, 분석 가치를 위해 보존 비용(스키마 복잡도)을 받아들였습니다.
