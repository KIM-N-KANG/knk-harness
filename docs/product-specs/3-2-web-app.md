...
오버레이 화면(별도 라우트 없음): 제작 목록의 스토리 옵션 메뉴+삭제 확인 다이얼로그(FE-SCREEN-013), 썸네일 이미지 뷰어(FE-SCREEN-003), 채팅 옵션 메뉴+채팅 삭제 확인 다이얼로그(FE-SCREEN-005, [3-1-client.md §3-1-5](./3-1-client.md#3-1-5-채팅-플레이와-sse-스트리밍)), 키워드 추가 다이얼로그, 선택 키워드 드로어, 생성 이탈 확인 다이얼로그, 스토리라인 재선택 경고 다이얼로그.
...
### 스토리 신고 진입점 (웹) — `Phase 2 · 계획`

신고 시트의 화면 계약(사유 3개 중 하나 필수·자유 서술 500자·성공 시 닫힘·실패 시 입력 보존)과 **채팅에서 신고해도 대상은 그 채팅이 참조하는 스토리**라는 점은 [`3-1-client.md §3-1-3` FE-SCREEN-003](./3-1-client.md#3-1-3-화면별-스펙) **스토리 신고**가 소유합니다. 웹은 두 화면의 진입점 사정이 달라 여기에 자리만 정합니다.

- **채팅방** — 헤더에 이미 옵션 메뉴가 있으므로(`chat-options-menu`, 현재 항목은 삭제하기 하나) 거기에 "신고하기"를 더합니다. 파괴적 항목인 삭제하기 위에 둡니다.
- **스토리 상세** — 헤더에 뒤로가기만 있어 옵션 메뉴를 새로 둬야 합니다. 목록 카드의 `story-options-menu`는 소유자만 쓰는 삭제 메뉴라 재사용하지 않습니다 — 상세 헤더 메뉴는 소유 여부와 무관하게 누구에게나 보이고 신고하기만 담습니다.

시트를 무엇으로 띄울지는 웹이 정합니다. 앱은 바텀 시트로 열지만, 웹은 기존 다이얼로그·드로어 중 무엇을 쓰든 위 계약만 지키면 됩니다.

안드로이드는 두 화면 모두 헤더 더보기 메뉴에 신고하기만 두고 삭제는 담지 않습니다. 상세 응답의 `author`가 항상 null이라 소유 여부를 판정할 수 없어서이며, 웹과 달리 상세에서 삭제를 제공하지 않는 이유도 같습니다([`3-3-android-app.md §3-3-3`](./3-3-android-app.md)).
...
| `stories/story-list`                      | 목록 표시·이동·내 스토리 카드 직접 삭제·재시도                                                        | US-2-1·2·4·6              |
| `stories/story-detail`                    | 상세 표시(썸네일·엔딩·턴 수·생성일)·스크롤 헤더·썸네일 뷰어·시작 상황 선택·채팅 시작 스피너·오류 상태 | US-4-1·2·3·4·6·7·8        |
