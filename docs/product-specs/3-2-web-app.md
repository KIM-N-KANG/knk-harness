...
| 이력      | `3-frontend.md` v0.17(2026-07-01~07-28)을 KNK-765에서 분리. 앱이 참조하는 경로 계약을 KNK-772에서 반영(2026-08-03) |
...
퍼널 진행·임시 저장 슬롯(`manyak:pending-creation-request`), 채팅 안내 플래그(`manyak:chat-tour-seen`·`manyak:chat-choices-hint-seen`), 재개 의도 세션스토리지(`manyak:story-draft-resume-intent`), 온보딩 경유 세션스토리지(`manyak:onboarding-entry`)도 웹 키 정본은 이 문서 소유입니다(계약은 각각 [`3-1-client.md §3-1-4`](./3-1-client.md#3-1-4-스토리-생성-퍼널)·[§3-1-5](./3-1-client.md#3-1-5-채팅-플레이와-sse-스트리밍)·[§3-1-3 FE-SCREEN-007](./3-1-client.md#3-1-3-화면별-스펙)).
...
이 표는 **사용자 화면 라우트 인벤토리**입니다(+ BFF 프록시·검색 크롤러 메타데이터 라우트). NextAuth·핸드오프 세션 등 인증 API 라우트는 [§3-2-4](#3-2-4-bff-프록시토큰-세션)의 인증 라우트 표가 소유합니다. 경로는 MVP 화면 라우트 7개(`/`·`/stories/new`·`/stories/[id]`·`/chats`·`/chats/[id]`·`/my/feedback`·`/onboarding`)와 Not Found, BFF 프록시 라우트 1개(`/api/[...path]`)에 `Phase 1 · 구현` 9개(`/login`·`/login/continue`·`/my`·`/my/invite`·`/my/link/continue`·`/terms`·`/privacy`·`/my/about`·`/share/[shareId]`), `Phase 1 · 계획` 2개(`/stories/new/general`·`/stories/[id]/edit`)를 더해 구성됩니다. 여기에 화면이 아닌 검색 크롤러용 메타데이터 라우트 2개(`/robots.txt`·`/sitemap.xml`)가 있습니다. 회원가입 별도 경로·대시보드·설정·관리자 경로는 없습니다. 동적 세그먼트 `[id]`는 문자열입니다.
...
오버레이 화면(별도 라우트 없음): 옵션 메뉴+삭제 확인 다이얼로그(스토리 상세), 썸네일 이미지 뷰어(FE-SCREEN-003), 채팅 옵션 메뉴+채팅 삭제 확인 다이얼로그(FE-SCREEN-005, [3-1-client.md §3-1-5](./3-1-client.md#3-1-5-채팅-플레이와-sse-스트리밍)), 키워드 추가 다이얼로그, 선택 키워드 드로어, 생성 이탈 확인 다이얼로그.

### 라우팅 규칙

- **`/terms`·`/privacy`·`/my/about` 경로는 안드로이드 앱과의 계약입니다.** 앱이 이 세 경로를 인앱 브라우저로 직접 열기 때문에(§법적 콘텐츠 소스, [`3-3-android-app.md §3-3-4`](./3-3-android-app.md#3-3-4-api-연동인증-세션)), **경로를 바꾸거나 제거하면 이미 배포된 앱에서 해당 화면이 열리지 않습니다**(`MUST` — 변경 시 앱 릴리스와 함께 조율). 세 경로는 온보딩 게이트 대상이 아니어야 합니다 — 현재 게이트 매처는 메인 3탭(`/`·`/chats`·`/my`) 정확 일치라 `/my/about`은 걸리지 않습니다. 매처를 접두사 일치로 바꾸면 앱에서 연 페이지가 온보딩으로 리다이렉트되므로 변경을 금지합니다. 또한 서비스 안내(`/my/about`)의 게스트 관련 안내는 앱 사용자에게 해당하지 않으므로 **웹 이용 기준임을 문구에 표기합니다**.
...
- **상단 헤더** — `(main)` 화면에서만 로고와 현재 섹션 라벨(`홈`·`채팅`·`마이`)을 표시합니다. 스크롤 여부와 무관하게 보더 없이 렌더합니다. 홈(`/`) 헤더는 게스트에게만 오른쪽 끝에 secondary 로그인 버튼(→ `/login`)을 노출합니다 — 세션 판별 중에는 표시하지 않아 회원에게 깜빡이지 않고, 채팅·마이 탭 헤더에는 두지 않습니다. 마이 탭 내부에도 로그인 진입점이 있습니다: 게스트는 로그인 버튼(→ `/login`), 회원은 프로필·로그아웃을 표시합니다. 게스트/회원 판정 소스는 [§3-2-4](#3-2-4-bff-프록시토큰-세션)입니다.
...
서비스이용약관·개인정보처리방침의 화면 계약과 "모든 플랫폼 동일 시행일·버전·본문" 동일성 계약은 [`3-1-client.md §3-1-3` FE-SCREEN-010](./3-1-client.md#3-1-3-화면별-스펙)이 소유합니다. 웹의 콘텐츠 정본은 웹 레포 `src/features/legal/content/terms-content.ts`·`privacy-content.ts`이며 페이지가 이를 렌더합니다(초기의 `docs/legal/*.md` 마크다운 초안은 TS 모듈로 정본을 일원화하며 삭제 — KNK-616). **안드로이드 앱은 이 웹 페이지를 인앱 브라우저(Chrome Custom Tabs)로 열어 같은 콘텐츠를 보여줍니다**(2026-08-03 결정 — [`3-3-android-app.md §3-3-4`](./3-3-android-app.md#3-3-4-api-연동인증-세션)). 콘텐츠 정본이 웹 하나로 유지되므로 동일성 계약이 구조적으로 충족되며, 별도 공유 정본을 만들지 않습니다.

### 온보딩 진입 게이트 (웹)

온보딩 노출 판정·화면 계약의 정본은 [`3-1-client.md §3-1-3` FE-SCREEN-007](./3-1-client.md#3-1-3-화면별-스펙)입니다. 웹은 그 게이트를 서버 프록시(`proxy.ts`)로 구현합니다.

- 서버 프록시가 메인 3탭(`/`·`/chats`·`/my`) 요청에서 열람 쿠키(`manyak_onboarding_seen`)와 NextAuth 세션 쿠키가 모두 없으면 원래 목적지를 `from` 쿼리에 담아 `/onboarding`으로 리다이렉트합니다. 클라이언트 리다이렉트는 홈이 먼저 그려져 화면이 깜빡이므로 첫 응답 전에 처리합니다.
...
- 온보딩 CTA는 세션스토리지(`manyak:onboarding-entry`)에 진입 기록을 남기고, 퍼널이 마운트 시 이를 소비해 온보딩 경유 여부를 판정합니다(온보딩 경유 퍼널의 뒤로가기 계약 — [`3-1-client.md §3-1-3` FE-SCREEN-007](./3-1-client.md#3-1-3-화면별-스펙)).
