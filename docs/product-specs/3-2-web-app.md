...
- **인물 이미지 스트리밍 — `Phase 2 · 구현`(KNK-1013·1015).** SSE 파서는 백엔드가 중계하는 `character_image {name, imageUrl}`를 알 수 없는 이벤트로 버리지 않고 `token`과 같은 순서로 내보냅니다. AI→백엔드 이벤트에 추가된 `imageName`은 백엔드가 웹에 중계하지 않고 웹 렌더에도 필요하지 않으므로 소비하지 않습니다. 진행 중 턴은 단일 문자열 대신 순서가 있는 `text | character_image` 조각 목록을 소유하며, 허용된 이미지 이벤트가 오면 `completed`를 기다리지 않고 즉시 렌더합니다. `completed` 뒤 상세 refetch로 받은 `aiOutput`은 엄격한 `[[URL]]` 마커를 같은 조각 목록으로 파싱하고, 진행 중 목록과 한 프레임에 교체합니다. 이어쓰기·재생성의 진행 중 렌더, 상세 턴, 공유 열람은 같은 조각 컴포넌트를 사용합니다. 저장 마커는 독립된 줄·빈 줄 하나 뒤의 비어 있지 않은 인물명 대사 라벨·허용 CDN 경로를 모두 만족할 때만 이미지로 바꾸고, 대체 텍스트에 쓸 이름은 뒤 대사 라벨에서 가져옵니다. 나머지 마커 모양 문자열은 본문으로 유지하며 이전 `[[인물이름:URL]]` 형식은 지원하지 않습니다. 텍스트 조각은 각각 문단으로, 이미지 조각은 그 문단의 형제가 되는 블록으로 렌더해 블록 이미지를 `<p>` 안에 중첩하지 않습니다. AI 메시지 컨테이너 전체에 `p-4`를 적용하지 않고 텍스트·이미지 조각 목록의 공통 래퍼에 `px-4`를 한 번 적용하며, 엔딩 배지도 별도 `px-4` 래퍼로 같은 좌우 16px 기준선을 공유합니다. 이미지 조각은 다른 썸네일과 같은 방식으로 shadcn `AspectRatio`에 `ratio={4 / 3}`을 넘기고 프레임에 `overflow-hidden rounded-xl border border-border bg-muted`를 적용합니다. 내부 `Image`는 `object-contain`으로 원본 전체를 표시합니다. 조각 목록의 `gap-5`를 기본 20px 간격으로 두고, 앞에 조각이 있는 이미지에 `mt-5`를 더해 앞 텍스트와는 40px, 뒤 텍스트와는 20px을 띄웁니다(KNK-1029). 공통 순서·실패·공유 계약은 [`3-1-client.md §3-1-5`](./3-1-client.md#3-1-5-채팅-플레이와-sse-스트리밍)가 소유합니다.
...
- (KNK-1222 웹 계획 — [`3-1-client.md §3-1-4` 제작 카드와 다중 완성 진행](./3-1-client.md#제작-카드와-다중-완성-진행--공통-계약knk-1222-android-구현--웹-계획)) 완성 요청은 이 슬롯에서 분리해 별도 키(예: `manyak:completion-requests`)의 `requestId` 배열로 보관하고, 슬롯의 `STORY_COMPLETION`은 첫 로드 때 옮깁니다. 완성 POST는 페이지 컴포넌트가 아니라 앱 셸 수준 모듈이 보내고, 퍼널은 저장 직후 `replace('/studio')`합니다. 아래 단일 슬롯·완성 폴링·채팅 `replace` 서술은 적용 전 현재 구현입니다.
...
| `/terms` `Phase 1 · 구현`               | FE-SCREEN-010 | `(legal)`   | 서비스 이용약관           | 로고 홈 링크 헤더(탭 없음) |
| `/privacy` `Phase 1 · 구현`             | FE-SCREEN-010 | `(legal)`   | 개인정보 처리방침         | 로고 홈 링크 헤더(탭 없음) |
| `/about` `Phase 1 · 구현`               | FE-SCREEN-011 | `(my)`      | 서비스 안내               | 로고 홈 링크 헤더 + 본문 `h1`(탭 없음) |
| `/share/[shareId]` `Phase 1 · 구현`          | FE-SCREEN-012 | `(share)`   | 채팅 공유 열람            | 없음(자체 헤더)        |

오버레이 화면(별도 라우트 없음): 제작 목록·채팅 목록의 카드 옵션 다이얼로그(축소판+항목, 같은 창에서 삭제 확인으로 전환 — FE-SCREEN-013·004), 스토리 상세·채팅방 헤더 옵션 메뉴+삭제 확인 다이얼로그(FE-SCREEN-003·005, [3-1-client.md §3-1-5](./3-1-client.md#3-1-5-채팅-플레이와-sse-스트리밍)), 스토리 신고 바텀 시트(네 진입점 공용, FE-SCREEN-003), 썸네일 이미지 뷰어(FE-SCREEN-003), 키워드 추가 다이얼로그, 선택 키워드 드로어, 생성 이탈 확인 다이얼로그, 스토리라인 재선택 경고 다이얼로그.
...
- **문서 제목(브라우저 탭)은 `제목 - 마냑`으로 통일합니다.** 루트 레이아웃이 제목 템플릿을 선언하므로 각 화면은 자기 제목만 선언하면 되고, 제목을 선언하지 않은 화면은 기본 문서 제목을 표시합니다 — 현재 값은 `마냑 - 나만의 스토리를 만들고 채팅으로 이어나가는 AI 스토리챗 서비스`이고, 정본은 `src/constants/site.ts`의 `DEFAULT_TITLE`입니다(카피는 변동값이라 이 문서의 값은 예시입니다). 브랜드명 단독으로는 검색엔진과 사용자가 서비스 성격을 알 수 없어 기본값에 카테고리 문구를 붙입니다(KNK-713). 고유 제목을 쓰는 화면은 스토리 제목을 쓰는 스토리 상세·채팅방·공유 열람과 약관·개인정보 처리방침·서비스 안내입니다. 법률 문서는 색인 대상이라 SERP에 구별되는 제목이 필요하며, 읽기 쉬운 띄어쓰기를 적용한 본문 제목(`서비스 이용약관`·`개인정보 처리방침`)을 탭 제목에도 재사용합니다. 정본은 `src/features/legal/content/terms-content.ts`·`privacy-content.ts`입니다(KNK-713·KNK-1037). 서비스 안내도 본문 `h1`의 정본(`src/features/about/constants.ts`)을 정적 메타데이터 제목으로 재사용해 "서비스 안내 - 마냑"을 표시합니다. 스토리 제목을 쓰는 셋은 로그인 세션·게스트 식별자나 공유 UUID로 상세를 읽어야 해 서버 메타데이터만으로는 제목이 확정되지 않으므로, 클라이언트에서 데이터를 받은 뒤 제목을 덮어씁니다 — 제목이 비어 있는 동안에는 손대지 않아 서버가 렌더한 기본 제목이 남습니다. 공유 열람은 서버가 공유본을 직접 읽어 링크 미리보기용 메타데이터도 만들지만, 그 조회가 실패해도 탭 제목은 클라이언트 덮어쓰기로 같은 형식을 유지합니다. 스토리 상세도 **오리지널 스토리에 한해** 서버가 공개 오리지널 목록에서 제목·한 줄 소개를 읽어 메타데이터(제목·description·canonical·오픈그래프, 이미지는 상세 조회의 원본 썸네일)를 만들고, 사용자 스토리나 조회 실패는 색인 차단만 담아 클라이언트 덮어쓰기에 맡깁니다(KNK-1183, 아래 색인 범위).
...
| 퍼널 완료 → 채팅 화면   | 스토리·채팅 생성 완료 | `replace`           | 동상. KNK-1222 적용 시 `replace('/studio')`로 바뀌고 채팅 생성 없음 |
...
| 온보딩 이탈 → 홈        | 헤더 "둘러보기"       | `replace`           | 동상. 로고 탭 홈 이동은 제거(KNK-1185), "나중에 하기"는 KNK-1004에서 제거 |
...
- **`(main)` 레이아웃** — 상단 헤더, 스크롤 콘텐츠 영역, 하단 네비게이션을 렌더합니다. 제작 화면의 `CreatedStoryList`는 모든 목록 상태를 감싸는 `section`에 `pb-2`(8px)를 적용합니다(하단 여백 조정, KNK-1207).
- **`(story)`·`(chat)` 그룹** — 레이아웃이 없습니다. 루트만 상속하며, 각 화면이 자체 헤더를 렌더합니다.
- **단일 프레임 flex 컬럼** — 각 화면은 루트 앱 프레임 안에서 헤더 / 스크롤 영역 / 푸터(하단 네비·CTA)로 나뉜 flex 컬럼입니다. 헤더·하단 네비·CTA는 스크롤 영역의 형제 요소라 스크롤과 무관하게 항상 제자리에 있고, 스크롤은 콘텐츠 영역 내부에서만 일어납니다. 하단 CTA 푸터는 추가 상단 패딩 없이 콘텐츠 바로 뒤에 붙이고 버튼의 가로·하단 안전 영역만 확보합니다. 스토리 생성 퍼널에서 스토리라인 선택 단계의 선택 키워드 행과 추가 정보 단계의 완성 비용 행은 공용 단계 레이아웃의 푸터 상단 슬롯으로 렌더해 CTA 행과 8px 간격을 공유하고 푸터와 함께 표시·숨김 처리합니다(KNK-1045). 스토리 상세·피드백·공유 채팅처럼 콘텐츠가 스크롤되는 화면은 `scroll-fade-b`로 경계를 만들고, 온보딩은 하단 고정 CTA 없이 헤더 / 스크롤 영역 두 요소로만 구성되며 시작 버튼과 푸터를 스크롤 콘텐츠 안에 두고, 스크롤 끝이 푸터라 페이드를 적용하지 않습니다(KNK-1185). 단, 스토리 생성 퍼널의 단계 CTA 푸터는 텍스트 입력 포커스 상태에서 `VisualViewport`(미지원 브라우저는 `window.innerHeight`)가 기준 높이보다 120px 이상 줄어들면 가상 키보드가 열린 것으로 보고 `display: none` 처리하며, 높이가 복원되면 다시 표시합니다. 포커스만으로 숨기지 않아 데스크톱 입력과 모바일 주소창 변화가 푸터를 불필요하게 감추지 않습니다(KNK-999). 스크롤 끝에서 앱 프레임 전체가 밀리는 러버밴드(macOS/iOS)가 생기지 않아야 합니다. 스크롤을 따라가지 않는 오버레이가 필요하면 콘텐츠 영역 위에 둡니다. 구현 규칙(클래스·속성)은 웹 레포 개발 문서(`AGENTS.md` 레이아웃·스크롤 절)가 소유합니다.
- **채팅 입력 비용 행** — 채팅방은 NextAuth 세션 상태가 `authenticated`일 때만 블럭·일반 입력 컴포저의 액션 행에 비용 문구를 렌더합니다. 비용 문구와 전송 버튼을 오른쪽 정렬 flex 그룹으로 묶어 두 모드에서 같은 8px 간격을 유지하고, 게스트·세션 확인 중에는 문구를 렌더하지 않습니다(KNK-1045).
...
- **상단 헤더** — `(main)` 화면에서 채팅·제작·마이의 현재 섹션 라벨을 20px semibold로 표시합니다. 홈(`/`)은 로고만 시각적으로 표시하고 `h1` "홈"은 스크린 리더 전용으로 유지합니다(KNK-1012). 스크롤 여부와 무관하게 보더 없이 렌더합니다. 홈·채팅·제작 헤더는 게스트에게만 오른쪽 끝에 secondary 로그인 버튼(→ `/login`, `buttonVariants`의 기본 `default` 사이즈·높이 40px, 2026-09-06 사용자 요청·KNK-1207)을 노출합니다 — 세션 판별 중에는 표시하지 않아 회원에게 깜빡이지 않고, 마이 탭 헤더에는 두지 않습니다. 마이 탭 내부에는 별도 로그인 진입점이 있습니다: 게스트는 본문의 로그인 버튼(→ `/login`), 회원은 프로필·로그아웃·회원 탈퇴를 표시합니다. 게스트/회원 판정 소스는 [§3-2-4](#3-2-4-bff-프록시토큰-세션)입니다.
- **회원 탈퇴 확인 화면** — `/my/account-deletion`은 헤더와 하단 CTA를 flex column의 in-flow 요소로 두고 확인 목록만 스크롤합니다. 미선택 체크박스는 1px `foreground-tertiary` 보더로 배경에서 구분하고, 체크박스와 문구 사이는 16px입니다. 하단 CTA는 활성·로딩 상태 모두 위험색 배경을 유지합니다(KNK-1052).
- **스토리 상세 헤더** — 기존 웹의 56px 높이는 유지하되 스크롤 본문 위에 absolute로 겹칩니다. 썸네일이 있으면 스크롤 위치를 히어로 높이에 대해 0~1로 정규화해 배경 불투명도와 아이콘·제목 색상을 바꾸며, 제목 자체는 본문의 `h1`이 헤더 아래로 사라졌을 때 페이드 인합니다. 별도 그라데이션 레이어는 두지 않습니다. 썸네일이 없거나 로딩·오류이면 처음부터 일반 배경 헤더로 렌더합니다(KNK-1012). 스크롤 컨테이너·히어로·본문 제목은 실제 DOM 마운트 상태를 effect 의존성에 반영해, 300ms 지연 스켈레톤과 `AnimatePresence` 전환을 거친 뒤에도 scroll listener와 제목 `IntersectionObserver`를 빠짐없이 연결합니다(KNK-1038·KNK-1039).
...
### 스토리 신고 진입점 (웹) — `Phase 2 · 구현`(KNK-1186)

신고 시트의 화면 계약(사유 3개 중 하나 필수·자유 서술 500자·성공 시 닫힘·실패 시 입력 보존·게스트 비노출)과 **채팅에서 신고해도 대상은 그 채팅이 참조하는 스토리**라는 점은 [`3-1-client.md §3-1-3` FE-SCREEN-003](./3-1-client.md#3-1-3-화면별-스펙) **스토리 신고**가 소유합니다. 웹은 진입점의 자리와 시트 구현만 여기서 정합니다.
...
안드로이드는 상세 헤더 메뉴에 신고하기와 `isOwner`일 때의 삭제하기를 두며 웹과 같은 구성입니다([`3-3-android-app.md §3-3-3`](./3-3-android-app.md)).
...
- **회원 탈퇴 뒤 세션 정리** — 브라우저는 생성된 TanStack Query 훅으로 BFF 프록시의 `DELETE /api/v1/users/me`를 호출합니다. `204`일 때만 React Query 캐시·진행 중인 생성 복구 슬롯·분석 사용자 식별자를 지우고 NextAuth `signOut`을 실행해 BFF 토큰 쿠키와 NextAuth 세션을 함께 폐기한 뒤 `/my`로 이동합니다. 탈퇴가 refresh 토큰을 먼저 무효화하므로 `signOut` 이벤트의 백엔드 로그아웃이 실패해도 로컬 쿠키 정리는 계속합니다(KNK-1052).
...
- 다이얼로그와 모달 바텀 시트는 열릴 때 포커스를 내부로 이동하되, 첫 버튼·입력란이 아니라 **팝업 자체에 두는 것을 기본**으로 합니다(KNK-715·760·1045). 첫 요소가 버튼이면 여는 즉시 포커스 링이 그려지고, 입력란이면 모바일 키보드가 함께 올라와 안내와 버튼을 가리기 때문입니다. 특정 요소에 초점이 필요한 오버레이만 `initialFocus`로 덮어씁니다. 게스트 한도와 신규 가입 초대 코드 안내는 공용 Drawer를 앱 프레임 컨테이너에 포털해 화면 아래에서 올라오는 모달 바텀 시트로 렌더합니다.
...
- 기본 서체는 Pretendard, 서사 텍스트(프롤로그·대화·스토리라인·추천 입력)는 자간 `-2%`(`letter-spacing: -0.02em`)·행간 `175%`(`line-height: 1.75`)의 MaruBuri를 적용합니다. 웹은 두 값을 공통 `.font-maruburi` 스타일 한 곳에서 관리해 사용 위치마다 행간 유틸리티가 갈리지 않게 합니다(KNK-1029).
...
원격 이미지는 Next.js 이미지 최적화 허용 목록으로 제한하되, 호스트 성격에 따라 범위를 다르게 잡습니다. Google 계정 이미지는 `lh3.googleusercontent.com` 호스트 전체를 허용합니다. 프로필 프리셋은 운영 `api.manyak.app`·개발 `dev-api.manyak.app`의 `/profile-presets/**`로 **경로까지 제한합니다** — API 호스트는 이미지 외 엔드포인트도 서빙하므로 최적화기가 임의 경로를 대신 가져오게 두지 않기 위함입니다. 이미지 CDN인 운영 `cdn.manyak.app`·개발 `dev-cdn.manyak.app`은 반대로 **경로 제한 없이 호스트 단위로 허용합니다**(KNK-1079, 2026-08-31). 이 두 도메인은 우리가 올린 이미지 자산 전용이라 경로를 나눠 얻는 이득이 적은 반면, 썸네일·인물 이미지·배경처럼 종류가 늘 때마다 설정을 고쳐야 하는 비용이 실제로 발생했기 때문입니다(썸네일 `/thumbnails/**`, 인물 이미지 `/characters/generated/**`·`/characters/originals/**`, 배경 `/backgrounds/**`). `remotePatterns` 항목에서 `pathname`을 생략하면 Next.js가 `**`로 폴백해 그 호스트의 모든 경로를 허용합니다. 새 이미지 종류가 이 CDN에 추가되면 설정을 고칠 필요가 없고, **새 호스트가 생길 때만** 항목을 추가합니다.

경로 허용 목록을 CDN에서 걷어내도 인물 이미지 방어는 그대로입니다. 인물 이미지는 백엔드가 저장·검증한 URL만 `character_image`와 `[[URL]]` 마커로 오는 계약이고 임의 사용자 URL은 허용하지 않습니다(KNK-1013). 웹은 이 설정과 별개로 렌더 직전에 HTTPS·정확한 호스트·인물 이미지 경로(`/characters/generated/`·`/characters/originals/`)를 다시 확인해, 스트림 이벤트와 저장 마커가 허용 목록 밖이면 Next 이미지 최적화 요청 자체를 만들지 않습니다(KNK-1015·1055, 정본: `manyak-web/src/features/chats/_shared/utils/chat-message-segments.ts`). 모델 출력 경로로 들어오는 URL을 실제로 막는 건 이 런타임 검사이고, `next.config.ts`는 최적화기가 대신 가져올 수 있는 호스트를 좁히는 역할입니다. 스토리 상세의 주변 인물 카드(`characters[].imageUrl`, [`3-1-client.md §3-1-3`](./3-1-client.md) FE-SCREEN-003)도 채팅과 같은 컴포넌트를 재사용하므로 같은 검사를 그대로 통과합니다(KNK-1058). 사용자 생성 스토리는 `generated`, 오리지널 스토리는 `originals` 경로를 사용합니다(KNK-1055).

환경별 호스트는 각각 등록해야 합니다. 개발 백엔드는 인물 이미지 전용 base URL(`MANYAK_CHARACTER_IMAGE_BASE_URL`)과 `thumbnailUrl`·`thumbnailUrlSm`에 `dev-cdn.manyak.app` 전체 URL을 실어 보내므로, 개발 CDN이 빠지면 개발 채팅의 저장 마커 렌더와 스토리 목록·상세 썸네일이 Next.js에서 차단돼 placeholder로 남습니다(KNK-1079). 프로필 프리셋도 환경별 base URL을 `profile_image_url`에 전체 URL로 저장하므로 개발 API 호스트가 필요합니다(KNK-388·KNK-827·KNK-832). 썸네일 파일명에는 한글이 들어갈 수 있으나(예: `.../thumbnails/generated/{uuid}/썸네일_기본_{hash}.webp`) `next/image`가 최적화 요청에서 URL 인코딩하므로 별도 처리는 하지 않습니다. 정본은 `manyak-web/next.config.ts`입니다.
...
| `stories/story-detail`                    | 상세 표시(썸네일·엔딩·턴 수·생성일)·스크롤 헤더·썸네일 뷰어·시작 상황 선택·채팅 시작 스피너·오류 상태·헤더 옵션 메뉴(신고 시트 성공/실패/게스트 비노출·내 스토리 삭제) | US-4-1·2·3·4·6·7·8, KNK-1186 |
...
| `chats/chat-list`                         | 채팅 목록·재개·빈 상태·삭제된 스토리 문구·카드 옵션 다이얼로그(축소판·삭제·신고)                        | US-5-1·2·3·4, KNK-1186    |
| `chats/chat-room`                         | 프롤로그·빈 입력의 무작위 추천 전송·스트리밍·선택지·실패·옵션 메뉴 삭제·신고·삭제된 스토리 헤더·응답 재생성·엔딩 배지 | US-5-3, US-6-1·2·3·4·8·10·13·14, KNK-1186 |
...
- 스토리 생성 퍼널이 완주 시 채팅 화면으로 진입하고, 완성 재시도가 스토리를 중복 생성하지 않아야 합니다(KNK-1222 적용 후에는 제작 화면 복귀·완성 중 카드·요청별 완료 전환으로 대체).
