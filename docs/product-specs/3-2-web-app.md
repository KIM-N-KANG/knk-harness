...
원격 이미지는 Next.js 이미지 최적화 허용 목록을 최소 경로로 제한합니다. Google 계정 이미지는 `lh3.googleusercontent.com`, 프로필 프리셋은 운영 `api.manyak.app/profile-presets/**`와 개발 `dev-api.manyak.app/profile-presets/**`, 스토리 썸네일은 `cdn.manyak.app/thumbnails/**`, 생성 인물 이미지는 운영 `cdn.manyak.app/characters/generated/**`와 개발 `dev-cdn.manyak.app/characters/generated/**`만 허용합니다. 인물 이미지는 백엔드가 저장·검증한 URL만 `character_image`와 `[[URL]]` 마커로 오는 계약이고 임의 사용자 URL은 허용하지 않습니다(KNK-1013). 웹도 방어적으로 렌더 직전에 HTTPS·정확한 호스트·`/characters/generated/**` 경로를 다시 확인해, 스트림 이벤트와 저장 마커가 허용 목록 밖이면 Next 이미지 최적화 요청 자체를 만들지 않습니다(KNK-1015, 2026-08-28). 개발 백엔드는 인물 이미지 전용 base URL(`MANYAK_CHARACTER_IMAGE_BASE_URL`)을 저장하므로 개발 CDN이 허용 목록에서 빠지면 개발 채팅의 저장 마커 렌더가 Next.js에서 차단됩니다. 프로필 프리셋도 환경별 base URL을 `profile_image_url`에 전체 URL로 저장하므로 개발 API 호스트가 필요합니다(KNK-388·KNK-827·KNK-832, 정본: `manyak-web/next.config.ts`).
