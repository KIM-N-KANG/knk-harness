...
- **스토리 상세 헤더** — 기존 웹의 56px 높이는 유지하되 스크롤 본문 위에 absolute로 겹칩니다. 썸네일이 있으면 스크롤 위치를 히어로 높이에 대해 0~1로 정규화해 배경 불투명도와 아이콘·제목 색상을 바꾸며, 제목 자체는 본문의 `h1`이 헤더 아래로 사라졌을 때 페이드 인합니다. 별도 그라데이션 레이어는 두지 않습니다. 썸네일이 없거나 로딩·오류이면 처음부터 일반 배경 헤더로 렌더합니다(KNK-1012).
