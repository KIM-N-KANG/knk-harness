...
- **단일 프레임 flex 컬럼** — 각 화면은 루트 앱 프레임 안에서 헤더 / 스크롤 영역 / 푸터(하단 네비·CTA)로 나뉜 flex 컬럼입니다. 헤더·하단 네비·CTA는 스크롤 영역의 형제 요소라 스크롤과 무관하게 항상 제자리에 있고, 스크롤은 콘텐츠 영역 내부에서만 일어납니다. 하단 CTA 푸터는 추가 상단 패딩 없이 콘텐츠 바로 뒤에 붙이고 버튼의 가로·하단 안전 영역만 확보합니다. 스토리 상세·피드백·공유 채팅처럼 콘텐츠가 스크롤되는 화면은 `scroll-fade-b`로 경계를 만들고, 온보딩처럼 내부 콘텐츠가 고정된 화면은 페이드 없이 같은 푸터 여백 규칙만 적용합니다(KNK-1012). 단, 스토리 생성 퍼널의 단계 CTA 푸터는 텍스트 입력 포커스 상태에서 `VisualViewport`(미지원 브라우저는 `window.innerHeight`)가 기준 높이보다 120px 이상 줄어들면 가상 키보드가 열린 것으로 보고 `display: none` 처리하며, 높이가 복원되면 다시 표시합니다. 포커스만으로 숨기지 않아 데스크톱 입력과 모바일 주소창 변화가 푸터를 불필요하게 감추지 않습니다(KNK-999). 스크롤 끝에서 앱 프레임 전체가 밀리는 러버밴드(macOS/iOS)가 생기지 않아야 합니다. 스크롤을 따라가지 않는 오버레이가 필요하면 콘텐츠 영역 위에 둡니다. 구현 규칙(클래스·속성)은 웹 레포 개발 문서(`AGENTS.md` 레이아웃·스크롤 절)가 소유합니다.
