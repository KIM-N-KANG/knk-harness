...
이 문서는 AI 서버, 백엔드와 모델 API가 주고받는 값과 실패를 정한다.
...
AI 서버가 API를 제공하고 백엔드가 사용한다. 백엔드 외의 호출자는 없다. 요청, 정상 응답과 422·502 오류 응답은 JSON이며 필드 이름은 `snake_case`를 사용한다. 500 응답은 실패 계약을 따른다. ([5-4 실패 계약](#5-4-실패-계약))

백엔드는 응답을 동기로 기다린다. 대기 한도는 시간 계약을 따른다. ([5-6 시간과 복구](#5-6-시간과-복구))
...
SDK 재시도와 AI 결과 보완은 복구 구분을 따른다. ([7-1-3 복구 구분](7-1-ERROR-HANDLING.md#7-1-3-복구-구분))
...
호출부는 공급자 SDK를 직접 호출하지 않고 공통 통로만 사용한다. 공통 통로는 모델 이름으로 공급자와 어댑터를 정한다.
...
컴파일 호출부는 이미지 생성 예외를 정해진 이미지 `error`로 바꾼다. ([5-4 실패 계약](#5-4-실패-계약))
