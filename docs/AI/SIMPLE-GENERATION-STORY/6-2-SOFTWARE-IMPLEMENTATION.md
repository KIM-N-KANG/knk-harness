...
본 문서는 ([6-1 AI 실행](6-1-AI-EXECUTION.md))에서 정의한 실행 절차를 구현할 파일, 컴포넌트와 의존성 구조를 정한다.

구성 요소를 실제 패키지와 파일에 배치하고, 공개 함수의 입력과 반환 타입 및 데이터 모델을 명시한다. 의존성과 외부 라이브러리의 관리 방식, 설정과 초기화 규칙, 네이밍과 예외 처리 규칙 및 금지 패턴을 정한다.
...
구성 요소별 구현 파일은 다음과 같다. ([4-2-3 소프트웨어 구조 설계](4-2-DESIGN-SOFTWARE-ARCHITECTURE.md#4-2-3-구성-요소))

- 위 파일은 모두 구현된 상태다. 평가 CLI는 미구현이며 별도의 평가 구현 문서에서 설계를 정한다. ([6-5 AI 평가 구현](6-5-AI-EVALUATION-IMPLEMENTATION.md))
- 채팅과 공유하는 파일을 바꾸면 채팅 테스트도 함께 실행한다.
- 의존 방향은 `api` → `services` → 호출 모듈 → 공급자 SDK다. 모든 계층에서 `schemas`와 `core`를 가져올 수 있다. 호출 모듈과 어댑터에서는 `api`와 스토리 서비스를 가져오지 않는다.
...
| `generate_storylines` | `services/story_llm.py` | `StorylinesRequest` | `StorylinesResponse` | 예 | 502 `HTTPException` <br> 예외의 `retry_count` 속성에 재호출 횟수 |
...
| LLM 호출 모듈 | `LlmRequest`, `LlmResult`, `TokenUsage` | 값을 바꿀 수 없는 dataclass | `src/services/llm/base.py` |
| 이미지 생성 모듈 | `ImageRequest`, `ImageResult` | 값을 바꿀 수 없는 dataclass | `src/services/image/base.py` |
...
| 공급자 SDK 응답과 예외 → 호출 모듈의 공용 타입 | 각 어댑터 |

<br>

**예외**

오류 코드와 응답 문구는 실패 계약, 복구 기준은 오류 처리를 따른다. ([5-4 계약](5-CONTRACT.md#5-4-실패-계약), [7-1 오류 처리](7-1-ERROR-HANDLING.md))
...
스토리라인 생성과 컴파일은 서비스가 요청 모델을 받아 프롬프트 조립, 모델 호출, 결과 검증과 보완, 호출 메타데이터를 포함한 최종 응답 조립을 맡는다. 라우터는 요청을 서비스에 전달하고 완성된 응답을 반환하며, 요청 단위 관측을 맡는다.
...
- 어댑터는 호출 모듈 안에서 필요할 때 가져온다. 호출 모듈의 맨 위에서 가져오면 관측 모듈과 순환 참조가 생긴다.
...
| 모델 등록부 | `src/services/llm/registry.py` | LLM 호출 모듈만 접근 |
...
모델 호출 경로를 검증할 때는 SDK 클라이언트를 테스트용으로 교체한다. 호출 모듈이나 호출부를 교체하면 인자 조립과 예외 변환을 검증할 수 없다.
...
- 공급자 SDK는 어댑터 파일에서만 가져온다. 스토리 서비스와 프롬프트 빌더는 호출 모듈과 해당 모듈의 공용 타입만 가져온다.
...
모듈 내부의 함수 분할, 자료 구조와 로그 문구는 구현자가 정한다. 공개 함수의 입력과 반환, 호출 모듈의 경계, 예외 분류와 상수의 위치는 바꾸지 않는다.

템플릿 치환은 프롬프트 조립, 실행 설정과 공급자별 인자 대응은 모델 호출을 따른다. ([6-1-4 AI 실행](6-1-AI-EXECUTION.md#6-1-4-프롬프트-조립), [6-1-5 AI 실행](6-1-AI-EXECUTION.md#6-1-5-모델-호출))
...
| 타입 | 모든 함수에 타입 힌트 작성, `Any` 지양 <br> 호출 모듈의 요청과 결과는 값을 바꿀 수 없는 dataclass <br> `LlmUsage`의 `provider`는 기본값 없이 키워드 인자로만 전달 |
...
| 상수 | 출력 한도, temperature, 보완 횟수, 시간 한도는 스토리 서비스 상단의 상수로 관리 <br> 값은 모델 호출 설정과 일치 |
...
시작 검사는 각 호출 모듈의 `validate_startup`이 맡는다.

| 검사 대상 | 통과 조건 |
|---|---|
| 텍스트 모델 | 모델이 등록부에 있음 |
| 공급자 API 키 | 비어 있지 않음 <br> 공백, 개행, 비ASCII 문자가 없음 |
| 공급자 주소 | 주소를 지정했다면 `http` 또는 `https`이고 호스트가 있음 |
| 추론 설정 | 어댑터가 모델의 추론 설정을 인자로 표현할 수 있음 |
| 이미지 모델과 설정 | 모델이 등록돼 있음 <br> 화질, 크기, 제한 시간 형식이 맞음 |

사용 가능한 모델, 공급자, 추론 설정과 허용 인자는 `src/services/llm/registry.py`에 정의한다.

| 담당 모듈 | 구현 위치 |
|---|---|
| LLM 호출 모듈 | `src/services/llm/__init__.py` |
| 이미지 생성 모듈 | `src/services/image/__init__.py` |
