...
1. `messageId`별 Redis 키를 확인합니다. 완료 키가 있으면 재발송하지 않습니다. 키가 없으면 `SET NX`로 처리 중 상태와 짧은 유효시간을 함께 기록해 선점합니다. 유효시간은 한 번 처리의 최대 소요 시간보다 길게 설정하며 구체적인 값은 구현 시 확정합니다. 다른 소비자가 처리 중이면 발송하지 않고 `RETRY`를 반환합니다. 처리 중 소비자가 종료되면 유효시간이 끝난 뒤 다시 선점해 처리합니다.
