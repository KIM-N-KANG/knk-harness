...
- **원문은 서버에 두지 않습니다.** 정본은 웹의 법적 콘텐츠 소스([웹 설계](../design/1-1-web-design.md#법적-콘텐츠-소스-웹))이고 서버는 **현행 버전만** 설정값으로 압니다: `manyak.legal.terms-version`(현재 `v1.2`, 2026-09-01 시행) · `manyak.legal.privacy-version`(현재 `v1.7`, 2026-09-25 시행). 버전 문자열은 웹 콘텐츠의 `version` 값을 그대로 씁니다. 만 14세 확인은 문서가 아니라 선언이라 버전을 `1`로 고정합니다. 문서를 개정하면 웹 콘텐츠와 이 설정값을 **같은 릴리스에서** 올립니다: 어긋나면 사용자가 보지 않은 버전에 동의한 기록이 생깁니다.
...
| 엔드포인트 | 요청 | 응답 |
| --- | --- | --- |
| `GET /users/me/consents` | 없음 | 200 `{ "terms": { "requiredVersion": "v1.2", "needsConsent": boolean }, "privacy": { "requiredVersion": "v1.7", "needsConsent": boolean }, "age14": { "requiredVersion": "1", "needsConsent": boolean } }` |
| `POST /users/me/consents` | `{ "terms": "v1.2", "privacy": "v1.7", "age14": "1" }` 각 필드 선택 | 200 갱신 후 상태(GET과 같은 스키마) |
