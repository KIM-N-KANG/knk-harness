...
Phase 1 보상·소모와 Phase 3 결제 범위 용어입니다. 이프는 회원 전용이며, 게스트는 이프 없이 디바이스 ID별 체험 한도로 사용하고 한도 소진 후 로그인으로 유도합니다. 계약·구조와 수치는 [`4-backend-server-spec.md §4-3-7`](4-backend-server-spec.md)이 정본입니다.
