...
- 목적·적용 범위를 확인한 뒤 해당 도메인의 용어와 표기 규칙을 찾습니다.
- 새 이름을 정할 때는 기존 용어와 계층별 네이밍 규칙을 함께 확인합니다.

## 목차

- [0-1. 목적과 적용 범위](#0-1-목적과-적용-범위)
- [0-2. 용어 결정 원칙](#0-2-용어-결정-원칙)
- [0-3. 핵심 도메인 용어](#0-3-핵심-도메인-용어)
- [0-4. 계층별 표기 컨벤션](#0-4-계층별-표기-컨벤션)
- [0-5. 동명이의·예약어 규칙](#0-5-동명이의예약어-규칙)
- [0-6. AI 서버 내부 용어](#0-6-ai-서버-내부-용어)
- [0-7. 참고 근거](#0-7-참고-근거)
...
| 오리지널 스토리 | original story | 마냑 공식 계정이 소유한 공개(`PUBLISHED`·`PUBLIC`) 스토리. 제작 없이도 플레이할 거리를 주려고 홈(`/`)에 노출한다(Phase 1 — KNK-932·975·983). 홈은 공개 스토리 목록의 "오리지널" 필터로 오리지널만 모아 보고, 내가 만든 스토리는 제작 탭(`/studio`)이 담당한다(KNK-988 화면 분리·KNK-994 URL 변경·KNK-1421 필터 — [`3-1-client-spec.md §3-1-3`](3-1-client-spec.md#3-1-3-화면별-스펙)). 조회는 `GET /stories?filter=original`이며 인증이 필요 없다(`GET /stories/originals`는 폐기 예정) | "공식 스토리", "추천 스토리"(추천 로직이 없음), "마냑 스토리" |
...
| 키 문장 | `key_sentence` | 주요 사건마다 두는 기준 문장. 사용자 입력이 키 문장과 관련되면 해당 주요 사건으로 전개하며, 관련성 판정은 AI의 정성 판정이다([`5-ai-server-spec.md §5-3-4`](5-ai-server-spec.md))(Phase 1) | "트리거 문장"(키워드북 트리거와 혼동) |
| 목표 사건 | `target_main_event` | 채팅이 현재 향해 진행 중인 주요 사건(채팅당 최대 1개). 진행 턴 수(`progress_turns`)와 함께 백엔드가 채팅 단위로 저장하고, 판정은 AI가 한다([`4-backend-server-spec.md §4-3-10`](4-backend-server-spec.md))(Phase 1) | "진행 중 사건" · "현재 사건" |
| 엔딩 | `ending` | 스토리의 종결 정의. Phase 1부터 시작 설정당 최대 10개로 두고(유형 없이 이름으로 식별), 본문을 제작 시 쓰지 않고 이름 · 조건 · 에필로그 가이드로 정의해 도달 시 생성한다. 간편 제작 컴파일은 3개를 생성한다. 스키마·도달 계약은 [`4-backend-server-spec.md §4-3-10`](4-backend-server-spec.md), 현행 구현(제목·내용·`condition_text`)은 레거시로 비활성 보존 | — |
...
| 썸네일 | thumbnail | 스토리의 대표 이미지. 등록할 때 팀 프리셋을 자동 연결한다. 간편 제작은 컴파일이 생성한 이미지를 우선 사용하고 생성에 실패하면 프리셋을 사용한다(KNK-1047). 스토리 소유자가 업로드한 이미지는 생성 이미지를 대체한다(KNK-1126). 공개 노출 시 검수 조건을 통과하지 못한 생성·업로드 이미지는 프리셋으로 대체한다 | "커버". 와이어는 `thumbnailUrl`과 `thumbnailUrlSm`을 사용한다 |
| 스토리 이미지 | `story_images` | 일반 제작·수정에서 스토리에 등록하는 이미지(스토리당 최대 10). 이름 · 트리거 문구(`trigger_text`) · 이미지 키로 구성하며, 채팅 중 표시 규칙은 [`4-backend-server-spec.md §4-3-9`](4-backend-server-spec.md)가 정의(Phase 1) | — |
| 인물 이미지 이름 | `image_name`(AI 요청) · `imageName`(SSE·응답) | 한 인물의 이미지 한 장을 구분하는 이름(예: `세린_기본`). 컴파일 AI가 지어 `character_images[].image_name`으로 보내고(`name`은 인물 이름 그대로), 백엔드가 uuid를 붙여 파일명으로 쓰고 `story_characters.image_name`에 저장한다. 채팅 요청·이벤트에서는 받은 값을 그대로 돌려주며 비어 있으면 빈 값이다. AI 서버는 구현(KNK-1014), 백엔드 저장·전달은 `Phase 2 · 계획`([`4-backend-server-spec.md §4-3-9`](4-backend-server-spec.md) 변경 계획) | "표정"(이름의 일부일 뿐 이름 자체가 아님) |
...
| 일반 제작 | general creation | 제작 폼에 스토리 구성 항목을 직접 입력하는 제작 방식(Phase 1). 기본 정보 · 스토리 설정 · 시작 설정 · 추천 입력을 직접 입력하며, 주요 사건·엔딩은 채팅 런타임 반영 계약([`4-backend-server-spec.md §4-3-10`](4-backend-server-spec.md))이, 이미지는 썸네일·채팅 이미지 표시 계약([`4-backend-server-spec.md §4-3-9`](4-backend-server-spec.md))이 확정됐다 | "일반 모드" · "일반 모드 제작". 채팅 입력 모드 "일반 입력"(`NORMAL`)과 다른 개념(§0-5) |
...
| AI 트레이스 연결 ID | `trace_creation_id` | 간편 제작 **스토리라인 생성 요청**의 클라이언트 생성 UUID(`story_creation_requests.request_id`). Langfuse 트레이스를 진행 단위로 묶는 열쇠이며 와이어 키는 `X-Manyak-Creation-Id`·`ai_call_logs.creation_id`입니다([`4-backend-server-spec.md §4-7`](4-backend-server-spec.md)). 위 진행(세션) ID와 **다른 값**입니다 — 세션 PK는 스토리라인 AI 호출이 성공해야 생겨 그 최초 호출 시점에는 forward할 수 없습니다 | 위 제품 분석 ID와 혼동 금지. 두 값을 직접 조인하지 않기 |
...
| 좋아요 | `like_count` | 스토리에 대한 좋아요 수. 실 집계 전환·등록/취소 API는 KNK-1024([`4-backend-server-spec.md §4-3-1`](4-backend-server-spec.md)) | — |
...
| 실시간 이미지 | `realtimeImage` | 채팅 턴 진행 중 AI가 그 장면에 맞춰 새로 생성하는 이미지. [AI 스펙](5-ai-server-spec.md#부모-이미지와-자식-이미지)의 “자식 이미지”와 같은 대상이며 클라이언트·백엔드 계약에서는 실시간 이미지(`realtimeImage`)를 쓴다. 인물의 기본 이미지(AI 스펙의 “부모 이미지”)와 구분한다 | 클라이언트·백엔드 계약에서 “자식 이미지” 표기 |
...
| 공식 용어 | 영문 식별자 | 정의 | 지양 표기 |
| --- | --- | --- | --- |
| 게스트 | guest | 로그인 없이 서비스를 쓰는 사용자(제품·UI 관점). 웹에서는 공개 홈·공개 스토리·공유 열람·서비스 안내·법적 문서 열람만 할 수 있고, 제작·채팅은 로그인과 필수 동의 뒤에 이용한다(2026-09-19) | "비로그인 사용자"는 설명문에서 허용 |
...
Phase 1 보상·소모와 Phase 3 결제 범위 용어입니다. 이프는 회원 전용입니다. 웹 게스트 체험 한도 이용은 2026-09-19부터 신규 경로에서 제공하지 않으며, 체험 잔여는 회원의 비용 표시에만 남습니다. 계약·구조와 수치는 [`4-backend-server-spec.md §4-3-7`](4-backend-server-spec.md)이 정본입니다.
...
| 회수 | `PURCHASE_REVERSAL` | `Phase 3 · 계획` 결제 환불에 따라 구매 로트 잔여를 줄이는 음수 원장 사유. 잔액 마이너스 금지, 이용내역 분류는 `EXPIRE` | AI 실패 후 재적립인 `REFUND`와 혼용 금지 |
| 출석체크 | attendance | KST 자정 기준 하루 1회 접속 보상을 받는 행위 | "데일리 체크인" |
| 초대 코드 | invite code, `invite_code` | 다른 사용자를 초대해 보상을 받는 코드. 다른 회원이 코드를 입력하면 양쪽에 적립한다(계정당 평생 1회 입력, 다른 회원이 내 코드를 입력해 내가 받는 초대자 보상은 KST 월 10회 상한 — KNK-567 개편, [`4-backend-server-spec.md §4-3-7`](4-backend-server-spec.md) 결정 기록) | "초대 링크"(링크 어트리뷰션 방식은 KNK-567로 폐기) · "추천인 코드" |
| 체험 한도 | trial limit | 이프 없이 쓸 수 있는 사용량 상한(서버 `GET /users/me/trials`). Phase 1 기준은 스토리라인 생성·재생성 5회, 스토리 간편 제작(컴파일) 1회, 모든 채팅방 합산 채팅 턴 5회(재생성 포함)다. 일반 제작 등록은 제외한다. 웹 게스트 선차단에는 더 이상 쓰지 않고(2026-09-19) 회원의 무료 체험 적용가 표시에 쓴다 | "무료 이프"(게스트는 이프가 없음) |
...
| 화자 라벨(인물명 라벨) / 지문 | `인물명: 대사` 표기 / `*별표*` 행동 묘사 — 프론트·백엔드·AI가 공유하는 와이어 표기 규약. 변경 계획(KNK-1002): AI 서버가 줄 머리의 이 라벨을 직접 찾아 인물 이미지 위치를 정한다(LLM 태그 대체, [`5-ai-server-spec.md §5-3-4`](5-ai-server-spec.md)) |
