...
| 게시물 검수 기준 | KNK-1120의 2026-09-21 승인 계약입니다. §5-3-6·§5-9-6은 API 미구현·실측 미실시이며 위 기준 코드의 구현과 구분합니다. |
...
명백한 위반이 있거나 이미지 읽기·최종 모델 호출에 실패하면 거절합니다.
OpenAI 호출 실패 시 DeepSeek로 대체 호출하고, 대체 호출이 성공하면 그 검수 결과를 반환합니다.
OpenAI가 정상적으로 반환한 내용 거절은 대체 호출 사유가 아닙니다.
...
| 게시물 검수 이미지 읽기·최종 모델 호출 | `REJECTED`, `issues: []`, 전용 `error_code`. [응답 계약](#5-9-6-게시물-검수) 참조 |
...
HTTP 경로·실행 실패의 HTTP 상태·전체 시간 예산·세부 재시도 횟수는 이 계약에서 확정하지 않습니다.
이미지 읽기 실패와 모델 호출 실패가 함께 발생한 경우의 오류 우선순위도 구현 계약에서 정해야 합니다.
입력 제한은 게시물에서 허용하는 제한을 따르며 별도 임의 상한을 추가하지 않습니다.
...
`thumbnailUrl`과 인물 `imageUrl`의 실제 이미지를 검사하며 URL 문자열만 보고 판정하지 않습니다.
...
최상위 필드를 `decision`, `issues`, `error_code` 순서로 직렬화합니다.
...
| 이미지 다운로드·읽기 실패 | `REJECTED` | `[]` | `IMAGE_READ_FAILED` |
| OpenAI·DeepSeek 모두 호출 실패 | `REJECTED` | `[]` | `MODEL_CALL_FAILED` |

이미지 읽기 실패도 `issues`에 유해성 위반이나 빈 필드의 문제 객체를 만들지 않습니다.
...
```json
{"decision": "APPROVED", "issues": [], "error_code": null}
```

최종 모델 호출 실패:

```json
{"decision": "REJECTED", "issues": [], "error_code": "MODEL_CALL_FAILED"}
```
...
- 실행 실패는 `REJECTED`·빈 `issues`·오류 코드로 반환하며 두 공급자의 실패를 승인으로 바꾸지 않습니다.
