...
| 게시물 검수 기준 | KNK-1120의 2026-09-21 승인 계약을 2026-09-23 구현 전 합의로 갱신했습니다(실패 코드 세분화·`storyId`·호출 횟수·시간 제한·HTTP 상태). §5-3-6·§5-9-6은 API 미구현·실측 미실시이며 위 기준 코드의 구현과 구분합니다. |
...
**승인된 계약이며 API 미구현·실측 미실시입니다.** 게시물의 품질이 아니라 금지 내용의 유무와
게시 가능 여부를 판단합니다. 백엔드가 저장된 게시물의 검수 대상 텍스트와 이미지 URL을 모아
한 요청으로 전달합니다. 공개 설정·최소 턴 수·ID·처리 상태는 제외합니다.
...
- **이미지 준비.** 서버가 `thumbnailUrl`·`imageUrl`의 이미지를 내려받아 실제 파일 내용으로 형식과
  크기를 확인한 뒤 모델에 전달합니다. 내려받기 실패, 이미지가 아닌 파일, 지원하지 않는 형식
  (JPEG·PNG·GIF·WebP 외), 장당 32MiB 초과는 모델을 호출하지 않고 각각의 오류 코드로 거절합니다.
- **모델 호출.** 시간 초과·공급자 오류·응답 거부·JSON 형식 불일치는 모두 호출 실패로 세고 다음 모델로
  넘어갑니다. 정상적으로 반환한 내용 거절은 대체 호출 사유가 아닙니다. 대체 호출이 성공하면 그 결과를
  반환합니다. 두 호출이 모두 실패하면 `MODEL_CALL_FAILED`입니다.
- **응답 검증.** 모델이 돌려준 문제 항목 중 입력에 없는 경로는 버립니다. 버린 뒤 거절 근거가 남지 않으면
  승인하지 않고 `MODEL_CALL_FAILED`로 거절합니다. 모델이 판독 불가라고 알린 이미지는 위반이 아니라
  `IMAGE_UNREADABLE`로 거절합니다.
...
| 게시물 검수 이미지 준비·모델 호출 | `REJECTED`, `issues: []`, 전용 `error_code`·`error_path`. HTTP는 200. [응답 계약](#5-9-6-게시물-검수) 참조 |
...
게시물 검수의 승인된 관측 계약은 Langfuse에서 최종 `decision`·`issues`·`error_code`와
모델 호출 기록을 확인하는 것입니다. 문제 항목의 `type`으로 텍스트·이미지를 구분하고,
기본 호출과 대체 호출의 결과·모델·공급자·사용량·지연을 같은 요청에 연결합니다.
요청의 `storyId`를 그 기록에 붙여 백엔드가 게시물 단위로 검수 기록을 찾을 수 있게 합니다.
...
게시물 검수는 API·이미지 전달·대체 호출·Langfuse 연결 및 실제 모델 품질 검증이 미완료입니다.
경로·HTTP 상태·호출 횟수·시간 제한·이미지 실패와 모델 실패의 순서는 [§5-3-6](#5-3-6-게시물-검수)에서
확정했습니다. 입력 제한은 게시물에서 허용하는 제한을 따르며, 이미지 장당 32MiB와 지원 형식만 서버가
추가로 확인합니다. 백엔드의 호출 대기 시간은 요청 전체 상한 150초보다 길어야 하며 그 값은 백엔드 명세가
정합니다. 평가 세트로 오탐·미탐을 측정하는 절차는 두지 않기로 했으며, 실측은 이미지 입력과 JSON 출력이
...
[동작과 검수 기준](#5-3-6-게시물-검수). 승인된 입력·응답 계약이며 API는 미구현입니다.

**입력**

게시물 구조의 camelCase 필드명을 유지합니다. 백엔드가 부분 수정 요청이 아니라 저장된 전체
검수 대상을 전달합니다. 없는 선택 항목은 생략할 수 있습니다. 필수 여부·텍스트 길이·배열 개수는
...
`thumbnailUrl`과 인물 `imageUrl`은 백엔드가 준 공개 서빙 URL이며, AI 서버가 허용된 호스트에서만
내려받아 실제 이미지를 검사합니다. URL 문자열만 보고 판정하지 않습니다.
최상위 `storyId`는 백엔드의 스토리 식별자입니다. 검수 판단에는 쓰지 않고 관측 기록 연결에만 씁니다.
...
최상위 필드를 `decision`, `issues`, `error_code`, `error_path` 순서로 직렬화합니다.
...
| `issues` | `array<object>` | 확인된 내용 위반 목록. 정상 승인 또는 실행 실패이면 빈 배열입니다. |
...
| `error_path` | `string` 또는 `null` | 이미지 실행 실패에서 해당 이미지의 입력 경로. 그 외는 `null`입니다. |
...
```json
{"decision": "APPROVED", "issues": [], "error_code": null, "error_path": null}
```

이미지 파일 불량:

```json
{"decision": "REJECTED", "issues": [], "error_code": "IMAGE_INVALID", "error_path": "characters[0].images[0].imageUrl"}
```

최종 모델 호출 실패:

```json
{"decision": "REJECTED", "issues": [], "error_code": "MODEL_CALL_FAILED", "error_path": null}
```
...
- OpenAI 호출 실패 뒤 DeepSeek가 성공하면 최종 검수 결과와 `error_code: null`을 반환합니다.
- 실행 실패는 HTTP 200에 `REJECTED`·빈 `issues`·오류 코드로 반환하며 두 공급자의 실패를 승인으로 바꾸지 않습니다.
- 내려받기 실패·파일 불량은 모델을 호출하지 않고 거절하며, 이미지 오류에는 `error_path`가 있습니다.
