...
| 게시물 검수 기준 | KNK-1120의 승인 계약을 2026-09-26 합의로 갱신했습니다(이미지 오류 목록·정상 이미지 검수 계속·백엔드 입력 검증 책임). §5-3-6·§5-9-6은 dev 반영 전 계약이며 실측 미실시입니다. 위 기준 코드의 구현과 구분합니다. |
...
| 자식 이미지 | 채팅 턴 내부 호출 | 부모 이미지·최근 대화·업로드 슬롯 → S3 직접 업로드·최종 URL, 실패 시 부모 대체 | Langfuse `이미지 생성:자식` 및 채팅 결과 기록 |
...
인증은 백엔드 경계가 담당합니다. 요청·REST 응답은 snake_case이며, SSE `completed`와 이미지 이벤트의 필드는 camelCase입니다. 성공 응답의 `meta`는 [운영과 관측](#5-6-운영과-관측)을 따릅니다. 요청 스키마 위반은 422입니다. 게시물 검수의 입력 필드명과 응답은
[전용 계약](#5-9-6-게시물-검수)을 따릅니다. 해당 입력은 게시물의 camelCase를 유지하고 응답에 `meta`를 추가하지 않습니다.
...
flowchart LR
    ...
    subgraph C["채팅 입력"]
        ...
        C_item4["<div style='width:240px;text-align:center;'><span>백엔드가 발급한 업로드 슬롯</span></div>"]
    end
    SELECT["부모가 있는 첫 화자 선택"]
    CHILD["부모 파일 + 대화로<br/>자식 이미지 생성"]
    RESULT["AI: 발급받은 주소로 S3 업로드<br/>성공이면 자식 URL, 실패면 부모 URL 선택"]
    subgraph O["채팅 출력"]
        direction LR
...
그림은 부모가 준비되어 있고 업로드 슬롯이 있는 채팅 흐름입니다. 슬롯이 없거나
...
flowchart LR
    ...
    subgraph O["자식 이미지 출력"]
        ...
        O_item2["<div style='width:240px;text-align:center;'><span>1024×768 WebP → AI가 S3 업로드 → 공개 URL</span></div>"]
    end
    I --> P
    P --> M
    R -->|참조 파일 첨부| M
...
| --- | --- |
| 자식 이미지 데이터 | WebP 데이터를 백엔드가 발급한 주소로 업로드합니다. base64는 내부에서만 사용하고 응답에는 넣지 않습니다. |
| 토큰 사용량 | 응답에 있으면 Langfuse에 기록합니다. 사용량이 없으면 0으로 추정하지 않습니다. |
| 오류 또는 시간 초과 | 부모 이미지 이름·URL을 응답하며 실패 이유는 관측에 기록합니다. |

**백엔드는 업로드 주소와 공개 주소를 발급하고, AI는 생성·업로드 결과에 따라 자식 또는 부모
이미지의 이름·URL을 응답합니다.** 이미지 모델은 저장 주소나 채팅 이벤트를 만들지 않습니다.

`image_slots`는 최대 한 개입니다. 생략하거나 빈 배열이면 기존 부모 이미지 스트리밍을 사용합니다.
null·두 개 이상·필수 필드 누락·잘못된 주소는 422입니다. 슬롯은 비어 있지 않은 `key`,
임시 쓰기 권한이 포함된 `upload_url`, 공개 주소 `public_url`을 받습니다. 두 주소는 HTTPS이며
사용자 정보·fragment·공백·443 외 포트를 허용하지 않습니다. 서명 문자열은 요청 모델에서 보존합니다.
AI는 `key`로 주소를 조립하거나 두 주소가 같은 파일인지 검사하지 않습니다.
`generate_child_image`는 기본 false인 호환 필드로만 받고 생성 여부에는 사용하지 않습니다.

채팅 턴·선택지 API의 요청 검증 오류는 HTTP 422이며, `detail` 배열의 각 항목에는
오류 종류 `type`·위치 `loc`·설명 `msg`만 포함합니다. 서명 URL이 응답에 포함되지 않도록
`input`·`ctx`는 제외합니다. 슬롯 오류뿐 아니라 일반 필수 필드 누락에도 같은 규칙을 적용합니다.

슬롯이 있으면 다음 순서로 처리합니다.

1. 본문 전체가 완성되면 **부모 이미지가 있는 인물 중 처음 말한 인물** 한 명을 선택합니다.
   앞서 말한 인물에게 부모 이미지가 없으면 건너뜁니다. 대상이 없으면 이미지 생성은 생략합니다.
2. 업로드 호스트가 허용 목록에 있는지 먼저 확인합니다. 미설정·허용되지 않은 호스트이면
   부모 다운로드·유료 생성·업로드를 하지 않고 부모 이미지를 사용합니다.
   선택한 부모 이미지와 현재 턴을 포함한 최근 최대 3턴으로 자식 이미지를 한 장 생성합니다.
   이전 이력은 인접한 USER·ASSISTANT 한 쌍을 한 턴으로 세어 최근 2쌍을 사용합니다.
   짝 없는 오프닝·메시지는 세지 않습니다. 현재 턴에는 사용자 입력과 완성된 본문 전체를 넣습니다.
3. 본문 완성 후 사건·엔딩 판정과 이미지 처리를 동시에 시작합니다. 이미지 생성·업로드 결과가
   정해질 때까지 앞 지문을 포함한 모든 텍스트 전송을 기다립니다. 결과가 정해지면 본문의
   지문 → 이미지 → 대사 → 뒤 지문 순서를 유지하면서 글을 작은 조각으로 나눠 전송합니다.
   이미지를 본문 맨 앞으로 옮기지 않습니다. 생성 대상이 없어도 글은 나눠 전송합니다.
   `completed`는 본문 전송과 판정까지 끝난 뒤 보냅니다. 다른 인물은 요청의 기본 이미지를 우선
   사용하고 기본 이미지가 없으면 기존 매핑을 사용합니다. 별칭 충돌은 전체 매핑으로 판단합니다.
4. 생성한 파일을 `upload_url`로 PUT합니다. Content-Type은 `image/webp`이며 재시도·리다이렉트
   없이 HTTP 2xx를 성공으로 처리합니다. AI는 응답 본문·S3 HEAD·공개 주소 조회로 검증하지 않습니다.
5. 성공하면 해당 `character_image`에 자식 이름과 `public_url`을 넣습니다. 생성·업로드 실패나
   시간 초과이면 부모 이름·URL을 유지합니다. `generatedImage`·base64·실패 코드는 전송하지 않습니다.
   본문 실패는 `error`로 종료하며 이미지·판정을 호출하지 않습니다. 본문 수집·이미지·판정 대기
   중에는 10초마다 `ping`을 보냅니다. 이미지 내부 오류가 뒤 대사나 채팅 완료를 중단하지 않습니다.

이미지는 부모 다운로드·생성·업로드를 합쳐 최대 30초, 판정은 최대 60초 기다립니다. 두 호출은 각각
턴 전체 120초에서 경과 시간과 저장·완료 처리 여유 15초를 뺀 시간까지만 사용합니다. 본문
수집과 이미지 처리에는 본문 순차 전송 시간을 미리 남겨 둡니다. 텍스트·이미지 전송 중 전체
마감 시각을 넘으면 남은 글을 한꺼번에 보내지 않고 `error`로 종료하며 `completed`는 보내지 않습니다.
이미지의 시간 초과는 전달 시각이 아닌 업로드까지 끝난 시각으로
판단합니다. 연결이 끊기면 진행 중인 호출을 취소하고 정리하며, 채팅을 나간 뒤 계속 생성하지
않습니다. 15초 여유는 백엔드 저장 완료 시간을 보장하지 않습니다.

**AI의 이미지 이벤트와 `completed.aiOutput`의 마커·`characterImages`는 같은 최종 이미지를
가리킵니다.** 선택한 인물의 매핑만 바꾸므로 같은 부모 URL을 쓰는 다른 인물은 유지합니다.
이미지 결과를 기다린 뒤 앞 지문부터 전송하며 부모를 먼저 보여 줬다가 자식으로 교체하지 않습니다.
글은 완성된 본문을 나눠 보내며, 모델이 생성 중인 토큰을 즉시 전달하는 방식은 아닙니다.
슬롯이 없는 요청은 기존 실시간 본문 스트리밍을 유지합니다. 결정 근거는
[이미지 대기 후 순차 전송](../adr/3-ai-server-adr.md#이미지-결과를-기다린-뒤-본문을-순차-전송)을 따릅니다.

백엔드는 같은 파일을 가리키는 업로드·공개 주소를 발급하고 최종 본문·이미지 참조를 해당 턴과
응답 버전에 저장합니다. 자식 파일 재업로드나 부모 주소를 자식 주소로 바꾸는 작업은 필요하지
않습니다. 재생성은 이전 파일을 덮어쓰지 않도록 저장 경로와 공개 주소를 구분해야 합니다.
저장 검증·과금·삭제는 백엔드 책임입니다. AI의 업로드 취소는 이미 저장된 파일을 삭제하지 않습니다.
결정 근거는 [자식 이미지 직접 업로드 결정](../adr/3-ai-server-adr.md#자식-이미지를-ai가-직접-업로드)을 따릅니다.
...
**승인된 계약이며 dev 반영 전·실측 미실시입니다.** 게시물의 품질이 아니라 금지 내용의 유무와
게시 가능 여부를 판단합니다. 백엔드가 저장된 게시물의 검수 대상 텍스트와 이미지 URL을 모아
한 요청으로 전달합니다. 공개 설정·최소 턴 수·ID·처리 상태는 판단에서 제외합니다.
`storyId`는 관측 기록 연결에만 사용합니다.
요청 구조와 결과 필드는 [§5-9-6](#5-9-6-게시물-검수)을 따릅니다.

텍스트와 실제 이미지, 이미지 속 문구와 인물 설정의 결합 맥락을 함께 검사합니다.
입력에서 직접 확인한 내용이 아래 조건을 명백히 충족할 때만 내용 위반으로 거절합니다.
애매한 수위·단순 언급·소재의 등장·추정한 의도만으로는 차단하지 않습니다.
문장력·재미·맞춤법·설정 완성도·영화 관람 등급은 평가하지 않습니다.

| `rule` | 금지 항목 | 명백한 차단 조건 |
| --- | --- | --- |
| `MINOR_SEXUAL_EXPLOITATION` | 미성년자 성착취 | 미성년자임이 명백한 대상의 성행위·성적 착취 묘사나 성적 이미지, 성착취 유도 |
| `EXPLICIT_SEXUAL_CONTENT` | 노골적인 성적 표현 | 성행위나 성기를 성적으로 노골적이고 구체적으로 묘사한 글·이미지 |
| `NONCONSENSUAL_SEXUAL_EXPLOITATION` | 비동의 성적 착취 | 성폭력의 성애화, 비동의 성적 촬영물·합성물의 제작·유포 유도 |
| `DRUGS` | 마약 | 마약임이 명백한 물질의 구체적인 제조·구매·판매·사용 방법, 거래 알선·사용 권장 |
| `EXTREME_GORE` | 극단적인 잔혹 표현 | 장기 노출·신체 절단·고문의 훼손된 신체나 고통을 적나라하고 구체적으로 묘사 |
| `SELF_HARM_PROMOTION` | 자살·자해 조장 | 실행의 직접적인 권유 또는 구체적인 실행 방법 제공 |
| `HATE_VIOLENCE_INCITEMENT` | 혐오 폭력 선동 | 인종·성별·종교 같은 정체성을 이유로 특정 집단에 대한 폭행·살해를 직접 선동 |

비노골적인 피해 고발·예방·교육·수사·회복 서술과 직접적인 권유·방법 제공을 구분합니다.
허구라는 이유만으로 명백한 금지 표현을 면제하지 않으며, 악역의 말만으로 작품 전체가
행위를 권장한다고 추정하지 않습니다. 나이·동의 여부·약물 종류를 만들어내지 않습니다.
욕설·공포·음주·흡연·비노골적인 성적 암시·일반적인 싸움이나 부상 자체는 차단 근거가 아닙니다.
구체적인 위해·범죄 실행 지원은 독립 검수 항목이 아니며 다른 코드로 바꾸어 적용하지 않습니다.
이는 오탐을 줄이는 판정 정책이며 정확도 100%를 보장한다는 뜻은 아닙니다.

판정은 `APPROVED`와 `REJECTED`만 사용합니다. 명백한 위반이 없고 검수가 완료되면 승인하며,
명백한 위반이 있거나 이미지 준비·모델 호출에 실패하면 거절합니다.
실행 실패는 내용 위반으로 기록하지 않고 전용 오류 코드로 구분합니다.

경로는 `POST /api/v1/moderation/story`입니다. 검수 결과는 승인·거절·실행 실패 모두 200으로
반환하며 백엔드는 본문의 `decision`과 `error_code`로 구분합니다. 요청 스키마 위반은 422,
예상하지 못한 서버 예외만 500입니다.

처리 순서는 이미지 준비 → OpenAI 호출 1회 → 실패 시 DeepSeek 호출 1회입니다.

- **이미지 준비.** 서버가 `thumbnailUrl`·`imageUrl`의 실제 파일 내용으로 형식과 크기를 확인합니다.
  내려받기 실패, 이미지가 아닌 파일, 미지원 형식(JPEG·PNG·GIF·WebP 외), 장당 32MiB 초과를
  이미지별 오류로 모읍니다. 한 장이 실패해도 나머지 검사를 계속하며 동시 다운로드는 최대 5장입니다.
  전체 준비 시간 30초가 지나면 미완료 이미지 각각을 `IMAGE_DOWNLOAD_FAILED`로 기록합니다.
  실패한 이미지의 URL은 모델 입력에서 제외하고 정상 이미지와 텍스트의 검수는 계속합니다.
  원래 배열 인덱스는 유지합니다. 이미지가 있었지만 모두 준비에 실패하면 모델을 호출하지 않습니다.
  이미지가 없는 요청은 텍스트를 검수합니다.
- **합계 용량.** 모델에 보낼 base64 이미지·텍스트·JSON을 합친 요청 본문은 48MiB 이하여야 합니다.
  백엔드는 검수 요청 전에 게시물 전체의 전송 예상 용량을 확인합니다. 초과하면 검수를 요청하지 않고
  사용자에게 이미지 크기나 장수를 줄이도록 안내합니다. 개별 파일이 모두 불량이라는 뜻이 아니므로
  이미지 전부를 교체하라고 안내하거나 같은 요청을 자동 재시도하지 않습니다.
  원본 파일 합계에 48MiB를 적용하지 않습니다. base64 변환에 따른 크기 증가와 텍스트·JSON·AI 프롬프트를
  포함한 여유를 고려해야 합니다. 백엔드의 구체적인 사전 상한·계산 방식은 백엔드 명세에서 정합니다.
  AI 서버는 마지막 방어로 실제 전송 본문의 48MiB 검사를 유지하며, 초과하면 모델을 호출하지 않습니다. 준비된 이미지가 있으면 해당 이미지들에 `IMAGE_INVALID`를
  기록하고, 이미지가 없는 요청의 본문 초과는 `MODEL_CALL_FAILED`로 반환합니다.
- **모델 호출.** 시간 초과·공급자 오류·응답 거부·JSON 형식 불일치는 모두 호출 실패로 세고 다음 모델로
  넘어갑니다. 정상적으로 반환한 내용 거절은 대체 호출 사유가 아닙니다. 대체 호출이 성공하면 그 결과를
  반환합니다. 두 호출이 모두 실패하면 `MODEL_CALL_FAILED`입니다. 이미 수집한 이미지 오류가
  있으면 그 목록을 보존하고 최상위 오류 코드는 [출력 우선순위](#5-9-6-게시물-검수)를 따릅니다.
- **응답 검증.** 모델이 돌려준 문제 항목 중 입력에 없는 경로는 버립니다. 버린 뒤 거절 근거가 남지 않으면
  승인하지 않고 `MODEL_CALL_FAILED`로 거절합니다. 모델이 판독 불가라고 알린 이미지는 위반이 아니라
  `IMAGE_UNREADABLE` 항목을 추가합니다. 준비 단계와 모델 단계의 오류를 합친 `image_errors`가
  하나라도 있으면 모델이 나머지 내용을 승인했더라도 최종 결과는 `REJECTED`입니다.

시간 제한은 이미지 준비 전체 30초, 모델 호출 1회 60초, 요청 전체 150초입니다. 값은 환경 변수로 조정합니다.
선택 이유는 [ADR](../adr/3-ai-server-adr.md#게시물의-명백한-금지-내용-검수와-실패-시-게시-차단)을 따릅니다.
...
| 게시물 검수 이미지 준비·모델 호출 | `REJECTED`, 확인된 위반은 `issues`, 실행 오류는 `error_code`·`image_errors`. HTTP는 200. [응답 계약](#5-9-6-게시물-검수) 참조 |
...
게시물 검수를 제외한 기존 생성 API의 성공 응답 `meta`는 `model`, `provider`, `prompt_versions`, `input_token_count`, `output_token_count`, `retry_count`를 담습니다(SSE completed는 camelCase). 본문·판정 토큰은 합산하고 선택지는 별도 집계합니다. 스토리라인은 응답을 받은 invalid 시도의 usage도 포함합니다. 예외로 usage를 못 받은 호출은 누락되므로 토큰 합계가 실제 청구액과 같다고 보장하지 않습니다. 런타임 금액 계산은 미구현입니다.
...
게시물 검수의 승인된 관측 계약은 Langfuse에서 최종 `decision`·`issues`·`error_code`·`image_errors`와
모델 호출 기록을 확인하는 것입니다. 문제 항목의 `type`으로 텍스트·이미지를 구분하고,
기본 호출과 대체 호출의 결과·모델·공급자·사용량·지연을 같은 요청에 연결합니다.
요청의 `storyId`는 스토리 `public_id(UUID)`이며 Langfuse 메타데이터의 `story_id`에 기록합니다.
채팅과 같은 키를 써서 백엔드가 한 스토리의 채팅·검수 기록을 함께 찾을 수 있게 합니다.
API 응답에 관측용 `meta`를 추가하지 않습니다. 검수 관측 연결은 미구현입니다.
...
게시물 검수의 dev 반영·Langfuse 연결 및 실제 모델 실측은 미완료입니다.
경로·HTTP 상태·호출 횟수·시간 제한·이미지 실패와 모델 실패의 순서는 [§5-3-6](#5-3-6-게시물-검수)에서
확정했습니다. 작성 필수 여부·글자 수·항목 개수는 백엔드가 검사합니다. AI 서버는 타입, 이미지 장당
32MiB, 지원 형식, 허용 호스트와 전송 본문 합계 48MiB를 확인합니다. 합계 초과의 사전 차단과 사용자
안내는 백엔드 책임입니다. 이 요구사항은 백엔드 구현 완료를 뜻하지 않습니다. 백엔드의 호출 대기 시간은
요청 전체 상한 150초보다 길어야 하며 그 값은 백엔드 명세가 정합니다.
백엔드는 게시 요청에서 기다리지 않고 검수 대기 상태에서 비동기로 호출할 계획입니다.
일시 실패는 1~2회 재시도하고 계속 실패하면 검수 대기로 남겨 운영자가 처리하는 방향입니다.
이 백엔드 처리 계획은 [PR #271의 확인](https://github.com/KIM-N-KANG/knk-harness/pull/271#pullrequestreview-5307272498)에
따르며, 세부 상태·재시도 정책은 백엔드 명세에서 관리합니다.
...
| `image_slots` | `object[]` | 최대 한 개; 생략·빈 배열이면 자식 생성 없음. null은 거부 |
| `image_slots[].key` | `string` | 필수 저장 키; 빈 문자열·공백만 있는 값 거부 |
| `image_slots[].upload_url` | `string` | 필수 HTTPS presigned PUT 주소 |
| `image_slots[].public_url` | `string` | 필수 HTTPS 공개 주소; 업로드 성공 시 응답에 사용 |
| `generate_child_image` | `boolean` | 기본 false인 호환 필드; 생성 여부는 image_slots로만 결정 |
...
| `error` | 본문 생성 실패 또는 슬롯 요청의 본문 순차 전송 중 시간 초과. `completed` 없이 종료 |

**Response body · 200 text/event-stream**

응답은 단일 JSON이 아니라 아래 SSE 프레임을 순차 전송합니다. `token`은 본문 조각마다,
`character_image`는 인물별 첫 대사 앞에 전송합니다. 본문 생성이 실패하면 이미지·판정을
시작하지 않고 `error`로 종료합니다. 슬롯 요청은 본문 생성과 이미지 처리가 끝난 뒤에도
본문 순차 전송 중 시간 초과로 `error`가 발생할 수 있습니다. 이때는 일부 텍스트·이미지가
이미 전달됐거나 판정이 이미 시작됐을 수 있으며, 진행 중인 판정을 정리하고 `completed` 없이 종료합니다.
...
| `character_image.imageName` | `string` | 업로드 성공 시 `{인물이름}_실시간_{UUID}`, 그 외 요청의 이미지 이름 |
| `character_image.imageUrl` | `string` | 업로드 성공 시 슬롯의 public_url, 실패 시 부모 URL |
| `error.code` | `string` | 오류 코드(LLM_ERROR) |
| `error.message` | `string` | 오류 설명 |

자식 이미지 업로드 성공 이벤트는 다음 형태입니다. 생성·업로드 실패 시 같은 세 필드에
부모 정보를 넣습니다. `generatedImage`·base64·실패 코드는 응답에 포함하지 않습니다.

```text
event: character_image
data: {"name":"도현","imageName":"도현_실시간_00000000-0000-4000-8000-000000000001","imageUrl":"https://example.com/images/dohyeon-child.webp"}
```

다음 완료 예시는 부모 이미지를 사용한 경우입니다. 자식 업로드 성공 시에는 본문 마커와
목록에 위 이벤트와 같은 자식 이름·URL이 들어가며, 백엔드의 주소 교체는 필요하지 않습니다.
...
| `endings[].achievement_condition` | `string` | 엔딩 달성 조건 |
| `endings[].epilogue` | `string` | 에필로그 연출 방향 |
| `image_slots` | `object[]` | 턴 요청과 같은 슬롯 형식·검증 규칙; 선택지 API에서는 생성·업로드에 사용하지 않음 |
| `generate_child_image` | `boolean` | 기본 false인 호환 필드; 선택지 API에서는 사용하지 않음 |
| `character_images` | `object[]` | 인물별 저장 이미지; 기본 빈 배열 |
| `character_images[].name` | `string` | 인물 이름 |
| `character_images[].image_name` | `string / null` | 이미지 이름; 생략·null 시 빈 문자열 |
| `character_images[].image_url` | `string` | 저장된 이미지 URL |
...
요청 검증 실패 시 422 응답은 채팅 턴과 동일하게 `detail` 항목에 `type`·`loc`·`msg`만
포함하며 입력값·검증 문맥은 반환하지 않습니다.
...
[동작과 검수 기준](#5-3-6-게시물-검수). 승인된 입력·응답 계약이며 dev 반영 전입니다.
...
다시 검사하지 않습니다. AI 서버는 필드 타입만 확인해 어긋나면 422를 반환하고, 빈 값·길이 초과·
개수 초과는 그대로 검수합니다. `storyId`만 필수입니다. 이미지 용량·지원 형식은 AI 서버가 내려받은
실제 파일로 검사합니다([§5-9-6 출력](#5-9-6-게시물-검수)의 `IMAGE_INVALID`).
`thumbnailUrl`과 인물 `imageUrl`은 검수 대상 원본의 서빙 URL(노출 폴백 적용 전)입니다.
백엔드가 목록·상세 화면에서 보여 주는 대체 이미지 URL을 보내지 않습니다. AI 서버는 허용된 호스트에서만
내려받아 실제 이미지를 검사하며, URL 문자열만 보고 판정하지 않습니다. 빈 문자열·공백뿐인 URL과
`null`·생략된 URL은 이미지 없음으로 처리하고 다운로드·이미지 오류 목록에서 제외합니다.
최상위 `storyId`는 백엔드의 스토리 `public_id(UUID)`입니다. 검수 판단에는 쓰지 않고 관측 기록 연결에만 씁니다.
...
```json
{
  ...
  "storySettings": {
    "worldSetting": "세계관 설정",
    "characterSetting": "인물 설정",
    "userRoleSetting": "사용자 역할",
    "ruleSetting": "진행 규칙"
  },
  "startSettings": [
    {
...
최상위 필드를 `decision`, `issues`, `error_code`, `image_errors` 순서로 직렬화합니다.
`issues` 내부에는 `code`나 `error_code`를 넣지 않으며 별도의 `meta`도 추가하지 않습니다.
HTTP 상태는 승인·거절·실행 실패 모두 200입니다.
...
| `error_code` | `string` 또는 `null` | 정상 검수는 승인·거절 모두 `null`. 실행 실패만 오류 코드를 넣습니다. |
| `image_errors` | `array<object>` | 확인한 이미지 실행 오류 전체. 오류가 없으면 `[]`입니다. |
| `image_errors[].path` | `string` | 해당 이미지의 원래 요청 경로. 요청의 배열 인덱스를 유지합니다. |
| `image_errors[].error_code` | `string` | `IMAGE_DOWNLOAD_FAILED`, `IMAGE_INVALID`, `IMAGE_UNREADABLE` 중 하나입니다. |

`thumbnailUrl`과 `characters[n].images[m].imageUrl`은 `IMAGE`입니다.
이미지 이름 `imageName`을 포함한 텍스트 필드는 `TEXT`입니다.
텍스트·이미지 결합 위반은 관련 필드별 문제 항목으로 표시합니다.

| 상황 | `decision` | `issues` | `error_code` | `image_errors` | 백엔드가 할 일 |
| --- | --- | --- | --- | --- | --- |
| 정상 검수·위반 없음 | `APPROVED` | `[]` | `null` | `[]` | 게시 |
| 정상 검수·명백한 위반 | `REJECTED` | 위반 항목 1개 이상 | `null` | `[]` | `issues`의 위치·기준·이유를 사용자에게 안내 |
| 이미지 내려받기 실패(접근 불가·시간 초과) | `REJECTED` | 확인된 내용 위반 목록 | `IMAGE_DOWNLOAD_FAILED` | 이미지별 오류 목록 | 인프라 문제. 다시 시도 |
| 이미지 파일 불량·용량 초과 | `REJECTED` | 확인된 내용 위반 목록 | `IMAGE_INVALID` | 이미지별 오류 목록 | 사용자에게 이미지 교체·용량 축소 안내 |
| 모델이 이미지를 판독하지 못함(흐림·너무 작음) | `REJECTED` | 확인된 내용 위반 목록 | `IMAGE_UNREADABLE` | 이미지별 오류 목록 | 사용자에게 이미지 교체 안내 |
| 이미지 오류 없이 두 모델 모두 호출·응답 검증 실패 | `REJECTED` | `[]` | `MODEL_CALL_FAILED` | `[]` | 인프라 문제. 다시 시도 |

오류가 여러 장이면 `image_errors`에 확인된 모든 오류를 원래 입력 순서대로 담습니다.
같은 경로는 한 번만 넣으며, 다운로드·파일 검사 결과와 모델의 판독 불가 결과를 합칩니다.
모델에 보내지 않은 이미지에 대한 모델의 문제 보고는 채택하지 않습니다.
준비 실패와 모델 호출 실패가 함께 발생해도 준비 단계의 오류 목록을 보존합니다.
최상위 `error_code`는 `IMAGE_INVALID` → `IMAGE_UNREADABLE` → `IMAGE_DOWNLOAD_FAILED` 순으로
선택합니다. 이미지 오류가 없을 때만 `MODEL_CALL_FAILED`를 사용합니다. 이미지 오류가 있으면
나머지 내용의 모델 승인·거절과 무관하게 최종 `decision`은 `REJECTED`입니다.
이미지 오류가 있어도 정상 모델 응답에서 확인한 텍스트·이미지 내용 위반은 `issues`에 모두 보존합니다.
백엔드는 `issues`와 `image_errors`를 함께 읽어 내용 수정과 이미지 교체 안내를 한 번에 표시합니다.
검수를 실행하지 못했거나 신뢰할 수 있는 모델 응답을 받지 못했다면 내용 위반을 만들어내지 않습니다.
백엔드 사전 검사를 통과했어도 실제 본문이 48MiB를 넘으면 AI 서버는 준비된 이미지 전체를
`IMAGE_INVALID`로 기록하고 기존 오류도 보존합니다. 이는 합계 제한을 위한 방어 응답이며,
각 파일이 깨졌다는 뜻은 아닙니다. 정상적인 합계 초과 안내는 검수 요청 전 백엔드에서 처리합니다.
모델이 놓친 문제까지 목록에 포함됨을 보장하는 것은 아닙니다.
...
```json
{"decision": "APPROVED", "issues": [], "error_code": null, "image_errors": []}
```

본문 위반·파일 불량·모델 판독 불가가 함께 있는 경우(인물이 두 명 이상이고 각 인물에 이미지가 있는 요청):

```json
{
  "decision": "REJECTED",
  "issues": [
    {"path": "startSettings[0].prologue", "type": "TEXT", "rule": "DRUGS", "reason": "마약 사용을 직접 권장합니다."}
  ],
...
```json
{"decision": "REJECTED", "issues": [], "error_code": "MODEL_CALL_FAILED", "image_errors": []}
```

**수용 기준**

- 백엔드는 검수 요청 전에 전체 전송 예상 용량을 확인하고, 초과하면 이미지 크기·장수 축소를 안내합니다.
- AI 서버의 실제 본문 48MiB 검사는 백엔드 사전 검사와 별개로 유지합니다.
- 애매한 표현·단순 언급만으로 내용 위반을 만들지 않습니다.
- 정상 거절의 모든 문제 항목은 실제 입력 경로·종류·7개 기준 중 하나·사유를 갖습니다.
- 이미지 URL을 읽을 수 없으면 이미지 내용을 추정해서 승인하지 않습니다.
- OpenAI 호출 실패 뒤 DeepSeek가 성공하면 그 결과와 준비 단계 오류를 합칩니다. 이미지 오류가 없고
  내용 판정이 정상일 때만 `error_code: null`을 반환합니다.
- 실행 실패는 HTTP 200에 `REJECTED`·오류 코드로 반환하며 두 공급자의 실패를 승인으로 바꾸지 않습니다.
- 이미지 실행 오류가 있어도 정상 모델 응답에서 확인한 내용 위반은 `issues`에 모두 보존합니다.
- 내려받기 실패·파일 불량이 있어도 정상 이미지가 남으면 AI 검수를 계속하고 모든 이미지 오류를 합칩니다.
- 이미지 오류가 있으면 나머지 내용을 모델이 승인해도 최종 승인하지 않습니다.
- 시간 초과로 미완료된 이미지도 각각 오류 목록에 포함하며, 이미지 경로의 배열 인덱스를 바꾸지 않습니다.
- 모델의 응답 거부·JSON 형식 불일치·입력에 없는 경로만 남은 응답은 승인이 아니라 호출 실패로 처리합니다.
- 모델 지시를 바꾸려는 게시물 내용은 검사할 데이터로 취급합니다.
- 프롬프트의 실제 판정 품질은 텍스트와 첨부 이미지 실측으로 검증합니다. JSON 형식 검사는 품질 검증을 대신하지 않습니다.
