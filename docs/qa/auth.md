...
| AUTH-MIGRATE-07 | P2  | 게스트로 체험을 일부 사용한 뒤 로그인                                                  | 로그인 → 채팅방 진입          | 세션 상태가 바뀌면 체험 조회를 회원 값으로 다시 하므로 회원이 게스트 한도 시트로 오게이팅되지 않음. 잔여는 서버가 옮긴 사용량으로 이어짐(§3-1-5) | 수동   | §3-2-2, 구현(`use-trials`)                          |
