...
| 관련 E2E | `manyak-web/e2e/my/login-page.spec.ts`, `e2e/auth/consent.spec.ts`(필수 동의 게이트), `e2e/stories/story-login-gate.spec.ts`·`e2e/chats/chat-login-gate.spec.ts`(로그인 필요 시트), `e2e/my/session-expiry.spec.ts`, `e2e/auth/in-app-login.spec.ts`(팝업 로그인), `e2e/auth/in-app-handoff.spec.ts`(기존 핸드오프 복구), `e2e/my/invite.spec.ts`(신규 가입 다이얼로그), `e2e/my/my-page.spec.ts`, `e2e/legal/legal.spec.ts`(로그인 화면 고지 없음), `manyak-web/e2e/visual/auth-visual.spec.ts` |
...
| AUTH-LOGIN-12 | P1  | 게스트                                                                    | 소셜 로그인 버튼 하나 탭 직후 관찰(로그인 화면·외부 핸드오프 랜딩·로그인 필요 시트 공통) | 탭한 버튼은 문구 공간을 유지한 중앙 스피너로 바뀌고 두 provider 버튼 모두 비활성화(중복·교차 클릭 방지). 페이지 이탈 없이 실패하면(팝업 차단 등) 실패 토스트와 함께 버튼이 다시 활성화. OAuth 화면에서 뒤로가기로 복귀(bfcache 포함)하면 버튼이 활성 상태로 돌아옴 | 수동                                          | FE-SCREEN-008 로그인 진행 중 상태, KNK-760               |
...
| AUTH-CONSENT-03 | P0 | 회원 세션 + 세 항목 모두 `needsConsent: true` + 이 탭 로그인 표시 있음(OAuth 복귀·같은 탭 새로고침) | 아무 화면 진입 | 제목 "서비스 이용을 위해 동의가 필요해요"(보조 설명 없음)·"전체 동의"·필수 3항목(만 14세 → 이용약관 → 처리방침 순)·같은 목록 마지막에 "[선택] 광고성 알림 수신 동의"와 보조 설명 "이벤트·혜택 소식과 출석 리마인드를 알림으로 받아요"(전체 동의에 포함, 제출 조건에는 미포함)·"동의하기"(비활성)의 모달 시트. 제목↔동의 영역·동의 영역↔버튼 간격 32px, 항목 줄 최소 48px·줄 간격 8px·체크박스 20px과 라벨 간격 12px. 동의 없이 나가는 버튼·스와이프 핸들 없음. 초기 초점은 체크박스가 아니라 시트 자체. Escape·배경 탭·핸들 스와이프로 닫히거나 내려가지 않음. 뒤로가기는 AUTH-CONSENT-08. 항목을 모두 체크해야 제출 버튼 활성(전체 동의로 일괄 체크) | ✅ e2e `auth/consent` · visual `auth-visual` | 동의 모델 시트 구성 |
...
| AUTH-CONSENT-10 | P0 | AUTH-CONSENT-03 상태 | 전체 동의 → 선택 항목만 다시 해제 → 제출 | 전체 동의는 선택 항목까지 켜고, 선택만 끄면 전체 동의가 풀리되 제출은 가능. 필수 동의만 `POST`, `PUT /users/me/push-settings` 없음, 처리 결과 다이얼로그 없음. 회원 기록은 첫 거절(세 번째 재진입에서 재질문) | ✅ e2e `auth/consent` | [웹 PWA 푸시](../spec/3-2-web-spec.md#pwa-푸시), KNK-1401 |
| AUTH-CONSENT-11 | P0 | AUTH-CONSENT-03 상태 | 선택 항목 체크 + 전체 동의 → 제출 | 필수 동의 저장 뒤 `GET` 설정 → 광고만 켠 전체 교체 `PUT` → 처리 결과 다이얼로그(전송자·일시·"광고 알림 수신 동의 완료"). 저장 실패는 "알림 설정을 저장하지 못했어요" 토스트만 | ✅ e2e `auth/consent` | 웹 PWA 푸시, §4-3-5 푸시 수신 동의 |
| AUTH-CONSENT-12 | P1 | 푸시 활성 빌드, 권한 미결정, 기기 플래그 없음 | 제출 탭 | 동의 기록 전에 브라우저 알림 권한 팝업이 먼저 뜨고, 응답과 무관하게 동의가 이어진다. 기기 플래그가 남아 다시 묻지 않음 | 수동(실기기) | 웹 PWA 푸시 |
| AUTH-CONSENT-13 | P1 | 푸시 활성 빌드, 권한 허용, 선택 항목을 한 번 거절한 회원, 광고 미동의 | 회원 상태로 페이지를 세 번째 새로 열기(초대 코드 안내 없음) | "이벤트·혜택 소식을 알림으로 받아 볼까요?" 시트. "광고 알림 받기"는 광고 PUT+통지, "닫기"·배경 탭·뒤로가기는 두 번째 거절로 닫힘. 이후 다시 묻지 않음 | 수동(실기기) · ✅ unit `marketing-consent-storage` | 웹 PWA 푸시 |
...
| AUTH-CONSENT-13 | P2 | 실기기: Instagram, Threads, KakaoTalk 인앱에서 로그인 필요 시트 → Google 팝업 또는 같은 탭의 Kakao 로그인 완료 | 복귀 관찰 | 검증된 원래 URL(쿼리와 해시 포함)로 돌아오고 그 위에서 동의 시트 표시. 동의 뒤 원래 화면 유지, 요청 자동 재실행 없음 | 수동(실기기) | [인앱 핸드오프](../spec/3-2-web-spec.md#인앱-브라우저와-로그인-핸드오프) |
...
| ID             | P   | 사전조건                                 | 절차                               | 기대 결과                                                                                                   | 자동화 | 근거                                                                  |
| --------------- | --- | ------------------------------------------------------------------------------------ | --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------- | ------------------------------------------------ |
| AUTH-HANDOFF-01 | P0  | 인앱 브라우저(인스타 등) UA + 게스트                                                 | 홈·제작 스토리 목록 진입                      | 전면 차단 오버레이 없이 게스트 목록을 열람하고 동의 후 생성과 채팅 이용. 한도 소진 시 로그인 필요 시트 표시. 감지 계측(`detected`)만 발생                                                                        | ✅ e2e `auth/in-app-handoff`  | §3-2-5 인앱 게스트 허용                           |
...
| AUTH-HANDOFF-13 | P0  | 카카오톡 인앱 UA + 게스트(스토리·채팅 로컬 ID 보유)                                  | "카카오로 시작하기" 탭 → Kakao 로그인 완료    | 핸드오프·외부 전환 없이 인앱 그 자리에서 로그인 완료. 같은 저장소의 게스트 데이터는 회원 필수 동의 완료 후 자동 마이그레이션으로 이관. 같은 화면의 Google 버튼은 Auth.js 팝업 시도(AUTH-POPUP-01) | 수동(실기기)                  | §3-2-5 분기 표, KNK-728                           |
...
| AUTH-POPUP-02 | P0 | 감지 대상 인앱 실기기, 신규 가입 또는 로컬 게스트 데이터 보유 | 로그인 화면 또는 기존 로그인 필요 시트 → Google 팝업 인증 완료 | 원래 탭에서 회원 필수 동의 시트 표시. 새로고침 후에도 동의 절차 유지. 동의 기록 성공 전 이관 없음, 완료 후 한 번 이관. 복귀 경로 유지. 시도한 소모 요청을 자동 재실행하지 않음 | ◐ e2e `auth/in-app-login`(팝업 인증 목킹과 동의 후 이관), 실기기 별도 | 웹 §3-2-5 |
...
| AUTH-LOGOUT-07 | P1 | 회원, 서버에서 완성이 끝난 제작 진행 카드 보유 | 제작 탭 새로고침(세션 판정 전 폴링 응답 도착) → 로그아웃 → 제작 탭 진입 | 게스트 서재가 비어 있음. 세션 판정 전 완성 반영이 회원 스토리 ID를 로컬 목록에 남기지 않음 | ◐ 단위 `creation-side-effects.test.ts`(loading 분기), 실기 확인은 수동 | [게스트 서재 상태 판정](../design/1-1-web-design.md#게스트-서재-상태-판정-웹), KNK-1357 |
