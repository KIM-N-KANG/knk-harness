...
| AUTH-CONSENT-03 | P0 | 회원 세션 + 세 항목 모두 `needsConsent: true` + 이 탭 로그인 표시 있음(OAuth 복귀·같은 탭 새로고침) | 아무 화면 진입 | 제목 "서비스 이용을 위해 동의가 필요해요"(보조 설명 없음)·"전체 동의"·필수 3항목·"동의하기"(비활성)의 모달 시트. 제목↔동의 영역·동의 영역↔버튼 간격 32px. 동의 없이 나가는 버튼·스와이프 핸들 없음. 초기 초점은 체크박스가 아니라 시트 자체. Escape·배경 탭·핸들 스와이프로 닫히거나 내려가지 않음. 항목을 모두 체크해야 제출 버튼 활성(전체 동의로 일괄 체크) | ✅ e2e `auth/consent` · visual `auth-visual` | 동의 모델 시트 구성 |
...
| AUTH-CONSENT-08 | P1 | AUTH-CONSENT-03 상태 | 동의하지 않고 탭을 닫음 → 새 탭으로 재진입 | 시트에 나가기 버튼이 없으므로 탭을 닫는 것이 거부 경로다. 재진입은 AUTH-CONSENT-05대로 로그아웃(2026-09-19 로그아웃 버튼 제거) | 수동 | 동의 모델 시트 구성, W-011 |
