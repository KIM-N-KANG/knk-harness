...
| AUTH-LOGIN-02 | P1  | 게스트                                                                    | 이관 1회 안내 확인                             | "계정마다 처음 로그인할 때 / 한 번만 이 기기의 스토리와 채팅을 그 계정에 저장해요"를 두 줄로 상시 노출                                                                 | ✅ e2e `legal/legal`·`visual/auth-visual`     | FE-SCREEN-008 이관 1회 안내, KNK-1037                    |
...
| AUTH-LOGIN-07 | P1  | 게스트 한도 다이얼로그 표시 상태                                          | "카카오로 시작하기"(위)·"Google로 시작하기"(아래) 중 하나 탭 → 로그인 완료           | `/login`을 거치지 않고 즉시 해당 provider OAuth 진입. 완료 후 홈이 아니라 다이얼로그가 떴던 원래 화면(현재 경로)으로 복귀. 버튼 아래에 약관 동의 고지(링크 `/terms`·`/privacy`) 표시, 닫기(X) 버튼 없이 바깥 터치·Escape로 닫힘(단, 로그인 진행 중에는 닫히지 않음 — AUTH-LOGIN-12) | 수동                                          | 구현(`login-callback-url`, `login-required-dialog`)      |
