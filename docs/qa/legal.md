...
두 페이지는 별도 API·상태 없이 정적 렌더되는 콘텐츠 화면이며 라우트 가드가 없습니다. 진입점은 `/login` 동의 고지의 밑줄 링크와 서비스 안내(`/about`, `미배포`)의 약관 및 정책 링크입니다. 두 진입점 모두 문서를 새 브라우저 탭에서 열어 원래 화면을 유지합니다. 문서 헤더에는 뒤로가기 대신 홈(`/`)으로 가는 마냑 로고 링크가 있습니다.
...
| LEGAL-ENTRY-08 | P1  | 서비스 안내(`/about`) 표시됨 `미배포`      | "약관 및 정책"의 "서비스 이용약관"·"개인정보 처리방침" 링크 각각 탭 | 각 링크 뒤에 새 창 아이콘이 보이고, 새 브라우저 탭에서 `/terms`·`/privacy`를 각각 열며 원래 `/about` 탭 유지 — 로그인 후에도 접근 가능한 상시 진입점 | ✅ e2e `my/service-info`        | FE-SCREEN-011                        |
