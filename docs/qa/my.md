...
| MY-CREDITS-11  | P1  | 다음 페이지 조회가 실패                    | 목록 끝까지 스크롤         | 이미 그린 목록을 지우지 않고 목록 아래 재시도 버튼만 표시하며 자동 재요청을 멈춤. 탭하면 다음 페이지를 이어 붙임                                  | ✅ e2e `my/credits`(실제 마지막 내역 로딩 후 스크롤)           | FE-SCREEN-008 화면 상태, §3-2-2 커서 목록 패칭, KNK-1207 |
