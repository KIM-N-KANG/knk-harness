...
| MY-BACKGROUND-03 | P2 | iOS Safari와 홈 화면 설치 PWA 실기기 | 첫 진입, 화면 이동, 테마 변경 후 상하 안전 영역 확인 | 문서 배경이 앱과 이어짐. 브라우저 자체 툴바 효과는 제외 | 수동 | [웹 반응형 계약](../spec/3-2-web-spec.md#반응형-모바일-전용) |
| MY-BACKGROUND-04 | P2 | 루트 오류 화면, 모바일과 데스크톱 폭 | 라이트 및 다크 테마에서 오류 화면 확인 | 일반 화면과 동일한 문서 배경 및 좌우 여백 규칙 적용 | 수동 | [웹 반응형 계약](../spec/3-2-web-spec.md#반응형-모바일-전용) |
